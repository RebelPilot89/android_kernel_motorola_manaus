	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			...
			if (g_print_dbg)
				aw_dev_info(
					aw882xx->dev,
					"reg_addr: 0x%04x, reg_data :0x%04x",
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	aw882xx_dev_control =
		devm_kzalloc(aw882xx->codec->dev,
			     sizeof(struct snd_kcontrol_new) * 5, GFP_KERNEL);
#else
	aw882xx_dev_control =
		devm_kzalloc(aw882xx->codec->dev,
			     sizeof(struct snd_kcontrol_new) * 3, GFP_KERNEL);
#endif
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_ramp",
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_attenuate",
		 aw882xx->aw_pa->channel);
					   ...
					   struct snd_ctl_elem_value *ucontrol)
{
	...
	aw882xx_dev_set_fade_time_bottom(ucontrol->value.integer.value[0],
					 true);
...
static struct snd_kcontrol_new aw882xx_controls[] = {
	...
	SOC_SINGLE_EXT("aw882xx_fadein_us_bottom", 0, 0, 1000000, 0,
		       aw882xx_get_fade_in_time,
		       aw882xx_set_fade_in_time_bottom),
#endif
	SOC_SINGLE_EXT("aw882xx_fadeout_us", 0, 0, 1000000, 0,
		       aw882xx_get_fade_out_time, aw882xx_set_fade_out_time),
#ifdef CONFIG_AW_MTK_RCV_SUPPORT
	SOC_SINGLE_EXT("aw882xx_rcv_en", 0, 0, 1000000, 0, aw882xx_get_rcv_en,
		       aw882xx_set_rcv_en),
#endif
...
static int aw882xx_name_append_suffix(struct aw882xx *aw882xx,
				      const char **name)
{
	char buf[50];
...
static void aw882xx_add_widgets(struct aw882xx *aw882xx)
{
	...
	for (i = 0; i < ARRAY_SIZE(aw882xx_dapm_widgets); i++) {
		if (aw_widgets[i].name) {
			ret = aw882xx_name_append_suffix(aw882xx,
							 &aw_widgets[i].name);
			if (ret) {
				aw_dev_err(aw882xx->dev,
					   "append widget name suffix failed!");
				return;
			}
		}
		...
		if (aw_widgets[i].sname) {
			...
			if (ret) {
				aw_dev_err(
					aw882xx->dev,
					"append widget sname suffix failed!");
				return;
			}
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(aw882xx_audio_map); i++) {
		if (aw_route[i].sink) {
			...
			if (ret < 0) {
				aw_dev_err(aw882xx->dev,
					   "append sink name suffix failed!");
				return;
			}
		}
		...
		if (aw_route[i].source) {
			...
			if (ret < 0) {
				aw_dev_err(aw882xx->dev,
					   "append source name suffix failed!");
				return;
			}
		}
	}
	...
	if ((dai_drv != NULL) && (num_dai > 0))
		for (i = 0; i < num_dai; i++) {
			snprintf(buf, 50, "%s-%x-%x", dai_drv[i].name, i2cbus,
				 addr);
			dai_drv[i].name =
				aw882xx_devm_kstrdup(aw882xx->dev, buf);
