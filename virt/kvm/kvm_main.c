...
struct kvm_stats_debugfs_item debugfs_entries[] __weak = { {} };
...
static bool __kvm_make_vcpus_request_mask(struct kvm *kvm, unsigned int req,
				 ...
				 unsigned long *vcpu_bitmap)
{
	...
	called = __kvm_make_vcpus_request_mask(kvm, req, NULL, vcpu_bitmap,
					       cpus);
				      ...
				      struct kvm_vcpu *except)
{
	...
	called = __kvm_make_vcpus_request_mask(kvm, req, except, NULL, cpus);
...
static int kvm_create_vm_debugfs(struct kvm *kvm, int fd)
{
	...
	kvm->debugfs_stat_data =
		kcalloc(kvm_debugfs_num_entries,
			sizeof(*kvm->debugfs_stat_data), GFP_KERNEL_ACCOUNT);
	if (!kvm->debugfs_stat_data)
		return -ENOMEM;
...
{
	__kvm_unmap_gfn(gfn_to_memslot(vcpu->kvm, map->gfn), map, cache, dirty,
			atomic);
	return 0;
}
...
static inline void update_halt_poll_stats(struct kvm_vcpu *vcpu, u64 poll_ns,
					  bool waited)
{
	if (waited)
...
out:
	...
	update_halt_poll_stats(vcpu, ktime_to_ns(ktime_sub(poll_end, start)),
			       waited);
