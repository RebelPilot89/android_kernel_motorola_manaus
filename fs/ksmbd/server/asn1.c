				  ...
				  char *ntlm_blob, int ntlm_blob_len)
{
	...
	int ntlmssp_len = 4 + compute_asn_hdr_len_bytes(ntlm_blob_len) * 2 +
			  ntlm_blob_len;
	int total_len = 4 +
			compute_asn_hdr_len_bytes(neg_result_len + oid_len +
						  ntlmssp_len) *
				2 +
	...
	encode_asn_tag(buf, &ofs, 0xa1, 0x30,
		       neg_result_len + oid_len + ntlmssp_len);
				   ...
				   size_t vlen)
{
	...
	sprint_oid(value, vlen, buf, sizeof(buf));
	if (!strcmp(buf, "1.3.6.1.4.1.311.2.2.10")) {
		mech_type = KSMBD_AUTH_NTLMSSP;
	} else if (!strcmp(buf, "1.2.840.48018.1.2.2")) {
		mech_type = KSMBD_AUTH_MSKRB5;
	} else if (!strcmp(buf, "1.2.840.113554.1.2.2")) {
		mech_type = KSMBD_AUTH_KRB5;
	} else if (!strcmp(buf, "1.2.840.113554.1.2.2.3")) {
