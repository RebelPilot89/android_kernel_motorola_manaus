...
static void __ksmbd_inode_close(struct ksmbd_file *fp)
{
	...
	if (ksmbd_stream_fd(fp) && (ci->m_flags & S_DEL_ON_CLS_STREAM)) {
		...
		err = ksmbd_vfs_remove_xattr(&filp->f_path, fp->stream.name);
		if (err)
...
static struct ksmbd_file *__ksmbd_lookup_fd(struct ksmbd_file_table *ft, u64 id)
{
	struct ksmbd_file *fp;
...
static void set_close_state_blocked_works(struct ksmbd_file *fp)
{
	...
	spin_lock(&fp->f_lock);
	list_for_each_entry (cancel_work, &fp->blocked_works, fp_entry) {
...
struct ksmbd_file *ksmbd_lookup_fd_cguid(char *cguid)
{
	...
	idr_for_each_entry (global_ft.idr, fp, id) {
		if (!memcmp(fp->create_guid, cguid, SMB2_CREATE_GUID_SIZE)) {
			fp = ksmbd_fp_get(fp);
			break;
		}
	}
...
{
	int num = __close_file_table_ids(&work->sess->file_table, work->tcon,
					 tree_conn_fd_check);
...
{
	filp_cache =
		kmem_cache_create("ksmbd_file_cache", sizeof(struct ksmbd_file),
