...
#include "../../internal.h" /* for vfs_path_lookup & filename_parentat */
					...
					struct path *path)
{
	...
	filename = filename_parentat(AT_FDCWD, filename, flags, parent_path,
				     &last, &type);
	if (IS_ERR(filename))
		return PTR_ERR(filename);
	err = 0;
	...
	d = lookup_one_len(last.name, parent_path->dentry, last.len);
	if (IS_ERR(d))
		goto err_out;
...
void ksmbd_vfs_query_maximal_access(struct dentry *dentry, __le32 *daccess)
{
	*daccess = cpu_to_le32(FILE_READ_ATTRIBUTES | READ_CONTROL);
	...
	if (!inode_permission(d_inode(dentry->d_parent), MAY_EXEC | MAY_WRITE))
...
int ksmbd_vfs_create(struct ksmbd_work *work, const char *name, umode_t mode)
{
	...
	dentry = ksmbd_vfs_kern_path_create(work, name, LOOKUP_NO_SYMLINKS,
					    &path);
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
		if (err != -ENOENT)
...
int ksmbd_vfs_mkdir(struct ksmbd_work *work, const char *name, umode_t mode)
{
	...
	dentry = ksmbd_vfs_kern_path_create(
		work, name, LOOKUP_NO_SYMLINKS | LOOKUP_DIRECTORY, &path);
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
		if (err != -EEXIST)
				      ...
				      int attr_name_len, char **attr_value)
{
	...
	for (name = xattr_list; name - xattr_list < xattr_list_len;
		...
		value_len = ksmbd_vfs_getxattr(dentry, name, attr_value);
		if (value_len < 0)
				 ...
				 size_t count)
{
	...
	v_len = ksmbd_vfs_getcasexattr(&init_user_ns, fp->filp->f_path.dentry,
				  ...
				  size_t count)
{
	...
	v_len = ksmbd_vfs_getcasexattr(user_ns, fp->filp->f_path.dentry,
				       fp->stream.name, fp->stream.size,
				       &stream_buf);
	if (v_len < 0) {
	...
	err = ksmbd_vfs_setxattr(&fp->filp->f_path, fp->stream.name,
				 (void *)stream_buf, size, 0, true);
	if (err < 0)
		goto out;
 ...
 * Return:	0 on success, otherwise error
 */
int ksmbd_vfs_write(struct ksmbd_work *work, struct ksmbd_file *fp, char *buf,
		    size_t count, loff_t *pos, bool sync, ssize_t *written)
{
	struct file *filp;
	loff_t offset = *pos;
	int err = 0;
		   ...
		   const char *newname)
{
	...
	dentry = ksmbd_vfs_kern_path_create(
		work, newname, LOOKUP_NO_SYMLINKS | LOOKUP_REVAL, &newpath);
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
...
retry:
	to = filename_parentat(AT_FDCWD, to, lookup_flags, &new_path, &new_last,
			       &new_type);
	if (IS_ERR(to)) {
		err = PTR_ERR(to);
		goto out1;
	...
	new_dentry =
		lookup_one_len(new_last.name, new_path.dentry, new_last.len);
	...
	err = vfs_rename(d_inode(old_parent), old_child,
			 new_path.dentry->d_inode, new_dentry, NULL, flags);
	if (err)
 ...
 * Return:	0 on success, otherwise error
 */
int ksmbd_vfs_truncate(struct ksmbd_work *work, struct ksmbd_file *fp,
		       loff_t size)
{
	int err = 0;
	struct file *filp;
	...
	if (!work->tcon->posix_extensions) {
		...
		if (size < inode->i_size) {
			err = check_lock_range(filp, size, inode->i_size - 1,
					       WRITE);
		} else {
			err = check_lock_range(filp, inode->i_size, size - 1,
					       WRITE);
		}
		       ...
		       bool get_write)
{
	...
	err = vfs_setxattr(path->dentry, attr_name, attr_value, attr_size,
			   flags);
	if (err)
			...
			loff_t off, loff_t len)
{
	...
	return vfs_fallocate(
		fp->filp, FALLOC_FL_ZERO_RANGE | FALLOC_FL_KEEP_SIZE, off, len);
}
			     ...
			     unsigned int d_type)
{
	...
	if (IS_ENABLED(CONFIG_UNICODE) && buf->um) {
		const struct qstr q_buf = { .name = buf->private,
					    .len = buf->used };
		const struct qstr q_name = { .name = name, .len = namlen };
							    ...
							    int acl_type)
{
	...
	for (i = 0; i < posix_acls->a_count; i++, pa_entry++, xa_entry++) {
		...
		case ACL_USER:
			...
			xa_entry->uid = posix_acl_uid_translate(&init_user_ns,
								pa_entry);
			break;
		case ACL_USER_OBJ:
		...
		case ACL_GROUP:
			...
			xa_entry->gid = posix_acl_gid_translate(&init_user_ns,
								pa_entry);
			break;
		case ACL_GROUP_OBJ:
...
int ksmbd_vfs_set_sd_xattr(struct ksmbd_conn *conn, const struct path *path,
			   struct smb_ntsd *pntsd, int len, bool get_write)
{
	int rc;
	...
	if (S_ISDIR(inode->i_mode))
		def_smb_acl =
			ksmbd_vfs_make_xattr_posix_acl(inode, ACL_TYPE_DEFAULT);

	rc = ndr_encode_posix_acl(&acl_ndr, &init_user_ns, inode, smb_acl,
				  def_smb_acl);
	if (rc) {
	...
	rc = ksmbd_vfs_setxattr(path, XATTR_NAME_SD, sd_ndr.data, sd_ndr.offset,
				0, get_write);
	if (rc < 0)
...
int ksmbd_vfs_set_dos_attrib_xattr(const struct path *path,
				   struct xattr_dos_attrib *da, bool get_write)
{
	struct ndr n;
	int err;
	...
	err = ksmbd_vfs_setxattr(path, XATTR_NAME_DOS_ATTRIBUTE, (void *)n.data,
				 n.offset, 0, get_write);
	if (err)
				...
				size_t *xattr_stream_name_size, int s_type)
{
	...
	buf = kasprintf(GFP_KERNEL, "%s%s%s", XATTR_NAME_STREAM, stream_name,
			type);
	if (!buf)
		return -ENOMEM;
...
int ksmbd_vfs_copy_file_ranges(
	struct ksmbd_work *work, struct ksmbd_file *src_fp,
	struct ksmbd_file *dst_fp, struct srv_copychunk *chunks,
	unsigned int chunk_count, unsigned int *chunk_count_written,
	unsigned int *chunk_size_written, loff_t *total_size_written)
{
	...
	*total_size_written = 0;
	...
	for (i = 0; i < chunk_count; i++) {
		...
		ret = vfs_copy_file_range(src_fp->filp, src_off, dst_fp->filp,
					  dst_off, len, 0);
...
int ksmbd_vfs_set_init_posix_acl(struct path *path)
{
...
int ksmbd_vfs_inherit_posix_acl(struct ksmbd_work *work, struct path *path,
				struct inode *parent_inode)
{
	struct posix_acl *acls;
