...
bool ksmbd_smb_request(struct ksmbd_conn *conn)
{
	...
	if (*proto != SMB1_PROTO_NUMBER && *proto != SMB2_PROTO_NUMBER &&
...
{
	if (idx == SMB2X_PROT && (server_conf.min_protocol >= SMB21_PROT ||
						...
						struct ksmbd_kstat *))
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (!dir->dot_dotdot[i]) { /* fill dot entry info */
			...
			ksmbd_vfs_fill_dentry_attrs(work, dentry, &ksmbd_kstat);
			    ...
			    char *shortname)
{
	...
	if (dot_present)
		...
		out[baselen + 4] = '\0';
	smbConvertToUTF16((__le16 *)shortname, out, PATH_MAX, conn->local_nls,
			  0);
	len = strlen(out) * 2;
	return len;
}
...
int ksmbd_smb_check_shared_mode(struct file *filp, struct ksmbd_file *curr_fp)
{
	...
	list_for_each_entry (prev_fp, &curr_fp->f_ci->m_fp_list, node) {
		...
		if (!(prev_fp->saccess & FILE_SHARE_DELETE_LE) &&
			...
			smb_shared_mode_error(SHARE_DELETE_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
		...
		if (!(prev_fp->saccess & FILE_SHARE_READ_LE) &&
			...
			smb_shared_mode_error(SHARE_READ_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
		...
		if (!(prev_fp->saccess & FILE_SHARE_WRITE_LE) &&
			...
			smb_shared_mode_error(SHARE_WRITE_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
		...
		if (prev_fp->daccess & (FILE_EXECUTE_LE | FILE_READ_DATA_LE) &&
			...
			smb_shared_mode_error(FILE_READ_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
		...
		if (prev_fp->daccess &
			...
			smb_shared_mode_error(FILE_WRITE_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
		...
		if (prev_fp->daccess & FILE_DELETE_LE &&
			...
			smb_shared_mode_error(FILE_DELETE_ERROR, prev_fp,
					      curr_fp);
			rc = -EPERM;
			break;
		}
