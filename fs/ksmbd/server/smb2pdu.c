...
int smb2_get_ksmbd_tcon(struct ksmbd_work *work)
{
	...
	if (cmd == SMB2_TREE_CONNECT_HE || cmd == SMB2_CANCEL_HE ||
	    cmd == SMB2_LOGOFF_HE) {
...
int init_smb2_neg_rsp(struct ksmbd_work *work)
{
	...
	err = ksmbd_iov_pin_rsp(
		work, rsp, sizeof(struct smb2_negotiate_rsp) + AUTH_GSS_LENGTH);
	if (err)
		return err;
...
int smb2_set_rsp_credits(struct ksmbd_work *work)
{
	...
	credit_charge =
		max_t(unsigned short, le16_to_cpu(req_hdr->CreditCharge), 1);
	...
	credits_requested =
		max_t(unsigned short, le16_to_cpu(req_hdr->CreditRequest), 1);
...
static void init_chained_smb2_rsp(struct ksmbd_work *work)
{
	...
	if (req->Command == SMB2_CREATE && rsp->Status == STATUS_SUCCESS) {
		work->compound_fid =
			((struct smb2_create_rsp *)rsp)->VolatileFileId;
		work->compound_pfid =
			((struct smb2_create_rsp *)rsp)->PersistentFileId;
...
bool is_chained_smb2_message(struct ksmbd_work *work)
{
	...
	if (next_cmd > 0) {
		...
		if ((u64)get_rfc1002_len(work->response_buf) +
				     ...
				     struct ksmbd_user *user, u64 id)
{
	...
	if (!prev_user || strcmp(user->name, prev_user->name) ||
...
static unsigned int assemble_neg_contexts(struct ksmbd_conn *conn,
					  struct smb2_negotiate_rsp *rsp)
{
	char *const pneg_ctxt =
		(char *)rsp + le32_to_cpu(rsp->NegotiateContextOffset);
	int neg_ctxt_cnt = 1;
	int ctxt_size;
				...
				int ctxt_len)
{
	...
	if (sizeof(struct smb2_encryption_neg_context) + cphs_size > ctxt_len) {
...
static void
decode_compress_ctxt(struct ksmbd_conn *conn,
				 ...
				 int ctxt_len)
{
	...
	for (i = 0; i < sign_algo_cnt; i++) {
		if (pneg_ctxt->SigningAlgorithms[i] ==
				      ...
				      unsigned int len_of_smb)
{
	...
	while (i++ < neg_ctxt_cnt) {
		...
		if (pctx->ContextType == SMB2_PREAUTH_INTEGRITY_CAPABILITIES) {
			...
			status = decode_preauth_ctxt(
				conn, (struct smb2_preauth_neg_context *)pctx,
				ctxt_len);
			...
			decode_sign_cap_ctxt(
				conn, (struct smb2_signing_capabilities *)pctx,
				ctxt_len);
		}
...
int smb2_handle_negotiate(struct ksmbd_work *work)
{
	...
	switch (conn->dialect) {
	case SMB311_PROT_ID:
		conn->preauth_info = kzalloc(
			sizeof(struct preauth_integrity_info), GFP_KERNEL);
		...
		status = deassemble_neg_contexts(
			conn, req, get_rfc1002_len(work->request_buf));
		if (status != STATUS_SUCCESS) {
...
static int generate_preauth_hash(struct ksmbd_work *work)
{
	...
	if (conn->binding) {
		...
		if (!preauth_sess) {
			preauth_sess =
				ksmbd_preauth_session_alloc(conn, sess->id);
			if (!preauth_sess)
				return -ENOMEM;
		}
...
static int ntlm_negotiate(struct ksmbd_work *work,
			  struct negotiate_message *negblob, size_t negblob_len,
...
static struct authenticate_message *
user_authblob(struct ksmbd_conn *conn, struct smb2_sess_setup_req *req)
{
	int sz;
			     ...
			     struct smb2_sess_setup_rsp *rsp)
{
	...
	retval = ksmbd_krb5_authenticate(sess, in_blob, in_len, out_blob,
					 &out_len);
	if (retval) {
...
out_err:
	...
	if (rc < 0) {
		...
		if (sess) {
			 ...
			 */
			if (sess->user &&
			    sess->user->flags & KSMBD_USER_FLAG_DELAY_SESSION)
				try_delay = true;
...
int smb2_tree_connect(struct ksmbd_work *work)
{
	...
	treename = smb_strndup_from_utf16(
		(char *)req + le16_to_cpu(req->PathOffset),
	...
	if (test_share_config_flag(share, KSMBD_SHARE_FLAG_PIPE)) {
		...
		if (test_tree_conn_flag(status.tree_conn,
			...
			rsp->MaximalAccess |=
				...
				FILE_WRITE_EA_LE | FILE_DELETE_LE |
...
static int smb2_create_open_flags(bool file_present, __le32 access,
				  __le32 disposition, int *may_flags,
				  bool is_dir)
{
		       ...
		       const struct path *path, bool get_write)
{
	...
	do {
		...
		if (!eabuf->EaValueLength) {
			...
			rc = ksmbd_vfs_setxattr(
				path, attr_name, value,
				le16_to_cpu(eabuf->EaValueLength), 0, true);
			if (rc < 0) {
				ksmbd_debug(
					SMB,
					       ...
					       char *stream_name, int s_type)
{
	...
	rc = ksmbd_vfs_casexattr_len(path->dentry, xattr_stream_name,
				     xattr_stream_size);
	if (rc >= 0)
		return 0;
	...
	for (name = xattr_list; name - xattr_list < xattr_list_len;
		...
		if (!strncmp(name, XATTR_USER_PREFIX, XATTR_USER_PREFIX_LEN) &&
		    !strncmp(&name[XATTR_USER_PREFIX_LEN], STREAM_PREFIX,
			     STREAM_PREFIX_LEN)) {
			err = ksmbd_vfs_remove_xattr(path, name);
			if (err)
...
int smb2_open(struct ksmbd_work *work)
{
	...
	if (req->CreateOptions &&
	    !(req->CreateOptions & CREATE_OPTIONS_MASK_LE)) {
	...
	if (req->FileAttributes &&
	    !(req->FileAttributes & FILE_ATTRIBUTE_MASK_LE)) {
	...
	if (req->CreateContextsOffset) {
		...
		context = smb2_find_context_vals(
			req, SMB2_CREATE_QUERY_MAXIMAL_ACCESS_REQUEST, 4);
		if (IS_ERR(context)) {
			rc = PTR_ERR(context);
			goto err_out2;
		} else if (context) {
		...
		context = smb2_find_context_vals(
			req, SMB2_CREATE_TIMEWARP_REQUEST, 4);
		if (IS_ERR(context)) {
			rc = PTR_ERR(context);
			goto err_out2;
		} else if (context) {
	...
	if (!rc) {
		...
		if (req->CreateOptions & FILE_DELETE_ON_CLOSE_LE) {
			...
			if (!test_tree_conn_flag(
				    tcon, KSMBD_TREE_CONN_FLAG_WRITABLE)) {
				ksmbd_debug(
					SMB,
	...
	open_flags = smb2_create_open_flags(
		file_present, daccess, req->CreateDisposition, &may_flags,
	...
	if (!file_present) {
		...
		if (ea_buf) {
			...
			rc = smb2_set_ea(
				&ea_buf->ea,
				le32_to_cpu(ea_buf->ccontext.DataLength), &path,
				false);
			if (rc == -EOPNOTSUPP)
				rc = 0;
			else if (rc)
		 ...
		 */
		if (daccess &
		    ~(FILE_READ_ATTRIBUTES_LE | FILE_READ_CONTROL_LE)) {
	...
	if (created) {
		...
		posix_acl_rc = ksmbd_vfs_inherit_posix_acl(
			work, &path, d_inode(path.dentry->d_parent));
		if (posix_acl_rc)
		...
		if (rc) {
			...
			if (rc) {
				if (posix_acl_rc)
					ksmbd_vfs_set_init_posix_acl(&path);
				...
				if (test_share_config_flag(
					...
					rc = ksmbd_vfs_set_sd_xattr(conn, &path,
								    pntsd,
								    pntsd_size,
								    false);
					kfree(pntsd);
	...
	if (!test_share_config_flag(work->tcon->share_conf,
		...
		rc = smb_grant_oplock(work, req_op_level, fp->persistent_id, fp,
	...
	if (req->CreateContextsOffset) {
		...
		az_req = (struct create_alloc_size_req *)smb2_find_context_vals(
			req, SMB2_CREATE_ALLOCATION_SIZE, 4);
		if (IS_ERR(az_req)) {
			rc = PTR_ERR(az_req);
			goto err_out1;
		} else if (az_req) {
		...
		context = smb2_find_context_vals(
			req, SMB2_CREATE_QUERY_ON_DISK_ID, 4);
		if (IS_ERR(context)) {
			rc = PTR_ERR(context);
			goto err_out1;
		} else if (context) {
	...
	if (req->FileAttributes || fp->f_ci->m_fattr == 0)
		fp->f_ci->m_fattr = cpu_to_le32(smb2_get_dos_mode(
			&stat, le32_to_cpu(req->FileAttributes)));
	...
	rsp->AllocationSize =
		S_ISDIR(stat.mode) ? 0 : cpu_to_le64(stat.blocks << 9);
	...
	if (maximal_access_ctxt) {
		...
		contxt_cnt++;
		create_mxac_rsp_buf(
			rsp->Buffer + le32_to_cpu(rsp->CreateContextsLength),
				       ...
				       struct ksmbd_kstat *ksmbd_kstat)
{
	...
	case FILE_FULL_DIRECTORY_INFORMATION: {
		...
		ffdinfo->EaSize = smb2_get_reparse_tag_special_file(
			ksmbd_kstat->kstat->mode);
		if (ffdinfo->EaSize)
	...
	case FILE_BOTH_DIRECTORY_INFORMATION: {
		...
		fbdinfo->EaSize = smb2_get_reparse_tag_special_file(
			ksmbd_kstat->kstat->mode);
		if (fbdinfo->EaSize)
	...
	case FILEID_FULL_DIRECTORY_INFORMATION: {
		...
		dinfo->EaSize = smb2_get_reparse_tag_special_file(
			ksmbd_kstat->kstat->mode);
		if (dinfo->EaSize)
	...
	case FILEID_BOTH_DIRECTORY_INFORMATION: {
		...
		fibdinfo->EaSize = smb2_get_reparse_tag_special_file(
			ksmbd_kstat->kstat->mode);
		if (fibdinfo->EaSize)
	...
	case SMB_FIND_FILE_POSIX_INFO: {
		...
		id_to_sid(from_kuid_munged(&init_user_ns,
					   ksmbd_kstat->kstat->uid),
			  SIDUNIX_USER,
			  (struct smb_sid *)&posix_info->SidBuffer[0]);
		id_to_sid(from_kgid_munged(&init_user_ns,
					   ksmbd_kstat->kstat->gid),
			  SIDUNIX_GROUP,
			  (struct smb_sid *)&posix_info->SidBuffer[16]);
...
static int process_query_dir_entries(struct smb2_query_dir_private *priv)
{
	...
	for (i = 0; i < priv->d_info->num_entry; i++) {
		...
		if (priv->info_level != FILE_NAMES_INFORMATION)
			ksmbd_vfs_fill_dentry_attrs(priv->work, dent,
						    &ksmbd_kstat);

		rc = smb2_populate_readdir_entry(priv->work->conn,
						 priv->info_level, priv->d_info,
						 &ksmbd_kstat);
		dput(dent);
		if (rc)
			return rc;
				   ...
				   int info_level)
{
	...
	next_entry_offset =
		ALIGN(struct_sz + conv_len, KSMBD_DIR_INFO_ALIGNMENT);
...
int smb2_query_dir(struct ksmbd_work *work)
{
	...
	srch_ptr = smb_strndup_from_utf16(
		(char *)req + le16_to_cpu(req->FileNameOffset),
		le16_to_cpu(req->FileNameLength), 1, conn->local_nls);
	if (IS_ERR(srch_ptr)) {
	...
	d_info.out_buf_len = smb2_calc_max_out_buf_len(
		work, 8, le32_to_cpu(req->OutputBufferLength));
...
again:
	...
	if (!d_info.data_count && d_info.out_buf_len >= 0) {
		...
		rsp->Buffer[0] = 0;
		rc = ksmbd_iov_pin_rsp(
			work, (void *)rsp,
			offsetof(struct smb2_query_directory_rsp, Buffer) + 1);
		if (rc)
			goto err_out;
	} else {
	no_buf_len:
		((struct file_directory_info *)((char *)rsp->Buffer +
						d_info.last_entry_offset))
			->NextEntryOffset = 0;
		...
		rc = ksmbd_iov_pin_rsp(work, (void *)rsp,
				       offsetof(struct smb2_query_directory_rsp,
						Buffer) +
					       d_info.data_count);
		if (rc)
			goto err_out;
				   ...
				   void *rsp_org)
{
	...
	case FILE_STANDARD_INFORMATION:
		...
		rc = buffer_check_err(le32_to_cpu(req->OutputBufferLength), rsp,
				      rsp_org);
		break;
		       ...
		       struct smb2_query_info_rsp *rsp, void *rsp_org)
{
	...
	if (req->InputBufferLength) {
		...
		ea_req = (struct smb2_ea_info_req
				  *)((char *)req +
	...
	buf_free_len = smb2_calc_max_out_buf_len(
		work, 8, le32_to_cpu(req->OutputBufferLength));
	if (buf_free_len < 0)
		return -EINVAL;
	...
	while (idx < xattr_list_len) {
		...
		if (!strncmp(&name[XATTR_USER_PREFIX_LEN], DOS_ATTRIBUTE_PREFIX,
		...
		buf_free_len -=
			(offsetof(struct smb2_ea_info, name) + name_len + 1);
		...
		next_offset = offsetof(struct smb2_ea_info, name) + name_len +
			      1 + value_len;
				    ...
				    struct ksmbd_file *fp, void *rsp_org)
{
	...
	conv_len = ksmbd_extract_shortname(conn, dentry->d_name.name,
					   file_info->FileName);
				 ...
				 struct ksmbd_file *fp, void *rsp_org)
{
	...
	while (idx < xattr_list_len) {
		...
		if (strncmp(&stream_name[XATTR_USER_PREFIX_LEN], STREAM_PREFIX,
			    STREAM_PREFIX_LEN))
			continue;

		stream_name_len =
			streamlen - (XATTR_USER_PREFIX_LEN + STREAM_PREFIX_LEN);
		...
		streamlen = snprintf(stream_buf, streamlen + 1, ":%s",
		...
		file_info =
			(struct smb2_file_stream_info *)&rsp->Buffer[nbytes];
				 ...
				 struct ksmbd_file *fp, void *rsp_org)
{
	 ...
	 *		  sub_auth(4 * 1(num_subauth)) + RID(4).
	 */
	id_to_sid(from_kuid_munged(&init_user_ns, inode->i_uid), SIDUNIX_USER,
		  (struct smb_sid *)&file_info->Sids[0]);
	id_to_sid(from_kgid_munged(&init_user_ns, inode->i_gid), SIDUNIX_GROUP,
				    ...
				    struct smb2_query_info_rsp *rsp)
{
	...
	case FS_ATTRIBUTE_INFORMATION: {
		...
		info->Attributes = cpu_to_le32(
			FILE_SUPPORTS_OBJECT_IDS | FILE_PERSISTENT_ACLS |
			FILE_UNICODE_ON_DISK | FILE_CASE_PRESERVED_NAMES |
		...
		len = smbConvertToUTF16((__le16 *)info->FileSystemName, "NTFS",
					PATH_MAX, conn->local_nls, 0);
		len = len * 2;
	...
	case FS_VOLUME_INFORMATION: {
		...
		serial_crc =
			crc32_le(serial_crc, share->name, strlen(share->name));
		serial_crc =
			crc32_le(serial_crc, share->path, strlen(share->path));
	...
	case FS_OBJECT_ID_INFORMATION: {
		...
		memcpy(info->extended_info.version_string, "1.1.0",
		       strlen("1.1.0"));
	...
	case FS_SECTOR_SIZE_INFORMATION: {
		...
		unsigned int sector_size = min_t(
			unsigned int, path.mnt->mnt_sb->s_blocksize, 4096);
			     ...
			     struct smb2_query_info_rsp *rsp)
{
	...
	if (addition_info &
	    ~(OWNER_SECINFO | GROUP_SECINFO | DACL_SECINFO |
	      PROTECTED_DACL_SECINFO | UNPROTECTED_DACL_SECINFO)) {
	...
	if (test_share_config_flag(work->tcon->share_conf,
				   KSMBD_SHARE_FLAG_ACL_XATTR))
		ppntsd_size = ksmbd_vfs_get_sd_xattr(
			work->conn, fp->filp->f_path.dentry, &ppntsd);

	/* Check if sd buffer size exceeds response buffer size */
	if (smb2_resp_buf_len(work, 8) > ppntsd_size)
		rc = build_sec_desc(pntsd, ppntsd, ppntsd_size, addition_info,
				    &secdesclen, &fattr);
	posix_acl_release(fattr.cf_acls);
	posix_acl_release(fattr.cf_dacls);
	kfree(ppntsd);
...
int smb2_close(struct ksmbd_work *work)
{
	...
	if (req->Flags == SMB2_CLOSE_FLAG_POSTQUERY_ATTRIB) {
		...
		rsp->AllocationSize = S_ISDIR(inode->i_mode) ?
					      0 :
		       ...
		       struct nls_table *local_nls)
{
	...
	if (strchr(new_name, ':')) {
		...
		rc = ksmbd_vfs_setxattr(&fp->filp->f_path, xattr_stream_name,
					NULL, 0, 0, true);
		if (rc < 0) {
			       ...
			       struct ksmbd_share_config *share)
{
	...
	if (test_share_config_flag(share, KSMBD_SHARE_FLAG_STORE_DOS_ATTRS) &&
		...
		rc = ksmbd_vfs_set_dos_attrib_xattr(&filp->f_path, &da, true);
		if (rc)
			ksmbd_debug(SMB,
...
static int
set_file_allocation_info(struct ksmbd_work *work, struct ksmbd_file *fp,
...
static int
set_file_disposition_info(struct ksmbd_file *fp,
			      ...
			      struct ksmbd_share_config *share)
{
	...
	case FILE_BASIC_INFORMATION: {
		...
		return set_file_basic_info(
			fp, (struct smb2_file_basic_info *)buffer, share);
	}
	case FILE_ALLOCATION_INFORMATION: {
		if (buf_len < sizeof(struct smb2_file_alloc_info))
			return -EINVAL;

		return set_file_allocation_info(
			work, fp, (struct smb2_file_alloc_info *)buffer);
	}
	case FILE_END_OF_FILE_INFORMATION: {
		if (buf_len < sizeof(struct smb2_file_eof_info))
			return -EINVAL;

		return set_end_of_file_info(
			work, fp, (struct smb2_file_eof_info *)buffer);
	}
	...
	case FILE_DISPOSITION_INFORMATION: {
		...
		return set_file_disposition_info(
			fp, (struct smb2_file_disposition_info *)buffer);
	}
	case FILE_FULL_EA_INFORMATION: {
		...
		return smb2_set_ea((struct smb2_ea_info *)buffer, buf_len,
				   &fp->filp->f_path, true);
	}
	case FILE_POSITION_INFORMATION: {
		...
		return set_file_position_info(
			fp, (struct smb2_file_pos_info *)buffer);
	}
	case FILE_MODE_INFORMATION: {
		if (buf_len < sizeof(struct smb2_file_mode_info))
			return -EINVAL;

		return set_file_mode_info(fp,
					  (struct smb2_file_mode_info *)buffer);
	}
	}
...
int smb2_set_info(struct ksmbd_work *work)
{
	...
	case SMB2_O_INFO_SECURITY:
		...
		rc = smb2_set_info_sec(
			fp, le32_to_cpu(req->AdditionalInformation),
...
static noinline int smb2_read_pipe(struct ksmbd_work *work)
{
	...
	if (rpc_resp) {
		...
		aux_payload_buf = kvmalloc(rpc_resp->payload_sz, GFP_KERNEL);
		if (!aux_payload_buf) {
			err = -ENOMEM;
			goto out;
		}

		memcpy(aux_payload_buf, rpc_resp->payload,
		       rpc_resp->payload_sz);

		nbytes = rpc_resp->payload_sz;
		err = ksmbd_iov_pin_rsp_read(work, (void *)rsp,
					     offsetof(struct smb2_read_rsp,
						      Buffer),
				      ...
				      size_t length)
{
	...
	err = ksmbd_conn_rdma_write(
		work->conn, data_buf, length,
		(struct smb2_buffer_desc_v1
			 *)((char *)req +
			    le16_to_cpu(req->ReadChannelInfoOffset)),
...
int smb2_read(struct ksmbd_work *work)
{
	...
	if (is_rdma_channel == true) {
		...
		err = smb2_set_remote_key_for_rdma(
			work,
			(struct smb2_buffer_desc_v1 *)((char *)req + ch_offset),
			req->Channel, req->ReadChannelInfoLength);
		if (err)
			goto out;
	}
				       ...
				       size_t length, bool sync)
{
	...
	ret = ksmbd_vfs_write(work, fp, data_buf, length, &offset, sync,
			      &nbytes);
	kvfree(data_buf);
	if (ret < 0)
		return ret;
...
int smb2_write(struct ksmbd_work *work)
{
	...
	if (is_rdma_channel == true) {
		...
		err = smb2_set_remote_key_for_rdma(
			work,
			(struct smb2_buffer_desc_v1 *)((char *)req + ch_offset),
			req->Channel, req->WriteChannelInfoLength);
		if (err)
			goto out;
	}
...
int smb2_lock(struct ksmbd_work *work)
{
	...
	list_for_each_entry_safe (smb_lock, tmp, &lock_list, llist) {
		...
		if ((prior_lock &
			     (SMB2_LOCKFLAG_EXCLUSIVE | SMB2_LOCKFLAG_SHARED) &&
	...
	skip:
		if (smb_lock->flags & SMB2_LOCKFLAG_UNLOCK) {
			...
			if (rc == FILE_LOCK_DEFERRED) {
				...
				rc = setup_async_work(
					work, smb2_remove_blocked_lock, argv);
				if (rc) {
					kfree(argv);
					err = -ENOMEM;
					goto out;
...
static int fsctl_copychunk(struct ksmbd_work *work,
			   ...
			   unsigned int cnt_code, unsigned int input_count,
			   ...
			   struct smb2_ioctl_rsp *rsp)
{
	...
	ret = ksmbd_vfs_copy_file_ranges(work, src_fp, dst_fp, chunks,
					...
					unsigned int out_buf_len)
{
	...
	ipv6_retry:
		...
		nii_rsp = (struct network_interface_info_ioctl_rsp *)&rsp
				  ->Buffer[nbytes];
...
static int fsctl_validate_negotiate_info(
	struct ksmbd_conn *conn, struct validate_negotiate_info_req *neg_req,
	struct validate_negotiate_info_rsp *neg_rsp, unsigned int in_buf_len)
{
	int ret = 0;
	int dialect;

	if (in_buf_len <
	    offsetof(struct validate_negotiate_info_req, Dialects) +
...
static int
fsctl_query_allocated_ranges(struct ksmbd_work *work, u64 id,
			     ...
			     unsigned int in_count, unsigned int *out_count)
{
	...
	*out_count = 0;
	...
	ret = ksmbd_vfs_fqar_lseek(fp, start, length, qar_rsp, in_count,
				   out_count);
	if (ret && ret != -E2BIG)
		*out_count = 0;
				   ...
				   struct file_sparse *sparse)
{
	...
	if (fp->f_ci->m_fattr != old_fattr &&
		...
		ret = ksmbd_vfs_get_dos_attrib_xattr(fp->filp->f_path.dentry,
						     &da);
		if (ret <= 0)
			goto out;

		da.attr = le32_to_cpu(fp->f_ci->m_fattr);
		ret = ksmbd_vfs_set_dos_attrib_xattr(&fp->filp->f_path, &da,
						     true);
		if (ret)
...
int smb2_ioctl(struct ksmbd_work *work)
{
	...
	case FSCTL_CREATE_OR_GET_OBJECT_ID: {
		...
		obj_buf = (struct file_object_buf_type1_ioctl_rsp *)&rsp
				  ->Buffer[0];
	...
	case FSCTL_VALIDATE_NEGOTIATE_INFO:
		...
		ret = fsctl_validate_negotiate_info(
			conn, (struct validate_negotiate_info_req *)buffer,
	...
	case FSCTL_SET_ZERO_DATA: {
		...
		zero_data = (struct file_zero_data_information *)buffer;
	...
	case FSCTL_QUERY_ALLOCATED_RANGES:
		...
		ret = fsctl_query_allocated_ranges(
			work, id, (struct file_allocated_range_buffer *)buffer,
	...
	case FSCTL_GET_REPARSE_POINT: {
		...
		reparse_ptr->ReparseTag = smb2_get_reparse_tag_special_file(
	...
	ret = ksmbd_iov_pin_rsp(work, rsp,
				sizeof(struct smb2_ioctl_rsp) + nbytes);
	if (!ret)
		return ret;
...
bool smb2_is_sign_req(struct ksmbd_work *work, unsigned int command)
{
	...
	if ((rcv_hdr2->Flags & SMB2_FLAGS_SIGNED) &&
	    command != SMB2_NEGOTIATE_HE && command != SMB2_SESSION_SETUP_HE &&
...
int smb3_decrypt_req(struct ksmbd_work *work)
{
	...
	sess = ksmbd_session_lookup_all(work->conn,
...
bool smb3_11_final_sess_setup_resp(struct ksmbd_work *work)
{
	...
	if (le16_to_cpu(rsp->Command) == SMB2_SESSION_SETUP_HE && sess->user &&
	    !user_guest(sess->user) && rsp->Status == STATUS_SUCCESS)
		return true;
	return false;
}
