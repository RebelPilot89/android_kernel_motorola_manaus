 ...
 */
static struct oplock_info *alloc_opinfo(struct ksmbd_work *work, u64 id,
					__u16 Tid)
{
...
static void copy_lease(struct oplock_info *op1, struct oplock_info *op2)
{
	...
	memcpy(lease2->lease_key, lease1->lease_key, SMB2_LEASE_KEY_SIZE);
...
{
	if (lease_state ==
	    (SMB2_LEASE_HANDLE_CACHING_LE | SMB2_LEASE_READ_CACHING_LE |
	     SMB2_LEASE_WRITE_CACHING_LE)) {
...
void create_lease_buf(u8 *rbuf, struct lease *lease)
{
	if (lease->version == 2) {
		...
		if (lease->flags == SMB2_LEASE_FLAG_PARENT_LEASE_KEY_SET_LE)
			memcpy(buf->lcontext.ParentLeaseKey,
			       lease->parent_lease_key, SMB2_LEASE_KEY_SIZE);
		buf->ccontext.DataOffset =
			cpu_to_le16(offsetof(struct create_lease_v2, lcontext));
		buf->ccontext.DataLength =
			cpu_to_le32(sizeof(struct lease_context_v2));
		buf->ccontext.NameOffset =
			cpu_to_le16(offsetof(struct create_lease_v2, Name));
		...
		buf->lcontext.LeaseState = lease->state;
		buf->ccontext.DataOffset =
			cpu_to_le16(offsetof(struct create_lease, lcontext));
		buf->ccontext.DataLength =
			cpu_to_le32(sizeof(struct lease_context));
		buf->ccontext.NameOffset =
			cpu_to_le16(offsetof(struct create_lease, Name));
		buf->ccontext.NameLength = cpu_to_le16(4);
		buf->Name[0] = 'R';
		buf->Name[1] = 'q';
		buf->Name[2] = 'L';
...
struct lease_ctx_info *parse_lease_state(void *open_req, bool is_dir)
{
	...
	if (sizeof(struct lease_context_v2) == le32_to_cpu(cc->DataLength)) {
		...
		memcpy(lreq->lease_key, lc->lcontext.LeaseKey,
		       SMB2_LEASE_KEY_SIZE);
		if (is_dir) {
					      ...
					      int tag_len)
{
	...
	do {
		...
		if ((next & 0x7) != 0 || next > remain_len ||
		    name_off != offsetof(struct create_context, Buffer) ||
		    name_len < 4 || name_off + name_len > cc_len ||
		    (value_off & 0x7) != 0 ||
		    (value_len &&
...
void create_durable_rsp_buf(char *cc)
{
	...
	buf->ccontext.DataOffset =
		cpu_to_le16(offsetof(struct create_durable_rsp, Data));
	buf->ccontext.DataLength = cpu_to_le32(8);
	buf->ccontext.NameOffset =
		cpu_to_le16(offsetof(struct create_durable_rsp, Name));
	buf->ccontext.NameLength = cpu_to_le16(4);
...
void create_mxac_rsp_buf(char *cc, int maximal_access)
{
	...
	buf->ccontext.DataOffset =
		cpu_to_le16(offsetof(struct create_mxac_rsp, QueryStatus));
	buf->ccontext.DataLength = cpu_to_le32(8);
	buf->ccontext.NameOffset =
		cpu_to_le16(offsetof(struct create_mxac_rsp, Name));
	buf->ccontext.NameLength = cpu_to_le16(4);
...
void create_disk_id_rsp_buf(char *cc, __u64 file_id, __u64 vol_id)
{
	...
	buf->ccontext.DataOffset =
		cpu_to_le16(offsetof(struct create_disk_id_rsp, DiskFileId));
...
void create_posix_rsp_buf(char *cc, struct ksmbd_file *fp)
{
	...
	buf->ccontext.DataOffset =
		cpu_to_le16(offsetof(struct create_posix_rsp, nlink));
	/*
	...
	buf->ccontext.NameOffset =
		cpu_to_le16(offsetof(struct create_posix_rsp, Name));
	 ...
	 */
	id_to_sid(from_kuid_munged(&init_user_ns, kuid), SIDOWNER,
		  (struct smb_sid *)&buf->SidBuffer[0]);
	id_to_sid(from_kgid_munged(&init_user_ns, kgid), SIDUNIX_GROUP,
		  (struct smb_sid *)&buf->SidBuffer[28]);
}
...
found:
	...
	list_for_each_entry_rcu (opinfo, &lt->lease_list, lease_entry) {
		...
		if (!(opinfo->o_lease->state & (SMB2_LEASE_HANDLE_CACHING_LE |
						SMB2_LEASE_WRITE_CACHING_LE)))
			goto op_next;
		ret = compare_guid_key(opinfo, conn->ClientGUID, lease_key);
		if (ret) {
