...
static bool ksmbd_ipc_validate_version(struct genl_info *m)
{
	if (m->genlhdr->version != KSMBD_GENL_VERSION) {
		pr_err("%s. ksmbd: %d, kernel module: %d. %s.\n",
		       ...
		       m->genlhdr->version, KSMBD_GENL_VERSION,
...
static int ipc_server_config_on_startup(struct ksmbd_startup_request *req)
{
	...
	if (check_mul_overflow((unsigned long)req->deadtime,
			       (unsigned long)SMB_ECHO_INTERVAL,
			       (unsigned long *)&server_conf.deadtime)) {
		ret = -EINVAL;
		goto out;
	}
...
out:
	if (ret) {
		pr_err("Server configuration error: %s %s %s\n",
		       req->netbios_name, req->server_string, req->work_group);
		return ret;
	}
				  ...
				  unsigned int handle)
{
	...
	ret = wait_event_interruptible_timeout(
		entry.wait, entry.response != NULL, IPC_WAIT_TIMEOUT);
	if (entry.response) {
...
struct ksmbd_tree_connect_response *ksmbd_ipc_tree_connect_request(
	struct ksmbd_session *sess, struct ksmbd_share_config *share,
	struct ksmbd_tree_connect *tree_conn, struct sockaddr *peer_addr)
{
	struct ksmbd_ipc_msg *msg;
	...
	strscpy(req->account, user_name(sess->user),
