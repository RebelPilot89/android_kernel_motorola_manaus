			 ...
			 struct path *path)
{
	...
	if (!err && unlikely(nd->flags & LOOKUP_MOUNTPOINT)) {
		err = handle_lookup_down(nd);
		nd->flags &=
			~LOOKUP_JUMPED; // no d_weak_revalidate(), please...
	}
	if (!err)
			  ...
			  const struct open_flags *op)
{
	...
	int error;
	if (susfs_sus_path_by_filename(pathname, &error,
		       ...
		       unsigned int dev)
{
	...
	status =
		susfs_sus_path_by_filename(fname, &error, SYSCALL_FAMILY_MKNOD);
	putname_safe(fname);
	if (status)
		return error;
#endif
...
long do_rmdir(int dfd, struct filename *name)
{
	...
	if (!IS_ERR(name)) {
		status = susfs_sus_path_by_filename(name, &error,
						    SYSCALL_FAMILY_RMDIR);
		if (status)
			return error;
	}
	error = 0;
			 ...
			 const char __user *newname)
{
	...
	status = susfs_sus_path_by_filename(fname, &error,
					    SYSCALL_FAMILY_SYMLINKAT_NEWNAME);
	putname_safe(fname);
	if (status)
		return error;
#endif
		 ...
		 struct filename *to, unsigned int flags)
{
	...
	status = susfs_sus_path_by_filename(from, &error,
					    SYSCALL_FAMILY_RENAMEAT2_OLDNAME);
	if (status)
		goto put_both;

	status = susfs_sus_path_by_filename(to, &error,
					    SYSCALL_FAMILY_RENAMEAT2_NEWNAME);
	if (status)
		goto put_both;
#endif
