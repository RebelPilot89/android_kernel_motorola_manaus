...
/* Porting helpers for 5.10 kernel */
...
#define filemap_invalidate_lock(mapping) down_write(&mapping->i_mmap_rwsem)
#define filemap_invalidate_unlock(mapping) up_write(&mapping->i_mmap_rwsem)

#define fault_in_iov_iter_readable(i, bytes) fault_in_pages_readable(i->iov->iov_base + i->iov_offset, bytes)
#define copy_page_from_iter_atomic(page, off, bytes, i) copy_page_from_iter(page, off, bytes, i)

#define bh_read(bh, flags) (lock_buffer(bh), get_bh(bh), bh->b_end_io = end_buffer_read_sync, submit_bh(REQ_OP_READ, 0, bh), wait_on_buffer(bh), buffer_uptodate(bh) ? 0 : -EIO)
#define sync_blockdev_nowait(bdev) (0)
#define size_add(a, b) ((a) + (b))
#define folio page
#define read_folio readpage
#define dirty_folio set_page_dirty
#define block_dirty_folio __set_page_dirty_buffers
#define block_invalidate_folio(page, offset, len) block_invalidatepage(page, offset, len)
#define invalidate_folio invalidatepage
#define iov_iter_count(i) ((i)->count)
#define bdev_nr_bytes(bdev) (i_size_read((bdev)->bd_inode))
#define alloc_inode_sb(sb, cache, gfp) kmem_cache_alloc(cache, gfp)
#ifndef FS_ALLOW_IDMAP
#define FS_ALLOW_IDMAP 0
#endif

static inline int ntfs_inode_init_owner(struct user_namespace *mnt_userns, struct inode *inode, const struct inode *dir, umode_t mode)
{
	inode_init_owner(inode, dir, mode);
	return 0;
}
#define inode_init_owner ntfs_inode_init_owner

static inline void ntfs_generic_fillattr(struct user_namespace *mnt_userns, struct inode *inode, struct kstat *stat)
{
	generic_fillattr(inode, stat);
}
#define generic_fillattr ntfs_generic_fillattr

static inline int ntfs_setattr_prepare(struct user_namespace *mnt_userns, struct dentry *dentry, struct iattr *attr)
{
	return setattr_prepare(dentry, attr);
}
#define setattr_prepare ntfs_setattr_prepare

static inline void ntfs_setattr_copy(struct user_namespace *mnt_userns, struct inode *inode, const struct iattr *attr)
{
	setattr_copy(inode, attr);
}
#define setattr_copy ntfs_setattr_copy

static inline int ntfs_blkdev_issue_discard(struct block_device *bdev, sector_t sector, sector_t nr_sects, gfp_t gfp_mask)
{
	return blkdev_issue_discard(bdev, sector, nr_sects, gfp_mask, 0);
}
#define blkdev_issue_discard ntfs_blkdev_issue_discard
