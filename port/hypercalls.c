...
//
// Merged with pKVM and hypercall enhancements from hypercalls-donor.c

#include <linux/arm-smccc.h>
#include <linux/kvm_host.h>

#include <asm/kvm_emulate.h>
#include <asm/kvm_pkvm.h>		/* added for protected mode support */

#include <kvm/arm_hypercalls.h>
#include <kvm/arm_psci.h>

/* Feature bitmaps – imported from donor */
...
/* PTP time retrieval helper (from donor) */
...
/* Default allowed hypercalls (from donor) */
static bool kvm_hvc_call_default_allowed(u32 func_id)
{
	switch (func_id) {
	/* Always‑allowed hypercalls */
	...
	case ARM_SMCCC_VENDOR_HYP_KVM_MMIO_GUARD_MAP_FUNC_ID:
		return true;
	default:
		/* PSCI 0.2 and up (function 0x00..0x1f) */
		if (ARM_SMCCC_OWNER_NUM(func_id) == ARM_SMCCC_OWNER_STANDARD &&
		    ARM_SMCCC_FUNC_NUM(func_id) <= 0x1f)
			return true;

		/* KVM PSCI 0.1 legacy range */
...
/* Check if a given hypercall is allowed by the feature bitmaps (from donor) */
...
/* Main hypercall handler – enhanced with donor logic */
...
/* ------------------------------------------------------------------
 * Firmware register handling (imported from donor)
 * ------------------------------------------------------------------ */
