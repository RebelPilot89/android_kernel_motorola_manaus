...
#ifndef CONFIG_SCSI_UFS_MEDIATEK_DBG
#define CONFIG_SCSI_UFS_MEDIATEK_DBG 1
#endif
...
#ifndef SPREAD_PRINTF
#define SPREAD_PRINTF(buff, size, m, fmt, ...) \
    do { \
        if (m) seq_printf(m, fmt, ##__VA_ARGS__); \
        else if (buff) snprintf(buff, size, fmt, ##__VA_ARGS__); \
    } while (0)
#endif
