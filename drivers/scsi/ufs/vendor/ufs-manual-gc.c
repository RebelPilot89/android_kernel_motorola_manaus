...
/* for manual gc */
static ssize_t manual_gc_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
			       ...
			       size_t count)
{
	...
	if (hba->dev_info.wspecversion >= 0x0310) {
		...
		ufshcd_query_flag_retry(
			hba, opcode,
			QUERY_FLAG_IDN_WB_BUFF_FLUSH_DURING_HIBERN8, index,
			NULL);
...
{
	struct ufs_mtk_host *ufs =
		container_of(timer, struct ufs_mtk_host, manual_gc.hrtimer);
...
{
	struct ufs_mtk_host *ufs =
		container_of(work, struct ufs_mtk_host, manual_gc.hibern8_work);
...
static struct attribute *pixel_sysfs_ufshcd_attrs[] = {
	&dev_attr_manual_gc.attr, &dev_attr_manual_gc_hold.attr, NULL
};

static struct attribute *pixel_sysfs_ufshcd_hagc_status_attrs[] = {
	&dev_attr_manual_gc_status.attr, NULL
};
...
{
	struct ufs_mtk_host *ufs =
		container_of(work, struct ufs_mtk_host, update_sysfs_work);
	struct ufs_hba *hba = ufs->hba;
	int err;

	err = sysfs_create_groups(&hba->dev->kobj, pixel_sysfs_hid_groups);
	if (err)
...
int pixel_init(struct ufs_hba *hba)
{
	...
	ret = register_trace_android_vh_ufs_update_sysfs(pixel_ufs_update_sysfs,
							 NULL);
	if (ret)
		return ret;
#endif
