...
static int aw99703_bl_enable_channel(struct aw99703_data *drvdata)
{
	...
	if (drvdata->channel == 3) {
		...
		ret = aw99703_i2c_write_bit(drvdata->client, AW99703_REG_LEDCUR,
		...
		ret = aw99703_i2c_write_bit(drvdata->client, AW99703_REG_LEDCUR,
					    AW99703_LEDCUR_CHANNEL_MASK, 0x98);
	}
...
{
	if (drvdata->pwm_mode) {
		aw99703_i2c_write_bit(drvdata->client, AW99703_REG_MODE,
...
{
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_TURNCFG,
...
{
	pr_info("%s enter\n", __func__);
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_TRANCFG,
...
static int aw99703_backlight_init(struct aw99703_data *drvdata)
{
	...
	/*mode:map type*/
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_MODE,
			      AW99703_MODE_MAP_MASK, drvdata->map_type);

	/*default OVPSEL 38V*/
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_BSTCTR1,
			      AW99703_BSTCTR1_OVPSEL_MASK, drvdata->ovp_level);
	...
	/*OCP SELECT*/
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_BSTCTR1,
			      AW99703_BSTCTR1_OCPSEL_MASK, drvdata->ocp_level);

	/*BSTCRT2 IDCTSEL*/
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_BSTCTR2,
			      AW99703_BSTCTR2_IDCTSEL_MASK,
			      AW99703_BSTCTR2_IDCTSEL_10UH);

	/*Backlight current full scale*/
	aw99703_i2c_write_bit(drvdata->client, AW99703_REG_LEDCUR,
...
static void __aw99703_work(struct aw99703_data *led, enum led_brightness value)
{
	mutex_lock(&led->lock);
...
static void aw99703_work(struct work_struct *work)
{
	struct aw99703_data *drvdata =
		container_of(work, struct aw99703_data, work);
				...
				struct aw99703_data *drvdata)
{
	...
	rc = of_property_read_u32(np, "aw99703,current-align-type",
				  &drvdata->led_current_align);
	if (rc != 0) {
	...
	drvdata->skip_first_trans =
		of_property_read_bool(np, "aw99703,skip-first-trans");
	pr_info("%s skip_first_trans --<%d>\n", __func__,
		drvdata->skip_first_trans);

	drvdata->reset_trans_delay =
		of_property_read_bool(np, "aw99703,reset-trans-delay");
	pr_info("%s reset_trans_delay --<%d>\n", __func__,
		drvdata->reset_trans_delay);

	drvdata->bl_low_remap =
		of_property_read_bool(np, "aw99703,bl-low-remap");
	if (drvdata->bl_low_remap) {
		pr_info("%s bl_low_remap --<%d>\n", __func__,
			drvdata->bl_low_remap);
	}

	rc = of_property_read_u32(np, "aw99703,default-brightness",
				  &drvdata->default_brightness);
	if (rc != 0) {
	...
	rc = of_property_read_u32(np, "aw99703,reinit-brightness",
				  &drvdata->reinit_brightness);
	if (!rc)
				     ...
				     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		aw99703_i2c_write(aw99703->client, (unsigned char)databuf[0],
...
static struct attribute *aw99703_attributes[] = { &dev_attr_reg.attr, NULL };
			 ...
			 const struct i2c_device_id *id)
{
	...
	bl_dev = backlight_device_register(AW99703_NAME, &client->dev, drvdata,
...
static const struct i2c_device_id aw99703_id[] = { { AW99703_NAME, 0 }, {} };
