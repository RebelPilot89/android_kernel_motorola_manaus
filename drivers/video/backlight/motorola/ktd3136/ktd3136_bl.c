...
static int ktd3136_masked_write(struct i2c_client *client, int reg, u8 mask,
				u8 val)
{
	int rc;
	u8 temp;
...
static int ktd3136_bl_enable_channel(struct ktd3136_data *drvdata)
{
	...
	if (drvdata->channel == 0) {
		...
		ret = ktd3136_write_reg(drvdata->client, REG_PWM,
					0x18); //b0001 1000
	} else if (drvdata->channel == 3) {
		pr_debug("turn all channel on!\n");
		ret = ktd3136_masked_write(drvdata->client, REG_PWM, 0x07,
					   0x07);
...
static void ktd3136_ramp_setting(struct ktd3136_data *drvdata)
{
	...
	if (drvdata->ramp_on_time == 0) { //512us
		...
		ktd3136_masked_write(drvdata->client, REG_RAMP_ON, 0xf0,
				     temp << 4);
		pr_debug("temp is %d\n", temp);
	}
...
static int ktd3136_backlight_enable(struct ktd3136_data *drvdata)
{
	int err = 0;
	pr_info("%s enter.\n", __func__);
	ktd3136_masked_write(drvdata->client, REG_MODE, 0xf8,
...
int ktd3136_set_brightness(struct ktd3136_data *drvdata, int brt_val)
{
	...
	if (brt_val > 0) {
		ktd3136_masked_write(drvdata->client, REG_MODE, 0x01,
				     0x01); //enalbe bl mode
	} else {
		ktd3136_masked_write(drvdata->client, REG_MODE, 0x01,
				     0x00); //disable bl mode
	}
	if (drvdata->using_lsb) {
		ktd3136_masked_write(drvdata->client, REG_RATIO_LSB, 0x07,
				     brt_val);
		ktd3136_masked_write(drvdata->client, REG_RATIO_MSB, 0xff,
				     brt_val >> 3);
	} else {
		ktd3136_masked_write(drvdata->client, REG_RATIO_LSB, 0x07,
				     ktd3136_brightness_table_reg4[brt_val]);
		ktd3136_masked_write(drvdata->client, REG_RATIO_MSB, 0xff,
				     ktd3136_brightness_table_reg5[brt_val]);
	}
...
static void __ktd3136_work(struct ktd3136_data *led, enum led_brightness value)
{
	mutex_lock(&led->lock);
...
{
	struct ktd3136_data *drvdata =
		container_of(work, struct ktd3136_data, work);
...
static void ktd3136_get_dt_data(struct device *dev,
				struct ktd3136_data *drvdata)
{
	int rc;
	...
	rc = of_property_read_u32(np, "ktd,current-align-type",
				  &drvdata->led_current_align);
	if (rc != 0) {
	...
	drvdata->skip_first_trans =
		of_property_read_bool(np, "ktd,skip-first-trans");
	pr_info("%s skip_first_trans --<%d>\n", __func__,
		drvdata->skip_first_trans);

	drvdata->reset_trans_delay =
		of_property_read_bool(np, "ktd,reset-trans-delay");
	...
	rc = of_property_read_u32(np, "ktd,default-brightness",
...
static void ktd3136_data_init(struct ktd3136_data *drvdata)
{
	...
	((drvdata->induct_current == 2600) || (drvdata->induct_current == 1)) ?
		update_value |= 0x08 :
		update_value;
				     ...
				     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		ktd3136_write_reg(ktd3136->client, (unsigned char)databuf[0],
...
static struct attribute *ktd3136_attributes[] = { &dev_attr_reg.attr, NULL };
			 ...
			 const struct i2c_device_id *id)
{
	...
	bl_dev = backlight_device_register(KTD3136_NAME, &client->dev, drvdata,
					   &ktd3136_bl_ops, &props);
#endif
...
static const struct i2c_device_id ktd3136_id[] = { { KTD3136_NAME, 0 }, {} };
