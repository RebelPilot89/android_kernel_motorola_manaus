...
static const u32 mt6853_spmi_regs[] = {
	[SPMI_OP_ST_CTRL] = 0x0000,   [SPMI_GRP_ID_EN] = 0x0004,
	[SPMI_OP_ST_STA] = 0x0008,    [SPMI_MST_SAMPL] = 0x000C,
	[SPMI_MST_REQ_EN] = 0x0010,   [SPMI_MST_RCS_CTRL] = 0x0014,
	[SPMI_SLV_3_0_EINT] = 0x0020, [SPMI_SLV_7_4_EINT] = 0x0024,
	[SPMI_SLV_B_8_EINT] = 0x0028, [SPMI_SLV_F_C_EINT] = 0x002C,
	[SPMI_REC_CTRL] = 0x0040,     [SPMI_REC0] = 0x0044,
	[SPMI_REC1] = 0x0048,	      [SPMI_REC2] = 0x004C,
	[SPMI_REC3] = 0x0050,	      [SPMI_REC4] = 0x0054,
	[SPMI_REC_CMD_DEC] = 0x005C,  [SPMI_DEC_DBG] = 0x00F8,
	[SPMI_MST_DBG] = 0x00FC,
};
...
static const u32 mt6873_spmi_regs[] = {
	[SPMI_OP_ST_CTRL] = 0x0000, [SPMI_GRP_ID_EN] = 0x0004,
	[SPMI_OP_ST_STA] = 0x0008,  [SPMI_MST_SAMPL] = 0x000c,
	[SPMI_MST_REQ_EN] = 0x0010, [SPMI_REC_CTRL] = 0x0040,
	[SPMI_REC0] = 0x0044,	    [SPMI_REC1] = 0x0048,
	[SPMI_REC2] = 0x004c,	    [SPMI_REC3] = 0x0050,
	[SPMI_REC4] = 0x0054,	    [SPMI_MST_DBG] = 0x00fc,
};

enum { SPMI_RCS_SR_BIT, SPMI_RCS_A_BIT };

enum { SPMI_RCS_MST_W = 1, SPMI_RCS_SLV_W = 3 };
...
static int pmif_arb_cmd(struct spmi_controller *ctrl, u8 opc, u8 sid)
{
	...
	ret = readl_poll_timeout_atomic(
		arb->spmimst_base + arb->spmimst_regs[SPMI_OP_ST_STA], rdata,
		(rdata & SPMI_OP_ST_BUSY) == SPMI_OP_ST_BUSY, PMIF_DELAY_US,
		PMIF_TIMEOUT);
	if (ret < 0)
			      ...
			      u16 addr, u8 *buf, size_t len)
{
	...
	if (ret < 0) {
		dev_err(&ctrl->dev,
			"check IDLE timeout, read 0x%x, sta=0x%x, SPMI_DBG=0x%x\n",
			addr, pmif_readl(arb, inf_reg->ch_sta),
			readl(arb->spmimst_base +
	...
	/* Send the command. */
	pmif_writel(arb, (opc << 30) | (sid << 24) | (bc << 16) | addr,
		    inf_reg->ch_send);
	...
	if (ret < 0) {
		dev_err(&ctrl->dev,
			"check WFVLDCLR timeout, read 0x%x, sta=0x%x, SPMI_DBG=0x%x\n",
			addr, pmif_readl(arb, inf_reg->ch_sta),
			readl(arb->spmimst_base +
			      arb->spmimst_regs[SPMI_MST_DBG]));
		raw_spin_unlock_irqrestore(&arb->lock, flags);
...
static struct pmif_irq_desc pmif_event_irq[] = {
	PMIF_IRQDESC(event_0), PMIF_IRQDESC(event_1), PMIF_IRQDESC(event_2),
	PMIF_IRQDESC(event_3), PMIF_IRQDESC(event_4),
};

static void pmif_irq_register(struct platform_device *pdev, struct pmif *arb,
			      int irq)
{
	int i = 0, ret = 0;
	...
	for (i = 0; i < ARRAY_SIZE(pmif_event_irq); i++) {
		...
		ret = devm_request_threaded_irq(
			&pdev->dev, irq, NULL, pmif_event_irq[i].irq_handler,
...
static irqreturn_t rcs_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < SPMI_MAX_SLAVE_ID; i++) {
		...
		if (arb->rcs_enable_hwirq[i] && slv_irq_sta) {
			dev_info(&arb->spmic->dev, "hwirq=%d, sta=0x%x\n", i,
				 slv_irq_sta);
...
static int rcs_irq_register(struct platform_device *pdev, struct pmif *arb,
			    int irq)
{
	int i, ret = 0;
	...
	arb->domain = irq_domain_add_linear(
		pdev->dev.of_node, SPMI_MAX_SLAVE_ID, &rcs_irq_domain_ops, arb);
	if (!arb->domain) {
	...
	ret = devm_request_threaded_irq(&pdev->dev, irq, NULL, rcs_irq_handler,
					IRQF_ONESHOT, rcs_irq_chip.name, arb);
	if (ret < 0) {
...
static int mtk_spmi_probe(struct platform_device *pdev)
{
	...
	spmi_controller_set_drvdata(
		ctrl, (void *)of_device_get_match_data(&pdev->dev));
	...
	if (arb->caps == 1) {
		...
		if (IS_ERR(arb->pmif_sys_ck)) {
			dev_notice(&pdev->dev,
				   "[PMIF]:failed to get ap clock: %ld\n",
		...
		if (IS_ERR(arb->pmif_tmr_ck)) {
			dev_notice(&pdev->dev,
				   "[PMIF]:failed to get tmr clock: %ld\n",
				   PTR_ERR(arb->pmif_tmr_ck));
			goto err_put_ctrl;
		}

		arb->spmimst_clk_mux =
			devm_clk_get(&pdev->dev, "spmimst_clk_mux");
		...
		if (err) {
			dev_notice(
				&pdev->dev,
				"[SPMIMST]:failed to enable spmi master clk\n");
			goto err_put_ctrl;
		}
	} else {
		...
		if (err)
			dev_notice(&pdev->dev,
				   "[PMIF]:failed to enable pmif_clk_mux\n");

		arb->spmimst_m_clk_mux =
			devm_clk_get(&pdev->dev, "spmimst_m_clk_mux");
		...
		if (err)
			dev_notice(&pdev->dev,
				   "[PMIF]:failed to enable spmimst_m_clk\n");

		arb->spmimst_p_clk_mux =
			devm_clk_get(&pdev->dev, "spmimst_p_clk_mux");
		...
		if (err)
			dev_notice(
				&pdev->dev,
				"[SPMIMST]:failed to enable spmimst_p_clk\n");

		arb->vlp_pmif_clk_mux =
			devm_clk_get(&pdev->dev, "vlp_pmif_clk_mux");
		...
		if (err)
			dev_notice(&pdev->dev,
				   "[PMIF]:failed to enable vlp_pmif_clk\n");

		arb->vlp_spmimst_m_clk_mux =
			devm_clk_get(&pdev->dev, "vlp_spmimst_m_clk_mux");
		...
		if (err)
			dev_notice(
				&pdev->dev,
				"[SPMIMST]:failed to enable vlp_spmimst_m_clk\n");

		arb->vlp_spmimst_p_clk_mux =
			devm_clk_get(&pdev->dev, "vlp_spmimst_p_clk_mux");
		...
		if (err)
			dev_notice(
				&pdev->dev,
				"[SPMIMST]:failed to enable vlp_spmimst_p_clk\n");
	}
#else
	dev_notice(&pdev->dev, "[PMIF]:no need to get clock at fpga\n");
#endif /* #if defined(CONFIG_MTK_FPGA) || defined(CONFIG_FPGA_EARLY_PORTING) */
	arb->chan.ch_sta =
		PMIF_SWINF_0_STA + (PMIF_CHAN_OFFSET * arb->soc_chan);
	arb->chan.wdata =
		PMIF_SWINF_0_WDATA_31_0 + (PMIF_CHAN_OFFSET * arb->soc_chan);
	arb->chan.rdata =
		PMIF_SWINF_0_RDATA_31_0 + (PMIF_CHAN_OFFSET * arb->soc_chan);
	arb->chan.ch_send =
		PMIF_SWINF_0_ACC + (PMIF_CHAN_OFFSET * arb->soc_chan);
	arb->chan.ch_rdy =
		PMIF_SWINF_0_VLD_CLR + (PMIF_CHAN_OFFSET * arb->soc_chan);

	raw_spin_lock_init(&arb->lock);
	arb->pmifThread_lock = wakeup_source_register(NULL, "pmif wakelock");
