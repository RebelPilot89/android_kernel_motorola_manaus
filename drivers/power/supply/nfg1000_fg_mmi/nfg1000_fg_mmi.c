...
#define NAKE_DWORD_8BITS(HH, HL, LH, LL)                                       \
...
u8 sha_256_ramdom_data[64] = { 0x20, 0x31, 0x0C, 0xFF, 0x52, 0xF5, 0x06,
			       0x2E, 0x1D, 0x74, 0x73, 0xDD, 0x75, 0xAD,
			       0x16, 0x6F, 0x09, 0x61, 0x5B, 0xD2, 0xE2,
			       0xA6, 0xE8, 0x62, 0x31, 0x5F, 0x17, 0xC9,
			       0xDF, 0x52, 0x45, 0xE6, 0xBA };
u8 sha_256_pass_word[64] = { 0xD0, 0x81, 0xCA, 0x11, 0xBF, 0xF1, 0xFA,
			     0xA9, 0xA6, 0xF8, 0x2A, 0xD5, 0x6C, 0x94,
			     0x47, 0x00, 0x93, 0x75, 0xE8, 0xE6, 0xC2,
			     0x97, 0x8B, 0x7D, 0x17, 0x44, 0xC7, 0x0E,
			     0x99, 0xB9, 0xC0, 0x67, 0xDF };
u8 app_flash_erase_order[195] = {
	0x00, 0x5F, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02, 0x00, 0x03, 0x00, 0x04,
...
u8 data_flash_erase_order[] = { 0x00, 0x07, 0x01, 0x86, 0x01, 0x87, 0x01,
				0x88, 0x01, 0x89, 0x01, 0x8A, 0x01, 0x8B,
				0x01, 0x8C, 0x01, 0x8D, 0x07 };
u8 nfg1000_Dataflash_updata_CMD[] = { 0x00, 0x02, 0x03, 0x04, 0x05, 0x06,
				      0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0D,
...
static int nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(struct mmi_fg_chip *dev,
							u16 reg, u8 *poutbuf,
							u32 len)
{
	u8 reg_data[2];
...
static int
nfg1000_i2c_BLOCK_command_write_with_CHECKSUM(struct mmi_fg_chip *dev, u16 reg,
					      u8 *pinbuf, u32 len)
{
	u8 databuf[36] = { 0 };
	int ret;
...
static int nfg1000_ota_unseal(struct mmi_fg_chip *di)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ret = nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
			di, seal_state_cmd, seal_state_read, 4);
		if (ret) {
...
static bool nfg1000_ota_program_check_fw_upgrade(struct mmi_fg_chip *di)
{
	...
	ret = nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
		di, FG_MAC_CMD_FW_VER, fw_ver_read, 12);
	if (ret) {
	...
	mmi_info(
		":Read From fuelgauge, Firmware version=[%02x %02x %02x %02x %02x]\n",
		fw_ver_read[0], fw_ver_read[1], fw_ver_read[2], fw_ver_read[3],
		fw_ver_read[4]);
	fg_print_buf("dts config fw_version", di->fw_version, 5);
	if (di->fw_version) {
		if (memcmp(fw_ver_read, di->fw_version, 5) < 0) {
			mmi_info(
				" current firmware version is not latest !need upgrading...\n");
			upgrade_status = true;
		}
	}
...
static u8 *nfg1000_upgrade_read_firmware(char *bin_name,
					 struct mmi_fg_chip *mmi_fg);
//battery parameter version check
static bool
nfg1000_ota_program_check_batt_params_version(struct mmi_fg_chip *di)
{
	bool upgrade_status = false;
	...
	if (nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
		    di, FG_MAC_CMD_PARAMS_VER, dataflash_read, 32) < 0)
		return upgrade_status;
	...
	if (!dev_sn) {
		if (nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
			    di, FG_MAC_CMD_BATT_SERIALNUM, dataflash_read,
			    32)) {
	...
	if (dev_sn && di->battid_cnt != 0 && di->batt_version_cnt != 0) {
		for (i = 0; i < di->battid_cnt; i++) {
			if ((strnstr(dev_sn, di->batt_serialnum_arry[i], 32)) &&
				...
				mmi_info(
					"Need to upgrading battery parameters: %s",
	...
	if (upgrade_status) {
		di->params_data =
			nfg1000_upgrade_read_firmware(batt_params_bin_name, di);
...
static int nfg1000_ota_program_step3_mcuAuth(struct mmi_fg_chip *di)
{
	...
	tmpMcuCode = NAKE_DWORD_8BITS(uReCode[3], uReCode[2], uReCode[1],
				      uReCode[0]);
...
static int nfg1000_ota_program_step6_CheckCrc(struct mmi_fg_chip *di)
{
	...
	Crc32tmp = NAKE_DWORD_8BITS(uReCode[3], uReCode[2], uReCode[1],
				    uReCode[0]);
	init_crc_table();
...
static int nfg1000_ota_program_step8_EraseDATA(struct mmi_fg_chip *di)
{
	...
	ret = fg_write_block(di, I2C_NO_REG_DATA, u8Data,
			     sizeof(data_flash_erase_order));
	if (ret < 0) {
...
static int nfg1000_ota_program_step10_CheckDataCrc(struct mmi_fg_chip *di)
{
	...
	init_crc_table();
	Crc32_code = crc32(0xFFFFFFFF, &di->params_data[CHEMID_START_DATAFLASH],
			   1024);
	if (Crc32_code != Crc32tmp) {
...
static int nfg1000_ota_updata_config(struct mmi_fg_chip *di)
{
	...
	for (i = 0; i < sizeof(nfg1000_Dataflash_updata_CMD);) {
		...
		ret = nfg1000_i2c_BLOCK_command_write_with_CHECKSUM(
			di, dataflash_base_addr + addr_cmd,
			&di->params_data[addr_cmd * 32], 32);
		...
		ret = nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
			di, dataflash_base_addr + addr_cmd, config_data_read,
			32);
		if (ret) {
		...
		for (j = 0; j < 32; j++) {
			if (config_data_read[j] !=
			    di->params_data[addr_cmd * 32 + j]) {
				retry_flag = 1;
				retry_cnt++;
				break;
			}
...
{
	struct mmi_fg_chip *di =
		container_of(work, struct mmi_fg_chip, fg_force_upgrade_work);
	int count = 1;
	...
	if (di->fw_data == NULL)
		di->fw_data = nfg1000_upgrade_read_firmware(
			"NFG1000A_firmware.bin", di);
	if (di->fw_data == NULL) {
...
{
	struct mmi_fg_chip *di =
		container_of(work, struct mmi_fg_chip, fg_upgrade_work);
	int count = 1;
	...
	if (nfg1000_ota_program_check_fw_upgrade(di) == false) {
		...
		for (count = 1; count <= 3; count++) {
			if (nfg1000_upgrade_APP(di) != 0) {
				mmi_err("nfg1000_upgrade_APP failed, retry=%d",
					count);
				if (count == 3) {
					mmi_err("nfg1000_upgrade_APP failed, use fake battery");
					goto upgrade_error;
	...
	if (nfg1000_ota_program_check_batt_params_version(di) == false) {
		...
		for (count = 1; count <= 3; count++) {
			if (nfg1000_upgrade_Params(di) != 0) {
				mmi_err("nfg1000_upgrade_Params failed, retry=%d",
					count);
				if (count == 3) {
				...
				mmi_info(
					"nfg1000_upgrade_Params successfully!!");
				break;
			}
		}
	}
...
struct fg_temp fg_temp_table[23] = {
	{ -40, 4397119 }, { -35, 3088598 }, { -30, 2197225 }, { -25, 1581880 },
	{ -20, 1151036 }, { -15, 846578 },  { -10, 628988 },  { -5, 471632 },
	{ 0, 357011 },	  { 5, 272499 },    { 10, 209709 },   { 15, 162650 },
	{ 20, 127080 },	  { 25, 100000 },   { 30, 79221 },    { 35, 63167 },
	{ 40, 50676 },	  { 45, 40903 },    { 50, 33194 },    { 55, 27090 },
	{ 60, 22224 },	  { 65, 18322 },    { 70, 15184 }
};
#else
struct fg_temp fg_temp_table[23] = {
...
static int fg_read_temperature(struct mmi_fg_chip *mmi)
{
	...
	if (fg_read_word(mmi, mmi->regs[BQ_FG_REG_TEMP], &temp) < 0) {
		...
		fgTemp =
			(int)((s16)temp) -
			K2C_TEMP_BASE; //Thermodynamic temperature to Celsius temperature
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
		int ret = fg_read_current(mmi, mA);
		if (ret < 0) {
			mmi_err("read current error, ret = %d\n", ret);
			return ret;
		}
#else
		fg_read_current(mmi, mA);
#endif
	}
...
int fg_get_charge_full_design(struct gauge_device *gauge_dev,
...
int fg_set_temp(struct gauge_device *gauge_dev, int temp)
{
	...
	if (!mmi->fake_battery) {
		data.temp = temp;
		nfg1000_i2c_BLOCK_command_write_with_CHECKSUM(
			mmi, FG_MAC_CMD_TEMPERATURE, data.hex, 2);
...
int fg_set_shutdown_threshold(struct gauge_device *gauge_dev, int shutd_vol)
{
	...
	if (!mmi->fake_battery) {
		...
		nfg1000_i2c_BLOCK_command_write_with_CHECKSUM(
			mmi, FG_MAC_CMD_POWEROFF_THRESHOLD, data.hex, 2);
...
static const u8 fg_dump_regs[] = {
	...
	0x16, 0x18, 0x1A, 0x1C, 0x1E, 0x20, 0x28, 0x2A, 0x2C,
	0x2E, 0x30, 0x66, 0x68, 0x6C, 0x6E, 0x70,
};
					    ...
					    char *buf)
{
	...
	if (nfg1000_i2c_BLOCK_command_read_with_CHECKSUM(
		    di, FG_MAC_CMD_PARAMS_VER, dataflash_ver_read, 32) == 0) {
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf, "%02x.%02x.%02x.%c%c\n", fw_ver_read[0],
		      fw_ver_read[1], fw_ver_read[2], fw_ver_read[3],
		      fw_ver_read[4]);
				       ...
				       const char *buf, size_t count)
{
	...
	if (mmi == NULL) {
		...
		mmi_info(
			"%s:do_upgrading:%d fake_battery:%d count:%ld buf:%s\n",
			__func__, mmi->do_upgrading, mmi->fake_battery, count,
			buf);
...
static struct attribute *fg_attributes[] = {
	...
	&dev_attr_Params_Ver.attr, &dev_attr_FW_Ver.attr,
	&dev_attr_FW_update.attr,  NULL,
};
...
static int mmi_parse_dt(struct mmi_fg_chip *mmi_fg)
{
	...
	rc = of_property_read_u32(np, "uirbat_pull_up_r_full",
				  &mmi_fg->rbat_pull_up_r);
	if (rc < 0) {
	...
	if (of_find_property(np, "latest_fw_version", &byte_len)) {
		mmi_fg->fw_version = (u8 *)devm_kzalloc(&mmi_fg->client->dev,
							byte_len, GFP_KERNEL);
		if (mmi_fg->fw_version == NULL) {
			mmi_err(" devm_kzalloc fail,exit parse dts");
			return -ENOMEM;
		}
		rc = of_property_read_u8_array(np, "latest_fw_version",
					       mmi_fg->fw_version,
					       byte_len / sizeof(u8));
		if (rc < 0) {
	...
	if (count > 0) {
		...
		mmi_fg->batt_serialnum_arry =
			devm_kzalloc(&mmi_fg->client->dev,
				     count * sizeof(char *), GFP_KERNEL);
		if (mmi_fg->batt_serialnum_arry) {
			for (i = 0; i < mmi_fg->battid_cnt; i++) {
				rc = of_property_read_string_index(
					np, "batt_serialnums", i,
					&mmi_fg->batt_serialnum_arry[i]);
				if (rc < 0)
					mmi_fg->battid_cnt = 0;
				else
					mmi_info(
						"support serialnum[%d](%s)\n",
						i,
						mmi_fg->batt_serialnum_arry[i]);
			}
		}
	...
	if (count > 0) {
		...
		mmi_fg->batt_version_arry = devm_kzalloc(
			&mmi_fg->client->dev, count * sizeof(u16), GFP_KERNEL);
		rc = of_property_read_u16_array(
			np, "batt_versions", mmi_fg->batt_version_arry, count);
		if (rc < 0) {
			...
			const struct i2c_device_id *id)
{
	...
	mmi->gauge_dev =
		gauge_device_register(mmi->gauge_dev_name, &client->dev, mmi,
				      &nfg1000_gauge_ops, &nfg1000_gauge_props);
	...
	if (mmi->fake_battery == false && is_atm_mode() == true) {
		INIT_WORK(&mmi->fg_force_upgrade_work,
			  nfg1000_force_upgrade_func);
		schedule_work(&mmi->fg_force_upgrade_work);
