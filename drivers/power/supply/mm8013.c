...
#include <linux/interrupt.h>  // Añade esta línea
...
int mm8013_check_cap_level(int uisoc)
{
	if (uisoc >= 100)
...
EXPORT_SYMBOL(mm8013_check_cap_level);
					...
					union power_supply_propval *val)
{
	...
	if (has_8013) {
		...
		case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
			...
			val->intval = mm8013_check_cap_level(tmp_val.intval);
			break;
		case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
			mm8013_soc(&tmp_val.intval);
			ui_soc = tmp_val.intval;
			ret = mm8013_check_cap_level(ui_soc);
				 ...
				 const struct i2c_device_id *id)
{
	...
	/* CAMBIO 1: Nombre unico para evitar conflictos y ser detectado por Android */
	psy_desc->name = "mm8013-battery"; 
	...
	/* CAMBIO 2: Usamos devm_ para un registro mas limpio y seguro */
	chip->mm8013_psy = devm_power_supply_register(&client->dev, psy_desc, &psy_cfg);
