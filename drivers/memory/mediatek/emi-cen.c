...
static inline void prepare_a2d_v1(struct emi_cen *cen)
{
	...
	s6s->dualrk_ch1 =
		test_bit(EMI_CONA_DUAL_RANK_EN_CHN1, &emi_cona) ? 1 : 0;
	...
	s6s->chn_4bank_mode =
		test_bit(EMI_CONH_2ND_CHN_4BANK_MODE, &emi_conh_2nd) ? 1 : 0;
}
					    ...
					    unsigned int bit)
{
	...
	magic = global_emi_cen->a2d_s6s.v1
			.magics[((bit >= 9) & (bit <= 16)) ? (bit - 9) : 0];
 ...
 * the fomular of translation is derived from the implementation of EMI.
 */
static int mtk_emicen_addr2dram_v1(unsigned long addr, struct emi_addr_map *map)
{
	struct a2d_s6s_v1 *s6s;
	...
	} else if (s6s->channels == 1 || s6s->channels == 2) {
		tmp = 0;
		switch (s6s->chnpos) {
		case 0:
			tmp = 7;
	...
	row_mask = (ch_ab_not_cd) ? ((map->rank) ? s6s->chab_row_mask[1] :
						   s6s->chab_row_mask[0]) :
				    ((map->rank) ? s6s->chcd_row_mask[1] :
						   s6s->chcd_row_mask[0]);
	col_mask = (ch_ab_not_cd) ? ((map->rank) ? s6s->chab_col_mask[1] :
						   s6s->chab_col_mask[0]) :
				    ((map->rank) ? s6s->chcd_col_mask[1] :
						   s6s->chcd_col_mask[0]);
...
use_a2d_magic_v2(unsigned long addr, unsigned long magic, unsigned int bit)
{
	unsigned int ret;
 ...
 * the fomular of translation is derived from the implementation of EMI.
 */
static int mtk_emicen_addr2dram_v2(unsigned long addr, struct emi_addr_map *map)
{
	struct a2d_s6s_v2 *s6s;
	unsigned long disph, hash;
	...
	if (!s6s->dual_rank_en)
		...
		if (!s6s->rank_pos)
			map->rank =
				((saddr >> 20) >= s6s->rank0_size_MB) ? 1 : 0;
		else {
			tmp = 1 + s6s->dw32_en;
	...
	if (tmp) {
		...
		map->bank = (saddr >> (1 + s6s->dw32_en + tmp)) %
			    (1 << ((map->rank) ? s6s->rank1_bank_width :
	...
	if (!s6s->rank_pos) {
		...
		tmp = 1 + s6s->dw32_en;
		tmp += (map->rank) ? s6s->rank1_bank_width :
				     s6s->rank0_bank_width;
		tmp += (map->rank) ? s6s->rank1_col_width :
				     s6s->rank0_col_width;
...
static ssize_t emicen_addr2dram_show(struct device_driver *driver, char *buf)
{
	...
	if (!ret)
		return snprintf(
			buf, PAGE_SIZE,
			"0x%lx\n->\nemi: %d\nchn: %d\nrank:%d\nbank:%d\nrow: %d\ncol: %d\n",
			addr, map.emi, map.channel, map.rank, map.bank, map.row,
			map.column);
	else
		return snprintf(buf, PAGE_SIZE, "0x%lx\n->failed\n", addr);
}

static ssize_t emicen_addr2dram_store(struct device_driver *driver,
...
static int emicen_probe(struct platform_device *pdev)
{
	...
	cen = devm_kzalloc(&pdev->dev, sizeof(struct emi_cen), GFP_KERNEL);
	if (!cen)
		return -ENOMEM;

	dev_comp = (struct mtk_emi_compatible *)of_device_get_match_data(
		&pdev->dev);
	cen->ver = (unsigned int)dev_comp->ver;

	ret = of_property_read_u32(emicen_node, "ch_cnt", &(cen->ch_cnt));
	if (ret) {
		dev_err(&pdev->dev, "No ch_cnt\n");
		return -ENXIO;
	}

	ret = of_property_read_u32(emicen_node, "rk_cnt", &(cen->rk_cnt));
	if (ret) {
	...
	cen->rk_size =
		devm_kmalloc_array(&pdev->dev, cen->rk_cnt,
				   sizeof(unsigned long long), GFP_KERNEL);
	if (!(cen->rk_size)) {
		return -ENOMEM;
	}

	ret = of_property_read_u64_array(emicen_node, "rk_size", cen->rk_size,
					 cen->rk_cnt);
	if (ret) {
	...
	ret = of_property_count_elems_of_size(emicen_node, "reg",
	...
	cen->emi_cen_base = devm_kmalloc_array(&pdev->dev, cen->emi_cen_cnt,
	...
	cen->emi_chn_base = devm_kmalloc_array(&pdev->dev, cen->ch_cnt,
					       sizeof(phys_addr_t), GFP_KERNEL);
	if (!(cen->emi_chn_base)) {
		return -ENOMEM;
	}
	...
	ret = of_property_read_u32(emicen_node, "a2d_disph", &(cen->disph));
	if (ret) {
		dev_info(&pdev->dev, "No a2d_disph\n");
		cen->disph = MTK_EMI_DISPATCH;
	}

	ret = of_property_read_u32(emicen_node, "a2d_hash", &(cen->hash));
	if (ret) {
		dev_info(&pdev->dev, "No a2d_hash\n");
		cen->hash = MTK_EMI_HASH;
	}

	ret = of_property_read_u32_array(emicen_node, "a2d_conf_offset",
					 emi_a2d_con_offset,
					 ARRAY_SIZE(emi_a2d_con_offset));
	if (ret)
		dev_info(&pdev->dev, "No a2d_conf_offset\n");

	ret = of_property_read_u32_array(emicen_node, "a2d_chn_conf_offset",
	...
	dev_info(&pdev->dev, "%s(%d) %s(%d), %s(%d)\n", "version", cen->ver,
		 "ch_cnt", cen->ch_cnt, "rk_cnt", cen->rk_cnt);
