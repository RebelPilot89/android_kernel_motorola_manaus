...
#define RDIST_FLAGS_LPI_ENABLED (1 << 2)
...
static int __init its_setup_lpi_prop_table(void)
{
	if (gic_rdists->flags & RDIST_FLAGS_RD_TABLES_PREALLOCATED) {
		...
		gic_rdists->prop_table_va =
			memremap(gic_rdists->prop_table_pa, LPI_PROPBASE_SZ,
				 MEMREMAP_WB);
		...
		lpi_id_bits =
			min_t(u32, GICD_TYPER_ID_BITS(gic_rdists->gicd_typer),
			      ITS_MAX_LPI_NRBITS);
...
static void its_wait_vpt_parse_complete(void)
{
	...
	WARN_ON_ONCE(readq_relaxed_poll_timeout_atomic(
		...
		!(val & GICR_VPENDBASER_Dirty), 10, 500));
}
