	...
	if (enable) {
		if (!c_event) {
			event = perf_event_create_kernel_counter(
				&dsu_cycle_event_attr, 0, NULL, dummy_handler,
				NULL);
			if (IS_ERR(event)) {
...
static int swpm_arm_pmu_enable(int cpu, int enable)
{
	...
	if (enable) {
		...
		if (!i_event && cpu >= boundary) {
			...
			if (IS_ERR(event)) {
				pr_notice(
					"create (%d) inst_spec counter error (%d)\n",
					cpu, (int)PTR_ERR(event));
				goto FAIL;
			}
		...
		if (!c_event && cpu >= boundary) {
			...
			if (IS_ERR(event)) {
				pr_notice(
					"create (%d) cycle counter error (%d)\n",
					cpu, (int)PTR_ERR(event));
				goto FAIL;
			}
...
{
	return (pmu_dsu_support << 24 | boundary << 20 | swpm_arm_pmu_status);
}
...
int __init swpm_arm_pmu_init(void)
{
	...
	if (!node) {
		pr_notice(
			"of_find_compatible_node unable to find swpm device node\n");
		goto END;
	}
	/* device node, device name, offset, variable */
	ret = of_property_read_u32_index(node, "pmu_boundary_num", 0,
					 &boundary);
	if (ret) {
	...
	/* device node, device name, offset, variable */
	ret = of_property_read_u32_index(node, "pmu_dsu_support", 0,
					 &pmu_dsu_support);
	if (ret) {
	...
	/* device node, device name, offset, variable */
	ret = of_property_read_u32_index(node, "pmu_dsu_type", 0,
					 &pmu_dsu_type);
	if (ret) {
