...
/* CRC table for CRC-16. Poly is 0x8005 (x^16 + x^15 + x^2 + 1) */
...
unsigned short osal_crc16(const unsigned char *buffer, unsigned int length)
{
...
int osal_printtimeofday(const unsigned char *prefix)
{
	...
	osal_do_gettimeofday(&tv);
	sec = (int)tv.tv_sec;
	usec = (int)(tv.tv_nsec / NSEC_PER_USEC);
	return osal_dbg_print("%s>sec=%d, usec=%d\n", prefix, sec, usec);
}

void osal_buffer_dump(const unsigned char *buf, const unsigned char *title,
		      unsigned int len, unsigned int limit)
{
	...
	for (k = 0; k < dump_len; k++) {
		if ((k + 1) % 16 != 0) {
			slen = osal_sprintf(p, "%02x ", buf[k]);
			p += slen;
		} else {
			slen = osal_sprintf(p, "%02x\n", buf[k]);
			pr_info("%s", str);
			memset(str, 0, sizeof(str));
			p = str;
		}
...
void osal_buffer_dump_data(const unsigned int *buf, const unsigned char *title,
			   unsigned int len, unsigned int limit, int flag)
{
	int k;
	unsigned int dump_len;
	char str[DBG_LOG_STR_SIZE];
	...
	for (k = 0; k < dump_len; k++) {
		count++;
		if (count % 8 != 0) {
			slen = osal_sprintf(p, "0x%08x,", buf[k]);
			p += slen;
		} else {
			slen = osal_sprintf(p, "0x%08x\n", buf[k]);
...
#define OPQ_DUMP_OP_BUF_SIZE   (12 + 12 + 17 + 10 + 4 + 13 + 10 + \
				(17 * OPQ_DUMP_OPDATA_PER_OP) + 1)
#define OPQ_DUMP_LINE_BUF_SIZE (OPQ_DUMP_OP_BUF_SIZE + 1)

static void _osal_opq_dump(const char *qName, P_OSAL_OP_Q pOpQ)
{
	unsigned int rd, wt, idx = 0, opDataIdx;
	P_OSAL_OP op;
	...
	while (rd != wt && idx < RB_SIZE(pOpQ)) {
		...
		if (op) {
			printed += snprintf(buf + printed,
					    OPQ_DUMP_LINE_BUF_SIZE - printed,
					    "[%u(%u)]%p:%u(%d)(%d)-%u-",
					    idx, (rd & RB_MASK(pOpQ)), op,
					    op->op.opId,
					    atomic_read(&op->ref_count),
					    op->result, op->op.u4InfoBit);
			for (opDataIdx = 0; opDataIdx < OPQ_DUMP_OPDATA_PER_OP;
			     opDataIdx++)
				printed += snprintf(buf + printed,
						    OPQ_DUMP_LINE_BUF_SIZE - printed,
						    "%zx,",
		...
		if (printed >= 1 && printed < (int)(sizeof(buf) - 1)) {
			buf[printed] = 0;
			pr_info("%s\n", buf);
		}
		rd++;
