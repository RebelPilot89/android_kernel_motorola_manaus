...
#include "../debug_utility/conninfra_dbg.h"
#include "consys_hw.h"
...
#include "../../../adaptor/include/conn_adaptor.h"
...
#endif
ssize_t connv2_coredump_emi_read(struct file *filp, char __user *buf,
...
struct wmt_platform_bridge g_plat_bridge = {
	...
	.conninfra_reg_is_bus_hang_cb = conninfra_conn_is_bus_hang,
#endif
#if defined(CONNINFRA_DBG_SUPPORT) && CONNINFRA_DBG_SUPPORT
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	if (count != (size + mcif_emi_size) || end_offset < start_offset) {
		pr_notice(
			"[%s][0x%x,0x%x] connsys emi_size=0x%x, mcif emi size=0x%x but buf_size=0x%x",
			__func__, start_offset, end_offset, size, mcif_emi_size,
			count);
		return 0;
	} else {
		pr_info("[%s] base=[%x] [0x%x~0x%x] size=0x%x", __func__,
			emi_dump_addr, start_offset, end_offset, size);
		emi_virt_addr_base =
			ioremap(emi_dump_addr + start_offset, size);
		...
		if (tmp_buf == NULL) {
			pr_notice("[%s] allocate buf fail, size = %d", __func__,
				  size);
		...
		if (retval != 0)
			pr_notice("[%s] copy to user fail: %d", __func__,
				  retval);
		iounmap(emi_virt_addr_base);
		osal_free(tmp_buf);
		...
		pr_info("[%s] base=[%x] [0x%x~0x%x] size=0x%x", __func__,
			mcif_emi_dump_addr, 0, mcif_emi_size, mcif_emi_size);
		mcif_emi_virt_addr_base =
			ioremap(mcif_emi_dump_addr, mcif_emi_size);
...
#define CONN_DUMP_STATE_BUF_SIZE 1024
	...
	ret = conninfra_core_dump_power_state(tmp_buf,
					      CONN_DUMP_STATE_BUF_SIZE);
	if (ret) {
...
int connv2_drv_init(void)
{
	...
	unsigned int drv_enum_converter[CONNDRV_TYPE_MAX] = {
		CONN_SUPPOPRT_DRV_BT_TYPE_BIT,
		CONN_SUPPOPRT_DRV_FM_TYPE_BIT,
		CONN_SUPPOPRT_DRV_GPS_TYPE_BIT,
		CONN_SUPPOPRT_DRV_WIFI_TYPE_BIT,
		0x0,
		0x0
	};
	int i = 0;
	...
	if (iret)
		...
		while (atomic_read(&g_connv2_hw_init_done) == 0) {
			...
			retry++;
			if (__ratelimit(&_rs))
				pr_info("g_connv2_hw_init_done = 0, retry = %d",
					retry);
		}
	}
	wmt_export_platform_bridge_register(&g_plat_bridge);

	INIT_WORK(&g_conninfra_pmic_work.pmic_work,
		  conninfra_dev_pmic_event_handler);
	...
	pr_info("[%s] radio support convert 0x%x => 0x%x", __func__,
		connv2_radio_support, adaptor_radio_support);

	g_connv2_drv_gen.drv_radio_support = adaptor_radio_support;
	iret = conn_adaptor_register_drv_gen(CONN_ADAPTOR_DRV_GEN_CONNAC_2,
					     &g_connv2_drv_gen);
	if (iret)
