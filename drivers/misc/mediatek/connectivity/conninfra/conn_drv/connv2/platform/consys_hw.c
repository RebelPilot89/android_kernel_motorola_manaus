...
#include "../../../../power_throttling/conn_power_throttling.h"
 ...
 */
static int _consys_hw_raise_voltage(enum consys_drv_type drv_type, bool raise,
				    bool onoff);
...
static unsigned long long
	g_conninfra_wakeup_rec_sec[CONNINFRA_WAKEUP_RECORD_NUM];
...
int consys_hw_pwr_off(unsigned int curr_status, unsigned int off_radio)
{
	...
	if (next_status == 0) {
		...
		if (consys_hw_ops->consys_plt_subsys_status_update)
			consys_hw_ops->consys_plt_subsys_status_update(
				false, off_radio);
...
int _consys_hw_pwr_on_rollback(enum conninfra_pwr_on_rollback_type type)
{
	...
	case CONNINFRA_PWR_ON_CONNINFRA_HW_POWER_FAIL:
		if (consys_hw_ops->consys_plt_conninfra_on_power_ctrl) {
			ret = consys_hw_ops->consys_plt_conninfra_on_power_ctrl(
				0);
			if (ret)
...
int consys_hw_spi_read(enum sys_spi_subsystem subsystem, unsigned int addr,
		       unsigned int *data)
{
	if (consys_hw_ops->consys_plt_spi_read)
		return consys_hw_ops->consys_plt_spi_read(subsystem, addr,
							  data);
	return -1;
}

int consys_hw_spi_write(enum sys_spi_subsystem subsystem, unsigned int addr,
			unsigned int data)
{
	if (consys_hw_ops->consys_plt_spi_write)
		return consys_hw_ops->consys_plt_spi_write(subsystem, addr,
							   data);
	return -1;
}

int consys_hw_spi_update_bits(enum sys_spi_subsystem subsystem,
			      unsigned int addr, unsigned int data,
			      unsigned int mask)
{
	if (consys_hw_ops->consys_plt_spi_update_bits)
		return consys_hw_ops->consys_plt_spi_update_bits(
...
int _consys_hw_raise_voltage(enum consys_drv_type drv_type, bool raise,
			     bool onoff)
{
...
int consys_hw_raise_voltage(enum consys_drv_type drv_type, bool raise,
			    bool onoff)
{
	int ret;
...
static void _consys_hw_conninfra_print_wakeup_record(void)
{
	...
	pr_info("conn_wakeup:%llu.%06lu:%d; %llu.%06lu:%d; %llu.%06lu:%d; %llu.%06lu:%d; %llu.%06lu:%d; %llu.%06lu:%d",
		sec[0], nsec[0], cnt[0], sec[1], nsec[1], cnt[1], sec[2],
		nsec[2], cnt[2], sec[3], nsec[3], cnt[3], sec[4], nsec[4],
		cnt[4], sec[5], nsec[5], cnt[5]);
}
...
static int _consys_hw_conninfra_wakeup(void)
{
	...
	if (consys_hw_ops->consys_plt_conninfra_wakeup) {
		...
		_consys_hw_conninfra_add_wakeup_record(
			g_conninfra_wakeup_ref_cnt);
	}
...
static void _consys_hw_conninfra_sleep(void)
{
	...
	if (g_conninfra_wakeup_ref_cnt < 0) {
		...
		aee_kernel_exception("conninfra", "%s count %d is unexpected.",
				     __func__, g_conninfra_wakeup_ref_cnt);
#else
		pr_notice("%s count %d is unexpected.", __func__,
			  g_conninfra_wakeup_ref_cnt);
#endif
		_consys_hw_conninfra_print_wakeup_record();
	} else
...
int consys_hw_bus_clock_ctrl(enum consys_drv_type drv_type,
...
int get_consys_platform_ops(struct platform_device *pdev)
{
	...
	g_conninfra_plat_data =
		(const struct conninfra_plat_data *)of_device_get_match_data(
			&pdev->dev);
	...
	pr_info("[%s] chipid=[%x] [%x]", __func__,
		g_conninfra_plat_data->chip_id, g_conninfra_plat_data->hw_ops);
	if (consys_hw_ops == NULL)
		consys_hw_ops = (const struct consys_hw_ops_struct *)
...
int consys_hw_tcxo_parser(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(pdev->dev.of_node, "tcxo_support",
				      &tcxo_support);
	if (!ret) {
	...
	if (IS_ERR(g_conninfra_pinctrl_ptr)) {
		...
		if (pinctrl_node) {
			pins_node = of_get_child_by_name(pinctrl_node,
							 "pins_cmd_dat");

			if (pins_node) {
				ret = of_property_read_u32(pins_node, "pinmux",
							   &pinmux);
	...
	pr_info("[%s] tcxo=%d pintctrl=%p", __func__, conn_hw_env.tcxo_support,
		g_conninfra_pinctrl_ptr);
	return 0;
}
...
unsigned int consys_hw_drv_support(struct platform_device *pdev)
{
	...
	if (count <= 0) {
		...
		pr_info("[%s] use default radio: [0x%x]", __func__,
			radio_support);
	 ...
	 * mawd: MAWD
	 */
	ret = of_property_read_string_array(pdev->dev.of_node, "radio-support",
					    drv_list, CONNDRV_TYPE_MAX);
	for (i = 0; i < count; i++) {
		   ...
		   struct conninfra_dev_cb *dev_cb)
{
	...
	if (emi_info) {
		...
		connectivity_export_conap_scp_init(
			consys_hw_get_ic_info(CONNSYS_SOC_CHIPID),
			(phys_addr_t)emi_info->emi_ap_phy_addr);
	} else {
	...
	coredump_mng_init(
		(void *)(g_conninfra_plat_data->platform_coredump_ops));
