...
#if defined(MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE) &&                       \
	MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE
...
void consys_set_if_pinmux_mt6886(unsigned int enable)
{
	...
	if (enable) {
		...
		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			...
			tcxo_pinctrl_set = pinctrl_lookup_state(
				g_conninfra_pinctrl_ptr, "conninfra_tcxo_set");
			if (IS_ERR(tcxo_pinctrl_set))
				return;

			ret = pinctrl_select_state(g_conninfra_pinctrl_ptr,
						   tcxo_pinctrl_set);
			if (ret)
		...
		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			...
			tcxo_pinctrl_clr = pinctrl_lookup_state(
				g_conninfra_pinctrl_ptr, "conninfra_tcxo_clr");
...
int consys_reg_init_mt6886(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			if (ret) {
				pr_err("Get Reg Index(%d-%s) failed", i,
				       consys_base_addr_index_to_str[i]);
				continue;
			}
			...
			pr_info("Get Index(%d-%s) phy(0x%zx) baseAddr=(0x%zx) size=(0x%zx)",
				...
				base_addr->phy_addr, base_addr->vir_addr,
				base_addr->size);
		}
...
int consys_reg_deinit_mt6886(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (conn_reg_mt6886.reg_base_addr[i].vir_addr) {
			pr_info("[%d] Unmap %s (0x%zx)", i,
				consys_base_addr_index_to_str[i],
				conn_reg_mt6886.reg_base_addr[i].vir_addr);
			iounmap((void __iomem *)conn_reg_mt6886.reg_base_addr[i]
					.vir_addr);
