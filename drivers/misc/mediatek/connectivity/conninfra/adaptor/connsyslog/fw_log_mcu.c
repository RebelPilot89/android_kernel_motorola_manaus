...
int fw_log_mcu_init(struct fw_log_mcu_info *info,
		    ...
		    const struct file_operations *ops)
{
	...
	info->mcu_dev = device_create(info->mcu_class, NULL, info->dev_id, NULL,
				      info->driver_name);
	...
	pr_info("[%s] conn_type=[%d] major=[%d] init done\n", __func__,
		info->conn_type, MAJOR(info->dev_id));
