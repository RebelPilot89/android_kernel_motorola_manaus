...
#define CONNINFRA_IOCTL_SET_COREDUMP_MODE                                      \
	_IOW(CONNINFRA_DEV_IOC_MAGIC, 1, unsigned int)
...
static struct conn_adaptor_drv_func_support
	g_drv_func_support[CONN_ADAPTOR_DRV_SIZE];
...
static ssize_t conn_adaptor_dev_write(struct file *filp, const char __user *buf,
				      size_t count, loff_t *f_pos);
static long conn_adaptor_dev_unlocked_ioctl(struct file *filp, unsigned int cmd,
					    unsigned long arg);
#ifdef CONFIG_COMPAT
static long conn_adaptor_dev_compat_ioctl(struct file *filp, unsigned int cmd,
					  unsigned long arg);
#endif
...
static atomic_t g_es_lr_flag_for_blank =
	ATOMIC_INIT(0); /* for ctrl blank flag */
...
static void conn_adaptor_set_coredump_mode(int mode)
{
	...
	for (i = 0; i < CONN_ADAPTOR_DRV_SIZE; i++) {
		if (atomic_read(&g_drv_gen_inst[i].enable) &&
			...
			(*(g_drv_gen_inst[i].drv_gen_cb.set_coredump_mode))(
				mode);
	}
}
...
int conn_adaptor_dev_open(struct inode *inode, struct file *file)
{
	...
	if (!wait_event_timeout(g_conn_adaptor_init_wq,
				g_conn_adaptor_init_status ==
					CONN_ADAPTOR_INIT_DONE,
				msecs_to_jiffies(CONNINFRA_DEV_INIT_TO_MS))) {
		if (__ratelimit(&_rs)) {
			pr_warn("wait_event_timeout (%d)ms,(%lu)jiffies,return -EIO\n",
				CONNINFRA_DEV_INIT_TO_MS,
				msecs_to_jiffies(CONNINFRA_DEV_INIT_TO_MS));
		}
		return -EIO;
	}
...
ssize_t conn_adaptor_dev_read(struct file *filp, char __user *buf, size_t count,
			      loff_t *f_pos)
{
	if (atomic_read(
		    &g_drv_gen_inst[CONN_ADAPTOR_DRV_GEN_CONNAC_2].enable) &&
	    g_drv_gen_inst[CONN_ADAPTOR_DRV_GEN_CONNAC_2]
		    .drv_gen_cb.coredump_emi_read)
		return (*(g_drv_gen_inst[CONN_ADAPTOR_DRV_GEN_CONNAC_2]
				  .drv_gen_cb.coredump_emi_read))(filp, buf,
								  count, f_pos);

	return 0;
}

ssize_t conn_adaptor_dev_write(struct file *filp, const char __user *buf,
			       size_t count, loff_t *f_pos)
{
	return 0;
}

static const char *g_adp_drv_name[CONN_ADAPTOR_DRV_SIZE] = { "WIFI", "BT",
							     "GPS", "FM" };
					    ...
					    unsigned long arg)
{
	...
	case CONNINFRA_IOCTL_GET_ADIE_CHIP_ID:
		if (arg >= CONN_ADAPTOR_DRV_SIZE)
			pr_notice(
				"[%s][CONNINFRA_IOCTL_GET_ADIE_CHIP_ID] invalid input: %lu",
				__func__, arg);
		else {
			retval = conn_adaptor_detect_adie_chipid(arg);
			pr_info("[%s] get adie [%s]=[0x%04x]", __func__,
				g_adp_drv_name[arg], retval);
		}
		break;
	}
...
static void conn_adaptor_dev_pwr_on_off_handler(struct work_struct *work)
{
	...
	for (i = 0; i < CONN_ADAPTOR_DRV_SIZE; i++) {
		if (atomic_read(&g_drv_gen_inst[i].enable) &&
			...
			(*(g_drv_gen_inst[i].drv_gen_cb.power_on_off_notify))(
				conn_adaptor_dev_get_disp_blank_state());
	}
}
	...
	case 0x40:
		for (i = 0; i < CONN_ADAPTOR_DRV_SIZE; i++) {
			if (atomic_read(&g_drv_gen_inst[i].enable) &&
				...
				sz = (*(g_drv_gen_inst[i]
						.drv_gen_cb.dump_power_state))(
					buf + offset, buf_sz - offset);
				if (sz > 0)
					offset += sz;
				if (offset >= buf_sz) {
...
int conn_adaptor_register_drv_gen(enum conn_adaptor_drv_gen drv_gen,
				  struct conn_adaptor_drv_gen_cb *cb)
{
	int i;
...
int conn_adaptor_unregister_drv_gen(enum conn_adaptor_drv_gen drv_gen)
{
	...
	memset(&g_drv_gen_inst[drv_gen].drv_gen_cb, 0,
	       sizeof(struct conn_adaptor_drv_gen_cb));
...
static int conn_adaptor_dev_do_drv_init(void)
{
	...
	memset(&g_drv_func_support, 0,
	       CONN_ADAPTOR_DRV_SIZE *
		       sizeof(struct conn_adaptor_drv_func_support));
	...
	iret = mtk_disp_notifier_register("conninfra_driver",
					  &conn_adaptor_fb_notifier);
#endif
#else
	...
	conn_adaptor_pm_notifier.notifier_call =
		conn_adaptor_pm_notifier_callback;
