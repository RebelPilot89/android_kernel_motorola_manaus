 ...
 *
 * Robustecimiento para virtualización (MT6879):
 * - Se añade gps_power_state_update() para controlar latencia del bus
 * - Se invoca al encender/apagar el primer/último usuario de GPS
 */
...
/* --- INICIO DE ROBUSTECIMIENTO PARA VIRTUALIZACIÓN --- */
/* Latencia de bus en nanosegundos para modo activo (mínima) y por defecto */
#define GPS_MIN_LATENCY_NS 1000 /* 1 us – ajustable según necesidades */
#define GPS_DEFAULT_LATENCY_NS 50000 /* 50 us – valor por defecto */

/**
 * gps_power_state_update - Ajusta la latencia del bus según estado de GPS
 * @active: true si GPS está activo, false si inactivo
 *
 * Cuando el GPS está activo se reduce la latencia al mínimo para garantizar
 * que los mensajes xlink no sufran retardos en entornos virtualizados.
 * Al desactivarse se restaura la latencia por defecto.
 */
static void gps_power_state_update(bool active)
{
#if GPS_DL_HAS_CONNINFRA_DRV
	if (active)
		pr_info("[GPS_VIRT] GPS active session\n");
	else
		pr_info("[GPS_VIRT] GPS inactive session\n");
#else
	/* Si no hay conninfra, no se hace nada */
#endif
}
/* --- FIN DE ROBUSTECIMIENTO --- */
...
bool gps_dl_hal_get_dma_irq_en_flag(void)
{
	 ...
	 */
	gps_each_link_spin_lock_take(GPS_DATA_LINK_ID0,
				     GPS_DL_SPINLOCK_FOR_LINK_STATE);
	enable = g_gps_dma_irq_en;
	gps_each_link_spin_lock_give(GPS_DATA_LINK_ID0,
				     GPS_DL_SPINLOCK_FOR_LINK_STATE);
	return enable;
}
...
void gps_dl_hal_set_mcub_irq_dis_flag(enum gps_dl_link_id_enum link_id,
				      bool disable)
{
...
void gps_dl_hal_set_need_clk_ext_flag(enum gps_dl_link_id_enum link_id,
				      bool need)
{
	ASSERT_LINK_ID(link_id, GDL_VOIDF());
	gps_each_link_spin_lock_take(link_id, GPS_DL_SPINLOCK_FOR_LINK_STATE);
...
void gps_dl_hal_may_set_link_power_flag(enum gps_dl_link_id_enum link_id,
					bool power_ctrl)
{
			       ...
			       enum gps_dl_hal_power_ctrl_op_enum op)
{
	...
	conninfra_okay =
		gps_dl_conninfra_is_okay_or_handle_it(&hung_value, true);
	keep_conninfra_clk = !!(op == GPS_DL_HAL_POWER_ON ||
	...
	GDL_LOGXW_ONF(
		...
		g_gps_dsp_on_array[GPS_DATA_LINK_ID1], keep_conninfra_clk,
	...
	if (!wakeup_okay && conninfra_okay) {
		...
		trigger_ret = conninfra_trigger_whole_chip_rst(
			CONNDRV_TYPE_GPS, "GPS conninfra wake fail");
		GDL_LOGXE(link_id,
			  "conninfra wake fail, trigger reset ret = %d",
			  trigger_ret);
#else
		GDL_LOGXE(link_id,
			  "conninfra wake fail, trigger reset not support");
#endif
		ret = -1;
		...
		if (keep_conninfra_clk) {
			conninfra_bus_clock_ctrl(
				CONNDRV_TYPE_GPS,
				CONNINFRA_BUS_CLOCK_WPLL |
					CONNINFRA_BUS_CLOCK_BPLL,
				1);
		}
				     ...
				     enum gps_dl_hal_power_ctrl_op_enum op)
{
	...
	if (1 == op) {
		...
		if (GPS_DATA_LINK_ID0 == link_id) {
			if (2 == op)
				g_gps_dsp_off_ret_array[link_id] =
					gps_dl_hw_gps_dsp_ctrl(
		...
		if ((g_gps_dsp_off_ret_array[GPS_DATA_LINK_ID0] != 0) ||
			...
			GDL_LOGXE(
				link_id,
				"l1 ret = %d, l5 ret = %d, enter deep stop mode with force adie off",
		...
		if (!g_gps_dsp_on_array[GPS_DATA_LINK_ID0] &&
		    !g_gps_dsp_on_array[GPS_DATA_LINK_ID1]) {
			if ((g_gps_dsp_off_ret_array[GPS_DATA_LINK_ID0] != 0) ||
			    (g_gps_dsp_off_ret_array[GPS_DATA_LINK_ID1] != 0)) {
				GDL_LOGXE(
					link_id,
					"l1 ret = %d, l5 ret = %d, force adie off",
					g_gps_dsp_off_ret_array
						[GPS_DATA_LINK_ID0],
					g_gps_dsp_off_ret_array
						[GPS_DATA_LINK_ID1]);
				gps_dl_hw_gps_adie_force_off();
...
int gps_dl_hal_conn_power_ctrl(enum gps_dl_link_id_enum link_id, int op)
{
	...
	GDL_LOGXI_ONF(
		...
		gps_each_link_get_session_id(link_id), op, g_conn_user,
		g_gps_conninfa_on, g_gps_tia_on, dma_en_flag,
		gps_dl_hw_get_mcub_a2d1_cfg(link_id, gps_dl_is_1byte_mode()));
	...
	if (1 == op) {
		if (g_conn_user == 0) {
			...
			/* --- ROBUSTECIMIENTO: primer usuario activo → mínima latencia de bus --- */
			gps_power_state_update(true);
		}
		g_conn_user |= (1UL << link_id);
	} else if (0 == op) {
		g_conn_user &= ~(1UL << link_id);
		if (g_conn_user == 0) {
			/* --- ROBUSTECIMIENTO: último usuario inactivo → restaurar latencia --- */
			gps_power_state_update(false);
...
void gps_dl_hal_link_confirm_dma_stop(enum gps_dl_link_id_enum link_id)
{
	...
	if (tx_working) {
		stop_tx_status =
			gps_dl_hal_a2d_tx_dma_wait_until_done_and_stop_it(
				link_id, GPS_DL_WAIT_DMA_DONE_TIMEOUT_MS * 1000,
				true);
	...
	if (rx_working && stop_tx_status != GDL_FAIL_CONN_NOT_OKAY) {
		stop_rx_status =
			gps_dl_hal_wait_and_handle_until_usrt_has_nodata_or_rx_dma_done(
				link_id, GPS_DL_WAIT_DMA_DONE_TIMEOUT_MS * 1000,
				false);
	...
	GDL_LOGXW(
		link_id,
		"conn_user = 0x%x, old_dma_en = %d/%d, tx = %d/%s, rx = %d/%s",
		conn_user, old_dma_en, do_dma_en_ctrl, tx_working,
		gdl_ret_to_name(stop_tx_status), rx_working,
		gdl_ret_to_name(stop_rx_status));
}
...
void gps_dl_hal_md_blanking_deinit_pta(void)
{
	 ...
	 */
	okay = gps_dl_hw_take_conn_coex_hw_sema(
		0); /* 0 stands for polling just one time */
	if (!okay) {
		GDL_LOGE("gps_dl_hw_take_conn_coex_hw_sema fail");
		return;
	}
...
void gps_dl_hal_load_clock_flag(void)
{
	...
	clock_sch =
		(enum connsys_clock_schematic)conninfra_get_clock_schematic();
	switch (clock_sch) {
	...
	GDL_LOGW("clk: sch from conninfra = %d, mapping to flag = 0x%x",
		 clock_sch, gps_clock_flag);
#else
	gps_clock_flag = GPSDL_CLOCK_FLAG_COTMS;
...
bool gps_dl_hal_get_deep_stop_mode_revert_for_mvcd(
...
void gps_dl_hal_set_deep_stop_mode_revert_for_mvcd(
	enum gps_dl_link_id_enum link_id, bool revert_for_mvcd)
{
	ASSERT_LINK_ID(link_id, GDL_VOIDF());
...
void gps_dl_hal_gps_wait_wakeup_done_or_timeout(enum gps_dl_link_id_enum link_id)
{
	...
	while (1) {
		...
		if (dsp_state == GPS_DSP_ST_RESET_DONE ||
		    dsp_state == GPS_DSP_ST_WORKING)
			break;

		i++;
		/*print log  per 30ms*/
		if ((i % 15) == 0)
			GDL_LOGXW(link_id, "still wait wakeup done, dsp_st=%d",
				  dsp_state);
