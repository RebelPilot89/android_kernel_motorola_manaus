 ...
 */
#if GPS_DL_ON_CTP
#define GPS_DL_HOST_REG_WR(host_addr, val)  do { \
	/* Use mb to make sure write value is done by CPU.  */\
	(*(conn_reg *)host_addr) = (val)); mb(); \
	} while (0)
#define GPS_DL_HOST_REG_RD(host_addr)       (\
	/* Use mb to make sure write value is done by CPU.	*/\
	{mb(); (*(conn_reg *)host_addr); })
#else
...
#define POLL_INTERVAL_US (10 * 100)
#define POLL_US      (1)
#define POLL_1_TIME  (0)
#define POLL_FOREVER (-1)
#define POLL_DEFAULT (20 * 1000 * POLL_US)
