...
#if GPS_DL_ON_LINUX
extern int mtk_dvfsrc_dynamic_opp0(int, int) __attribute__((weak));
#ifndef VCOREOPP_GPS
#define VCOREOPP_GPS 0
#endif
#endif
...
{
	if (mtk_dvfsrc_dynamic_opp0)
		mtk_dvfsrc_dynamic_opp0(VCOREOPP_GPS, deny_opp0);
}

void gps_mcudl_set_opp_vote_phase(enum gps_mcudl_hal_opp_vote_phase phase,
				  bool is_in)
{
...
void gps_mcudl_vote_to_deny_opp0_for_coinninfra_on(bool vote)
{
	...
	if (vote) {
		if (gps_mcudl_is_voting_for_coinninfra_on) {
			d_us = gps_dl_tick_get_us() -
			       gps_mcudl_vote_for_coinninfra_on_us;
...
bool gps_mcudl_hw_conn_force_wake(bool enable)
{
	...
	if (vote_bitmask == 0) {
		...
		if (conn_on_vote) {
			...
			d_us = gps_dl_tick_get_us() -
			       gps_mcudl_vote_for_coinninfra_on_us;
			gps_mcudl_is_voting_for_coinninfra_on = false;
			GDL_LOGE("abnormal vote end-2, d_us=%lu, w_ok=%d", d_us,
				 wake_okay);
		}
		return wake_okay;
	}
	...
	GDL_LOGW("vote end: d_us=%lu, bitmask=0x%08x, is_cv=%d, w_ok=%d", d_us,
		 vote_bitmask, conn_on_vote, wake_okay);
	return wake_okay;
}
