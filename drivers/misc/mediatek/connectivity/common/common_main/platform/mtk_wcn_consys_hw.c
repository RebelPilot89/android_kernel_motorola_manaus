...
static INT32 wmt_thermal_register(struct platform_device *pdev)
{
	...
	tz = devm_thermal_zone_of_sensor_register(&pdev->dev, 0, NULL,
						  &tz_wmt_thermal_ops);
...
static INT32 mtk_wmt_probe(struct platform_device *pdev)
{
	...
	if (gConEmiPhyBase) {
		...
		if (wmt_consys_ic_ops->consys_ic_emi_coredump_remapping)
			wmt_consys_ic_ops->consys_ic_emi_coredump_remapping(
				&pEmibaseaddr, 1);
#ifdef CONFIG_MTK_CONNSYS_DEDICATED_LOG_PATH
		if (wmt_consys_ic_ops->consys_ic_dedicated_log_path_init)
			wmt_consys_ic_ops->consys_ic_dedicated_log_path_init(
				pdev);
#endif
	} else {
		WMT_PLAT_PR_ERR(
	...
	WMT_PLAT_PR_INFO(
		"register connsys restore cb for complying with IPOH function\n");
	register_swsusp_restore_noirq_func(ID_M_CONNSYS,
					   mtk_wcn_consys_hw_restore, NULL);
#endif
	...
	if (consys_pinctrl) {
		pinctl_node =
			of_parse_phandle(pdev->dev.of_node, "pinctrl-1", 0);
		if (pinctl_node) {
			pins_node = of_get_child_by_name(pinctl_node,
							 "pins_cmd_dat");
			if (pins_node) {
				pin_ret = of_property_read_u32(
					pins_node, "pinmux", &pinmux);
				if (pin_ret)
					pin_ret = of_property_read_u32(
						pins_node, "pins", &pinmux);
				gps_lna_pin_num = (pinmux >> 8) & 0xff;
				WMT_PLAT_PR_INFO(
					"GPS LNA gpio pin number:%d, pinmux:0x%08x.\n",
	...
	wifi_ant_swap_gpio_pin_num =
		of_get_named_gpio(pdev->dev.of_node, "wifi_ant_swap_gpio", 0);
...
static void plat_resume_handler(struct work_struct *work)
{
	...
	if (wmt_lib_dmp_consys_state(&dmp_info, dmp_cnt, 0) ==
	    MTK_WCN_BOOL_TRUE) {
		len += snprintf(dmp_info_buf + len, DBG_LOG_STR_SIZE - len,
				"0x%08x", dmp_info.cpu_pcr[0]);

		for (i = 1; i < dmp_cnt; i++)
			len += snprintf(dmp_info_buf + len,
					DBG_LOG_STR_SIZE - len, ";0x%08x",
					dmp_info.cpu_pcr[i]);

		len += snprintf(dmp_info_buf + len, DBG_LOG_STR_SIZE - len,
				";0x%08x", dmp_info.state.lp[1]);

		len += snprintf(dmp_info_buf + len, DBG_LOG_STR_SIZE - len,
				";0x%08x", dmp_info.state.gating[1]);

		len += snprintf(dmp_info_buf + len, DBG_LOG_STR_SIZE - len,
				";%llu", dmp_info.state.sleep_counter[0]);
		for (i = 1; i < WMT_SLEEP_COUNT_MAX; i++) {
			len += snprintf(dmp_info_buf + len,
					DBG_LOG_STR_SIZE - len, ",%llu",
					dmp_info.state.sleep_counter[i]);
		}

		len += snprintf(dmp_info_buf + len, DBG_LOG_STR_SIZE - len,
				";%llu", dmp_info.state.sleep_timer[0]);
		for (i = 1; i < WMT_SLEEP_COUNT_MAX; i++) {
			len += snprintf(dmp_info_buf + len,
					DBG_LOG_STR_SIZE - len, ",%llu",
					dmp_info.state.sleep_timer[i]);
		}
		...
		if ((wmt_lib_get_drv_status(WMTDRV_TYPE_WMT) !=
			...
			WMT_PLAT_PR_INFO(
				"TOP:0,0;MCU:0,0;BT:0,0;WIFI:0,0;GPS:0,0\n");
		}
	}
}
	...
	if (wmt_consys_ic_ops->consys_ic_sleep_info_enable_ctrl &&
		...
		if (atomic64_read(&g_sleep_counter_enable) == 1) {
			for (i = 0; i < WMT_SLEEP_COUNT_MAX; i++) {
				...
				sleep_timer = 0;
				wmt_consys_ic_ops
					->consys_ic_sleep_info_read_ctrl(
						i, &sleep_counter,
						&sleep_timer);
				if (state) {
...
INT32 mtk_wcn_consys_sleep_info_restore(VOID)
{
	...
	if (NULL != wmt_consys_ic_ops &&
		...
		if ((wmt_lib_get_drv_status(WMTDRV_TYPE_WMT) ==
			...
			if (atomic64_read(&g_sleep_counter_enable) == 1)
				wmt_consys_ic_ops
					->consys_ic_sleep_info_enable_ctrl(1);
			osal_unlock_unsleepable_lock(&g_sleep_counter_spinlock);
		}
	}
...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on, UINT32 co_clock_type)
{
	...
	if (on) {
		...
		if (wmt_consys_ic_ops->consys_ic_reset_emi_coredump)
			wmt_consys_ic_ops->consys_ic_reset_emi_coredump(
				pEmibaseaddr);
		...
		if (co_clock_type) {
			...
			if (wmt_consys_ic_ops->consys_ic_clock_buffer_ctrl)
				wmt_consys_ic_ops->consys_ic_clock_buffer_ctrl(
					ENABLE);
		}
		...
		if (co_clock_type) {
			...
			if (wmt_consys_ic_ops->consys_ic_vcn28_hw_mode_ctrl)
				wmt_consys_ic_ops->consys_ic_vcn28_hw_mode_ctrl(
					DISABLE);
		} else {
			/*if NOT co-clock: */
			...
			if (wmt_consys_ic_ops->consys_ic_hw_vcn28_ctrl)
				wmt_consys_ic_ops->consys_ic_hw_vcn28_ctrl(
					ENABLE);
		}
		...
		if (wmt_consys_ic_ops->consys_ic_wifi_ctrl_switch_conn_mode)
			wmt_consys_ic_ops
				->consys_ic_wifi_ctrl_switch_conn_mode();
...
INT32 mtk_wcn_consys_hw_efuse_paldo_ctrl(UINT32 enable, UINT32 co_clock_type)
{
	if (co_clock_type) {
		...
		if (enable)
			...
			WMT_PLAT_PR_INFO(
				"turn off vcn28 for efuse usage in co-clock mode\n");
	}
	return 0;
}
...
INT32 mtk_wcn_consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	if (enable)
		WMT_PLAT_PR_INFO(
			"turn on vcn28 for fm/gps usage in co-clock mode\n");
	else
		WMT_PLAT_PR_INFO(
			"turn off vcn28 for fm/gps usage in co-clock mode\n");
	return 0;
}
...
INT32 mtk_wcn_consys_detect_adie_chipid(UINT32 co_clock_type)
{
	...
	if (wmt_consys_ic_ops &&
	    wmt_consys_ic_ops->consys_ic_adie_chipid_detect) {
		...
		if (chipid > 0) {
			...
			if (wmt_consys_ic_ops->consys_ic_emi_set_remapping_reg)
				wmt_consys_ic_ops
					->consys_ic_emi_set_remapping_reg();
		} else
	...
	if (on) {
		if (wmt_consys_ic_ops->consys_ic_need_gps) {
			if (wmt_consys_ic_ops->consys_ic_need_gps() ==
				...
				iRet += wmt_plat_gpio_ctrl(PIN_I2S_GRP,
							   PIN_STA_INIT);
			}
		} else {
		...
		WMT_PLAT_PR_DBG(
			"CONSYS-HW, BGF IRQ unregistered and disabled\n");
...
INT32 mtk_wcn_consys_hw_pwr_on(UINT32 co_clock_type)
{
	...
	if (!gConEmiPhyBase) {
		WMT_PLAT_PR_ERR(
			"EMI base address is invalid, CONNSYS can not be powered on!");
...
INT32 mtk_wcn_consys_hw_rst(UINT32 co_clock_type)
{
	...
	WMT_PLAT_PR_INFO(
		"CONSYS-HW, hw_rst start, eirq should be disabled before this step\n");
	...
	WMT_PLAT_PR_INFO(
		"CONSYS-HW, hw_rst finish, eirq should be enabled after this step\n");
	return iRet;
}
...
INT32 mtk_wcn_consys_set_dbg_mode(UINT32 flag)
{
	...
	vir_addr = mtk_wcn_consys_emi_virt_addr_get(
		EXP_APMEM_CTRL_CHIP_FW_DBGLOG_MODE);
	if (!vir_addr) {
...
INT32 mtk_wcn_consys_set_dynamic_dump(PUINT32 str_buf)
{
	...
	vir_addr = mtk_wcn_consys_emi_virt_addr_get(
		EXP_APMEM_CTRL_CHIP_DYNAMIC_DUMP);
	if (!vir_addr) {
		WMT_PLAT_PR_ERR("get vir address fail\n");
		return -2;
	}
...
INT32 mtk_wcn_consys_poll_cpucpr_dump(UINT32 times, UINT32 sleep_ms)
{
	if (wmt_consys_ic_ops->consys_ic_poll_cpupcr_dump)
		return wmt_consys_ic_ops->consys_ic_poll_cpupcr_dump(times,
								     sleep_ms);
	return 0;
}
...
INT32 mtk_wcn_consys_read_irq_info_from_dts(PINT32 irq_num, PUINT32 irq_flag)
{
	if (wmt_consys_ic_ops->consys_ic_read_irq_info_from_dts)
		return wmt_consys_ic_ops->consys_ic_read_irq_info_from_dts(
			g_pdev, irq_num, irq_flag);
	else
		return 0;
}
...
INT32 mtk_wcn_consys_reg_ctrl(UINT32 is_write,
			      enum CONSYS_BASE_ADDRESS_INDEX index,
			      UINT32 offset, PUINT32 value)
{
	INT32 iRet = -1;
	...
	if (node) {
		if (of_property_read_u32_array(node, "reg", reg_info,
					       reg_info_size)) {
	...
	if (reg_info[index * 4 + 3] < offset) {
		WMT_PLAT_PR_ERR(
			"Access overflow of address(0x%x), offset(0x%x)!\n",
...
INT32 mtk_consys_is_ant_swap_enable_by_hwid(VOID)
{
	if (wmt_consys_ic_ops->consys_ic_is_ant_swap_enable_by_hwid)
		return wmt_consys_ic_ops->consys_ic_is_ant_swap_enable_by_hwid(
			wifi_ant_swap_gpio_pin_num);
	else
		return 0;
}
...
VOID mtk_wcn_consys_ic_get_ant_sel_cr_addr(PUINT32 default_invert_cr,
					   PUINT32 default_invert_bit)
{
	if (wmt_consys_ic_ops->consys_ic_get_ant_sel_cr_addr)
		wmt_consys_ic_ops->consys_ic_get_ant_sel_cr_addr(
			default_invert_cr, default_invert_bit);
}
...
VOID mtk_wcn_consys_set_vcn33_1_voltage(UINT32 voltage)
{
	...
	if (wmt_consys_ic_ops &&
	    wmt_consys_ic_ops->consys_ic_set_vcn33_1_voltage)
...
INT32 mtk_wcn_consys_get_debug_reg_ary_size(VOID)
{
	...
	if (wmt_consys_ic_ops &&
	    wmt_consys_ic_ops->consys_ic_get_debug_reg_ary_size)
