...
#if !defined(CONFIG_MTK_CONNECTIVITY_COMMON)
/*
 * Vendor-local implementation is compiled only when in-tree
 * MTK_CONNECTIVITY_COMMON is NOT enabled. This avoids duplicate
 * symbol definitions when the canonical `common/` implementation
 * is present in-tree.
 */

static MTK_WCN_BOOL mtk_wcn_wmt_pwr_on(VOID);
static MTK_WCN_BOOL mtk_wcn_wmt_func_ctrl(ENUM_WMTDRV_TYPE_T type,
                                          ENUM_WMT_OPID_T opId);
static MTK_WCN_BOOL mtk_wmt_gps_suspend_ctrl_by_type(MTK_WCN_BOOL gps_l1,
                                                     MTK_WCN_BOOL gps_l5,
                                                     MTK_WCN_BOOL suspend);
...
static MTK_WCN_BOOL mtk_wcn_wmt_func_ctrl(ENUM_WMTDRV_TYPE_T type,
  ...
  if (type == WMTDRV_TYPE_WIFI)
    ...
    pSignal->timeoutValue = (pOp->op.opId == WMT_OPID_FUNC_ON)
                                ? MAX_FUNC_ON_TIME
                                : MAX_FUNC_OFF_TIME;
...
INT32 mtk_wcn_wmt_psm_ctrl(MTK_WCN_BOOL flag) { return -EFAULT; }
...
INT8 mtk_wcn_wmt_therm_ctrl(ENUM_WMTTHERM_TYPE_T eType) {
  ...
  if (bRet == MTK_WCN_BOOL_FALSE) {
    ...
    pOpData->au4OpData[1] =
        (eType == WMTTHERM_READ) ? 0xFF : MTK_WCN_BOOL_FALSE;
  } else
    WMT_DBG_FUNC("OPID(%d) type(%zu) return(%zu) ok\n\n", pOpData->opId,
                 pOpData->au4OpData[0], pOpData->au4OpData[1]);
...
UINT32 mtk_wcn_wmt_adie_workable(VOID) { return wmt_lib_get_adie_workable(); }
...
MTK_WCN_BOOL mtk_wcn_wmt_assert_timeout(ENUM_WMTDRV_TYPE_T type, UINT32 reason,
                                        INT32 timeout) {
  MTK_WCN_BOOL bRet;
...
MTK_WCN_BOOL mtk_wcn_wmt_assert_keyword(ENUM_WMTDRV_TYPE_T type,
                                        PUINT8 keyword) {
  MTK_WCN_BOOL bRet;
 ...
 */
ENUM_WMT_FLASH_PATCH_STATUS
mtk_wcn_wmt_flash_patch_ctrl(ENUM_WMT_FLASH_PATCH_CTRL ctrlId, PUINT8 pBuf,
                             UINT32 length, ENUM_WMT_FLASH_PATCH_SEQ seq,
                             ENUM_WMT_FLASH_PATCH_TYPE type, PUINT32 version,
                             UINT32 checksum) {
  ...
  case WMT_FLASH_PATCH_DOWNLOAD:
    if ((pBuf == NULL) || (length <= 0) || (length > 1000)) {
      WMT_ERR_FUNC(
          "error parameter detected, ctrlId:%d, pBuf:%p,length(0x%x).\n",
          ctrlId, pBuf, length);
  ...
  pSignal = &pOp->signal;
  pSignal->timeoutValue = (ctrlId == WMT_FLASH_PATCH_DOWNLOAD)
                              ? MAX_FUNC_ON_TIME
                              : MAX_EACH_WMT_CMD;

  pOp->op.opId = (ctrlId == WMT_FLASH_PATCH_DOWNLOAD)
                     ? WMT_OPID_FLASH_PATCH_DOWN
                     : WMT_OPID_FLASH_PATCH_VER_GET;
  ...
  WMT_INFO_FUNC("CMD_TEST, opid (%d), ret(%d),retVal(%zu) result(%s)\n",
                pOp->op.opId, bRet, pOp->op.au4OpData[6],
  ...
  if (bRet == MTK_WCN_BOOL_FALSE) {
    ...
    case WMT_FLASH_PATCH_VERSION_GET:
      ...
      break;
    case WMT_FLASH_PATCH_DOWNLOAD:
      eRet = (pOp->op.au4OpData[6] == 0) ? WMT_FLASH_PATCH_DOWNLOAD_OK
                                         : WMT_FLASH_PATCH_DOWNLOAD_FAIL;
      break;
    default:
      WMT_ERR_FUNC("error ctrlId:%d detected.\n", ctrlId);
      eRet = WMT_FLASH_PATCH_PARA_ERR;
...
INT8 mtk_wcn_wmt_co_clock_flag_get(void) { return wmt_lib_co_clock_get(); }
...
INT32 mtk_wcn_wmt_wlan_reg(P_MTK_WCN_WMT_WLAN_CB_INFO pWmtWlanCbInfo) {
  ...
  mtk_wcn_wlan_emi_mpu_set_protection =
      pWmtWlanCbInfo->wlan_emi_mpu_set_protection_cb;
 ...
 */
ENUM_WMT_ANT_RAM_STATUS mtk_wcn_wmt_ant_ram_ctrl(ENUM_WMT_ANT_RAM_CTRL ctrlId,
                                                 PUINT8 pBuf, UINT32 length,
                                                 ENUM_WMT_ANT_RAM_SEQ seq) {
  ...
  if ((ctrlId == WMT_ANT_RAM_DOWNLOAD) &&
      ((pBuf == NULL) || (length <= 0) || (length > 1000) ||
       (seq >= WMT_ANT_RAM_SEQ_MAX) || (seq < WMT_ANT_RAM_START_PKT))) {
    eRet = WMT_ANT_RAM_PARA_ERR;
    WMT_ERR_FUNC(
  ...
  pOp->op.opId = (ctrlId == WMT_ANT_RAM_DOWNLOAD) ? WMT_OPID_ANT_RAM_DOWN
                                                  : WMT_OPID_ANT_RAM_STA_GET;
  pOp->op.au4OpData[0] = (size_t)pBuf;
  pOp->op.au4OpData[1] = length;
  pOp->op.au4OpData[2] = seq;
  ...
  WMT_INFO_FUNC("CMD_TEST, opid (%d), ret(%d),retVal(%zu) result(%s)\n",
                pOp->op.opId, bRet, pOp->op.au4OpData[2],
                MTK_WCN_BOOL_FALSE == bRet ? "failed" : "succeed");
  ...
  if (bRet == MTK_WCN_BOOL_FALSE) {
    ...
    eRet = (ctrlId == WMT_ANT_RAM_DOWNLOAD)
               ? WMT_ANT_RAM_DOWN_OK
               : ((pOp->op.au4OpData[2] == 1) ? WMT_ANT_RAM_EXIST
                                              : WMT_ANT_RAM_NOT_EXIST);
  }
...
MTK_WCN_BOOL mtk_wcn_wmt_do_reset(ENUM_WMTDRV_TYPE_T type) {
  INT32 iRet = -1;
  UINT8 *drv_name[] = {
      [0] = "DRV_TYPE_BT",     [1] = "DRV_TYPE_FM",  [2] = "DRV_TYPE_GPS",
      [3] = "DRV_TYPE_WIFI",   [4] = "DRV_TYPE_WMT", [5] = "DRV_TYPE_ANT",
      [11] = "DRV_TYPE_GPSL5",
  };
...
VOID mtk_wcn_wmt_set_wifi_ver(UINT32 Value) { wmt_lib_soc_set_wifiver(Value); }
...
VOID mtk_wcn_wmt_dump_wmtd_backtrace(VOID) { wmt_lib_dump_wmtd_backtrace(); }
...
VOID mtk_wmt_set_ext_ldo(UINT32 flag) { wmt_lib_set_ext_ldo(flag); }
EXPORT_SYMBOL(mtk_wmt_set_ext_ldo);

INT32 mtk_wmt_gps_mcu_ctrl(PUINT8 p_tx_data_buf, UINT32 tx_data_len,
                           PUINT8 p_rx_data_buf, UINT32 rx_data_buf_len,
                           PUINT32 p_rx_data_len) {
  return wmt_lib_gps_mcu_ctrl(p_tx_data_buf, tx_data_len, p_rx_data_buf,
                              rx_data_buf_len, p_rx_data_len);
}
...
static MTK_WCN_BOOL mtk_wmt_gps_suspend_ctrl_by_type(MTK_WCN_BOOL gps_l1,
  ...
  pSignal->timeoutValue =
      (MTK_WCN_BOOL_FALSE == suspend) ? MAX_FUNC_ON_TIME : MAX_FUNC_OFF_TIME;
...
MTK_WCN_BOOL mtk_wmt_gps_suspend_ctrl(MTK_WCN_BOOL suspend) {
  return mtk_wmt_gps_suspend_ctrl_by_type(MTK_WCN_BOOL_TRUE, MTK_WCN_BOOL_TRUE,
                                          suspend);
}
EXPORT_SYMBOL(mtk_wmt_gps_suspend_ctrl);

MTK_WCN_BOOL mtk_wmt_gps_l1_suspend_ctrl(MTK_WCN_BOOL suspend) {
  return mtk_wmt_gps_suspend_ctrl_by_type(MTK_WCN_BOOL_TRUE, MTK_WCN_BOOL_FALSE,
                                          suspend);
}
EXPORT_SYMBOL(mtk_wmt_gps_l1_suspend_ctrl);

MTK_WCN_BOOL mtk_wmt_gps_l5_suspend_ctrl(MTK_WCN_BOOL suspend) {
  return mtk_wmt_gps_suspend_ctrl_by_type(MTK_WCN_BOOL_FALSE, MTK_WCN_BOOL_TRUE,
                                          suspend);
}
EXPORT_SYMBOL(mtk_wmt_gps_l5_suspend_ctrl);

INT32 mtk_wcn_wmt_mpu_lock_aquire(VOID) { return wmt_lib_mpu_lock_aquire(); }
EXPORT_SYMBOL(mtk_wcn_wmt_mpu_lock_aquire);

VOID mtk_wcn_wmt_mpu_lock_release(VOID) { wmt_lib_mpu_lock_release(); }
EXPORT_SYMBOL(mtk_wcn_wmt_mpu_lock_release);

#endif /* !CONFIG_MTK_CONNECTIVITY_COMMON */
