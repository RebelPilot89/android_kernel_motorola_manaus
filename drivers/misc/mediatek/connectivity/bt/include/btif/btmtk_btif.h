...
enum bt_psm_state { PSM_ST_SLEEP, PSM_ST_NORMAL_TR, PSM_ST_MAX };
...
enum bt_direction_type { NONE, TX, RX };
...
struct wmt_pkt_param {
	...
	union {
		...
		struct {
			...
			uint8_t subop;
			uint8_t start_addr
				[4]; /* start address of cal data, little endian */
...
static inline void bt_wake_lock_init(struct bt_wake_lock *plock)
{
	if (plock) {
		...
		if (!plock->ws)
			...
			pm_qos_add_request(&plock->qos_req,
...
static inline void bt_hold_wake_lock_timeout(struct bt_wake_lock *plock,
					     uint32_t ms)
{
	if (plock && plock->ws) {
...
int btmtk_inttrx_DynamicAdjustTxPower(uint8_t mode, int8_t set_val,
				      BT_RX_EVT_HANDLER_CB cb,
				      bool is_blocking);
...
void add_dump_packet(const uint8_t *buffer, const uint32_t length,
		     enum bt_direction_type type);
