...
#define CONN_HOST_CSR_TOP_CR_HOSTCSR2BGF_ON_DBG_SEL                            \
	(CONN_HOST_CSR_TOP_START + 0xC04)
 ...
 * 0x18810000
 */
#define BGF_BT_CFG_BGF_CKGEN0_HCLK_SRC_SEL_FRZ_WIN_EN                          \
	(BGF_REG_INFO_BASE_ADDR + 0x100)
...
static int32_t bgfsys_check_conninfra_ready(void)
{
	...
	if (conninfra_cfg_id_rdy) {
		for (i = 0; i < retry; i++) {
			value = REG_READL(
					CONN_INFRA_CFG_ON_CONN_INFRA_CFG_PWRCTRL1) &
				CONN_INFRA_RDY;
		...
		if (!conninfra_reg_readable()) {
			...
			if (hang_ret > 0) {
				...
				conninfra_trigger_whole_chip_rst(
					CONNDRV_TYPE_BT, "bus hang");
			}
			BTMTK_ERR(
				"conninfra not readable, but not bus hang ret = %d",
				hang_ret);
		}
	}
...
static void inline bt_dump_bgfsys_mcusys_flag(void)
{
	...
	if (base == NULL) {
		BTMTK_ERR("ioremap [0x%08x] fail",
			  BGF_DRIVER_DUMP_BASE + 0xA00);
		return;
	}
...
static inline void bt_dump_bgfsys_debug_cr(void)
{
	...
	if (conninfra_reg_readable() && !bt_is_bgf_bus_timeout()) {
		BTMTK_INFO(
			"[BGF Bus hang debug CR (18800410~18000444, 18802214~18802220)] Count = (18)");
		...
		BTMTK_INFO(
			"conninfra is not readable, skip [BGF Bus hang debug CR]");
...
host_csr_only:
	for (i = 0x0F; i >= 0x01; i--) {
		...
		BTMTK_INFO(
			"%s: REG[0x18060128] write[0x%08x], REG[0x18060148] read[0x%08x]",
...
static void bgfsys_dump_conn_wt_slp_ctrl_reg(void)
{
	...
	if (base) {
		ret = snprintf(pos, (end - pos + 1), " 0x180050A8 = [0x%08x]",
			       REG_READL(base));
		pos += ret;
		iounmap(base);
	} else
...
static inline int32_t bgfsys_power_on(void)
{
	...
	/* power on bgfsys on */
	value = 0x42540080 | (REG_READL(CONN_INFRA_RGU_BGFSYS_ON_TOP_PWR_CTL) &
			      ~(BITS(16, 31) | BIT(7)));
	...
	if (!conninfra_reg_readable()) {
		if (conninfra_is_bus_hang() > 0) {
			BTMTK_ERR(
				"%s: check conninfra status fail after set CONN_INFRA_CFG_BT_PWRCTLCR0!",
				__func__);
			goto error;
		}
	}
	...
	value = 0x80000 |
		(bt_read_cr(BGF_MCUSYS_DLY_CHAIN_CTL) & ~BITS(16, 19));
	bt_write_cr(BGF_MCUSYS_DLY_CHAIN_CTL, value, FALSE);

	/* enable hclk switch freeze window */
	SET_BIT(BGF_BT_CFG_BGF_CKGEN0_HCLK_SRC_SEL_FRZ_WIN_EN,
		HLCK_SWITCH_FRZ_WINDOW_B);

	/* host_csr debug bgfsys_on_top select setting */
	value = 0x300001 |
		(REG_READL(CONN_HOST_CSR_TOP_CR_HOSTCSR2BGF_ON_DBG_SEL) &
		 BITS(22, 31));
	...
	value = REG_READL(CONN_HOST_CSR_TOP_CR_HOSTCSR2BGF_ON_DBG_SEL) &
		BITS(22, 31);
	REG_WRITEL(CONN_HOST_CSR_TOP_CR_HOSTCSR2BGF_ON_DBG_SEL, value);
...
static inline int32_t bgfsys_power_off(void)
{
	...
	/* power off bgfsys on */
	value = 0x42540000 | (REG_READL(CONN_INFRA_RGU_BGFSYS_ON_TOP_PWR_CTL) &
			      ~(BITS(16, 31) | BIT(7)));
	REG_WRITEL(CONN_INFRA_RGU_BGFSYS_ON_TOP_PWR_CTL, value);
	...
	if (remap_addr) {
		...
		do {
			...
			BTMTK_DBG(
				"CONN_WT_SLP_CTL_REG_WB_SLP_TOP_CK_2_WB_SLP_TOP_CK_2 = 0x%08x",
				value);
			usleep_range(500, 550);
			retry--;
		} while (value != 0 && retry > 0);
	...
	if (remap_addr) {
		...
		SET_BIT(remap_addr,
			CONN_INFRA_CFG_EMI_CTL_BT_SW_CONN_SRCCLKENA_BT);
		CLR_BIT(remap_addr,
			CONN_INFRA_CFG_EMI_CTL_BT_SW_CONN_SRCCLKENA_BT);
		CLR_BIT(remap_addr,
			CONN_INFRA_CFG_EMI_CTL_BT_SW_CONN_AP_BUS_REQ_BT);
		CLR_BIT(remap_addr,
			CONN_INFRA_CFG_EMI_CTL_BT_SW_CONN_APSRC_REQ_BT);
