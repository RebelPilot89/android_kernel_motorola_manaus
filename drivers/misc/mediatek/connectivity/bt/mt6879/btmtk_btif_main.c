...
static int32_t bt_reg_init(void)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			BTMTK_DBG(
				"Get Index(%d) phy(0x%zx) baseAddr=(0x%zx) size=(0x%zx)",
...
int32_t bt_reg_deinit(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (bt_reg.reg_base_addr[i].vir_addr) {
			iounmap((void __iomem *)bt_reg.reg_base_addr[i]
					.vir_addr);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
int btmtk_disp_notify_cb(struct notifier_block *nb, unsigned long value,
			 void *v)
{
	int *data = (int *)v;
	int32_t new_state = 0;
	...
	*/
	...
	if (value == MTK_DISP_EARLY_EVENT_BLANK) {
		...
		if (cif_dev->bt_state == FUNC_ON) {
			BTMTK_INFO("%s: blank state [%ld]->[%ld], and send cmd",
				   __func__, cif_dev->blank_state, new_state);
			cif_dev->blank_state = new_state;
			btmtk_intcmd_wmt_blank_status(g_sbdev->hdev,
						      cif_dev->blank_state);
		} else {
			BTMTK_INFO("%s: blank state [%ld]->[%ld]", __func__,
				   cif_dev->blank_state, new_state);
			cif_dev->blank_state = new_state;
		}
	}
...
static int btmtk_fb_notifier_callback(struct notifier_block *self,
				      ...
				      unsigned long event, void *dummy)
{
	...
	case PM_SUSPEND_PREPARE:
		if (cif_dev->bt_state != FUNC_ON) {
			BTMTK_INFO("%s: bt_state[%d], event[%ld]", __func__,
				   cif_dev->bt_state, event);
		}
	...
	do {
		...
		if ((retry & 0xF) == 0) { /* retry % 16 == 0 */
			if (((retry < LPCR_POLLING_RTY_LMT &&
			      retry >= LPCR_MASS_DUMP_LMT) ||
			     (retry == 2048) || (retry == 32)) &&
			    ((retry & 0x1F) == 0)) {
				BTMTK_WARN(
					"[DRV_OWN] failed in %d ms, retry[%d]",
					(LPCR_POLLING_RTY_LMT - retry) / 2,
					retry);
				bt_dump_cif_own_cr();
...
int bt_chip_reset_flow(enum bt_reset_level rst_level, enum consys_drv_type drv,
		       char *reason)
{
	...
	if (retry == 0) {
		BTMTK_ERR(
			"Unable to trigger pre_chip_rst due to unfinish previous reset");
		return -1;
	}
	...
	if (is_1st_reset) {
		...
		/* 2. Wait for IRQ */
		if (!wait_for_completion_timeout(&cif_dev->rst_comp,
		...
		if (cif_dev->bt_state != RESET_START &&
			...
			if (g_bt_trace_pt)
				bt_dbg_tp_evt(TP_ACT_RST, TP_PAR_RST_DUMP, 0,
					      NULL);
			bt_dump_bgfsys_debug_cr();
			connsys_coredump_start(cif_dev->coredump_handle,
					       dump_property, drv, reason);
		} else
			BTMTK_WARN("BT state [%d], skip coredump",
				   cif_dev->bt_state);
		up(&cif_dev->halt_sem);
#endif
	}
	...
	if (cif_dev->bt_state == FUNC_OFF ||
		...
		BTMTK_INFO(
			"BT is already off or reset success, skip power off [%d, %d]",
	...
	btmtk_fops_set_state(
		bdev,
		BTMTK_FOPS_STATE_OPENED); // to comform to the common part state
...
static int32_t bt_do_calibration(void)
{
	...
	if (ret == -1) {
		BTMTK_ERR(
			"%s: error return in recving calibration event, reset",
			__func__);
		return CONNINFRA_CB_RET_CAL_FAIL_POWER_ON;
	} else if (ret) {
		BTMTK_ERR("%s: error return in sent calibration cmd", __func__);
		return (cif_dev->bt_precal_state) ?
			       CONNINFRA_CB_RET_CAL_FAIL_POWER_ON :
			       CONNINFRA_CB_RET_CAL_FAIL_POWER_OFF;
	} else
		return (cif_dev->bt_precal_state) ?
			       CONNINFRA_CB_RET_CAL_PASS_POWER_ON :
...
int bt_pwrctrl_level_change_cb(enum conn_pwr_event_type type, void *data)
{
	...
	case CONN_PWR_EVENT_LEVEL:
		...
		BTMTK_INFO("%s: lp_cur_bat_lv = %d", __func__,
			   cif_dev->dy_pwr.lp_cur_lv);
		btmtk_inttrx_DynamicAdjustTxPower(HCI_CMD_DY_ADJ_PWR_SET,
						  set_val, NULL, FALSE);
		break;
...
void bt_pwrctrl_register_evt(void)
{
	...
	conn_pwr_register_event_cb(
		CONN_PWR_DRV_BT, (CONN_PWR_EVENT_CB)bt_pwrctrl_level_change_cb);
}
...
static void btmtk_btif_enter_deep_idle(struct work_struct *pwork)
{
	...
	if (idle_ctrl->is_dpidle) {
		...
		if (g_bt_trace_pt)
			bt_dbg_tp_evt(TP_ACT_DPI_ENTER,
				      (ret == 0 ? TP_PAR_PASS : TP_PAR_FAIL), 0,
				      NULL);
		if (ret)
			BTMTK_ERR("[DP_IDLE] BTIF enter dpidle failed(%d)",
				  ret);
		else
...
static int32_t btmtk_btif_dpidle_ctrl(u_int8_t enable)
{
	...
	if (idle_ctrl->task != NULL) {
		...
		if (!enable) {
			...
			ret = mtk_wcn_btif_dpidle_ctrl(g_btif_id,
						       BTIF_DPIDLE_DISABLE);
			idle_ctrl->is_dpidle = (ret) ? TRUE : FALSE;

			if (g_bt_trace_pt)
				bt_dbg_tp_evt(TP_ACT_DPI_EXIT,
					      (ret == 0 ? TP_PAR_PASS :
							  TP_PAR_FAIL),
					      0, NULL);
			if (ret)
				BTMTK_ERR(
					"[DP_IDLE] BTIF exit dpidle failed(%d)",
					ret);
			else
				BTMTK_DBG("[DP_IDLE] BTIF exit dpidle success");
		} else {
			BTMTK_DBG(
				"[DP_IDLE] create timer for enable deep idle");
 ...
 */
static int32_t _check_wmt_evt_over_hci(uint8_t *buffer, uint16_t len,
				       uint8_t expected_op,
				       struct wmt_pkt_param *p_evt_params)
{
	...
	if (opcode != expected_op) {
		BTMTK_ERR(
			"WMT OpCode is unexpected! opcode[0x%02X], expect[0x%02X]",
			opcode, expected_op);
		goto check_error;
	}

	param_len = (p_wmt_evt->hdr.param_len[1] << 8) |
		    (p_wmt_evt->hdr.param_len[0]);
	/* Sanity check */
	...
	case WMT_OPCODE_FUNC_CTRL:
		if (param_len != sizeof(p_wmt_evt->params.u.func_ctrl_evt)) {
			BTMTK_ERR(
				"Unexpected event param len %d for WMT OpCode 0x%x!",
				param_len, opcode);
			break;
		}
...
int32_t btmtk_dispatch_event(struct sk_buff *skb)
{
	...
	BTMTK_DBG("%s: event code=[0x%02x], length = %d", __func__, event_code,
		  skb->len);

	if (event_code == 0xE4) {
		/* WMT event */
		p_inter_cmd->result =
			_check_wmt_evt_over_hci(skb->data, skb->len,
		 ...
		 */
		memcpy((void *)&p_inter_cmd->wmt_event_params.u, skb->data,
		       skb->len);
...
void btmtk_check_event_timeout(struct sk_buff *skb)
{
	...
	if (event_code == HCI_EVT_COMPLETE_EVT) {
		...
		if (cmd_list_check(cmd_opcode) == FALSE) {
			BTMTK_ERR("%s No match command %4X", __func__,
				  cmd_opcode);
		} else {
...
int32_t btmtk_wcn_btif_open(void)
{
	...
	ret = mtk_wcn_btif_rx_cb_register(
		g_btif_id, (MTK_WCN_BTIF_RX_CB)bt_receive_data_cb);
	if (ret) {
...
int btmtk_btif_send_cmd_if(struct btmtk_dev *bdev, struct sk_buff *skb,
...
int btmtk_btif_send_and_recv(struct btmtk_dev *bdev, struct sk_buff *skb,
			     ...
			     int delay, int retry, int pkt_type)
{
	...
	ret = wait_event_timeout(bdev->p_wait_event_q,
				 bdev->evt_skb != NULL ||
					 tx_state == BTMTK_TX_SKIP_VENDOR_EVT,
				 2 * HZ);
			...
			int retry, int pkt_type)
{
	...
	if (cmd[0] == 0x01) {
		if ((cmd[1] == 0x5D && cmd[2] == 0xFC) ||
			...
			BTMTK_DBG("%s add opcode %4X in cmd queue", __func__,
				  cmd_opcode);
	...
	while (tx_len > 0 && _retry < retry) {
		...
		if (ret < 0) {
			BTMTK_ERR("BTIF write failed(%d) on retry(%d)", ret,
				  _retry - 1);
			return -1;
		}
		tx_len -= ret;
...
static void btmtk_btif_internal_trx_work(struct work_struct *work)
{
	...
	btmtk_send_data(g_sbdev->hdev, cif_dev->internal_trx.buf,
			cif_dev->internal_trx.buf_len);
	if (!wait_for_completion_timeout(&cif_dev->internal_trx.comp,
					 msecs_to_jiffies(2000)))
...
int btmtk_btif_internal_trx(uint8_t *buf, uint32_t count,
			    BT_RX_EVT_HANDLER_CB cb, bool send_to_stack,
			    bool is_blocking)
{
	struct btmtk_btif_dev *cif_dev =
		(struct btmtk_btif_dev *)g_sbdev->cif_dev;

	spin_lock_irqsave(&cif_dev->internal_trx.lock,
			  cif_dev->internal_trx.flag);
	if (cif_dev->internal_trx.cb) {
		BTMTK_ERR("unable to do internal trx before previous trx done");
		return -1;
	}
	cif_dev->internal_trx.cb = cb;
	spin_unlock_irqrestore(&cif_dev->internal_trx.lock,
			       cif_dev->internal_trx.flag);
...
int btmtk_btif_event_filter(struct btmtk_dev *bdev, struct sk_buff *skb)
{
	...
	if (test_bit(BTMTK_TX_WAIT_VND_EVT, &bdev->tx_state)) {
		...
		if (btmtk_dispatch_event(skb) != WMT_EVT_SKIP) {
			...
			if (!bdev->evt_skb) {
				BTMTK_ERR(
					"%s: WMT event copy to evt_skb failed",
					__func__);
			...
			if (test_and_clear_bit(BTMTK_TX_WAIT_VND_EVT,
					       &bdev->tx_state)) {
				BTMTK_DBG("%s: clear bit BTMTK_TX_WAIT_VND_EVT",
					  __func__);
				wake_up(&bdev->p_wait_event_q);
				BTMTK_DBG("%s: wake_up p_wait_event_q",
					  __func__);
			} else {
				BTMTK_ERR(
					"%s: test_and_clear_bit(BTMTK_TX_WAIT_VND_EVT) fail",
					__func__);
				if (bdev->evt_skb)
					kfree_skb(bdev->evt_skb);
				bdev->evt_skb = NULL;
	...
	if (cif_dev->internal_trx.cb != NULL) {
		if (skb->data[0] == HCI_EVT_COMPLETE_EVT &&
		    cif_dev->internal_trx.opcode ==
			    skb->data[3] + (skb->data[4] << 8)) {
			BTMTK_INFO_RAW(
				skb->data, (int)skb->len,
				"[internal trx] len[%d] RX: ", (int)skb->len);
...
#if SUPPORT_COREDUMP
	/* 7. Init coredump */
	cif_dev->coredump_handle =
		connsys_coredump_init(CONN_DEBUG_TYPE_BT, &bt_coredump_cb);
#endif

	/* 8. Register screen on/off & suspend/wakup notify callback */
	cif_dev->blank_state = WMT_PARA_SCREEN_ON;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	if (mtk_disp_notifier_register("btmtk_disp_notifier",
				       &btmtk_disp_notifier)) {
...
static u_int8_t bt_tx_wait_for_msg(struct btmtk_dev *bdev)
{
	...
	if (cif_dev->bt_state == RESET_START)
		...
		BTMTK_DBG(
			"skb [%d], rx_ind [%d], bgf2ap_ind [%d], bt_conn2ap_ind [%d], sleep_flag [%d], wakeup_flag [%d], force_on [%d]",
		...
		return (!skb_queue_empty(&cif_dev->tx_queue) ||
			cif_dev->rx_ind || cif_dev->bgf2ap_ind ||
			cif_dev->bt_conn2ap_ind ||
			(!cif_dev->psm.force_on &&
			 cif_dev->psm
				 .sleep_flag) // only check sleep_flag if force_on is FALSE
			|| cif_dev->psm.wakeup_flag || kthread_should_stop());
	}
}
...
int32_t btmtk_tx_thread(void *arg)
{
	...
	do {
		strncpy(state_tag,
			(psm->state == PSM_ST_SLEEP ? "[ST_SLEEP]" :
						      "[ST_NORMAL]"),
			15);
		BTMTK_DBG("%s -- wait_event_interruptible", state_tag);
		wait_event_interruptible(cif_dev->tx_waitq,
					 bt_tx_wait_for_msg(bdev));
		BTMTK_DBG("%s -- wakeup", state_tag);
		if (kthread_should_stop()) {
			BTMTK_INFO("%s %s should stop now...", state_tag,
				   __func__);
		...
		case PSM_ST_SLEEP:
			...
			if (wakeup_ret) {
				...
				if (cif_dev->bt_state == FUNC_ON) {
					BTMTK_ERR(
						"%s FATAL: btmtk_cif_fw_own_clr error!! going to reset",
						state_tag);
					bt_trigger_reset();
				} else
					BTMTK_WARN(
						"%s bt_state[%d] is not FUNC_ON, skip reset",
						state_tag, cif_dev->bt_state);
				break;
			} else {
		...
		case PSM_ST_NORMAL_TR:
			...
			if (cif_dev->rx_ind) {
				BTMTK_DBG("%s wakeup by BTIF_WAKEUP_IRQ",
					  state_tag);
			...
			while (!skb_queue_empty(&cif_dev->tx_queue)) {
				...
				if (skb_len >= 3 && skb->data[0] == 0x01 &&
				    skb->data[1] == 0x1B &&
				    skb->data[2] == 0xFD) {
					kfree_skb(skb);
					BTMTK_INFO(
						"%s enable check command timeout function",
						state_tag);
				...
				if ((ret < 0) || (ret != skb_len)) {
					BTMTK_ERR(
						"%s FATAL: TX packet error!! (%u/%d)",
						state_tag, skb_len, ret);
					break;
				}

				if (psm->sleep_flag) {
					BTMTK_DBG(
						"%s more data to send and wait for event, ignore sleep",
						state_tag);
					psm->sleep_flag = FALSE;
				}
			}
			 ...
			 */
			if (cif_dev->bt_state == FUNC_ON &&
			    cmd_list_isempty() && psm->sleep_flag &&
			    !psm->force_on) {
				// wait if btif tx is not finish yet
				for (ii = 0; ii < 5; ii++) {
					if (mtk_btif_is_tx_complete(g_btif_id) >
					    0)
						break;
					else
						usleep_range(USLEEP_1MS_L,
							     USLEEP_1MS_H);
					if (ii == 4)
						BTMTK_INFO(
							"%s mtk_btif_is_tx_complete run 5 times",
							state_tag);
				}
				// re-run while loop
				if (ii == 4) {
				...
				if (sleep_ret) {
					if (cif_dev->bt_state == FUNC_ON) {
						BTMTK_ERR(
							"%s FATAL: btmtk_cif_fw_own_set error!! going to reset",
							state_tag);
						bt_trigger_reset();
					} else
						BTMTK_WARN(
