...
static const struct btmtk_cif_state g_cif_state[] = {
	...
	{ BTMTK_STATE_DISCONNECT, BTMTK_STATE_DISCONNECT,
...
__weak int btmtk_cif_rx_packet_handler(struct hci_dev *hdev,
				       struct sk_buff *skb)
{
	BTMTK_ERR("weak function %s not implement", __func__);
	return -1;
...
{
	uint8_t efuse_r[READ_EFUSE_CMD_LEN] = {
		0x01, 0x6F, 0xFC, 0x0E, 0x01, 0x0D, 0x0A, 0x00, 0x02,
		0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	}; /*4 sub block number(sub block 0~3)*/

	uint8_t efuse_r_event[READ_EFUSE_EVT_HDR_LEN] = { 0x04, 0xE4, 0x1E,
							  0x02, 0x0D, 0x1A,
							  0x00, 02,   04 };
	/*check event
	...
	efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 3] =
		((sub_block + 1) & 0xFF00) >> 8;
	efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 4] = (sub_block + 2) & 0xFF;
	efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 5] =
		((sub_block + 2) & 0xFF00) >> 8;
	efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 6] = (sub_block + 3) & 0xFF;
	efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 7] =
		((sub_block + 3) & 0xFF00) >> 8;

	ret = btmtk_main_send_cmd(bdev, efuse_r, READ_EFUSE_CMD_LEN,
				  efuse_r_event, READ_EFUSE_EVT_HDR_LEN, 0, 0,
	...
	if (memcmp(bdev->io_buf, efuse_r_event, READ_EFUSE_EVT_HDR_LEN) == 0) {
		...
		if (efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET] == bdev->io_buf[9] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 1] ==
			    bdev->io_buf[10] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 2] ==
			    bdev->io_buf[15] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 3] ==
			    bdev->io_buf[16] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 4] ==
			    bdev->io_buf[21] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 5] ==
			    bdev->io_buf[22] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 6] ==
			    bdev->io_buf[27] &&
		    efuse_r[READ_EFUSE_CMD_BLOCK_OFFSET + 7] ==
			    bdev->io_buf[28]) {
			BTMTK_DBG("address compare ok");
			/*Get value*/
			sub_block_addr_in_event =
				((addr / 16) / 4); /*cal block num*/
			temp = addr % 16;
...
void btmtk_free_setting_file(struct btmtk_dev *bdev)
{
	...
	btmtk_free_fw_cfg_struct(bdev->bt_cfg.phase1_wmt_cmd,
				 PHASE1_WMT_CMD_COUNT);
...
void btmtk_set_chip_state(struct btmtk_dev *bdev, unsigned char new_state)
{
	static const char *const state_msg[BTMTK_STATE_MSG_NUM] = {
		"UNKNOWN", "INIT",   "DISCONNECT", "PROBE",   "WORKING",
		"SUSPEND", "RESUME", "FW_DUMP",	   "STANDBY", "SUBSYS_RESET",
	};
	...
	BTMTK_INFO("%s: %s(%d) -> %s(%d)", __func__,
		   state_msg[bdev->interface_state], bdev->interface_state,
		   state_msg[new_state], new_state);
...
void btmtk_fops_set_state(struct btmtk_dev *bdev, unsigned char new_state)
{
	...
	BTMTK_INFO("%s: FOPS_%s(%d) -> FOPS_%s(%d)", __func__,
		   fstate_msg[bdev->fops_state], bdev->fops_state,
		   fstate_msg[new_state], new_state);
	FOPS_MUTEX_LOCK();
...
static void btmtk_main_info_initialize(void)
{
	...
	main_info.fwdump_ws =
		wakeup_source_register(NULL, "btmtk_fwdump_wakelock");
	main_info.woble_ws =
		wakeup_source_register(NULL, "btmtk_woble_wakelock");
...
void btmtk_hci_snoop_print_to_log(void)
{
	...
	BTMTK_INFO(
		"Using A5 A5 to separator the head 32 bytes and the tail 32 bytes data");
	...
	for (counter = 0; counter < HCI_SNOOP_ENTRY_NUM; counter++) {
		if (main_info.hci_cmd_len[index] > 0)
			BTMTK_INFO_RAW(main_info.hci_cmd_buf[index],
				       main_info.hci_cmd_len[index],
				       "time(%u)-act_len(%d)-len(%d):",
				       main_info.hci_cmd_timestamp[index],
				       main_info.hci_cmd_actual_len[index],
				       main_info.hci_cmd_len[index]);
		index++;
	...
	for (counter = 0; counter < HCI_SNOOP_ENTRY_NUM; counter++) {
		if (main_info.hci_event_len[index] > 0)
			BTMTK_INFO_RAW(main_info.hci_event_buf[index],
				       main_info.hci_event_len[index],
				       "time(%u)-act_len(%d)-len(%d):",
				       main_info.hci_event_timestamp[index],
				       main_info.hci_event_actual_len[index],
				       main_info.hci_event_len[index]);
		index++;
		if (index >= HCI_SNOOP_ENTRY_NUM)
			index = 0;
	...
	for (counter = 0; counter < HCI_SNOOP_ENTRY_NUM; counter++) {
		if (main_info.hci_adv_event_len[index] > 0)
			BTMTK_INFO_RAW(
				main_info.hci_adv_event_buf[index],
				main_info.hci_adv_event_len[index],
				"time(%u)-act_len(%d)-len(%d):",
				main_info.hci_adv_event_timestamp[index],
				main_info.hci_adv_event_actual_len[index],
				main_info.hci_adv_event_len[index]);
		index++;
		if (index >= HCI_SNOOP_ENTRY_NUM)
			index = 0;
	...
	for (counter = 0; counter < HCI_SNOOP_ENTRY_NUM; counter++) {
		if (main_info.hci_acl_len[index] > 0) {
			BTMTK_INFO_RAW(main_info.hci_acl_buf[index],
				       main_info.hci_acl_len[index],
				       "time(%u)-act_len(%d)-len(%d):",
				       main_info.hci_acl_timestamp[index],
				       main_info.hci_acl_actual_len[index],
				       main_info.hci_acl_len[index]);
		}
		index++;
		if (index >= HCI_SNOOP_ENTRY_NUM)
...
void btmtk_hci_snoop_save_cmd(u32 len, u8 *buf)
{
	...
	if (buf && len > 0 && main_info.hci_cmd_index < HCI_SNOOP_ENTRY_NUM) {
		if (len < HCI_SNOOP_BUF_SIZE) {
			copy_len = len;
			copy_tail_len = 0;
		} else if (len > HCI_SNOOP_BUF_SIZE &&
			   len <= HCI_SNOOP_BUF_SIZE * 2)
			copy_tail_len = len - copy_len;

		main_info.hci_cmd_len[main_info.hci_cmd_index] =
			copy_len & 0xff;
		main_info.hci_cmd_actual_len[main_info.hci_cmd_index] =
			len & 0xffff;
		main_info.hci_cmd_timestamp[main_info.hci_cmd_index] =
			btmtk_hci_snoop_get_microseconds();
		memset(main_info.hci_cmd_buf[main_info.hci_cmd_index], 0,
		       HCI_SNOOP_MAX_BUF_SIZE);
		memcpy(main_info.hci_cmd_buf[main_info.hci_cmd_index], buf,
		       copy_len & 0xff);
		...
		if (copy_tail_len > 0) {
			...
			memcpy(main_info.hci_cmd_buf[main_info.hci_cmd_index] +
				       copy_len,
			       separator_char, SEPARATOR_LEN);
			memcpy(main_info.hci_cmd_buf[main_info.hci_cmd_index] +
...
void btmtk_hci_snoop_save_adv_event(u32 len, u8 *buf)
{
	...
	if (buf && len > 0 &&
		...
		main_info.hci_adv_event_len[main_info.hci_adv_event_index] =
			copy_len & 0xff;
		main_info
			.hci_adv_event_actual_len[main_info.hci_adv_event_index] =
			len & 0xffff;
		main_info.hci_adv_event_timestamp[main_info.hci_adv_event_index] =
			btmtk_hci_snoop_get_microseconds();
		memset(main_info.hci_adv_event_buf[main_info.hci_adv_event_index],
		       0, HCI_SNOOP_MAX_BUF_SIZE);
		memcpy(main_info.hci_adv_event_buf[main_info.hci_adv_event_index],
		       buf, copy_len);
		/* save less then 32 bytes data in the buffer tail, using A5 A5 to
		 * separator the head 32 bytes data and the tail 32 bytes data
		 */
		if (copy_tail_len > 0) {
			copy_tail_buf = buf + len - copy_tail_len;
			main_info.hci_adv_event_len
				[main_info.hci_adv_event_index] +=
				(copy_tail_len + SEPARATOR_LEN) & 0xff;
			memcpy(main_info.hci_adv_event_buf
					       [main_info.hci_adv_event_index] +
				       copy_len,
			       separator_char, SEPARATOR_LEN);
			memcpy(main_info.hci_adv_event_buf
...
void btmtk_hci_snoop_save_event(u32 len, u8 *buf)
{
	...
	if (buf && len > 0 && main_info.hci_event_index < HCI_SNOOP_ENTRY_NUM) {
		...
		main_info.hci_event_len[main_info.hci_event_index] =
			copy_len & 0xff;
		main_info.hci_event_actual_len[main_info.hci_event_index] =
			len & 0xffff;
		main_info.hci_event_timestamp[main_info.hci_event_index] =
			btmtk_hci_snoop_get_microseconds();
		memset(main_info.hci_event_buf[main_info.hci_event_index], 0,
		       HCI_SNOOP_MAX_BUF_SIZE);
		memcpy(main_info.hci_event_buf[main_info.hci_event_index], buf,
		       copy_len);
		/* save less then 32 bytes data in the buffer tail, using A5 A5 to
		 * separator the head 32 bytes data and the tail 32 bytes data
		 */
		if (copy_tail_len > 0) {
			...
			main_info.hci_event_len[main_info.hci_event_index] +=
				(copy_tail_len + SEPARATOR_LEN) & 0xff;
			memcpy(main_info.hci_event_buf[main_info.hci_event_index] +
				       copy_len,
			       separator_char, SEPARATOR_LEN);
			memcpy(main_info.hci_event_buf[main_info.hci_event_index] +
				       copy_len + SEPARATOR_LEN,
...
void btmtk_hci_snoop_save_acl(u32 len, u8 *buf)
{
	...
	if (buf && len > 0 && main_info.hci_acl_index < HCI_SNOOP_ENTRY_NUM) {
		...
		main_info.hci_acl_len[main_info.hci_acl_index] =
			copy_len & 0xff;
		main_info.hci_acl_actual_len[main_info.hci_acl_index] =
			len & 0xffff;
		main_info.hci_acl_timestamp[main_info.hci_acl_index] =
			btmtk_hci_snoop_get_microseconds();
		memset(main_info.hci_acl_buf[main_info.hci_acl_index], 0,
		       HCI_SNOOP_MAX_BUF_SIZE);
		memcpy(main_info.hci_acl_buf[main_info.hci_acl_index], buf,
		       copy_len & 0xff);
		/* save less then 32 bytes data in the buffer tail, using A5 A5 to
		 * separator the head 32 bytes data and the tail 32 bytes data
		 */
		if (copy_tail_len > 0) {
			...
			main_info.hci_acl_len[main_info.hci_acl_index] +=
				(copy_tail_len + SEPARATOR_LEN) & 0xff;
			memcpy(main_info.hci_acl_buf[main_info.hci_acl_index] +
				       copy_len,
			       separator_char, SEPARATOR_LEN);
			memcpy(main_info.hci_acl_buf[main_info.hci_acl_index] +
				       copy_len + SEPARATOR_LEN,
...
void btmtk_hci_snoop_print(u32 len, const u8 *buf)
{
	...
	if (buf && len > 0) {
		...
		if (copy_tail_len > 0) {
			...
			memcpy(hci_snoop_buf + copy_len, separator_char,
			       SEPARATOR_LEN);
		...
		if (hci_snoop_len > 0)
			BTMTK_INFO_RAW(hci_snoop_buf, hci_snoop_len,
				       "act_len(%d)-len(%d)-buf(%p):", len,
				       hci_snoop_len, buf);
	}
}
...
h4_recv_buf(struct hci_dev *hdev, struct sk_buff *skb,
	    ...
	    const struct h4_recv_pkt *pkts, int pkts_count)
{
	...
	while (count) {
		...
		if (!skb) {
			...
			if (!skb) {
				BTMTK_ERR("%s,skb is invalid, buffer[0] = %d!",
					  __func__, buffer[0]);
				if (is_mt66xx(bdev->chip_id))
					btmtk_set_sleep(hdev, FALSE);
				else {
					btmtk_hci_snoop_print(count_dbg,
							      buffer_dbg);
		...
		if (skb->len == (&pkts[i])->hlen) {
			...
			case 1:
				...
				if (skb_tailroom(skb) < dlen) {
					BTMTK_ERR(
						"%s, skb_tailroom is not enough, dlen:%d!",
						__func__, dlen);
					if (is_mt66xx(bdev->chip_id))
						btmtk_set_sleep(hdev, FALSE);
					else {
			...
			case 2:
				...
				if (skb_tailroom(skb) < dlen) {
					BTMTK_ERR(
						"%s, skb_tailroom is not enough in case 2, dlen:%d!",
						__func__, dlen);
					if (is_mt66xx(bdev->chip_id))
						btmtk_set_sleep(hdev, FALSE);
					else {
			...
			default:
				...
				BTMTK_ERR("%s, Unsupported variable length!",
					  __func__);
				if (is_mt66xx(bdev->chip_id))
					btmtk_set_sleep(hdev, FALSE);
				else {
...
#if ENABLESTP
static inline struct sk_buff *mtk_add_stp(struct btmtk_dev *bdev,
					  struct sk_buff *skb)
{
	...
	if (unlikely(skb_headroom(skb) < sizeof(*shdr)) ||
		...
		BTMTK_DBG(
			"%s, add pskb_expand_head, headroom = %d, tailroom = %d",
...
static const unsigned char *mtk_stp_split(struct btmtk_dev *bdev,
					  ...
					  int *sz_h4)
{
	...
	while (bdev->stp_cursor < 6 && count > 0) {
		...
		pr_info("fill stp format (%02x, %d, %d)\n",
			bdev->stp_pad[bdev->stp_cursor], bdev->stp_cursor,
			count);
		bdev->stp_cursor++;
		data++;
		count--;
	...
	if (!bdev->stp_dlen && bdev->stp_cursor >= 6) {
		...
		pr_info("stp format (%02x, %02x)", shdr->prefix,
			bdev->stp_dlen);
...
#if ENABLESTP
		bdev->rx_skb =
			h4_recv_buf(hdev, bdev->rx_skb, p_h4, sz_h4,
				    mtk_recv_pkts, ARRAY_SIZE(mtk_recv_pkts));
#else
		bdev->rx_skb =
			h4_recv_buf(hdev, bdev->rx_skb, data, count,
				    mtk_recv_pkts, ARRAY_SIZE(mtk_recv_pkts));
#endif
...
int btmtk_recv_event(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	BTMTK_DBG_RAW(skb->data, skb->len, "%s, recv evt(hci_recv_frame)",
		      __func__);
...
int btmtk_main_send_cmd(struct btmtk_dev *bdev, const uint8_t *cmd,
			const int cmd_len, const uint8_t *event,
			const int event_len, int delay, int retry, int pkt_type)
{
	struct sk_buff *skb = NULL;
	int ret = 0;
	int state = BTMTK_STATE_INIT;

	if (bdev == NULL || bdev->hdev == NULL || cmd == NULL || cmd_len <= 0) {
		BTMTK_ERR("%s, invalid parameters!", __func__);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (!is_mt66xx(bdev->chip_id) &&
	    memcmp(cmd, main_info.wmt_over_hci_header,
		   WMT_OVER_HCI_HEADER_SIZE) &&
	    pkt_type != BTMTK_TX_ACL_FROM_DRV &&
	    bdev->power_state != BTMTK_DONGLE_STATE_POWER_ON) {
		BTMTK_WARN(
			"%s: chip power isn't on, ignore this command, state is %d",
	...
	if (state == BTMTK_STATE_FW_DUMP) {
		BTMTK_WARN(
			"%s: FW dumping ongoing, don't send any cmd to FW!!!",
			__func__);
		ret = -1;
		goto exit;
	}
	...
	ret = main_info.hif_hook.send_and_recv(bdev, skb, event, event_len,
...
{
	u8 cmd[CHECK_LD_PATCH_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x05, 0x01,
					   0x17, 0x01, 0x00, 0x01 };
	u8 event[CHECK_LD_PATCH_EVT_HDR_LEN] = { 0x04, 0xE4, 0x05, 0x02,
						 0x17, 0x01, 0x00, /* 0x02 */ };
	int ret = -1;
	...
	ret = btmtk_main_send_cmd(bdev, cmd, CHECK_LD_PATCH_CMD_LEN, event,
				  CHECK_LD_PATCH_EVT_HDR_LEN, DELAY_TIMES,
			     ...
			     u32 *code_len, u8 retry)
{
	...
	do {
		...
		if (err == 0) {
			...
			*image = NULL;
			BTMTK_INFO(
				"%s: request_firmware %d times fail, maybe file not exist, err = %d",
				__func__, 10, err);
			return -1;
		}
		BTMTK_INFO(
			"%s: request_firmware fail, maybe file not exist, err = %d, retry = %d",
			__func__, err, retry);
		msleep(100);
	} while (retry-- > 0);
	...
	*image = vmalloc(ALIGN_4(fw_entry->size));
	if (*image == NULL) {
		*code_len = 0;
		...
		BTMTK_ERR("%s: vmalloc failed!! error code = %d", __func__,
			  err);
		return -1;
	}
...
static void btmtk_print_bt_patch_info(struct btmtk_dev *bdev, u8 *fwbuf)
{
	...
	if (is_mt7922(bdev->chip_id) || is_mt7961(bdev->chip_id))
		globalDesrc =
			(struct _Global_Descr *)(fwbuf +
						 FW_ROM_PATCH_HEADER_SIZE);
	...
	if (patchHdr) {
		...
		BTMTK_INFO("[btmtk] Magic Number = 0x%08x",
			   patchHdr->u4MagicNum);
				       ...
				       bool patch_flag)
{
	...
	if (bdev == NULL || cmd == NULL || event == NULL ||
	    sectionMap == NULL) {
	...
	if (fw_state == 0) {
		...
		cmd[9] =
			dma_flag; /* 1:using DMA to download, 0:using legacy wmt cmd*/
		cmd_len = SEC_MAP_NEED_SEND_SIZE + PATCH_HEADER_SIZE;

		if (patch_flag) {
			for (i = 0; i < SECTION_SPEC_NUM; i++) {
				revert_SecSpec =
					be2cpu32(sectionMap->u4SecSpec[i]);
				memcpy(&cmd[PATCH_HEADER_SIZE] +
					       i * sizeof(u32),
				       (u8 *)&revert_SecSpec, sizeof(u32));
			}
		} else
			memcpy(&cmd[PATCH_HEADER_SIZE],
			       (u8 *)(sectionMap->u4SecSpec),
		...
		ret = btmtk_main_send_cmd(bdev, cmd, cmd_len, event, event_len,
					  ...
					  BTMTK_TX_CMD_FROM_DRV);
		if (ret < 0) {
			BTMTK_ERR("%s: send wmd dl cmd failed, terminate!",
				  __func__);
			return PATCH_ERR;
		}
...
static int btmtk_load_fw_patch_using_wmt_cmd(struct btmtk_dev *bdev, u8 *image,
					     u8 *fwbuf, u8 *event,
					     int event_len, u32 patch_len,
					     int offset)
{
	int ret = 0;
	u32 cur_len = 0;
	...
	while (1) {
		...
		if (sent_len > 0) {
			...
			ret = btmtk_main_send_cmd(bdev, image,
						  sent_len + PATCH_HEADER_SIZE,
						  event, event_len, delay,
						  retry, BTMTK_TX_ACL_FROM_DRV);
			if (ret < 0) {
...
void btmtk_send_hw_err_to_host(struct btmtk_dev *bdev)
{
	...
	BTMTK_ERR("%s reset_stack_flag = %d!!", __func__,
		  main_info.reset_stack_flag);
	if (main_info.reset_stack_flag) {
		...
		if (skb == NULL) {
			...
			BTMTK_DBG_RAW(skb->data, skb->len,
				      "%s: hw err event:", __func__);
			hci_recv_frame(bdev->hdev, skb);
		}
	}
}

static int btmtk_send_fw_rom_patch_79xx(struct btmtk_dev *bdev, u8 *fwbuf,
					bool patch_flag)
{
	u8 *pos;
	int loop_count = 0;
	...
	u8 event[LD_PATCH_EVT_LEN] = {
		0x04, 0xE4, 0x05, 0x02, 0x01, 0x01, 0x00, 0x00
	}; /* event[7] is status*/
#if LD_PATCH_TIME
	...
	globalDescr =
		(struct _Global_Descr *)(fwbuf + FW_ROM_PATCH_HEADER_SIZE);
	...
	do {
		sectionMap = (struct _Section_Map *)(fwbuf +
						     FW_ROM_PATCH_HEADER_SIZE +
						     FW_ROM_PATCH_GD_SIZE +
						     FW_ROM_PATCH_SEC_MAP_SIZE *
							     loop_count);
		...
		if (patch_flag) {
			...
			dma_flag = be2cpu32(sectionMap->bin_info_spec
						    .u4DLModeCrcType) &
				   0xFF;
		} else {
			/* BT is little-endian */
			 ...
			 */
			dma_flag = le2cpu32(sectionMap->bin_info_spec
						    .u4DLModeCrcType) &
				   0xFF;
		}
		BTMTK_INFO(
		...
		if (dl_size > 0) {
			retry = 20;
			do {
				patch_status = btmtk_send_wmt_download_cmd(
					bdev, pos, 0, event,
					LD_PATCH_EVT_LEN - 1, sectionMap, 0,
					dma_flag, patch_flag);
				BTMTK_INFO("%s: patch_status %d", __func__,
					   patch_status);

				if (patch_status > PATCH_READY ||
				    patch_status == PATCH_ERR) {
					BTMTK_ERR("%s: patch_status error",
						  __func__);
					ret = -1;
					goto err;
					...
					BTMTK_INFO(
						...
						__func__, loop_count);
					goto next_section;
				} else if (patch_status ==
					   PATCH_IS_DOWNLOAD_BY_OTHER) {
					msleep(100);
					retry--;
				} else if (patch_status ==
					   PATCH_NEED_DOWNLOAD) {
			...
			if (patch_status == PATCH_IS_DOWNLOAD_BY_OTHER) {
				BTMTK_WARN(
					"%s: Hold by another fun more than 2 seconds",
					__func__);
				ret = -1;
				goto err;
			}
			...
			if (dma_flag == PATCH_DOWNLOAD_USING_DMA &&
			    main_info.hif_hook.dl_dma) {
				/* using DMA to download fw patch*/
				ret = main_info.hif_hook.dl_dma(bdev, pos,
								fwbuf, dl_size,
								section_offset);
				if (ret < 0) {
					BTMTK_ERR(
						"%s: btmtk_load_fw_patch_using_dma failed!",
						__func__);
					goto err;
				}
			} else {
				/* using legacy wmt cmd to download fw patch */
				ret = btmtk_load_fw_patch_using_wmt_cmd(
					bdev, pos, fwbuf, event,
					LD_PATCH_EVT_LEN - 1, dl_size,
					section_offset);
				if (ret < 0) {
					BTMTK_ERR(
						"%s: btmtk_load_fw_patch_using_wmt_cmd failed!",
						__func__);
					goto err;
				}
			}
		 ...
		 */
			...
			if (loop_count == 0) {
				...
				if (tv_bgf.tv_sec != 0 ||
				    tv_bgf.tv_nsec / 1000 != 0) {
					if (tv_ilm.tv_sec >= tv_bgf.tv_sec)
						dlt_dma = (tv_ilm.tv_sec -
							   tv_bgf.tv_sec) *
							  1000;
					else
						dlt_dma = (~(tv_bgf.tv_sec -
							     tv_ilm.tv_sec) +
							   1) *
							  1000;

					dlt_dma += (tv_ilm.tv_nsec / 1000 -
						    tv_bgf.tv_nsec / 1000) /
						   1000;
				}
				if (tv_ilm.tv_sec >= tv_start.tv_sec)
					dlt_all = (tv_ilm.tv_sec -
						   tv_start.tv_sec) *
						  1000;
				else
					dlt_all = (~(tv_start.tv_sec -
						     tv_ilm.tv_sec) +
						   1) *
						  1000;

				dlt_all += (tv_ilm.tv_nsec / 1000 -
					    tv_start.tv_nsec / 1000) /
					   1000;

				BTMTK_INFO(
					"LD PATCH 1 tv_start: tv_sec:%zu, tv_usec:%zu.",
					tv_start.tv_sec,
					tv_start.tv_nsec / 1000);
				BTMTK_INFO(
					"LD PATCH 2 tv_bgf: tv_sec:%zu, tv_usec:%zu.",
					tv_bgf.tv_sec, tv_bgf.tv_nsec / 1000);
				BTMTK_INFO(
					"LD PATCH 3 tv_ilm: tv_sec:%zu, tv_usec:%zu.",
					tv_ilm.tv_sec, tv_ilm.tv_nsec / 1000);

				if (dlt_dma != 0)
					BTMTK_INFO(
						"LD PATCH time: ILM_DMA:%ums, ALL:%ums.",
						dlt_dma, dlt_all);
				else
					BTMTK_INFO(
						"LD PATCH time: ILM_DMA:%ums.",
						dlt_all);
			}
		}
#endif
...
int btmtk_load_rom_patch_79xx(struct btmtk_dev *bdev, bool patch_flag)
{
	...
	if (patch_flag) {
		if (bdev->flavor)
			...
			(void)snprintf(bdev->rom_patch_bin_file_name,
				       MAX_BIN_FILE_NAME_LEN,
				       "WIFI_MT%04x_patch_mcu_1a_%x_hdr.bin",
				       bdev->chip_id & 0xffff,
				       (bdev->fw_version & 0xff) + 1);
		else
			(void)snprintf(bdev->rom_patch_bin_file_name,
				       MAX_BIN_FILE_NAME_LEN,
	...
	btmtk_load_code_from_bin(&rom_patch, bdev->rom_patch_bin_file_name,
				 NULL, &rom_patch_len, 10);

	if (!rom_patch) {
		BTMTK_ERR(
			"%s: please assign a rom patch(/etc/firmware/%s)or(/lib/firmware/%s)",
			__func__, bdev->rom_patch_bin_file_name,
...
int btmtk_load_rom_patch_766x(struct btmtk_dev *bdev)
{
	...
	int retry = 20;
	u8 *pos = NULL;
	u8 event[LD_PATCH_EVT_LEN] = { 0x04, 0xE4, 0x05, 0x02,
				       0x01, 0x01, 0x00, 0x00 };
	u8 *rom_patch = NULL;
	unsigned int rom_patch_len = 0;
	...
	do {
		...
		if (patch_status > MT766X_PATCH_NEED_DOWNLOAD ||
		    patch_status == PATCH_ERR) {
...
/* TODO*/
	...
	if (patch_status == PATCH_IS_DOWNLOAD_BY_OTHER) {
		BTMTK_WARN("%s: Hold by another fun more than 2 seconds",
			   __func__);
		ret = -1;
		goto err1;
	}
...
int btmtk_load_rom_patch(struct btmtk_dev *bdev)
{
	...
	if (is_mt7663(bdev->chip_id))
		...
		if (err < 0) {
			BTMTK_ERR(
				"%s: btmtk_load_rom_patch_79xx bt patch failed!",
				__func__);
			return err;
		}
		...
		if (err < 0) {
			BTMTK_WARN(
				"%s: btmtk_load_rom_patch_79xx wifi patch failed!",
				__func__);
			err = 0;
		}
#endif
	...
	for (i = 0; i < btmtk_intf_num; i++) {
		...
		if (g_bdev[i]->hdev == NULL) {
			if (i == 0)
				...
				g_bdev[i]->dongle_index =
					g_bdev[i - 1]->dongle_index + 1;
			...
			g_bdev[i]->cif_state =
				(struct btmtk_cif_state *)g_cif_state;
			break;
		}
	}
...
void btmtk_release_dev(struct btmtk_dev *bdev)
{
	...
	if (tmp_bdev != NULL) {
		for (i = 0; i < btmtk_intf_num; i++) {
			 ...
			 * Identified chip_id from cap_init.
			 */
			if (memcmp(tmp_bdev, g_bdev[i], sizeof(*tmp_bdev)) ==
			    0) {
...
static int btmtk_send_hci_reset_cmd(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_main_send_cmd(bdev, cmd, HCI_RESET_CMD_LEN, event,
				  HCI_RESET_EVT_LEN, 0, 0,
...
int btmtk_send_wmt_reset(struct btmtk_dev *bdev)
{
	...
	u8 cmd[WMT_RESET_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x05, 0x01,
				      0x07, 0x01, 0x00, 0x04 };
	/* To-Do, for event check */
	u8 event[WMT_RESET_EVT_LEN] = { 0x04, 0xE4, 0x05, 0x02,
					0x07, 0x01, 0x00, 0x00 };
	int ret = -1;
	...
	if (bdev->power_state == BTMTK_DONGLE_STATE_POWER_OFF) {
		ret = btmtk_main_send_cmd(bdev, cmd, WMT_RESET_CMD_LEN, event,
					  WMT_RESET_EVT_LEN, DELAY_TIMES, 0,
					  BTMTK_TX_CMD_FROM_DRV);
	...
	if (bdev->power_state != BTMTK_DONGLE_STATE_POWER_ON) {
		BTMTK_WARN("%s: end of Incorrect state:%d", __func__,
			   bdev->power_state);
		return -EBADFD;
	}
...
int btmtk_send_wmt_power_on_cmd(struct btmtk_dev *bdev)
{
	...
	u8 cmd[WMT_POWER_ON_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x06, 0x01,
					 0x06, 0x02, 0x00, 0x00, 0x01 };
	u8 event[WMT_POWER_ON_EVT_HDR_LEN] = {
		0x04, 0xE4, 0x05, 0x02, 0x06, 0x01, 0x00
	}; /* event[7] is key */
...
retry_again:
	...
	ret = btmtk_main_send_cmd(bdev, cmd, WMT_POWER_ON_CMD_LEN, event,
				  WMT_POWER_ON_EVT_HDR_LEN, WMT_DELAY_TIMES,
				  RETRY_TIMES, BTMTK_TX_CMD_FROM_DRV);
	if (ret < 0) {
		...
		default:
			BTMTK_WARN(
				"%s: Unknown result: %02X", __func__,
				bdev->io_buf[WMT_POWER_ON_EVT_RESULT_OFFSET]);
			bdev->power_state = BTMTK_DONGLE_STATE_ERROR;
			ret = -1;
			break;
		}
...
{
	/* Support 7668 and 7663 and 7961 */
	u8 cmd[WMT_POWER_OFF_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x06, 0x01,
					  0x06, 0x02, 0x00, 0x00, 0x00 };
	/* To-Do, for event check */
	u8 event[WMT_POWER_OFF_EVT_HDR_LEN] = { 0x04, 0xE4, 0x05, 0x02,
						0x06, 0x01, 0x00 };
	int ret = -1;
	...
	ret = btmtk_main_send_cmd(bdev, cmd, WMT_POWER_OFF_CMD_LEN, event,
				  WMT_POWER_OFF_EVT_HDR_LEN, DELAY_TIMES,
				  RETRY_TIMES, BTMTK_TX_CMD_FROM_DRV);
	if (ret < 0) {
		BTMTK_ERR("%s: failed(%d)", __func__, ret);
		bdev->power_state = BTMTK_DONGLE_STATE_ERROR;
...
{
	u8 dft_enable_cmd[PICUS_ENABLE_CMD_LEN] = { 0x01, 0x5D, 0xFC, 0x04,
						    0x00, 0x00, 0x02, 0x02 };
	u8 *enable_cmd = NULL;
	u8 enable_event[PICUS_ENABLE_EVT_HDR_LEN] = { 0x04, 0x0E, 0x08,
						      0x01, 0x5D, 0xFC,
						      0x00, 0x00, 0x00 };
	int enable_len = 0;
	int ret = -1; /* if successful, 0 */
	...
	if (is_mt7922(bdev->chip_id) || is_mt7961(bdev->chip_id))
		ret = btmtk_main_send_cmd(bdev, enable_cmd, enable_len,
					  enable_event,
					  PICUS_ENABLE_EVT_HDR_LEN, DELAY_TIMES,
					  RETRY_TIMES, BTMTK_TX_PKT_FROM_HOST);
	else
...
{
	u8 dft_disable_cmd[PICUS_DISABLE_CMD_LEN] = { 0x01, 0x5D, 0xFC, 0x04,
						      0x00, 0x00, 0x02, 0x00 };
	u8 dft_disable_event[PICUS_DISABLE_EVT_HDR_LEN] = { 0x04, 0x0E, 0x08,
							    0x01, 0x5D, 0xFC,
							    0x00, 0x00, 0x00 };
	int ret = -1; /* if successful, 0 */
	...
	if (is_mt7922(bdev->chip_id) || is_mt7961(bdev->chip_id))
		ret = btmtk_main_send_cmd(
			bdev, dft_disable_cmd, PICUS_DISABLE_CMD_LEN,
...
int btmtk_load_fw_cfg_setting(char *block_name,
			      struct fw_cfg_struct *save_content, int counter,
			      u8 *searchcontent,
			      enum fw_cfg_index_len index_length)
{
	return 0;
#if 0
	int ret = 0, i = 0;
				       ...
				       struct btmtk_dev *bdev)
{
	...
	*code_len = 0;
	...
	BTMTK_INFO("%s: begin setting_file_name = %s", __func__,
		   setting_file_name);
	err = request_firmware(&fw_entry, setting_file_name, dev);
	if (err != 0 || fw_entry == NULL) {
		BTMTK_INFO(
			"%s: request_firmware fail, maybe file not exist, err = %d, fw_entry = %p",
	...
	BTMTK_INFO("%s: setting file request_firmware size %zu success",
		   __func__, fw_entry->size);
	if (bdev->setting_file != NULL) {
		kfree(bdev->setting_file);
		bdev->setting_file = NULL;
	}
	bdev->setting_file = kzalloc(
		fw_entry->size + 1, GFP_KERNEL); /* alloc setting file memory */
	if (bdev->setting_file == NULL) {
		BTMTK_ERR("%s: kzalloc size %zu failed!!", __func__,
			  fw_entry->size);
		release_firmware(fw_entry);
		err = -1;
		goto end;
	...
	*code_len = fw_entry->size;
	...
	BTMTK_INFO("%s: setting_file len (%d) assign done", __func__,
		   *code_len);
end:
	return err;
}

static bool btmtk_parse_bt_cfg_file(char *item_name, char *text,
				    u8 *searchcontent)
{
	bool ret = true;
	int temp_len = 0;
	...
	memset(search, 0, SEARCH_LEN);
	(void)snprintf(search, SEARCH_LEN, "%s",
		       item_name); /* EX: SUPPORT_UNIFY_WOBLE */
				   ...
				   u8 *searchcontent, struct btmtk_dev *bdev)
{
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_unify_woble);
		BTMTK_INFO("%s: bt_cfg_content->support_unify_woble = %d",
			   __func__, bt_cfg_content->support_unify_woble);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_UNIFY_WOBLE);
	}
	...
	if (ret) {
		if (kstrtoul(text, 10, &text_value) == 0)
			...
			BTMTK_WARN("%s: kstrtoul failed %s!", __func__,
				   BT_UNIFY_WOBLE_TYPE);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_UNIFY_WOBLE_TYPE);
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_woble_by_eint);
		BTMTK_INFO("%s: bt_cfg_content->support_woble_by_eint = %d",
			   __func__, bt_cfg_content->support_woble_by_eint);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_WOBLE_BY_EINT);
	}
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_dongle_reset);
		BTMTK_INFO("%s: bt_cfg_content->support_dongle_reset = %d",
			   __func__, bt_cfg_content->support_dongle_reset);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_RESET_DONGLE);
	}
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_full_fw_dump);
		BTMTK_INFO("%s: bt_cfg_content->support_full_fw_dump = %d",
			   __func__, bt_cfg_content->support_full_fw_dump);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_FULL_FW_DUMP);
	}
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_woble_wakelock);
		BTMTK_INFO("%s: bt_cfg_content->support_woble_wakelock = %d",
			   __func__, bt_cfg_content->support_woble_wakelock);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_WOBLE_WAKELOCK);
	}

	ret = btmtk_parse_bt_cfg_file(BT_WOBLE_FOR_BT_DISABLE, text,
				      searchcontent);
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_woble_for_bt_disable);
		BTMTK_INFO(
			"%s: bt_cfg_content->support_woble_for_bt_disable = %d",
			__func__, bt_cfg_content->support_woble_for_bt_disable);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_WOBLE_FOR_BT_DISABLE);
	}

	ret = btmtk_parse_bt_cfg_file(BT_RESET_STACK_AFTER_WOBLE, text,
				      searchcontent);
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->reset_stack_after_woble);
		BTMTK_INFO("%s: bt_cfg_content->reset_stack_after_woble = %d",
			   __func__, bt_cfg_content->reset_stack_after_woble);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_auto_picus);
		BTMTK_INFO("%s: bt_cfg_content->support_auto_picus = %d",
			   __func__, bt_cfg_content->support_auto_picus);
		if (bt_cfg_content->support_auto_picus == true) {
			ret = btmtk_load_fw_cfg_setting(
				BT_AUTO_PICUS_FILTER,
				&bt_cfg_content->picus_filter, 1, searchcontent,
				FW_CFG_INX_LEN_NONE);
			if (ret)
			...
			ret = btmtk_load_fw_cfg_setting(
				BT_AUTO_PICUS_ENABLE,
				&bt_cfg_content->picus_enable, 1, searchcontent,
				FW_CFG_INX_LEN_NONE);
			if (ret)
				BTMTK_WARN("%s: search item %s is invalid!",
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_picus_to_host);
		BTMTK_INFO("%s: bt_cfg_content->support_picus_to_host = %d",
			   __func__, bt_cfg_content->support_picus_to_host);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_PICUS_TO_HOST);
	}
	...
	if (ret) {
		btmtk_bt_cfg_item_value_to_bool(
			text, &bt_cfg_content->support_bt_single_sku);
		BTMTK_INFO("%s: bt_cfg_content->support_bt_single_sku = %d",
			   __func__, bt_cfg_content->support_bt_single_sku);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_SINGLE_SKU);
	}

	ret = btmtk_load_fw_cfg_setting(BT_PHASE1_WMT_CMD,
					bt_cfg_content->phase1_wmt_cmd,
					PHASE1_WMT_CMD_COUNT, searchcontent,
					FW_CFG_INX_LEN_3);
	if (ret)
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_PHASE1_WMT_CMD);

	ret = btmtk_load_fw_cfg_setting(BT_VENDOR_CMD,
					bt_cfg_content->vendor_cmd,
					VENDOR_CMD_COUNT, searchcontent,
					FW_CFG_INX_LEN_3);
	if (ret)
		BTMTK_WARN("%s: search item %s is invalid!", __func__,
			   BT_VENDOR_CMD);
...
static void btmtk_load_bt_cfg(char *cfg_name, struct device *dev,
			      struct btmtk_dev *bdev)
{
	u32 code_len = 0;
...
{
	u8 cmd[SET_STP_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x09, 0x01, 0x04, 0x05,
				    0x00, 0x03, 0x11, 0x0E, 0x00, 0x00 };
	u8 event[SET_STP_EVT_LEN] = { 0x04, 0xE4, 0x06, 0x02, 0x04,
				      0x02, 0x00, 0x00, 0x03 };
	int ret = 0;

	ret = btmtk_main_send_cmd(bdev, cmd, SET_STP_CMD_LEN, event,
				  SET_STP_EVT_LEN, 0, 0, BTMTK_TX_CMD_FROM_DRV);
...
{
	u8 cmd[SET_STP1_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x0C, 0x01, 0x08,
				     0x08, 0x00, 0x02, 0x01, 0x00, 0x01,
				     0x08, 0x00, 0x00, 0x80 };
	u8 event[SET_STP1_EVT_LEN] = { 0x04, 0xE4, 0x10, 0x02, 0x08, 0x0C, 0x00,
				       0x00, 0x00, 0x00, 0x01, 0x08, 0x00, 0x00,
				       0x80, 0x63, 0x76, 0x00, 0x00 };
	int ret = 0;

	ret = btmtk_main_send_cmd(bdev, cmd, SET_STP1_CMD_LEN, event,
				  SET_STP1_EVT_LEN, 0, 0,
				  BTMTK_TX_CMD_FROM_DRV);
...
static int btmtk_send_hci_tci_set_sleep_cmd_766x(struct btmtk_dev *bdev)
{
	u8 cmd[SET_SLEEP_CMD_LEN] = { 0x01, 0x7A, 0xFC, 0x07, 0x05, 0x40,
				      0x06, 0x40, 0x06, 0x00, 0x00 };
	u8 event[SET_SLEEP_EVT_LEN] = {
	...
	ret = btmtk_main_send_cmd(bdev, cmd, SET_SLEEP_CMD_LEN, event,
				  SET_SLEEP_EVT_LEN, 0, 0,
				  BTMTK_TX_PKT_FROM_HOST);
...
int btmtk_cap_init(struct btmtk_dev *bdev)
{
	...
	if (is_mt7922(bdev->chip_id) || is_mt7961(bdev->chip_id)) {
		...
		main_info.hif_hook.reg_read(bdev, FW_VERSION,
					    &bdev->fw_version);
	} else {
	...
	if ((bdev->fw_version & 0xff) == 0xff) {
		BTMTK_ERR("%s: failed, wrong FW version : 0x%x !", __func__,
			  bdev->fw_version);
		return -1;
	}
...
static int btmtk_send_vendor_cfg(struct btmtk_dev *bdev)
{
	...
	for (index = 0; index < VENDOR_CMD_COUNT; index++) {
		if (bdev->bt_cfg.vendor_cmd[index].content &&
			...
			ret = btmtk_main_send_cmd(
				bdev, bdev->bt_cfg.vendor_cmd[index].content,
				bdev->bt_cfg.vendor_cmd[index].length, event,
				EVT_HDR_LEN, 0, 0, BTMTK_TX_PKT_FROM_HOST);
			if (ret < 0) {
				BTMTK_ERR(
					"%s: Send vendor cmd failed(%d)! Index: %d",
					__func__, ret, index);
				goto exit;
			}
	...
	for (index = 0; index < PHASE1_WMT_CMD_COUNT; index++) {
		if (bdev->bt_cfg.phase1_wmt_cmd[index].content &&
			...
			ret = btmtk_main_send_cmd(
				...
				bdev->bt_cfg.phase1_wmt_cmd[index].length,
				event, EVT_HDR_LEN, DELAY_TIMES, RETRY_TIMES,
				BTMTK_TX_CMD_FROM_DRV);
			if (ret < 0) {
				BTMTK_ERR(
					"%s: Send phase1 wmt cmd failed(%d)! Index: %d",
					__func__, ret, index);
				goto exit;
			}
...
int btmtk_send_init_cmds(struct btmtk_dev *bdev)
{
	...
	if (ret < 0) {
		if (bdev->power_state != BTMTK_DONGLE_STATE_POWER_ON) {
			BTMTK_ERR("%s, btmtk_send_wmt_power_on_cmd failed!",
				  __func__);
...
int btmtk_send_assert_cmd(struct btmtk_dev *bdev)
{
	int ret = 0;
	int state;
	u8 cmd[ASSERT_CMD_LEN] = { 0x01, 0x6F, 0xFC, 0x05, 0x01,
				   0x02, 0x01, 0x00, 0x08 };
	struct sk_buff *skb = NULL;
	...
	ret = main_info.hif_hook.send_cmd(bdev, skb, WMT_DELAY_TIMES,
					  RETRY_TIMES,
					  (int)BTMTK_TX_CMD_FROM_DRV);
	if (ret < 0)
...
static int btmtk_send_txpower_cmd(struct btmtk_dev *bdev)
{
	...
	u8 cmd[TXPOWER_CMD_LEN] = { 0x01, 0x2C, 0xFC, 0x0C, 0x00, 0x00,
	...
	ret = btmtk_main_send_cmd(bdev, cmd, TXPOWER_CMD_LEN, event,
				  TXPOWER_EVT_LEN, 0, 0, BTMTK_TX_CMD_FROM_DRV);
...
static int btmtk_set_power_value(char *str, int resolution, int is_edr)
{
	...
	if (resolution == RES_DOT_25) {
		...
		if (strstr(str, ".")) {
			...
			if (ret < 0)
				return ret;
			if (decimal != 25 && decimal != 75 && decimal != 5 &&
			    decimal != 50)
				return ERR_PWR;
			if (decimal == 5)
				decimal = 50;
		...
		if (is_edr) {
			...
			if (power >= EDR_MIN_LV9_R2)
			   ...
			   (power > BLE_MAX_R2 || power < BLE_MIN_R2))
			return ERR_PWR;
		...
		if (is_edr) {
			...
			if (power >= EDR_MIN_LV9_R1)
			   ...
			   (power > BLE_MAX_R1 || power < BLE_MIN_R1))
			return ERR_PWR;
...
static void btmtk_requset_country_cb(const struct firmware *fw, void *context)
{
	...
	BTMTK_INFO("%s request %s success", __func__,
		...
		if (read_next) {
			if (strncmp(ptr, "[BT1]", 5) == 0) {
				...
				if (ret != CHECK_SINGLE_SKU_ALL) {
					BTMTK_ERR("Parse power fail, ret = %d",
						  ret);
					break;
				}
...
void btmtk_set_country_code_from_wifi(char *code)
{
	...
	if (strlen(code) == COUNTRY_CODE_LEN) {
		...
		memcpy(main_info.PWS.country_code, code,
...
static int bt_close(struct hci_dev *hdev)
{
	...
	struct btmtk_dev *bdev = NULL;
	struct btmtk_btif_dev *cif_dev =
		(struct btmtk_btif_dev *)g_sbdev->cif_dev;
	...
	if (state != BTMTK_STATE_WORKING && state != BTMTK_STATE_STANDBY) {
		 ...
		 */
		BTMTK_WARN("%s: not in working state and standby state(%d).",
			   __func__, state);
		goto unlock;
	}
	...
	if (!is_mt66xx(bdev->chip_id)) {
		...
		if (state != BTMTK_STATE_WORKING &&
			 ...
			 */
			BTMTK_WARN(
				"%s: not in working state and standby state(%d).",
				__func__, state);
			goto unlock;
		}
		...
		if (state != BTMTK_STATE_STANDBY) {
			...
			if (ret < 0) {
				BTMTK_ERR("%s, btmtk_send_deinit_cmds failed",
					  __func__);
				goto unlock;
			}
		}
...
unlock:
	...
	BTMTK_INFO("%s: end, reset_stack_flag = %d", __func__,
		   main_info.reset_stack_flag);
	return 0;
}
...
static int bt_send_frame(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (main_info.reset_stack_flag) {
		BTMTK_WARN("%s: reset_stack_flag (%d)!", __func__,
			   main_info.reset_stack_flag);
		ret = -EFAULT;
		goto exit;
	}
	...
	if (!is_mt66xx(bdev->chip_id))
		btmtk_dispatch_fwlog_bluetooth_kpi(bdev, skb->data, skb->len,
	...
	if (!is_mt66xx(bdev->chip_id)) {
		...
		if (memcmp(skb->data, main_info.read_iso_packet_size_cmd,
			...
			bdev->iso_threshold =
				skb->data[READ_ISO_PACKET_SIZE_CMD_HDR_LEN] +
				(skb->data[READ_ISO_PACKET_SIZE_CMD_HDR_LEN + 1]
				 << 8);
			BTMTK_INFO("%s: Ble iso pkt size is %d", __func__,
				   bdev->iso_threshold);
		}
		...
		if (hci_skb_pkt_type(skb) == HCI_COMMAND_PKT) {
			...
			if (skb->len == FW_COREDUMP_CMD_LEN &&
			    !memcmp(skb->data, fw_coredump_cmd,
				    FW_COREDUMP_CMD_LEN)) {
				BTMTK_INFO(
					"%s: Dongle FW Assert Triggered by BT Stack!",
					__func__);
				btmtk_hci_snoop_print_to_log();
			} else if (skb->len == HCI_RESET_CMD_LEN &&
				   !memcmp(skb->data, reset_cmd,
					   HCI_RESET_CMD_LEN))
				BTMTK_INFO(
		...
		ret = main_info.hif_hook.send_cmd(bdev, skb, 0, 0,
						  (int)BTMTK_TX_PKT_FROM_HOST);
		if (ret < 0)
			BTMTK_ERR("%s failed!!", __func__);
	} else {
		ret = main_info.hif_hook.send_cmd(bdev, skb, 0, 5,
						  (int)BTMTK_TX_PKT_FROM_HOST);
	}
...
static void btmtk_rx_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&bdev->rx_q))) {
		...
		if (hci_skb_pkt_type(skb) == HCI_EVENT_PKT) {
			...
			if (skb->data[0] == 0x3E)
				btmtk_hci_snoop_save_adv_event(skb->len,
							       skb->data);
			else
		...
		if (is_mt66xx(bdev->chip_id)) {
			...
			/* for bluetooth kpi */
			btmtk_dispatch_fwlog_bluetooth_kpi(
				bdev, skb->data, skb->len,
				hci_skb_pkt_type(skb));
		...
		if (err < 0) {
			if (err != -ENXIO)
				BTMTK_ERR("%s btmtk_rx_work failed, err = %d",
					  __func__, err);
			return;
		}
	}
...
void btmtk_free_hci_device(struct btmtk_dev *bdev, int hci_bus_type)
{
	...
	if (fstate == BTMTK_FOPS_STATE_OPENED ||
		...
		BTMTK_WARN("%s: fstate = %d , set reset_stack_flag", __func__,
			   fstate);
		if (main_info.reset_stack_flag == HW_ERR_NONE)
...
int btmtk_allocate_hci_device(struct btmtk_dev *bdev, int hci_bus_type)
{
	...
	bdev->workqueue = alloc_workqueue(
		"BTMTK_RX_WQ", WQ_HIGHPRI | WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	if (!bdev->workqueue) {
...
static int btmtk_main_allocate_memory(struct btmtk_dev *bdev)
{
	...
	if (bdev->rom_patch_bin_file_name == NULL) {
		bdev->rom_patch_bin_file_name =
			kzalloc(MAX_BIN_FILE_NAME_LEN, GFP_KERNEL);
		if (!bdev->rom_patch_bin_file_name) {
			BTMTK_ERR(
				"%s: alloc memory fail (bdev->rom_patch_bin_file_name)",
				__func__);
			goto end;
		}
	}
	...
	if (bdev->io_buf == NULL) {
		...
		if (!bdev->io_buf) {
			BTMTK_ERR("%s: alloc memory fail (bdev->io_buf)",
				  __func__);
			goto err2;
		}
	}
	...
	if (bdev->bt_cfg_file_name == NULL) {
		bdev->bt_cfg_file_name =
			kzalloc(MAX_BIN_FILE_NAME_LEN, GFP_KERNEL);
		if (!bdev->bt_cfg_file_name) {
			BTMTK_ERR(
				"%s: alloc memory fail (bdev->bt_cfg_file_name)",
				__func__);
			goto err1;
		}
	}
	...
	if (bdev->country_file_name == NULL) {
		bdev->country_file_name =
			kzalloc(MAX_BIN_FILE_NAME_LEN, GFP_KERNEL);
		if (!bdev->country_file_name) {
			BTMTK_ERR(
				"%s: alloc memory fail (bdev->country_file_name)",
				__func__);
			goto err0;
		}
	}
...
static int btmtk_reboot_notify(struct notifier_block *nb, unsigned long event,
			       void *unused)
{
	int ret = 0;
	int i = 0;
	...
	if (event == SYS_RESTART) {
		...
		for (i = 0; i < btmtk_intf_num; i++) {
			...
			if (g_bdev[i]->hdev != NULL) {
				...
				fstate = btmtk_fops_get_state(bdev);
				if (fstate != BTMTK_FOPS_STATE_OPENED) {
					BTMTK_WARN("%s: fops is not opened(%d)",
						   __func__, fstate);
					continue;
				}
				...
				if (BTMTK_CIF_IS_NULL(bdev, cif_event)) {
					/* Error */
					BTMTK_WARN("%s parameter is NULL",
						   __func__);
					continue;
				}
				...
				/* Set Entering state */
				btmtk_set_chip_state((void *)bdev,
						     cif_state->ops_enter);

				btmtk_fops_set_state(bdev,
						     BTMTK_FOPS_STATE_CLOSING);
				...
				if (ret < 0)
					BTMTK_ERR(
						"%s, btmtk_send_deinit_cmds failed",
						__func__);
#if 0
				/* Flush RX works */
				flush_work(&bdev->rx_work);
				...
				if (main_info.hif_hook.cif_mutex_unlock)
					main_info.hif_hook.cif_mutex_unlock(
						bdev);

				btmtk_fops_set_state(bdev,
						     BTMTK_FOPS_STATE_CLOSED);

				/* Set End/Error state */
				if (ret == 0)
					btmtk_set_chip_state(
						(void *)bdev,
						cif_state->ops_end);
				else
					btmtk_set_chip_state(
...
static int main_init(void)
{
	...
	if (btmtk_intf_num < BT_MCU_MINIMUM_INTERFACE_NUM) {
		...
		BTMTK_WARN("%s minimum interface is %d", __func__,
			   btmtk_intf_num);
	}
...
static int main_exit(void)
{
	...
	BTMTK_INFO("%s: Unregister reboot_notifier callback success.",
		   __func__);
	/* Is it necessary? bt_close will be called by reboot. */
#if (USE_DEVICE_NODE != 1)
