...
struct scp_clk_hw scp_clk_hw_regs[MAX_SCP_CLK_VERSION] = {
	[SCP_CLK_VER_1] = { REG_DEFINE(clk_high_en, 0x4, 0x1, 1) REG_DEFINE(
		ulposc2_en, 0x6C, 0x1, 5) REG_DEFINE(ulposc2_cg, 0x5C, 0x1, 1)
				    REG_DEFINE(sel_clk, 0x0, 0xF, 8) }
};

struct scp_pmic_regs scp_pmic_hw_regs[MAX_SCP_DVFS_CHIP_HW] = {
	[MT6853] = { REG_DEFINE_WITH_INIT(sshub_op_mode, 0x1520, 0x1, 11, 0, 1)
			     REG_DEFINE_WITH_INIT(sshub_op_en, 0x1514, 0x1, 11,
						  1, 1)
				     REG_DEFINE_WITH_INIT(sshub_op_cfg, 0x151a,
							  0x1, 11, 1, 1) },
	[MT6873] = { REG_DEFINE_WITH_INIT(
		sshub_op_mode, 0x15a0, 0x1, 11, 0,
		1) REG_DEFINE_WITH_INIT(sshub_op_en, 0x1594, 0x1, 11, 1,
					1) REG_DEFINE_WITH_INIT(sshub_op_cfg,
								0x159a, 0x1, 11,
								1, 1)
			     REG_DEFINE_WITH_INIT(sshub_buck_en, 0x15aa, 0x1, 0,
						  1, 0)
				     REG_DEFINE_WITH_INIT(sshub_ldo_en, 0x1f28,
							  0x1, 0, 0, 0)
					     REG_DEFINE_WITH_INIT(pmrc_en,
								  0x1ac, 0x1, 2,
								  0, 1) },
	[MT6893] = { REG_DEFINE_WITH_INIT(
		sshub_op_mode, 0x15a0, 0x1, 11, 0,
		1) REG_DEFINE_WITH_INIT(sshub_op_en, 0x1594, 0x1, 11, 1,
					1) REG_DEFINE_WITH_INIT(sshub_op_cfg,
								0x159a, 0x1, 11,
			     ...
			     REG_DEFINE_WITH_INIT(sshub_buck_en, 0x15aa, 0x1, 0,
				     ...
				     REG_DEFINE_WITH_INIT(sshub_ldo_en, 0x1f28,
					     ...
					     REG_DEFINE_WITH_INIT(pmrc_en,
								  0x1ac, 0x1, 2,
								  0, 1) },
	[MT6833] = { REG_DEFINE_WITH_INIT(sshub_op_mode, 0x1520, 0x1, 11, 0, 1)
			     REG_DEFINE_WITH_INIT(sshub_op_en, 0x1514, 0x1, 11,
						  1, 1)
				     REG_DEFINE_WITH_INIT(sshub_op_cfg, 0x151a,
							  0x1, 11, 1, 1) },
...
static void slp_ipi_init(void)
{
	...
	ret = mtk_ipi_register(&scp_ipidev, IPI_OUT_C_SLEEP_0, NULL, NULL,
			       &scp_ipi_ackdata0);
	if (ret) {
	...
	if (dvfs.core_nums == 2) {
		ret = mtk_ipi_register(&scp_ipidev, IPI_OUT_C_SLEEP_1, NULL,
				       NULL, &scp_ipi_ackdata1);
		if (ret)
...
static int scp_get_vcore_table(unsigned int gear)
{
	...
	arm_smccc_smc(MTK_SIP_SCP_DVFS_CONTROL, VCORE_ACQUIRE, gear, 0, 0, 0, 0,
		      0, &res);
	if (!(res.a0))
		return res.a1;
...
int scp_resource_req(unsigned int req_type)
{
	...
	arm_smccc_smc(MTK_SIP_SCP_DVFS_CONTROL, RESOURCE_REQ, req_type, 0, 0, 0,
		      0, 0, &res);
	if (!res.a0)
...
static int scp_set_pmic_vcore(unsigned int cur_freq)
{
	...
	if (idx >= 0 && idx < dvfs.scp_opp_nums) {
		...
		ret_vc = regulator_set_voltage(
			reg_vcore, dvfs.opp[idx].tuned_vcore,
			dvfs.opp[dvfs.scp_opp_nums - 1].vcore + 100000);

		ret_vs = regulator_set_voltage(
			reg_vsram, dvfs.opp[idx].vsram,
	...
	if (ret_vc != 0 || ret_vs != 0) {
		...
		pr_notice(
			"[%s]: ERROR: scp vcore/vsram setting error, (%d, %d)\n",
...
int scp_request_freq_vcore(void)
{
	...
	if (scp_current_freq != scp_expected_freq) {
		...
		scp_resource_req(SCP_REQ_26M | SCP_REQ_INFRA | SCP_REQ_SYSPLL);
		...
		do {
			...
			if (timeout <= 0) {
				pr_notice(
					"set freq fail, current(%d) != expect(%d)\n",
...
int scp_request_freq_vlp(void)
{
	...
	if (last_scp_expected_freq == scp_expected_freq) {
		pr_debug(
			"[SCP] Skip DFS: resending the same freq request: %dMhz\n",
	...
	if (pll_ctrl_flag == PLL_ENABLE) {
		if (!dvfs.pre_mux_en) {
			ret = clk_prepare_enable(
				mt_scp_pll.clk_mux); /* should not enable twice */
			if (ret) {
		...
		clk_disable_unprepare(
			mt_scp_pll.clk_mux); /* should not disable twice */
...
void mt_scp_dvfs_state_dump(void)
{
	...
	scp_status = ((scp_state & IN_DEBUG_IDLE) == IN_DEBUG_IDLE) ?
			     "idle mode" :
		     ...
		     ((scp_state & ENTERING_ACTIVE) == ENTERING_ACTIVE) ?
							    "enter active" :
		     ((scp_state & IN_ACTIVE) == IN_ACTIVE) ? "active mode" :
							      "none of state";
	...
	if (dvfs.vlp_support) {
		...
		pr_info("scp status: %s, cpu-off config: %s, power status: %s\n",
			scp_status,
			((slp_pwr_ctrl & R_CPU_OFF) == R_CPU_OFF) ? "enable" :
								    "disable",
...
static int mt_scp_dvfs_state_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "scp status: %s\n",
		   ((scp_state & IN_DEBUG_IDLE) == IN_DEBUG_IDLE) ?
			   "idle mode" :
		   ((scp_state & ENTERING_SLEEP) == ENTERING_SLEEP) ?
			   "enter sleep" :
		   ((scp_state & IN_SLEEP) == IN_SLEEP) ? "sleep mode" :
	...
	if (dvfs.vlp_support) {
		...
		seq_printf(m, "cpu-off config: %s, power status: %s\n",
			   ((slp_pwr_ctrl & R_CPU_OFF) == R_CPU_OFF) ?
				   "enable" :
				   "disable",
			   ((power_status & POW_ON) == POW_ON) ? "on" : "off");
	}
 ...
 ***********************************/
static ssize_t mt_scp_dvfs_sleep_cnt_proc_write(struct file *file,
 ...
 * write scp dvfs sleep
 ***********************************/
static ssize_t mt_scp_dvfs_sleep_proc_write(struct file *file,
					    const char __user *buffer,
					    size_t count, loff_t *data)
{
	char desc[64], cmd[32];
	...
	if (!strcmp(cmd, "sleep")) {
		if (slp_cmd < SCP_SLEEP_OFF || slp_cmd > SCP_SLEEP_ON) {
...
static int mt_scp_dvfs_create_procfs(void)
{
	...
	const struct pentry entries[] = { PROC_ENTRY(scp_dvfs_state),
	...
	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create(entries[i].name, 0664, dir, entries[i].fops)) {
			pr_notice(
				"ERROR: %s: create /proc/scp_dvfs/%s failed\n",
...
bool sync_ulposc_cali_data_to_scp(void)
{
	...
	for (i = 0; i < dvfs.ulposc_hw.cali_nums; i++) {
		*p = dvfs.ulposc_hw.cali_freq[i];
		...
		pr_notice("[%s]: ipi to scp: freq=%d, cali_val=0x%x\n",
			  __func__, dvfs.ulposc_hw.cali_freq[i], *(p + 1));

		ret = mtk_ipi_send_compl(&scp_ipidev, IPI_OUT_C_SLEEP_0,
					 IPI_SEND_WAIT, &ipi_data[0],
					 PIN_OUT_C_SIZE_SLEEP_0, 500);
		if (ret != IPI_ACTION_DONE) {
			pr_notice(
				"[%s]: ipi send ulposc cali val(%d, 0x%x) fail\n",
	...
	scp_reg_read(dvfs.clk_hw->scp_clk_regmap, &dvfs.clk_hw->_sel_clk,
		     &sel_clk);
	if ((sel_clk & (SCP_ULPOSC_SEL_CORE | SCP_ULPOSC_SEL_PERI)) == 0) {
		pr_notice(
			"[%s]:ERROR scp is not switched to ULPOSC, CLK_SW_SEL=0x%x\n",
			__func__, sel_clk);
		WARN_ON(1);
		cali_ok = false;
	} else {
					      ...
					      unsigned int target)
{
	...
	pr_notice("[%s]: cur: %dMHz, target: %dMHz calibrate failed\n",
		  __func__, FM_CNT2FREQ(cur), FM_CNT2FREQ(target));
	return 0;
}
...
static unsigned int __init _get_ulposc_clk_by_fmeter_wrapper(void)
{
	...
	if (result == 0) {
		pr_notice(
			"[%s]: mt_get_fmeter_freq() return %d, pls check CCF configs\n",
			__func__, result);
		WARN_ON(1);
	}
...
static unsigned int __init get_ulposc_clk_by_fmeter_vlp(void)
{
	...
	scp_reg_read(dvfs.ulposc_hw.fmeter_regmap,
		     &dvfs.ulposc_hw.clkdbg_regs->_clk26cali_0,
		     &vlp_fqmtr_con0_bk);
	scp_reg_read(dvfs.ulposc_hw.fmeter_regmap,
		     &dvfs.ulposc_hw.clkdbg_regs->_clk26cali_1,
		     &vlp_fqmtr_con1_bk);
	...
	*/
	...
	if (!is_fmeter_timeout) {
		scp_reg_read(dvfs.ulposc_hw.fmeter_regmap,
			     &dvfs.ulposc_hw.clkdbg_regs->_cal_cnt, &result);
	} else {
		result = 0;
	}
...
static void __init set_ulposc_cali_value_ext(unsigned int cali_val)
{
	...
	ret = scp_reg_update(dvfs.ulposc_hw.ulposc_regmap,
			     &dvfs.ulposc_hw.ulposc_regs->_cali_ext, cali_val);
...
static void __init set_ulposc_cali_value(unsigned int cali_val)
{
	...
	ret = scp_reg_update(dvfs.ulposc_hw.ulposc_regmap,
			     &dvfs.ulposc_hw.ulposc_regs->_cali, cali_val);
...
static int __init ulposc_cali_process_vlp(unsigned int cali_idx,
					  unsigned short *cali_res1,
					  unsigned short *cali_res2)
{
	...
	if (!dvfs.vlpck_bypass_phase1) {
		/* fixed in phase1 */
		set_ulposc_cali_value(
			dvfs.ulposc_hw.ulposc_regs->_cali.init_config);
		min = CAL_MIN_VAL_EXT;
		max = CAL_MAX_VAL_EXT;
		do {
			mid = (min + max) / 2;
	...
	current_val = get_ulposc_clk_by_fmeter_vlp();
	diff_by_min = (current_val > target_val) ? (current_val - target_val) :
						   (target_val - current_val);

	set_ulposc_cali_value(max);
	current_val = get_ulposc_clk_by_fmeter_vlp();
	diff_by_max = (current_val > target_val) ? (current_val - target_val) :
						   (target_val - current_val);
	...
	*cali_res2 = (diff_by_min < diff_by_max) ? min : max;
	...
	pr_notice("[%s]: target: %uMhz, calibrated = %uMHz\n", __func__,
		  FM_CNT2FREQ(target_val), FM_CNT2FREQ(current_val));
...
static int smc_turn_on_ulposc2(void)
{
	...
	arm_smccc_smc(MTK_SIP_SCP_DVFS_CONTROL, ULPOSC2_TURN_ON, 0, 0, 0, 0, 0,
		      0, &res);
	return res.a0;
}
...
static int smc_turn_off_ulposc2(void)
{
	...
	arm_smccc_smc(MTK_SIP_SCP_DVFS_CONTROL, ULPOSC2_TURN_OFF, 0, 0, 0, 0, 0,
		      0, &res);
	return res.a0;
}
...
static int __init mt_scp_dvfs_do_ulposc_cali_process(void)
{
	...
	for (i = 0; i < dvfs.ulposc_hw.cali_nums; i++) {
		...
		if (dvfs.vlpck_support)
			ret = ulposc_cali_process_vlp(
				i, &dvfs.ulposc_hw.cali_val_ext[i],
				&dvfs.ulposc_hw.cali_val[i]);
		else
			ret = ulposc_cali_process(i,
						  &dvfs.ulposc_hw.cali_val[i]);
		if (ret) {
						 ...
						 struct ulposc_cali_hw *cali_hw)
{
	...
	if ((ret / CALI_CONFIG_ELEM_CNT) <= 0 ||
		...
		pr_notice(
			"[%s]: cali config count does not equal to cali nums\n",
			__func__);
		return ret;
	}

	cali_hw->cali_configs =
		kcalloc(cali_hw->cali_nums, sizeof(struct ulposc_cali_config),
			GFP_KERNEL);
	...
	for (i = 0; i < cali_hw->cali_nums; i++) {
		ret = of_property_read_u32_index(
			node, "ulposc-cali-config", (i * CALI_CONFIG_ELEM_CNT),
					     ...
					     struct ulposc_cali_hw *cali_hw)
{
	...
	if (ret != cali_hw->cali_nums) {
		pr_notice(
			"[%s]: target nums does not equals to ulposc-cali-num\n",
			__func__);
		return ret;
	}

	if (dvfs.vlpck_support) {
		cali_hw->cali_val_ext = kcalloc(
			cali_hw->cali_nums, sizeof(unsigned short), GFP_KERNEL);
	...
	cali_hw->cali_val =
		kcalloc(cali_hw->cali_nums, sizeof(unsigned short), GFP_KERNEL);
	if (!cali_hw->cali_val) {
		ret = -ENOMEM;
	...
	for (i = 0; i < cali_hw->cali_nums; i++) {
		ret = of_property_read_u32_index(node, "ulposc-cali-target", i,
						 &tmp);
		if (ret) {
					      ...
					      struct ulposc_cali_hw *cali_hw)
{
	...
	if (!dvfs.ccf_fmeter_support) {
		...
		if (ret) {
			pr_notice(
				"[%s]: find clk-dbg-ver failed with err: %d\n",
				__func__, ret);
			return ret;
		}
...
static int mt_scp_dump_sleep_count(void)
{
	...
	pr_notice(
		"[SCP] [%s:%d] - scp_sleep_cnt_0 = %d, scp_sleep_cnt_1 = %d\n",
					      ...
					      struct ulposc_cali_hw *cali_hw)
{
	...
	if (!dvfs.ccf_fmeter_support) {
		cali_hw->fmeter_regmap = syscon_regmap_lookup_by_phandle(
			node, FM_CLK_PHANDLE_NAME);
	...
	cali_hw->ulposc_regmap =
		syscon_regmap_lookup_by_phandle(node, ULPOSC_CLK_PHANDLE_NAME);
					      ...
					      struct ulposc_cali_hw *cali_hw)
{
	...
	dvfs.ulposc_hw.do_ulposc_cali =
		of_property_read_bool(node, "do-ulposc-cali");
	if (!dvfs.ulposc_hw.do_ulposc_cali) {
	...
	dvfs.clk_hw->scp_clk_regmap = syscon_regmap_lookup_by_phandle(
		node, SCP_CLK_CTRL_PHANDLE_NAME);
...
static int __init mt_scp_dts_clk_init(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_SUPPORTED_PLL_NUM; i++) {
		...
		if (IS_ERR(mt_scp_pll.clk_pll[i])) {
			dev_notice(&pdev->dev, "cannot get %dst clock parent\n",
				   i);
					    ...
					    struct dvfs_opp **opp)
{
	...
	*opp = kcalloc(dvfs.scp_opp_nums, sizeof(struct dvfs_opp), GFP_KERNEL);
	...
	for (i = 0; i < dvfs.scp_opp_nums; i++) {
		ret = of_property_read_u32_index(
			node, "dvfs-opp", i * OPP_ELEM_CNT, &(*opp)[i].vcore);
		if (ret) {
					 ...
					 struct device_node *node)
{
	...
	if (IS_ERR(regmap)) {
		dev_notice(&pdev->dev, "Get gpio regmap fail: %ld\n",
			   PTR_ERR(regmap));
	...
	dvfs.bypass_pmic_rg_access =
		of_property_read_bool(node, "no-pmic-rg-access");
	...
	if (!regmap)
		regmap = dev_get_regmap(&pmic_pdev->dev, NULL);
#endif
...
static int __init mt_scp_dts_init(struct platform_device *pdev)
{
	...
	*/
	...
	if (dvfs.vlp_support) {
		...
		if (ret) {
			pr_notice(
				"[%s]: no vow-lp-enable-gear property, set gear to -1\n",
				__func__);
			dvfs.vow_lp_en_gear = -1;
		}
	}
	...
	if (dvfs.vlpck_support) {
		dvfs.vlpck_bypass_phase1 =
			of_property_read_bool(node, "vlpck-bypass-phase1");
		pr_notice("[%s]: Use %d-phase VLP_CKSYS in calibration flow\n",
			  __func__, dvfs.vlpck_bypass_phase1 ? 1 : 2);
	} else {
		dvfs.vlpck_bypass_phase1 = false;
	}

	ret = of_property_read_u32(node, "scp-cores", &dvfs.core_nums);
	...
	if (dvfs.vlp_support)
		...
		dvfs.pmic_sshub_en =
			of_property_read_bool(node, "pmic-sshub-support");
	 ...
	 * be used temporarily.
	 */
	dvfs.ccf_fmeter_support =
		of_property_read_bool(node, "ccf-fmeter-support");
	if (!dvfs.ccf_fmeter_support) {
		pr_notice(
			"[%s]: fmeter api havn't been provided, use legacy one\n",
			__func__);
	} else {
		/* enum FMETER_TYPE */
		if (dvfs.vlpck_support)
		...
		if (ret < 0) {
			pr_notice(
				"[%s]: failed to init ccf fmeter api, id: %d, type: %d\n",
				__func__, dvfs.ccf_fmeter_id,
				dvfs.ccf_fmeter_type);
			goto DTS_FAILED;
		}
	}
	...
	if (!dvfs.vlp_support) {
		...
		reg_vcore =
			devm_regulator_get_optional(&pdev->dev, "sshub-vcore");
		if (IS_ERR(reg_vcore) || !reg_vcore) {
			pr_notice(
				"regulator vcore sshub supply is not available\n");
			ret = PTR_ERR(reg_vcore);
			goto PASS;
		}

		reg_vsram =
			devm_regulator_get_optional(&pdev->dev, "sshub-vsram");
		if (IS_ERR(reg_vsram) || !reg_vsram) {
			pr_notice(
				"regulator vsram sshub supply is not available\n");
