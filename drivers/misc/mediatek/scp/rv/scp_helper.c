...
extern int register_3way_semaphore_notifier(struct notifier_block *nb)
	__attribute__((weak));
extern int unregister_3way_semaphore_notifier(struct notifier_block *nb)
	__attribute__((weak));
		...
		if (scp_dvfs_feature_enable()) {
			...
			while (!sync_ulposc_cali_data_to_scp()) {
				...
				pr_notice("[SCP] cali #%d fail\n",
					  ++cali_times);
				msleep(2000);
				if (atomic_read(&scp_reset_status) ==
					    RESET_STATUS_START_WDT ||
				    cali_times >= 20) {
					pr_notice(
						"[SCP] cali fail, do recovery\n");
					atomic_set(&scp_reset_status,
						   RESET_STATUS_START);
...
int reset_scp(int reset)
{
	...
	if (scp_enable[SCP_A_ID]) {
		...
		{
#endif
			writel((unsigned int)scp_mem_base_phys,
			       DRAM_RESV_ADDR_REG);
 ...
 */
static int scp_pm_event(struct notifier_block *notifier, unsigned long pm_event,
			void *unused)
{
	int retval;
...
scp_A_status_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
	if (scp_ready[SCP_A_ID])
					    ...
					    char *buf)
{
	...
	scp_dump_last_regs();
	scp_show_last_regs();
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_status = %08x\n",
			 c0_m->status);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_pc = %08x\n",
			 c0_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_lr = %08x\n",
			 c0_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_sp = %08x\n",
			 c0_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_pc_latch = %08x\n",
			 c0_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_lr_latch = %08x\n",
			 c0_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h0_sp_latch = %08x\n",
			 c0_m->sp_latch);
	if (!scpreg.twohart)
		goto core1;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_pc = %08x\n",
			 c0_t1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_lr = %08x\n",
			 c0_t1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_sp = %08x\n",
			 c0_t1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_pc_latch = %08x\n",
			 c0_t1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_lr_latch = %08x\n",
			 c0_t1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0h1_sp_latch = %08x\n",
			 c0_t1_m->sp_latch);
core1:
	if (scpreg.core_nums == 1)
		goto end;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_status = %08x\n",
			 c1_m->status);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_pc = %08x\n",
			 c1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_lr = %08x\n",
			 c1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_sp = %08x\n",
			 c1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_pc_latch = %08x\n",
			 c1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_lr_latch = %08x\n",
			 c1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h0_sp_latch = %08x\n",
			 c1_m->sp_latch);
	if (!scpreg.twohart)
		goto end;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_pc = %08x\n",
			 c1_t1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_lr = %08x\n",
			 c1_t1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_sp = %08x\n",
			 c1_t1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_pc_latch = %08x\n",
			 c1_t1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_lr_latch = %08x\n",
			 c1_t1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1h1_sp_latch = %08x\n",
			 c1_t1_m->sp_latch);
...
static inline ssize_t
scp_ipi_test_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
...
/******************************************************************************
 *****************************************************************************/
static struct miscdevice scp_device = { .minor = MISC_DYNAMIC_MINOR,
					.name = "scp",
#if IS_ENABLED(CONFIG_MTK_TINYSYS_SCP_DEBUG_SUPPORT)
...
static ssize_t scp_debug_read(struct file *filp, char __user *buf, size_t count,
			      loff_t *pos)
{
	char *buffer = NULL;
	size_t n = 0, size = 0;
			       ...
			       size_t count, loff_t *ppos)
{
	...
	offset = scp_get_reserve_mem_size(SCP_A_LOGGER_MEM_ID) -
		 DEBUG_CMD_BUFFER_SZ;

	if (!vaddr || offset < 0)
		return -EFAULT;

	if (copy_from_user(vaddr + offset, buffer,
			   min(count, (size_t)DEBUG_CMD_BUFFER_SZ)))
		return -EFAULT;
...
static int create_files(void)
{
	...
	ret = device_create_file(scp_device.this_device, &dev_attr_scpctl);
	...
	debugfs_create_file("scp_dbg", S_IFREG | 0644, NULL, NULL,
			    &scp_debug_ops);
#endif
	return 0;
}
...
RESERVEDMEM_OF_DECLARE(scp_reserve_mem_init, SCP_MEM_RESERVED_KEY,
		       scp_reserve_mem_of_init);
...
#define MEMORY_TBL_ELEM_NUM (3)
	...
	if (num != NUMS_MEM_ID) {
		pr_notice(
			"[SCP] number of entries of reserved memory %u / %u\n",
			num, NUMS_MEM_ID);
		BUG_ON(1);
		return -1;
	}
	...
	pr_notice("[SCP] %s is called, 0x%llx, 0x%x", __func__,
		  scp_mem_base_phys, (unsigned int)scp_mem_size);

	if (!(((scp_mem_base_phys >= (0x40000000ULL)) &&
	       (scp_mem_base_phys < (0xA0000000ULL))) ||
	      ((scp_mem_base_phys >= (0x150000000ULL)) &&
	       (scp_mem_base_phys < (0x1A0000000ULL))) ||
	      ((scp_mem_base_phys >= (0x250000000ULL)) &&
	       (scp_mem_base_phys < (0x2A0000000ULL)))) ||
	...
	scp_mem_num =
		of_property_count_u32_elems(pdev->dev.of_node, "scp_mem_tbl") /
		MEMORY_TBL_ELEM_NUM;
	if (scp_mem_num <= 0) {
	...
	for (i = 0; i < scp_mem_num; i++) {
		...
		if (i == 0 && scpreg.secure_dump) {
			...
			ret = of_property_read_u32(pdev->dev.of_node,
						   "secure_dump_size", &m_size);
		} else {
			ret = of_property_read_u32_index(
				pdev->dev.of_node, "scp_mem_tbl",
	...
	for (id = 0; id < NUMS_MEM_ID; id++) {
		...
		pr_notice(
			"[SCP][rsrv_mem-%d] phy:0x%llx - 0x%llx, len:0x%llx\n",
			id, start_phys, start_phys + len - 1, len);
		pr_notice(
			"[SCP][rsrv_mem-%d] vir:0x%llx - 0x%llx, len:0x%llx\n",
...
static void scp_control_feature(enum feature_id id, bool enable)
{
	...
	if (scp_ready[SCP_A_ID]) {
		...
		pr_notice(
			"[SCP]Not send SCP DVFS request because SCP is down\n");
		WARN_ON(1);
	}
...
void scp_reset_wait_timeout(void)
{
	...
	while (timeout--) {
		...
		core1_halt = scpreg.core_nums == 2 ?
				     readl(R_CORE1_STATUS) & B_CORE_HALT :
				     1;
...
void scp_sys_reset_ws(struct work_struct *ws)
{
	...
	if (scpreg.secure_dump) {
		if ((scp_reset_type == RESET_TYPE_TIMEOUT) ||
			...
			pr_notice("[SCP] rstn core0 %x core1 %x\n",
				  ...
				  readl(R_CORE1_SW_RSTN_SET));
		} else {
			/* reset type scp WDT or CMD*/
			/* make sure scp is in idle state */
			scp_reset_wait_timeout();
			scp_do_rstn_set(
				1); /* write CORE_REBOOT_OK to SCP_GPR_CORE0_REBOOT */
			pr_notice("[SCP] rstn core0 %x core1 %x\n",
				  readl(R_CORE0_SW_RSTN_SET),
				  readl(R_CORE1_SW_RSTN_SET));
		}
	...
	if (debug_timeout_reset_flag == 1)
		mod_timer(&scp_ready_timer[SCP_A_ID].tl,
			  jiffies + SCP_READY_TIMEOUT * 10);
	else
		mod_timer(&scp_ready_timer[SCP_A_ID].tl,
			  jiffies + SCP_READY_TIMEOUT);
...
void scp_recovery_init(void)
{
	...
	scp_regdump_virt = ioremap(scp_region_info_copy.regdump_start,
...
{
	enum { feaure_tbl_item_size = 3 };
	int i, ret, feature_num, feature_id, frequency, core_id;

	feature_num = of_property_count_u32_elems(pdev->dev.of_node,
						  "scp_feature_tbl") /
		      3;
	if (feature_num <= 0) {
	...
	for (i = 0; i < feature_num; ++i) {
		...
		if (ret) {
			pr_notice(
				"[SCP] %s: can't get feature id(%d):line %d\n",
				__func__, i, __LINE__);
			return -1;
		}
		...
		if (feature_id != feature_table[i].feature) {
			pr_notice(
				"[SCP] %s: feature id don't match(%d:%d):line %d\n",
...
static bool scp_ipi_table_init(struct mtk_mbox_device *scp_mboxdev,
			       struct platform_device *pdev)
{
	...
	scp_mboxdev->info_table =
		vzalloc(sizeof(struct mtk_mbox_info) * scp_mboxdev->count);
	...
	scp_mboxdev->pin_send_table = vzalloc(sizeof(struct mtk_mbox_pin_send) *
					      scp_mboxdev->send_count);
	...
	scp_mboxdev->pin_recv_table = vzalloc(sizeof(struct mtk_mbox_pin_recv) *
					      scp_mboxdev->recv_count);
	...
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 0,
					 &scp_ipi_legacy_id[0].out_id_0);
	if (ret) {
		pr_notice("[SCP]%s:Cannot get out_id_0\n", __func__);
	}
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 1,
					 &scp_ipi_legacy_id[0].out_id_1);
	if (ret) {
		pr_notice("[SCP]%s:Cannot get out_id_1\n", __func__);
	}
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 2,
					 &scp_ipi_legacy_id[0].in_id_0);
	if (ret) {
		pr_notice("[SCP]%s:Cannot get in_id_0\n", __func__);
	}
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 3,
					 &scp_ipi_legacy_id[0].in_id_1);
	if (ret) {
		pr_notice("[SCP]%s:Cannot get in_id_1\n", __func__);
	}
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 4,
					 &scp_ipi_legacy_id[0].out_size);
	if (ret) {
		pr_notice("[%s]:Cannot get out_size\n", __func__);
	}
	ret = of_property_read_u32_index(pdev->dev.of_node, "legacy_table", 5,
					 &scp_ipi_legacy_id[0].in_size);
	if (ret) {
		pr_notice("[SCP]%s:Cannot get in_size\n", __func__);
	}
	scp_ipi_legacy_id[0].msg_0 =
		vzalloc(scp_ipi_legacy_id[0].in_size * MBOX_SLOT_SIZE);
...
static int scp_device_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "core_nums", &scpreg.core_nums);
	if (!scpreg.core_nums) {
	...
	of_property_read_u32(pdev->dev.of_node, "twohart", &scpreg.twohart);
	pr_notice("[SCP] scpreg.twohart = %d\n", scpreg.twohart);

	/* secure_dump */
	scpreg.secure_dump = 0;
	if (!of_property_read_string(pdev->dev.of_node, "secure_dump",
				     &secure_dump)) {
	...
	if (!of_property_read_string(pdev->dev.of_node, "debug_timeout_reset",
				     &debug_timeout_reset)) {
...
int notify_scp_semaphore_event(struct notifier_block *nb, unsigned long event,
			       void *v)
{
	...
	if (event == NOTIFIER_SCP_3WAY_SEMAPHORE_GET) {
		...
		status = (status == SEMAPHORE_SUCCESS) ? NOTIFY_STOP :
							 NOTIFY_BAD;
...
static int __init scp_init(void)
{
	...
	if (mbox_check_recv_table(IPI_IN_SCP_READY_0))
		mtk_ipi_register(&scp_ipidev, IPI_IN_SCP_READY_0,
				 (void *)scp_A_ready_ipi_handler, NULL,
				 &msg_scp_ready0);
	else
