...
static int init_cpufreq_table(void)
{
	...
	for (i = 0; i < cluster_nr; i++) {
		...
		cluster_ppm_info[i].dvfs_tbl =
			kcalloc(opp_nr, sizeof(*cluster_ppm_info[i].dvfs_tbl),
				GFP_KERNEL);
		if (!cluster_ppm_info[i].dvfs_tbl) {
			ret = -ENOMEM;
			pr_info("%s: Failed to allocate dvfs table for cid %d",
				TAG, i);
...
static inline bool perf_do_check(u64 wallclock)
{
	...
	if ((s64)(wallclock - checked_timestamp) >= (s64)(2 * NSEC_PER_MSEC)) {
...
static void __maybe_unused perf_common(void *data, struct rq *rq)
{
	u64 wallclock;
