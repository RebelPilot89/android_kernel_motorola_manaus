...
static void __maybe_unused mtk_freq_qos_add_request(
	void *data, struct freq_constraints *qos, struct freq_qos_request *req,
...
mtk_freq_qos_update_request(void *data, struct freq_qos_request *req, int value)
{
	int cid = 0;
	const char *caller_info = find_and_get_symobls(
		(unsigned long)__builtin_return_address(1));
...
int insert_freq_qos_hook(void)
{
	...
	ret = register_trace_android_vh_freq_qos_add_request(
		mtk_freq_qos_add_request, NULL);
	if (ret) {
	...
	ret = register_trace_android_vh_freq_qos_update_request(
		mtk_freq_qos_update_request, NULL);
	if (ret) {
...
#if IS_ENABLED(CONFIG_ANDROID_VENDOR_HOOKS)
	unregister_trace_android_vh_freq_qos_add_request(
		mtk_freq_qos_add_request, NULL);
