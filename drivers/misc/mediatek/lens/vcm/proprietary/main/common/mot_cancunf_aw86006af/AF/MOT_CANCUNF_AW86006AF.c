...
static inline int
AW86006AF_GetVCMInfo(__user struct stAF_MotorInfo *pstMotorInfo)
{
				 ...
				 unsigned long a_u4Param)
{
	...
	case OISIOC_G_GYRO_OFFSET_CALI:
		ret = gyro_offset_cali_run(
			(struct motOISGOffsetResult *)a_u4Param);
		break;
...
int MOT_CANCUNF_AW86006AF_Release(struct inode *a_pstInode,
...
int MOT_CANCUNF_AW86006AF_PowerDown(struct i2c_client *pstAF_I2Cclient,
				    int *pAF_Opened)
{
	AW_LOGI("Start");
