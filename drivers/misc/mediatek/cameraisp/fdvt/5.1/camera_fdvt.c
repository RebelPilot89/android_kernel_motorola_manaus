...
#define log_dbg(format, args...) pr_info(FDTAG "[%s] " format, __func__, ##args)
#else
#define log_dbg(format, args...)
#endif

#define log_inf(format, args...) pr_info(FDTAG "[%s] " format, __func__, ##args)
#define log_notice(format, args...)                                            \
	pr_notice(FDTAG "[%s] " format, __func__, ##args)
#define log_wrn(format, args...) pr_info(FDTAG "[%s] " format, __func__, ##args)
#define log_err(format, args...) pr_info(FDTAG "[%s] " format, __func__, ##args)
...
struct FDVT_REQUEST_STRUCT {
	...
	enum FDVT_FRAME_STATUS_ENUM fdvt_frame_status[MAX_FDVT_FRAME_REQUEST];
...
struct FDVT_REQUEST_RING_STRUCT {
	...
	struct FDVT_REQUEST_STRUCT req_struct[MAX_FDVT_REQUEST_RING_SIZE];
};
...
#define IRQ_LOG_KEEPER(irq, ppb, log_t, fmt, ...)                                     \
	do {                                                                          \
		char *ptr;                                                            \
		char *des;                                                            \
		signed int ava_len;                                                   \
		...
		ptr = des = (char *)&(                                                \
			sv_log[irq]._str[ppb][log_t]                                  \
					[sv_log[irq]._cnt[ppb][log_t]]);              \
		ava_len = str_leng - 1 - sv_log[irq]._cnt[ppb][log_t];                \
		if (ava_len > 1) {                                                    \
			ret = snprintf((char *)(des), ava_len,                        \
				       "[%d.%06d]" fmt,                               \
				       sv_log[irq]._lastIrqTime.sec,                  \
				       sv_log[irq]._lastIrqTime.usec,                 \
				       ##__VA_ARGS__);                                \
			if (ret < 0) {                                                \
				log_err("snprintf fail(%d)\n", ret);                  \
			}                                                             \
			if ('\0' !=                                                   \
			    sv_log[irq]._str[ppb][log_t][str_leng - 1]) {             \
			...
			if (src->_cnt[ppb][log_t] != 0) {                             \
				...
				ret = snprintf((char *)(des), ava_len, fmt,           \
					       ##__VA_ARGS__);                        \
				if (ret < 0) {                                        \
					log_err("snprintf fail(%d)\n", ret);          \
				}                                                     \
...
static struct dma_buf *aie_imem_sec_alloc(u32 size, bool IsSecure)
{
	...
	my_dma_buf = dma_heap_buffer_alloc(dma_heap, size, O_RDWR | O_CLOEXEC,
...
static void *aie_get_va(struct dma_buf *my_dma_buf)
{
...
static void fdvt_free_iova(struct imem_buf_info *bufinfo)
{
	if (bufinfo->iova) {
		/*free iova*/
		dma_buf_unmap_attachment(bufinfo->attach, bufinfo->sgt,
					 DMA_BIDIRECTIONAL);
...
fdvt_get_irq_state(unsigned int type, unsigned int user_number,
		   unsigned int stus, enum FDVT_PROCESS_ID_ENUM which_req,
		   int process_id)
{
	unsigned int ret = 0;
...
static inline void fdvt_reset(void)
{
	...
	if (fdvt_info.user_count > 1) {
		...
		log_dbg("Curr user_count(%d) users exist",
				...
				struct FDVT_MEM_RECORD *dmabuf)
{
	...
	if (fdvt_sec_dma.handler_first_time == 0) {
		fdvt_sec_dma.YUVConfig.dmabuf = dma_buf_get(
		...
		fdvt_sec_dma.RSConfig.dmabuf = dma_buf_get(
		...
		fdvt_sec_dma.RSOutBuf.dmabuf = dma_buf_get(
		...
		fdvt_sec_dma.FDConfig.dmabuf = dma_buf_get(
		...
		fdvt_sec_dma.FDOutBuf.dmabuf = dma_buf_get(
		...
		if (basic_config->FDVT_METADATA_TO_GCE.FD_POSE_Config_Handler) {
			fdvt_sec_dma.FD_POSE.dmabuf =
				dma_buf_get(basic_config->FDVT_METADATA_TO_GCE
						    .FD_POSE_Config_Handler);
			fdvt_sec_dma.FD_POSE_Config_Handler =
				dmabuf_to_secure_handle(
					fdvt_sec_dma.FD_POSE.dmabuf);
		}

		fdvt_sec_dma.FDResultBuf_MVA.dmabuf = dma_buf_get(
	...
	if (basic_config->FDVT_METADATA_TO_GCE.ImgSrcUV_Handler) {
		dmabuf->ImgSrcUV.dmabuf = dma_buf_get(
	...
	if (request->state == FDVT_REQUEST_STATE_PENDING) {
		...
		for (j = 0; j < MAX_FDVT_FRAME_REQUEST; j++) {
			if (FDVT_FRAME_STATUS_ENQUE ==
				...
				if (request->frame_config[j]
						    .FDVT_METADATA_TO_GCE
						    .SecMemType == 3 &&
				    request->frame_config[j].FDVT_IS_SECURE)
					fdvt_sec_fd2handler(
						&request->frame_config[j],
						&request->frame_dmabuf[j]);
...
static bool config_fdvt(void)
{
	...
	for (k = 0; k < MAX_FDVT_REQUEST_RING_SIZE; k++) {
		i = (*hw_process_idx + k) % MAX_FDVT_REQUEST_RING_SIZE;
		...
		if (request->state == FDVT_REQUEST_STATE_PENDING) {
			...
			if (j != MAX_FDVT_FRAME_REQUEST) {
				log_err("FDVT Config state is wrong! idx j(%d), hw_process_idx(%d), state(%d)\n",
	...
	for (k = 0; k < MAX_FDVT_REQUEST_RING_SIZE; k++) {
		...
		if (request->state == FDVT_REQUEST_STATE_PENDING) {
			...
			*hw_process_idx = (*hw_process_idx + 1) %
					  MAX_FDVT_REQUEST_RING_SIZE;
		}
	}
...
static bool update_fdvt(pid_t *process_id)
{
	...
	for (i = *hw_process_idx; i < MAX_FDVT_REQUEST_RING_SIZE; i++) {
		request = &fdvt_req_ring.req_struct[i];
		if (request->state == FDVT_REQUEST_STATE_RUNNING) {
			...
			if (j != MAX_FDVT_FRAME_REQUEST) {
				...
				if (MAX_FDVT_FRAME_REQUEST == next_idx ||
				    (MAX_FDVT_FRAME_REQUEST > next_idx &&
...
*/
			...
			/*else {*/
			IRQ_LOG_KEEPER(
				FDVT_IRQ_TYPE_INT_FDVT_ST, m_CurrentPPB,
				_LOG_ERR,
...
#if !BYPASS_REG
{
	...
	if (basic_config->IS_LEGACY == 0) {
		...
		if (success) {
			...
			if (basic_config->enROI == false) {
				...
				} else if (basic_config->SRC_IMG_FMT ==
						   FMT_YUV_2P ||
			...
			srcbuf32 = (unsigned long)image_buffer_Y &
				   0x00000000ffffffff;
		...
		success =
			mmu_get_dma_buffer(&mmu, basic_config->FDVT_IMG_UV_FD);
		...
		if (success) {
			...
			srcbuf32 = (unsigned long)image_buffer_UV &
				   0x00000000ffffffff;
...
#if CHECK_SERVICE_IF_0
	if (basic_config->IS_LEGACY == 0) {
		cmdq_pkt_flush_threaded(pkt, rsc_cmdq_cb_destroy,
					(void *)records);
	} else {
#endif
		cmdq_pkt_flush(pkt);
	...
	if (basic_config->FD_MODE == 0) {
		...
		cmdqRecWrite(handle, FDVT_ENABLE_HW, 0x00000101, CMDQ_REG_MASK);
...
static void fdvt_tzmp2(struct fdvt_config *basic_config,
		       struct FDVT_MEM_RECORD *dmabuf,
		       struct FDVT_SEC_MetaDataToGCE *dmabuf_metadata)
{
	dmabuf_metadata->ImgSrcY_Handler =
		basic_config->FDVT_METADATA_TO_GCE.ImgSrcY_Handler;
	dmabuf_metadata->ImgSrcUV_Handler =
		basic_config->FDVT_METADATA_TO_GCE.ImgSrcUV_Handler;
	...
	dmabuf_metadata->ImgSrc_Y_Size =
		basic_config->FDVT_METADATA_TO_GCE.ImgSrc_Y_Size;
	dmabuf_metadata->ImgSrc_UV_Size =
		basic_config->FDVT_METADATA_TO_GCE.ImgSrc_UV_Size;
	dmabuf_metadata->YUVConfigSize =
		basic_config->FDVT_METADATA_TO_GCE.YUVConfigSize;
	dmabuf_metadata->YUVOutBufSize =
		basic_config->FDVT_METADATA_TO_GCE.YUVOutBufSize;
	dmabuf_metadata->RSConfigSize =
		basic_config->FDVT_METADATA_TO_GCE.RSConfigSize;
	dmabuf_metadata->RSOutBufSize =
		basic_config->FDVT_METADATA_TO_GCE.RSOutBufSize;
	dmabuf_metadata->FDConfigSize =
		basic_config->FDVT_METADATA_TO_GCE.FDConfigSize;
	dmabuf_metadata->FDOutBufSize =
		basic_config->FDVT_METADATA_TO_GCE.FDOutBufSize;
	dmabuf_metadata->FD_POSE_ConfigSize =
		basic_config->FDVT_METADATA_TO_GCE.FD_POSE_ConfigSize;
	dmabuf_metadata->FDResultBufSize =
		basic_config->FDVT_METADATA_TO_GCE.FDResultBufSize;
	dmabuf_metadata->FDMode = basic_config->FDVT_METADATA_TO_GCE.FDMode;
	dmabuf_metadata->srcImgFmt =
		basic_config->FDVT_METADATA_TO_GCE.srcImgFmt;
	dmabuf_metadata->srcImgWidth =
		basic_config->FDVT_METADATA_TO_GCE.srcImgWidth;
	dmabuf_metadata->srcImgHeight =
		basic_config->FDVT_METADATA_TO_GCE.srcImgHeight;
	dmabuf_metadata->maxWidth = basic_config->FDVT_METADATA_TO_GCE.maxWidth;
	dmabuf_metadata->maxHeight =
		basic_config->FDVT_METADATA_TO_GCE.maxHeight;
	dmabuf_metadata->rotateDegree =
		basic_config->FDVT_METADATA_TO_GCE.rotateDegree;
	dmabuf_metadata->featureTH =
		basic_config->FDVT_METADATA_TO_GCE.featureTH;
	dmabuf_metadata->SecMemType =
		basic_config->FDVT_METADATA_TO_GCE.SecMemType;
	dmabuf_metadata->enROI = basic_config->FDVT_METADATA_TO_GCE.enROI;
	dmabuf_metadata->src_roi.x1 =
		basic_config->FDVT_METADATA_TO_GCE.src_roi.x1;
	dmabuf_metadata->src_roi.x2 =
		basic_config->FDVT_METADATA_TO_GCE.src_roi.x2;
	dmabuf_metadata->src_roi.y1 =
		basic_config->FDVT_METADATA_TO_GCE.src_roi.y1;
	dmabuf_metadata->src_roi.y2 =
		basic_config->FDVT_METADATA_TO_GCE.src_roi.y2;
	dmabuf_metadata->enPadding =
		basic_config->FDVT_METADATA_TO_GCE.enPadding;
	dmabuf_metadata->src_padding.down =
		basic_config->FDVT_METADATA_TO_GCE.src_padding.down;
	dmabuf_metadata->src_padding.up =
		basic_config->FDVT_METADATA_TO_GCE.src_padding.up;
	dmabuf_metadata->src_padding.left =
		basic_config->FDVT_METADATA_TO_GCE.src_padding.left;
	dmabuf_metadata->src_padding.right =
		basic_config->FDVT_METADATA_TO_GCE.src_padding.right;
	dmabuf_metadata->SRC_IMG_STRIDE =
		basic_config->FDVT_METADATA_TO_GCE.SRC_IMG_STRIDE;
	dmabuf_metadata->pyramid_width =
		basic_config->FDVT_METADATA_TO_GCE.pyramid_width;
	dmabuf_metadata->pyramid_height =
		basic_config->FDVT_METADATA_TO_GCE.pyramid_height;
	dmabuf_metadata->isReleased =
		basic_config->FDVT_METADATA_TO_GCE.isReleased;
}
...
#if !BYPASS_REG
{
	...
	if (basic_config->FDVT_IS_SECURE != 0)
		cmdq_sec_pkt_set_data(pkt, 1LL << CMDQ_SEC_FDVT,
				      1LL << CMDQ_SEC_FDVT, CMDQ_SEC_ISP_FDVT,
				      CMDQ_METAEX_FD);
#else
	if (basic_config->FDVT_IS_SECURE != 0) {
		cmdq_sec_pkt_set_data(pkt, 1LL << CMDQ_SEC_FDVT,
	...
	if (basic_config->FDVT_METADATA_TO_GCE.SecMemType == 3)
		...
		if (!fdvt_sec_dma.tzmp1_first_time) {
			fdvt_sec_dma.FDResultBuf_MVA.dmabuf =
				aie_imem_sec_alloc(
					basic_config->FDVT_METADATA_TO_GCE
						.FDResultBufSize,
					0);
			...
			fdvt_sec_dma.FDResultBuf_MVA.iova = fdvt_get_sec_iova(
				fdvt_sec_dma.FDResultBuf_MVA.dmabuf,
	...
	if (basic_config->FD_MODE == 0) {
		...
		if (basic_config->FDVT_METADATA_TO_GCE.SecMemType == 3) {
			...
			cmdq_pkt_write(pkt, NULL, FDVT_YUV2RGB_CON_BASE_ADR_HW,
				       dmabuf_metadata.YUVConfig_IOVA,
				       CMDQ_REG_MASK);
			cmdq_sec_pkt_set_payload(
				pkt, 1, sizeof(dmabuf_metadata),
				(unsigned int *)&dmabuf_metadata);
		} else {
			cmdq_sec_pkt_write_reg(
				pkt, FDVT_RS_CON_BASE_ADR_HW,
				basic_config->FDVT_RSCON_BASE_ADR,
				CMDQ_IWC_PH_2_MVA, 0,
				basic_config->FDVT_RSCON_BUFSIZE, 0x280);
			cmdq_sec_pkt_write_reg(
				pkt, FDVT_FD_CON_BASE_ADR_HW,
				basic_config->FDVT_FD_CON_BASE_ADR,
				CMDQ_IWC_PH_2_MVA, 0,
				basic_config->FDVT_FD_CON_BUFSIZE, 0x280);
			cmdq_sec_pkt_write_reg(
				pkt, FDVT_YUV2RGB_CON_BASE_ADR_HW,
				basic_config->FDVT_YUV2RGBCON_BASE_ADR,
				CMDQ_IWC_PH_2_MVA, 0,
				basic_config->FDVT_YUV2RGBCON_BUFSIZE, 0x280);
			cmdq_sec_pkt_set_payload(
				pkt, 1,
				sizeof(basic_config->FDVT_METADATA_TO_GCE),
				(unsigned int *)&basic_config
					->FDVT_METADATA_TO_GCE);
		}
		...
		if (basic_config->FDVT_METADATA_TO_GCE.SecMemType == 3) {
			...
			cmdq_sec_pkt_write_reg(
				...
				basic_config->FDVT_FD_POSE_CON_BASE_ADR,
				CMDQ_IWC_PH_2_MVA, 0,
				basic_config->FDVT_FD_POSE_CON_BUFSIZE, 0x280);
		}
...
static signed int fdvt_dump_reg(void)
{
	...
	for (i = 0; i < MAX_FDVT_REQUEST_RING_SIZE; i++) {
		log_inf("FDVT Req:state:%d, procID:0x%08X, caller_id:0x%08X, enque_req_num:%d,
			...
			frame_rd_idx : % d\n ",
					 request->state,
			request->process_id, request->caller_id,
			request->enque_req_num, request->frame_wr_idx,
		...
		for (j = 0; j < MAX_FDVT_FRAME_REQUEST;) {
			log_inf("FDVT:FrameStatus[%d]:%d, FrameStatus[%d]:%d, FrameStatus[%d]:%d,
					...
					d\n ",
					j,
				request->fdvt_frame_status[j], j + 1,
				request->fdvt_frame_status[j + 1], j + 2,
				request->fdvt_frame_status[j + 2], j + 3,
				request->fdvt_frame_status[j + 3]);
			j = j + 4;
		}
	}
#endif
	...
	FDVT_WR32(DMA_DEBUG_SEL_REG,
		  (((unsigned int)FDVT_RD32(DMA_DEBUG_SEL_REG)) & 0xFFFFFF00) |
			  0x13);
	...
	FDVT_WR32(DMA_DEBUG_SEL_REG,
		  (((unsigned int)FDVT_RD32(DMA_DEBUG_SEL_REG)) & 0xFFFF00FF) |
			  (1 << 8));
	FDVT_WR32(DMA_DEBUG_SEL_REG,
	...
	for (i = 0; i < count_of_ports; i++) {
		...
		if (ret == 0) {
			log_inf("config M4U Port %s to %s SUCCESS\n",
				iommu_get_port_name(
					M4U_PORT_L20_IPE_FDVT_RDA_DISP + i),
				       ...
				       unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (pReg[i].addr < FDVT_REG_RANGE &&
			...
			log_err("wrong address(0x%p), FDVT_BASE(0x%p), addr(0x%lx)\n",
				(ISP_FDVT_BASE + pReg[i].addr), ISP_FDVT_BASE,
...
static signed int fdvt_wait_irq(FDVT_WAIT_IRQ_STRUCT *wait_irq)
{
	...
	if (wait_irq->status & FDVT_INT_ST) {
		...
		log_err("No Such Stats can be waited!! irq type/User/Sts/pid(0x%x/%d/0x%x/%d)\n",
			wait_irq->type, wait_irq->user_key, wait_irq->status,
			wait_irq->process_id);
	}
	...
	log_inf("before wait_event:Tout(%d), clear(%d), type(%d), IrqStat(0x%08X),
		...
		usrKey(% d)\n ",
		wait_irq->timeout,
		wait_irq->clear, wait_irq->type, irqStatus, wait_irq->status,
		wait_irq->user_key);
	log_inf("before wait_event:ProcID(%d), FdvtIrq(0x%08X), WriteReq(0x%08X),
		ReadReq(0x % 08X),
		which_req(% d)\n ",
		wait_irq->process_id,
		fdvt_info.irq_info.fdvt_irq_cnt, fdvt_info.write_req_idx,
		fdvt_info.read_req_idx, which_req);
#endif

	/* 2. start to wait signal */
	timeout = wait_event_interruptible_timeout(
		fdvt_info.wait_queue_head,
		fdvt_get_irq_state(wait_irq->type, wait_irq->user_key,
				   wait_irq->status, which_req,
				   wait_irq->process_id),
		fdvt_ms_to_jiffies(wait_irq->timeout));
...
/* Store irqinfo status in here to redeuce time of spin_lock_irqsave */
...
#ifdef FDVT_MULTIPROCESS_TIMING_ISSUE
			fdvt_info.read_req_idx = (fdvt_info.read_req_idx + 1) %
		...
		log_inf("no timeout:Tout(%d), clr(%d), type(%d), IrqStat(0x%08X),
			...
			usrKey(% d)\n ",
			wait_irq->timeout,
			wait_irq->clear, wait_irq->type, irqStatus,
 ...
 *
 *****************************************************************************/
static long FDVT_ioctl(struct file *pFile, unsigned int Cmd,
		       unsigned long Param)
{
	signed int ret = 0;
		...
		if (copy_from_user(&irq_info, (void *)Param,
			...
			if (irq_info.user_key >= IRQ_USER_NUM_MAX ||
				...
				log_err("invalid userKey(%d), max(%d), force userkey = 0\n",
					irq_info.user_key, IRQ_USER_NUM_MAX);
				irq_info.user_key = 0;
			}
			/*
			log_inf(
...
*/
			...
			if (copy_to_user((void *)Param, &irq_info,
					 sizeof(FDVT_WAIT_IRQ_STRUCT)) != 0) {
	...
	case FDVT_ENQNUE_NUM:
		...
		if (copy_from_user(&enqueNum, (void *)Param, sizeof(int)) ==
		    0) {
			request = &fdvt_req_ring
					   .req_struct[fdvt_req_ring.write_idx];
			if (FDVT_REQUEST_STATE_EMPTY == request->state) {
				if (enqueNum > MAX_FDVT_FRAME_REQUEST ||
				    enqueNum < 0) {
					log_err("FDVT Enque Num is bigger than enqueNum or negtive:%d\n",
						enqueNum);
					break;
				}
				spin_lock_irqsave(spinlock_lrq_ptr, flags);
				request->process_id = pUserInfo->pid;
				request->enque_req_num = enqueNum;
				spin_unlock_irqrestore(spinlock_lrq_ptr, flags);
				log_dbg("FDVT_ENQNUE_NUM:%d\n", enqueNum);
			} else {
				log_err("WFME Enque request state is not empty:%d, writeIdx:%d, readIdx:%d\n",
					request->state, fdvt_req_ring.write_idx,
					fdvt_req_ring.read_idx);
			}
		} else {
	 ...
	 */
			...
			if (request->state == FDVT_REQUEST_STATE_EMPTY &&
				...
				log_err("No Buffer! write_idx(%d), Stat(%d), frame_wr_idx(%d), enque_req_num(%d)\n",
					fdvt_req_ring.write_idx, request->state,
					request->frame_wr_idx,
					request->enque_req_num);
			}
#ifdef FDVT_USE_GCE
	...
	case FDVT_ENQUE_REQ:
		if (copy_from_user(&fdvt_FdvtReq, (void *)Param,
			...
			log_dbg("FDVT_ENQNUE_NUM:%d, pid:%d\n",
				fdvt_FdvtReq.m_ReqNum, pUserInfo->pid);
		...
		break;
	case FDVT_DEQUE_NUM:
		request = &fdvt_req_ring.req_struct[fdvt_req_ring.read_idx];
		if (request->state == FDVT_REQUEST_STATE_FINISHED) {
			...
			log_err("DEQUE_NUM:No Buffer: read_idx(%d) state(%d) frame_rd_idx(%d) enque_req_num(%d)\n",
				fdvt_req_ring.read_idx, request->state,
	...
	case FDVT_DEQUE:
		...
		if (request->state == FDVT_REQUEST_STATE_FINISHED &&
			...
			if (FDVT_FRAME_STATUS_FINISHED ==
				...
				if (request->frame_config[request->frame_rd_idx]
					    .FDVT_IS_SECURE &&
			...
			if (request->frame_rd_idx == request->enque_req_num) {
	...
	case FDVT_DEQUE_REQ:
		if (copy_from_user(&fdvt_FdvtReq, (void *)Param,
			...
			spin_lock_irqsave(spinlock_lrq_ptr, flags);
			request = &fdvt_req_ring
					   .req_struct[fdvt_req_ring.read_idx];
			if (FDVT_REQUEST_STATE_FINISHED == request->state) {
				dequeNum = request->enque_req_num;
			...
			request->frame_rd_idx = 0;
			request->enque_req_num = 0;
			fdvt_req_ring.read_idx = (fdvt_req_ring.read_idx + 1) %
						 MAX_FDVT_REQUEST_RING_SIZE;
 ...
 *****************************************************************************/
static int
compat_get_FDVT_read_register_data(compat_FDVT_REG_IO_STRUCT __user *data32,
...
static int
compat_put_FDVT_read_register_data(compat_FDVT_REG_IO_STRUCT __user *data32,
				   FDVT_REG_IO_STRUCT __user *data)
{
	compat_uint_t count;
	/*compat_uptr_t uptr;*/
...
static long FDVT_ioctl_compat(struct file *filp, unsigned int cmd,
			      unsigned long arg)
{
	long ret;
	...
	case COMPAT_FDVT_READ_REGISTER: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, FDVT_READ_REGISTER,
						 (unsigned long)data);
	...
	case COMPAT_FDVT_WRITE_REGISTER: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, FDVT_WRITE_REGISTER,
						 (unsigned long)data);
		return ret;
	}
	...
	case COMPAT_FDVT_DEQUE_REQ: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, FDVT_DEQUE_REQ,
						 (unsigned long)data);
...
static signed int FDVT_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	log_inf("[%s] mmap:vm_pgoff(0x%lx) pfn(0x%lx) phy(0x%lx)", __func__,
...
FDVT_M4U_TranslationFault_callback(int port, unsigned int mva, void *data)
#else
...
static signed int FDVT_probe(struct platform_device *pDev)
{
	...
	nr_fdvt_devs += 1;
	_fdvt_dev =
		krealloc(fdvt_devs, sizeof(struct fdvt_device) * nr_fdvt_devs,
			 GFP_KERNEL);
	if (!_fdvt_dev) {
	...
	*(FDVT_dev->dev->dma_mask) =
	...
	if (FDVT_dev->irq > 0 && nr_fdvt_devs == 1) {
		...
		for (i = 0; i < FDVT_IRQ_TYPE_AMOUNT; i++) {
			if (strcmp(pDev->dev.of_node->name,
				...
				ret = request_irq(
					FDVT_dev->irq,
					(irq_handler_t)FDVT_IRQ_CB_TBL[i].isr_fp,
					irq_info[2],
					(const char *)FDVT_IRQ_CB_TBL[i]
						.device_name,
					NULL);
				if (ret) {
					dev_dbg(&pDev->dev,
				...
				log_inf("nr_fdvt_devs=%d, devnode(%s), irq=%d, ISR: %s\n",
					nr_fdvt_devs, pDev->dev.of_node->name,
					FDVT_dev->irq,
					FDVT_IRQ_CB_TBL[i].device_name);
				break;
		...
		if (i >= FDVT_IRQ_TYPE_AMOUNT) {
			log_inf("No corresponding ISR!!: nr_fdvt_devs=%d, devnode(%s), irq=%d\n",
				nr_fdvt_devs, pDev->dev.of_node->name,
				FDVT_dev->irq);
		}
	...
	if (nr_fdvt_devs == 1) {
		...
		dev = device_create(pFDVTClass, NULL, FDVTDevNo, NULL,
				    FDVT_DEV_NAME);
		if (IS_ERR(dev)) {
			ret = PTR_ERR(dev);
			dev_dbg(&pDev->dev,
		...
		spin_unlock(&fdvt_info.spinlock_fdvt_ref);
		/*  */
		fdvt_info.irq_info.mask[FDVT_IRQ_TYPE_INT_FDVT_ST] =
			INT_ST_MASK_FDVT;
		//cmdq_base = NULL;
 ...
 *
 *****************************************************************************/
static struct platform_driver FDVTDriver = { .probe = FDVT_probe,
					     .remove = FDVT_remove,
...
static int fdvt_dump_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < MAX_FDVT_REQUEST_RING_SIZE; i++) {
		...
		seq_printf(
			...
			request->state, request->process_id, request->caller_id,
			request->enque_req_num, request->frame_wr_idx,
			request->frame_rd_idx);
		...
		for (j = 0; j < MAX_FDVT_FRAME_REQUEST;) {
			seq_printf(
				...
				j, request->fdvt_frame_status[j], j + 1,
				request->fdvt_frame_status[j + 1], j + 2,
				request->fdvt_frame_status[j + 2], j + 3,
				request->fdvt_frame_status[j + 3]);
			j = j + 4;
			      ...
			      size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%23s %23s", addrSzBuf, valSzBuf) == 2) {
		...
		if (addr >= FDVT_START_HW && addr <= DMA_BW_SELF_TEST_HW &&
			...
			log_inf("Write-Address Range exceeds the size of hw fdvt!! addr:0x%x, value:0x%x\n",
				addr, val);
		}
		...
		if (addr >= FDVT_START_HW && addr <= DMA_BW_SELF_TEST_HW &&
		    (addr & 0x3) == 0) {
			val = FDVT_RD32(
				(ISP_FDVT_BASE + (addr - FDVT_BASE_HW)));
			log_inf("Read Request - addr:0x%x,value:0x%x\n", addr,
				val);
		} else {
			log_inf("Read-Address Range exceeds the size of hw fdvt!! addr:0x%x, value:0x%x\n",
				addr, val);
		}
	}
	return count;
...
static signed int __init FDVT_Init(void)
{
...
#if CHECK_SERVICE_IF_0
	proc_entry = proc_create("fdvt_dump", 0444, isp_fdvt_dir,
				 &fdvt_dump_proc_fops);

	proc_entry = proc_create("fdvt_reg", 0644, isp_fdvt_dir,
				 &fdvt_reg_proc_fops);
#endif

	/* isr log */
	if (PAGE_SIZE < ((FDVT_IRQ_TYPE_AMOUNT * NORMAL_STR_LEN *
			  ((DBG_PAGE + INF_PAGE + ERR_PAGE) + 1)) *
			 LOG_PPNUM)) {
		i = 0;
		while (i < ((FDVT_IRQ_TYPE_AMOUNT * NORMAL_STR_LEN *
			     ((DBG_PAGE + INF_PAGE + ERR_PAGE) + 1)) *
			    LOG_PPNUM)) {
			i += PAGE_SIZE;
		}
	...
	cmdqCoreRegisterCB(CMDQ_GROUP_FDVT, FDVT_ClockOnCallback,
			   FDVT_DumpCallback, FDVT_ResetCallback,
	...
	mtk_iommu_register_fault_callback(
		M4U_PORT_FDVT_RDA, FDVT_M4U_TranslationFault_callback, NULL);
	mtk_iommu_register_fault_callback(
		M4U_PORT_FDVT_RDB, FDVT_M4U_TranslationFault_callback, NULL);
	mtk_iommu_register_fault_callback(
		M4U_PORT_FDVT_WRA, FDVT_M4U_TranslationFault_callback, NULL);
	mtk_iommu_register_fault_callback(
		M4U_PORT_FDVT_WRB, FDVT_M4U_TranslationFault_callback, NULL);
#else
...
#ifdef FDVT_USE_GCE
			fdvt_info.irq_info.status[FDVT_IRQ_TYPE_INT_FDVT_ST] |=
				FDVT_INT_ST;
			fdvt_info.irq_info.process_id[FDVT_PROCESS_ID_FDVT] =
				process_id;
			fdvt_info.irq_info.fdvt_irq_cnt++;
			fdvt_info.process_id[fdvt_info.write_req_idx] =
				process_id;
