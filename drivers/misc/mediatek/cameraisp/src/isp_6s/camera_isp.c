...
static unsigned int g_cqBaseAddr[ISP_CAM_C_IDX - ISP_CAM_A_IDX + 1][25] = {
	{ 0 }
};
static unsigned int g_cq0NextBA[ISP_CAM_C_IDX - ISP_CAM_A_IDX + 1][1] = {
	{ 0 }
};
#if Lafi_WAM_CQ_ERR
static unsigned int g_cqDoneStatus[ISP_CAM_C_IDX - ISP_CAM_A_IDX + 1] = { 0 };
static union FBC_CTRL_2 g_fbc_ctrl2[ISP_CAM_C_IDX - ISP_CAM_A_IDX + 1]
				   [_cam_max_];
#endif
...
/* CAM_REG_TG_INTER_ST 0x3b3c, CAMSV_REG_TG_INTER_ST 0x016C */
unsigned int ISP_RD32_TG_CAMX_FRM_CNT(unsigned int IrqType,
			     ...
			     enum ISP_IRQ_TYPE_ENUM cam)
{
	if (enter) {
		...
		trace_ISP__Pass1_CAM_enter(cam, imgo_en, rrzo_en, imgo_bpp,
					   rrzo_bpp, imgo_xsize, imgo_ysize,
					   rrzo_xsize, rrzo_ysize, rrz_src_w,
					   rrz_src_h, rrz_dst_w, rrz_dst_h,
...
static void ISP_RecordCQAddr(enum ISP_DEV_NODE_ENUM regModule)
{
	...
	if (twinStatus.Bits.TWIN_EN == MTRUE) {
		...
		if (IS_2RAW_PLATFORM(g_platform_id)) {
			if (twinStatus.Bits.TWIN_MODULE == CAM_C)
				LOG_NOTICE(
					"twin module(CAM_C) is invalid! recover fail\n");
		}
	...
	for (i = 0; i < reg_module_count; i++) {
		...
		ISP_GetDmaPortsStatus(tmp_module, &DmaEnStatus[index][0]);
		//read inner register
		tmp_module = reg_module_array[i] - ISP_CAMSYS_RAWC_CONFIG_IDX;
 ...
 *
 ******************************************************************************/
static void ISP_DumpDebugData(enum ISP_IRQ_TYPE_ENUM module,
			      unsigned int debug_idx)
{
	...
	IRQ_LOG_KEEPER(
		...
		smi_dbg_data_local, smi_dbg_data_local_be, smi_dbg_data_case0,
		smi_dbg_data_case0, fifo_dbg_data_case0, fifo_dbg_data_case1,
...
void dumpAllRegs(enum ISP_DEV_NODE_ENUM module)
{
	...
	for (i = 0; i < ISP_REG_RANGE; i += 0x0020) {
		IRQ_LOG_KEEPER(irq_type, m_CurrentPPB, _LOG_INF, STR_REG,
			       log_ba + i, ISP_RD32(isp_devs[module].regs + i),
	...
	if (DmaEnStatus[_aao_]) {
		...
		ISP_WR32(CAM_REG_FBC_AAO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_AAO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_AAO_CTL2(reg_module),
			 fbc_ctrl2[_aao_].Raw);
	}

	if (DmaEnStatus[_flko_]) {
		fbc_ctrl2[_flko_].Raw =
		...
		ISP_WR32(CAM_REG_FBC_FLKO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_FLKO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_FLKO_CTL2(reg_module),
			 fbc_ctrl2[_flko_].Raw);
	}

	if (DmaEnStatus[_aaho_]) {
		fbc_ctrl2[_aaho_].Raw =
		...
		ISP_WR32(CAM_REG_FBC_AAHO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_AAHO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_AAHO_CTL2(reg_module),
			 fbc_ctrl2[_aaho_].Raw);
	}
	...
	if (DmaEnStatus[_afo_]) {
		...
		ISP_WR32(CAM_REG_FBC_AFO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_AFO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_AFO_CTL2(reg_module),
			 fbc_ctrl2[_afo_].Raw);
	}
	...
	if (DmaEnStatus[_pdo_]) {
		...
		ISP_WR32(CAM_REG_FBC_PDO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_PDO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_PDO_CTL2(reg_module),
			 fbc_ctrl2[_pdo_].Raw);
	}

	if (DmaEnStatus[_tsfso_]) {
		fbc_ctrl2[_tsfso_].Raw =
			ISP_RD32(CAM_REG_FBC_TSFSO_CTL2(reg_module));
		ISP_WR32(CAM_REG_FBC_TSFSO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_TSFSO_CTL1(reg_module)) |
			  0x100));
		ISP_WR32(CAM_REG_FBC_TSFSO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_TSFSO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_TSFSO_CTL2(reg_module),
			 fbc_ctrl2[_tsfso_].Raw);
	}

	if (DmaEnStatus[_ltmso_]) {
		fbc_ctrl2[_ltmso_].Raw =
			ISP_RD32(CAM_REG_FBC_LTMSO_CTL2(reg_module));
		ISP_WR32(CAM_REG_FBC_LTMSO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_LTMSO_CTL1(reg_module)) |
			  0x100));
		ISP_WR32(CAM_REG_FBC_LTMSO_CTL1(reg_module),
			 (ISP_RD32(CAM_REG_FBC_LTMSO_CTL1(reg_module)) &
			  (~(0x100))));
		ISP_WR32(CAM_REG_FBC_LTMSO_CTL2(reg_module),
			 fbc_ctrl2[_ltmso_].Raw);
	}
	IRQ_LOG_KEEPER(irq_type, m_CurrentPPB, _LOG_INF,
...
#define ISP_MODULE_GROUPS 7
	...
	if ((dmaerr[_aaho_] | dmaerr[_flko_] | dmaerr[_ufeo_] | dmaerr[_afo_] |
	     dmaerr[_ufgo_] | dmaerr[_rsso_]) != NO_DMA_ERROR)
		IRQ_LOG_KEEPER(
			module, m_CurrentPPB, _LOG_ERR,
			"AAHO=0x%x,FLKO=0x%x,UFEO=0x%x,AFO=0x%x,UFGO=0x%x,RSSO=0x%x\n",
			dmaerr[_aaho_], dmaerr[_flko_], dmaerr[_ufeo_],
			dmaerr[_afo_], dmaerr[_ufgo_], dmaerr[_rsso_]);
	...
	if ((dmaerr[_lmvo_] | dmaerr[_yuvbo_] | dmaerr[_tsfso_] |
		...
		IRQ_LOG_KEEPER(
			...
			dmaerr[_lmvo_], dmaerr[_yuvbo_], dmaerr[_tsfso_],
			dmaerr[_pdo_], dmaerr[_crzo_], dmaerr[_crzbo_]);

	if ((dmaerr[_yuvco_] | dmaerr[_crzo_r2_] | dmaerr[_rsso_r2_] |
	     dmaerr[_yuvo_]) != NO_DMA_ERROR)
		IRQ_LOG_KEEPER(
			module, m_CurrentPPB, _LOG_ERR,
	...
	if ((dmaerr[_rawi_] | dmaerr[_rawi_r3_]) != NO_DMA_ERROR)
		IRQ_LOG_KEEPER(
			...
			0, dmaerr[_rawi_], dmaerr[_rawi_r3_]);

	if ((dmaerr[_bpci_] | dmaerr[_lsci_] | dmaerr[_bpci_r2_] |
	     dmaerr[_pdi_] | dmaerr[_ufdi_r2_]) != NO_DMA_ERROR)
		IRQ_LOG_KEEPER(
			module, m_CurrentPPB, _LOG_ERR,
			"%s:BPCI=0x%x,LSCI=0x%x,BPCI_R2=0x%x,PDI=0x%x,UFDI_R2=0x%x\n",
			cam, dmaerr[_bpci_], dmaerr[_lsci_], dmaerr[_bpci_r2_],
			dmaerr[_pdi_], dmaerr[_ufdi_r2_]);
	...
	IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_ERR,
		       "%s Module[%d] Req/Rdy Status: 0=(0x%08x 0x%08x)", cam,
		       innerRegModule, moduleReqStatus[0], moduleRdyStatus[0]);
	IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_ERR,
...
static inline void Prepare_Enable_ccf_clock(enum ISP_DEV_NODE_ENUM module)
{
	...
	if (ret < 0)
		LOG_NOTICE(
			"cannot pm runtime get ISP_CAMSYS_CONFIG_IDX mtcmos\n");

	if (module == ISP_CAM_A_IDX) {
		ret = pm_runtime_get_sync(isp_devs[ISP_CAM_A_IDX].dev);
		if (ret < 0)
			LOG_NOTICE(
				"cannot pm runtime get ISP_CAM_A_IDX mtcmos\n");
	}
	...
	if (module == ISP_CAM_B_IDX) {
		...
		if (ret < 0)
			LOG_NOTICE(
				"cannot pm runtime get ISP_CAM_B_IDX mtcmos\n");
	}
	...
	if (IS_3RAW_PLATFORM(g_platform_id)) {
		if (module == ISP_CAM_C_IDX) {
			...
			if (ret < 0)
				LOG_NOTICE(
					"cannot pm runtime get ISP_CAM_C_IDX mtcmos\n");
		}
	}
	...
	if (module == ISP_CAMSV0_IDX) {
		if (IS_LARB14_CGPDN_NECESSARY(g_platform_id)) {
			...
			if (ret)
				LOG_NOTICE(
					"cannot pre-en CAMSYS_LARB14_CGPDN clock\n");
		}
	}
	...
	if (IS_CAM2MM_GALS_CGPDN_NECESSARY(g_platform_id)) {
		...
		if (ret)
			LOG_NOTICE(
				"cannot pre-en CAMSYS_CAM2MM_GALS_CGPDN clock\n");
	}
	...
	if (IS_CAMSV_TOP0_AVAILABLE(g_platform_id)) {
		if (module == ISP_CAMSV0_IDX || module == ISP_CAMSV1_IDX) {
			...
			if (ret)
				LOG_NOTICE(
					"cannot pre-en ISP_CAM_CAMSV0 clock\n");
		}
	}
	...
	if (module == ISP_CAM_C_IDX) {
		if (IS_3RAW_PLATFORM(g_platform_id)) {
			...
			if (ret)
				LOG_NOTICE(
					"cannot pre-en ISP_CAM_LARB18_RAWC clock\n");
			ret = clk_prepare_enable(isp_clk.ISP_CAM_SUBSYS_RAWC);
			if (ret)
				LOG_NOTICE(
					"cannot pre-en ISP_CAM_SUBSYS_RAWC clock\n");
			ret = clk_prepare_enable(isp_clk.ISP_CAM_TG_RAWC);
			if (ret)
				LOG_NOTICE(
					"cannot pre-en ISP_CAM_TG_RAWC clock\n");
		}
	}
...
static inline void Disable_Unprepare_ccf_clock(enum ISP_DEV_NODE_ENUM module)
{
	...
	if (module == ISP_CAM_C_IDX) {
		if (IS_3RAW_PLATFORM(g_platform_id)) {
			...
			if (ret < 0)
				LOG_NOTICE(
					"cannot pm runtime put ISP_CAM_C_IDX mtcmos\n");
		}
	}
	...
	if (module == ISP_CAM_B_IDX) {
		...
		if (ret < 0)
			LOG_NOTICE(
				"cannot pm runtime put ISP_CAM_B_IDX mtcmos\n");
	}
	...
	if (module == ISP_CAM_A_IDX) {
		...
		if (ret < 0)
			LOG_NOTICE(
				"cannot pm runtime put ISP_CAM_A_IDX mtcmos\n");
	}

	ret = pm_runtime_put_sync(isp_devs[ISP_CAMSYS_CONFIG_IDX].dev);
	if (ret < 0)
		LOG_NOTICE(
			"cannot pm runtime put ISP_CAMSYS_CONFIG_IDX mtcmos\n");
}
...
static void ISP_EnableClock(enum ISP_DEV_NODE_ENUM module, bool En)
{
	if (IS_2RAW_PLATFORM(g_platform_id)) {
		if (module == ISP_CAM_C_IDX) {
			LOG_NOTICE(
				"Not supported to ctrl clock: ISP_CAM_C_IDX\n");
			return;
		}
	}
	...
	if (En) {
		...
		LOG_INF("camsyscg org:0x%x,%x,%x,%x new:0x%x,%x,%x,%x cnt:%d\n",
			cg_con1, cg_con3, cg_con5, cg_con7, cg_con2, cg_con4,
			cg_con6, cg_con8, G_u4EnableClockCount[module]);
		...
		Prepare_Enable_ccf_clock(
			module); /* !!cannot be used in spinlock!! */
	...
	case ISP_CAM_C_IDX: {
		...
		while ((ISP_RD32(CAM_REG_CTL_SW_CTL(module)) & 0x2) != 0x2) {
			...
			if (((usec - m_usec) > timeoutMs) &&
			    (bDumped == MFALSE)) {
				LOG_INF("%d: wait SW idle timeout, reg(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x)\n",
					module,
					(unsigned int)ISP_RD32(
...
static int ISP_ReadReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	pReg = kmalloc((pRegIo->Count) * sizeof(struct ISP_REG_STRUCT),
		       GFP_ATOMIC);
	...
	if (copy_from_user(pReg, (void __user *)pRegIo->pData,
			   sizeof(struct ISP_REG_STRUCT) * pRegIo->Count) !=
	    0) {
	...
	for (i = 0; i < pRegIo->Count; i++) {
		...
		if (put_user(reg.Val, (unsigned int *)&(pRegIo->pData->Val)) !=
		    0) {
...
{ /* FIX to avoid build warning */
	...
	spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[dev_node_irqinfo_Type]),
			       flags);

	/* 2. force to wake up the user that are waiting for that signal */
	if (ISP_CheckUseCamWaitQ(dev_node_irqinfo_Type,
				 irqinfo->EventInfo.St_type,
				 irqinfo->EventInfo.Status)) {
		...
		wake_up_interruptible(
			&IspInfo.WaitQueueHead[dev_node_irqinfo_Type]);
	}
...
static int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (IspInfo.DebugMask & ISP_DBG_INT) {
		if (WaitIrq->EventInfo.Status &
		    IspInfo.IrqInfo.Mask[dev_node_WaitIrq_Type]
					[WaitIrq->EventInfo.St_type]) {
			if (WaitIrq->EventInfo.UserKey > 0) {
				LOG_DBG("+WaitIrq Clear(%d), Type(%d), Status(0x%08X), Timeout(%d/%d),user(%d)\n",
					...
					WaitIrq->EventInfo.Status, Timeout,
	...
	if (WaitIrq->EventInfo.Clear == ISP_IRQ_CLEAR_STATUS) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[dev_node_WaitIrq_Type]),
				  flags);
		IspInfo.IrqInfo.Status[dev_node_WaitIrq_Type]
				      [WaitIrq->EventInfo.St_type]
				      [WaitIrq->EventInfo.UserKey] &=
			(~WaitIrq->EventInfo.Status);
	...
	if (WaitIrq->EventInfo.Clear == ISP_IRQ_CLEAR_NONE) {
		if (IspInfo.IrqInfo.Status[dev_node_WaitIrq_Type]
					  [WaitIrq->EventInfo.St_type]
					  [WaitIrq->EventInfo.UserKey] &
		    WaitIrq->EventInfo.Status) {
#ifdef ENABLE_WAITIRQ_LOG
	...
	if (WaitIrq->EventInfo.UserKey == 1) {
		...
		if (irq3a_print_vf_off[dev_node_WaitIrq_Type]) {
			LOG_NOTICE(
				...
				irq3a_wait_cnt, WaitIrq->EventInfo.Clear,
				dev_node_WaitIrq_Type,
				WaitIrq->EventInfo.St_type, irqStatus,
				WaitIrq->EventInfo.Status,
				WaitIrq->EventInfo.Timeout,
	...
	if (ISP_CheckUseCamWaitQ(dev_node_WaitIrq_Type,
				 WaitIrq->EventInfo.St_type,
				 WaitIrq->EventInfo.Status)) {
	...
	if (Timeout == 0) {
		...
		LOG_INF("Done WaitIrq Clear(%d) Type(%d) StType(%d) Status(0x%08X) WaitStatus(0x%08X) Timeout(%d) key(%d)\n",
			WaitIrq->EventInfo.Clear, dev_node_WaitIrq_Type,
			WaitIrq->EventInfo.St_type, irqStatus,
			WaitIrq->EventInfo.Status, WaitIrq->EventInfo.Timeout,
			WaitIrq->EventInfo.UserKey);
...
static inline void ISP_StopHW(unsigned int module)
{
	...
	while ((ISP_RD32(CAM_REG_CTL_SW_CTL(module)) & 0x2) != 0x2) {
		...
		if ((sec - m_sec) > timeoutMsRst) {
			...
			if (sec_on) {
				LOG_INF("%d: wait SW idle timeout, reg(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x)\n",
					module,
					(unsigned int)ISP_RD32(
						CAM_REG_TG_SEN_MODE(module)),
					(unsigned int)ISP_RD32(
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_GET_INT_ERR:
		...
		if (IS_CAMSV_TOP0_NOT_AVAILABLE(g_platform_id)) {
			if (copy_to_user(
				    (void *)Param, (void *)g_ISPIntStatus,
			...
			if (copy_to_user(
				    (void *)((struct ISP_RAW_INT_STATUS *)Param +
					     ISP_IRQ_TYPE_INT_CAMSV_START_ST),
				    (void *)((struct ISP_RAW_INT_STATUS *)
						     g_ISPIntStatus +
	...
	case ISP_WRITE_REGISTER: {
		if (copy_from_user(&RegIo, (void *)Param,
			...
			if ((RegIo.Count * sizeof(struct ISP_REG_STRUCT)) >
			    0xFFFFF000) {
				Ret = -EFAULT;
	...
	case ISP_CLEAR_IRQ: {
		if (copy_from_user(&ClearIrq, (void *)Param,
			...
			LOG_DBG("ISP_CLEAR_IRQ:Type(%d),Status(0x%x),st_status(%d),IrqStatus(0x%x)\n",
				dev_node_irq, ClearIrq.EventInfo.Status,
				ClearIrq.EventInfo.St_type,
				IspInfo.IrqInfo
					.Status[dev_node_irq]
	...
	case ISP_REGISTER_IRQ_USER_KEY:
		if (copy_from_user(
			...
			RegUserKey.userName[sizeof(RegUserKey.userName) - 1] =
				'\0';
...
#define TS_TYPE (1)
	...
	case ISP_NOTE_CQTHR0_BASE: {
		...
		memset((void *)&cq0_note, 0x0,
		       sizeof(struct ISP_CQ0_NOTE_INFO));
		...
		case 1: /* 2 RAWS to 1 tg for static twin mode */
			if (multiRAWConfig.master_module ==
				...
				if ((Addr[ISP_IRQ_TYPE_INT_CAM_A_ST] != 0) &&
					...
					spin_lock(&(IspInfo.SpinLockClock));
					if (G_u4EnableClockCount[ISP_CAM_A_IDX] !=
						    0 &&
					    G_u4EnableClockCount[ISP_CAM_C_IDX] !=
						    0) {
						spin_unlock(&(
							IspInfo.SpinLockClock));
				...
				if ((Addr[ISP_IRQ_TYPE_INT_CAM_B_ST] != 0) &&
					...
					spin_lock(&(IspInfo.SpinLockClock));
					if (G_u4EnableClockCount[ISP_CAM_B_IDX] !=
						    0 &&
					    G_u4EnableClockCount[ISP_CAM_C_IDX] !=
						    0) {
						spin_unlock(&(
				...
				if ((Addr[ISP_IRQ_TYPE_INT_CAM_A_ST] != 0) &&
					...
					if (G_u4EnableClockCount[ISP_CAM_A_IDX] !=
						    0 &&
					    G_u4EnableClockCount[ISP_CAM_B_IDX] !=
						    0) {
						spin_unlock(&(
							IspInfo.SpinLockClock));
		...
		if (copy_from_user(Dapc_Reg, (void *)Param,
			...
			if (Dapc_Reg[1] == MTRUE) {
				...
				lock_reg.CAM_REG_CTL_SEL2[Dapc_Reg[0]] =
					Dapc_Reg[8];
				LOG_INF("[DAPC]EN:0x%x EN2:0x%x EN3:0x%x\n",
					lock_reg.CAM_REG_CTL_EN[Dapc_Reg[0]],
					lock_reg.CAM_REG_CTL_EN2[Dapc_Reg[0]],
					lock_reg.CAM_REG_CTL_EN3[Dapc_Reg[0]]);
				LOG_INF("[DAPC]DMA:0x%x DMA2:0x%x SEL:0x%x SEL2:0x%x\n",
					lock_reg.CAM_REG_CTL_DMA_EN[Dapc_Reg[0]],
					lock_reg.CAM_REG_CTL_DMA2_EN[Dapc_Reg[0]],
					lock_reg.CAM_REG_CTL_SEL[Dapc_Reg[0]],
					lock_reg.CAM_REG_CTL_SEL2[Dapc_Reg[0]]);
	...
	case ISP_SET_VIR_CQCNT: {
		...
		if (copy_from_user(&_cq_cnt, (void *)Param,
			...
			LOG_NOTICE("Virtual CQ count copy_from_user failed\n");
			Ret = -EFAULT;
		}
	} break;
	case ISP_POWER_CTRL: {
		...
		if (copy_from_user(&pwrctl, (void *)Param,
			...
			LOG_NOTICE("isp pwr_ctrl(%d,%d) enableClkCnt(%d)\n",
				   dev_node_idx, pwrctl[1],
				   G_u4EnableClockCount[dev_node_idx]);
...
static unsigned int NodeName_to_DevIdx(const char *name)
{
	...
	else if (strncmp(name, "camsys_rawa_legacy",
			 strlen("camsys_rawa_legacy")) == 0)
		return ISP_CAMSYS_RAWA_CONFIG_IDX;
	else if (strncmp(name, "camsys_rawb_legacy",
			 strlen("camsys_rawb_legacy")) == 0)
		return ISP_CAMSYS_RAWB_CONFIG_IDX;
	else if (strncmp(name, "camsys_rawc_legacy",
			 strlen("camsys_rawc_legacy")) == 0)
		return ISP_CAMSYS_RAWC_CONFIG_IDX;
	else if (strncmp(name, "cam1_inner_legacy",
			 strlen("cam1_inner_legacy")) == 0)
		return ISP_CAM_A_INNER_IDX;
	else if (strncmp(name, "cam2_inner_legacy",
			 strlen("cam2_inner_legacy")) == 0)
		return ISP_CAM_B_INNER_IDX;
	else if (strncmp(name, "cam3_inner_legacy",
			 strlen("cam3_inner_legacy")) == 0)
...
static void ISP_add_device_link(struct platform_device *pDev)
{
	...
	mtk_larb = of_count_phandle_with_args(pDev->dev.of_node,
					      "mediatek,larb", NULL);
	mtk_larbs = of_count_phandle_with_args(pDev->dev.of_node,
					       "mediatek,larbs", NULL);
	...
	for (i = 0; i < larb_num; i++) {
		larb_node =
			of_parse_phandle(pDev->dev.of_node, mtk_larb_str, i);
		if (!larb_node) {
		...
		if (of_property_read_u32(larb_node, "mediatek,larb-id",
					 &larb_id))
...
/* Get platform_device parameters */
	...
	if ((dev_idx >= ISP_DEV_NODE_NUM) ||
	    (dev_idx < ISP_CAMSYS_CONFIG_IDX)) {
		LOG_NOTICE(
	...
	*(isp_devs[dev_idx].dev->dma_mask) = (u64)DMA_BIT_MASK(34);
	...
	if ((strncmp(pDev->dev.of_node->name, "cam1_legacy",
		     strlen("cam1_legacy")) == 0) ||
	    (strncmp(pDev->dev.of_node->name, "cam2_legacy",
		     strlen("cam2_legacy")) == 0) ||
	    (strncmp(pDev->dev.of_node->name, "cam3_legacy",
		     strlen("cam3_legacy")) == 0) ||
	    (strncmp(pDev->dev.of_node->name, "camisp_legacy",
	...
	if (isp_devs[dev_idx].irq > 0) {
		...
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if ((strcmp(pDev->dev.of_node->name,
				...
				LOG_INF("G_u4DevNodeCt=%d, devnode(%s), irq=%d, ISR: %s\n",
					atomic_read(&G_u4DevNodeCt),
					pDev->dev.of_node->name,
					isp_devs[dev_idx].irq,
					IRQ_CB_TBL[i].device_name);
	...
	if (atomic_read(&G_u4DevNodeCt) == 1) {
		...
		if (IS_CAM2MM_GALS_CGPDN_NECESSARY(g_platform_id)) {
			isp_clk.CAMSYS_CAM2MM_GALS_CGPDN = devm_clk_get(
				&pDev->dev, "CAMSYS_MAIN_CAM2MM_GALS_CGPDN");
		}

		if (IS_CCU_AVAILABLE(g_platform_id)) {
			isp_clk.CAMSYS_TOP_MUX_CCU = devm_clk_get(
				&pDev->dev, "TOPCKGEN_TOP_MUX_CCU");
		...
		if (IS_3RAW_PLATFORM(g_platform_id)) {
			isp_clk.ISP_CAM_LARB18_RAWC = devm_clk_get(
				&pDev->dev, "CAMSYS_RAWCLARB18_CGPDN");
			isp_clk.ISP_CAM_SUBSYS_RAWC = devm_clk_get(
				&pDev->dev, "CAMSYS_RAWCCAM_CGPDN");
		...
		if (IS_LARB14_CGPDN_NECESSARY(g_platform_id)) {
			if (IS_ERR(isp_clk.CAMSYS_LARB14_CGPDN)) {
				LOG_NOTICE(
					"cannot get CAMSYS_LARB14_CGPDN clock\n");
		...
		if (IS_LARB15_CGPDN_NECESSARY(g_platform_id)) {
			if (IS_ERR(isp_clk.CAMSYS_LARB15_CGPDN)) {
				LOG_NOTICE(
					"cannot get CAMSYS_LARB15_CGPDN clock\n");
...
#if CAM_USE_PM_WAKELOCKS
		isp_wake_lock =
			wakeup_source_register(&pDev->dev, "isp_lock_wakelock");
#else
	wake_lock_init(&isp_wake_lock, WAKE_LOCK_SUSPEND, "isp_lock_wakelock");
		...
		isr_log_wq = alloc_ordered_workqueue(
			"ISP_BH_Workqueue",
		...
		IspInfo.IrqInfo.WarnMask[ISP_IRQ_TYPE_INT_CAM_C_ST][SIGNAL_INT] =
			INT_ST_MASK_CAM_WARN;
	...
	for (i = ISP_CAM_A_IDX; i < ISP_DEV_NODE_NUM; i++) {
		...
		if (!G_u4EnableClockCount[i]) {
			...
			LOG_DBG("dev(%d) - X. UserCount=%d,Cnt:%d\n", i,
				IspInfo.UserCount, G_u4EnableClockCount[i]);
...
static int __init ISP_Init(void)
{
	...
	debugfs_create_x32("platform", 0644, camisp_dbg_root, &g_platform_id);
#endif
	...
	for (j = RUNTIME_ISP_CAMSV_START_ST; j <= ISP_IRQ_TYPE_INT_CAMSV_END_ST;
	     j++)
...
static void __exit ISP_Exit(void)
{
	...
	/*  */
	for (i = RUNTIME_ISP_CAMSV_START_ST; i <= ISP_IRQ_TYPE_INT_CAMSV_END_ST;
	     i++)
				     ...
				     unsigned int *FBCurStat)
{
	...
	if (dma_en & _TSFSO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_TSFSO_CTL2(module));
	...
	if (dma_en & _AAO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_AAO_CTL2(module));
	...
	if (dma_en & _AAHO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_AAHO_CTL2(module));
	...
	if (dma_en & _FLKO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_FLKO_CTL2(module));
	...
	if (dma_en & _AFO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_AFO_CTL2(module));
	...
	if (dma_en & _PDO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_PDO_CTL2(module));
	...
	if (dma_en & _LTMSO_R1_EN_) {
		fbc_ctrl2_STT.Raw = ISP_RD32(CAM_REG_FBC_LTMSO_CTL2(module));
				   ...
				   struct S_START_T *pTstp)
{
	...
	if (pTstp) {
		*pTstp =
			IspInfo.TstpQInfo[module].Dmao[dma_id].TimeQue
		     ...
		     unsigned int reg_module_count)
{
	...
	for (i = 0; i < reg_module_count; i++) {
		...
		LOG_NOTICE("(CQ thread 0x%08x)(CQ state 0x%08x)", dbgPort[12],
			   dbgPort[13]);
		     ...
		     unsigned int *reg_module_count)
{
	...
	case ISP_IRQ_TYPE_INT_CAM_C_ST:
		*reg_module = ISP_CAM_C_IDX;
		break;
	default:
		LOG_NOTICE("Wrong IRQ module: %d", (unsigned int)irq_module);
		break;
	}
...
int CQ_Recover_middle(unsigned int reg_module, unsigned int *reg_module_array,
		      unsigned int reg_module_count)
{
	union CAMCQ_CQ_CTL_ cq_ctrl;
	...
	do {
		...
		for (i = 0; i < reg_module_count; i++) {
			tmp_module = reg_module_array[i];
			ISP_WR32(CAM_REG_CTL_SW_CTL(tmp_module), 0x0);
			ISP_WR32(CAM_REG_CTL_SW_CTL(tmp_module),
				 0x1); /*SW_RST_TRIG*/
		...
		for (i = 0; i < reg_module_count; i++) {
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ9 base:0x2%x(0x2%x)\n", index,
					   g_cqBaseAddr[index][9],
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR9_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][9] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR9_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR9_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][9]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ10 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR10_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][10] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR10_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR10_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][10]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ11 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR11_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][11] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR11_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR11_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][11]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ12 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR12_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][12] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR12_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR12_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][12]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ13 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR13_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][13] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR13_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR13_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][13]);
				...
				LOG_NOTICE("[%d]CQ14 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR14_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][14] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR14_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR14_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][14]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				LOG_NOTICE("[%d]CQ17 base:0x2%x(0x2%x)\n",
					   ...
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR17_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][17] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR17_BASEADDR(
						     tmp_module))) != 0) {
					ISP_WR32(CAM_REG_CQ_THR17_BASEADDR(
							 tmp_module),
						 g_cqBaseAddr[index][17]);
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
					...
					if ((usec - m_usec) > timeoutMs) {
						LOG_NOTICE(
							"wait CQ0 timeout0x%x,0x%x\n",
							(unsigned int)ISP_RD32(
								CAM_REG_CTL_START_ST(
									tmp_module)),
							cq_done);
						ret = MFALSE;
						break;
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR0_DONE_ST;
				}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR1_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR1_DONE_ST;
				}
			}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR9_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR9_DONE_ST;
				}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR10_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR10_DONE_ST;
				}
			}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR11_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR11_DONE_ST;
				}
			}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR12_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR12_DONE_ST;
				}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR13_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR13_DONE_ST;
				}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR14_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR14_DONE_ST;
				}
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR16_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR16_DONE_ST;
				}
				LOG_NOTICE("wait CQ16 start 0x%x, base 0x%x\n",
					   (unsigned int)ISP_RD32(
						   CAM_REG_CTL_START_ST(
							   tmp_module)),
					   (unsigned int)ISP_RD32(
						   CAM_REG_CQ_THR16_BASEADDR(
							   tmp_module)));
				if ((g_cqBaseAddr[index][16] -
				     (unsigned int)ISP_RD32(
					     CAM_REG_CQ_THR16_BASEADDR(
						     tmp_module))) == 0)
					ret = MFALSE;
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR17_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR17_DONE_ST;
				}
		...
		if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
			...
			if (cq_ctrl.Bits.CAMCQ_CQ_EN == 0x1) {
				...
				while ((ISP_RD32(CAM_REG_CTL_START_ST(
						tmp_module)) &
					en_ctlStart.Bits.CQ_THR24_START) ||
				       (cq_done == 0)) {
					sec = ktime_get(); /* ns */
					do_div(sec, 1000); /* usec */
					usec = do_div(
					...
					DmaStatus6.Raw = g_cqDoneStatus[index];
					cq_done |=
						DmaStatus6.Bits.CQ_THR24_DONE_ST;
				}
			}
		}
		reset_count--;
...
int CQ_Recover_final(unsigned int reg_module, unsigned int *reg_module_array,
		     unsigned int reg_module_count)
{
	unsigned int i = 0;
			...
			enum ISP_DEV_NODE_ENUM cam_idx)
{
	...
	isStagger =
		((ISP_RD32(CAM_REG_TG_SEN_MODE(reg_module)) & 0x00400000)) >>
		22;
	...
	if ((IrqStatus & SOF_INT_ST) &&
	    (((ISP_RD32(CAM_REG_TG_SEN_MODE(reg_module)) & 0x00040000) >> 18) ==
	     0x1) &&
	    (twinStatus.Bits.TWIN_EN == MTRUE) && (cq_recovery[module] == 1)) {
		if ((ISP_RD32(CAM_REG_RRZO_FH_SPARE_2(reg_module)) !=
		     pre_magic_num) &&
		    (CAM_FST_LAST_WORKING_FRAME ==
		     Irq_CAM_FrameStatus(reg_module, module, 0,
					 &FBC_CurState))) {
			if (g_cq0NextBA[CAM_A][0] != 0 &&
			    twinStatus.Bits.SLAVE_CAM_NUM == 0x1) {
				if (ISP_RD32(CAM_REG_CQ_THR0_BASEADDR(
					    reg_module)) !=
					    g_cq0NextBA[CAM_A][0] ||
				   ...
				   twinStatus.Bits.SLAVE_CAM_NUM == 0x2) {
				if ((ISP_RD32(CAM_REG_CQ_THR0_BASEADDR(
					     reg_module)) !=
				     g_cq0NextBA[CAM_A][0]) ||
				    (ISP_RD32(CAM_REG_CQ_THR0_BASEADDR(
	...
	if (IrqStatus & HW_PASS1_DON_ST) {
		...
		spin_lock(&(IspInfo.SpinLockIrq[module]));
		IrqStatusX =
			ISP_RD32(CAM_REG_CTL_RAW_INT_STATUSX(inner_reg_module));
		Irq2StatusX = ISP_RD32(
			CAM_REG_CTL_RAW_INT2_STATUSX(inner_reg_module));
		Irq3StatusX = ISP_RD32(
			CAM_REG_CTL_RAW_INT3_STATUSX(inner_reg_module));
		Irq4StatusX = ISP_RD32(
			CAM_REG_CTL_RAW_INT4_STATUSX(inner_reg_module));
		Irq5StatusX = ISP_RD32(
			CAM_REG_CTL_RAW_INT5_STATUSX(inner_reg_module));
		spin_unlock(&(IspInfo.SpinLockIrq[module]));
#if 0
			...
			if (Irq2StatusX != 0) {
				IRQ_LOG_KEEPER(
					...
					(sof_count[module]) ?
						(sof_count[module] - 1) :
						(sof_count[module]),
					HwP1Done_cnt[module], Irq2StatusX,
					(unsigned int)ISP_RD32(
					...
					(unsigned int)ISP_RD32(
						CAM_REG_CTL_RAW_INT_STATUSX(
							ISP_CAM_C_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_BASE_ADDR(
						ISP_CAM_A_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_BASE_ADDR(
						ISP_CAM_B_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_BASE_ADDR(
						ISP_CAM_C_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_9(
						ISP_CAM_A_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_9(
						ISP_CAM_B_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_9(
						ISP_CAM_C_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_10(
						ISP_CAM_A_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_10(
						ISP_CAM_B_INNER_IDX)),
					ISP_RD32(CAM_REG_FLKO_FH_FH_SPARE_10(
						ISP_CAM_C_INNER_IDX)),
					ISP_RD32(CAM_REG_CQ_THR14_BASEADDR(
						ISP_CAM_A_INNER_IDX)),
					ISP_RD32(CAM_REG_CQ_THR14_BASEADDR(
						ISP_CAM_B_INNER_IDX)),
					ISP_RD32(CAM_REG_CQ_THR14_BASEADDR(
						ISP_CAM_C_INNER_IDX)),
					ISP_RD32(CAM_REG_FBC_FLKO_CTL2(
						ISP_CAM_A_INNER_IDX)),
					ISP_RD32(CAM_REG_FBC_FLKO_CTL2(
						ISP_CAM_B_INNER_IDX)),
					ISP_RD32(CAM_REG_FBC_FLKO_CTL2(
	...
	if ((IrqStatus & SOF_INT_ST) && isStagger == 1)
		IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_ERR, "CAM%c DCIF_SOF",
			       'A' + cardinalNum);
	...
	if (((IrqStatus & SOF_INT_ST) && isStagger == 0) ||
		...
		FrameStatus[module] = Irq_CAM_FrameStatus(
			reg_module, module, irqDelay, &FBC_CurState);
		...
		if ((ISP_RD32(CAM_REG_DMA_CQ_COUNTER(reg_module))) !=
			...
			IRQ_LOG_KEEPER(
				...
				g_virtual_cq_cnt[module], IrqStatus);
		}
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			{
				unsigned long long cur_timestp =
					(unsigned long long)sec * 1000000 +
					usec;
			 ...
			 */
				...
				if (IspInfo.TstpQInfo[module]
					    .DmaEnStatus[_ufgo_]) {
					ISP_CompensateMissingSofTime(
						reg_module, module, _ufgo_, sec,
						usec, frmPeriod);
				}
				if (IspInfo.TstpQInfo[module]
					    .DmaEnStatus[_lmvo_]) {
					ISP_CompensateMissingSofTime(
						reg_module, module, _lmvo_, sec,
						usec, frmPeriod);
				}
				if (IspInfo.TstpQInfo[module]
					    .DmaEnStatus[_lcso_]) {
...
/* must after current ISP_PushBufTimestamp() */
			...
			if (sec_on)
				IRQ_LOG_KEEPER(
					...
					cur_v_cnt,
					(unsigned int)(ISP_RD32(
						CAM_REG_FBC_IMGO_CTL1(
							reg_module))),
					(unsigned int)(ISP_RD32(
						CAM_REG_FBC_IMGO_CTL2(
							reg_module))),
					(unsigned int)(ISP_RD32(
						CAM_REG_FBC_RRZO_CTL1(
							reg_module))),
					(unsigned int)(ISP_RD32(
					...
					(unsigned int)ISP_RD32(
						CAM_REG_CQ_THR0_BASEADDR(
							ISP_CAM_B_INNER_IDX)),
					(unsigned int)ISP_RD32(CAM_REG_CTL_MISC(
						ISP_CAM_A_IDX)),
					(unsigned int)ISP_RD32(CAM_REG_CTL_MISC(
						ISP_CAM_A_INNER_IDX)),
					(unsigned int)ISP_RD32(CAM_REG_CTL_MISC(
				...
				IRQ_LOG_KEEPER(
					...
					(unsigned int)ISP_RD32(
						CAM_REG_CTL_EN(ISP_CAM_A_IDX)),
					(unsigned int)ISP_RD32(CAM_REG_CTL_EN(
						ISP_CAM_A_INNER_IDX)),
					(unsigned int)ISP_RD32(
						CAM_REG_CTL_EN(ISP_CAM_B_IDX)),
					(unsigned int)ISP_RD32(CAM_REG_CTL_EN(
					...
					(unsigned int)ISP_RD32(CAM_REG_CTL_EN2(
						ISP_CAM_A_INNER_IDX)),
					(unsigned int)ISP_RD32(
			...
			if (snprintf(gPass1doneLog[module]._str, P1DONE_STR_LEN,
				     "\\") < 0)
				LOG_NOTICE("Error: snprintf fail\n");
...
/* sw workaround for CQ byebye */
			...
			ret = CQ_Recover_middle(reg_module, reg_module_array,
						reg_module_count);
			if (ret == -1)
				goto EXIT_CQ_RECOVER;
			ret = CQ_Recover_final(reg_module, reg_module_array,
					       reg_module_count);
			if (ret == -1)
				goto EXIT_CQ_RECOVER;
		EXIT_CQ_RECOVER:
