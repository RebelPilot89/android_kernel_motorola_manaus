...
#ifdef PDA_MMQOS
// mmqos relate
static const char *const mmqos_names_rdma[] = { "l25_pdai_a0", "l25_pdai_a1",
						"l26_pdai_b0", "l26_pdai_b1" };
#define PDA_MMQOS_RDMA_NUM ARRAY_SIZE(mmqos_names_rdma)
struct icc_path *icc_path_pda_rdma[PDA_MMQOS_RDMA_NUM];

static const char *const mmqos_names_wdma[] = { "l25_pdao_a", "l26_pdao_b" };
...
static void pda_mmqos_init(void)
{
	...
	for (i = 0; i < PDA_MMQOS_RDMA_NUM; ++i) {
		LOG_INF("rdma index: %d, mmqos name: %s\n", i,
			mmqos_names_rdma[i]);
		icc_path_pda_rdma[i] =
			of_mtk_icc_get(g_dev1, mmqos_names_rdma[i]);
	}
	...
	for (i = 0; i < PDA_MMQOS_WDMA_NUM; ++i) {
		LOG_INF("wdma index: %d, mmqos name: %s\n", i,
			mmqos_names_wdma[i]);
		icc_path_pda_wdma[i] =
			of_mtk_icc_get(g_dev1, mmqos_names_wdma[i]);
	}
}
...
static void pda_mmqos_bw_set(void)
{
	...
	int Mach_Input_Total_pixel_Itar =
		...
		Mach_Frame_Size_FOV + (1 * Mach_ROI_Max_Width);
	// int Mach_Input_Total_pixel_Iref = Mach_Frame_Size_FOV;
	int Required_Operation_Cycle =
		(int)(Mach_Input_Total_pixel_Itar * Operation_Margin *
		      (Search_Range + 1)) /
			Search_Range +
		1;
	...
	double OperationTime =
		(double)Required_Operation_Cycle / Freqency / 1000.0;

	// WDMA BW estimate
	double WDMA_PEAK_BW =
		(double)WDMA_Data / (double)OperationTime / 1000.0;
	double WDMA_AVG_BW =
		(double)WDMA_Data * Frame_Rate * (1.33) / 1000000.0;

	// RDMA BW estimate
	double RDMA_PEAK_BW =
		(double)RDMA_Data / (double)OperationTime / 1000.0;
	double RDMA_AVG_BW =
		(double)RDMA_Data / (1000000.0 / (double)Frame_Rate);
	...
	LOG_INF("RDMA_BW AVG/PEAK: %d/%d, WDMA_BW AVG/PEAK: %d/%d\n",
		...
		(int)MBps_to_icc(WDMA_AVG_BW), (int)MBps_to_icc(WDMA_PEAK_BW));
...
static void pda_reset(unsigned int PDA_Index)
{
	...
	while (time_before(jiffies, end)) {
		if ((PDA_RD32(PDA_devs[PDA_Index].m_pda_base +
			      PDA_PDA_DMA_RST_REG) &
		     MASK_BIT_ZERO)) {
			// equivalent to hardware reset
			PDA_WR32(PDA_devs[PDA_Index].m_pda_base +
					 PDA_PDA_TOP_CTL_REG,
				 PDA_HW_RESET);
			// clear reset signal
			PDA_WR32(PDA_devs[PDA_Index].m_pda_base +
		...
		LOG_INF("PDA%d Wait EMI request, DMA_RST:0x%x\n", PDA_Index,
			PDA_RD32(PDA_devs[PDA_Index].m_pda_base +
				 PDA_PDA_DMA_RST_REG));
...
static int Get_Input_Addr_From_DMABUF(struct PDA_Data_t_v2 *pda_PdaConfig)
{
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		LOG_INF("Left image MVA MSB = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAI_P1_BASE_ADDR_MSB_REG));
		LOG_INF("Left image MVA = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAI_P1_BASE_ADDR_REG));
	}
#endif
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		LOG_INF("Right image MVA MSB = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAI_P2_BASE_ADDR_MSB_REG));
		LOG_INF("Right image MVA = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAI_P2_BASE_ADDR_REG));
	}
#endif
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		PDA_WR32(PDA_devs[i].m_pda_base +
				 PDA_PDATI_P1_BASE_ADDR_MSB_REG,
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		PDA_WR32(PDA_devs[i].m_pda_base +
				 PDA_PDATI_P2_BASE_ADDR_MSB_REG,
...
static int Get_Output_Addr_From_DMABUF(struct PDA_Data_t_v2 *pda_PdaConfig)
{
	...
	g_OutputBufferAddr =
		(unsigned long)sg_dma_address(g_output_mmu.sgt->sgl);
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		LOG_INF("Output buffer MVA MSB = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAO_P1_BASE_ADDR_MSB_REG));
		LOG_INF("Output buffer MVA = 0x%x\n",
			PDA_RD32(PDA_devs[i].m_pda_base +
				 PDA_PDAO_P1_BASE_ADDR_REG));
	}
#endif
...
static void HWDMASettings(struct PDA_Data_t_v2 *pda_PdaConfig)
{
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		...
		// --------- DMA Secure part -------------
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_SECURE_REG,
			 0x9daf851f);
		...
		// Left image
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P1_CON0_REG,
			 0x10000134);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P1_CON1_REG,
			 0x104d004d);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P1_CON2_REG,
			 0x009a009a);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P1_CON3_REG,
			 0x00e700e7);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P1_CON4_REG,
			 0x009a009a);

		// Left table
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P1_CON0_REG,
			 0x1000004c);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P1_CON1_REG,
			 0x10130013);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P1_CON2_REG,
			 0x00260026);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P1_CON3_REG,
			 0x00390039);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P1_CON4_REG,
			 0x00260026);

		// Right image
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P2_CON0_REG,
			 0x10000134);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P2_CON1_REG,
			 0x104d004d);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P2_CON2_REG,
			 0x009a009a);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P2_CON3_REG,
			 0x00e700e7);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAI_P2_CON4_REG,
			 0x009a009a);

		// Right table
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P2_CON0_REG,
			 0x1000004c);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P2_CON1_REG,
			 0x10130013);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P2_CON2_REG,
			 0x00260026);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P2_CON3_REG,
			 0x00390039);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDATI_P2_CON4_REG,
			 0x00260026);

		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_XSIZE_REG,
			 0x0000057f);

		// Output
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_CON0_REG,
			 0x10000040);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_CON1_REG,
			 0x10100010);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_CON2_REG,
			 0x00200020);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_CON3_REG,
			 0x00300030);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_CON4_REG,
			 0x00200020);

		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_DMA_EN_REG,
			 0x0000001f);
		...
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_DCM_DIS_REG,
			 0x0000007F);
		...
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_TOP_CTL_REG,
			 0x00000000);
...
static int ProcessROIData(struct PDA_Data_t_v2 *pda_data,
			  unsigned int RoiProcNum, unsigned int ROIIndex,
			  unsigned int isFixROI, unsigned int p_index)
{
	...
	if (isFixROI) {
		...
		if (pda_data->PDA_FrameSetting.PDA_CFG_1.Bits.PDA_PR_XNUM ==
		    0) {
		...
		for (j = (ROIIndex / xnum); j < ynum; j++) {
			...
			for (; i < xnum; i++) {
				...
				if (nLocalBufIndex >= PDA_MAXROI_PER_ROUND ||
				    nLocalBufIndex < 0) {
				...
				if (j != 0 && j != ynum - 1) {
					g_rgn_y_buf[nLocalBufIndex] =
						y_p + nHeight * j -
						nHeight * hoverlap / 100;
					g_rgn_h_buf[nLocalBufIndex] =
						nHeight +
						2 * nHeight * hoverlap / 100;
				} else if (j == 0) {
					g_rgn_y_buf[nLocalBufIndex] =
						y_p + nHeight * j;
					g_rgn_h_buf[nLocalBufIndex] =
				...
				g_rgn_iw_buf[nLocalBufIndex] =
					g_rgn_w_buf[nLocalBufIndex] *
					(pda_data->PDA_FrameSetting.PDA_CFG_1
						 .Bits.PDA_PAT_WIDTH /
					 pda_data->PDA_FrameSetting.PDA_CFG_1
						 .Bits.PDA_PR_XNUM);
	...
	for (i = 0; i < RoiProcNum; i++) {
		...
		if (g_rgn_w_buf[i] % nTempVar != 0) {
			...
			LOG_INF("ROI_%d_w: %d, PDA_PR_XNUM: %d\n", nROIIndex,
				g_rgn_w_buf[i], nTempVar);
			PDA_Data->Status = -10;
			return -1;
		}

		nTempVar =
			PDA_Data->PDA_FrameSetting.PDA_CFG_1.Bits.PDA_PR_YNUM;
		if (g_rgn_y_buf[i] % nTempVar != 0) {
			LOG_INF("ROI_%d ROI boundary must be on the boundary of patch\n",
				nROIIndex);
			LOG_INF("ROI_%d_y: %d, PDA_PR_YNUM: %d\n", nROIIndex,
				g_rgn_y_buf[i], nTempVar);
			PDA_Data->Status = -10;
			return -1;
		}
		...
		if (g_rgn_iw_buf[i] > 3280) {
			LOG_INF("ROI_IW_%d (%d) out of range, w:%d, pat_width:%d\n",
				nROIIndex, g_rgn_iw_buf[i], g_rgn_w_buf[i],
				PDA_Data->PDA_FrameSetting.PDA_CFG_1.Bits
					.PDA_PAT_WIDTH);
			PDA_Data->Status = -23;
			return -1;
		}
...
static void FillRegSettings(struct PDA_Data_t_v2 *pda_PdaConfig,
			    unsigned int RoiProcNum, unsigned long OuputAddr,
			    unsigned int PDA_Index)
{
	...
	for (pair = 0; pair <= ROI_MAX_INDEX; pair += 2) {
		PDA_WR32(
			(PDA_devs[PDA_Index].m_pda_base + 0x004 * (RegIndex++)),
		...
		if (pair == ROI_MAX_INDEX && pair % 2 == 0) {
			...
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base +
				  0x004 * (RegIndex++)),
				 g_rgn_x_buf[pair + 1] * 65536 +
					 g_rgn_iw_buf[pair]);
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base +
				  0x004 * (RegIndex++)),
				 g_rgn_w_buf[pair + 1] * 65536 +
					 g_rgn_y_buf[pair + 1]);
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base +
				  0x004 * (RegIndex++)),
				 g_rgn_iw_buf[pair + 1] * 65536 +
					 g_rgn_h_buf[pair + 1]);
		}
	}
...
static void TF_dump_log(unsigned int hw_trigger_num)
{
	...
	unsigned int Debug_Sel[] = {
		0x00008120, 0x0000400e, 0x0000c000, 0x11000000, 0x12000000,
		0x13000000, 0x14000000, 0x15000000, 0x16000000, 0x14000000,
		0x14100000, 0x14200000, 0x14300000, 0x14400000, 0x14500000,
		0x21000000, 0x22000000, 0x23000000, 0x24000000, 0x25000000,
		0x24000000, 0x24100000, 0x24200000, 0x24300000, 0x24400000,
		0x24500000, 0x31000000, 0x32000000, 0x33000000, 0x34000000,
		0x35000000, 0x34000000, 0x34100000, 0x34200000, 0x34300000,
		0x34400000, 0x34500000, 0x41000000, 0x42000000, 0x43000000,
		0x44000000, 0x45000000, 0x44000000, 0x44100000, 0x44200000,
		0x44300000, 0x44400000, 0x44500000, 0x51000000, 0x52000000,
		0x53000000, 0x54000000, 0x55000000, 0x54000000, 0x54100000,
		0x54200000, 0x54300000, 0x54400000, 0x54500000
	};
	...
	for (i = 0; i < hw_trigger_num; i++) {
		for (sel_index = 0; sel_index < Length_Arr; ++sel_index) {
			...
			LOG_INF("PDA_%d DEBUG_SEL/DEBUG_DATA: 0x%x/0x%x\n", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDA_DEBUG_SEL_REG),
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDA_DEBUG_DATA_REG));
		}
	}
...
#ifdef PDA_TF_DUMP_71_1
int pda_tfault_callback(int port, dma_addr_t mva, void *data)
{
	unsigned int i = 0;
	...
	LOG_INF("ImageSize/TableSize/OutputSize/ROInumber: %d/%d/%d/%d\n",
		g_pda_Pdadata.ImageSize, g_pda_Pdadata.TableSize,
		g_pda_Pdadata.OutputSize, g_pda_Pdadata.ROInumber);

	LOG_INF("FD_L_Img/R_Img/L_Tbl/R_Tbl/Out: %d/%d/%d/%d/%d\n",
		g_pda_Pdadata.FD_L_Image, g_pda_Pdadata.FD_R_Image,
		g_pda_Pdadata.FD_L_Table, g_pda_Pdadata.FD_R_Table,
		g_pda_Pdadata.FD_Output);

	LOG_INF("Status/Timeout/nNumerousROI: %d/%d/%d\n", g_pda_Pdadata.Status,
		g_pda_Pdadata.Timeout, g_pda_Pdadata.nNumerousROI);
	...
	for (i = 0; i < g_pda_Pdadata.ROInumber; ++i) {
		LOG_INF("rgn_x/y/h/w/iw[%d]: 0x%x/0x%x/0x%x/0x%x/0x%x\n", i,
			g_pda_Pdadata.rgn_x[i], g_pda_Pdadata.rgn_y[i],
			g_pda_Pdadata.rgn_h[i], g_pda_Pdadata.rgn_w[i],
...
static signed int pda_wait_irq(struct PDA_Data_t_v2 *pda_data,
			       unsigned int hw_trigger_num)
{
	int ret = 0;
	unsigned int i = 0;
	...
	ret = wait_event_interruptible_timeout(
		g_wait_queue_head, check_pda_status(hw_trigger_num),
	...
	for (i = 0; i < hw_trigger_num; i++) {
		if (PDA_devs[i].HWstatus < 0) {
			...
			LOG_INF("PDA%d PDA_PDAI_P1_ERR_STAT_REG = 0x%x", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDAI_P1_ERR_STAT_REG));
			LOG_INF("PDA%d PDA_PDATI_P1_ERR_STAT_REG = 0x%x", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDATI_P1_ERR_STAT_REG));
			LOG_INF("PDA%d PDA_PDAI_P2_ERR_STAT_REG = 0x%x", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDAI_P2_ERR_STAT_REG));
			LOG_INF("PDA%d PDA_PDATI_P2_ERR_STAT_REG = 0x%x", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDATI_P2_ERR_STAT_REG));
			LOG_INF("PDA%d PDA_PDAO_P1_ERR_STAT_REG = 0x%x", i,
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDAO_P1_ERR_STAT_REG));
...
static irqreturn_t pda_irqhandle(signed int Irq, void *DeviceId)
{
	...
	if (g_u4EnableClockCount > 0) {
		// read pda status
		nPdaStatus = PDA_RD32(PDA_devs[0].m_pda_base +
				      PDA_PDA_ERR_STAT_REG) &
			     PDA_STATUS_REG;
	}
...
static irqreturn_t pda2_irqhandle(signed int Irq, void *DeviceId)
{
	...
	if (g_u4EnableClockCount > 0) {
		// read pda status
		nPdaStatus = PDA_RD32(PDA_devs[1].m_pda_base +
				      PDA_PDA_ERR_STAT_REG) &
			     PDA_STATUS_REG;
	}
...
static int PDAProcessFunction(unsigned int nUserROINumber,
			      unsigned int nROIcount, unsigned int isFixROI,
			      unsigned int p_index)
{
	...
	while (nROIcount > 0) {
		...
		for (i = 0; i < g_PDA_quantity; i++) {
			...
			if (i == 0)
				nCurrentProcRoiIndex =
					(nUserROINumber - nROIcount);
			else if (i > 0)
				nCurrentProcRoiIndex +=
					g_CurrentProcRoiNum[i - 1];
			else
			...
			if (g_OutputBufferOffset + (g_CurrentProcRoiNum[i] *
						    OUT_BYTE_PER_ROI) >
			...
			FillRegSettings(&g_pda_Pdadata, g_CurrentProcRoiNum[i],
					(nOutputAddr + g_OutputBufferOffset),
					i);
		}
		...
		for (i = 0; i < hw_trigger_num; i++) {
			CheckAddress = PDA_RD32(PDA_devs[i].m_pda_base +
						PDA_PDAI_P1_BASE_ADDR_REG);
			CheckAddressMSB =
				PDA_RD32(PDA_devs[i].m_pda_base +
					 PDA_PDAI_P1_BASE_ADDR_MSB_REG);
...
static long PDA_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
		      unsigned long a_u4Param)
{
	...
	case PDA_GET_VERSION:
		...
		if (copy_from_user(&Init_Data, (void *)a_u4Param,
		...
		if (copy_to_user((void *)a_u4Param, &Init_Data,
				 sizeof(struct PDA_Init_Data)) != 0) {
	...
	case PDA_ENQUE_WAITIRQ_V2:
		...
		if (copy_from_user(&g_pda_Pdadata, (void *)a_u4Param,
		...
		if (g_pda_Pdadata.ROInumber > PDAROIARRAYMAX || ret) {
			...
			LOG_INF("rgn:%d,fix:%d,xnum:%d,siz:%d,FD:%d,cfg0:0x%x,sta:%d/%d\n",
				...
				g_pda_Pdadata.xnum[0], g_pda_Pdadata.OutputSize,
				g_pda_Pdadata.FD_Output,
				g_pda_Pdadata.PDA_FrameSetting.PDA_CFG_0.Raw,
				g_pda_Pdadata.Status, g_pda_Pdadata.Timeout);
		...
		g_reasonable_IRQCount =
			1 + (int)((nUserROINumber - 1) /
				  (PDA_MAXROI_PER_ROUND * g_PDA_quantity));
		// reset PDA0/PDA1 IRQ count
		g_PDA0_IRQCount = 0;
		g_PDA1_IRQCount = 0;
#ifdef FOR_DEBUG
		...
		if (PDAProcessFunction(nUserROINumber, nROIcount, MFALSE, 0) <
		    0) {
	...
	FIX_ROI:
		...
		for (i = 0; i < g_pda_Pdadata.nNumerousROI; ++i) {
			nUserROINumber =
				g_pda_Pdadata.xnum[i] * g_pda_Pdadata.ynum[i];
			...
			if (PDAProcessFunction(nUserROINumber, nROIcount, MTRUE,
					       i) < 0) {
			...
			g_OutputBufferOffset +=
				nUserROINumber * OUT_BYTE_PER_ROI;
#ifdef FOR_DEBUG
			LOG_INF("g_OutputBufferOffset: %d\n",
				g_OutputBufferOffset);
#endif
	...
	EXIT_WITHOUT_FREE_IOVA:
		...
		LOG_INF("kernel total cost time (%d)\n",
			(total_time_end.tv_nsec - total_time_begin.tv_nsec) /
				1000);
#endif

		if (copy_to_user((void *)a_u4Param, &g_pda_Pdadata,
				 sizeof(struct PDA_Data_t_v2)) != 0) {
...
static int PDA_probe(struct platform_device *pdev)
{
	...
	// must porting in dts
	larbs = of_count_phandle_with_args(pdev->dev.of_node, "mediatek,larbs",
					   NULL);
	LOG_INF("larb_num:%d\n", larbs);
	for (i = 0; i < larbs; i++)
	...
	for (i = 0; i < PDA_CLK_NUM; ++i) {
		...
		pda_clk[i].CG_PDA_TOP_MUX =
			devm_clk_get(&pdev->dev, clk_names[i]);
	...
	// camsys node
	camsys_node =
		of_find_compatible_node(NULL, NULL, "mediatek,isp_unit_test");
	if (!camsys_node) {
	...
	// mraw node
	mraw_node = of_find_compatible_node(NULL, NULL,
					    "mediatek,mt6983-camsys_mraw");
	if (!mraw_node) {
	...
	if (PDA_devs[0].irq > 0) {
		if (PDA_devs[0].irq > 0 && g_PDA_quantity > 0) {
			...
			nRet = of_property_read_u32_array(node, "interrupts",
							  irq_info,
							  ARRAY_SIZE(irq_info));
			if (nRet) {
...
static struct platform_driver
	PDA2Driver = { .probe = PDA2_probe,
		       .remove = PDA2_remove,
		       .driver = {
			       .name = "camera-pda2",
			       .owner = THIS_MODULE,
			       .of_match_table =
				       of_match_ptr(gpda2_of_device_id),
		       } };
...
static int __init camera_pda_init(void)
{
	...
	mtk_iommu_register_fault_callback(
		M4U_PORT_L25_CAM_PDAI_A0,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);
	mtk_iommu_register_fault_callback(
		M4U_PORT_L25_CAM_PDAI_A1,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);
	mtk_iommu_register_fault_callback(
		M4U_PORT_L25_CAM_PDAO_A,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);

	// PDA 1
	mtk_iommu_register_fault_callback(
		M4U_PORT_L26_CAM_PDAI_B0,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);
	mtk_iommu_register_fault_callback(
		M4U_PORT_L26_CAM_PDAI_B1,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);
	mtk_iommu_register_fault_callback(
		M4U_PORT_L26_CAM_PDAO_B,
		(mtk_iommu_fault_callback_t)pda_tfault_callback, NULL, false);
#endif
