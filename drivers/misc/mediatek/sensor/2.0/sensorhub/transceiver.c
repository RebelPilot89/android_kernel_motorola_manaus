				    ...
				    void *private_data)
{
	...
	if (end_time - start_time > timeout_ns && __ratelimit(&ratelimit))
		printk_deferred(
			"time monitor:%llu, %llu, %llu, %llu, %llu, %llu\n",
...
static void transceiver_copy_config(struct transceiver_config *dst,
				    ...
				    uint8_t bias_len, uint8_t cali_len,
				    uint8_t temp_len)
{
	...
	if (bias_len > sizeof(src->word) || cali_len > sizeof(src->word) ||
	    temp_len > sizeof(src->word)) {
		pr_err_ratelimited("can't copy config %u %u %u %u %u\n",
				   src->sensor_type, src->action, bias_len,
				   cali_len, temp_len);
		return;
	}
				   ...
				   uint32_t write_position)
{
	...
	while (1) {
		...
		for (i = 0; i < (ret / item_size); i++) {
			if (transceiver_translate_super(dev, &evt, &buffer[i]) <
			    0)
				continue;
			transceiver_report(dev, &evt);
		}
...
static int transceiver_comm_with(int sensor_type, int cmd, void *data,
				 uint8_t length)
{
	int ret = 0;
...
static int transceiver_enable(struct hf_device *hf_dev, int sensor_type, int en)
{
	int ret = 0;
	...
	if (en) {
		...
		ret = transceiver_comm_with(
			sensor_type, SENS_COMM_CTRL_DISABLE_CMD, NULL, 0);
...
static int transceiver_batch(struct hf_device *hf_dev, int sensor_type,
			     int64_t delay, int64_t latency)
{
	...
	ret = transceiver_comm_with(sensor_type, SENS_COMM_CTRL_ENABLE_CMD,
				    &batch, sizeof(batch));
	if (ret >= 0)
		state->batch = batch;
	mutex_unlock(&dev->enable_lock);
...
static int transceiver_flush(struct hf_device *hf_dev, int sensor_type)
{
	int ret = 0;
	struct transceiver_device *dev = hf_dev->private_data;
	struct transceiver_state *state = NULL;
	...
	state->flush++;
	mutex_unlock(&dev->flush_lock);
	ret = transceiver_comm_with(sensor_type, SENS_COMM_CTRL_FLUSH_CMD, NULL,
				    0);
	if (ret < 0) {
		mutex_lock(&dev->flush_lock);
		if (state->flush > 0)
...
static int transceiver_calibration(struct hf_device *hf_dev, int sensor_type)
{
	return transceiver_comm_with(sensor_type, SENS_COMM_CTRL_CALI_CMD, NULL,
				     0);
}

static int transceiver_config(struct hf_device *hf_dev, int sensor_type,
			      void *data, uint8_t length)
{
	...
	if (((SENSOR_TYPE_LIGHT == sensor_type ||
	      SENSOR_TYPE_LIGHT_1 == sensor_type) &&
#else
	if (((SENSOR_TYPE_LIGHT == sensor_type) &&
#endif
	     (14 != ((uint32_t *)data)[0])) ||
	...
	return transceiver_comm_with(sensor_type, SENS_COMM_CTRL_CONFIG_CMD,
				     data, length);
}

static int transceiver_selftest(struct hf_device *hf_dev, int sensor_type)
{
	return transceiver_comm_with(sensor_type, SENS_COMM_CTRL_SELF_TEST_CMD,
				     NULL, 0);
}

static int transceiver_rawdata(struct hf_device *hf_dev, int sensor_type,
			       int en)
{
	int ret = 0;
	...
	if (en)
		ret = transceiver_comm_with(
			sensor_type, SENS_COMM_CTRL_ENABLE_RAW_CMD, NULL, 0);
	else
		ret = transceiver_comm_with(
			sensor_type, SENS_COMM_CTRL_DISABLE_RAW_CMD, NULL, 0);
	return ret;
}
...
static void transceiver_restore_sensor(struct transceiver_device *dev)
{
	...
	for (index = 0; index < dev->support_size; index++) {
		...
		while (flush-- > 0) {
			ret = transceiver_comm_with(
				sensor_type, SENS_COMM_CTRL_FLUSH_CMD, NULL, 0);
			if (ret < 0)
					...
					unsigned long pm_event, void *unused)
{
	...
	case PM_POST_SUSPEND:
		transceiver_comm_with(SENSOR_TYPE_INVALID,
				      SENS_COMM_CTRL_UNMASK_NOTIFY_CMD, NULL,
				      0);
		timesync_resume();
		return NOTIFY_DONE;
...
static int transceiver_shm_cfg(struct share_mem_config *cfg, void *private_data)
{
	unsigned long flags = 0;
	struct transceiver_device *dev = private_data;
