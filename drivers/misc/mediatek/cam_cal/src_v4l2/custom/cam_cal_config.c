		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if(pCamCalData->sensorID == MOT_AITO_S5KGN8_SENSOR_ID) {
		//af posture calibration data
		unsigned char AF_POSTURE[20];
		unsigned int af_posture_data_offset = 0x19DE;
		 ...
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
   struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	if(Module_info.flag != 0x40) {
		must_log("[MODULE_INF flag=%d] flag is Invalid\n",Module_info.flag);
		return CamCalReturnErr[pCamCalData->Command];
	}
		 ...
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (!CalLscData) {
		must_log("%s line=%d kmalloc return NULL failed\n", __func__, __LINE__);
		return CamCalReturnErr[pCamCalData->Command];
	}
	...
	debug_log("[LSC_INFO]u4Offset=0x%x lsc table_size=%d", start_addr + 1, CUSTOM_LSC_DATA_SIZE);
		 ...
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (!CalPdafData) {
		must_log("%s  line=%dkmalloc return NULL failed\n", __func__, __LINE__);
		return CamCalReturnErr[pCamCalData->Command];
	}
					  ...
					  unsigned int sensorID)
{
	...
	if (cam_cal_config->sensor_id == sensorID) {
		must_log("%s sensor_id matched\n",cam_cal_config->name);
		result = CAM_CAL_ERR_NO_ERR;
	} else {
		must_log("%s sensor_id not matched\n",cam_cal_config->name);
		return result;
	}
