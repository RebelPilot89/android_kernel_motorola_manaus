...
int (*fpsgo2msync_hint_frameinfo_fp)(unsigned int render_tid,
				     unsigned int reader_bufID,
				     unsigned int target_fps,
				     unsigned long q2q_time,
				     unsigned long q2q_time2);
...
#define mtk_fstb_dprintk_always(fmt, args...) pr_debug("[FSTB]" fmt, ##args)
...
static DECLARE_WORK(fps_base_stats_work, (void *)fstb_base_fps_stats);
...
static void enable_fstb_base_timer(void)
{
	...
	ktime = ktime_set(0, ADJUST_INTERVAL_US * 100);
...
{
	fpsgo_systrace_c_fstb(-200, 0, fstb_active, "fstb_active");
	fpsgo_systrace_c_fstb(-200, 0, fstb_active_dbncd, "fstb_active_dbncd");
...
static struct FSTB_FRAME_INFO *add_new_frame_info(int pid,
...
int switch_thread_max_fps(int pid, int set_max)
{
	...
	hlist_for_each_entry (iter, &fstb_frame_infos, hlist) {
		...
		fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, set_max,
					  "sbe_set_max_fps");
...
int switch_ui_ctrl(int pid, int set_ctrl)
{
	...
	hlist_for_each_entry (iter, &fstb_frame_infos, hlist) {
		...
		fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, set_ctrl,
					  "sbe_set_ctrl");
		fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid,
					  iter->sbe_state, "sbe_state");
	}
	...
	if (!found) {
		...
		new_frame_info = add_new_frame_info(
			pid, 0, RENDER_INFO_HWUI_TYPE, RENDER_INFO_VIDEO_NONE);
		if (new_frame_info == NULL)
			goto out;
...
static int switch_redner_fps_range(char *proc_name, pid_t pid, int nr_level,
				   struct fps_level *level)
{
	...
	hlist_for_each_entry_safe (rtfiter, n, &fstb_render_target_fps, hlist) {
		if ((mode == 0 &&
		     !strncmp(proc_name, rtfiter->process_name, 16)) ||
	...
	if (rtfiter == NULL && nr_level) {
		...
		if (mode == 0) {
			...
			if (!strncpy(new_render_target_fps->process_name,
				     proc_name, 16)) {
		...
		memcpy(new_render_target_fps->level, level,
		       nr_level * sizeof(struct fps_level));
...
int switch_process_fps_range(char *proc_name, int nr_level,
			     struct fps_level *level)
{
...
void fpsgo_ctrl2fstb_dfrc_fps(int fps)
{
	...
	if (fps <= CFG_MAX_FPS_LIMIT && fps >= CFG_MIN_FPS_LIMIT &&
		...
		min_fps_limit =
			min(dfps_ceiling, fps_levels[nr_fps_levels - 1].end);
		     ...
		     unsigned int cur_max_freq, u64 ulID)
{
	...
	if (iter->weighted_gpu_time_begin < iter->weighted_gpu_time_end &&
		...
		memmove(iter->weighted_gpu_time,
			...
			sizeof(long long) * (iter->weighted_gpu_time_end -
					     iter->weighted_gpu_time_begin));
		memmove(iter->weighted_gpu_time_ts,
			&(iter->weighted_gpu_time_ts
				  [iter->weighted_gpu_time_begin]),
			sizeof(long long) * (iter->weighted_gpu_time_end -
					     iter->weighted_gpu_time_begin));

		/*reset index*/
		iter->weighted_gpu_time_end = iter->weighted_gpu_time_end -
					      iter->weighted_gpu_time_begin;
		iter->weighted_gpu_time_begin = 0;
	}

	if (cur_max_freq > 0 && cur_max_freq >= cur_freq && t_gpu > 0LL &&
	    t_gpu < 1000000000LL) {
	...
	mtk_fstb_dprintk(
		...
		cur_time_us, ktime_to_us(ktime_get()) - cur_time_us, t_gpu,
		cur_freq, cur_max_freq);

	fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, (int)t_gpu, "t_gpu");
	fpsgo_systrace_c_fstb(iter->pid, iter->bufid, (int)cur_freq,
			      "cur_gpu_cap");
	fpsgo_systrace_c_fstb(iter->pid, iter->bufid, (int)cur_max_freq,
			      "max_gpu_cap");
...
static int get_gpu_frame_time(struct FSTB_FRAME_INFO *iter)
{
	...
	if (iter->weighted_gpu_time_end - iter->weighted_gpu_time_begin) {
		if (iter->sorted_weighted_gpu_time
			    [QUANTILE *
			     (iter->weighted_gpu_time_end -
			      iter->weighted_gpu_time_begin) /
			     100] > INT_MAX)
			...
			ret = iter->sorted_weighted_gpu_time
				      [QUANTILE *
				       (iter->weighted_gpu_time_end -
					iter->weighted_gpu_time_begin) /
				       100];
...
void eara2fstb_get_tfps(int max_cnt, int *is_camera, int *pid,
			unsigned long long *buf_id, int *tfps, int *rfps,
			int *hwui, char name[][16])
{
	int count = 0;
	struct FSTB_FRAME_INFO *iter;
	struct hlist_node *n;
			  ...
			  int tfps)
{
	...
	hlist_for_each_entry_safe (iter, n, &fstb_frame_infos, hlist) {
		if (pid == iter->pid && buf_id == iter->bufid) {
			...
			fpsgo_systrace_c_fstb_man(pid, buf_id, diff,
						  "eara_diff");

			if (iter->target_fps_notifying &&
			    tfps == iter->target_fps_notifying) {
				iter->target_fps_v2 =
					iter->target_fps_notifying;
			     ...
			     int notify_eara)
{
	...
	if (notify_eara && eara_pre_change_single_fp)
		ret = eara_pre_change_single_fp(iter->pid, iter->bufid,
						target_fps);
	...
	if ((notify_eara && (ret == -1)) ||
		...
		iter->target_fps_notifying = 0;
		fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, 0,
					  "fstb_notifying");
		fpsgo_systrace_c_fstb(iter->pid, iter->bufid,
...
int fpsgo_fbt2fstb_update_cpu_frame_info(
	...
	unsigned long long Q2Q_time, long long Runnging_time,
	unsigned int Curr_cap, unsigned int Max_cap, unsigned long long mid,
	unsigned long long enqueue_length, unsigned long long dequeue_length)
{
	...
	if (iter->weighted_cpu_time_begin < iter->weighted_cpu_time_end &&
		...
		memmove(iter->weighted_cpu_time,
			...
			sizeof(long long) * (iter->weighted_cpu_time_end -
					     iter->weighted_cpu_time_begin));
		memmove(iter->weighted_cpu_time_ts,
			&(iter->weighted_cpu_time_ts
				  [iter->weighted_cpu_time_begin]),
			sizeof(long long) * (iter->weighted_cpu_time_end -
					     iter->weighted_cpu_time_begin));

		/*reset index*/
		iter->weighted_cpu_time_end = iter->weighted_cpu_time_end -
					      iter->weighted_cpu_time_begin;
		iter->weighted_cpu_time_begin = 0;
	}

	if (max_cpu_cap > 0 && Max_cap > Curr_cap && cpu_time_ns > 0LL &&
	    cpu_time_ns < 1000000000LL) {
	...
	iter->weighted_cpu_time[iter->weighted_cpu_time_end] = wct;
...
out:
	...
	if (fstb_is_cam_active || iter->hwui_flag == RENDER_INFO_HWUI_TYPE ||
		...
		if (iter->target_fps && iter->target_fps != -1 &&
		    iter->target_fps_diff && !iter->target_fps_margin &&
		    !iter->target_fps_margin_gpu) {
			eara_fps =
				iter->target_fps * 1000 + iter->target_fps_diff;
			eara_fps /= 1000;
			eara_fps =
				clamp(eara_fps, min_fps_limit, max_fps_limit);
		}
		tolerence_fps = iter->target_fps_margin_gpu;
	} else {
		eara_fps = iter->target_fps_v2;
		if (iter->target_fps_v2 && iter->target_fps_v2 != -1 &&
		    iter->target_fps_diff && !iter->target_fps_margin_v2) {
	...
	if (fpsgo2msync_hint_frameinfo_fp)
		fpsgo2msync_hint_frameinfo_fp(
			(unsigned int)iter->pid, iter->bufid, iter->target_fps,
			Q2Q_time, Q2Q_time - enqueue_length - dequeue_length);
				      ...
				      unsigned long long cur_queue_end_ts)
{
	...
	target_fps =
		fpsgo_fstb2xgf_get_target_fps(pid, bufID, &margin,
					      cur_dequeue_start_ts,
	...
	if (!fstb_self_ctrl_fps_enable) {
		...
		fpsgo_main_trace(
			"[fstb][%d][0x%llx] | fstb_self_ctrl_fps_enable:%d",
	...
	if (target_fps <= 0) {
		...
		hlist_for_each_entry (rtfiter, &fstb_render_target_fps, hlist) {
			if (!strncmp(iter->proc_name, rtfiter->process_name,
				     16) ||
			    rtfiter->pid == iter->pid) {
				for (i = rtfiter->nr_level - 1; i >= 0; i--) {
					if (rtfiter->level[i].start >=
					    target_fps) {
						target_fps_new =
							target_fps >= rtfiter->level[i]
										.end ?
								target_fps :
								rtfiter->level[i]
									.end;
				...
				if (i < 0)
					target_fps_new =
						rtfiter->level[0].start;

				if (target_fps_new == rtfiter->level[0].start &&
				    margin)
					margin = 0;
			}
		}
	...
	fpsgo_main_trace(
		"[fstb][%d][0x%llx] | target_fps:%d(%d)(%d)(%d) margin:%d",
		iter->pid, iter->bufid, iter->target_fps_v2, target_fps_old,
		target_fps_new, target_fps, iter->target_fps_margin_v2);
	fpsgo_systrace_c_fstb(iter->pid, iter->bufid, iter->target_fps_v2,
			      "target_fps_v2");
	fpsgo_systrace_c_fstb(iter->pid, iter->bufid,
...
static void fstb_notifier_wq_cb(struct work_struct *psWork)
{
	struct FSTB_NOTIFIER_PUSH_TAG *vpPush =
		FSTB_CONTAINER_OF(psWork, struct FSTB_NOTIFIER_PUSH_TAG, sWork);

	if (!vpPush)
		return;

	fstb_calculate_target_fps(vpPush->pid, vpPush->bufid,
				  vpPush->cur_dequeue_start_ts,
				  vpPush->cur_queue_end_ts);

	kfree(vpPush);
}

void fpsgo_comp2fstb_prepare_calculate_target_fps(
	...
	unsigned long long cur_queue_end_ts)
{
	...
	vpPush = (struct FSTB_NOTIFIER_PUSH_TAG *)kmalloc(
		sizeof(struct FSTB_NOTIFIER_PUSH_TAG), GFP_ATOMIC);
...
static long long get_cpu_frame_time(struct FSTB_FRAME_INFO *iter)
{
	...
	if (iter->weighted_cpu_time_end - iter->weighted_cpu_time_begin) {
		if (iter->sorted_weighted_cpu_time
			    [QUANTILE *
			     (iter->weighted_cpu_time_end -
			      iter->weighted_cpu_time_begin) /
			     100] > INT_MAX)
			...
			ret = iter->sorted_weighted_cpu_time
				      [QUANTILE *
				       (iter->weighted_cpu_time_end -
					iter->weighted_cpu_time_begin) /
				       100];
...
static void fstb_set_cam_active(int active)
{
	...
	if (set_cam_active_fpsgo_off) {
		if (fstb_is_cam_active) {
			...
			mtk_fstb_dprintk_always("[FSTB] %s: FPSGO_OFF",
						__func__);
			fpsgo_systrace_c_fstb(-100, 0, 1, "cam_fpsgo_off");
		} else {
			fpsgo_switch_enable(1);
			mtk_fstb_dprintk_always("[FSTB] %s: FPSGO_ON",
						__func__);
...
static int fstb_get_queue_fps2(struct FSTB_FRAME_INFO *iter)
{
	...
	tmp_jump_check_num =
		min(JUMP_CHECK_NUM, iter->target_fps * JUMP_CHECK_Q_PCT / 100);
	tmp_jump_check_num = tmp_jump_check_num <= 1 ? 2 : tmp_jump_check_num;
				       ...
				       int api, int hwui_flag, int video_flag)
{
	...
	if (iter == NULL) {
		...
		new_frame_info =
			add_new_frame_info(pid, bufID, hwui_flag, video_flag);
		if (new_frame_info == NULL)
			goto out;
	...
	if (iter->queue_time_begin < 0 || iter->queue_time_end < 0 ||
	...
	while (iter->queue_time_begin < iter->queue_time_end) {
		if (iter->queue_time_ts[iter->queue_time_begin] <
		    ts - (long long)FRAME_TIME_WINDOW_SIZE_US * 1000)
	...
	if (iter->queue_time_begin < iter->queue_time_end &&
		...
		memmove(iter->queue_time_ts,
			...
			sizeof(unsigned long long) * (iter->queue_time_end -
						      iter->queue_time_begin));
		/*reset index*/
		iter->queue_time_end =
	...
	if (iter->queue_time_end - iter->queue_time_begin >=
	    tmp_jump_check_num) {
		...
		fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, tmp_q_fps,
					  "tmp_q_fps");
		fpsgo_systrace_c_fstb(iter->pid, iter->bufid,
				      tmp_jump_check_num, "tmp_jump_check_num");

		if (tmp_q_fps >= iter->target_fps + iter->target_fps_margin2) {
			tmp_target_fps = calculate_fps_limit(iter, tmp_q_fps);
		}
		...
		if (iter->vote_i < JUMP_VOTE_MAX_I) {
			...
			memmove(iter->vote_fps,
				&(iter->vote_fps[JUMP_VOTE_MAX_I -
						 tmp_jump_check_num + 1]),
		...
		if (iter->vote_i >= tmp_jump_check_num) {
			tmp_vote_fps =
				mode(&(iter->vote_fps[iter->vote_i -
						      tmp_jump_check_num]),
				     tmp_jump_check_num);
...
static int fstb_get_queue_fps1(struct FSTB_FRAME_INFO *iter, long long interval,
			       int *is_fps_update)
{
	...
	for (j = i + 1; j < iter->queue_time_end; j++) {
		if ((iter->queue_time_ts[j] - iter->queue_time_ts[j - 1]) <
		    DISPLAY_FPS_FILTER_NS) {
			avg_frame_interval += (iter->queue_time_ts[j] -
	...
	*is_fps_update = frame_count ? 1 : 0;
	...
	if (iter->queue_fps == -1 || frame_count)
		...
		retval = 0;
	fpsgo_systrace_c_fstb_man(iter->pid, iter->bufid, retval, "queue_fps");
...
static int fps_update(struct FSTB_FRAME_INFO *iter)
{
	...
	iter->queue_fps = fstb_get_queue_fps1(iter, FRAME_TIME_WINDOW_SIZE_US,
					      &is_fps_update);
...
static int calculate_fps_limit(struct FSTB_FRAME_INFO *iter, int target_fps)
{
	...
	hlist_for_each_entry (rtfiter, &fstb_render_target_fps, hlist) {
		mtk_fstb_dprintk("%s %s %d %s %d\n", __func__, iter->proc_name,
				 iter->pid, rtfiter->process_name,
				 rtfiter->pid);
	...
	case 2:
		if (ret_fps >= max_fps_limit) {
			iter->target_fps_margin = 0;
			iter->target_fps_margin_dbnc_a = margin_mode_dbnc_a;
			iter->target_fps_margin_dbnc_b = margin_mode_dbnc_b;
		} else if (asfc_turn) {
...
static int cal_target_fps(struct FSTB_FRAME_INFO *iter)
{
	...
	if (iter->fps_raise_flag == 1) {
		...
		fpsgo_systrace_c_fstb(iter->pid, iter->bufid, (int)target_limit,
				      "tmp_target_limit");
	} else {
		target_limit = iter->target_fps;
	}
...
void fpsgo_fbt2fstb_query_fps(int pid, unsigned long long bufID,
			      ...
			      int *fps_margin, int tgid, unsigned long long mid,
	...
	*cooler_on = 0;
	...
	if (!iter) {
		...
		*target_fps = max_fps_limit;
		...
		total_time = div64_u64(total_time,
				       (*target_fps) + tolerence_fps >
						       max_fps_limit ?
					       max_fps_limit :
					       (*target_fps) + tolerence_fps);
		v_c_time = total_time;
		...
		if (fstb_is_cam_active ||
			...
			if (iter->target_fps && iter->target_fps != -1 &&
				...
				eara_fps =
					clamp(eara_fps, min_mlimit, max_mlimit);
				*cooler_on = 1;

				*target_fps = eara_fps / 1000;
				tolerence_fps = iter->target_fps_margin * 1000;
				total_time =
					(unsigned long long)FSTB_MSEC_DIVIDER;
				total_time = div64_u64(
					total_time,
					(eara_fps + tolerence_fps) >
							max_mlimit ?
						max_mlimit :
						(eara_fps + tolerence_fps));
			...
			if (iter->target_fps_v2 && iter->target_fps_v2 != -1 &&
				...
				*target_fps = eara_fps / 1000;
				tolerence_fps =
					iter->target_fps_margin_v2 * 1000;
				total_time =
					(unsigned long long)FSTB_MSEC_DIVIDER;
				total_time = div64_u64(
					total_time,
...
void fstb_cal_powerhal_fps(void)
{
	...
	hlist_for_each_entry (iter, &fstb_frame_infos, hlist) {
		...
		if (tfps_to_powerhal_enable)
			powerfps_arrray[i].fps = iter->target_fps_v2 > 0 ?
							 iter->target_fps_v2 :
							 -1;
		else
			powerfps_arrray[i].fps =
				iter->queue_fps > 0 ? iter->queue_fps : -1;
	...
	sort(powerfps_arrray, i, sizeof(struct FSTB_POWERFPS_LIST),
	     cmp_powerfps, NULL);

	for (j = 0; j < i; j++) {
		if (powerfps_arrray[j].pid != powerfps_arrray[j + 1].pid) {
			mtk_fstb_dprintk_always("%s %d %d %d\n", __func__, j,
						powerfps_arrray[j].pid,
						powerfps_arrray[j].fps);
			fstb_sentcmd(powerfps_arrray[j].pid,
				     powerfps_arrray[j].fps);
		}
	}
}
...
static void fstb_fps_stats(struct work_struct *work)
{
	...
	hlist_for_each_entry_safe (iter, n, &fstb_frame_infos, hlist) {
		...
		if (fps_update(iter)) {
			...
			iter->vote_i = 0;
			fpsgo_systrace_c_fstb_man(iter->pid, 0, dfps_ceiling,
						  "dfrc");
			fpsgo_systrace_c_fstb(iter->pid, iter->bufid,
					      iter->target_fps,
					      "fstb_target_fps1");
			...
			mtk_fstb_dprintk("%s pid:%d target_fps:%d\n", __func__,
					 iter->pid, iter->target_fps);
					...
					struct kobj_attribute *attr, char *buf)
{
	...
	for (n = rb_first(&video_pid_tree); n != NULL; n = rb_next(n)) {
		...
		length = scnprintf(pid_str + pos,
				   FPSGO_SYSFS_MAX_BUFF_SIZE - pos, "%d\t%u\n",
				   video_instance->pid,
				   video_instance->count_instance);
		pos += length;
	}
	rcu_read_unlock();
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			if (kstrtoint(acBuffer, 0, &arg) != 0)
				goto out;
			mtk_fstb_dprintk_always("%s %d\n", __func__, arg);
			fpsgo_systrace_c_fstb_man(arg > 0 ? arg : -arg, 0,
						  arg > 0, "force_ctrl");
			if (arg > 0)
				    ...
				    struct kobj_attribute *attr, char *buf)
{
	...
	length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos, "%d ",
			   nr_fps_levels);
	pos += length;

	for (i = 0; i < nr_fps_levels; i++) {
		length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				   "%d-%d ", fps_levels[i].start,
				   fps_levels[i].end);
		pos += length;
	}
	length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos, "\n");
				     ...
				     const char *buf, size_t count)
{
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			...
			if (!substr ||
			    kstrtoint(substr, 10, &new_nr_fps_levels) != 0 ||
			...
			for (i = 0; i < new_nr_fps_levels; i++) {
				...
				/* maybe contiguous */
				if (strchr(substr, '-')) {
					if (sscanf(substr, "%d-%d", &start_fps,
						   &end_fps) != 2) {
						ret = -EINVAL;
						goto err;
					}
					...
					if (kstrtoint(substr, 10, &start_fps) !=
					    0) {
						ret = -EINVAL;
						goto err;
					}
			...
			ret = !set_soft_fps_level(new_nr_fps_levels,
						  new_levels);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	hlist_for_each_entry (rtfiter, &fstb_render_target_fps, hlist) {
		length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				   "%s %d %d ", rtfiter->process_name,
				   rtfiter->pid, rtfiter->nr_level);
		pos += length;
		for (i = 0; i < rtfiter->nr_level; i++) {
			length = scnprintf(temp + pos,
					   FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
					   "%d-%d ", rtfiter->level[i].start,
					   rtfiter->level[i].end);
			pos += length;
		}
		length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			...
			if (mode == 0) {
				...
				if (switch_thread_fps_range(pid, nr_level,
							    level))
					ret = -EINVAL;
			}
		}
				   ...
				   size_t count)
{
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			if (kstrtoint(acBuffer, 0, &arg) == 0 && arg >= 0 &&
			    arg <= 1)
				adopt_low_fps = arg;
		}
	}
				...
				size_t count)
{
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			if (sscanf(acBuffer, "%d %d", &k_enable, &klog_on) >=
			    1) {
		...
		if (iter) {
			if (fstb_is_cam_active ||
				...
				length = scnprintf(
					...
					iter->pid, iter->bufid, iter->proc_name,
...
int mtk_fstb_init(void)
{
	...
	if (!fpsgo_sysfs_create_dir(NULL, "fstb", &fstb_kobj)) {
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_fpsgo_status);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_fstb_debug);
		...
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_margin_mode);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_margin_mode_gpu_dbnc_b);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_margin_mode_gpu_dbnc_a);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_margin_mode_gpu);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_fstb_tune_window_size);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_fstb_fps_list);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_fstb_soft_level);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_jump_check_num);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_jump_check_q_pct);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_adopt_low_fps);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_set_render_max_fps);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_fstb_self_ctrl_fps_enable);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_set_ui_ctrl);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_fstb_fps_bypass_max);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_fstb_fps_bypass_min);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_set_cam_active);
		fpsgo_sysfs_create_file(fstb_kobj,
					&kobj_attr_tfps_to_powerhal_enable);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_set_video_pid);
		fpsgo_sysfs_create_file(fstb_kobj, &kobj_attr_clear_video_pid);
		fpsgo_sysfs_create_file(fstb_kobj,
	...
	wq1 = alloc_ordered_workqueue("%s", WQ_MEM_RECLAIM | WQ_HIGHPRI,
				      "mt_fstb");
	if (!wq1)
		goto err;
	wq2 = alloc_ordered_workqueue("%s", WQ_MEM_RECLAIM | WQ_HIGHPRI,
				      "mt_fstb_base");
	if (!wq2)
		goto err;
...
int __exit mtk_fstb_exit(void)
{
	...
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fpsgo_status);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_debug);
	fpsgo_sysfs_remove_file(fstb_kobj,
				&kobj_attr_fstb_tune_error_threshold);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_tune_quantile);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode_dbnc_b);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode_dbnc_a);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode_gpu_dbnc_b);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode_gpu_dbnc_a);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_margin_mode_gpu);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_tune_window_size);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_fps_list);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_soft_level);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_jump_check_num);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_jump_check_q_pct);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_adopt_low_fps);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_set_render_max_fps);
	fpsgo_sysfs_remove_file(fstb_kobj,
				&kobj_attr_fstb_self_ctrl_fps_enable);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_set_ui_ctrl);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_fps_bypass_max);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_fps_bypass_min);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_set_cam_active);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_tfps_to_powerhal_enable);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_set_video_pid);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_clear_video_pid);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_fstb_video_pid_list);
	fpsgo_sysfs_remove_file(fstb_kobj, &kobj_attr_set_cam_active_fpsgo_off);
