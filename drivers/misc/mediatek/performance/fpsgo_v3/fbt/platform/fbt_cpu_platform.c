...
static int platform_fpsgo_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "gcc_enable", &retval);
	if (!ret)
		plat_gcc_enable = retval;
	else
		FPSGO_LOGE("%s unable to get plat_gcc_enable\n", __func__);

	ret = of_property_read_u32(node, "cpu_limit", &retval);
	if (!ret)
...
static int generate_cpu_mask(void)
{
	...
	ret = of_property_read_u32_array(node, "fbt_cpu_mask", mask_int,
					 FPSGO_PREFER_TOTAL);
...
static int generate_sbe_rescue_enable(void)
{
	...
	ret = of_property_read_u32(node, "sbe_resceue_enable", &retval);
	if (!ret)
