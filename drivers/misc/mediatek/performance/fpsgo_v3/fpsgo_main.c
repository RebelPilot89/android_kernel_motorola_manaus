...
static void fpsgo_notifier_wq_cb_connect(int pid, int connectedAPI,
					 unsigned long long id)
{
	FPSGO_LOGI("[FPSGO_CB] connect: pid %d, API %d, id %llu\n", pid,
		   connectedAPI, id);
...
static void fpsgo_notifier_wq_cb_bqid(int pid, unsigned long long bufID,
				      int queue_SF, unsigned long long id,
				      int create)
{
	FPSGO_LOGI(
...
static void fpsgo_notifier_wq_cb(void)
{
	...
	if (!list_empty(&head)) {
		vpPush = list_first_entry(&head, struct FPSGO_NOTIFIER_PUSH_TAG,
					  queue_list);
	...
	case FPSGO_NOTIFIER_CONNECT:
		fpsgo_notifier_wq_cb_connect(vpPush->pid, vpPush->connectedAPI,
					     vpPush->identifier);
		break;
	...
	case FPSGO_NOTIFIER_BQID:
		fpsgo_notifier_wq_cb_bqid(vpPush->pid, vpPush->bufID,
					  vpPush->queue_SF, vpPush->identifier,
					  vpPush->create);
		break;
	...
	case FPSGO_NOTIFIER_SBE_RESCUE:
		fpsgo_notifier_wq_cb_sbe_rescue(vpPush->pid, vpPush->enable,
						vpPush->enhance);
		break;
	default:
...
void fpsgo_notify_qudeq(int qudeq, unsigned int startend, int pid,
			unsigned long long id)
{
	unsigned long long cur_ts;
	struct FPSGO_NOTIFIER_PUSH_TAG *vpPush;
	...
	vpPush = (struct FPSGO_NOTIFIER_PUSH_TAG *)fpsgo_alloc_atomic(
		sizeof(struct FPSGO_NOTIFIER_PUSH_TAG));
	if (!vpPush) {
...
void fpsgo_notify_bqid(int pid, unsigned long long bufID, int queue_SF,
...
void fpsgo_switch_enable(int enable)
{
	...
	if (fpsgo_is_force_enable() != FPSGO_FREE &&
	    enable != fpsgo_is_force_enable())
		return;
...
int fpsgo_fstb_process_fps_range(char *proc_name, int nr_level,
				 struct fps_level *level)
{
	return switch_process_fps_range(proc_name, nr_level, level);
}

int fpsgo_fstb_thread_fps_range(pid_t pid, int nr_level,
				struct fps_level *level)
{
...
static void fpsgo_cpu_frequency_tracer(void *ignore, unsigned int frequency,
				       unsigned int cpu_id)
{
	int cpu = 0, cluster = 0;
...
#define FOR_EACH_INTEREST(i)                                                   \
	for (i = 0;                                                            \
	     i < sizeof(fpsgo_tracepoints) / sizeof(struct tracepoints_table); \
	     i++)
...
static void tracepoint_cleanup(void)
{
	...
	FOR_EACH_INTEREST(i)
	{
		if (fpsgo_tracepoints[i].registered) {
			tracepoint_probe_unregister(fpsgo_tracepoints[i].tp,
						    fpsgo_tracepoints[i].func,
						    NULL);
...
static int __init fpsgo_init(void)
{
	...
	ret = tracepoint_probe_register(fpsgo_tracepoints[0].tp,
					fpsgo_tracepoints[0].func, NULL);
	if (ret) {
