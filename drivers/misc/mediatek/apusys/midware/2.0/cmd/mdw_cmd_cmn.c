...
void mdw_cmd_cmdbuf_out(struct mdw_fpriv *mpriv, struct mdw_cmd *c)
{
	...
	if (mdw_mem_invalidate(mpriv, c->cmdbufs))
		mdw_drv_warn(
			"s(0x%llx)c(0x%llx) invalidate cmdbufs(%llu) fail\n",
	...
	for (i = 0; i < c->num_subcmds; i++) {
		...
		for (j = 0; j < ksubcmd->info->num_cmdbufs; j++) {
			...
			if (ksubcmd->cmdbufs[j].direction != MDW_CB_IN) {
				mdw_trace_begin(
					"apumdw:cbs_copy_out|cb:%u-%u size:%llu type:%u",
					i, j, ksubcmd->ori_cbs[j]->size,
					ksubcmd->info->type);
	...
	for (i = 0; i < c->num_subcmds; i++) {
		...
		for (j = 0; j < ksubcmd->info->num_cmdbufs; j++) {
			...
			if (m->vaddr == NULL ||
				...
				mdw_drv_err(
					"sc(0x%llx-%u) cb#%u invalid range(%p/%u/%llu)\n",
					c->kid, i, j, m->vaddr,
					ksubcmd->cmdbufs[j].size, m->size);
				ret = -EINVAL;
				goto free_cmdbufs;
			...
			if (ksubcmd->cmdbufs[j].direction != MDW_CB_OUT) {
				mdw_trace_begin(
					"apumdw:cbs_copy_in|cb:%u-%u size:%u type:%u",
					i, j, ksubcmd->cmdbufs[j].size,
					ksubcmd->info->type);
		...
		if (ret) {
			mdw_drv_err(
				"sc(0x%llx-%u) dev(%u) validate cb(%u) fail(%d)\n",
				c->kid, i, ksubcmd->info->type,
				ksubcmd->info->num_cmdbufs, ret);
			goto free_cmdbufs;
		}
	}
...
int mdw_fence_init(struct mdw_cmd *c, int fd)
{
	...
	fence_ctx = mdw_fence_ctx_alloc(c->mpriv->mdev);
	dma_fence_init(&c->fence->base_fence, &mdw_fence_ops, &c->fence->lock,
		       fence_ctx, 0);

	mdw_flw_debug("fence init, c(0x%llx) ctx(%llu)\n", (uint64_t)c,
		      fence_ctx);
...
static void mdw_cmd_release(struct mdw_cmd *c)
{
	...
	if (!list_empty(&c->u_item))
		list_del_init(&c->u_item);
...
int mdw_cmd_sanity_check(struct mdw_cmd *c)
{
	...
	if (c->exec_infos->size !=
		...
		mdw_drv_err(
			"s(0x%llx)cmd invalid(0x%llx/0x%llx) einfo(%llu/%lu)\n",
...
void mdw_cmd_check_rets(struct mdw_cmd *c, int ret)
{
	...
	if (is_dma) {
		dma_exception(
			"dma exec fail:ret(%d/0x%llx)pid(%d/%d)c(0x%llx)\n",
			ret, c->einfos->c.sc_rets, c->pid, c->tgid, c->kid);
	}
}
...
int mdw_cmd_ioctl_del(struct mdw_fpriv *mpriv, union mdw_cmd_args *args)
{
	...
	mdw_trace_begin("%s", "apumdw:user_delete");

	mutex_lock(&mpriv->mtx);
	list_for_each_entry_safe (c, tmp, &mpriv->cmds, u_item) {
		list_del_init(&c->u_item);
		mdw_cmd_delete(c);
		ret = 0;
		break;
	...
	if (ret)
		mdw_drv_warn("can't find command to delete\n");
	mutex_unlock(&mpriv->mtx);
