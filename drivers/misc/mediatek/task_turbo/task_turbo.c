...
#define SCHED_FEAT(name, enabled) [__SCHED_FEAT_##name] = { 0 },
...
static uint32_t latency_turbo =
	SUB_FEAT_LOCK | SUB_FEAT_BINDER | SUB_FEAT_SCHED;
...
static int task_turbo_idle_cpu(int cpu);
...
static void probe_android_rvh_prepare_prio_fork(void *ignore,
...
static void probe_android_rvh_finish_prio_fork(void *ignore,
					       struct task_struct *p)
{
...
static void
probe_android_rvh_rtmutex_prepare_setprio(void *ignore, struct task_struct *p,
	...
	if (unlikely(is_turbo_task(p))) {
		if (!dl_prio(p->prio) && !rt_prio(p->prio)) {
			...
			if (backup >= MIN_NICE && backup <= MAX_NICE) {
				...
				trace_turbo_rtmutex_prepare_setprio(turbo_data,
								    p);
...
static void probe_android_vh_rwsem_write_finished(void *ignore,
...
static void probe_android_vh_alter_rwsem_list_add(void *ignore,
...
static void
probe_android_vh_binder_transaction_init(void *ignore,
					 struct binder_transaction *t)
{
	t->android_vendor_data1 = 0;
}

static void probe_android_vh_binder_set_priority(void *ignore,
						 struct binder_transaction *t,
						 struct task_struct *task)
{
	if (binder_start_turbo_inherit(t->from ? t->from->task : NULL, task)) {
		t->android_vendor_data1 = (u64)task;
	}
}

static void
probe_android_vh_binder_restore_priority(void *ignore,
...
static void probe_android_vh_alter_futex_plist_add(void *ignore,
						   struct plist_node *q_list,
						   struct plist_head *hb_chain,
						   bool *already_on_hb)
{
	...
	if (!sub_feat_enable(SUB_FEAT_LOCK) && !is_turbo_task(current)) {
		*already_on_hb = false;
		return;
	}
...
static void probe_android_rvh_select_task_rq_fair(void *ignore,
						  struct task_struct *p,
...
int find_best_turbo_cpu(struct task_struct *p)
{
	...
	for (i = 0; i < domain_cnt; i++) {
		...
		for_each_cpu (iter_cpu, &domain->possible_cpus) {
			 ...
			 * to improve latency
			 */
			if (task_turbo_idle_cpu(iter_cpu)) {
				new_cpu = iter_cpu;
				goto out;
			}

			spare_cap = max_t(long,
					  capacity_of(iter_cpu) -
						  cpu_util_without(iter_cpu, p),
					  0);
			/*
...
static void rwsem_start_turbo_inherit(struct rw_semaphore *sem)
{
	...
	if (should_inherit) {
		...
		if (owner && !is_rwsem_reader_owned(sem) &&
		    !is_turbo_task(owner) && !inherited_owner) {
			start_turbo_inherit(owner, RWSEM_INHERIT,
				       ...
				       struct task_struct *to)
{
	...
	if (!is_turbo_task(from) || !test_turbo_cnt(from)) {
		from_turbo_data = get_task_turbo_t(from);
		trace_turbo_inherit_failed(
			from_turbo_data->turbo,
...
static inline bool cgroup_check_set_turbo(struct task_struct *p)
{
	...
	return (turbo_data->render ||
		(p == p->group_leader && p->real_parent->pid != 1));
}
...
static void probe_android_vh_cgroup_set_task(void *ignore, int ret,
					     struct task_struct *p)
{
	struct task_turbo_t *turbo_data;
...
static void probe_android_vh_syscall_prctl_finished(void *ignore, int option,
						    struct task_struct *p)
{
...
void init_hmp_domains(void)
{
	...
	for (id = 0; id < maxid; id++) {
		...
		domain = (struct hmp_domain *)kmalloc(sizeof(struct hmp_domain),
						      GFP_KERNEL);
		if (domain) {
...
static int __init init_task_turbo(void)
{
	...
	pr_info("task_turbo disabled: CONFIG_ANDROID_VENDOR_HOOKS is not set\n");
	return 0;
#else
	...
	ret = register_trace_android_vh_rwsem_init(probe_android_vh_rwsem_init,
						   NULL);
	if (ret) {
		ret_erri_line = __LINE__;
		goto failed;
