			...
			const char *aed_str, int db_opt)
{
	...
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(
		mdee->md_id, SMEM_USER_RAW_MDSS_DBG);
	...
	if (scnprintf(buff, AED_STR_LEN, "md%d:%s%s", md_id + 1, aed_str,
		      img_inf) < 0)
		buff[0] = 0;
...
err_exit1:
	if (dump_flag & CCCI_AED_DUMP_CCIF_REG) {
		...
		ccci_md_dump_info(
			mdee->md_id, DUMP_FLAG_CCIF | DUMP_FLAG_CCIF_REG,
	...
	if (buff == NULL) {
		...
		if (md_dbg_dump_flag & (1U << MD_DBG_DUMP_SMEM))
			aed_md_exception_api(ex_log_addr, ex_log_len,
					     md_img_addr, md_img_len, buf_fail,
					     db_opt);
		else
					  ...
					  char *ex_info)
{
	...
	case MD_EX_CC_C2K_EXCEPTION:
		...
		if (debug_info->fatal_error.err_code1 == 0x3104) {
			...
			scnprintf(ex_info_temp, EE_BUF_LEN_UMOLY, "%s",
				  ex_info);
			...
			scnprintf(ex_info, EE_BUF_LEN_UMOLY,
				  "%s%s, MD base = 0x%08X\n\n", ex_info_temp,
				  mdee->ex_mpu_string,
				  (unsigned int)mem_layout->md_bank0
					  .base_ap_view_phy);
	...
	case MD_EX_DUMP_EMI_CHECK:
		...
		scnprintf(ex_info, EE_BUF_LEN_UMOLY,
			  ...
			  debug_info->data.data1, debug_info->data.channel,
			  debug_info->data.reserved);
		break;
...
static void mdee_info_dump_v2(struct ccci_fsm_ee *mdee)
{
	...
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(
		mdee->md_id, SMEM_USER_RAW_MDCCCI_DBG);
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(
		mdee->md_id, SMEM_USER_RAW_MDSS_DBG);
	struct rtc_time tm;
	...
	CCCI_ERROR_LOG(
		...
		tm.tm_min, tm.tm_sec, (unsigned int)tv.tv_nsec,
	...
	if (dumper->ex_core_num > 1) {
		CCCI_NORMAL_LOG(md_id, FSM, "%s+++++++%s", ex_info_temp,
				ex_info);
		scnprintf(ex_info_buf, EE_BUF_LEN_UMOLY, "%smd%d:%s",
			  ex_info_temp, md_id + 1, ex_info);
	...
	CCCI_MEM_LOG_TAG(md_id, FSM, "Dump MD EX log, 0x%x, 0x%x\n",
			 dumper->more_info, (unsigned int)md_state);
			      ...
			      struct ex_PL_log *ex_PLloginfo)
{
	...
	case MD_EX_PL_FATALERR_BUF:
		...
		if (ex_PLloginfo->content.fatalerr.ex_analy.owner[0] != 0xCC) {
			strmncopy(ex_PLloginfo->content.fatalerr.ex_analy.owner,
				  debug_info->fatal_error.offender,
				  sizeof(ex_PLloginfo->content.fatalerr.ex_analy
						 .owner),
	...
	case MD_EX_PL_ASSERT_NATIVE:
		...
		CCCI_DEBUG_LOG(md_id, FSM, "p filename2:(%s)\n",
			       debug_info->assert.file_name);
		debug_info->assert.line_num =
				...
				struct ex_md32_log *ex_md32LogInfo)
{
	...
	scnprintf(core_name_temp, MD_CORE_NAME_DEBUG, "%s",
...
static void mdee_info_prepare_v2(struct ccci_fsm_ee *mdee)
{
	...
	if ((dumper->more_info == MD_EE_CASE_NORMAL) &&
	    (ccci_fsm_get_md_state(mdee->md_id) == BOOT_WAITING_FOR_HS1)) {
		debug_info = &dumper->debug_info[0];
		ex_PLloginfo = ex_pl_info;
		off_core_num++;
		scnprintf(debug_info->core_name, MD_CORE_NAME_DEBUG,
			  "(MCU_PCORE)");
	...
	for (core_id = 0; core_id < MD_CORE_NUM; core_id++) {
		CCCI_DEBUG_LOG(
			...
			(core_id + 1), ex_overview->core_num,
		...
		CCCI_NORMAL_LOG(md_id, FSM, "core_id(0x%x/%d), %s\n", core_id,
				off_core_num, debug_info->core_name);
		...
		case MD_L1CORE:
			ex_PLloginfo =
				(struct ex_PL_log
					 *)((char *)ex_overview +
					    ex_overview->main_reson[core_id]
						    .core_offset);
			ex_pl_info->envinfo.ELM_status =
				ex_PLloginfo->envinfo.ELM_status;
			ee_case = mdee_pl_core_parse(md_id, debug_info,
						     ex_PLloginfo);
			break;
		case MD_CS_ICC:
			/* Fall through */
			...
			ex_csLogInfo =
				(struct ex_cs_log
					 *)((char *)ex_overview +
					    ex_overview->main_reson[core_id]
						    .core_offset);
			ee_case = mdee_cs_core_parse(md_id, debug_info,
						     ex_csLogInfo);
			break;
		case MD_MD32_DFE:
			/* Fall through */
			...
			ex_md32LogInfo =
				(struct ex_md32_log
					 *)((char *)ex_overview +
					    ex_overview->main_reson[core_id]
						    .core_offset);
			ee_case = mdee_md32_core_parse(md_id, debug_info,
						       ex_md32LogInfo);
			break;
		default:
			ee_case = 0;
	...
	if (off_core_num == 0) {
		...
		ex_PLloginfo =
			(struct ex_PL_log *)((char *)ex_overview +
					     ex_overview->main_reson[MD_PCORE]
						     .core_offset);
		ex_pl_info->envinfo.ELM_status =
			ex_PLloginfo->envinfo.ELM_status;
		off_core_num++;
...
static void mdee_dumper_v2_set_ee_pkg(struct ccci_fsm_ee *mdee, char *data,
				      int len)
{
	struct mdee_dumper_v2 *dumper = mdee->dumper_obj;
