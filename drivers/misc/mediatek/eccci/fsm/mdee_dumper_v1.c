...
err_exit:
	...
	if ((md_id == MD_SYS3) || (dumper->more_info == MD_EE_CASE_NORMAL &&
				   md_state == BOOT_WAITING_FOR_HS1)) {
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, &dumper->ex_info,
				   sizeof(struct ex_log_t));
...
static void mdee_dumper_info_prepare_v1(struct ccci_fsm_ee *mdee)
{
	...
	if (*((char *)ex_info + CCCI_EXREC_OFFSET_OFFENDER) != 0xCC) {
		...
		debug_info->fatal_error
			.offender[sizeof(debug_info->fatal_error.offender) - 1] =
			'\0';
	} else {
...
static void mdee_dumper_v1_set_ee_pkg(struct ccci_fsm_ee *mdee, char *data,
				      int len)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
	int cpy_len =
					...
					int more_info)
{
	...
	if (level == MDEE_DUMP_LEVEL_BOOT_FAIL) {
		if (md_state == BOOT_WAITING_FOR_HS1) {
			...
			/* Handshake 1 fail */
			ccci_aed_v1(mdee, CCCI_AED_DUMP_CCIF_REG, ex_info,
				    DB_OPT_DEFAULT);
