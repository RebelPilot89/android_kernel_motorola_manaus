...
#if defined(FEATURE_SCP_CCCI_SUPPORT) && defined(CCCI_KMODULE_ENABLE)
#if __has_include("scp_ipi.h")
#include "scp_ipi.h"
#elif __has_include("../../scp/rv/scp_ipi.h")
...
static int ccci_scp_ipi_send(int md_id, int op_id, void *data)
{
	...
	while (1) {
		ipi_status = mtk_ipi_send(&scp_ipidev, IPI_OUT_APCCCI_0, 0,
					  &scp_ipi_tx_msg,
					  (sizeof(scp_ipi_tx_msg) / 4), 1);
...
static int scp_set_clk_cg(unsigned int on)
{
	...
	if (!(on == 0 || on == 1)) {
		CCCI_ERROR_LOG(MD_SYS1, FSM, "%s:on=%u is invalid\n", __func__,
			       on);
		return -1;
	}
	...
	if (on == 0) {
		if (!ccci_scp_ctl.ccif2_ap_base ||
		    !ccci_scp_ctl.ccif2_md_base) {
			CCCI_ERROR_LOG(MD_SYS1, FSM, "%s can't ack ccif2\n",
				       __func__);
		} else {
			ccci_write32(ccci_scp_ctl.ccif2_ap_base, APCCIF_ACK,
				     0xFFFF);
			ccci_write32(ccci_scp_ctl.ccif2_md_base, APCCIF_ACK,
				     0xFFFF);
			CCCI_NORMAL_LOG(MD_SYS1, FSM,
					"%s, ack ccif2 reg done!\n", __func__);
		}
	}
...
static void ccci_notify_atf_set_scpmem(void)
{
	...
	arm_smccc_smc(MTK_SIP_KERNEL_CCCI_CONTROL, SCP_CLK_SET_DONE, 0, 0, 0, 0,
		      0, 0, &res);
...
static void ccci_scp_md_state_sync_work(struct work_struct *work)
{
	struct ccci_fsm_scp *scp_ctl =
		container_of(work, struct ccci_fsm_scp, scp_md_state_sync_work);
...
static void ccci_scp_ipi_rx_work(struct work_struct *work)
{
	...
	while (!skb_queue_empty(&scp_ipi_rx_skb_list.skb_list)) {
		...
		if (skb == NULL) {
			CCCI_ERROR_LOG(-1, CORE, "ccci_skb_dequeue fail\n");
			return;
		}
...
static int apsync_event(struct notifier_block *this, unsigned long event,
			void *ptr)
{
	switch (event) {
...
static int __maybe_unused ccif_scp_clk_init(struct device *dev)
{
	int idx = 0;

	for (idx = 0; idx < ARRAY_SIZE(scp_clk_table); idx++) {
		scp_clk_table[idx].clk_ref =
			devm_clk_get(dev, scp_clk_table[idx].clk_name);
		if (IS_ERR(scp_clk_table[idx].clk_ref)) {
			CCCI_ERROR_LOG(-1, FSM, "%s:scp get %s failed\n",
...
static int __maybe_unused fsm_scp_hw_init(struct ccci_fsm_scp *scp_ctl,
					  struct device *dev)
{
