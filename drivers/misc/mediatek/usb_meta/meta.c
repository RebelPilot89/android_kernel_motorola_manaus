			  ...
			  struct usb_configuration *config)
{
	while (!list_empty(&config->functions)) {
		...
		f = list_first_entry(&config->functions, struct usb_function,
				     list);
...
static void mtk_usb_remove_config(struct usb_composite_dev *cdev,
				  struct usb_configuration *config)
{
	unsigned long flags;
				      ...
				      struct usb_composite_dev *cdev)
{
	...
	ret = fsg_common_set_cdev(fsg_opts->common, cdev, m_config.can_stall);
	if (ret) {
...
static struct android_usb_function *supported_functions[] = {
	&acm_function, &mass_storage_function, NULL
};
				  ...
				  struct usb_composite_dev *cdev)
{
	...
	for (; (f = *functions++); index++) {
		...
		pr_notice("[USB]%s: f->dev_name = %s, f->name = %s\n", __func__,
			  f->dev_name, f->name);
		f->dev = device_create(android_class, dev->dev, MKDEV(0, index),
				       f, f->dev_name);
		if (IS_ERR(f->dev)) {
...
static int android_bind_enabled_functions(struct android_dev *dev,
					  struct usb_configuration *c)
{
	struct android_usb_function *f;
	int ret;
			       ...
			       size_t size)
{
	...
	while (b) {
		...
		if (err)
			pr_info("android_usb: Cannot enable '%s' (%d)", name,
				err);
	}
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		dev->enabled = true;
		pr_notice(
			"[USB]%s: enable 0->1 case, idVendor=0x%x, idProduct=0x%x\n",
		...
		pr_notice(
			"[USB]%s: enable 1->0 case, idVendor=0x%x, idProduct=0x%x\n",
			__func__, device_desc.idVendor, device_desc.idProduct);
		android_disable(dev);
		list_for_each_entry (f, &dev->enabled_functions, enabled_list) {
...
static ssize_t log_store(struct device *pdev, struct device_attribute *attr,
			 const char *buff, size_t size)
{
	struct android_dev *dev = dev_get_drvdata(pdev);
	char buf[256], n;
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	strncpy(manufacturer_string, "Android",
...
static int android_create_device(struct android_dev *dev)
{
	...
	dev->dev = device_create(android_class, NULL, MKDEV(0, 0), NULL,
				 "android0");
	if (IS_ERR(dev->dev))
...
static int usb_meta_probe(struct platform_device *pdev)
{
	...
	android_usb_driver.gadget_driver.function =
		(char *)android_usb_driver.name;
	android_usb_driver.gadget_driver.driver.name = android_usb_driver.name;
	android_usb_driver.gadget_driver.max_speed =
		android_usb_driver.max_speed;
	...
	pr_info("%s: udc_name='%s'", __func__,
		android_usb_driver.gadget_driver.udc_name);
