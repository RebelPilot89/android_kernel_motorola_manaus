...
static struct usb_function *musb_ep_to_func(struct musb_ep *mep)
{
	...
	addr = ((ep->address & 0x80) >> 3) | (ep->address & 0x0f);
...
#define is_buffer_mapped(req)                                                  \
	(is_dma_capable() && (req->map_state != UN_MAPPED))
				  ...
				  struct musb *musb, struct musb_ep *musb_ep)
{
	...
	if (dma->is_compatible)
		compatible =
			dma->is_compatible(musb_ep->dma, musb_ep->packet_sz,
					   request->request.buf,
	...
	if (request->request.dma == DMA_ADDR_INVALID) {
		...
		dma_addr = dma_map_single(
			musb->controller, request->request.buf, length,
		...
		dma_sync_single_for_device(
			musb->controller, request->request.dma, length,
			request->tx ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
				    ...
				    struct musb *musb)
{
	...
	if (request->map_state == MUSB_MAPPED) {
		dma_unmap_single(musb->controller, request->request.dma, length,
				 request->tx ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
 ...
 */
void musb_g_giveback(struct musb_ep *ep, struct usb_request *request,
	...
	__acquires(ep->musb->lock)
{
	...
	if (request->status == 0)
		DBG(1, "%s done request %p,  %d/%d\n", ep->end_point.name,
		    request, req->request.actual, req->request.length);
	else
...
static void nuke(struct musb_ep *ep, const int status)
{
	...
	while (!list_empty(&ep->req_list)) {
		req = list_first_entry(&ep->req_list, struct musb_request,
				       list);
...
static void txstate(struct musb *musb, struct musb_request *req)
{
	...
	DBG(1, "hw_ep%d, maxpacket %d, fifo count %d, txcsr %03x\n", epnum,
	    musb_ep->packet_sz, fifo_count, csr);
	...
	DBG(1, "%s TX/IN %s len %d/%d, txcsr %04x, fifo %d/%d\n",
	    musb_ep->end_point.name, use_dma ? "dma" : "pio", request->actual,
	    request->length, musb_readw(epio, MUSB_TXCSR), fifo_count,
...
get_function_descriptors(struct usb_function *f, enum usb_device_speed speed)
{
...
static int musb_gadget_dequeue(struct usb_ep *ep, struct usb_request *request)
{
	...
	if (musb_ep->req_list.next != &req->list || musb_ep->busy)
		...
		mtk_qmu_enable(musb, musb_ep->hw_ep->epnum,
			       (musb_ep->is_in ? TXQ : RXQ));
		musb_g_giveback(musb_ep, request, -ECONNRESET);
	}
#else
...
static int musb_gadget_set_halt(struct usb_ep *ep, int value)
{
	...
	if (musb_ep->is_in) {
		...
		csr = musb_readw(epio, MUSB_RXCSR);
		csr |= MUSB_RXCSR_P_WZC_BITS | MUSB_RXCSR_FLUSHFIFO |
...
#if IS_ENABLED(CONFIG_MTK_MD_DIRECT_TETHERING_SUPPORT)
	|| defined(CONFIG_MTK_MD_DIRECT_LOGGING_SUPPORT).suspend_control =
		musb_gadget_suspend_control,
...
#if IS_ENABLED(CONFIG_XEN)
	dma_ops = get_dma_ops(musb->controller);

	if (dma_ops) {
		DBG(0, "musb controller dma ops is non-null\n");
		set_dma_ops(&musb->g.dev, dma_ops);
	}
#endif
...
void musb_g_reset(struct musb *musb) __releases(musb->lock)
	__acquires(musb->lock)
{
	...
	DBG(2, "<== %s driver '%s'\n",
	    (devctl & MUSB_DEVCTL_BDEVICE) ? "B-Device" : "A-Device",
	...
	musb->g.speed =
		(power & MUSB_POWER_HSMODE) ? USB_SPEED_HIGH : USB_SPEED_FULL;
