...
#define EEMGPU_USE_LEGACY_GPUFREQ                                              \
	(IS_ENABLED(CONFIG_MTK_GPU_SUPPORT) &&                                 \
	 !IS_ENABLED(CONFIG_MTK_GPUFREQ_V2))

#if EEMGPU_USE_LEGACY_GPUFREQ
#include "../../../gpu/mediatek/gpufreq/v1/include/mtk_gpufreq_v1.h"
#endif
...
static int get_devinfo(struct platform_device *pdev)
{
	...
	if (mcl50) {
		for (i = 0; i < count; i++) {
			of_property_read_u32_index(node, "devinfo-mcl50", i,
						   &efuse);
			val[i] = efuse;
			eemg_error("[PTP_DUMP] RES%d: 0x%X\n", i, val[i]);
		}
	...
	golden_temp = ((devinfo & efuse)) >>
		      find_first_bit((unsigned long *)&efuse, 32);
...
#ifdef CONFIG_EEMG_AEE_RR_REC
		aee_kernel_warning("mt_eem", "@%s():%d, EEMGCORESEL %x != %x\n",
				   __func__, __LINE__, eemg_read(EEMGCORESEL),
				   coresel);
#endif
...
void base_ops_set_phase_gpu(struct eemg_det *det, enum eemg_phase phase)
{
	...
	eemg_write(EEMG_LIMITVALS, ((det->VMAX << 24) & 0xff000000) |
...
#ifdef CONFIG_EEMG_AEE_RR_REC
		aee_kernel_warning("mt_eem", "@%s():%d, EEMG_CHKSHIFT %x\n",
				   __func__, __LINE__,
				   eemg_read(EEMG_CHKSHIFT));
#endif
...
#ifdef CONFIG_EEMG_AEE_RR_REC
			aee_kernel_warning("mt_eem", "@%s():%d, EEMGEN %x\n",
					   __func__, __LINE__,
					   eemg_read(EEMGEN));
#endif
...
static void eemg_calculate_aging_margin(struct eemg_det *det, int start_oft,
					int end_oft)
{
	...
	for (i = 0; i < det->num_freq_tbl; i++) {
		if (i == 0)
			...
			det->volt_aging[i] =
				start_oft - ((offset * i) / det->num_freq_tbl);
	}
}
		...
		case EEMG_CTRL_GPU:
			if (i < 8) {
				aee_rr_rec_ptp_gpu_volt_2(
					((unsigned long long)(ndet->volt_tbl_pmic
								      [i])
					 << (8 * i)) |
...
static void get_volt_table_in_thread(struct eemg_det *det)
{
	...
	ndet = (det->loo_role == HIGH_BANK) ? id_to_eemg_det(det->loo_couple) :
					      det;
	eemg_debug("@@! In %s\n", __func__);
	...
	for (i = 0; i < ndet->num_freq_tbl; i++) {
		...
		if (ndet->isTempInv) {
			low_temp_offset = (ndet->isTempInv == EEM_HIGH_T) ?
						  ndet->high_temp_off :
						  ndet->low_temp_off;
		}
		...
		case EEMG_CTRL_GPU:
			if ((i == 0) && (gpu_vb != 0 && gpu_vb_turn_pt != 0))
				ndet->volt_tbl_pmic[i] = min(
					(unsigned int)(clamp(ndet->ops->eemg_2_pmic(
								     ndet,
								     (ndet->ops->volt_2_eemg(
									     ndet,
									     gpu_vb_volt))),
							     ndet->ops->eemg_2_pmic(
								     ndet,
								     ndet->VMIN),
							     ndet->ops->eemg_2_pmic(
								     ndet,
		...
		if ((i > 0) &&
		    (ndet->volt_tbl_pmic[i] > ndet->volt_tbl_pmic[i - 1]) &&
...
static void eemg_init_ctrl(struct eemg_ctrl *ctrl)
{
	...
	if (1) {
		...
		ctrl->thread =
			kthread_run(eemg_volt_thread_handler, ctrl, ctrl->name);
			  ...
			  struct platform_device *pdev)
{
	...
	if (det->ops->get_freq_table_gpu) {
		h_det = (det->loo_role == HIGH_BANK) ?
				det :
				id_to_eemg_det(det->loo_couple);
		l_det = (det->loo_role == LOW_BANK) ?
				det :
				id_to_eemg_det(det->loo_couple);
		det->ops->get_freq_table_gpu(det, h_det->max_freq_khz,
					     l_det->max_freq_khz);
	}
...
static void eemg_fill_freq_table(struct eemg_det *det)
{
	...
	if (det->loo_role == HIGH_BANK) {
		...
		if (det->turn_pt < 8) {
			...
			for (i = (det->turn_pt - 7), j = 1; i < det->turn_pt;
			     i++, j++) {
				shift_bits = (j % 4) * 8;
...
static void read_volt_from_VOP(struct eemg_det *det)
{
	...
	if (det->loo_role == HIGH_BANK) {
		...
		if ((det->num_freq_tbl - det->turn_pt) <= 8) {
			/* For opp8~15 */
			for (i = det->turn_pt + 1, j = 1; i < det->num_freq_tbl;
			     i++, j++) {
				shift_bits = (j % 4) * 8;
				readvop = (j < 4) ? temp30 : temp74;
				det->volt_tbl[i] =
			...
			/* prepare middle opp */
			for (i = (det->turn_pt + 1); i < det->num_freq_tbl - 7;
			     i++) {
				det->volt_tbl[i] = interpolate(
...
static inline void handle_init_err_isr(struct eemg_det *det)
{
	...
	eemg_error("BD/MD/DCB/DCM/MTD=0x%X,%X,%X,%X,%X\n", det->BDES, det->MDES,
		   det->DCBDET, det->DCMDET, det->MTDES);
	eemg_error("DETWINDOW/VMIN/VMAX/DTHI/DTLO=0x%X,%X,%X,%X,%X\n",
		   det->DETWINDOW, det->VMIN, det->VMAX, det->DTHI, det->DTLO);
	eemg_error("DETMAX/DVTFIXED/VCO/DCCONFIG/=0x%X,%X,%X,%X\n", det->DETMAX,
		   det->DVTFIXED, det->VCO, det->DCCONFIG);
	...
	for (i = 0; i < sizeof(struct eemg_devinfo) / sizeof(unsigned int); i++)
		eemg_error("M_HW_RES%d= 0x%08X\n", i, val[i]);

	eemg_error("EEM init err: EEMGEN(%p) = 0x%X, EEMGINTSTS(%p) = 0x%X\n",
		   EEMGEN, eemg_read(EEMGEN), EEMGINTSTS,
		   eemg_read(EEMGINTSTS));
	...
	eemg_error("EEMG_DESCHAR (%p) = 0x%X,0x%X,0x%X\n", EEMG_DESCHAR,
		   eemg_read(EEMG_DESCHAR), eemg_read(EEMG_TEMPCHAR),
		   eemg_read(EEMG_DETCHAR));
...
static inline void handle_mon_mode_isr(struct eemg_det *det)
{
	...
	case EEMG_CTRL_GPU:
		...
		if (temp_long != 0) {
			aee_rr_rec_ptp_temp(
				temp_long << (8 * EEMG_GPU_IS_SET_VOLT) |
				(temp_cur &
				 ~((0xFFULL) << (8 * EEMG_GPU_IS_SET_VOLT))));
		}
#endif
		break;
	case EEMG_CTRL_GPU_HI:
		...
		if (temp_long != 0) {
			aee_rr_rec_ptp_temp(
				temp_long << (8 * EEMG_GPU_HI_IS_SET_VOLT) |
				(temp_cur &
				 ~((0xFFULL)
...
static inline void handle_mon_err_isr(struct eemg_det *det)
{
	...
	eemg_error(
		...
		EEMGCORESEL, eemg_read(EEMGCORESEL), EEMG_THERMINTST,
		eemg_read(EEMG_THERMINTST));
	eemg_error("EEM0DINTST(%p) = 0x%08X", EEMGODINTST,
		   eemg_read(EEMGODINTST));
	eemg_error("EEMGINTSTSRAW(%p) = 0x%08X, EEMGINTEN(%p) = 0x%08X\n",
		   EEMGINTSTSRAW, eemg_read(EEMGINTSTSRAW), EEMGINTEN,
		   eemg_read(EEMGINTEN));
	eemg_error("====================================================\n");
	...
	eemg_error("====================================================\n");
	eemg_error("EEM mon err: EEMGCORESEL(%p) = 0x%08X,", EEMGCORESEL,
		   eemg_read(EEMGCORESEL));
	eemg_error(" EEMG_THERMINTST(%p) = 0x%08X, EEMGODINTST(%p) = 0x%08X",
		   EEMG_THERMINTST, eemg_read(EEMG_THERMINTST), EEMGODINTST,
		   eemg_read(EEMGODINTST));
	...
	aee_kernel_warning(
		...
		__func__, __LINE__, det->name, det->VBOOT, EEMGEN,
		eemg_read(EEMGEN), EEMGINTSTS, eemg_read(EEMGINTSTS));
#endif
...
static void eemg_dconfig_set_det(struct eemg_det *det, struct device_node *node)
{
	...
	case EEMG_DET_GPU:
		rc1 = of_property_read_u32(node, "eemg-initmon-gpu",
					   &doe_initmon);
		rc2 = of_property_read_u32(node, "eemg-clamp-gpu", &doe_clamp);
...
static int eemg_probe(struct platform_device *pdev)
{
...
#if SUPPORT_DCONFIG
	if (of_property_read_u32(node, "eemg-status", &doe_status) < 0) {
	...
	/* set EEM IRQ */
	ret = request_irq(eemg_irq_number, eemg_isr, IRQF_TRIGGER_HIGH, "eemg",
			  NULL);
	if (ret) {
	...
	for_each_ctrl(ctrl) eemg_init_ctrl(ctrl);

	eemg_debug("finish eemg_init_ctrl\n");

	/* for slow idle */
	for_each_det(det) eemg_init_det(det, &eemg_devinfo, pdev);
...
#if SUPPORT_DCONFIG
	for_each_det(det) eemg_dconfig_set_det(det, node);
#endif
	...
	for (i = EEMG_PHASE_INIT01; i <= EEMG_PHASE_MON; i++) {
		...
		if (eemg_log_en) {
			seq_printf(m,
				   ...
				   det->eemg_26c[i], det->vop30[i],
				   det->eemg_eemEn[i]);

			if (det->eemg_eemEn[i] == (0x5 | SEC_MOD_SEL)) {
				seq_printf(
					m,
					"EEMG_LOG: Bank_number = [%d] (%d) - (",
					det->ctrl_id,
...
static int eemg_status_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "bank = %d, feature:%d, T(%d) - (", det->ctrl_id,
		   det->features, det->ops->get_temp_gpu(det));
				      ...
				      loff_t *pos)
{
	...
	case 1:
		...
		hrtimer_start(&eemg_log_timer, ns_to_ktime(LOG_INTERVAL),
			      HRTIMER_MODE_REL);
		break;
...
static int create_procfs(struct platform_device *pdev)
{
	...
	struct pentry det_entries[] = {
		PROC_ENTRY(eemg_debug),	    PROC_ENTRY(eemg_status),
		PROC_ENTRY(eemg_cur_volt),  PROC_ENTRY(eemg_offset),
	...
	if (ctrl_EEMG_Enable != 0 && eemg_checkEfuse == 1) {
		for (i = 0; i < ARRAY_SIZE(eemg_entries); i++) {
			if (!proc_create(eemg_entries[i].name, 0664, eemg_dir,
					 eemg_entries[i].fops)) {
				eemg_error("[%s]: create /proc/eem/%s failed\n",
					   __func__, eemg_entries[i].name);
				FUNC_EXIT(FUNC_LV_HELP);
				return -3;
			}
		}
...
void eemg_set_pi_efuse(enum eemg_det_id id, unsigned int pi_efuse,
