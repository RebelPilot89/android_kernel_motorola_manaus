...
static void qos_scmi_handler(u32 r_feature_id, scmi_tinysys_report *report)
{
	...
	if (cmd == QOS_IPI_QOS_BOUND)
		qos_notifier_call_chain(arg, get_qos_bound());
	else
...
int qos_ipi_to_sspm_scmi_command(unsigned int cmd, unsigned int p1,
				 ...
				 unsigned int p4)
{
	...
	case QOS_IPI_SCMI_SET:
		ret = scmi_tinysys_common_set(_tinfo->ph, scmi_qos_id, cmd, p1,
					      p2, p3, p4);
		if (ret) {
	...
	case QOS_IPI_SCMI_GET:
		ret = scmi_tinysys_common_get(_tinfo->ph, scmi_qos_id, cmd,
					      &rvalue);
		if (ret) {
