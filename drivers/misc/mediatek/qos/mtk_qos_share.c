...
static void qos_share_sspm_setup(void)
{
	...
	qos_ipi_to_sspm_scmi_command(qos_ipi_d.cmd,
				     ...
				     qos_ipi_d.u.qos_share_init.dram_size, 0,
				     0);
...
static void qos_get_rec_addr(phys_addr_t *phys, phys_addr_t *virt,
			     unsigned long long *size)
{
	...
	*size = sspm_reserve_mem_get_size(QOS_MEM_ID);
	...
	pr_info("qos: rec phy_addr = 0x%llx, virt_addr=0x%llx, size = %llu\n",
		(unsigned long long)*phys, (unsigned long long)*virt, *size);

#endif
}

static int qos_reserve_mem_init(phys_addr_t *virt, unsigned long long *size)
{
	int i;
	unsigned char *ptr;
...
int qos_init_rec_share(void)
{
	...
	qos_get_rec_addr(&rec_phys_addr, &rec_virt_addr, &rec_size);
