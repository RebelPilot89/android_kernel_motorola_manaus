...
#define EMI_CEN_NODE "mediatek,common-emicen"
#define EMI_CHN_NODE "mediatek,common-emichn"
...
int __init mt_dcm_dts_map(void)
{
	...
	if (!dcm_dramc0_ao_base) {
		dcm_pr_info("error: cannot iomap dcm_dramc0_ao_base");
		return -1;
	}
	...
	if (!dcm_dramc1_ao_base) {
		dcm_pr_info("error: cannot iomap dcm_dramc1_ao_base");
		return -1;
	}
	...
	if (!dcm_ddrphy0_ao_base) {
		dcm_pr_info("error: cannot iomap dcm_ddrphy0_ao_base");
		return -1;
	}
	...
	if (!dcm_ddrphy1_ao_base) {
		dcm_pr_info("error: cannot iomap dcm_ddrphy1_ao_base");
		return -1;
	}
	...
	dcm_chen0_emi_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_chen0_emi_base) {
		dcm_pr_info("error: cannot iomap dcm_chen0_emi_base");
		return -1;
	}
	...
	if (!dcm_chn0_emi_base) {
		dcm_pr_info("error: cannot iomap dcm_chn0_emi_base");
		return -1;
	}
	dcm_chn1_emi_base = (unsigned long)of_iomap(node, 1);
	if (!dcm_chn1_emi_base) {
		dcm_pr_info("error: cannot iomap dcm_chn1_emi_base");
		return -1;
	}
