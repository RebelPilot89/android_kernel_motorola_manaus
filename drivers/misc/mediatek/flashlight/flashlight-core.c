...
#include "../power_throttling/mtk_battery_oc_throttling.h"
#include "../power_throttling/mtk_low_battery_throttling.h"
#include "../power_throttling/mtk_bp_thl.h"
#endif
	...
	if (pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur))
		if (fdev->low_pt_level >= 0 && level > fdev->low_pt_level) {
			...
			pr_info("Set level to (%d) since pt(%d,%d,%d), pt strict(%d)\n",
				level, pt_low_vol, pt_low_bat, pt_over_cur,
				pt_strict);
		}
#endif
...
static int pt_trigger(void)
{
	...
	list_for_each_entry (fdev, &flashlight_list, node) {
		...
		if (pt_strict) {
			pr_info_ratelimited(
				"PT trigger(%d,%d,%d) disable flashlight\n",
				pt_low_vol, pt_low_bat, pt_over_cur);
			fl_enable(fdev, 0);
		} else {
			pr_info_ratelimited(
				"PT trigger(%d,%d,%d) decrease duty: %d\n",
				pt_low_vol, pt_low_bat, pt_over_cur,
				fdev->low_pt_level);
				       ...
				       const char *buf, size_t size)
{
	...
	pr_debug("(%d, %d, %d), (%d, %d)\n", fl_arg.type, fl_arg.ct,
		 fl_arg.part, fl_arg.level, fl_arg.dur);
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry (fdev, &flashlight_list, node) {
		...
		ret = snprintf(status_tmp, FLASHLIGHT_TORCH_STATUS_TMPBUF_SIZE,
			       "%d %d %d %d\n", fdev->dev_id.type,
			       fdev->dev_id.ct, fdev->dev_id.part,
			       fdev->torch_status);
		if (ret < 0)
...
static int __init flashlight_init(void)
{
	...
	register_bp_thl_notify(&pt_low_bat_callback,
