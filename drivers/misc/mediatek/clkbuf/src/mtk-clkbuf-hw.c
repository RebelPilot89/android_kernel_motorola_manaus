...
static unsigned int CLK_BUF_STATUS[XO_NUMBER] = {
	CLOCK_BUFFER_HW_CONTROL, CLOCK_BUFFER_SW_CONTROL,
	CLOCK_BUFFER_SW_CONTROL, CLOCK_BUFFER_HW_CONTROL,
	CLOCK_BUFFER_DISABLE,	 CLOCK_BUFFER_DISABLE,
	CLOCK_BUFFER_SW_CONTROL
};

static unsigned int CLK_BUF_OUTPUT_IMPEDANCE[XO_NUMBER] = {
	CLK_BUF_OUTPUT_IMPEDANCE_6, CLK_BUF_OUTPUT_IMPEDANCE_4,
	CLK_BUF_OUTPUT_IMPEDANCE_6, CLK_BUF_OUTPUT_IMPEDANCE_4,
	CLK_BUF_OUTPUT_IMPEDANCE_0, CLK_BUF_OUTPUT_IMPEDANCE_0,
	CLK_BUF_OUTPUT_IMPEDANCE_6
};

static unsigned int CLK_BUF_CONTROLS_DESENSE[XO_NUMBER] = {
	CLK_BUF_CONTROLS_FOR_DESENSE_0, CLK_BUF_CONTROLS_FOR_DESENSE_4,
	CLK_BUF_CONTROLS_FOR_DESENSE_0, CLK_BUF_CONTROLS_FOR_DESENSE_4,
	CLK_BUF_CONTROLS_FOR_DESENSE_0, CLK_BUF_CONTROLS_FOR_DESENSE_0,
	CLK_BUF_CONTROLS_FOR_DESENSE_0
};

static unsigned int CLK_BUF_DRIVING_CURRENT[XO_NUMBER] = {
	CLK_BUF_DRIVING_CURR_1, CLK_BUF_DRIVING_CURR_1, CLK_BUF_DRIVING_CURR_1,
	CLK_BUF_DRIVING_CURR_1, CLK_BUF_DRIVING_CURR_1, CLK_BUF_DRIVING_CURR_1,
 ...
 * of data we present.
 */
static char XO_NAME[XO_NUMBER][7] = { "XO_SOC", "XO_WCN", "XO_NFC", "XO_CEL",
				      "XO_AUD", "XO_PD",  "XO_EXT" };
...
static enum CLK_BUF_TYPE pmic_clk_buf_swctrl[XO_NUMBER] = {
	CLK_BUF_SW_ENABLE, CLK_BUF_SW_DISABLE, CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_ENABLE, CLK_BUF_SW_DISABLE, CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_ENABLE
};
#else /* For Bring-up */
static enum CLK_BUF_TYPE pmic_clk_buf_swctrl[XO_NUMBER] = {
	...
	CLK_BUF_SW_ENABLE, CLK_BUF_SW_DISABLE, CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_ENABLE
};
#endif
...
static inline void clkbuf_update(u32 dts, u32 id, u32 val)
{
	...
	regmap_update_bits(cfg[dts].regmap, cfg[dts].ofs[id], mask, val);
...
{
	if (!clk_buf_get_init_sta() || id < 0 || id >= XO_NUMBER || id == -1)
		return false;
...
static int _clk_buf_xo_get_name(s32 id, char *name)
{
	...
	if (pmic_op && pmic_op->pmic_clk_buf_get_xo_name)
		ret = pmic_op->pmic_clk_buf_get_xo_name(id, name);
	else
		return CLK_BUF_NOT_READY;
...
static unsigned int _clk_buf_mode_get(s32 id)
{
	...
	if (pmic_op && pmic_op->pmic_clk_buf_get_xo_mode)
		ret = pmic_op->pmic_clk_buf_get_xo_mode(id, &val);
	else
		return CLK_BUF_NOT_READY;
...
static u32 _clk_buf_en_get(s32 id)
{
	...
	if (pmic_op && pmic_op->pmic_clk_buf_get_xo_sw_en) {
		ret = pmic_op->pmic_clk_buf_get_xo_sw_en(id, &onoff);
		if (ret) {
...
static ssize_t _clk_buf_show_status_info_internal(char *buf)
{
	...
	for (i = 0; i < XO_NUMBER; i++) {
		...
		if (buf_en >= 0)
			len += snprintf(
				...
				i, XO_M_NAME[id][buf_mode], buf_mode,
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			"DCXO_ENABLE=0x%x, flight mode = %d ", val[0],
			clk_buf_get_flight_mode());

	ret = _clk_buf_get_bblpm_en_stat();
	if (ret >= 0)
		bblpm_stat = ret;
	len += snprintf(buf + len, PAGE_SIZE - len, "bblpm = %d\n",
			(ret >= 0) ? bblpm_stat : ret);
				  ...
				  size_t count)
{
	...
	if (sscanf(buf, "%31s %x %x %x %x %x %x %x", cmd, &clk_buf_en[XO_SOC],
		   ...
		   &clk_buf_en[XO_CEL], &clk_buf_en[XO_AUD], &clk_buf_en[XO_PD],
		   &clk_buf_en[XO_EXT]) != (XO_NUMBER + 1))
		return -EPERM;
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < XO_NUMBER; i++) {
		...
		len += snprintf(buf + len, PAGE_SIZE - len, "EN_STAT(%s)=%d\n",
				xo_name[id], xo_stat);
	}
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "bblpm en_stat(%d)\n",
			bblpm_stat);

	len += snprintf(buf + len, PAGE_SIZE - len, "bblpm cond(0x%x)\n",
			bblpm_cond);
...
static void _clk_buf_read_dts_misc_node(struct device_node *node)
{
	...
	ret = of_property_read_string(node, "mediatek,bblpm-support", &str);
...
static int _clk_buf_dts_init_internal(struct device_node *node, u32 idx)
{
	...
	for (i = 0; i < clkbuf_dts[idx].len && interval == 0; i++) {
		ret = of_property_read_u32_index(node, clkbuf_dts[idx].prop,
		...
		ret = of_property_read_u32_index(
			node, clkbuf_dts[idx].prop,
			clkbuf_dts[idx].idx + i * 2 + 1, &cfg[idx].bit[i]);
		if (ret) {
			pr_info("[%s]: find %s property failed\n", __func__,
				clkbuf_dts[idx].prop);
			goto no_property;
...
int clk_buf_dts_init(struct platform_device *pdev)
{
	...
	for (i = 0; i < REGMAP_NUM; i++) {
		...
		regmap = syscon_regmap_lookup_by_phandle(node, base_n[i]);
		if (IS_ERR(regmap)) {
			dev_err(&pdev->dev, "Cannot find %s controller: %ld\n",
				base_n[i], PTR_ERR(regmap));
			goto no_compatible;
		}
...
void clk_buf_get_bringup_node(struct platform_device *pdev)
{
	...
	if (!get_bringup_state_done) {
		...
		ret = of_property_read_string(node, "mediatek,bring-up", &str);
		if (ret || (!strcmp(str, "enable"))) {
...
int clk_buf_xo_init(void)
{
	...
	if (_get_ufs_dev_state() != DEV_ON)
		CLK_BUF_STATUS[xo_match_idx[XO_EXT]] = CLOCK_BUFFER_DISABLE;

	if (_get_nfc_dev_state() != DEV_ON)
		CLK_BUF_STATUS[xo_match_idx[XO_NFC]] = CLOCK_BUFFER_DISABLE;
