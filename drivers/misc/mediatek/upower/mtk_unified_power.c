...
static int __init upower_init(void)
{
	...
	if (err < 0) {
		...
		/* cannot proceed without tables - abort initialization */
		return -ENODEV;
	}
	/* ensure upower_tbl_infos has been populated by platform code */
	if (!upower_tbl_infos) {
		upower_error("%s: upower_tbl_infos not ready\n", __func__);
		return -ENODEV;
	}
	upower_init_rownum();
