	...
	list_for_each_entry (pbm_policy, &pbm_policy_list, cpu_pbm_list) {
		...
		if (num_cpus == 0 || total_power_weight == 0) {
			pr_info("warning: num_cpus=%d total_power_weight=%d\n",
				num_cpus, total_power_weight);
			continue;
		}

		granted_power = limit_power * pbm_policy->power_weight /
				total_power_weight / num_cpus;
		frequency = cpu_power_to_freq(pbm_policy, granted_power);
		if (frequency)
			freq_qos_update_request(&pbm_policy->qos_req,
						frequency);
	}
}
...
static void mtk_gpufreq_set_power_limit_by_pbm(int limit_power)
{
	...
	if (pbm_gpu_cb.set_limit != NULL)
		pbm_gpu_cb.set_limit(TARGET_DEFAULT, LIMIT_PBM, limit_power,
				     GPUPPM_KEEP_IDX);
#else
	return;
#endif
...
{
#if IS_ENABLED(CONFIG_MTK_GPUFREQ_V2)
	if (pbm_gpu_cb.get_max_pb != NULL && gpu_max_pb == 0) {
		gpu_max_pb = pbm_gpu_cb.get_max_pb(TARGET_DEFAULT);
	}
	if (pbm_gpu_cb.get_min_pb != NULL && gpu_min_pb == 0) {
...
static void pbm_allocate_budget_manager(void)
{
	...
	if (mt_pbm_debug) {
		...
		if (((abs(pre_tocpu - tocpu) >= 10) && cpu > tocpu) ||
			...
			pr_info_ratelimited(
				"(C/G)=%d,%d => (D/M1/F/C/G)=%d,%d,%d,%d,%d (Multi:%d),%d\n",
				cpu, gpu, dlpt, md1, flash, tocpu, togpu,
				multiple, cpu_lower_bound);
		} else {
			pre_tocpu = tocpu;
...
static int _mt_pbm_pm_callback(struct notifier_block *nb, unsigned long action,
			       void *ptr)
{
	switch (action) {
...
static void pbm_cpu_frequency_tracer(void *ignore, unsigned int frequency,
				     unsigned int cpu_id)
{
	...
	list_for_each_entry (pbm_policy, &pbm_policy_list, cpu_pbm_list) {
		...
		pbm_policy->power =
			cpu_freq_to_power(pbm_policy, pbm_policy->freq) *
			pbm_policy->num_cpus;
	}
...
#define FOR_EACH_INTEREST(i)                                                   \
	for (i = 0;                                                            \
	     i < sizeof(pbm_tracepoints) / sizeof(struct tracepoints_table);   \
	     i++)
...
static void tracepoint_cleanup(void)
{
	...
	FOR_EACH_INTEREST(i)
	{
		if (pbm_tracepoints[i].registered) {
			tracepoint_probe_unregister(pbm_tracepoints[i].tp,
						    pbm_tracepoints[i].func,
						    NULL);
...
static int mt_pbm_manual_mode_proc_show(struct seq_file *m, void *v)
{
	...
	if (pbm_ctrl.manual_mode > 0) {
		...
		seq_printf(m, "request (C/G)=%lu,%lu=>(D/M1/F)=%lu,%lu,%lu\n",
			   hpf_ctrl.loading_cpu, hpf_ctrl.loading_gpu,
			   hpf_ctrl.loading_dlpt, hpf_ctrl.loading_md1,
...
static ssize_t mt_pbm_manual_mode_proc_write(struct file *file,
					     ...
					     size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%20s %d %d %d %d %d %d", cmd, &manual_mode,
		   &loading_dlpt, &loading_md1, &loading_cpu, &loading_gpu,
		   &loading_flash) != 7) {
...
static ssize_t mt_pbm_stop_proc_write(struct file *file,
				      const char __user *buffer, size_t count,
				      loff_t *data)
{
	char desc[64], cmd[21];
...
static int pbm_probe(struct platform_device *pdev)
{
	...
	ret = tracepoint_probe_register(pbm_tracepoints[0].tp,
					pbm_tracepoints[0].func, NULL);
	...
	for_each_possible_cpu (cpu) {
		...
		if (policy->cpu == cpu) {
			...
			if (pbm_policy->em) {
				pbm_policy->max_perf_state =
					pbm_policy->em->nr_perf_states;
			} else {
			...
			list_add_tail(&pbm_policy->cpu_pbm_list,
				      &pbm_policy_list);
		}
	}
...
static int pbm_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry_safe (pbm_policy, pbm_policy_t, &pbm_policy_list,
				  cpu_pbm_list) {
