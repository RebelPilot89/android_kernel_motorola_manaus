			    ...
			    struct ethhdr *eth, struct sk_buff *skb)
{
	...
	if (htons(request->ar_hrd) != ARPHRD_ETHER ||
	    htons(request->ar_pro) != ETH_P_IP || request->ar_hln != ETH_ALEN ||
	    request->ar_pln != 4 || htons(request->ar_op) != ARPOP_REQUEST) {
		CCMNI_DBG_MSG(
			md_id,
...
static int ccmni_open(struct net_device *dev)
{
	...
	if (ccmni->md_id < 0 || ccmni->md_id >= MAX_MD_NUM ||
		...
		CCMNI_INF_MSG(
			-1,
			"%s : invalid md_id or index:md_id = %d,index = %d\n",
	...
	queue_delayed_work(ccmni->worker, &ccmni->pkt_queue_work,
...
static netdev_tx_t ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (ret == CCMNI_ERR_MD_NO_READY || ret == CCMNI_ERR_TX_INVAL) {
		...
		CCMNI_DBG_MSG(ccmni->md_id,
			      ...
			      ccmni->index, (dev->stats.tx_packets + 1), is_ack,
			      ret);
		return NETDEV_TX_OK;
	...
	if (ccmni->tx_met_time == 0) {
		...
		ccmni->tx_met_bytes = dev->stats.tx_bytes;
				 ...
				 ccmni->tx_met_time +
					 msecs_to_jiffies(MET_LOG_TIMER))) {
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCCCMNICFG:
		...
		CCMNI_DBG_MSG(
			...
			dev->name, (md_id + 1), (ifr->ifr_ifru.ifru_ivalue + 1),
			atomic_read(&ccmni->usage),
			atomic_read(&ccmni_tmp->usage),
			atomic_read(&ccmni_irat->usage), ccmni_irat->dev->name);
		break;
	...
	case SIOCFWDFILTER:
		...
		if (flt_tmp.ver != 0x40 && flt_tmp.ver != 0x60) {
			CCMNI_INF_MSG(
				...
				flt_act.action, dev->name, flt_tmp.ver,
		...
		if (flt_act.action == CCMNI_FLT_ADD) { /* add new filter */
			...
			CCMNI_INF_MSG(
				...
				dev->name, i, flt_tmp.ver, flt_tmp.s_pref,
			...
			for (i = 0; i < CCMNI_FLT_NUM; i++) {
				if (ccmni->flt_tbl[i].ver == 0)
					continue;
				if (!memcmp(&ccmni->flt_tbl[i], &flt_tmp,
					    cmp_len)) {
					CCMNI_INF_MSG(
						ccmni->md_id,
			...
			if (i >= CCMNI_FLT_NUM) {
				CCMNI_INF_MSG(
					...
					dev->name, flt_tmp.ver, flt_tmp.s_pref,
					flt_tmp.d_pref, flt_tmp.ipv4.saddr,
					flt_tmp.ipv4.daddr, ccmni->flt_cnt);
				return -ENXIO;
			}
...
static int ccmni_napi_poll(struct napi_struct *napi, int budget)
{
	...
	if (ctlb->ccci_ops->napi_poll)
		return ctlb->ccci_ops->napi_poll(md_id, ccmni->index, napi,
						 budget);
	else
		return 0;
#endif
				  ...
				  struct net_device *dev)
{
	...
	ccmni->worker =
		alloc_workqueue("ccmni%d_rx_q_worker",
 ...
 */
static void __maybe_unused mtk_dis_ipv6_lla(void *ignore,
					    struct net_device *dev, bool *ret)
{
...
static int ccmni_init(int md_id, struct ccmni_ccci_ops *ccci_info)
{
	...
	ret = register_trace_android_vh_ipv6_gen_linklocal_addr(
		mtk_dis_ipv6_lla, NULL);
	if (ret) {
	...
	for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
		...
		if (ctlb->ccci_ops->md_ability & MODEM_CAP_CCMNI_MQ)
			...
			dev = alloc_etherdev_mqs(sizeof(struct ccmni_instance),
						 2, 1);
		else
		...
		scnprintf(dev->name, sizeof(dev->name), "%s%d",
			  ctlb->ccci_ops->name, i);
	...
	snprintf(ctlb->wakelock_name, sizeof(ctlb->wakelock_name), "ccmni_md%d",
		 (md_id + 1));
	ctlb->ccmni_wakelock =
		wakeup_source_register(NULL, ctlb->wakelock_name);
	if (!ctlb->ccmni_wakelock) {
		CCMNI_PR_DBG(md_id, "%s %d: init wakeup source fail!", __func__,
			     __LINE__);
		return -1;
	}
...
static void ccmni_dump(int md_id, int ccmni_idx, unsigned int flag)
{
	...
	if (ctlb->ccci_ops->md_ability & MODEM_CAP_CCMNI_MQ) {
		...
		CCMNI_INF_MSG(
			...
			qdisc->q.qlen, ack_qdisc->q.qlen, dev->stats.tx_dropped,
			qdisc->qstats.drops, ack_qdisc->qstats.drops,
		...
		CCMNI_INF_MSG(
			...
			dev->stats.rx_packets, dev->stats.rx_bytes,
			ccmni->rx_gro_cnt, dev->stats.tx_packets,
			dev->stats.tx_bytes, dev->qdisc->q.qlen,
			dev->stats.tx_dropped, dev->qdisc->qstats.drops,
			dev->stats.rx_dropped,
			atomic_long_read(&dev->rx_dropped),
			ccmni->tx_busy_cnt[0], ccmni->tx_busy_cnt[1],
			dev->state, dev->flags, dev_queue->state);
