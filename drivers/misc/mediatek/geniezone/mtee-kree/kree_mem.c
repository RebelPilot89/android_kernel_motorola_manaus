...
static struct KREE_SHM_RUNLENGTH_ENTRY *
shmem_param_run_length_encoding(int numOfPA, int *runLeng_arySize, int64_t *ary)
{
				     ...
				     const char *tag)
{
	...
	ret = _allocFunc(TZCMD_MEM_SECUREMEM_ALLOC_WITH_TAG, session,
			 mem_handle, alignment, size,
			 "KREE_ZallocSecurememWithTag");
	return ret;
}
				      ...
				      const char *tag)
{
	...
	ret = _allocFunc(TZCMD_MEM_SECUREMEM_ZALLOC_WITH_TAG, session,
			 mem_handle, alignment, size,
			 "KREE_ZallocSecurememWithTag");
	return ret;
}
				  ...
				  KREE_SHAREDMEM_PARAM *param, uint32_t cmd)
{
	...
	ret = kree_register_sharedmem(session, cm_hd, param->buffer,
				      param->size, param->mapAry, cmd,
				      param->region_id);
	...
	ret = gz_do_m4u_map(*cm_hd, (phys_addr_t)param->buffer, param->size,
			    param->region_id);
#endif
...
TZ_RESULT KREE_AppendSecureMultichunkmem(KREE_SESSION_HANDLE session,
					 ...
					 KREE_SHAREDMEM_PARAM *param)
{
	return _kree_mcm_Append(session, cm_handle, param,
				 ...
				 uint32_t alignment, uint32_t size)
{
	...
	*secmHandle = 0;
	...
	ret = KREE_AllocSecureMultichunkmem(session, chm_handle, &mem_handle,
					    alignment, size);
				  ...
				  uint32_t alignment, uint32_t size)
{
	...
	*secmHandle = 0;
	...
	ret = KREE_ZallocSecureMultichunkmem(session, chm_handle, &mem_handle,
					     alignment, size);
	if ((ret != TZ_RESULT_SUCCESS) || (!mem_handle)) {
		KREE_ERR("[%s]alloc Fail: mem_hd=0x%x, ret=0x%x)\n", __func__,
			 mem_handle, ret);
...
{
	return _kree_mcm_Append(session, cm_handle, param,
				TZCMD_MEM_APPEND_MULTI_CHUNKMEM);
}
...
#if IS_ENABLED(CONFIG_GZ_VPU_WITH_M4U)
	if (is_region_on)
		ret = gz_do_m4u_map(session, pa, size, region_id);
	else
