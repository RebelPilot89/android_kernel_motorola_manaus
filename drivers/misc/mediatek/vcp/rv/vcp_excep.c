...
static struct reg_save_st reg_save_list[] = {
	...
	{ 0x1EC24000, 0x170 }, { 0x1EC30000, 0x180 }, { 0x1EC32000, 0x260 },
	{ 0x1EC33000, 0x120 }, { 0x1EC40000, 0x180 }, { 0x1EC42000, 0x260 },
	{ 0x1EC43000, 0x120 }, { 0x1EC50000, 0x330 }, { 0x1EC51000, 0x10 },
	{ 0x1EC51400, 0x70 },  { 0x1EC52000, 0x340 }, { 0x1ECA5000, 0x110 },
};
...
void vcp_dump_last_regs(int mmup_enable)
{
	...
	pr_notice(
		...
		readl(VCP_BUS_DBG_AR_TRACK0_L), readl(VCP_BUS_DBG_AR_TRACK1_L),
		readl(VCP_BUS_DBG_AR_TRACK2_L), readl(VCP_BUS_DBG_AR_TRACK3_L),
		readl(VCP_BUS_DBG_AR_TRACK4_L), readl(VCP_BUS_DBG_AR_TRACK5_L),
		readl(VCP_BUS_DBG_AR_TRACK6_L), readl(VCP_BUS_DBG_AR_TRACK7_L));
	pr_notice(
		"W %08x %08x %08x %08x %08x %08x %08x %08x\n",
		readl(VCP_BUS_DBG_AW_TRACK0_L), readl(VCP_BUS_DBG_AW_TRACK1_L),
		readl(VCP_BUS_DBG_AW_TRACK2_L), readl(VCP_BUS_DBG_AW_TRACK3_L),
		readl(VCP_BUS_DBG_AW_TRACK4_L), readl(VCP_BUS_DBG_AW_TRACK5_L),
		readl(VCP_BUS_DBG_AW_TRACK6_L), readl(VCP_BUS_DBG_AW_TRACK7_L));
...
static unsigned int vcp_crash_dump(enum vcp_core_id id)
{
...
#ifdef VCP_DEBUG_REMOVED
	//TO DO: SMC aee dump
	memcpy_from_vcp((void *)get_MDUMP_addr(MDUMP_L2TCM), (void *)(VCP_TCM),
			(VCP_A_TCM_SIZE));
	...
	if ((int)(vcp_region_info_copy.ap_dram_size) <= 0) {
		...
		/* copy dram data*/
		memcpy((void *)&(pMemoryDump->dram), vcp_ap_dram_virt,
		       dram_size);
...
static void vcp_prepare_aed_dump(char *aed_str, enum vcp_core_id id)
{
	...
	pr_notice("[VCP] %s ends, @%p, size = %x\n", __func__, vcp_dump.ramdump,
		  vcp_dump.ramdump_length);
}
...
void vcp_aed(enum VCP_RESET_TYPE type, enum vcp_core_id id)
{
	...
	aed_common_exception_api("vcp", NULL, 0, NULL, 0, vcp_dump.detail_buff,
				 DB_OPT_DEFAULT);
#endif
...
static ssize_t vcp_A_dump_show(struct file *filep, struct kobject *kobj,
			       ...
			       loff_t offset, size_t size)
{
	...
	if (offset >= 0 && offset < vcp_dump.ramdump_length) {
		...
		if (offset == 0 || size == (vcp_dump.ramdump_length - offset))
			pr_notice(
				"[VCP] %s ramdump cleaned of:0x%llx sz:0x%zx\n",
