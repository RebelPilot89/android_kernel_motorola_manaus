	...
	do {                                                                   \
		...
		if (ret > 0)                                                   \
			aee_kernel_warning_api(__FILE__, __LINE__,             \
					       ...
					       vcp_name,                       \
					       "[VCP] error:" string, ##args); \
		pr_info("[VCP] error:" string, ##args);                        \
	} while (0)
#else
#define vcp_aee_print(string, args...) pr_info("[VCP] error:" string, ##args)
...
static void vcp_enable_dapc(void)
{
	...
	onoff = 1;
	arm_smccc_smc(MTK_SIP_KERNEL_DAPC_MMUP_CONTROL, onoff, 0, 0, 0, 0, 0, 0,
		      &res);
}
...
static void vcp_disable_dapc(void)
{
	...
	arm_smccc_smc(MTK_SIP_TINYSYS_VCP_CONTROL,
		      MTK_TINYSYS_VCP_KERNEL_OP_RESET_SET, 1, 0, 0, 0, 0, 0,
		      &res);
...
void vcp_disable_pm_clk(enum feature_id id)
{
	...
	if (pwclkcnt == 0) {
		...
		pr_info("[VCP][Debug] bus_dbg_out[0x%x]: 0x%x, waitCnt=%u\n",
			VCP_BUS_DEBUG_OUT, readl(VCP_BUS_DEBUG_OUT), waitCnt);
	...
	if (pwclkcnt < 0) {
		for (i = 0; i < NUM_FEATURE_ID; i++)
			pr_info("[VCP][Warning] %s Check feature id %d enable cnt %d\n",
				__func__, feature_table[i].feature,
...
static int vcp_pm_event(struct notifier_block *notifier, unsigned long pm_event,
			void *unused)
{
	int retval = 0;
	uint32_t waitCnt = 0;
	...
	case PM_SUSPEND_PREPARE:
		...
		if ((!is_suspending) && pwclkcnt) {
			...
			retval = pm_runtime_put_sync(
				vcp_io_devs[VCP_IOMMU_256MB1]);
			if (retval)
				pr_debug("[VCP] %s: pm_runtime_put_sync\n",
					 __func__);
			clk_disable_unprepare(vcpsel);
	...
	case PM_POST_SUSPEND:
		...
		if (is_suspending && pwclkcnt) {
			...
			if (retval)
				pr_debug(
					"[VCP] %s: clk_prepare_enable vcp26m\n",
					__func__);
			retval = clk_prepare_enable(vcpclk);
			if (retval)
				pr_debug(
					"[VCP] %s: clk_prepare_enable vcpclk\n",
					__func__);
			retval = clk_prepare_enable(vcpsel);
			if (retval)
				pr_debug(
					"[VCP] %s: clk_prepare_enable vcpsel\n",
					__func__);
...
void vcp_set_clk(void)
{
	...
	for (i = 0; i < clk_retry; i++) {
		ret = clk_set_parent(vcpsel,
				     vcp26m); /* guarantee status sync */
		if (ret)
			pr_notice(
				"[VCP] %s: Failed to set parent %s of %s ret %d\n",
...
int reset_vcp(int reset)
{
	...
	if (vcp_enable[VCP_A_ID]) {
...
#if VCP_BOOT_TIME_OUT_MONITOR
		vcp_ready_timer[VCP_A_ID].tl.expires =
			jiffies + VCP_READY_TIMEOUT;
		...
		pr_notice("[VCP] %s: CORE0_RSTN_CLR %x %x %x ret %lu clk %d\n",
			  __func__, readl(DRAM_RESV_ADDR_REG),
			  readl(DRAM_RESV_SIZE_REG), readl(R_CORE0_SW_RSTN_CLR),
			  res.a0, mt_get_fmeter_freq(vcpreg.femter_ck, CKGEN));
	}
	pr_debug("[VCP] %s: done\n", __func__);
...
vcp_A_status_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
	if (vcp_ready[VCP_A_ID])
					    ...
					    char *buf)
{
	...
	vcp_dump_last_regs(mmup_enable_count());
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_status = %08x\n",
			 c0_m->status);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_pc = %08x\n",
			 c0_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_lr = %08x\n",
			 c0_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_sp = %08x\n",
			 c0_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_pc_latch = %08x\n",
			 c0_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_lr_latch = %08x\n",
			 c0_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_sp_latch = %08x\n",
			 c0_m->sp_latch);
	if (!vcpreg.twohart)
		goto core1;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_pc = %08x\n",
			 c0_t1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_lr = %08x\n",
			 c0_t1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_sp = %08x\n",
			 c0_t1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_pc_latch = %08x\n",
			 c0_t1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_lr_latch = %08x\n",
			 c0_t1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c0_t1_sp_latch = %08x\n",
			 c0_t1_m->sp_latch);
core1:
	if (vcpreg.core_nums == 1)
		goto end;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_status = %08x\n",
			 c1_m->status);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_pc = %08x\n",
			 c1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_lr = %08x\n",
			 c1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_sp = %08x\n",
			 c1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_pc_latch = %08x\n",
			 c1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_lr_latch = %08x\n",
			 c1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_sp_latch = %08x\n",
			 c1_m->sp_latch);
	if (!vcpreg.twohart)
		goto end;
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_pc = %08x\n",
			 c1_t1_m->pc);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_lr = %08x\n",
			 c1_t1_m->lr);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_sp = %08x\n",
			 c1_t1_m->sp);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_pc_latch = %08x\n",
			 c1_t1_m->pc_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_lr_latch = %08x\n",
			 c1_t1_m->lr_latch);
	len += scnprintf(buf + len, PAGE_SIZE - len, "c1_t1_sp_latch = %08x\n",
			 c1_t1_m->sp_latch);
				   ...
				   const char *buf, size_t n)
{
	...
	if (kstrtouint(buf, 10, &value) == 0) {
		...
		vcp_dbg_log = value;
		pr_debug(
			"[VCP] vcp_ee_enable(vcp_dbg_log) = %d(1:enable, 0:disable)\n",
			vcp_ee_enable);
	}
	return n;
}
...
static inline ssize_t
vcp_ipi_test_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
	...
	if (vcp_ready[VCP_A_ID]) {
		...
		return scnprintf(buf, PAGE_SIZE, "VCP A ipi send ret=%d\n",
				 ret);
	} else
		return scnprintf(buf, PAGE_SIZE, "VCP A is not ready\n");
}
...
/******************************************************************************
 *****************************************************************************/
static struct miscdevice vcp_device = { .minor = MISC_DYNAMIC_MINOR,
					.name = "vcp",
...
static int create_files(void)
{
...
#if VCP_RECOVERY_SUPPORT
	ret = device_create_file(vcp_device.this_device, &dev_attr_wdt_reset);
	if (unlikely(ret != 0))
		return ret;

	ret = device_create_file(vcp_device.this_device, &dev_attr_vcp_reset);
	if (unlikely(ret != 0))
		return ret;
	...
	ret = device_create_file(vcp_device.this_device, &dev_attr_log_filter);
	if (unlikely(ret != 0))
		return ret;

	ret = device_create_file(vcp_device.this_device, &dev_attr_vcpctl);
...
RESERVEDMEM_OF_DECLARE(vcp_reserve_mem_init, VCP_MEM_RESERVED_KEY,
		       vcp_reserve_mem_of_init);
...
#define MEMORY_TBL_ELEM_NUM (2)
	...
	if (num != NUMS_MEM_ID) {
		pr_notice(
			"[VCP] number of entries of reserved memory %u / %u\n",
			num, NUMS_MEM_ID);
		BUG_ON(1);
		return -1;
	}
	...
	pr_notice("[VCP] %s is called, 0x%x, 0x%x", __func__,
		  (unsigned int)vcp_mem_base_phys, (unsigned int)vcp_mem_size);
	...
	vcp_mem_num =
		of_property_count_u32_elems(pdev->dev.of_node, "vcp_mem_tbl") /
		MEMORY_TBL_ELEM_NUM;
	if (vcp_mem_num <= 0) {
	...
	if (pdev->dev.dma_parms) {
		ret = dma_set_max_seg_size(&pdev->dev,
	...
	for (id = 0; id < NUMS_MEM_ID; id++) {
		vcp_reserve_mblock[id].start_virt = (__u64)dma_alloc_coherent(
			&pdev->dev, vcp_reserve_mblock[id].size,
			&vcp_reserve_mblock[id].start_phys, GFP_KERNEL);
		...
		if ((vcp_reserve_mblock[id].start_phys +
		     vcp_reserve_mblock[id].size) > iova_upper)
	...
	for (id = 0; id < NUMS_MEM_ID; id++) {
		vcp_reserve_mblock[id].start_phys =
			vcp_mem_base_phys + accumlate_memory_size;
		vcp_reserve_mblock[id].start_virt =
			vcp_mem_base_virt + accumlate_memory_size;
		accumlate_memory_size += vcp_reserve_mblock[id].size;
	...
	for (id = 0; id < NUMS_MEM_ID; id++) {
		...
		pr_notice(
			"[VCP][rsrv_mem-%d] phy:0x%llx - 0x%llx, len:0x%llx\n",
			id, start_phys, start_phys + len - 1, len);
		pr_notice(
			"[VCP][rsrv_mem-%d] vir:0x%llx - 0x%llx, len:0x%llx\n",
...
void set_vcp_mpu(void)
{
	...
	mtk_emimpu_set_apc(&md_region, MPU_DOMAIN_D0, MTK_EMIMPU_NO_PROTECTION);
	mtk_emimpu_set_apc(&md_region, MPU_DOMAIN_D3, MTK_EMIMPU_NO_PROTECTION);
...
void vcp_wait_core_stop_timeout(int mmup_enable)
{
	...
	while (timeout--) {
		...
		core0_halt =
			(val == (B_CORE_GATED | B_HART0_HALT | B_HART1_HALT));
		val = readl(R_CORE1_STATUS);
		core1_halt = (vcpreg.core_nums == 2) ?
				     (val == (B_CORE_GATED | B_HART0_HALT |
					      B_HART1_HALT)) :
				     1;
...
void vcp_sys_reset_ws(struct work_struct *ws)
{
	...
	pr_notice("[VCP] %s(): vcp_reset_type %d remain %x times, en_cnt %d\n",
		  __func__, vcp_reset_type, vcp_reset_counts,
		  mmup_enable_count());
	...
	if (vcp_reset_by_cmd == 0 && vcp_ee_enable) {
		vcp_aee_print(
			"[VCP] %s(): vcp_reset_type %d remain %x times, encnt %d\n",
			__func__, vcp_reset_type, vcp_reset_counts,
			mmup_enable_count());
	}
	...
	if ((vcp_reset_type == RESET_TYPE_TIMEOUT) ||
		...
		/* stop vcp */
		arm_smccc_smc(MTK_SIP_TINYSYS_VCP_CONTROL,
			      MTK_TINYSYS_VCP_KERNEL_OP_RESET_SET, 0, 0, 0, 0,
			      0, 0, &res);

		dsb(SY); /* may take lot of time */
		pr_notice("[VCP] rstn core0 %x core1 %x ret %lu\n",
			  readl(R_CORE0_SW_RSTN_SET),
			  readl(R_CORE1_SW_RSTN_SET), res.a0);
	} else {
		/* reset type vcp WDT or CMD*/
		/* make sure vcp is in idle state */
		vcp_wait_core_stop_timeout(mmup_enable_count());
		arm_smccc_smc(MTK_SIP_TINYSYS_VCP_CONTROL,
			      MTK_TINYSYS_VCP_KERNEL_OP_RESET_SET, 1, 0, 0, 0,
			      0, 0, &res);
	...
	/* start vcp */
	arm_smccc_smc(MTK_SIP_TINYSYS_VCP_CONTROL,
		      MTK_TINYSYS_VCP_KERNEL_OP_RESET_RELEASE, 1, 0, 0, 0, 0, 0,
		      &res);

	pr_notice("[VCP] %s: CORE0_RSTN_CLR %x %x %x ret %lu clk %d\n",
		  __func__, readl(DRAM_RESV_ADDR_REG),
		  readl(DRAM_RESV_SIZE_REG), readl(R_CORE0_SW_RSTN_CLR), res.a0,
...
void vcp_region_info_init(void)
{
	...
	memcpy_from_vcp(&vcp_region_info_copy, vcp_region_info,
...
void vcp_recovery_init(void)
{
	...
	vcp_loader_virt = ioremap_wc(vcp_region_info_copy.ap_loader_start,
	...
	vcp_regdump_virt = ioremap_wc(vcp_region_info_copy.regdump_start,
...
static bool vcp_ipi_table_init(struct mtk_mbox_device *vcp_mboxdev,
			       struct platform_device *pdev)
{
	enum table_item_num { send_item_num = 3, recv_item_num = 4 };
	...
	vcp_mboxdev->send_count =
		of_property_count_u32_elems(pdev->dev.of_node, "send_table") /
		send_item_num;
	...
	vcp_mboxdev->recv_count =
		of_property_count_u32_elems(pdev->dev.of_node, "recv_table") /
		recv_item_num;
	...
	vcp_mboxdev->info_table =
		vzalloc(sizeof(struct mtk_mbox_info) * vcp_mboxdev->count);
	...
	vcp_mboxdev->pin_send_table = vzalloc(sizeof(struct mtk_mbox_pin_send) *
					      vcp_mboxdev->send_count);
	...
	for (i = 0; i < vcp_mboxdev->send_count; ++i) {
		...
		ret = of_property_read_u32_index(
			pdev->dev.of_node, "send_table", i * send_item_num + 2,
	...
	vcp_mboxdev->pin_recv_table = vzalloc(sizeof(struct mtk_mbox_pin_recv) *
					      vcp_mboxdev->recv_count);
	...
	for (i = 0; i < vcp_mboxdev->recv_count; ++i) {
		...
		ret = of_property_read_u32_index(
			pdev->dev.of_node, "recv_table", i * recv_item_num + 2,
...
static int vcp_io_device_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "vcp-support", &vcp_support);
...
static int vcp_device_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "core_nums", &vcpreg.core_nums);
	if (!vcpreg.core_nums) {
	...
	of_property_read_u32(pdev->dev.of_node, "femter_ck", &vcpreg.femter_ck);
	of_property_read_u32(pdev->dev.of_node, "twohart", &vcpreg.twohart);
	...
	ret = of_property_read_string_index(pdev->dev.of_node, "clock-names", 0,
					    &clk_name);
	...
	ret = of_property_read_string_index(pdev->dev.of_node, "clock-names", 1,
					    &clk_name);
	...
	ret = of_property_read_string_index(pdev->dev.of_node, "clock-names", 2,
					    &clk_name);
...
static int __init vcp_init(void)
{
	...
	mtk_ipi_register(&vcp_ipidev, IPI_IN_VCP_READY_0,
			 (void *)vcp_A_ready_ipi_handler, NULL,
			 &msg_vcp_ready0);
#endif
