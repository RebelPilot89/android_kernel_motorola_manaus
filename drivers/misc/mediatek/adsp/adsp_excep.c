...
static u32 dump_adsp_internal_mem(struct adsp_priv *pdata, void *buf,
				  size_t size)
{
	u32 n = 0;
	...
	n += copy_from_buffer(buf + n, size - n, adspsys->cfg,
			      adspsys->cfg_size, 0, -1);
	n += copy_from_buffer(buf + n, size - n, adspsys->cfg2,
			      adspsys->cfg2_size, 0, -1);
	n += copy_from_buffer(buf + n, size - n, pdata->itcm, pdata->itcm_size,
			      0, -1);
	n += copy_from_buffer(buf + n, size - n, pdata->dtcm, pdata->dtcm_size,
			      0, -1);
...
static int dump_buffer(struct adsp_exception_control *ctrl, int coredump_id)
{
	...
	total = adspsys->cfg_size + adspsys->cfg2_size + pdata->itcm_size +
		pdata->dtcm_size + pdata->sysram_size +
	...
	n += copy_from_buffer(buf + n, total - n, pdata->sysram,
			      pdata->sysram_size, 0, -1);
	...
	pr_debug("%s, vmalloc size %u, buffer %p, dump_size %u", __func__,
		 total, buf, n);
	return n;
}
...
void get_adsp_misc_buffer(unsigned long *vaddr, unsigned long *size)
{
	...
	for (id = 0; id < get_adsp_core_total(); id++) {
		...
		if (!pdata || !pdata->log_ctrl || !pdata->log_ctrl->inited ||
		    !pdata->log_ctrl->priv)
			goto ERROR;
...
{
	struct adsp_exception_control *ctrl =
		container_of(ws, struct adsp_exception_control, aed_work);
	struct adsp_priv *pdata = NULL;
	...
	if (ret) {
		...
		if (is_infrabus_timeout())
			...
			aee_kernel_exception_api(
				__FILE__, __LINE__, DB_OPT_DEFAULT, "[ADSP]",
...
void adsp_wdt_handler(int irq, void *data, int cid)
{
	...
	if (!adsp_aed_dispatch(EXCEP_RUNTIME, data))
		pr_info("%s, already resetting, ignore core%d wdt", __func__,
			pdata->id);
}
...
void get_adsp_aee_buffer(unsigned long *vaddr, unsigned long *size)
{
	...
	if (pdata) {
		n += copy_from_buffer(buf + n, len - n, adspsys->cfg,
				      adspsys->cfg_size, 0, -1);
		n += copy_from_buffer(buf + n, len - n, adspsys->cfg2,
				      adspsys->cfg2_size, 0, -1);
		n += copy_from_buffer(buf + n, len - n, pdata->dtcm,
				      pdata->dtcm_size, 0, -1);
	}
			      ...
			      loff_t offset, size_t size)
{
	...
	mutex_lock(&ctrl->lock);
	if (ctrl->buf_backup) {
		n = copy_from_buffer(buf, -1, ctrl->buf_backup, ctrl->buf_size,
				     offset, size);
				 ...
				 loff_t offset, size_t size)
{
	...
	threshold[2] =
		threshold[1] + adsp_get_reserve_mem_size(ADSP_B_LOGGER_MEM_ID);

	if (offset >= 0 && offset < threshold[0]) {
		n = copy_from_buffer(buf, -1, adsp_ke_buffer, threshold[0],
				     offset, size);
		...
		n = copy_from_adsp_shared_memory(buf, offset - threshold[1],
				  ...
				  loff_t offset, size_t size)
{
	...
	threshold[0] = adsp_get_reserve_mem_size(ADSP_A_LOGGER_MEM_ID);
	threshold[1] =
		threshold[0] + adsp_get_reserve_mem_size(ADSP_B_LOGGER_MEM_ID);
