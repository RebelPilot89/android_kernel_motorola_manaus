...
static void slbc_scmi_handler(u32 r_feature_id, scmi_tinysys_report *report)
{
	...
	case IPI_SLBC_ACP_REQUEST_TO_AP:
		...
		if (d.type == TP_ACP) {
			...
			pr_info("#@# %s(%d) wrong cmd(%s) and type(0x%x)\n",
				__func__, __LINE__,
				"IPI_SLBC_ACP_REQUEST_TO_AP", d.type);
		break;
	case IPI_SLBC_ACP_RELEASE_TO_AP:
		...
		if (d.type == TP_ACP) {
			...
			pr_info("#@# %s(%d) wrong cmd(%s) and type(0x%x)\n",
				__func__, __LINE__,
				"IPI_SLBC_ACP_RELEASE_TO_AP", d.type);
		break;
...
int slbc_scmi_set(void *buffer, int slot)
{
	...
	ret = scmi_tinysys_common_set(_tinfo->ph, scmi_slbc_id, slbc_ipi_d->cmd,
				      slbc_ipi_d->arg, 0, 0, 0);
...
int slbc_scmi_get(void *buffer, int slot, void *ptr)
{
	...
	ret = scmi_tinysys_common_get(_tinfo->ph, scmi_slbc_id, slbc_ipi_d->cmd,
				      rvalue);
