...
module_platform_driver(usb_boost_driver);
...
int audio_freq_hold(void)
{
	...
	if (of_device_is_compatible(np, "mediatek,mt6983-usb_boost") ||
		...
		device_property_read_u32(gdev, "small-core",
					 &(cpu_freq_audio[0]));
		device_property_read_u32(gdev, "medium-core",
					 &(cpu_freq_audio[1]));
		device_property_read_u32(gdev, "big-core",
					 &(cpu_freq_audio[2]));

		USB_BOOST_NOTICE("%s: request cpu freq(%d) (%d) (%d)\n",
				 __func__, cpu_freq_audio[0], cpu_freq_audio[1],
				 cpu_freq_audio[2]);
	...
	if (of_device_is_compatible(np, "mediatek,mt6855-usb_boost") ||
		...
		list_for_each_entry (req_policy, &usb_policy_list, list) {
			...
			if (!ret)
				USB_BOOST_NOTICE(
					"%s: fail to update freq constraint (policy:%d)\n",
					__func__, req_policy->policy->cpu);

			if (req_policy->policy->cpu == 6 &&
			    cpu_freq_audio[1] > 0)
				ret = freq_qos_update_request(
					&req_policy->qos_req,
					cpu_freq_audio[1]);
