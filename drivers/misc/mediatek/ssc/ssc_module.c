...
#else
#define ssc_aee_print(string, args...) pr_info("[SSC] error:" string, ##args)
#endif
...
static int ssc_vlogic_bound_event(struct notifier_block *notifier,
...
static int ssc_vlogic_bound_call_chain(unsigned int val,
				       unsigned int request_id)
{
	int ret;

	ret = blocking_notifier_call_chain(&vlogic_bound_chain, val,
					   &request_id);
...
int ssc_enable_vlogic_bound(int request_id)
{
	...
	if (atomic_inc_return(&vlogic_bound_counter) == 1) {
		return ssc_vlogic_bound_call_chain(SSC_ENABLE_VLOGIC_BOUND,
						   request_id);
	}
...
int ssc_disable_vlogic_bound(int request_id)
{
	...
	if (atomic_dec_return(&vlogic_bound_counter) == 0) {
		return ssc_vlogic_bound_call_chain(SSC_DISABLE_VLOGIC_BOUND,
						   request_id);
	}
...
static void ssc_notification_handler(u32 feature_id,
				     scmi_tinysys_report *report)
{
#define LOG_BUF_SIZE 30
	...
	if (report->p1 == SSC_STATUS_ERR) {
		...
		for (i = 1; i < SSC_TIMEOUT_NUM; i++) {
			timeout_mask_shift =
				(i - 1) * 2 + SSC_TIMEOUT_MASK_SHIFT;
			if ((timeout & (0x1U << i)) &&
			    ((timeout & (0x3 << timeout_mask_shift)) == 0x0)) {
				log_size +=
					scnprintf(log_buf + log_size,
...
static int __init ssc_init(void)
{
	...
	if (ssc_node) {
		ret = of_property_read_u32(ssc_node, MTK_SSC_SAFE_VLOGIC_STRING,
					   &safe_vlogic_uV);
		...
		ret = of_property_read_u32(ssc_node, "ssc_disable",
					   &ssc_disable);
	...
	if (ssc_node) {
		ret = of_property_read_u32(ssc_node, "gpueb-support",
					   &gpueb_enable);

		pr_info("[SSC] gpueb enable = 0x%x\n", gpueb_enable);

		if (gpueb_enable == 0)
			gpufreq_set_limit(TARGET_DEFAULT, LIMIT_SRAMRC,
					  GPUPPM_KEEP_IDX, 60000);
	...
	ret = of_property_read_u32(tinfo->sdev->dev.of_node, "scmi_ssc",
				   &ssc_scmi_feature_id);

	pr_info("[SSC] ssc scmi id = %d\n", ssc_scmi_feature_id);

	scmi_tinysys_register_event_notifier(
		ssc_scmi_feature_id, (f_handler_t)ssc_notification_handler);

	ret = scmi_tinysys_event_notify(ssc_scmi_feature_id, 1);
	if (ret < 0)
		pr_info("[SSC] SCMI notify register fail\n");

	ret = of_property_read_u32(tinfo->sdev->dev.of_node, "scmi_plt",
				   &plt_scmi_feature_id);
	ret = scmi_tinysys_common_set(tinfo->ph, plt_scmi_feature_id,
				      PLT_SSC_INIT, 0, 0, 0, 0);
