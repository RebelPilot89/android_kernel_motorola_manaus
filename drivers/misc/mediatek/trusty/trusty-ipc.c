...
static int vds_add_channel(struct tipc_virtio_dev *vds, struct tipc_chan *chan)
{
	...
	mutex_lock(&vds->lock);
	if (vds->state == VDS_ONLINE) {
		ret = idr_alloc(&vds->addr_idr, chan, TIPC_MIN_LOCAL_ADDR,
				TIPC_ANY_ADDR - 1, GFP_KERNEL);
		if (ret > 0) {
			chan->local = ret;
...
int ise_chan_shutdown(struct tipc_chan *chan)
{
	...
	if (chan->state == TIPC_CONNECTED || chan->state == TIPC_CONNECTING) {
		...
		ISE_FOOTPRINT("srv='%s', ree=%d, ise=%d (%d)(DISC_REQ)\n",
			      chan->srv_name, chan->local, chan->remote,
			      chan->state);
...
static int dn_wait_for_reply(struct tipc_dn_chan *dn, int timeout)
{
	...
	while (max_retry > 0) {
		...
		ret = wait_for_completion_interruptible_timeout(
			&dn->reply_comp, msecs_to_jiffies(REPLY_RETRY_TIMEOUT));
		if (ret != 0)
			break;
		ise_check_vqs_call();
		max_retry--;
...
{
	return ise_fast_call32(dev->parent, SMC_FC_ELF_PA_INFO, (u32)elf_paddr,
			       (u32)(elf_paddr >> 32), (u32)elf_size);
}
			    ...
			    const struct tipc_send_msg_req __user *arg)
{
	...
	if (shm_vaddr == NULL) {
		shm_vaddr =
			trusty_shm_alloc(MAX_ELF_SZ, GFP_KERNEL | __GFP_ZERO);
		if (!shm_vaddr) {
			ret = -ENOMEM;
...
static long tipc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case TIPC_IOC_SEND_MSG:
		return filp_send_ioctl(
			filp, (const struct tipc_send_msg_req __user *)arg);
	default:
...
static ssize_t tipc_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	while (list_empty(&dn->rx_msg_queue)) {
		...
		mutex_lock(&dn->timer_lock);
		if (!dn->event_status)
			mod_timer(&dn->tipc_timer,
				  jiffies + msecs_to_jiffies(TIPC_TIMEOUT));
		mutex_unlock(&dn->timer_lock);

		ISE_FOOTPRINT("wait for read!\n");
		dn->event_status =
			wait_event_interruptible(dn->readq, _got_rx(dn));
		if (dn->event_status)
			return -ERESTARTSYS;
			     ...
			     struct tipc_conn_rsp_body *rsp, size_t len)
{
	...
	if (chan) {
		...
		if (chan->state == TIPC_CONNECTING) {
			if (!rsp->status) {
				ISE_FOOTPRINT(
					"srv='%s', ree=%d, ise=%d (%d)(CONN_RSP)\n",
					chan->srv_name, chan->local,
					rsp->remote, chan->state);
			     ...
			     struct tipc_disc_req_body *req, size_t len)
{
	...
	if (chan) {
		...
		pr_info("%s:%d srv='%s', ree=%d, ise=%d (%d)\n", __func__,
			__LINE__, chan->srv_name, chan->local, chan->remote,
			chan->state);
...
static ssize_t ise_read(struct file *file, char __user *buffer, size_t count,
			loff_t *ppos)
{
	char msg_buf[1024] = { 0 };
	char *p = msg_buf;
