				  ...
				  unsigned long util, unsigned long max)
{
	...
	mtk_map_util_freq((void *)sg_policy, util, freq, policy->related_cpus,
			  &next_freq);
	if (next_freq) {
		freq = next_freq;
	} else {
 ...
 */
unsigned long mtk_cpu_util(int cpu, unsigned long util_cfs, unsigned long max,
...
DEFINE_PER_CPU(spinlock_t, cpufreq_idle_cpu_lock) =
	__SPIN_LOCK_UNLOCKED(cpufreq_idle_cpu_lock);
...
static unsigned long sugov_get_util(struct sugov_cpu *sg_cpu)
{
	...
	if (per_cpu(pre_rt_throttled, sg_cpu->cpu) != rq->rt.rt_throttled) {
		...
		per_cpu(cpufreq_idle_cpu, sg_cpu->cpu) =
			(rq->nr_running) ? 0 : 1;
	}
...
static void update_min_rate_limit_ns(struct sugov_policy *sg_policy)
{
	...
	WRITE_ONCE(sg_policy->min_rate_limit_ns,
