...
static void task_rotate_work_func(struct work_struct *work)
{
	struct task_rotate_work *wr =
		container_of(work, struct task_rotate_work, w);

	int ret = -1;
	struct rq *src_rq, *dst_rq;

	ret = migrate_swap(wr->src_task, wr->dst_task, task_cpu(wr->dst_task),
			   task_cpu(wr->src_task));

	if (ret == 0) {
		trace_sched_big_task_rotation(wr->src_cpu, wr->dst_cpu,
					      wr->src_task->pid,
					      wr->dst_task->pid, true);
	}
...
#if IS_ENABLED(CONFIG_ANDROID_VENDOR_OEM_DATA)
...
void rotat_task_newtask(void __always_unused *data, struct task_struct *p,
