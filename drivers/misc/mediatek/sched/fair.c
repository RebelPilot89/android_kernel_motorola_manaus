 ...
 * }
 */
static unsigned long mtk_cpu_util_next(int cpu, struct task_struct *p,
				       int dst_cpu, unsigned long util_freq,
 ...
 */
static unsigned long mtk_compute_energy(struct task_struct *p, int dst_cpu,
					struct perf_domain *pd)
{
	...
	unsigned long util_cfs_base, util_cfs_cur, util_cfs_energy_base,
		util_cfs_energy_cur;
	...
	for_each_cpu_and (cpu, pd_mask, cpu_online_mask) {
		...
		util_cfs_base = mtk_cpu_util_next(
			cpu, p, -1, util_freq, util_est, &util_cfs_energy_base);
		/*
		...
		cpu_energy_util = mtk_cpu_util(cpu, util_cfs_energy_base,
					       cpu_cap, ENERGY_UTIL, NULL);
#else
		cpu_energy_util = schedutil_cpu_util(
			cpu, util_cfs_energy_base, cpu_cap, ENERGY_UTIL, NULL);
#endif
		...
		if (cpu == dst_cpu) {
			util_cfs_cur = mtk_cpu_util_next(cpu, p, dst_cpu,
							 util_freq, util_est,
							 &util_cfs_energy_cur);
			/*
			 * Busy time computation: utilization clamping is not
...
#if IS_ENABLED(CONFIG_MTK_CPUFREQ_SUGOV_EXT)
			sum_util_cur +=
				mtk_cpu_util(cpu, util_cfs_energy_cur, cpu_cap,
					     ENERGY_UTIL, NULL);
#else
			sum_util_cur +=
				schedutil_cpu_util(cpu, util_cfs_energy_cur,
						   cpu_cap, ENERGY_UTIL, NULL);
#endif
			/*
			 * Performance domain frequency: utilization clamping
...
#else
			cpu_util_cur =
				schedutil_cpu_util(cpu, util_cfs_cur, cpu_cap,
						   FREQUENCY_UTIL, tsk);
#endif
		} else {
		...
		trace_sched_energy_util(-1, max_util_base, sum_util_base, cpu,
					util_cfs_base, util_cfs_energy_base,
					cpu_util_base);
		trace_sched_energy_util(dst_cpu, max_util_cur, sum_util_cur,
					cpu, util_cfs_cur, util_cfs_energy_cur,
					cpu_util_cur);
	...
	trace_sched_compute_energy(-1, pd_mask, energy_base, max_util_base,
				   sum_util_base);
...
void mtk_find_energy_efficient_cpu(void *data, struct task_struct *p,
				   int prev_cpu, int sync, int *new_cpu)
{
	...
	if (!uclamp_min_ls)
		...
		latency_sensitive =
			(p->uclamp_req[UCLAMP_MIN].value > 0 ? 1 : 0) ||
	...
	for (; pd; pd = pd->next) {
...
#if IS_ENABLED(CONFIG_MTK_THERMAL_AWARE_SCHEDULING)
				if (idle &&
				    idle->exit_latency >= min_exit_lat &&
				    cpu_cap == target_cap)
					continue;
#else
		...
		if (!latency_sensitive && max_spare_cap_cpu >= 0) {
			cur_delta =
				mtk_compute_energy(p, max_spare_cap_cpu, pd);
		...
		if (latency_sensitive) {
			if (max_spare_cap_cpu_ls_idle >= 0) {
				cur_delta = mtk_compute_energy(
					p, max_spare_cap_cpu_ls_idle, pd);
				if (cur_delta <= best_delta) {
					best_delta = cur_delta;
					best_idle_cpu =
...
fail:
	...
	*new_cpu = -1;
	...
	trace_sched_find_energy_efficient_cpu(best_delta, best_energy_cpu,
					      best_idle_cpu,
					      idle_max_spare_cap_cpu,
					      sys_max_spare_cap_cpu);
...
static struct task_struct *detach_a_hint_task(struct rq *src_rq, int dst_cpu)
{
	...
	list_for_each_entry_reverse (p, &src_rq->cfs_tasks, se.group_node) {
		...
		if (latency_sensitive && task_util <= dst_capacity) {
			best_task = p;
			break;
...
static int mtk_active_load_balance_cpu_stop(void *data)
{
	...
	if (task_cpu(target_task) != busiest_cpu ||
	    ...
	    task_running(busiest_rq, target_task) || target_rq == busiest_rq)
		goto out_unlock;
...
int migrate_running_task(int this_cpu, struct task_struct *p, struct rq *target,
			 int reason)
{
	...
	if (active_balance) {
		...
		stop_one_cpu_nowait(cpu_of(target),
				    mtk_active_load_balance_cpu_stop, p,
				    &target->active_balance_work);
	}
...
void mtk_sched_newidle_balance(void *data, struct rq *this_rq,
			       struct rq_flags *rf, int *pulled_task, int *done)
{
	...
	trace_sched_next_new_balance(
		now_ns, per_cpu(next_update_new_balance_time_ns, this_cpu));
	...
	for_each_cpu (cpu, cpu_active_mask) {
		...
		if (p) {
			trace_sched_force_migrate(p, this_cpu,
						  MIGR_IDLE_BALANCE);
