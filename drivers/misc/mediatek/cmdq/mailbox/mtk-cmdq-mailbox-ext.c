...
void cmdq_dump_usage(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		cmdq_msg(
			...
			__func__, usage[0], usage[1], usage[2], usage[3],
			usage[4], usage[5], usage[6], usage[7], usage[8],
			usage[9], usage[10], usage[11], usage[12], usage[13],
			usage[14], usage[15], usage[16], usage[17], usage[18],
			usage[19], usage[20], usage[21], usage[22], usage[23]);
	}
}
...
static void cmdq_clk_disable(struct cmdq *cmdq)
{
	...
	if (usage == -1)
		...
		cmdq_err("ref count error after dec:%d suspend:%s", usage,
...
dma_addr_t cmdq_thread_get_pc(struct cmdq_thread *thread)
{
	...
	return CMDQ_REG_REVERT_ADDR(
		(dma_addr_t)readl(thread->base + CMDQ_THR_CURR_ADDR));
}
...
void cmdq_thread_set_spr(struct mbox_chan *chan, u8 id, u32 val)
{
	if (id >= CMDQ_GPR_CNT_ID) {
		struct cmdq *cmdq =
			container_of(chan->mbox, typeof(*cmdq), mbox);

		writel(val, cmdq->base + GCE_GPR_R0_START +
				    (id - CMDQ_GPR_CNT_ID) * 4);
	} else {
		struct cmdq_thread *thread =
			(struct cmdq_thread *)chan->con_priv;
...
static int cmdq_thread_suspend(struct cmdq *cmdq, struct cmdq_thread *thread)
{
	...
	if (readl_poll_timeout_atomic(
		    thread->base + CMDQ_THR_CURR_STATUS, status,
		    status & CMDQ_THR_STATUS_SUSPENDED, 0, 100)) {
		cmdq_err("suspend GCE %hu thread %u failed", cmdq->hwid,
			 thread->idx);
		if (thread->chan)
...
int cmdq_thread_reset(struct cmdq *cmdq, struct cmdq_thread *thread)
{
	...
	if (readl_poll_timeout_atomic(
		    thread->base + CMDQ_THR_WARM_RESET, warm_reset,
		    !(warm_reset & CMDQ_THR_DO_WARM_RESET), 0, 10)) {
	...
	*insert_pos = &prev_task->list_entry;
	...
	if (next_task && next_task != prev_task) {
		cmdq_msg("reorder pkt:0x%p(%u) next pkt:0x%p(%u) pc:%pa",
			 task->pkt, task->pkt->priority, next_task->pkt,
			 next_task->pkt->priority, &curr_pa);
...
void cmdq_init_cmds(void *dev_cmdq)
{
	...
	if (readl_poll_timeout_atomic(thread->base + CMDQ_THR_CURR_ADDR, pc,
				      pc == end, 0, 100)) {
		cmdq_err("clear event instructions timeout pc:%#lx end:%#lx",
			 (unsigned long)pc, (unsigned long)end);
		writel(CMDQ_THR_SUSPEND, thread->base + CMDQ_THR_SUSPEND_TASK);
		if (readl_poll_timeout_atomic(
			    thread->base + CMDQ_THR_CURR_STATUS, status,
			    status & CMDQ_THR_STATUS_SUSPENDED, 0, 1000)) {
...
static void cmdq_task_exec(struct cmdq_pkt *pkt, struct cmdq_thread *thread)
{
	...
	if (list_empty(&thread->task_busy_list)) {
		if (mminfra_power_cb && !mminfra_power_cb()) {
			cmdq_err(
				"task running when mminfra power off,cmdq:%pa id:%u usage:%d",
				&cmdq->base_pa, cmdq->hwid,
				atomic_read(&cmdq->usage));
			dump_stack();
		}
		...
		cmdq_log(
			"task pa:%pa iova:%pa size:%zu thread->base=0x%p thread:%u",
		...
		cmdq_log("set pc:%pa end:%pa pkt:0x%p", &task->pa_base, &end_pa,
			 pkt);
		...
		if (curr_pa == end_pa - CMDQ_INST_SIZE || curr_pa == end_pa) {
			...
			cmdq_log("set pc:%pa pkt:0x%p", &task->pa_base,
				 task->pkt);
		} else {
		...
		cmdq_log("set end:%pa pkt:0x%p", &pkt_end_pa, last_task->pkt);
				   ...
				   dma_addr_t pa_curr)
{
	...
	cmdq_util_user_err(
		...
		task->pkt->cmd_buf_size, &pa_curr, inst ? *inst : -1);
}
				    ...
				    struct list_head *removes)
{
	...
	cmdq_log("irq flag:%#x gce:%lx idx:%u", irq_flag,
		 (unsigned long)cmdq->base_pa, thread->idx);
	...
	cmdq_log("task status %pa~%pa err:%d", &curr_pa, &task_end_pa, err);
#if IS_ENABLED(CONFIG_MTK_IRQ_MONITOR_DEBUG)
	...
	if (thread->dirty) {
		cmdq_log("task in error dump thread:%u pkt:0x%p", thread->idx,
			 task ? task->pkt : NULL);
		return;
	}
	...
	list_for_each_entry_safe (task, tmp, &thread->task_busy_list,
		...
		if (task->end_time)
			cmdq_util_err(
				"thd:%d pkt:%p is done,start:%llu end:%llu",
				thread->idx, task->pkt, task->exec_time,
				task->end_time);
		...
		if (!curr_task || curr_pa == task_end_pa - CMDQ_INST_SIZE) {
			...
			cmdq_err("pkt:0x%p thread:%u err:%d", curr_task->pkt,
				 thread->idx, err);
	...
	if (!task) {
		...
		thread->timer_mod = sched_clock();
		cmdq_log("mod_timer pkt:0x%p timeout:%u thread:%u", task->pkt,
			 thread->timeout_ms, thread->idx);
	}
#if IS_ENABLED(CONFIG_MTK_IRQ_MONITOR_DEBUG)
	end[end_cnt] = sched_clock();
	if (end[end_cnt] - start >= 1000000 && !time) /* 1ms */
		cmdq_util_err(
			...
			end[end_cnt] - start, end[0] - start, end[1] - end[0],
...
static irqreturn_t cmdq_irq_handler(int irq, void *dev)
{
	...
	if (atomic_read(&cmdq->usage) <= 0) {
		cmdq_err("%s irq:%d cmdq:%pa suspend:%d usage:%d", __func__,
			 irq, &cmdq->base_pa, cmdq->suspended,
			 atomic_read(&cmdq->usage));
	...
	cmdq_log("gce:%lx irq: %#x, %#x", (unsigned long)cmdq->base_pa,
		 (u32)irq_status, (u32)(irq_status ^ CMDQ_IRQ_MASK));
	if (!(irq_status ^ CMDQ_IRQ_MASK)) {
		cmdq_msg("not handle for empty status:0x%x", (u32)irq_status);
		return IRQ_NONE;
	}
#if IS_ENABLED(CONFIG_MTK_IRQ_MONITOR_DEBUG)
	end[end_cnt++] = sched_clock();
	...
	while (!kthread_should_stop()) {
		...
		if (irq & BIT(gce_thread_nr)) {
			...
			list_for_each_entry_safe (task, tmp, &cmdq->irq_removes,
				...
				spin_unlock_irqrestore(&cmdq->irq_removes_lock,
						       flags);
				kfree(task);
				spin_lock_irqsave(&cmdq->irq_removes_lock,
						  flags);
			}
			spin_unlock_irqrestore(&cmdq->irq_removes_lock, flags);
		...
		cmdq_util_dump_smi();
		cmdq_util_aee("CMDQ", "%s: hwid:%hu irq:%#lx", __func__,
			      cmdq->hwid, irq);
	}
...
{
	struct cmdq_thread *thread =
		container_of(work_item, struct cmdq_thread, timeout_work);
	struct cmdq *cmdq = container_of(thread->chan->mbox, struct cmdq, mbox);
	...
	if (task) {
		mod_timer(&thread->timeout,
			  jiffies + msecs_to_jiffies(thread->timeout_ms));
		thread->timer_mod = sched_clock();
		cmdq_thread_err_reset(cmdq, thread, task->pa_base,
				      thread->priority);
		cmdq_thread_resume(thread);
	} else {
		cmdq_thread_resume(thread);
...
void cmdq_thread_dump_spr(struct cmdq_thread *thread)
{
	...
	cmdq_util_user_msg(thread->chan,
			   ...
			   thread->idx, spr[0], spr[1], spr[2], spr[3], dbg[0],
			   dbg[1]);
		      ...
		      u64 **inst_out, dma_addr_t *pc_out)
{
	...
	u32 warn_rst, en, suspend, status, irq, irq_en, cnt, wait_token, cfg,
		prefetch, pri = 0;
	size_t size = 0;
	...
	if (atomic_read(&cmdq->usage) <= 0) {
		cmdq_err("%s gce off cmdq:%p thread:%u", __func__, cmdq,
			 thread->idx);
		dump_stack();
	...
	list_for_each_entry (task, &thread->task_busy_list, list_entry) {
		...
		if (curr_pa == cmdq_task_get_end_pa(task->pkt))
			...
			curr_va =
				(u64 *)cmdq_task_current_va(curr_pa, task->pkt);
		if (!curr_va)
			continue;
		inst = *curr_va;
		pkt = task->pkt;
	...
	if (pkt) {
		cmdq_util_user_msg(
			chan,
			"cur pkt:0x%p size:%zu va:0x%p pa:%pa priority:%u", pkt,
			size, va_base, &pa_base, pri);
...
void cmdq_mbox_dump_dbg(void *mbox_cmdq, void *chan, const bool lock)
{
	...
	if (chan)
		cmdq_util_user_msg(
			...
			id, dbg0[0], dbg0[1], dbg0[2], dbg2[0], dbg2[1],
			dbg2[2], dbg2[3], dbg2[4], dbg2[5], dbg3);
	else
		cmdq_util_msg(
			"id:%u dbg0:%#x %#x %#x dbg2:%#x %#x %#x %#x %#x %#x dbg3:%#x",
			id, dbg0[0], dbg0[1], dbg0[2], dbg2[0], dbg2[1],
			  ...
			  const bool dump_prev)
{
	...
	cmdq_util_msg("%s: cmdq:%pa hwid:%hu usage:%d", __func__,
		      &cmdq->base_pa, cmdq->hwid, usage);
	...
	for (i = 0; i < ARRAY_SIZE(cmdq->thread); i++) {
		...
		if (list_empty(&thread->task_busy_list) ||
			...
			if (lock)
				spin_unlock_irqrestore(&thread->chan->lock,
						       flags);
			continue;
		}
		...
		cmdq_util_msg("thd idx:%u pc:%pa end:%pa", thread->idx,
			      &curr_pa, &end_pa);
		cmdq_thread_dump_spr(thread);
...
void cmdq_mbox_thread_remove_task(struct mbox_chan *chan, struct cmdq_pkt *pkt)
{
	struct cmdq_thread *thread = (struct cmdq_thread *)chan->con_priv;
	struct cmdq *cmdq = container_of(thread->chan->mbox, struct cmdq, mbox);
	struct cmdq_task *task, *tmp;
	...
	if (curr_task) {
		task = list_first_entry(&thread->task_busy_list, typeof(*task),
					list_entry);
	...
	if (last_task) {
		...
		task = list_last_entry(&thread->task_busy_list, typeof(*task),
				       list_entry);
...
static s32 cmdq_config_prefetch(struct device_node *np, struct cmdq *cmdq)
{
	...
	if (prefetch_cnt > ARRAY_SIZE(prefetchs)) {
		cmdq_err("prefetch count more than expect:%u", prefetch_cnt);
		prefetch_cnt = ARRAY_SIZE(prefetchs);
	}

	err = of_property_read_u32_array(np, "prefetch_size", prefetchs,
					 prefetch_cnt);
	if (err != 0) {
		/* print log but do notify error hw setting */
		cmdq_err("read prefetch count:%u size error:%d", prefetch_cnt,
			 err);
		return -EINVAL;
	}
...
static void cmdq_config_dma_mask(struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node, "dma_mask_bit", &dma_mask_bit);
...
static void cmdq_config_default_token(struct device *dev, struct cmdq *cmdq)
{
	...
	cmdq->tokens =
		devm_kcalloc(dev, count, sizeof(*cmdq->tokens), GFP_KERNEL);
	ret = of_property_read_u16_array(dev->of_node, "default_tokens",
					 cmdq->tokens, count);
	if (ret < 0) {
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	dev_notice(dev,
		   ...
		   (unsigned long)cmdq->base, (unsigned long)cmdq->base_pa);
	...
	cmdq->buf_dump_wq = alloc_ordered_workqueue(
		"%s", WQ_MEM_RECLAIM | WQ_HIGHPRI, "cmdq_buf_dump");
...
static const struct gce_plat gce_plat_v5 = { .thread_nr = 32,
					     .shift = 3,
					     .mminfra = BIT(30) };
...
static struct platform_driver cmdq_drv = { .probe = cmdq_probe,
					   .remove = cmdq_remove,
					   .driver = {
...
s32 cmdq_mbox_disable(void *chan)
{
	...
	if (mbox_usage < 0)
		cmdq_util_aee("CMDQ", "hwid:%hu idx:%d usage:d", cmdq->hwid, i,
			      mbox_usage);
	cmdq_clk_disable(cmdq);
	...
	if ((mbox_usage == 0) && (atomic_read(&cmdq->usage) > 0))
		cmdq_err(
			"mbox_disable when task running,cmdq:%pa id:%u usage:%d",
			&cmdq->base_pa, cmdq->hwid, atomic_read(&cmdq->usage));
	if (mminfra_power_cb && !mminfra_power_cb()) {
		cmdq_err(
			"mminfra power off when task running,cmdq:%pa id:%u usage:%d",
			&cmdq->base_pa, cmdq->hwid, atomic_read(&cmdq->usage));
		dump_stack();
	}
...
phys_addr_t cmdq_mbox_get_dummy_reg(void *chan)
{
	...
	return cmdq->thread[22].gce_pa + CMDQ_THR_BASE + CMDQ_THR_SIZE * 22 +
...
phys_addr_t cmdq_mbox_get_spr_pa(void *chan, u8 spr)
{
	...
	return thread->gce_pa + CMDQ_THR_BASE +
	       CMDQ_THR_SIZE * ((u64)thread->idx) + CMDQ_THR_SPR + 4 * spr;
}
...
s32 cmdq_task_get_task_info_from_thread_unlock(struct mbox_chan *chan,
					       struct list_head *task_list_out,
					       u32 *task_num_out)
{
	struct cmdq_thread *thread;
	struct cmdq_task *task;
...
s32 cmdq_task_get_pkt_from_thread(struct mbox_chan *chan,
				  struct cmdq_pkt **pkt_list_out,
				  u32 pkt_list_size, u32 *pkt_count_out)
{
	struct cmdq_thread *thread;
	struct cmdq_task *task;
	u32 pkt_num = 0;
...
void cmdq_event_verify(void *chan, u16 event_id)
{
	...
	cmdq_msg("chan:%lx cmdq:%lx event:%u", (unsigned long)chan,
		 (unsigned long)cmdq, event_id);
