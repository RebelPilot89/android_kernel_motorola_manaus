				 ...
				 size_t count, loff_t *f_pos)
{
	...
	if ((void *)v == (void *)&cfg.pause) {
		...
		pmsr_scmi_data.action = (cfg.pause) ? PMSR_TOOL_ACT_PAUSE :
						      PMSR_TOOL_ACT_RESUME;

		ret = scmi_tinysys_common_set(tinfo->ph, scmi_apmcupm_id,
					      pmsr_scmi_data.uid,
					      pmsr_scmi_data.action, 0, 0, 0);
#elif IS_ENABLED(CONFIG_MTK_TINYSYS_SSPM_V2)
		pmsr_ipi_data.uid = 0x0;
		pmsr_ipi_data.action = (cfg.pause) ? PMSR_TOOL_ACT_PAUSE :
						     PMSR_TOOL_ACT_RESUME;
		ret = mtk_ipi_send_compl(&sspm_ipidev, IPIS_C_SPM_SUSPEND,
					 IPI_SEND_POLLING, &pmsr_ipi_data, 8,
					 2000);
#endif
		...
		if (cfg.enable == true) {
			for (i = 0; i < SET_CH_MAX; i++) {
				...
				ret = scmi_tinysys_common_set(
					tinfo->ph, scmi_apmcupm_id,
					pmsr_scmi_data.uid,
					pmsr_scmi_data.action,
					pmsr_scmi_data.param1,
					pmsr_scmi_data.param2,
			...
			if (cfg.fake)
				pmsr_scmi_data.action |=
					PMSR_TOOL_ACT_TEST_FAKE;
			ret = scmi_tinysys_common_set(
				tinfo->ph, scmi_apmcupm_id, pmsr_scmi_data.uid,
				pmsr_scmi_data.action, 0, 0, 0);
#elif IS_ENABLED(CONFIG_MTK_TINYSYS_SSPM_V2)
	...
	for (i = 0; i < SET_CH_MAX; i++)
		if (cfg.ch[i].id < 32)
			log("ch%d: signal %u id %u montype %u (%s)\n", i,
			    cfg.ch[i].signal, cfg.ch[i].id, cfg.ch[i].montype,
...
static int pmsr_procfs_init(void)
{
	...
	if (pmsr_droot) {
		...
		proc_create_data("speed_mode", 0644, pmsr_droot,
				 &local_ipi_fops,
				 (void *)&(cfg.pmsr_speed_mode));
		proc_create_data("window_len", 0644, pmsr_droot,
				 &local_ipi_fops,
		...
		for (i = 0; i < SET_CH_MAX; i++) {
			...
			if (ch[i]) {
				proc_create_data("signal", 0644, ch[i],
						 &local_ipi_fops,
						 (void *)&(cfg.ch[i].signal));
				proc_create_data("id", 0644, ch[i],
						 &local_ipi_fops,
						 (void *)&(cfg.ch[i].id));
				proc_create_data("montype", 0644, ch[i],
						 &local_ipi_fops,
