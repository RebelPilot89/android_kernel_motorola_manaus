...
#endif
int __weak dbg_cm_mgr_platform_show(char *buff)
{
	return 0;
}
...
int cm_mgr_check_dts_setting(struct platform_device *pdev)
{
	...
	opp_count = of_count_phandle_with_args(node, "cm_mgr_cpu_opp_to_dram",
					       NULL);
	...
	ret = of_property_read_string(node, "status", (const char **)&buf);
	if (!ret) {
	...
	ret = of_property_read_string(node, "use_bcpu_weight",
				      (const char **)&buf);
	if (!ret) {
		if (!strcmp(buf, "enable"))
	...
	pr_info("#@# %s(%d) cm_mgr_use_bcpu_weight %d\n", __func__, __LINE__,
		cm_mgr_use_bcpu_weight);

	ret = of_property_read_string(node, "use_cpu_to_dram_map",
				      (const char **)&buf);
	if (!ret) {
		if (!strcmp(buf, "enable"))
	...
	pr_info("#@# %s(%d) cm_mgr_use_cpu_to_dram_map %d\n", __func__,
		__LINE__, cm_mgr_use_cpu_to_dram_map);

	ret = of_property_read_string(node, "use_cpu_to_dram_map_new",
				      (const char **)&buf);
	if (!ret) {
		if (!strcmp(buf, "enable"))
	...
	ret = of_property_read_u32_array(
		node, "cm_mgr,cp_down", cpu_power_ratio_down, cm_mgr_num_array);
	...
	ret = of_property_read_u32_array(node, "cm_mgr,dt_down",
					 ...
					 cm_mgr_num_array);
	ret = of_property_read_u32_array(
		node, "cm_mgr,dt_up", debounce_times_up_adb, cm_mgr_num_array);
	ret = of_property_read_u32_array(node, "cm_mgr,vp_down",
					 vcore_power_ratio_down,
					 cm_mgr_num_array);
	ret = of_property_read_u32_array(
		node, "cm_mgr,vp_up", vcore_power_ratio_up, cm_mgr_num_array);
...
#if defined(CONFIG_MTK_TINYSYS_SSPM_V2) && defined(USE_CM_MGR_AT_SSPM)
	cm_mgr_to_sspm_command(IPI_CM_MGR_SSPM_ENABLE, cm_mgr_sspm_enable);
#endif /* CONFIG_MTK_TINYSYS_SSPM_V2 && defined(USE_CM_MGR_AT_SSPM) */

	cm_mgr_to_sspm_command(IPI_CM_MGR_EMI_DEMAND_CHECK,
			       cm_mgr_emi_demand_check);

	cm_mgr_to_sspm_command(IPI_CM_MGR_LOADING_LEVEL, cm_mgr_loading_level);
