...
#define LOG_DBG(fmt, args...)                                                  \
	pr_debug(DRIVER_NAME " [DBG]"                                          \
			     "<%s:%d>" fmt,                                    \
		 __func__, __LINE__, ##args)
#define LOG_INFO(fmt, args...)                                                 \
	pr_info(DRIVER_NAME " [INFO]"                                          \
			    "<%s:%d>" fmt,                                     \
		__func__, __LINE__, ##args)
#define LOG_ERR(fmt, args...)                                                  \
	pr_err(DRIVER_NAME " [ERR]"                                            \
			   "<%s:%d>" fmt,                                      \
	       __func__, __LINE__, ##args)
...
static void check_and_send(void)
{
	...
	for (i = 0; i < hall_sensor_dev->gpio_num; i++) {
		LOG_INFO("hall report gpio%d = %d", i,
			 gpio_get_value(hall_sensor_dev->gpio_list[i].gpio));
		if (gpio_get_value(hall_sensor_dev->gpio_list[i].gpio) > 0)
			report_val |= hall_sensor_dev->gpio_list[i]
					      .gpio_high_report_val;
		else if (gpio_get_value(hall_sensor_dev->gpio_list[i].gpio) ==
			 0)
			report_val |= hall_sensor_dev->gpio_list[i]
					      .gpio_low_report_val;
	}
	if (hall_sensor_dev->pen_detect &&
		...
		hall_sensor_dev->report_val = report_val;
		input_report_abs(hall_sensor_dev->hall_dev, ABS_DISTANCE,
				 report_val);
 ...
 * the folio hall driver which uses the same default.
 */
...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	//tcmd node
	ret = of_property_read_string(np, "hall,factory-class-name",
				      &hall_class.name);
	...
	ret = of_property_read_u32(np, "hall,nirq-gpio-num",
	...
	input_set_abs_params(hall_sensor_dev->hall_dev, ABS_DISTANCE, -1, 100,
			     0, 0);
	...
	hall_sensor_dev->sensors_pen_cdev.name =
		hall_sensor_dev->hall_dev->name;
	...
	err = sensors_classdev_register(&hall_sensor_dev->hall_dev->dev,
					&hall_sensor_dev->sensors_pen_cdev);
	if (err < 0)
		LOG_ERR("create cap sensor_class  file failed (%d)\n", err);

	hall_sensor_dev->gpio_list =
		kzalloc(sizeof(struct hall_gpio) * hall_sensor_dev->gpio_num,
			GFP_KERNEL);
	...
	for (i = 0; i < hall_sensor_dev->gpio_num; i++) {
		...
		hall_sensor_dev->gpio_list[i].gpio =
			of_get_named_gpio_flags(np, gpio_name, 0, &flags);
		LOG_INFO("hall_sensor %s gpio = %d val = %d:%d", gpio_name,
			 hall_sensor_dev->gpio_list[i].gpio,
			 hall_sensor_dev->gpio_list[i].gpio_high_report_val,
			 hall_sensor_dev->gpio_list[i].gpio_low_report_val);
		if (gpio_is_valid(hall_sensor_dev->gpio_list[i].gpio)) {
			ret = gpio_request(hall_sensor_dev->gpio_list[i].gpio,
					   gpio_name);
			if (ret) {
			...
			hall_sensor_dev->gpio_list[i].irq =
				gpio_to_irq(hall_sensor_dev->gpio_list[i].gpio);
			LOG_INFO("hall_sensor %s irq = %d, gpio = %d",
				 gpio_name, hall_sensor_dev->gpio_list[i].irq,
				 hall_sensor_dev->gpio_list[i].gpio);

			ret = request_threaded_irq(
				hall_sensor_dev->gpio_list[i].irq, NULL,
				hall_sensor_interrupt_handler,
				IRQF_ONESHOT | IRQF_TRIGGER_RISING |
					IRQF_TRIGGER_FALLING,
				gpio_name, hall_sensor_dev);
			if (ret) {
	...
	if (IS_ERR(hall_sensor_dev->hall_vdd)) {
		...
		if (ret)
			LOG_ERR("fail to enable hall_vdd: %d", ret);
...
fail_for_irq:
	for (i = 0; i < hall_sensor_dev->gpio_num; i++) {
		if (hall_sensor_dev->gpio_list[i].irq)
			free_irq(hall_sensor_dev->gpio_list[i].irq,
				 hall_sensor_dev);
		if (gpio_is_valid(hall_sensor_dev->gpio_list[i].gpio))
