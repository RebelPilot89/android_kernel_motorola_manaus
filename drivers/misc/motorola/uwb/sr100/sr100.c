...
#define SR100_DBG_MSG(msg...)                                                  \
	...
	case SR100_FULL_DEBUG:                                                 \
		...
		break;                                                         \
	default:                                                               \
		printk(KERN_ERR "[NXP-SR100] :  Wrong debug level %d",         \
		       debug_level);                                           \
		break;                                                         \
	}
...
struct sr100_dev {
	...
	struct mutex
		sr100_access_lock; /* Hold mutex lock to between read and write */
...
static void sr100_start_throughput_measurement(unsigned int type)
{
	if (type == READ_THROUGH_PUT) {
		memset(&sr100_through_put_t.rstart_tv, 0x00,
		       sizeof(struct timeval));
		do_gettimeofday(&sr100_through_put_t.rstart_tv);
	} else if (type == WRITE_THROUGH_PUT) {
		memset(&sr100_through_put_t.wstart_tv, 0x00,
		       sizeof(struct timeval));
		do_gettimeofday(&sr100_through_put_t.wstart_tv);

	} else {
		printk(KERN_ALERT
		       " sr100_start_throughput_measurement: wrong type = %d",
		       type);
	}
}
/******************************************************************************
...
{
	if (type == READ_THROUGH_PUT) {
		memset(&sr100_through_put_t.rstop_tv, 0x00,
		       sizeof(struct timeval));
		...
		memset(&sr100_through_put_t.wstop_tv, 0x00,
		       sizeof(struct timeval));
		...
		printk(KERN_ALERT
		       " sr100_stop_throughput_measurement: wrong type = %d",
		       type);
	}
}
#endif
...
{
	struct sr100_dev *sr100_dev = container_of(
		filp->private_data, struct sr100_dev, sr100_device);
...
static int sr100_dev_transceive(struct sr100_dev *sr100_dev, int op_mode,
				int count)
{
	int ret, retry_count;
	...
	case SR100_WRITE_MODE: {
		...
		/* UCI Header write */
		ret = spi_write(sr100_dev->spi, sr100_dev->tx_buffer,
	...
	case SR100_READ_MODE: {
		...
		ret = wait_event_interruptible_timeout(sr100_dev->read_wq,
						       sr100_dev->irq_received,
						       sr100_dev->timeOutInMs);
		if (ret == 0) {
      ...
      if(!gpio_get_value(sr100_dev->irq_gpio)){
		...
		ret = spi_read(sr100_dev->spi, (void *)sr100_dev->rx_buffer,
			       NORMAL_MODE_HEADER_LEN);
		if (ret < 0) {
		...
		if (sr100_dev->totalBtyesToRead > MAX_UCI_PKT_SIZE) {
			printk("Length %d  exceeds the max limit %d....",
			       (int)sr100_dev->totalBtyesToRead,
			       (int)SR100_RXBUF_SIZE);
			ret = -1;
			goto transcive_end;
		}
		if (sr100_dev->totalBtyesToRead > 0) {
			ret = spi_read(sr100_dev->spi,
				       (void *)(sr100_dev->rx_buffer +
						NORMAL_MODE_HEADER_LEN),
				       sr100_dev->totalBtyesToRead);
			if (ret < 0) {
		...
		sr100_dev->read_count =
			(unsigned int)(sr100_dev->totalBtyesToRead +
				       NORMAL_MODE_HEADER_LEN);
		retry_count = 0;
		do {
			usleep_range(5, 10);
 ...
 *               otherwise indicate each error code
 ****************************************************************************/
static ssize_t sr100_hbci_read(struct sr100_dev *sr100_dev, char *buf,
			       size_t count)
{
	int ret = -EIO;
	ret = wait_event_interruptible(sr100_dev->read_wq,
				       sr100_dev->irq_received);
	if (ret) {
...
first_irq_wait:
	...
	if (ret == spi_transcive_success) {
		if (copy_to_user(buf, sr100_dev->rx_buffer,
				 sr100_dev->read_count)) {
			pr_info("sr100_dev_read: copy to user failed\n");
			ret = -EFAULT;
			goto read_end;
