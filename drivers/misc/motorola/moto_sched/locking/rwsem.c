		    ...
		    struct list_head *head)
{
	...
	if (prio > 0) {
		list_for_each_safe (pos, n, head) {
			...
			if (waiter && waiter->task->prio > MAX_RT_PRIO &&
			    prio > task_get_mvp_prio(waiter->task, true)) {
				cond_trace_printk(
					unlikely(is_debuggable(DEBUG_BASE)),
					"rwsem_list_add %d prio=%d(%d)index=%d\n",
					tsk->pid, prio,
					task_get_mvp_prio(waiter->task, true),
					index);
...
inline bool test_wait_timeout(struct rw_semaphore *sem)
{
	...
	if (ret) {
		cond_trace_printk(
			...
			__func__, __LINE__, task->comm, task->pid, task->tgid,
			task->prio, task_get_ux_type(task), timeout, timeout,
			timeout + msecs_to_jiffies(WAIT_TIMEOUT),
			timeout + msecs_to_jiffies(WAIT_TIMEOUT), jiffies,
			jiffies);
	}
...
static void android_vh_alter_rwsem_list_add_handler(void *unused,
						    struct rwsem_waiter *waiter,
...
static void android_vh_rwsem_wake_handler(void *unused,
					  struct rw_semaphore *sem)
{
	...
	if (unlikely(!locking_opt_enable(0) || !sem)) {
		return;
	}
	...
	if (boost && (atomic_long_read(&sem->owner) != owner ||
...
static void android_vh_rwsem_wake_finish_handler(void *unused,
						 struct rw_semaphore *sem)
{
	if (unlikely(!locking_opt_enable(0))) {
		return;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
static void
android_vh_record_pcpu_rwsem_time_early(void *unused,
					...
					struct percpu_rw_semaphore *sem)
{
	...
	if (sem == &cgroup_threadgroup_rwsem) {
		lock_protect_update_starttime(current, settime_jiffies,
					      "percpu_rwsem", sem);
	}
}
#endif
...
{
#ifdef ENABLE_REORDER_LIST
	register_trace_android_vh_alter_rwsem_list_add(
		android_vh_alter_rwsem_list_add_handler, NULL);
#endif

#ifdef ENABLE_INHERITE
	register_trace_android_vh_rwsem_wake(android_vh_rwsem_wake_handler,
					     NULL);
	register_trace_android_vh_rwsem_wake_finish(
		android_vh_rwsem_wake_finish_handler, NULL);
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
	register_trace_android_vh_record_pcpu_rwsem_time_early(
		android_vh_record_pcpu_rwsem_time_early, NULL);
#endif
}
...
#endif
#ifdef ENABLE_INHERITE
	unregister_trace_android_vh_rwsem_wake(android_vh_rwsem_wake_handler,
					       NULL);
	unregister_trace_android_vh_rwsem_wake_finish(
		android_vh_rwsem_wake_finish_handler, NULL);
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
	unregister_trace_android_vh_record_pcpu_rwsem_time_early(
		android_vh_record_pcpu_rwsem_time_early, NULL);
#endif
}
