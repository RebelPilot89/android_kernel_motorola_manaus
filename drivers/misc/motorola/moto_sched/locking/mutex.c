	...
	list_for_each_safe (pos, n, head) {
		...
		if (waiter && waiter->task &&
			...
			cond_trace_printk(
				...
				current->pid, prio,
				task_get_mvp_prio(waiter->task, true), index);
...
static bool mutex_list_add(struct task_struct *task, struct list_head *entry,
...
static void android_vh_alter_mutex_list_add_handler(void *unused,
						    struct mutex *lock,
...
static void android_vh_mutex_wait_start_handler(void *unused,
						struct mutex *lock)
{
	struct task_struct *owner_ts = NULL;
	bool boost = false;

	if (unlikely(!locking_opt_enable(0) || !lock)) {
		return;
	}
...
{
	register_trace_android_vh_alter_mutex_list_add(
		android_vh_alter_mutex_list_add_handler, NULL);
	register_trace_android_vh_mutex_wait_start(
		android_vh_mutex_wait_start_handler, NULL);
	register_trace_android_vh_mutex_unlock_slowpath(
		android_vh_mutex_unlock_slowpath_handler, NULL);
}

void unregister_mutex_vendor_hooks(void)
{
	unregister_trace_android_vh_alter_mutex_list_add(
		android_vh_alter_mutex_list_add_handler, NULL);
	unregister_trace_android_vh_mutex_wait_start(
		android_vh_mutex_wait_start_handler, NULL);
	unregister_trace_android_vh_mutex_unlock_slowpath(
		android_vh_mutex_unlock_slowpath_handler, NULL);
}
