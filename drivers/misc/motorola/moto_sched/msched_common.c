...
static inline bool task_in_ux_related_group(struct task_struct *p)
{
	...
	if (is_enabled(UX_ENABLE_AUDIO) && is_scene(UX_SCENE_AUDIO)) {
		...
		// don't boost camera when audio active
		   ...
		   (p->tgid == global_camera_tgid ||
		    ux_type & UX_TYPE_CAMERASERVICE) &&
		   p->prio < 120) {
		return true;
	}
	...
	if (is_heavy_scene()) {
		...
		if (p->tgid == global_launcher_tgid ||
		    ...
		    p->tgid == global_sysui_tgid || p->tgid == global_sf_tgid ||
...
int task_get_mvp_prio(struct task_struct *p, bool with_inherit)
{
	...
	// high priority audio
		 ...
		 (ux_type & UX_TYPE_AUDIO ||
		  ((ux_type & UX_TYPE_AUDIOSERVICE) &&
		   ((p->prio == 101 || p->prio == 104) ||
		    (is_heavy_scene() && is_scene(UX_SCENE_AUDIO) &&
		     p->prio <= 120)))))
		prio = UX_PRIO_AUDIO;
	// input & animation & low latency binder
	else if (ux_type &
		 (UX_TYPE_INPUT | UX_TYPE_ANIMATOR |
		  UX_TYPE_LOW_LATENCY_BINDER | UX_TYPE_GESTURE_MONITOR))
		prio = UX_PRIO_ANIMATOR;
	// main & render thread of top app, launcher and top UI.
	else if (ux_type &
			 (UX_TYPE_TOPAPP | UX_TYPE_LAUNCHER | UX_TYPE_TOPUI) ||
	...
	// system lock & service mgr
		...
		prio = UX_PRIO_SYSTEM;
	// inherit lock & binder
	else if (with_inherit &&
		 (ux_type & (UX_TYPE_INHERIT_BINDER | UX_TYPE_INHERIT_LOCK)))
		prio = UX_PRIO_OTHER;
	// others high & others low but small tasks.
	else if (task_in_ux_related_group(p) &&
		 (p->prio <= moto_boost_prio ||
		  moto_task_util(p) < moto_boost_task_util))
		prio = UX_PRIO_OTHER;

	cond_trace_printk(
		unlikely(is_debuggable(DEBUG_BASE)),
		"pid=%d tgid=%d prio=%d scene=%d ux_type=%d task_util=%lu mvp_prio=%d\n",
		p->pid, p->tgid, p->prio, moto_sched_scene, ux_type,
		moto_task_util(p), prio);
...
{
	return task_has_ux_type(p, UX_TYPE_TOPAPP | UX_TYPE_LAUNCHER |
					   UX_TYPE_TOPUI | UX_TYPE_ANIMATOR);
}
...
unsigned int task_get_mvp_limit(struct task_struct *p, int mvp_prio)
{
	...
	if (mvp_prio == UX_PRIO_TOPAPP)
		...
		return CAMERA_LIMIT;
	else if (mvp_prio == UX_PRIO_SYSTEM ||
		 (p->tgid == global_systemserver_tgid))
...
void queue_ux_task(struct rq *rq, struct task_struct *task, int enqueue)
{
	if (is_enabled(UX_ENABLE_LOCK) && !enqueue) {
		...
		if (task_has_ux_type(task, UX_TYPE_INHERIT_LOCK)) {
			if (jiffies_to_nsecs(jiffies) - wts->inherit_start >
			    MAX_INHERIT_GRAN) {
				cond_trace_printk(
					unlikely(is_debuggable(DEBUG_BASE)),
					"lock_clear_inherited_ux_type %s  %d  ux_type %d  cost=%llu\n",
					"dequeue task", task->pid, wts->ux_type,
		...
		if (task_has_ux_type(task, UX_TYPE_KERNEL)) {
			if (jiffies_to_nsecs(jiffies) -
				    wts->boost_kernel_start >
			    MAX_INHERIT_GRAN) {
				cond_trace_printk(
					unlikely(is_debuggable(DEBUG_BASE)),
...
void binder_ux_type_set(struct task_struct *task)
{
	...
	if (task && ((task_in_top_related_group(current) &&
		      task->group_leader->prio < MAX_RT_PRIO) ||
		     (current->group_leader->prio < MAX_RT_PRIO &&
		      task_in_top_related_group(task)) ||
		     (current->group_leader->prio < MAX_RT_PRIO &&
		      task->group_leader->prio < MAX_RT_PRIO) ||
		     (task_in_top_related_group(current) &&
		      task_in_top_related_group(task))))
	...
	cond_trace_printk(
		...
		current->tgid, current->group_leader->prio, task->tgid,
		task->group_leader->prio, task_get_ux_type(task));
}
EXPORT_SYMBOL(binder_ux_type_set);

bool lock_inherit_ux_type(struct task_struct *owner, struct task_struct *waiter,
			  char *lock_name)
{
	...
	cond_trace_printk(
		...
		lock_name, waiter->pid, owner->pid, waiter_wts->ux_type,
		owner_wts->ux_type, waiter_wts->inherit_depth);
...
bool lock_clear_inherited_ux_type(struct task_struct *owner, char *lock_name)
{
	...
	cond_trace_printk(
		unlikely(is_debuggable(DEBUG_BASE)),
		"lock_clear_inherited_ux_type %s  %d  ux_type %d cost=%llu\n",
		lock_name, owner->pid, owner_wts->ux_type,
		(jiffies_to_nsecs(jiffies) - owner_wts->inherit_start) /
			1000000U);
...
void lock_protect_update_starttime(struct task_struct *tsk,
				   unsigned long settime_jiffies,
				   char *lock_name, void *pointer)
{
	struct moto_task_struct *waiter_wts =
		(struct moto_task_struct *)tsk->android_oem_data1;
	if (unlikely(!locking_opt_enable(0)))
		return;
	...
	if (unlikely(is_debuggable(DEBUG_LOCK))) {
		if (settime_jiffies == 0) {
			if (waiter_wts->boost_kernel_lock_depth == 0) {
				printk(KERN_ERR
				       "LOCK_PERF(%s)kernel boost mismatch(%d)!!",
				       lock_name,
				       waiter_wts->boost_kernel_lock_depth);
			}
			if (task_has_ux_type(tsk, UX_TYPE_KERNEL)) {
				u64 sleep = (jiffies_to_nsecs(jiffies) -
					     waiter_wts->boost_kernel_start) /
					    1000000U;
				if (sleep > 40) {
					cond_trace_printk(
						true,
						...
						lock_name, tsk->prio,
						waiter_wts
							->boost_kernel_lock_depth,
						sleep);
				}
				if (sleep > 100) {
					printk(KERN_ERR
					       "LOCK_PERF (%s) running too long prio=%d locked=%d cost=%llu",
					       lock_name, tsk->prio,
					       waiter_wts
						       ->boost_kernel_lock_depth,
					       sleep);
				...
				printk(KERN_ERR
				       "LOCK_PERF rwsem didn't boost!!!");
			}
		} else {
			if (waiter_wts->boost_kernel_lock_depth > 32) {
				cond_trace_printk(
					true, "(%s)kernel boost mismatch(%d)!!",
					lock_name,
					waiter_wts->boost_kernel_lock_depth);
			}
		}
...
#define UX_TYPE_TO_DUP                                                         \
	(UX_TYPE_AUDIOSERVICE | UX_TYPE_NATIVESERVICE |                        \
	 UX_TYPE_CAMERASERVICE | UX_TYPE_ANIMATOR)
static void android_vh_dup_task_struct(void *unused, struct task_struct *task,
...
{
	register_trace_android_vh_dup_task_struct(android_vh_dup_task_struct,
						  NULL);
}
