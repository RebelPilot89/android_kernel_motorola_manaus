...
static long copy_from_user_compatible(void *dst, const void __user *src,
				      size_t size)
{
			  ...
			  struct binder_transaction_data *tr)
{
	...
	if (!(tr->flags & TF_ONE_WAY) && target_proc &&
	    (NULL != target_proc->tsk) && (NULL != proc->tsk) &&
	    (task_uid(target_proc->tsk).val > MIN_USERAPP_UID) &&
	    (proc->pid != target_proc->pid) && is_frozen_tg(target_proc->tsk)) {
		moto_binder_write_status(CALL_BINDER, task_uid(proc->tsk).val,
					 task_tgid_nr(proc->tsk),
					 task_uid(target_proc->tsk).val,
					 task_tgid_nr(target_proc->tsk), 0, 0);
	}

	if ((tr->flags & TF_ONE_WAY) /*report async binder call*/
	    && target_proc && (NULL != target_proc->tsk) &&
	    (NULL != proc->tsk) &&
	    (task_uid(target_proc->tsk).val > MIN_USERAPP_UID) &&
	    (proc->pid != target_proc->pid) && is_frozen_tg(target_proc->tsk)) {
		buf_data_size = tr->data_size > INTERFACETOKEN_BUFF_SIZE ?
					...
					tr->data_size;
		if (!copy_from_user_compatible(buf_data,
					       (char *)tr->data.ptr.buffer,
					       buf_data_size)) {
			...
			if (target_proc->tsk->signal->oom_score_adj <= 250) {
				moto_binder_write_status(
					ASYNC_BINDER, task_uid(proc->tsk).val,
					task_tgid_nr(proc->tsk),
					task_uid(target_proc->tsk).val,
					task_tgid_nr(target_proc->tsk), 0,
					tr->code);
			}
		}
	}
			  ...
			  struct binder_transaction_data *tr)
{
	...
	if (target_proc && (NULL != target_proc->tsk) && (NULL != proc->tsk) &&
	    (task_uid(target_proc->tsk).val <= MAX_SYSTEM_UID) &&
	    (proc->pid != target_proc->pid) && is_frozen_tg(target_proc->tsk)) {
		moto_binder_write_status(REPLY_BINDER, task_uid(proc->tsk).val,
					 task_tgid_nr(proc->tsk),
					 task_uid(target_proc->tsk).val,
					 task_tgid_nr(target_proc->tsk), 0, 0);
	}
...
void binder_alloc_handler(size_t size, struct binder_alloc *alloc, int is_async)
{
	...
	*/
	if (is_async && (alloc->free_async_space <
		...
		if (p != NULL && is_frozen_tg(p)) {
			moto_binder_write_status(ASYNC_ALLOC_FULL,
...
static struct moto_binder_transaction_log_entry *
moto_binder_transaction_log_add(struct moto_binder_transaction_log *log)
{
...
void moto_binder_write_status(int call_type, int caller_uid, int caller_pid,
			      int target_uid, int target_pid, int arg1,
			      int arg2)
{
	struct moto_binder_transaction_log_entry *e;
	...
	moto_binder_send_uevent(call_type, caller_uid, caller_pid, target_uid,
				target_pid, arg1, arg2);
}
...
int proc_binder_status_show(struct seq_file *m, void *unused)
{
	...
	for (i = 0; i < count; i++) {
		...
		seq_printf(m, "%d: %s from %d:%d to %d:%d %d %d\n", e->seq_id,
			   binder_type_to_string(e->call_type), e->caller_uid,
			   e->caller_pid, e->target_uid, e->target_pid, e->arg1,
			   e->arg2);
	}
	return 0;
}
			...
			struct binder_transaction *tr)
{
	...
	*to_pid = -1;
	...
	} else if (tr->to_thread != NULL && tr->to_thread->proc != NULL &&
...
void proc_binder_check_status_print(struct seq_file *m, int caller_uid,
				    int caller_pid, int target_uid,
				    int target_pid, int arg1, int arg2)
{
	seq_printf(m, "from %d:%d to %d:%d %d %d\n", caller_uid, caller_pid,
		   target_uid, target_pid, arg1, arg2);
}

void proc_binder_check_status_show_detail(struct seq_file *m,
					  struct binder_proc *proc, int to_uid)
{
		...
		if (thread->task != NULL) {
						...
						if (btrans->to_thread != NULL &&
							...
							if (from_uid !=
								...
								binder_inner_proc_unlock(
									proc);
								proc_binder_check_status_print(
									m,
									from_uid,
									from_pid,
									to_uid,
			...
			if (btrans) {
				...
				if (btrans->to_thread != NULL &&
					...
					need_reply = (int)(!(btrans->flags &
							     TF_ONE_WAY));
					get_uid_pid(&from_pid, &from_uid,
						    &to_pid, &to_uid, btrans);
	...
	if (proc->tsk != NULL && !empty) {
		list_for_each_entry (w, &proc->todo, entry) {
			btrans = container_of(w, struct binder_transaction,
					      work);
			if (w != NULL && w->type == BINDER_WORK_TRANSACTION &&
			    btrans != NULL) {
				spin_lock(&btrans->lock);
				get_uid_pid(&from_pid, &from_uid, &to_pid,
					    &to_uid, btrans);
				need_reply =
					(int)(!(btrans->flags & TF_ONE_WAY));
				if (btrans->to_thread != NULL &&
				    btrans->to_thread == thread &&
...
int proc_binder_check_status_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry (proc, binder_procs, proc_node) {
		if (proc != NULL && (proc->tsk != NULL) &&
		    (task_uid(proc->tsk).val == moto_binder_check_uid)) {
			proc_binder_check_status_show_detail(
				m, proc, moto_binder_check_uid);
		}
	}
...
int register_moto_binder_hooks(void)
{
	...
	rc = register_trace_android_vh_binder_preset(binder_preset_handler,
						     NULL);
	if (rc != 0) {
	...
	rc = register_trace_android_vh_binder_alloc_new_buf_locked(
		binder_alloc_handler, NULL);
	if (rc != 0) {
