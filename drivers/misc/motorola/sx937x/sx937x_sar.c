...
#define LOG_INFO(fmt, args...)                                                 \
	pr_info(LOG_TAG "[INFO]"                                               \
			"<%s><%d>" fmt,                                        \
		__func__, __LINE__, ##args)
#define LOG_DBG(fmt, args...)                                                  \
	pr_debug(LOG_TAG "[DBG]"                                               \
			 "<%s><%d>" fmt,                                       \
		 __func__, __LINE__, ##args)
	...
	if (this && this->bus) {
		...
		while (true) {
			...
			if (num_retried++ < NUM_RETRY_ON_I2C_ERR) {
				LOG_ERR("i2c write reg 0x%x error %d. Going to retry",
					reg_addr, ret);
...
static int sx937x_i2c_read_16bit(psx93XX_t this, u16 reg_addr, u32 *data32)
{
	...
	if (this && this->bus) {
		...
		while (true) {
			...
			if (ret > 0) {
				data32[0] = ((u32)buf[0] << 24) |
					    ((u32)buf[1] << 16) |
					    ((u32)buf[2] << 8) | ((u32)buf[3]);
				break;
			}
			if (num_retried++ < NUM_RETRY_ON_I2C_ERR) {
				LOG_ERR("i2c read reg 0x%x error %d. Going to retry",
					reg_addr, ret);
				if (SLEEP_BETWEEN_RETRY != 0)
					msleep(SLEEP_BETWEEN_RETRY);
			} else {
...
static int read_regStat(psx93XX_t this)
{
	u32 data = 0;
	if (this) {
		if (sx937x_i2c_read_16bit(this, SX937X_IRQ_SOURCE, &data) >
		    0) //bug
			return (data & 0x00FF);
	}
	return 0;
...
static void read_dbg_raw(psx93XX_t this)
{
	...
	if (ref_ph_a != -1) {
		sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + ref_ph_a * 4,
				      &uData);
		ref_a_use = (s32)uData >> 10;
	}
	if (ref_ph_b != -1) {
		sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + ref_ph_b * 4,
				      &uData);
		ref_b_use = (s32)uData >> 10;
	}
	if (ref_ph_c != -1) {
		sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + ref_ph_c * 4,
				      &uData);
	...
	if (ph >= 0 && ph <= 7) {
		...
		sx937x_i2c_read_16bit(this, SX937X_AVERAGE_PH0 + ph * 4,
				      &uData);
		avg = (s32)uData >> 10;
		sx937x_i2c_read_16bit(this, SX937X_DIFF_PH0 + ph * 4, &uData);
		diff = (s32)uData >> 10;
		sx937x_i2c_read_16bit(this, SX937X_OFFSET_PH0 + ph * 4 * 3,
				      &uData);
		...
		LOG_DBG("SMTC_DBG PH= %d USE= %d RAW= %d PH%d_USE= %d PH%d_USE= %d PH%d_USE= %d STATE= %d AVG= %d DIFF= %d OFF= %d ADC_MIN= %d ADC_MAX= %d DLT= %d SMTC_END\n",
			ph, ant_use, ant_raw, ref_ph_a, ref_a_use, ref_ph_b,
			ref_b_use, ref_ph_c, ref_c_use, state, avg, diff, off,
			adc_min, adc_max, use_flt_dlt_var);
	} else {
...
static void read_rawData(psx93XX_t this)
{
	...
	if (this) {
		...
		if (ref_ph_a != -1) {
			sx937x_i2c_read_16bit(
				this, SX937X_USEFUL_PH0 + ref_ph_a * 4, &uData);
			ref_a_use = (s32)uData >> 10;
		}
		if (ref_ph_b != -1) {
			sx937x_i2c_read_16bit(
				this, SX937X_USEFUL_PH0 + ref_ph_b * 4, &uData);
			ref_b_use = (s32)uData >> 10;
		}
		if (ref_ph_c != -1) {
			sx937x_i2c_read_16bit(
				this, SX937X_USEFUL_PH0 + ref_ph_c * 4, &uData);
			ref_c_use = (s32)uData >> 10;
		}

		for (csx = 0; csx < 8; csx++) {
			index = csx * 4;
			sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + index,
					      &uData);
			useful = (s32)uData >> 10;
			sx937x_i2c_read_16bit(this, SX937X_AVERAGE_PH0 + index,
					      &uData);
			average = (s32)uData >> 10;
			sx937x_i2c_read_16bit(this, SX937X_DIFF_PH0 + index,
					      &uData);
			diff = (s32)uData >> 10;
			sx937x_i2c_read_16bit(
				this, SX937X_OFFSET_PH0 + index * 3, &uData);
			...
			LOG_DBG("SMTC_DAT PH= %d DIFF= %d USE= %d PH%d_USE= %d PH%d_USE= %d PH%d_USE= %d STATE= %d OFF= %d AVG= %d SMTC_END\n",
				csx, diff, useful, ref_ph_a, ref_a_use,
				ref_ph_b, ref_b_use, ref_ph_c, ref_c_use, state,
				offset, average);
		}
				      ...
				      const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", 1)) {
		...
		for (i = 0; i < pdata->headset_operate_reg_num; i++) {
			sx937x_i2c_write_16bit(
				global_sx937x,
				pdata->headset_operate_reg[i].reg,
				pdata->headset_operate_reg[i].val);
	...
	if (!strncmp(buf, "0", 1)) {
		...
		for (i = 0; i < pdata->headset_operate_reg_num; i++) {
			sx937x_i2c_write_16bit(
				global_sx937x,
				pdata->headset_operate_reg_bck[i].reg,
				pdata->headset_operate_reg_bck[i].val);
			LOG_INFO("set Reg 0x%x Value: 0x%x\n",
				 pdata->headset_operate_reg_bck[i].reg,
				 pdata->headset_operate_reg_bck[i].val);
		}
				      ...
				      struct class_attribute *attr, char *buf)
{
	...
	if (this) {
		for (csx = 0; csx < 8; csx++) {
			sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + csx * 4,
					      &uData);
			useful = (s32)uData >> 10;
			sx937x_i2c_read_16bit(
				this, SX937X_AVERAGE_PH0 + csx * 4, &uData);
			average = (s32)uData >> 10;
			sx937x_i2c_read_16bit(this, SX937X_DIFF_PH0 + csx * 4,
					      &uData);
			diff = (s32)uData >> 10;
			sx937x_i2c_read_16bit(
				this, SX937X_OFFSET_PH0 + csx * 12, &uData);
			offset = (u16)(uData & 0x3FFF);
			p += snprintf(
				p, PAGE_SIZE,
				"PH= %d Useful = %d Average = %d DIFF = %d Offset = %d \n",
...
static struct class_attribute class_attr_register_read =
	__ATTR(register_read, 0660, sx937x_register_read_show,
	       sx937x_register_read_store);
...
#ifdef CONFIG_CAPSENSE_HEADSET_STATE
		i = 0;
		LOG_INFO("set headset reg num:%d",
			 pdata->headset_operate_reg_num);
		while (i < pdata->headset_operate_reg_num) {
			sx937x_i2c_read_16bit(
				this, pdata->headset_operate_reg_bck[i].reg,
				&(pdata->headset_operate_reg_bck[i].val));
...
#endif
		/*******************************************************************************/
		sx937x_i2c_write_16bit(this, SX937X_COMMAND,
				       0xF); //enable phase control
	} else {
...
static int initialize(psx93XX_t this)
{
	...
	if (this) {
		...
		for (retry = 10; retry > 0; retry--) {
			if (sx937x_i2c_write_16bit(this, SX937X_DEVICE_RESET,
						   0xDE) >= 0)
				break;
...
static void touchProcess(psx93XX_t this)
{
	...
	if (this && (pDevice = this->pDevice)) {
		...
		for (counter = 0; counter < numberOfButtons; counter++) {
			...
			touchFlag = i & (pCurrentButton->ProxMask |
					 pCurrentButton->BodyMask);
			if (touchFlag == (pCurrentButton->ProxMask |
					  pCurrentButton->BodyMask)) {
...
static int read_dt_regs(struct device *dev, const char *dt_field, int *num_regs,
			struct smtc_reg_data **regs)
{
	int byte_len;
	int rc;
	...
	if (of_find_property(node, dt_field, &byte_len)) {
		*regs = (struct smtc_reg_data *)devm_kzalloc(dev, byte_len,
							     GFP_KERNEL);
		...
		*num_regs = byte_len / sizeof(struct smtc_reg_data);
		...
		rc = of_property_read_u32_array(node, dt_field, (u32 *)*regs,
						byte_len / sizeof(u32));
...
static bool parse_flip_dt_params(struct sx937x_platform_data *pdata,
				 struct device *dev)
{
	struct device_node *node = dev->of_node;
	...
	if (read_dt_regs(dev, "flip-open-regs", &pdata->num_flip_open_regs,
			 &pdata->flip_open_regs))
		return false;

	if (read_dt_regs(dev, "flip-closed-regs", &pdata->num_flip_closed_regs,
			 &pdata->flip_closed_regs))
		return false;
...
static int sx937x_parse_dt(struct sx937x_platform_data *pdata,
			   struct device *dev)
{
	...
	rc = of_property_read_u32(dNode, "Semtech,power-supply-type",
				  &pdata->power_supply_type);
	if (rc < 0) {
		pdata->power_supply_type = SX937X_POWER_SUPPLY_TYPE_PMIC_LDO;
		LOG_INFO(
			"pmic ldo is the default if not set power-supply-type in dt\n");
	}
	...
	case SX937X_POWER_SUPPLY_TYPE_EXTERNAL_LDO:
		...
		pdata->eldo_gpio = of_get_named_gpio_flags(
			dNode, "Semtech,eldo-gpio", 0, &flags);
	...
	of_property_read_u32(dNode, "Semtech,button-flag",
	...
	pdata->ref_phase_b = -1;
	pdata->ref_phase_c = -1;
	if (of_property_read_u32(dNode, "Semtech,ref-phases-a",
				 &pdata->ref_phase_a)) {
		LOG_ERR("[SX937x]: %s - get ref-phases error\n", __func__);
		return -ENODEV;
	}
	if (of_property_read_u32(dNode, "Semtech,ref-phases-b",
				 &pdata->ref_phase_b)) {
		LOG_ERR("[SX937x]: %s - get ref-phases-b error\n", __func__);
		return -ENODEV;
	}
	if (of_property_read_u32(dNode, "Semtech,ref-phases-c",
				 &pdata->ref_phase_c)) {
		LOG_ERR("[SX937x]: %s - get ref-phases-c error\n", __func__);
		return -ENODEV;
	}
	LOG_INFO(
		"[SX937x]: %s ref_phase_a= %d ref_phase_b= %d ref_phase_c= %d\n",
	...
	if (pdata->headset_operate_reg_num > 0) {
		...
		if (of_property_read_u32_array(
			    dNode, "Semtech,headset-reg",
			    (u32 *)&(pdata->headset_operate_reg[0]),
			    sizeof(struct smtc_reg_data) *
				    pdata->headset_operate_reg_num /
				    sizeof(u32)))
			return -ENOMEM;
		if (of_property_read_u32_array(
			    dNode, "Semtech,headset-reg",
			    (u32 *)&(pdata->headset_operate_reg_bck[0]),
			    sizeof(struct smtc_reg_data) *
				    pdata->headset_operate_reg_num /
				    sizeof(u32)))
			return -ENOMEM;
				...
				unsigned int enable)
{
		...
		if (strcmp(sensors_cdev->name, psmtcButtons[i].name) == 0) {
			if (enable == 1) {
				LOG_INFO("enable cap sensor : %s\n",
					 sensors_cdev->name);
				sx937x_i2c_read_16bit(global_sx937x,
						      SX937X_GENERAL_SETUP,
						      &temp);
				...
				input_report_abs(psmtcButtons[i].input_dev,
						 ABS_DISTANCE, 0);
	...
	if (disableFlag) {
		...
		sx937x_i2c_read_16bit(global_sx937x, SX937X_GENERAL_SETUP,
				      &temp);
		LOG_DBG("read reg 0x%x val 0x%x\n", SX937X_GENERAL_SETUP, temp);
		temp = temp & 0xFFFFFF00;
		LOG_DBG("set reg 0x%x val 0x%x\n", SX937X_GENERAL_SETUP, temp);
		sx937x_i2c_write_16bit(global_sx937x, SX937X_GENERAL_SETUP,
				       temp);
...
static int ps_notify_callback(struct notifier_block *self, unsigned long event,
			      void *p)
{
...
static void write_flip_regs(int num_regs, struct smtc_reg_data *regs)
{
	...
	for (i = 0; i < num_regs; i++) {
		...
		sx937x_i2c_write_16bit(global_sx937x, regs[i].reg, regs[i].val);
	}
}

static void update_flip_regs(struct sx937x_platform_data *data,
			     unsigned long state)
{
	if (data->phone_flip_update_regs) {
		if (state == data->phone_flip_open_val) {
			...
			write_flip_regs(data->num_flip_open_regs,
					data->flip_open_regs);
		} else {
			/* Flip closed */
			LOG_DBG("Writing %d regs on close\n",
				data->num_flip_closed_regs);
			write_flip_regs(data->num_flip_closed_regs,
					data->flip_closed_regs);
		}
	}
}
			...
			const struct i2c_device_id *id)
{
	...
	pplatData = devm_kzalloc(
		&client->dev, sizeof(struct sx937x_platform_data), GFP_KERNEL);
	if (!pplatData) {
	...
	if (this) {
		...
		this->pDevice = pDevice = devm_kzalloc(
			&client->dev, sizeof(sx937x_t), GFP_KERNEL);
		...
		if (pDevice) {
			...
			/* class internal kobject is private in modern kernels */
			...
			for (i = 0; i < pButtonInformationData->buttonSize;
				...
				if (pplatData->button_used_flag >> i & 0x01) {
					pButtonInformationData->buttons[i].used =
						true;
					pButtonInformationData->buttons[i]
						.state = IDLE;
					pButtonInformationData->buttons[i]
						.input_dev =
						input_allocate_device();
					if (!pButtonInformationData->buttons[i]
						     .input_dev)
						return -ENOMEM;
					pButtonInformationData->buttons[i]
						.input_dev->name =
					...
					input_report_abs(pButtonInformationData
								 ->buttons[i]
								 .input_dev,
							 ABS_DISTANCE, -1);
					input_sync(pButtonInformationData
							   ->buttons[i]
							   .input_dev);

					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev
						.sensors_enable =
					...
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev.name =
						pButtonInformationData
							->buttons[i]
							.name;
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev.vendor =
						"semtech";
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev.version =
						1;
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev.type =
						SENSOR_TYPE_MOTO_CAPSENSE;
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev
						.max_range = "5";
					pButtonInformationData->buttons[i]
					...
					pButtonInformationData->buttons[i]
						...
						.delay_msec = 100;
					pButtonInformationData->buttons[i]
						.sensors_capsensor_cdev.enabled =
						0;
					pButtonInformationData->buttons[i]
						.enabled = false;
		...
		if (of_property_read_bool(client->dev.of_node, "extcon")) {
			pplatData->flip_notif.notifier_call =
				flip_notify_callback;
			...
			if (IS_ERR(pplatData->ext_flip_det)) {
				...
				if (extcon_register_notifier(
					    pplatData->ext_flip_det,
					    EXTCON_MECHANICAL,
					    &pplatData->flip_notif))
	...
	if (pplatData->esd_reinit_on) {
		INIT_DELAYED_WORK(&this->i2c_watchdog_work,
				  sx937x_i2c_watchdog_work);
		schedule_delayed_work(
			&this->i2c_watchdog_work,
...
#endif
{
	...
	if (this && (pDevice = this->pDevice)) {
		...
		for (i = 0; i < pDevice->pbuttonInformation->buttonSize; i++) {
			pCurrentbutton =
				&(pDevice->pbuttonInformation->buttons[i]);
			if (pCurrentbutton->used == true) {
				sensors_classdev_unregister(&(
					pCurrentbutton->sensors_capsensor_cdev));
				input_unregister_device(
					pCurrentbutton->input_dev);
			}
		}
	}
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
...
static struct i2c_device_id sx937x_idtable[] = { { DRIVER_NAME, 0 }, {} };
...
static void sx937x_register_err(psx93XX_t this)
{
	...
	for (ph = 0; ph < NUM_PHASES; ph++) {
		if (phen & 1 << ph) {
			sx937x_i2c_read_16bit(this, SX937X_USEFUL_PH0 + ph * 4,
					      &reg_val);
		...
		LOG_DBG("phen = %d useful[%d][%d] = %d\n", phen, ph,
			check_round, ph_useful[ph][check_round]);
	}
...
static void sx937x_reinitialize(psx93XX_t this)
{
	...
	if (this && (pdata = pDevice->hw)) {
		...
		sx937x_i2c_read_16bit(this, SX937X_GENERAL_SETUP,
				      &temp); //0x8024
		for (i = 0; i < pDevice->pbuttonInformation->buttonSize; i++) {
			pCurrentbutton =
				&(pDevice->pbuttonInformation->buttons[i]);
