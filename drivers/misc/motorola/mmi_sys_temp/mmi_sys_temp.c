...
#ifdef CONFIG_COMPAT
static long mmi_sys_temp_compat_ioctl(struct file *file, unsigned int cmd,
				      unsigned long arg)
{
...
static int mmi_sys_temp_get(struct thermal_zone_device *thermal, int *temp)
{
...
static void psy_changed_work_func(struct work_struct *work)
{
	...
	dev_info(&sys_temp_dev->pdev->dev, "%s\n", buf);
	return;
}
...
static int mmi_sys_temp_probe(struct platform_device *pdev)
{
	...
	if (num_sensors <= 0) {
		dev_err(&pdev->dev, "bad number of sensors: %d\n", num_sensors);
		return -EINVAL;
	}

	num_sensors_listener =
		of_property_count_strings(node, SENSOR_LISTENER_NAMES);
	if (num_sensors_listener <= 0) {
		dev_err(&pdev->dev, "bad number of sensors-listener: %d\n",
			num_sensors_listener);
	}
	...
	sys_temp_dev->sensor = (struct mmi_sys_temp_sensor *)devm_kzalloc(
		&pdev->dev, (num_sensors * sizeof(struct mmi_sys_temp_sensor)),
		GFP_KERNEL);
	if (!sys_temp_dev->sensor) {
		dev_err(&pdev->dev, "Unable to alloc memory for sensor\n");
		return -ENOMEM;
	}

	for (i = 0; i < num_sensors; i++) {
		ret = of_property_read_string_index(
			node, TEMP_NODE_SENSOR_NAMES, i,
		...
		sys_temp_dev->sensor[i].tz_dev = thermal_zone_device_register(
			sys_temp_dev->sensor[i].name, 0, 0,
			&sys_temp_dev->sensor[i], &mmi_sys_temp_ops, NULL, 0,
			0);
	...
	sys_temp_dev->sensor_listener =
		(struct mmi_sys_temp_sensor *)devm_kzalloc(
			&pdev->dev,
			(num_sensors_listener *
	...
	for (i = 0; i < num_sensors_listener; i++) {
		...
		if (sys_temp_dev->sensor_listener[i].name) {
			...
			dev_err(&pdev->dev, "Invalid sensor listener name\n");
			goto err_sensors_listener;
		}
	}
