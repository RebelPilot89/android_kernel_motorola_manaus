...
static u32 km_session_counter =
	0; /* Designed to be only one keymaster session.*/
...
static int hwkm_open_session(void)
{
	...
	do {
		...
		mc_ret = mc_malloc_wsm(g_device_id, 0, sizeof(tci_t),
				       (uint8_t **)&km_tci, 0);
		if (mc_ret != MC_DRV_OK) {
...
int hwkm_program_key(const u8 *wrapped_key, u32 wrapped_key_size, u32 gie_para,
			   ...
			   u8 *raw_secret, u32 raw_secret_size)
{
	...
	/* Check the input paremeters */
	if (wrapped_key == NULL || raw_secret == NULL ||
