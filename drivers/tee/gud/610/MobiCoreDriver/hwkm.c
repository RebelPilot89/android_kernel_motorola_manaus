...
static kmtciMessage_t *km_tci;
static u32 km_session_counter; /* Designed to be only one keymaster session. */
					...
					u32 wrapped_key_size, u32 actual_size)
{
	 ...
	 * actually pad additional 24 bytes 0x01 to satisfy the fscrypt keyring
		     ...
		     u32 gie_para, storage_key_type_t type)
{
	...
	/* Open the session with mcDaemon */
	if (km_session_counter == 0) {
		pr_notice("(%s): open session for the first time\n", __func__);
		res = hwkm_open_session();
		if (res != 0) {
			mc_dev_err(MC_DRV_ERR_UNKNOWN, "open session failed");
			return -EINVAL;
		}
	}

	/* Map transfer data buffer */
	mc_ret = mc_map(&g_km_session, (void *)wrapped_key, wrapped_key_size,
			&wrapped_key_map);
	if (mc_ret != MC_DRV_OK) {
	...
	km_tci->command.header.commandId =
		CMD_ID_TEE_UNWRAP_AND_INSTALL_AES_STORAGE_KEY;
	km_tci->aes_storage_key_unwrap_install.wrapped_key_data.data =
		(uint32_t)wrapped_key_map.secure_virt_addr;
	km_tci->aes_storage_key_unwrap_install.wrapped_key_data.data_length =
		wrapped_key_map.secure_virt_len;
	 ...
	 * We will not close the session but let the caller catch the error
	 * returned from the secure world and make a decision for further
	 * operation e.g. retry or not.
	 */
	res = mc_ret;
	if (km_tci->response.header.returnCode != 0) {
		mc_dev_err(mc_ret,
			   "secure side error for keyslot programming: %d\n",
			   ...
			   u8 *raw_secret, u32 raw_secret_size)
{
	...
	if (wrapped_key == NULL ||
		...
		mc_dev_err(mc_ret,
			   "invalid input, wrapped_key_size=%d, raw_secret_size=%d\n",
			   wrapped_key_size, raw_secret_size);
		return -EINVAL;
	}
	...
	/* If the wrapped key is padded with known fields (in our case 0x01),
	 * validate the remaining data to see if it's corrupted.
	 */
	if (check_wrapped_key_corrupted(wrapped_key, wrapped_key_size,
					hwkm_wrapped_key_size)) {
	...
	mc_ret = mc_map(&g_km_session, (void *)wrapped_key,
			hwkm_wrapped_key_size, &wrapped_key_map);
	if (mc_ret != MC_DRV_OK) {
		mc_dev_err(mc_ret,
			   "mc_map failed, wrapped key buf returned: 0x%08x",
			   mc_ret);
		hwkm_close_session();
		return -EINVAL;
	}
	mc_ret = mc_map(&g_km_session, (void *)raw_secret, raw_secret_size,
			&raw_secret_map);
	if (mc_ret != MC_DRV_OK) {
		mc_dev_err(mc_ret,
			   "mc_map failed, raw secret buf returned: 0x%08x",
			   mc_ret);
		mc_unmap(&g_km_session, (void *)wrapped_key, &wrapped_key_map);
		hwkm_close_session();
		return -EINVAL;
	...
	/* Fill tci structure per command ID */
	km_tci->command.header.commandId =
		CMD_ID_TEE_UNWRAP_AND_DERIVE_AES_STORAGE_SECRET;
	km_tci->aes_storage_key_unwrap_derive.wrapped_key_data.data =
		(uint32_t)wrapped_key_map.secure_virt_addr;
	km_tci->aes_storage_key_unwrap_derive.wrapped_key_data.data_length =
		wrapped_key_map.secure_virt_len;
	km_tci->aes_storage_key_unwrap_derive.raw_secret.data =
		(uint32_t)raw_secret_map.secure_virt_addr;
	km_tci->aes_storage_key_unwrap_derive.raw_secret.data_length =
		raw_secret_map.secure_virt_len;
	...
	if (km_tci->response.header.returnCode != 0) {
		mc_dev_err(mc_ret,
			   "secure side error for key derivation: %d\n",
			   km_tci->response.header.returnCode);
		res = -EOPNOTSUPP;
	}
