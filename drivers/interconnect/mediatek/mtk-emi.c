...
static struct mtk_icc_node *mt8183_icc_nodes[] = {
	[MT8183_SLAVE_DDR_EMI] = &ddr_emi,  [MT8183_MASTER_MCUSYS] = &mcusys,
	[MT8183_MASTER_GPU] = &gpu,	    [MT8183_MASTER_MMSYS] = &mmsys,
	[MT8183_MASTER_MM_VPU] = &mm_vpu,   [MT8183_MASTER_MM_DISP] = &mm_disp,
	[MT8183_MASTER_MM_VDEC] = &mm_vdec, [MT8183_MASTER_MM_VENC] = &mm_venc,
	[MT8183_MASTER_MM_CAM] = &mm_cam,   [MT8183_MASTER_MM_IMG] = &mm_img,
...
static void emi_icc_trace_bw_consumers(struct icc_node *n, bool is_hrt)
{
	...
	hlist_for_each_entry (r, &n->req_list, req_node) {
		if (!r->dev)
			continue;
		if (is_hrt)
			trace_mtk_pm_qos_update_request(150, r->avg_bw / 1000,
							dev_name(r->dev));
		else {
			trace_mtk_pm_qos_update_request(130, r->avg_bw / 1000,
							dev_name(r->dev));
			trace_mtk_pm_qos_update_request(140, r->peak_bw / 1000,
							dev_name(r->dev));
		}
	}
}
...
static int emi_icc_set(struct icc_node *src, struct icc_node *dst)
{
	...
	if (node->ep == 1) {
		...
		mtk_dvfsrc_send_request(src->provider->dev,
					...
					node->sum_avg);
#if IS_ENABLED(CONFIG_MTK_DVFSRC_HELPER)
		trace_mtk_pm_qos_update_request(30, src->avg_bw / 1000,
						src->name);
		trace_mtk_pm_qos_update_request(40, src->peak_bw / 1000,
						src->name);
		...
		mtk_dvfsrc_send_request(src->provider->dev,
					...
					node->sum_avg);
#if IS_ENABLED(CONFIG_MTK_DVFSRC_HELPER)
		trace_mtk_pm_qos_update_request(50, src->avg_bw / 1000,
						src->name);
