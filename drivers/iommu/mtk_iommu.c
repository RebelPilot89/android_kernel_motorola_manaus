...
#define F_INT_MMU_MAU_INT_EN(MMU, MAU, MAUCNT)                                 \
	BIT(14 + (MMU) * (MAUCNT) + (MAU))
...
#define F_INT_MMU_MAU_INT_STA(MMU, MAU, MAUCNT)                                \
	BIT(14 + (MMU) * (MAUCNT) + (MAU))
...
#define MTK_IOMMU_HAS_FLAG(pdata, _x) ((((pdata)->flags) & (_x)) == (_x))
...
enum iova_type { NORMAL, PROTECTED, SECURE, IOVA_TYPE };
...
static struct mtk_iommu_iova_region mt6855_multi_dom[] __maybe_unused = {
	{ .iova_base = SZ_4G,
	  .size = (SZ_4G * 3),
	  .type = NORMAL }, /*0, NORMAL */
	{ .iova_base = SZ_4K,
	  .size = (SZ_4G * 1 - SZ_4K - SZ_64M),
	  .type = NORMAL }, /* 1,CAM_MDP */
	{ .iova_base = 0xFC000000ULL,
	  .size = (SZ_64M),
	  .type = PROTECTED }, /* 2,AIE_RESV_64M */
	{ .iova_base = 0x106000000ULL,
	  .size = SZ_32M,
	  .type = NORMAL }, /* 3,LK_RESV:32MB */
	{ .iova_base = 0x110000000ULL,
	  .size = SZ_512M,
	  .type = PROTECTED }, /* 4,VDO_UP_512MB_1 */
	{ .iova_base = 0x130000000ULL,
	  .size = SZ_512M,
	  .type = PROTECTED }, /* 5,VDO_UP_512MB_2 */
	{ .iova_base = 0x150000000ULL,
	  .size = SZ_256M,
	  .type = PROTECTED }, /* 6,VDO_UP_256MB_1 */
	{ .iova_base = 0x160000000ULL,
	  .size = SZ_256M,
	  .type = PROTECTED }, /* 7,VDO_UP_256MB_1 */
	{ .iova_base = 0x170000000ULL,
	  .size = 0x90000000,
	  .type = NORMAL }, /* 8,VDEC */
};
...
static struct mtk_iommu_iova_region mt6879_multi_dom_mm[] __maybe_unused = {
	...
	{ .iova_base = 0x106000000ULL,
	  .size = SZ_32M,
	  .type = PROTECTED }, /* 1,LK_RESV:32MB */
	{ .iova_base = 0x108000000ULL,
	  .size = SZ_64M,
	  .type = PROTECTED }, /* 2,CCU0:64MB */
	{ .iova_base = 0x10C000000ULL,
	  .size = SZ_64M,
	  .type = PROTECTED }, /* 3,CCU1:64MB */
	{ .iova_base = 0x110000000ULL,
	  .size = SZ_512M,
	  .type = PROTECTED }, /* 4,VDO_UP_512MB_1 */
	{ .iova_base = 0x130000000ULL,
...
static struct mtk_iommu_iova_region mt6879_multi_dom_apu[] __maybe_unused = {
	{ .iova_base = SZ_4K,
	  .size = (SZ_4G * 4 - SZ_4K),
	  .type = NORMAL }, /*0. APU_DATA */
	{ .iova_base = SZ_4K,
	  .size = (SZ_512M - SZ_4K),
	  .type = SECURE }, /* 1,APU_SECURE:512M */
	{ .iova_base = SZ_1G,
	  .size = (SZ_1G * 3ULL),
	  .type = NORMAL }, /* 2,APU_CODE:3GB */
	{ .iova_base = 0x106000000ULL,
	  .size = SZ_32M,
	  .type = NORMAL }, /* 3,LK_RESV:32MB */
};
...
static struct mtk_iommu_iova_region mt6895_multi_dom_mm[] __maybe_unused = {
	{ .iova_base = SZ_4K,
	  .size = (SZ_4G * 4 - SZ_4K),
	  .type = NORMAL }, /*0. NORMAL */
	{ .iova_base = 0x20000000ULL,
	  .size = 0x12c00000,
	  .type = NORMAL }, /* 1,VDEC 300MB */
	{ .iova_base = 0x106000000ULL,
	  .size = SZ_32M,
	  .type = NORMAL }, /* 2,LK_RESV:32MB */
	{ .iova_base = 0x108000000ULL,
	  .size = SZ_64M,
	  .type = PROTECTED }, /* 3,CCU0:64MB */
	{ .iova_base = 0x10C000000ULL,
	  .size = SZ_64M,
	  .type = PROTECTED }, /* 4,CCU1:64MB */
	{ .iova_base = 0x110000000ULL,
	  .size = SZ_512M,
	  .type = PROTECTED }, /* 5,VDO_UP_512MB_1 */
	{ .iova_base = 0x130000000ULL,
	  .size = SZ_512M,
	  .type = PROTECTED }, /* 6,VDO_UP_512MB_2 */
	{ .iova_base = 0x150000000ULL,
	  .size = SZ_256M,
	  .type = PROTECTED }, /* 7,VDO_UP_256MB_1 */
	{ .iova_base = 0x160000000ULL,
	  .size = SZ_256M,
	  .type = PROTECTED }, /* 8,VDO_UP_256MB_1 */
};
			   ...
			   u64 *pa, u32 *type, u32 *lvl)
{
	...
	pa[NS_TAB] = (FIELD_GET(GENMASK(2, 0), ns_res) << 32) |
		     (ns_res & (~(PAGE_SIZE - 1)));
	...
	pa[PROT_TAB] = (FIELD_GET(GENMASK(2, 0), prot_res) << 32) |
		       (prot_res & (~(PAGE_SIZE - 1)));
...
static bool mtee_hypmmu_type2_enabled(void)
{
	...
	arm_smccc_smc(HYP_PMM_GET_HYPMMU_TYPE2_EN, 0, 0, 0, 0, 0, 0, 0,
		      &smc_res);
...
static int mtee_hypmmu_type2_inv(unsigned long iova, size_t size, int tab_id)
{
	...
	arm_smccc_smc(HYP_PMM_HYPMMU_TYPE2_INV, sa, ea, tab_id, 0, 0, 0, 0,
		      &smc_res);
	if (smc_res.a0) {
...
static int mtee_hypmmu_reg_share_region(u32 base_page_no, u32 size_in_pages,
					u32 tab_id)
{
	struct arm_smccc_res smc_res;

	arm_smccc_smc(HYP_PMM_REG_HYPMMU_SHARE_REGION, base_page_no,
		      size_in_pages, tab_id, 0, 0, 0, 0, &smc_res);
...
static int iova_is_secure(struct mtk_iommu_data *data, unsigned long iova,
			  size_t size)
{
	int i;
	...
	for (i = 0; i < data->plat_data->iova_region_nr; i++) {
		...
		if (iova >= region->iova_base &&
		    iova_end < (region->iova_base + region->size) &&
				  ...
				  unsigned long enable)
{
	...
	if (enable) {
		...
		regval = F_INT_TRANSLATION_FAULT | F_INT_MAIN_MULTI_HIT_FAULT |
			 ...
			 F_INT_ENTRY_REPLACEMENT_FAULT | F_INT_TLB_MISS_FAULT |
			 F_INT_MISS_TRANSACTION_FIFO_FAULT;

		if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOMMU_MAU_EN))
			regval |= F_INT_MAIN_MAU_INT_EN(
				data->plat_data->mau_count);
...
static inline void mtk_iommu_isr_setup(struct mtk_iommu_data *data,
				       unsigned long enable)
{
	bool has_pm = !!data->dev->pm_domain;
...
{
	struct mtk_iommu_data *data =
		from_timer(data, t, iommu_isr_pause_timer);
...
static void mtk_iommu_isr_record(struct mtk_iommu_data *data)
{
	...
	if (!data->isr_cnt ||
	    time_after(jiffies, data->first_jiffies + data->isr_cnt * HZ)) {
		data->isr_cnt = 1;
...
static void mtk_iommu_tlb_flush_check(struct mtk_iommu_data *data, bool range)
{
	...
	if (range && (tlb_en & F_MMU_INV_RANGE)) {
		...
		pr_warn("%s, dump: 0x20:0x%x, 0x12c:0x%x\n", __func__,
			readl_relaxed(data->base + REG_MMU_INVALIDATE),
...
static void mtk_iommu_tlb_flush_all(struct mtk_iommu_data *data)
{
	...
	for_each_m4u(data, head)
	{
		...
		if (has_pm &&
			...
			if ((data->plat_data->iommu_type == MM_IOMMU &&
			     pd_sta[data->plat_data->iommu_id] ==
				     POWER_OFF_STA) ||
			    (data->plat_data->iommu_type != MM_IOMMU &&
			     pm_runtime_get_if_in_use(data->dev) <= 0)) {
		...
		if (MTK_IOMMU_HAS_FLAG(data->plat_data, TLB_SYNC_EN)) {
			...
			if (!sync_en) {
				pr_info("skip flush all polling, 0x%x, (%d, %d)\n",
					ctrl_reg, data->plat_data->iommu_type,
					data->plat_data->iommu_id);
				goto skip_polling;
			}
			/* tlb sync */
			ret = readl_poll_timeout_atomic(
				data->base + REG_MMU_CPE_DONE, tmp, tmp != 0,
				10, 1000);
			if (ret) {
	...
	for_each_m4u(data, head)
	{
		...
		if (ret) {
			pr_warn("Partial TLB flush timed out, (%d, %d), iova:0x%llx,0x%zx\n",
				data->plat_data->iommu_type,
				data->plat_data->iommu_id, iova, size);
			if (MTK_IOMMU_HAS_FLAG(data->plat_data, TLB_SYNC_EN))
...
static void __maybe_unused mtk_iommu_dump_tf_iova(struct mtk_iommu_data *data,
						  enum iommu_bank bank,
						  u64 fault_iova)
{
	...
	for (i = 0, tf_iova_tmp = fault_iova; i < MTK_IOMMU_TF_IOVA_DUMP_NUM;
	     i++) {
		if (i > 0)
			tf_iova_tmp -= SZ_4K;
		fake_pa = mtk_iommu_iova_to_phys(&data->m4u_dom->domain,
						 tf_iova_tmp);
		if (hypmmu_type2_en == true)
			hw_pa[NS_TAB] =
				mtee_iova_to_phys(tf_iova_tmp,
		...
		pr_err("error, type2_en:%d, index:%d, lvl:%u, pg_type:0x%x, falut_iova:0x%lx, fault_pa:0x%llx ~ 0x%llx\n",
		       hypmmu_type2_en, i, lvl[NS_TAB], pg_type[NS_TAB],
		       tf_iova_tmp, (u64)fake_pa, hw_pa[NS_TAB]);
		if (!fake_pa && i > 0)
			break;
	}
...
static int mtk_iommu_mau_dump_status(struct mtk_iommu_data *data, int slave,
				     int mau, bool aee_dump);
#endif

static void mtk_iommu_isr_other(struct mtk_iommu_data *data, u32 int_state0,
				u32 int_state1)
{
	enum mtk_iommu_type type = data->plat_data->iommu_type;
	int id = data->plat_data->iommu_id;
	...
	if (int_state0 & F_INT_TBW_FAULT) {
		...
		if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOVA_34_EN)) {
			va_33_32 = FIELD_GET(F_L2_TBWALK_FAULT_VA_33_32_MASK,
					     fault_iova);
			fault_iova =
				fault_iova & F_L2_TBWALK_FAULT_VA_31_12_MASK;
	...
	if ((int_state1 & F_INT_TRANSLATION_FAULT) == 0) {
		if ((int_state1 & F_REG_MMU0_FAULT_MASK) ||
			...
			pr_notice(
				"%s, iommu:(%d,%d)_%d int_state1:0x%x happens\n",
	...
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOMMU_MAU_EN)) {
		...
		if (int_state1 & F_REG_MMU_MAU_INT_MASK(slave_id, mau_count)) {
			pr_notice(
				"%s, iommu:(%d,%d)_%d int_state1:0x%x mau int happens\n",
				__func__, type, id, slave_id, int_state1);
			for (mau = 0; mau < mau_count; mau++) {
				if (int_state1 &
				    F_INT_MMU_MAU_INT_STA(slave_id, mau,
							  mau_count)) {
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_SECURE)
					mtk_iommu_mau_dump_status(
						data, slave_id, mau, false);
#endif
				}
			}
		}
...
static irqreturn_t mtk_iommu_isr(int irq, void *dev_id)
{
	...
	if (ret)
		pr_notice(
			"%s, iommu:(%d,%d) failed to enable secure debug:%d\n",
	...
	if (int_state1 & F_INT_TRANSLATION_FAULT) {
		...
		pr_info("%s, iommu:(%d,%d) reg_raw_data: int_status:0x%x,0x%x, int_id:0x%x, int_va:0x%llx, int_pa:0x%llx\n",
			__func__, type, id, int_state0, int_state1, regval,
			fault_iova, fault_pa);

		if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOVA_34_EN)) {
			va34_32 = FIELD_GET(F_MMU_INVAL_VA_34_32_MASK,
					    fault_iova);
			fault_iova = fault_iova & F_MMU_INVAL_VA_31_12_MASK;
			fault_iova |= (u64)va34_32 << 32;
		}
		...
		pr_info("%s base:0x%x fault type=0x%x iova=0x%llx pa=0x%llx layer=%d %s, pgtable=0x%x, tfrp_pa=0x%llx, 0x114=0x%x\n",
			dev_name(dev), table_base, int_state1, fault_iova,
			fault_pa, layer, (write ? "write" : "read"),
			dom->cfg.arm_v7s_cfg.ttbr, data->protect_base,
			readl_relaxed(base + REG_MMU_IVRP_PADDR));
		mtk_iommu_dump_tf_iova(data, IOMMU_BK0, fault_iova);
		report_custom_iommu_fault(fault_iova, fault_pa, regval, type,
					  id);
#else
		...
		if (report_iommu_fault(&dom->domain, data->dev, fault_iova,
			...
			dev_err_ratelimited(
				...
				int_state1, fault_iova, fault_pa, fault_larb,
				fault_port, layer, write ? "write" : "read");
		}
#endif
	}
	...
	if (ret)
		pr_notice(
			"%s, iommu:(%d,%d) failed to disable secure debug:%d\n",
			__func__, type, id, ret);
	...
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, GET_DOM_ID_LEGACY)) {
		...
		if (domid >= plat_data->iova_region_nr) {
			dev_err(dev, "iommu domain id(%d/%d) is error.\n",
				domid, plat_data->iova_region_nr);
			return -EINVAL;
		}
	...
	for (i = 0; i < plat_data->iova_region_nr; i++) {
		...
		if (region->type == PROTECTED) {
			...
			pr_info("%s: tab_id:%d, reg base=0x%x size=0x%x\n",
				__func__, plat_data->tab_id, base_page_no,
				size_in_pages);
			mtee_hypmmu_reg_share_region(
				base_page_no, size_in_pages, plat_data->tab_id);
		}
	}
}
				     ...
				     unsigned int domid)
{
	...
	for_each_m4u(data_temp, head)
	{
		...
		if (data_temp->m4u_dom) {
			...
			dom->domain.pgsize_bitmap =
				data_temp->m4u_dom->cfg.pgsize_bitmap;
			goto update_iova_region;
		}
	}

	dom->cfg = (struct io_pgtable_cfg){
		.quirks = IO_PGTABLE_QUIRK_ARM_NS | IO_PGTABLE_QUIRK_NO_PERMS |
...
update_iova_region:
	...
	dom->domain.geometry.aperture_end =
		region->iova_base + region->size - 1;
	dom->domain.geometry.force_aperture = true;

	pr_info("%s, dom:%u, start:%pa, end:%pa, table:0x%x\n", __func__, domid,
		&dom->domain.geometry.aperture_start,
		&dom->domain.geometry.aperture_end, dom->cfg.arm_v7s_cfg.ttbr);
	return 0;
}
				   ...
				   struct device *dev)
{
	...
	if (!data->m4u_dom) { /* Initialize the M4U HW */
		...
		pr_info("%s, iommu_dev:%s(%d,%d), user_dev:%s, pgtable:0x%lx -- 0x%x -- 0x%x, tab_id:%d\n",
			...
			data->plat_data->iommu_type, data->plat_data->iommu_id,
			dev_name(dev), (unsigned long)dom->cfg.arm_v7s_cfg.ttbr,
...
static size_t mtk_iommu_unmap(struct iommu_domain *domain, unsigned long iova,
				 ...
				 struct iommu_iotlb_gather *gather)
{
	...
	if (!hypmmu_type2_en) {
		ret = iova_secure_map(dom->data, gather->start, length,
				      false); /* clean bank1 */
		if (ret)
			pr_warn("%s failed\n", __func__);
	} else {
...
static struct iommu_group *mtk_iommu_device_group(struct device *dev)
{
	...
	if (!group) {
		pr_info("%s create group, data:(%d,%d)-->(%d,%d), dev:%s, domid:%d\n",
			__func__, c_data->plat_data->iommu_type,
			c_data->plat_data->iommu_id,
			data->plat_data->iommu_type, data->plat_data->iommu_id,
			dev_name(dev), domid);
...
static int mtk_iommu_hw_init(const struct mtk_iommu_data *data)
{
	...
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, SET_TBW_ID))
		writel_relaxed(data->plat_data->tbw_reg_val,
			       data->base + REG_MMU_TBW_ID);
	...
	for (i = IOMMU_BK0; i < IOMMU_BK_NUM; i++) {
		...
		if (devm_request_irq(dev, irq, mtk_iommu_isr, 0, dev_name(dev),
				     (void *)data)) {
			if (i == IOMMU_BK0)
				writel_relaxed(0, data->base +
...
static int mtk_iommu_pd_callback(struct notifier_block *nb, unsigned long flags,
				 void *data)
{
				       ...
				       const struct mau_config_info *mau_cfg)
{
	...
	/*enable interrupt*/
	iommu_set_field_by_mask(base, REG_MMU_INT_MAIN_CONTROL,
				F_INT_MMU_MAU_INT_EN(slave, mau, mau_count),
				F_INT_MMU_MAU_INT_EN(slave, mau, mau_count));

	/*config start addr*/
	writel_relaxed(mau_cfg->start, base + REG_MMU_MAU_SA(slave, mau));
	writel_relaxed(mau_cfg->start_bit32,
		       base + REG_MMU_MAU_SA_EXT(slave, mau));

	/*config end addr*/
	writel_relaxed(mau_cfg->end, base + REG_MMU_MAU_EA(slave, mau));
	writel_relaxed(mau_cfg->end_bit32,
		       base + REG_MMU_MAU_EA_EXT(slave, mau));

	/*config larb id*/
	writel_relaxed(mau_cfg->larb_mask,
		       base + REG_MMU_MAU_LARB_EN(slave, mau));

	/*config port id*/
	writel_relaxed(mau_cfg->port_mask,
		       base + REG_MMU_MAU_PORT_EN(slave, mau));
	...
	pr_notice(
		"%s, iommu:(%d,%d), slave:%d, mau:%d, mau_reg: start=0x%x(0x%x), end=0x%x(0x%x), wr:0x%x, virt:0x%x, io:0x%x, larb:0x%x, port:0x%x\n",
		__func__, type, id, slave, mau,
				    ...
				    struct mau_config_info *mau_cfg)
{
	...
	mau_cfg->io = !!(
		iommu_get_field_by_mask(base, REG_MMU_MAU_IO(slave, mau_count),
					F_MMU_MAU_BIT_VAL(1, mau)));

	mau_cfg->wr = !!(
		iommu_get_field_by_mask(base, REG_MMU_MAU_RW(slave, mau_count),
					F_MMU_MAU_BIT_VAL(1, mau)));

	mau_cfg->virt = !!(
		iommu_get_field_by_mask(base, REG_MMU_MAU_VA(slave, mau_count),
					F_MMU_MAU_BIT_VAL(1, mau)));
				     ...
				     int mau, bool aee_dump)
{
	...
	if (!mtk_iommu_power_get(data)) {
		pr_notice(
			"%s, iommu:(%d,%d), slave:%d, mau:%d, power off dev:%s\n",
	...
	if (status & (1 << mau)) {
		pr_notice(
			"%s: iommu:(%d,%d), slave:%d, mau_assert:%d, status:0x%x\n",
			__func__, type, id, slave, mau, status);

		assert_id =
			readl_relaxed(base + REG_MMU_MAU_ASRT_ID(slave, mau));
		assert_address =
			readl_relaxed(base + REG_MMU_MAU_AA(slave, mau));
		assert_b32 =
			readl_relaxed(base + REG_MMU_MAU_AA_EXT(slave, mau));

		falut_id = (assert_id & F_MMU_MAU_ASRT_ID_VAL) << 2;
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_DBG)
		port_name = mtk_iommu_get_port_name(type, id, falut_id);
		if (aee_dump)
			report_iommu_mau_fault(assert_id, falut_id,
					       (port_name ? port_name :
							    "port_unknown"),
					       assert_address, assert_b32);
#endif
		pr_notice(
			"%s: ASRT_ID=0x%x, FALUT_ID=0x%x(%s), AA=0x%x, AA_EXT=0x%x\n",
			__func__, assert_id, falut_id,
			(port_name ? port_name : "port_unknown"),
			assert_address, assert_b32);

		/* Clear bits for MAU set */
		writel_relaxed((1 << mau),
			       base + REG_MMU_MAU_CLR(slave, mau_count));
		...
		if (!ret) {
			pr_info("%s, iommu:(%d,%d), slave:%d, mau:%d, mau_cfg: start=0x%x(0x%x), end=0x%x(0x%x), wr:0x%x, virt:0x%x, io:0x%x, larb:0x%x, port:0x%x\n",
				__func__, type, id, slave, mau, mau_cfg.start,
				mau_cfg.start_bit32, mau_cfg.end,
				mau_cfg.end_bit32, mau_cfg.wr, mau_cfg.virt,
				mau_cfg.io, mau_cfg.larb_mask,
				mau_cfg.port_mask);
		}
	} else
...
static int mtk_iommu_mau_reg_restore(struct mtk_iommu_data *data)
{
	...
	for (slave = 0; slave < MTK_IOMMU_MMU_COUNT; slave++) {
		for (mau = 0; mau < mau_count; mau++) {
			writel_relaxed(*(p_reg++),
				       base + REG_MMU_MAU_SA(slave, mau));
			writel_relaxed(*(p_reg++),
				       base + REG_MMU_MAU_SA_EXT(slave, mau));
			writel_relaxed(*(p_reg++),
				       base + REG_MMU_MAU_EA(slave, mau));
			writel_relaxed(*(p_reg++),
				       base + REG_MMU_MAU_EA_EXT(slave, mau));
			writel_relaxed(*(p_reg++),
				       base + REG_MMU_MAU_LARB_EN(slave, mau));
		...
		writel_relaxed(*(p_reg++),
			       base + REG_MMU_MAU_IO(slave, mau_count));
		writel_relaxed(*(p_reg++),
			       base + REG_MMU_MAU_RW(slave, mau_count));
		writel_relaxed(*(p_reg++),
			       base + REG_MMU_MAU_VA(slave, mau_count));
	}
...
static void mtk_iommu_mau_init(struct mtk_iommu_data *data)
{
	...
	for (slave = 0; slave < MTK_IOMMU_MMU_COUNT; slave++) {
		for (mau = 0; mau < data->plat_data->mau_count; mau++) {
			mau_config =
				mtk_iommu_get_mau_config(type, id, slave, mau);
			if (mau_config != NULL)
				mtk_iommu_mau_start_monitor(data, slave, mau,
							    mau_config);
		}
	}
#endif
...
static int mtk_iommu_probe(struct platform_device *pdev)
{
	...
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOMMU_MAU_EN)) {
		...
		reg->mau = kmalloc(MMU_MAU_REG_BACKUP_SIZE,
	...
	if (!res) {
		dev_err(dev, "failed to get IOMMU memory resource\n");
		return -EINVAL;
	}
...
skip_irq:
	 ...
	 */
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, IOMMU_SEC_BK_EN)) {
		int bk_nr = of_count_phandle_with_args(
			dev->of_node, "mediatek,iommu_banks", NULL);
		...
		for (i = 0; i < bk_nr; i++) {
			...
			ret = of_property_read_u32(bk_node, "mediatek,bank-id",
						   &bk_id);
			if (ret) {
			...
			if (!MTK_IOMMU_HAS_FLAG(data->plat_data,
						IOMMU_NO_IRQ)) {
				data->bk_irq[bk_id] =
					platform_get_irq(bk_dev, 0);
				if (data->bk_irq[bk_id] < 0) {
					dev_err(dev,
						"Get iommu_bank:%d irq fail\n",
						bk_id);
					return data->bk_irq[bk_id];
				}
			}
			data->bk_dev[bk_id] = &bk_dev->dev;
			bk_res[bk_id] = platform_get_resource(
				bk_dev, IORESOURCE_MEM, 0);
			bk_pa = bk_res[bk_id]->start;
			dev_info(
				data->bk_dev[bk_id],
				"Get iommu bank:%u(%s) irq:%d ,pa:%pa, success\n",
				bk_id, dev_name(data->bk_dev[bk_id]),
				data->bk_irq[bk_id], &bk_pa);
		}
	}
out:
	...
	if (data->plat_data->iommu_type == APU_IOMMU &&
		...
		apunode =
			of_parse_phandle(dev->of_node, "mediatek,apu_power", 0);
		if (!apunode) {
	...
	for (i = 0; i < larb_nr; i++) {
		...
		disp_power_on |=
			of_property_read_bool(larbnode, "init-power-on");
	...
	repeat:
		...
		string_nr =
			of_property_count_strings(smicomm_node, "compatible");
		if (string_nr < 0) {
		...
		for (i = 0; i < string_nr; i++) {
			ret = of_property_read_string_index(
				smicomm_node, "compatible", i, &compat_name);
			if (ret) {
				dev_err(dev,
					"%s err, find compatible name fail, ret:%d\n",
					__func__, ret);
				return -EINVAL;
			}
...
skip_smi:
	...
	ret = iommu_device_sysfs_add(&data->iommu, dev, NULL, "mtk-iommu.%pa",
				     &ioaddr);
	if (ret)
	...
	if (data->plat_data->iommu_type == MM_IOMMU) {
		...
		mtk_pd_notifiers[iommu_id].notifier_call =
			mtk_iommu_pd_callback;
...
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_SECURE)
static int mtk_dump_reg(const struct mtk_iommu_data *data, unsigned int start,
			unsigned int length)
{
	int i = 0;
	void __iomem *base = data->base;
...
static int mtk_dump_rs_sta_info(const struct mtk_iommu_data *data, int mmu)
{
	...
	return mtk_dump_reg(data, REG_MMU_RS_VA(mmu, 0),
...
static const struct dev_pm_ops mtk_iommu_pm_ops = { SET_RUNTIME_PM_OPS(
	mtk_iommu_runtime_suspend, mtk_iommu_runtime_resume,
	NULL) SET_LATE_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
					   pm_runtime_force_resume) };

static const struct mtk_iommu_plat_data mt2712_data = {
	.m4u_plat = M4U_MT2712,
	.flags = HAS_4GB_MODE | HAS_BCLK | HAS_VLD_PA_RNG | NOT_STD_AXI_MODE |
		 SHARE_PGTABLE,
...
static const struct mtk_iommu_plat_data mt6833_data = {
	...
	.flags = HAS_SUB_COMM | OUT_ORDER_WR_EN | WR_THROT_EN | HAS_BCLK |
		 ...
		 HAS_SMI_SUB_COMM | IOMMU_SEC_BK_EN,
	.inv_sel_reg = REG_MMU_INV_SEL_GEN2,
	.iommu_id = DISP_IOMMU,
	.iommu_type = MM_IOMMU,
...
static const struct mtk_iommu_plat_data mt6789_data = {
	...
	.flags = HAS_SUB_COMM | OUT_ORDER_WR_EN | GET_DOM_ID_LEGACY |
		 NOT_STD_AXI_MODE | IOVA_34_EN | SHARE_PGTABLE |
		 IOMMU_NO_SMCCC | IOMMU_SEC_BK_EN | HAS_SMI_SUB_COMM |
...
static const struct mtk_iommu_plat_data mt6855_data_disp = {
	...
	.flags = HAS_SUB_COMM | OUT_ORDER_WR_EN | GET_DOM_ID_LEGACY |
		 ...
		 IOMMU_SEC_BK_EN | SKIP_CFG_PORT | IOVA_34_EN | HAS_BCLK |
		 HAS_SMI_SUB_COMM,
	.inv_sel_reg = REG_MMU_INV_SEL_GEN2,
	.iommu_id = DISP_IOMMU,
	.iommu_type = MM_IOMMU,
...
static const struct mtk_iommu_plat_data mt6895_data_disp = {
	...
	.flags = HAS_SUB_COMM | OUT_ORDER_WR_EN | GET_DOM_ID_LEGACY |
		 NOT_STD_AXI_MODE | TLB_SYNC_EN | IOMMU_SEC_BK_EN |
		 SKIP_CFG_PORT | IOVA_34_EN | IOMMU_MAU_EN | HAS_SMI_SUB_COMM |
...
static const struct mtk_iommu_plat_data mt8173_data = {
	.m4u_plat = M4U_MT8173,
	.flags = HAS_4GB_MODE | HAS_BCLK | RESET_AXI | HAS_LEGACY_IVRP_PADDR |
		 SHARE_PGTABLE,
	.inv_sel_reg = REG_MMU_INV_SEL_GEN1,
	.iova_region = single_domain,
	.iova_region_nr = ARRAY_SIZE(single_domain),
...
static const struct mtk_iommu_plat_data mt8192_data = {
	.m4u_plat = M4U_MT8192,
	.flags = HAS_BCLK | HAS_SUB_COMM | OUT_ORDER_WR_EN | WR_THROT_EN |
...
static const struct of_device_id mtk_iommu_of_ids[] = {
	...
	{ .compatible = "mediatek,mt6879-apu-iommu0",
	  .data = &mt6879_data_apu0 },
	{ .compatible = "mediatek,mt6879-disp-iommu",
	  .data = &mt6879_data_disp },
...
static struct platform_driver
	mtk_iommu_driver = { .probe = mtk_iommu_probe,
