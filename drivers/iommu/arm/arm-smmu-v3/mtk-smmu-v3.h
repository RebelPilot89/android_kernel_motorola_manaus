...
#define MTK_SMMU_HAS_FLAG(pdata, _x) ((((pdata)->flags) & (_x)) == (_x))
...
#define SMMU_TBU_CNT(id)                                                       \
	((id) == SOC_SMMU ? (SMMU_TBU_CNT_MAX - 1) : SMMU_TBU_CNT_MAX)
...
enum mtk_smmu_type { MM_SMMU, APU_SMMU, SOC_SMMU, GPU_SMMU, SMMU_TYPE_NUM };

enum mm_smmu { MM_SMMU_MDP, MM_SMMU_DISP, MM_SMMU_NUM };

enum apu_smmu { APU_SMMU_M0, APU_SMMU_NUM };

enum soc_smmu { SOC_SMMU_M4, SOC_SMMU_M6, SOC_SMMU_M7, SOC_SMMU_NUM };
...
enum mtk_smmu_tfm_type { SMMU_TFM_READ, SMMU_TFM_WRITE, SMMU_TFM_TYPE_NUM };
...
struct mtk_iommu_fault_event {
	struct mtk_smmu_fault_param mtk_fault_param[SMMU_TFM_TYPE_NUM]
						   [SMMU_TBU_CNT_MAX];
...
static inline struct mtk_smmu_data *
to_mtk_smmu_data(struct arm_smmu_device *smmu)
{
...
static inline int mtk_smmu_set_pm_ops(u32 smmu_type,
				      const struct mtk_pm_ops *ops)
{
...
void mtk_smmu_reg_dump(enum mtk_smmu_type type, struct device *master_dev,
		       int sid);
int mtk_smmu_tf_detect(enum mtk_smmu_type type, struct device *master_dev,
		       u32 sid, u32 tbu, u32 *axids, u32 num_axids,
		       struct mtk_smmu_fault_param *param);
#else
static inline void mtk_smmu_reg_dump(enum mtk_smmu_type type,
				     struct device *master_dev, int sid)
{
}

static inline int mtk_smmu_tf_detect(enum mtk_smmu_type type,
				     struct device *master_dev, u32 sid,
				     u32 tbu, u32 *axids, u32 num_axids,
				     struct mtk_smmu_fault_param *param)
{
	return 0;
}
...
#else
static inline int
mtk_smmu_register_pmu_device(struct smmuv3_pmu_device *pmu_device)
{
	return 0;
}

static inline void
mtk_smmu_unregister_pmu_device(struct smmuv3_pmu_device *pmu_device)
{
}
