...
unsigned char g_rpmb_key[RPMB_SZ_KEY] = {
	0x64, 0x76, 0xEE, 0xF0, 0xF1, 0x6B, 0x30, 0x47, 0xE9, 0x79, 0x31,
	0x58, 0xF6, 0x42, 0xDA, 0x46, 0xF7, 0x3B, 0x53, 0xFD, 0xC5, 0xF8,
	0x84, 0xCE, 0x03, 0x73, 0x15, 0xBC, 0x54, 0x47, 0xD4, 0x6A
};

static int rpmb_cal_hmac(struct rpmb_frame *frame, int blk_cnt, u8 *key,
			 u8 *key_mac)
{
	int i;
	u8 *buf, *buf_start;
	...
	if (hmac_sha256(key, RPMB_SZ_KEY, buf_start, RPMB_SZ_CAL_HMAC * blk_cnt,
			key_mac) != 0)
...
static int nl_rpmb_cmd_req(const struct rpmb_data *rpmbd)
{
	...
	case RPMB_WRITE_DATA:
		...
		memcpy(write_buf, rpmbd->icmd.frames, req->reliable_write_size);
	...
	ret = wait_event_timeout(wait_rpmb, rpmb_done_flag,
...
static int rpmb_req_ioctl_write_data_ufs(struct rpmb_ioc_param *param)
{
	...
	while (left_blkcnt) {
		...
		for (iCnt = 0; iCnt < tran_blkcnt; iCnt++) {
			...
			ret = copy_from_user(
				rpmbdata.icmd.frames[iCnt].data,
				param->databytes + (i * MAX_RPMB_TRANSFER_BLK *
							    RPMB_SZ_DATA +
						    (iCnt * RPMB_SZ_DATA)),
				tran_size);
			if (ret) {
...
/*
		...
		if (hmac_sha256(rpmb_key, RPMB_SZ_KEY,
				rpmbdata.ocmd.frames->data, RPMB_SZ_CAL_HMAC,
				hmac) != 0)
			MSG(ERR, "hmac_sha256() return error!\n");

		if (memcmp(hmac, rpmbdata.ocmd.frames->key_mac, RPMB_SZ_MAC) !=
		    0) {
			MSG(ERR, "%s, hmac compare error!!!\n", __func__);
			ret = RPMB_HMAC_ERROR;
			break;
...
static int rpmb_req_ioctl_read_data_ufs(struct rpmb_ioc_param *param)
{
	...
	while (left_blkcnt) {
		 ...
		 */
		if (hmac_sha256(rpmb_key, RPMB_SZ_KEY, dataBuf_start,
				size_for_hmac * tran_blkcnt, hmac) != 0)
			MSG(ERR, "hmac_sha256() return error!\n");
...
static int emmc_rpmb_send_command(struct mmc_card *card, u8 *buf, __u16 blks,
...
int emmc_rpmb_req_start(struct mmc_card *card, struct emmc_rpmb_req *req)
{
	...
	if (type == RPMB_WRITE_DATA)
		err = emmc_rpmb_send_command(card, data_frame, blks, type,
					     RPMB_REQ);
	else
		err = emmc_rpmb_send_command(card, data_frame, 1, type,
					     RPMB_REQ);
				   ...
				   struct rpmb_ioc_param *param)
{
	...
	blkaddr = param->addr;
	write_blks_one_time =
		MIN(MAX_RPMB_TRANSFER_BLK, card->ext_csd.rel_sectors * 2);
	while (left_blkcnt) {
		...
		hmac_sha256(param->keybytes, 32, dataBuf_start,
			    284 * tran_blkcnt, rpmb_frame[iCnt].mac);
	...
	for (iCnt = 0; iCnt < total_blkcnt; iCnt++) {
		...
		memcpy(rpmb_frame->data, param->data + iCnt * RPMB_SZ_DATA,
		       tran_size);
				  ...
				  struct rpmb_ioc_param *param)
{
		...
		for (iCnt = 0; iCnt < tran_blkcnt; iCnt++) {
			...
			memcpy(param->databytes +
				       i * MAX_RPMB_TRANSFER_BLK *
					       RPMB_SZ_DATA +
				       (iCnt * RPMB_SZ_DATA),
			       rpmb_frame[iCnt].data, tran_size);
			left_size -= tran_size;
		}
	...
	for (iCnt = 0; iCnt < total_blkcnt; iCnt++) {
		...
		memcpy(param->data + RPMB_SZ_DATA * iCnt, rpmb_frame->data,
		       tran_size);
...
static enum mc_result rpmb_gp_open_session(void)
{
	...
	do {
		...
		MSG(INFO, "uuid[0]=%d, uuid[1]=%d, uuid[2]=%d, uuid[3]=%d\n",
		    rpmb_gp_uuid.value[0], rpmb_gp_uuid.value[1],
		    rpmb_gp_uuid.value[2], rpmb_gp_uuid.value[3]);

		rpmb_gp_session.device_id = rpmb_gp_devid;

		/* open session */
		mc_ret = mc_open_session(&rpmb_gp_session, &rpmb_gp_uuid,
					 (uint8_t *)rpmb_gp_dci,
					 (u32)sizeof(struct dciMessage_t));
		...
		rpmb_gp_Dci_th =
			kthread_run(rpmb_gp_listenDci, NULL, "rpmb_gp_Dci");
...
#if IS_ENABLED(CONFIG_SCSI_UFS_MEDIATEK)
static long rpmb_ioctl_ufs(struct file *pfile, unsigned int cmd,
			   unsigned long arg)
{
	int err = 0;
	unsigned long n;
	...
	case RPMB_IOCTL_READ_DATA:
		...
		if (err != 0) {
			MSG(ERR,
			    "%s, rpmb_req_ioctl_read_data_ufs() error!(%x)\n",
			    __func__, err);
			break;
		}
	...
	case RPMB_IOCTL_WRITE_DATA:
		...
		if (err != 0)
			MSG(ERR,
			    "%s, rpmb_req_ioctl_write_data_ufs() error!(%x)\n",
			    __func__, err);
...
static int rpmb_create_netlink(void)
{
	...
	rpmb_mtk_sock =
		netlink_kernel_create(&init_net, NETLINK_RPMB_MTK, &cfg);
...
static int dt_get_boot_type(void)
{
	...
	if (node) {
		tags = (struct tag_bootmode *)of_get_property(node, "atag,boot",
							      (int *)&size);
	} else
...
#if IS_ENABLED(CONFIG_TRUSTONIC_TEE_SUPPORT)
	mobicore_node =
		of_find_compatible_node(NULL, NULL, "trustonic,mobicore");
	if (!mobicore_node) {
		MSG(ERR, "find trustonic,mobicore fail\n");
		goto fake_out;
	}

	ret = of_property_read_u32(mobicore_node, "trustonic,real-drv",
				   &real_drv);
	if (ret || !real_drv) {
		MSG(INFO, "MobiCore dummy driver, ret=%d, real_drv=%d", ret,
		    real_drv);
		goto fake_out;
	}
