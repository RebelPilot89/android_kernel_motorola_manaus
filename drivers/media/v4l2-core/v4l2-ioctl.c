...
static void v4l_print_fmtdesc(const void *arg, bool write_only)
{
	...
	pr_cont("index=%u, type=%s, flags=0x%x, pixelformat=%c%c%c%c, mbus_code=0x%04x, description='%.*s'\n",
		...
		p->mbus_code, (int)sizeof(p->description), p->description);
}
...
static void v4l_print_buffer(const void *arg, bool write_only)
{
	...
	pr_cont("%02d:%02d:%02d.%09ld index=%d, type=%s, request_fd=%d, flags=0x%08x, field=%s, sequence=%d, memory=%s",
		...
		p->index, prt_names(p->type, v4l2_type_names), p->request_fd,
		p->flags, prt_names(p->field, v4l2_field_names), p->sequence,
		prt_names(p->memory, v4l2_memory_names));
...
static void v4l_print_ext_controls(const void *arg, bool write_only)
{
	...
	pr_cont("which=0x%x, count=%d, error_idx=%d, request_fd=%d", p->which,
		p->count, p->error_idx, p->request_fd);
	for (i = 0; i < p->count; i++) {
...
static bool check_ext_ctrls(struct v4l2_ext_controls *c, unsigned long ioctl)
{
	...
	for (i = 0; i < c->count; i++) {
		if (V4L2_CTRL_ID2WHICH(c->controls[i].id) != c->which) {
			c->error_idx =
				ioctl == VIDIOC_TRY_EXT_CTRLS ? i : c->count;
			return false;
		}
	}
	return true;
...
static int check_fmt(struct file *file, enum v4l2_buf_type type)
{
	...
	const u32 meta_caps = V4L2_CAP_META_CAPTURE | V4L2_CAP_META_OUTPUT;
...
static void v4l_sanitize_format(struct v4l2_format *fmt)
{
	...
	if (fmt->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE ||
		...
		fmt->fmt.pix_mp.num_planes = min_t(
			u32, fmt->fmt.pix_mp.num_planes, VIDEO_MAX_PLANES);
...
static void v4l_fill_fmtdesc(struct v4l2_fmtdesc *fmt)
{
	...
	case V4L2_PIX_FMT_CNF4:
		descr = "4-bit Depth Confidence (Packed)";
		break;
	...
	case V4L2_PIX_FMT_AYUV32:
		descr = "32-bit AYUV 8-8-8-8";
		break;
	case V4L2_PIX_FMT_XYUV32:
		descr = "32-bit XYUV 8-8-8-8";
		break;
	case V4L2_PIX_FMT_VUYA32:
		descr = "32-bit VUYA 8-8-8-8";
		break;
	case V4L2_PIX_FMT_VUYX32:
		descr = "32-bit VUYX 8-8-8-8";
		break;
	...
	case V4L2_PIX_FMT_SBGGR14:
		descr = "14-bit Bayer BGBG/GRGR";
		break;
	case V4L2_PIX_FMT_SGBRG14:
		descr = "14-bit Bayer GBGB/RGRG";
		break;
	case V4L2_PIX_FMT_SGRBG14:
		descr = "14-bit Bayer GRGR/BGBG";
		break;
	case V4L2_PIX_FMT_SRGGB14:
		descr = "14-bit Bayer RGRG/GBGB";
		break;
	...
	case V4L2_META_FMT_D4XX:
		descr = "Intel D4xx UVC Metadata";
		break;
	case V4L2_META_FMT_VIVID:
		descr = "Vivid Metadata";
		break;
	...
	default:
		...
		case V4L2_PIX_FMT_MPEG2_SLICE:
			descr = "MPEG-2 Parsed Slice Data";
			break;
		...
		case V4L2_PIX_FMT_HEVC_SLICE:
			descr = "HEVC Parsed Slice Data";
			break;
		case V4L2_PIX_FMT_FWHT:
			descr = "FWHT";
			break; /* used in vicodec */
		case V4L2_PIX_FMT_FWHT_STATELESS:
			descr = "FWHT Stateless";
			break; /* used in vicodec */
			   ...
			   void *fh, void *arg)
{
	...
	return check_ext_ctrls(p, VIDIOC_S_EXT_CTRLS) ?
		       ops->vidioc_s_ext_ctrls(file, fh, p) :
		       -EINVAL;
}
			     ...
			     struct file *file, void *fh, void *arg)
{
	...
	return check_ext_ctrls(p, VIDIOC_TRY_EXT_CTRLS) ?
		       ops->vidioc_try_ext_ctrls(file, fh, p) :
		       -EINVAL;
}
...
#define INFO_FL_CLEAR(v4l2_struct, field)                                      \
	((offsetof(struct v4l2_struct, field) +                                \
	  sizeof_field(struct v4l2_struct, field))                             \
	 << 16)
	...
	default:
		...
		if (v4l2_is_known_ioctl(cmd)) {
			...
			*always_copy = flags & INFO_FL_ALWAYS_COPY;
			trace_android_vh_clear_mask_adjust(
				v4l2_ioctls[_IOC_NR(cmd)].ioctl, &n);
		}
