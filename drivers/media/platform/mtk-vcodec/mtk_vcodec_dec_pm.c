...
int mtk_vcodec_init_dec_pm(struct mtk_vcodec_dev *mtkdev)
{
	...
	for (larb_index = 0; larb_index < MTK_VDEC_MAX_LARB_COUNT;
	     larb_index++) {
		node = of_parse_phandle(pdev->dev.of_node, "mediatek,larbs",
					larb_index);
		if (!node)
			break;
		...
		mtk_v4l2_debug(2, "larbvdecs[%d] = %p", larb_index,
			       pm->larbvdecs[larb_index]);
		pdev = mtkdev->plat_dev;
	}

	memset(clks_data, 0x00, sizeof(struct mtk_vdec_clks_data));
	while (!of_property_read_string_index(pdev->dev.of_node, "clock-names",
					      clk_id, &clk_name)) {
		mtk_v4l2_debug(2, "init clock, id: %d, name: %s", clk_id,
			       clk_name);
		...
		if (IS_SPECIFIC_CLK_TYPE(clk_name, MTK_VDEC_CLK_MAIN_PREFIX)) {
			clks_data->main_clks[clks_data->main_clks_len].clk_id =
				clk_id;
			clks_data->main_clks[clks_data->main_clks_len].clk_name =
				clk_name;
			clks_data->main_clks_len++;
		} else if (IS_SPECIFIC_CLK_TYPE(clk_name,
						MTK_VDEC_CLK_SOC_PREFIX)) {
			clks_data->soc_clks[clks_data->soc_clks_len].clk_id =
				clk_id;
			clks_data->soc_clks[clks_data->soc_clks_len].clk_name =
				clk_name;
			clks_data->soc_clks_len++;
		} else if (IS_SPECIFIC_CLK_TYPE(clk_name,
						MTK_VDEC_CLK_CORE_PREFIX)) {
			clks_data->core_clks[clks_data->core_clks_len].clk_id =
				clk_id;
			clks_data->core_clks[clks_data->core_clks_len].clk_name =
				clk_name;
			clks_data->core_clks_len++;
		} else if (IS_SPECIFIC_CLK_TYPE(clk_name,
						MTK_VDEC_CLK_LAT_PREFIX)) {
			clks_data->lat_clks[clks_data->lat_clks_len].clk_id =
				clk_id;
			clks_data->lat_clks[clks_data->lat_clks_len].clk_name =
				clk_name;
	...
	for (i = 0; i < clks_data->main_clks_len; i++) {
		mtk_v4l2_debug(8, "main_clks id: %d, name: %s",
			       clks_data->main_clks[i].clk_id,
			       clks_data->main_clks[i].clk_name);
	}
	// dump soc clocks
	for (i = 0; i < clks_data->soc_clks_len; i++) {
		mtk_v4l2_debug(8, "core_clks id: %d, name: %s",
			       clks_data->soc_clks[i].clk_id,
			       clks_data->soc_clks[i].clk_name);
	}
	// dump core clocks
	for (i = 0; i < clks_data->core_clks_len; i++) {
		mtk_v4l2_debug(8, "core_clks id: %d, name: %s",
			       clks_data->core_clks[i].clk_id,
			       clks_data->core_clks[i].clk_name);
	}
	// dump lat clocks
	for (i = 0; i < clks_data->lat_clks_len; i++) {
		mtk_v4l2_debug(8, "lat_clks id: %d, name: %s",
			       clks_data->lat_clks[i].clk_id,
			       clks_data->lat_clks[i].clk_name);
	}
#endif
...
static void mtk_vdec_hw_break(struct mtk_vcodec_dev *dev, int hw_id)
{
	...
	if (hw_id == MTK_VDEC_CORE) {
		...
		if (vdec_vld_addr == NULL || vdec_misc_addr == NULL) {
			mtk_v4l2_debug(
				4,
				"VDEC codec:0x%08x(%c%c%c%c) HW break fail since vdec_vld_addr 0x%lx vdec_misc_addr 0x%lx",
				fourcc, fourcc & 0xFF, (fourcc >> 8) & 0xFF,
				(fourcc >> 16) & 0xFF, (fourcc >> 24) & 0xFF,
				(unsigned long)vdec_vld_addr,
				(unsigned long)vdec_misc_addr);
			return;
		}
		...
		while (((cg_status & 0x11) != 0x11) ||
			...
			if (usec > timeout) {
				mtk_v4l2_err(
					"VDEC HW break timeout. codec:0x%08x(%c%c%c%c) ufo %d",
					fourcc, fourcc & 0xFF,
					(fourcc >> 8) & 0xFF,
					(fourcc >> 16) & 0xFF,
					(fourcc >> 24) & 0xFF, is_ufo);
				if (vdec_gcon_addr > 0) {
					value = readl(vdec_gcon_addr +
						      (0 << 2));
					mtk_v4l2_err(
						"[DEBUG][GCON] 0x%x(%d) = 0x%lx",
						0 << 2, 0, value);
					value = readl(vdec_gcon_addr +
						      (6 << 2));
					mtk_v4l2_err(
				...
				for (offset = 64; offset <= 79; offset++) {
					value = readl(vdec_misc_addr +
						      (offset << 2));
					mtk_v4l2_err(
						"[DEBUG][MISC] 0x%x(%d) = 0x%lx",
	...
	} else if (dev->pm.mtkdev->vdec_hw_ipm == VCODEC_IPM_V2 &&
		   hw_id == MTK_VDEC_LAT) {
		ctx = dev->curr_dec_ctx[hw_id];
		if (ctx)
			fourcc = ctx->q_data[MTK_Q_DATA_SRC].fmt->fourcc;
		...
		if (vdec_lat_vld_addr == NULL || vdec_lat_misc_addr == NULL) {
			mtk_v4l2_debug(
				4,
				"VDEC codec:0x%08x(%c%c%c%c) HW break fail since vdec_lat_vld_addr 0x%lx vdec_lat_misc_addr 0x%lx",
				fourcc, fourcc & 0xFF, (fourcc >> 8) & 0xFF,
				(fourcc >> 16) & 0xFF, (fourcc >> 24) & 0xFF,
		...
		while (!((cg_status & 0x1) && (cg_status & 0x10))) {
			...
			if (usec > timeout) {
				mtk_v4l2_err(
					"VDEC HW %d break timeout. codec:0x%08x(%c%c%c%c)",
					hw_id, fourcc, fourcc & 0xFF,
					(fourcc >> 8) & 0xFF,
					(fourcc >> 16) & 0xFF,
					(fourcc >> 24) & 0xFF);
				for (offset = 64; offset <= 79; offset++) {
					value = readl(vdec_lat_misc_addr +
						      (offset << 2));
					mtk_v4l2_err(
						"[DEBUG][LAT_MISC] 0x%x(%d) = 0x%lx",
						offset << 2, offset, value);
				}
				for (idx = 0; idx < 4; idx++) {
					offset = misc_offset[idx];
...
void mtk_vcodec_dec_clock_on(struct mtk_vcodec_pm *pm, int hw_id)
{
	...
	for (larb_index = 0; larb_index < MTK_VDEC_MAX_LARB_COUNT;
		...
		if (pm->larbvdecs[larb_index]) {
			...
			if (ret)
				mtk_v4l2_err(
					"Failed to get vdec larb. index: %d, hw_id: %d",
					larb_index, hw_id);
		}
	}
	...
	for (j = 0; j < clks_data->main_clks_len; j++) {
		...
		if (clk_id >= 0) {
			...
			if (ret)
				mtk_v4l2_err(
					"clk_prepare_enable id: %d, name: %s fail %d",
					clk_id,
					clks_data->main_clks[j].clk_name, ret);
		}
	}
			...
			if (clk_id >= 0) {
				...
				if (ret)
					mtk_v4l2_err(
						"clk_prepare_enable id: %d, name: %s fail %d",
						clk_id,
						clks_data->core_clks[j].clk_name,
						ret);
			}
		}
	...
	if (pm->mtkdev->vdec_hw_ipm == VCODEC_IPM_V2) {
		...
		if (atomic_inc_return(&pm->dec_active_cnt) == 1) {
			...
			vdec_racing_addr = dev->dec_reg_base[VDEC_RACING_CTRL] +
...
static void __maybe_unused mtk_vdec_dump_addr_reg(
...
#define OUTPUT_UFO_NUM 4
	const unsigned int output_ufo_reg[OUTPUT_UFO_NUM] = {
		0x7C, 0x80, 0x84, 0x88
	}; // LEN_Y, LEN_C, BS_Y, BS_C
#define REF_MC_NUM 7
	const unsigned int ref_mc_base[REF_MC_NUM] = { 0x3DC, 0xB60, 0x45C,
						       0xBE0, 0x4DC, 0xC60,
						       0xD28 };
// P_L0_Y, P_L0_C, B_L0_Y, B_L0_C, B_L1_Y, B_L1_C, REF
#define UBE_CORE_VLD_NUM 3
	const unsigned int ube_core_vld_reg[UBE_CORE_VLD_NUM] = { 0xB0, 0xB4,
								  0xB8 };
	...
	if (hw_id != MTK_VDEC_CORE &&
	    (hw_id != MTK_VDEC_LAT || vdec_hw_ipm == VCODEC_IPM_V1)) {
	...
	case DUMP_VDEC_OUT_BUF:
		...
		for (i = 0; i < OUTPUT_MC_NUM; i++) {
			...
			mtk_v4l2_err("[CORE][MC] 0x%x(%d) = 0x%lx",
				     output_mc_reg[i], output_mc_reg[i] / 4,
				     value);
		}
		if (is_ufo) {
		...
		for (i = 0; i < 32; i++) {
			for (j = 0; j < 6; j++)
				values[j] = readl(mc_addr +
						  (ref_mc_base[j] + i * 4));
			mtk_v4l2_err(
				"[CORE][MC] 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx",
				ref_mc_base[0] + i * 4, ref_mc_base[0] / 4 + i,
				values[0], ref_mc_base[1] + i * 4,
				ref_mc_base[1] / 4 + i, values[1],
				ref_mc_base[2] + i * 4, ref_mc_base[2] / 4 + i,
				values[2], ref_mc_base[3] + i * 4,
				ref_mc_base[3] / 4 + i, values[3],
				ref_mc_base[4] + i * 4, ref_mc_base[4] / 4 + i,
				values[4], ref_mc_base[5] + i * 4,
				ref_mc_base[5] / 4 + i, values[5]);
		}
		for (i = 0; i < 4; i++)
			values[i] = readl(mc_addr + i * 4);
		mtk_v4l2_err(
			"[CORE][MC] 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx",
			i * 4, i, values[0], i * 4, i, values[1], i * 4, i,
			values[2], i * 4, i, values[3]);
		for (i = 0; i < 6; i++)
			values[i] = readl(mc_addr + (ref_mc_base[6] + i * 4));
		mtk_v4l2_err(
			"[CORE][MC] 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx, 0x%x(%d) = 0x%lx",
			ref_mc_base[6], ref_mc_base[6] / 4 + 0, values[0],
			ref_mc_base[6] + 1 * 4, ref_mc_base[6] / 4 + 1,
			values[1], ref_mc_base[6] + 2 * 4,
			ref_mc_base[6] / 4 + 2, values[2],
			ref_mc_base[6] + 3 * 4, ref_mc_base[6] / 4 + 3,
			values[3], ref_mc_base[6] + 4 * 4,
			ref_mc_base[6] / 4 + 4, values[4],
			ref_mc_base[6] + 5 * 4, ref_mc_base[6] / 4 + 5,
			values[5]);
		break;
	case DUMP_VDEC_MV_BUF:
		...
		mtk_v4l2_err("[CORE][MV] 0x%x(%d) = 0x%lx", 0x20C, 0x20C / 4,
			     value);
		switch (fourcc) {
		...
		for (i = start; i < end; i++) {
			...
			mtk_v4l2_err("[CORE][MV] 0x%x(%d) = 0x%lx", i * 4, i,
				     value);
		}
		break;
		...
		if (hw_id == MTK_VDEC_LAT) {
			...
			if (vld_addr != NULL) {
				for (i = 0; i < UBE_CORE_VLD_NUM; i++) {
					value = readl(vld_addr +
						      ube_core_vld_reg[i]);
					mtk_v4l2_err(
						"[CORE][VLD] 0x%x(%d) = 0x%lx",
						ube_core_vld_reg[i],
...
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_DBG)
static int mtk_vdec_translation_fault_callback(int port, dma_addr_t mva,
					       void *data)
{
	...
	if (port_idx == VDEC_M4U_PORT_LAT0_UFO ||
		...
		mtk_v4l2_err("unknown larb port %d of m4u port 0x%x",
			     MTK_M4U_TO_LARB(port), port);
		return 0;
	}
	...
	if (ctx) {
		fourcc = ctx->q_data[MTK_Q_DATA_SRC].fmt->fourcc;
		mtk_v4l2_err(
			"[%d] codec:0x%08x(%c%c%c%c) %s(%d) TF larb %d port %s(%x) mva 0x%llx",
			ctx->id, fourcc, fourcc & 0xFF, (fourcc >> 8) & 0xFF,
			(fourcc >> 16) & 0xFF, (fourcc >> 24) & 0xFF,
			(hw_id == MTK_VDEC_LAT) ? "LAT" : "CORE", hw_id,
			MTK_M4U_TO_LARB(port), dec_port_name[port_idx], port,
			(u64)mva);
	} else {
		mtk_v4l2_err(
			"ctx NULL codec unknown, %s(%d) TF larb %d port %s(%x) mva 0x%llx",
			(hw_id == MTK_VDEC_LAT) ? "LAT" : "CORE", hw_id,
			MTK_M4U_TO_LARB(port), dec_port_name[port_idx], port,
			(u64)mva);
	}
	...
	if (port == dev->dec_m4u_ports[VDEC_M4U_PORT_LAT0_VLD] ||
		...
		if (port == dev->dec_m4u_ports[VDEC_M4U_PORT_UFO] ||
		    port == dev->dec_m4u_ports[VDEC_M4U_PORT_UFO_ENC])
			mtk_vdec_dump_addr_reg(dev, MTK_VDEC_CORE,
					       DUMP_VDEC_REF_BUF);
		...
		if (vdec_hw_ipm == VCODEC_IPM_V2) {
			if (hw_id == MTK_VDEC_CORE) {
				mtk_vdec_dump_addr_reg(dev, hw_id,
						       DUMP_VDEC_UBE_BUF);
				mtk_vdec_dump_addr_reg(dev, hw_id,
						       DUMP_VDEC_OUT_BUF);
			} else {
...
static int mtk_vdec_uP_translation_fault_callback(int port, dma_addr_t mva,
						  void *data)
{
	struct mtk_vcodec_dev *dev = (struct mtk_vcodec_dev *)data;
	...
	for (hw_id = 0; hw_id < MTK_VDEC_HW_NUM; hw_id++) {
		...
		if (dec_ctx[hw_id]) {
			...
			dec_fourcc[hw_id] = dec_ctx[hw_id]
						    ->q_data[MTK_Q_DATA_SRC]
						    .fmt->fourcc;
			for (i = 0; i < 4; i++)
				dec_codec_name[hw_id][i] =
					(dec_fourcc[hw_id] >> (i * 8)) & 0xFF;
		} else {
			dec_ctx_id[hw_id] = 0;
			dec_fourcc[hw_id] = 0;
	...
	mtk_v4l2_err(
		"current dec ctx: LAT ctx_id %d codec:%s(0x%08x), CORE ctx_id %d codec:%s(0x%08x) (ipm v%d)",
		dec_ctx_id[MTK_VDEC_LAT], dec_codec_name[MTK_VDEC_LAT],
		dec_fourcc[MTK_VDEC_LAT], dec_ctx_id[MTK_VDEC_CORE],
		dec_codec_name[MTK_VDEC_CORE], dec_fourcc[MTK_VDEC_CORE],
		vdec_hw_ipm);
	...
	for (i = 0; i < NUM_MAX_VDEC_M4U_PORT; i++) {
		if (dev->dec_m4u_ports[i] != 0 && i < VDEC_M4U_PORT_UP_1)
			mtk_iommu_register_fault_callback(
				dev->dec_m4u_ports[i],
				mtk_vdec_translation_fault_callback,
				(void *)dev, false);
		if (dev->dec_m4u_ports[i] != 0 && i >= VDEC_M4U_PORT_UP_1)
			mtk_iommu_register_fault_callback(
				dev->dec_m4u_ports[i],
				mtk_vdec_uP_translation_fault_callback,
				(void *)dev, false);
	}
#endif
}
