...
#if defined(CONFIG_MTK_SCP) && defined(SCP_ISP_MEM2_ID)
...
dma_addr_t mtk_cam_smem_iova_to_scp_addr(struct device *dev, dma_addr_t iova)
{
...
#if defined(CONFIG_MTK_SCP) && defined(SCP_ISP_MEM2_ID)
static int mtk_cam_smem_get_sgtable(struct device *dev, struct sg_table *sgt,
					    ...
					    enum dma_data_direction dir)
{
	...
	dev_dbg(dev, "__dma_map_area:scp_addr:%pad,vaddr:%pK,size:%d,dir:%d\n",
		&scp_addr, cpu_addr, sgl->length, dir);
...
static int mtk_cam_reserved_drm_sg_init(struct mtk_cam_smem_dev *smem_dev)
{
	...
	dev_info(dev, "%s dev:0x%pK base:%pad size:%u MiB\n", __func__,
		 smem_dev->dev, &smem_dev->smem_base,
	...
	ret = sg_alloc_table_from_pages(sgt, pages, n_pages, 0, size_align,
					GFP_KERNEL);
	if (ret) {
	...
	ret = dma_declare_coherent_memory(dev, smem_dev->smem_base, dma_addr,
					  size_align, DMA_MEMORY_EXCLUSIVE);
	if (ret) {
					     ...
					     const char *name)
{
	...
	vb2_dma_contig_set_max_seg_size(child, U32_MAX);
