...
int mtk_cam_working_buf_pool_init(struct mtk_cam_ctx *ctx)
{
	...
	for (i = 0; i < CAM_CQ_BUF_NUM; i++) {
		...
		dev_dbg(ctx->cam->dev,
			...
			__func__, i, buf->meta_buffer.va,
			&buf->meta_buffer.iova, buf->meta_buffer.fd,
			buf->meta_buffer.size);
...
void mtk_cam_working_buf_pool_release(struct mtk_cam_ctx *ctx)
{
	...
	dev_dbg(ctx->cam->dev,
		...
		ctx->stream_id, smem.va, smem.len);
}
...
void mtk_cam_working_buf_put(struct mtk_cam_working_buf_entry *buf_entry)
{
	...
	list_add_tail(&buf_entry->list_entry, &ctx->buf_pool.cam_freelist.list);
...
mtk_cam_working_buf_get(struct mtk_cam_ctx *ctx)
{
	...
	buf_entry =
		list_first_entry(&ctx->buf_pool.cam_freelist.list,
				      ...
				      int working_buf_size)
{
	...
	dev_info(ctx->cam->dev,
		 ...
		 ctx->stream_id, ctx->img_buf_pool.cam_freeimglist.cnt);
...
int mtk_cam_sv_working_buf_pool_init(struct mtk_cam_ctx *ctx)
{
	...
	for (i = 0; i < CAMSV_WORKING_BUF_NUM; i++) {
		struct mtk_camsv_working_buf_entry *buf =
			&ctx->buf_pool.sv_working_buf[i];
		buf->ctx = ctx;
...
void mtk_cam_sv_working_buf_put(struct mtk_camsv_working_buf_entry *buf_entry)
{
	struct mtk_cam_ctx *ctx = buf_entry->ctx;
	...
	list_add_tail(&buf_entry->list_entry, &ctx->buf_pool.sv_freelist.list);
	ctx->buf_pool.sv_freelist.cnt++;
...
mtk_cam_mraw_working_buf_get(struct mtk_cam_ctx *ctx)
{
	...
	buf_entry =
		list_first_entry(&ctx->buf_pool.mraw_freelist.list,
				 struct mtk_mraw_working_buf_entry, list_entry);
	list_del(&buf_entry->list_entry);
	ctx->buf_pool.mraw_freelist.cnt--;
	spin_unlock(&ctx->buf_pool.mraw_freelist.lock);
