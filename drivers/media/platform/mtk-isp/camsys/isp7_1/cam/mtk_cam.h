...
static inline bool raw_pipe_data_check(int a)
{
	...
	result = (a >= 0 &&
		  a < (MTKCAM_SUBDEV_RAW_END - MTKCAM_SUBDEV_RAW_START)) ?
			 true :
			 false;
	return result;
}
...
struct mtk_cam_request {
	...
	struct mtk_cam_req_raw_pipe_data
		raw_pipe_data[MTKCAM_SUBDEV_RAW_END - MTKCAM_SUBDEV_RAW_START];
	s64 sync_id;
	atomic_t ref_cnt;
};
...
struct mtk_cam_ctx {
	...
	struct mtk_camsv_working_buf_list
		sv_using_buffer_list[MAX_SV_PIPES_PER_STREAM];
	struct mtk_camsv_working_buf_list
		sv_processing_buffer_list[MAX_SV_PIPES_PER_STREAM];

	struct mtk_mraw_working_buf_list
		mraw_using_buffer_list[MAX_MRAW_PIPES_PER_STREAM];
	struct mtk_mraw_working_buf_list
		mraw_composed_buffer_list[MAX_MRAW_PIPES_PER_STREAM];
	struct mtk_mraw_working_buf_list
		mraw_processing_buffer_list[MAX_MRAW_PIPES_PER_STREAM];
...
static inline int mtk_cam_req_get_num_s_data(struct mtk_cam_request *req,
					     int pipe_id)
{
...
static inline void
mtk_cam_s_data_reset_vbuf(struct mtk_cam_request_stream_data *s_data,
			  int node_id)
{
	int idx = mtk_cam_s_data_get_vbuf_idx(s_data, node_id);
	if (idx >= 0)
...
static inline void mtk_cam_pad_fmt_enable(struct v4l2_mbus_framefmt *framefmt,
					  bool enable)
{
	if (enable)
...
void mtk_cam_s_data_update_timestamp(
	struct mtk_cam_buffer *buf, struct mtk_cam_request_stream_data *s_data);
...
int mtk_cam_dev_config(struct mtk_cam_ctx *ctx, bool streaming,
		       bool config_pipe);
void mtk_cam_apply_pending_dev_config(
	struct mtk_cam_request_stream_data *s_data);
...
mtk_cam_get_req_s_data(struct mtk_cam_ctx *ctx, unsigned int pipe_id,
		       unsigned int frame_seq_no);
struct mtk_raw_pipeline *
mtk_cam_dev_get_raw_pipeline(struct mtk_cam_device *cam, unsigned int id);
