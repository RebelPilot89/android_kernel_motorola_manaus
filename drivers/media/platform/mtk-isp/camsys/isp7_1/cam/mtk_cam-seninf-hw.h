...
#define SET_REG_KEYS_NAMES                                                     \
	"RG_SETTLE_CK", "RG_SETTLE_DT", "RG_HS_TRAIL_EN", "RG_HS_TRAIL_PARAM", \
		"RG_CSI_IRQ_STAT", "RG_CSI_RESYNC_CYCLE", "RG_MUX_IRQ_STAT",   \
		"RG_CAMMUX_IRQ_STAT", "REG_VSYNC_IRQ_EN", "RG_CSI_IRQ_EN",
...
struct mtk_cam_seninf_ops {
	int (*_init_iomem)(struct seninf_ctx *ctx, void __iomem *if_base,
	...
	int (*_set_top_mux_ctrl)(struct seninf_ctx *ctx, int mux_idx,
				 int seninf_src);
	...
	int (*_set_cammux_vc)(struct seninf_ctx *ctx, int cam_mux, int vc_sel,
			      int dt_sel, int vc_en, int dt_en);
	int (*_set_cammux_src)(struct seninf_ctx *ctx, int src, int target,
			       int exp_hsize, int exp_vsize, int dt);
	int (*_set_vc)(struct seninf_ctx *ctx, int seninfIdx,
		       struct seninf_vcinfo *vcinfo);
	int (*_set_mux_ctrl)(struct seninf_ctx *ctx, int mux, int hsPol,
			     int vsPol, int src_sel, int pixel_mode);
	int (*_set_mux_crop)(struct seninf_ctx *ctx, int mux, int start_x,
	...
	int (*_set_cammux_chk_pixel_mode)(struct seninf_ctx *ctx, int cam_mux,
					  int pixelMode);
	int (*_set_test_model)(struct seninf_ctx *ctx, int mux, int cam_mux,
			       int pixelMode);
	...
	ssize_t (*_show_status)(struct device *dev,
				struct device_attribute *attr, char *buf);
	int (*_switch_to_cammux_inner_page)(struct seninf_ctx *ctx, bool inner);
	int (*_set_cammux_next_ctrl)(struct seninf_ctx *ctx, int src,
				     int target);
	int (*_update_mux_pixel_mode)(struct seninf_ctx *ctx, int mux,
				      int pixel_mode);
	int (*_irq_handler)(int irq, void *data);
	int (*_set_sw_cfg_busy)(struct seninf_ctx *ctx, bool enable, int index);
	int (*_set_cam_mux_dyn_en)(struct seninf_ctx *ctx, bool enable,
				   int cam_mux, int index);
	int (*_reset_cam_mux_dyn_en)(struct seninf_ctx *ctx, int index);
	int (*_enable_global_drop_irq)(struct seninf_ctx *ctx, bool enable,
				       int index);
	int (*_enable_cam_mux_vsync_irq)(struct seninf_ctx *ctx, bool enable,
					 int cam_mux);
	...
	ssize_t (*_show_err_status)(struct device *dev,
				    struct device_attribute *attr, char *buf);
