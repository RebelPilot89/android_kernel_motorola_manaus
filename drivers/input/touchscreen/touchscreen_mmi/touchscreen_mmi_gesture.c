...
struct ts_mmi_sensor_platform_data {
	...
	struct ts_mmi_dev *touch_cdev __maybe_unused;
};
...
static inline void
ts_mmi_touch_event_report_event_release(int index, struct input_dev *input_dev)
{
				     ...
				     struct ts_mmi_dev *touch_cdev)
{
	...
	if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_LEFT)
		return (tev->x > (touch_cdev->pdata.max_x - gs_distance)) ?
			       true :
			       false;
...
static inline bool ts_mmi_touch_event_rescue_slot(int slot_id)
{
	if (events_data->poison_events[slot_id].is_slot_poisoned) {
		ts_mmi_touch_clear_event(
			&events_data->poison_events[slot_id].center);
					...
					struct ts_mmi_dev *touch_cdev)
{
	...
	if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_ALL)
		return false;
	if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_LEFT)
		return (tev->x > (touch_cdev->pdata.max_x - trigger_distance)) ?
			       true :
			       false;
	if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_RIGHT)
...
{
	int delta_y = abs(tev->y -
			  events_data->poison_events[tev->id].center.event.y);
	return (delta_y > events_data->touch_cdev->poison_distance) ? true :
								      false;
}
...
static inline bool is_stack_at_poison_center(struct touch_event_data *tev)
{
	if (events_data->poison_events[tev->id].center.time) {
		unsigned long long duration = timediff_ms(
			ktime_to_timespec(
				events_data->poison_events[tev->id].center.time),
			ktime_to_timespec(ktime_get()));

		return (duration > events_data->touch_cdev->poison_timeout) ?
			       true :
			       false;
	}
...
{
	return ts_mmi_touch_log_event(
		&events_data->poison_events[tev->id].center, tev);
}
#endif /* TS_MMI_TOUCH_GESTURE_POISON_EVENT */
...
static int ts_mmi_gesture_handler(struct gesture_event_data *gev)
{
	...
	case 2:
		...
		input_report_abs(sensor_pdata->input_sensor_dev, ABS_X,
				 gev->evdata.x);
		input_report_abs(sensor_pdata->input_sensor_dev, ABS_Y,
				 gev->evdata.y);
...
static int ts_mmi_cli_gesture_handler(struct gesture_event_data *gev)
{
	...
	case 2:
		...
		if (gev->evdata.y == 0)
			gev->evdata.y = touch_cdev->pdata.fod_y;
		input_report_abs(cli_sensor_pdata->input_sensor_dev, ABS_X,
				 gev->evdata.x);
		input_report_abs(cli_sensor_pdata->input_sensor_dev, ABS_Y,
				 gev->evdata.y);
		pr_info("%s: zero tap; x=%x, y=%x\n", __func__, gev->evdata.x,
			gev->evdata.y);
		break;
						  ...
						  struct input_dev *input_dev)
{
	...
	if (tev->type == TS_COORDINATE_ACTION_PRESS) {
		...
		if (need_handle_poison_event(tev->id)) {
			if (events_data->poison_events[tev->id]
				    .is_slot_poisoned) {
		...
		if (need_handle_poison_event(tev->id)) {
			if (events_data->poison_events[tev->id]
				    .is_slot_poisoned) {
				if (ts_mmi_touch_event_is_in_poison_trigger(
					    tev, touch_cdev)) {
					dev_dbg(DEV_TS,
						"%s: slot(%d) is poisoned\n",
						__func__, tev->id);
					tev->skip_report = true;
					tev->type = TS_COORDINATE_ACTION_NONE;
				} else {
					events_data->poison_events[tev->id]
						.is_slot_poisoned = false;
					pr_info("%s: Poison slot(%d) temperary rescued because outof trigger area\n",
						__func__, tev->id);
				}
			} else if (is_far_from_poison_center(tev) &&
				   ts_mmi_touch_event_is_in_suppression(
					   tev, touch_cdev)) {
				update_poison_center(tev);
				...
				dev_info(
					DEV_TS,
					"%s: slot(%d) is stacked near poison center. Mark this slot poisoned\n",
					__func__, tev->id);
				ts_mmi_touch_event_report_event_release(
					tev->id, input_dev);
				events_data->poison_events[tev->id]
					.is_slot_poisoned = true;
				tev->skip_report = true;
				tev->type = TS_COORDINATE_ACTION_NONE;
			}
		}
				      ...
				      struct input_dev *input_dev)
{
	...
	case TS_COORDINATE_ACTION_PRESS:
		...
		pr_info("%s: [P]Finger %d: Down, x=%d, y=%d, major=%d, minor=%d",
			__func__, tev->id, tev->x, tev->y, tev->major,
			tev->minor);
#endif
	...
	case TS_COORDINATE_ACTION_RELEASE:
		...
		if (touch_cdev->pdata.fps_detection) {
			if ((touch_cdev->delay_baseline_update) &&
				...
				_TRY_TO_CALL(update_baseline,
					     TS_MMI_UPDATE_BASELINE_ON);
...
int ts_mmi_gesture_init(struct ts_mmi_dev *touch_cdev)
{
	...
	events_data = devm_kzalloc(
		DEV_TS, sizeof(struct ts_mmi_touch_events_data), GFP_KERNEL);
	if (!events_data) {
		dev_err(DEV_TS, "%s: Failed to allocate memory", __func__);
	...
	if (err) {
		dev_err(DEV_TS, "%s: Unable to register device, err=%d",
			__func__, err);
	...
	touch_cdev->mdata->exports.report_touch_event =
		ts_mmi_touch_event_handler;
...
int ts_mmi_cli_gesture_init(struct ts_mmi_dev *touch_cdev)
{
	...
	sensor_input_dev = input_allocate_device();
	if (!sensor_input_dev) {
		dev_err(DEV_TS, "%s: Failed to allocate input device",
			__func__);
		goto exit;
	}
...
int ts_mmi_palm_remove(struct ts_mmi_dev *touch_cdev)
{
	...
	devm_kfree(&palm_sensor_pdata->input_sensor_dev->dev,
		   palm_sensor_pdata);
	palm_sensor_pdata = NULL;
