...
static DEVICE_ATTR(sample, (S_IRUGO | S_IWUSR | S_IWGRP), fts_sample_show,
		   fts_sample_store);
static DEVICE_ATTR(stowed, (S_IWUSR | S_IWGRP | S_IRUGO), fts_stowed_show,
		   fts_stowed_store);
#ifdef CONFIG_FTS_LAST_TIME
			      ...
			      const char *buf, size_t size)
{
	...
	if (ts_data->power_disabled) {
		FTS_ERROR(
			"The touch is in sleep state, restore the value when resume");
		ret = size;
		goto exit;
	}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "0x%02x",
			 ts_data->set_mode.interpolation);
}
				...
				size_t size)
{
	...
	if ((ts_data->power_disabled == false) &&
	    (ts_data->suspended == true)) {
		if (mode)
			ret = fts_write_reg(FTS_REG_POWER_MODE,
					    FTS_REG_POWER_MODE_STANDBY);
		else
			ret = fts_write_reg(FTS_REG_POWER_MODE,
					    FTS_REG_POWER_MODE_ACTIVE);
		if (ret < 0) {
			FTS_ERROR("failed to set report rate, mode = %lu",
				  mode);
			goto exit;
		}
	} else {
		FTS_INFO(
			"Skip stowed mode setting suspended:%d, power_disabled:%d.\n",
...
{
	return scnprintf(TO_CHARP(cdata), TS_MMI_MAX_VENDOR_LEN, "%s",
			 "focaltech");
}
...
#endif
	num_read_chars =
		scnprintf(TO_CHARP(cdata), TS_MMI_MAX_ID_LEN, "%02x", fwver);
			       ...
			       enum ts_mmi_pm_mode to)
{
	...
	case TS_MMI_PM_GESTURE:
		...
		if (ts_data->imports && ts_data->imports->get_gesture_type) {
			ret = ts_data->imports->get_gesture_type(ts_data->dev,
								 &gesture_type);
		}

		if (gesture_type & TS_MMI_GESTURE_SINGLE) {
			gesture_command |= (1 << 7);
			FTS_INFO("Enable GESTURE_CLI_SINGLE command: %02x",
				 gesture_command);
		}
		if (gesture_type & TS_MMI_GESTURE_DOUBLE) {
			gesture_command |= (1 << 4);
			FTS_INFO("Enable GESTURE_CLI_DOUBLE command: %02x",
				 gesture_command);
		}
...
#endif
			FTS_INFO("Enable GESTURE_CLI_ZERO command: %02x",
				 gesture_command);
		}
...
exit:
	...
	if (pdata->interpolation_ctrl && ts_data->get_mode.interpolation) {
		...
		if (ret >= 0) {
			ts_data->set_mode.interpolation =
				ts_data->get_mode.interpolation;
			ts_data->set_mode.report_rate_mode =
				ts_data->get_mode.report_rate_mode;

			FTS_INFO(
				"Success to %s interpolation mode\n",
				ts_data->get_mode.report_rate_mode ==
						REPORT_RATE_CMD_240HZ ?
					"REPORT_RATE_240HZ" :
					(ts_data->get_mode.report_rate_mode ==
							 REPORT_RATE_CMD_360HZ ?
						 "REPORT_RATE_360HZ" :
						 "Unsupported"));
		}
	}
	...
	if (pdata->edge_ctrl) {
		ret = fts_set_edge_mode(ts_data->get_mode);
		if (!ret) {
			memcpy(ts_data->set_mode.edge_mode,
			       ts_data->get_mode.edge_mode,
			       sizeof(ts_data->get_mode.edge_mode));
			msleep(20);
			FTS_INFO(
				"Success to set edge area = %02x, rotation = %02x",
				ts_data->get_mode.edge_mode[1],
				ts_data->get_mode.edge_mode[0]);
		}
	}
...
static int fts_mmi_post_suspend(struct device *dev)
{
	...
	if (pdata->stowed_mode_ctrl && ts_data->get_mode.stowed &&
		...
		ret = fts_write_reg(FTS_REG_POWER_MODE,
				    FTS_REG_POWER_MODE_STANDBY);
		if (ret < 0) {
			FTS_ERROR("Failed to set stowed mode %d",
				  ts_data->get_mode.stowed);
		} else {
			ts_data->set_mode.stowed = ts_data->get_mode.stowed;
			FTS_INFO("Enable stowed mode %d success.",
				 ts_data->set_mode.stowed);
		}
		mutex_unlock(&ts_data->mode_lock);
	}
