...
enum topology_scenario { PATH_MML_NOPQ_P0 = 0, PATH_MML_PQ_P0, PATH_MML_MAX };
...
enum cmdq_clt_usage { MML_CLT_PIPE0, MML_CLT_MAX };
...
static u8 engine_reset_bit[MML_ENGINE_TOTAL] = {
	[MML_RDMA0] = 3,  [MML_HDR0] = 5,   [MML_AAL0] = 6,
	[MML_RSZ0] = 7,	  [MML_TDSHP0] = 8, [MML_COLOR0] = 9,
	[MML_WROT0] = 10, [MML_DLI0] = 12,  [MML_DLO0] = 26,
};
...
static void tp_dump_path(const struct mml_topology_path *path)
{
	...
	for (i = 0; i < path->node_cnt; i++) {
		mml_log("[topology]engine %hhu (%p) prev %p next %p %p comp %p tile idx %hhu out %hhu",
			path->nodes[i].id, &path->nodes[i], path->nodes[i].prev,
			path->nodes[i].next[0], path->nodes[i].next[1],
			path->nodes[i].comp, path->nodes[i].tile_eng_idx,
...
static void tp_dump_path_short(struct mml_topology_path *path)
{
	...
	for (i = 0; i < path->node_cnt; i++)
		len += snprintf(path_desc + len, sizeof(path_desc) - len,
				" %hhu", path->nodes[i].id);
			  ...
			  const struct path_node *route)
{
	...
	for (i = 0; i < MML_MAX_PATH_NODES; i++) {
		...
		if (!path->nodes[i].comp)
			mml_err("[topology]no comp idx:%hhu engine:%hhu", i,
				eng);
		...
		if (!connect_eng[0] && next0) {
			...
			mml_err("[topology]connect fail idx:%hhu engine:%hhu next0:%hhu next1:%hhu from:%hhu %hhu",
				i, eng, next0, next1, connect_eng[0],
				connect_eng[1]);
		}
	}
	path->node_cnt = i;
...
static inline bool tp_need_resize(struct mml_frame_info *info)
{
	...
	mml_msg("[topology]%s target %ux%u crop %ux%u", __func__, w, h,
		info->dest[0].crop.r.width, info->dest[0].crop.r.height);

	return info->dest_cnt != 1 || info->dest[0].crop.r.width != w ||
	       info->dest[0].crop.r.height != h ||
	       info->dest[0].crop.x_sub_px || info->dest[0].crop.y_sub_px ||
	       info->dest[0].crop.w_sub_px || info->dest[0].crop.h_sub_px ||
...
{
	enum topology_scenario scene[2] = { PATH_MML_NOPQ_P0,
					    PATH_MML_NOPQ_P0 };
	bool en_rsz;
	bool en_pq;
		     ...
		     struct mml_frame_config *cfg)
{
	...
	switch (mml_dual) {
	case MML_DUAL_DISABLE:
		cfg->dual = false;
		break;
	case MML_DUAL_ALWAYS:
		cfg->dual = true;
		break;
	case MML_DUAL_NORMAL:
	default:
	...
	if (cfg->dual && path[1] && path[1] != path[0])
		tp_dump_path_short(path[1]);

	return 0;
}

static enum mml_mode tp_query_mode(struct mml_dev *mml,
...
static struct cmdq_client *get_racing_clt(struct mml_topology_cache *cache,
					  u32 pipe)
{
	/* not support inline rot in this platform */
	return NULL;
