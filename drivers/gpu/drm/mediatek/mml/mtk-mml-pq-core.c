		      ...
		      struct mml_pq_sub_task *sub_task)
{
	...
	mml_pq_msg(
		"%s sub_task[%p] sub_task->mbox_list[%p] chan[%p] chan->msg_list[%p]",
		__func__, sub_task, &sub_task->mbox_list, chan,
		&chan->msg_list);
		       ...
		       struct mml_pq_sub_task **out_sub_task)
{
	...
	mml_pq_trace_ex_begin("%s", __func__);
	mml_pq_msg("%s chan[%p] chan->msg_list[%p]", __func__, chan,
		   &chan->msg_list);

	mutex_lock(&chan->msg_lock);
	temp = list_first_entry_or_null(&chan->msg_list, typeof(*temp),
					mbox_list);
	if (temp) {
	...
	mml_pq_msg(
		"%s temp[%p] temp->result[%p] sub_task->job_id[%llu] chan[%p] chan_job_id[%llx]",
		__func__, temp, temp->result, temp->job_id, chan,
		chan->job_idx);
	mml_pq_msg(
		"%s chan_job_id[%llx] temp->mbox_list[%p] chan->job_list[%p]",
...
void mml_pq_comp_config_clear(struct mml_task *task)
{
	...
	mml_pq_log("%s task_job_id[%d] job_id[%llx] dual[%d]", __func__,
		   task->job.jobid, job_id, task->config->dual);
	mutex_lock(&chan->msg_lock);
	if (atomic_read(&chan->msg_cnt)) {
		list_for_each_entry_safe (sub_task, tmp, &chan->msg_list,
					  mbox_list) {
			mml_pq_log(
				"%s msg sub_task[%p] msg_list[%p] sub_job_id[%llx]",
				__func__, sub_task, &chan->msg_list,
				sub_task->job_id);
		...
		mutex_unlock(&chan->msg_lock);
		mutex_lock(&chan->job_lock);
		list_for_each_entry_safe (sub_task, tmp, &chan->job_list,
					  mbox_list) {
			mml_pq_log(
				"%s job sub_task[%p] job_list[%p] sub_job_id[%llx]",
				__func__, sub_task, &chan->job_list,
				sub_task->job_id);
			if (sub_task->job_id == job_id)
				list_del(&sub_task->mbox_list);
		}
			 ...
			 struct mml_pq_sub_task **out_sub_task)
{
	...
	list_for_each_entry_safe (sub_task, tmp, &chan->job_list, mbox_list) {
		mml_pq_msg(
			"%s sub_task[%p] chan->job_list[%p] sub_job_id[%llx]",
			__func__, sub_task, &chan->job_list, sub_task->job_id);
		if (sub_task->job_id == job_id) {
			*out_sub_task = sub_task;
			mml_pq_msg("%s find sub_task:%p id:%llx", __func__,
				   sub_task, job_id);
			list_del(&sub_task->mbox_list);
			break;
		}
...
s32 mml_pq_task_create(struct mml_task *task)
{
	...
	pq_task->aal_readback.readback_data.pipe0_hist = kzalloc(
		sizeof(u32) * (AAL_HIST_NUM + AAL_DUAL_INFO_NUM), GFP_KERNEL);
	pq_task->aal_readback.readback_data.pipe1_hist = kzalloc(
		sizeof(u32) * (AAL_HIST_NUM + AAL_DUAL_INFO_NUM), GFP_KERNEL);
...
static void release_comp_config_result(void *data)
{
	...
	if (result == last_result) {
		mml_pq_msg("%s result == last_result", __func__);
		result = NULL;
	}
	if (!result)
		return;
...
static void release_pq_task(struct kref *ref)
{
	struct mml_pq_task *pq_task =
		container_of(ref, struct mml_pq_task, ref);
	...
	mml_pq_msg("%s aal_hist[%p] hdr_hist[%p]", __func__,
		   pq_task->aal_hist[0], pq_task->hdr_hist[0]);
...
static void remove_sub_task(struct mml_pq_task *pq_task,
			       ...
			       struct mml_pq_readback_buffer *hist)
{
	...
	mml_pq_rb_msg("%s get offset job_id[%d] engine_start[%d]", __func__,
		      task->job.jobid, engine_start);
	...
	while (engine_start < (engine + 1) * MAX_ENG_RB_BUF) {
		if (rb_buf_pool[engine_start] != INVALID_OFFSET_ADDR) {
			...
			mml_pq_rb_msg(
				"%s get offset job_id[%d]engine[%d]offset[%d]eng_start[%d]",
				__func__, task->job.jobid, engine,
				hist->va_offset, engine_start);
			break;
		}
		engine_start++;
	}
	...
	mml_pq_rb_msg(
		"%s all end job_id[%d] engine[%d] hist_va[%p] hist_pa[%llx]",
			       ...
			       struct mml_pq_readback_buffer *hist)
{
	...
	mml_pq_rb_msg(
		"%s start job_id[%d] hist_va[%p] engine_start[%08x] offset[%d]",
		__func__, task->job.jobid, hist->va, engine_start,
		hist->va_offset);
				...
				struct mml_pq_readback_buffer **hist)
{
	...
	temp_buffer = list_first_entry_or_null(
		&rb_buf_list, typeof(*temp_buffer), buffer_list);
	if (temp_buffer) {
		*hist = temp_buffer;
		list_del(&temp_buffer->buffer_list);
		mml_pq_rb_msg(
			"%s aal get buffer from list jobid[%d] va[%p] pa[%llx]",
			__func__, task->job.jobid, temp_buffer->va,
			temp_buffer->pa);
	} else {
		...
		buffer_num++;
		*hist = temp_buffer;
		mml_pq_rb_msg("%s aal reallocate jobid[%d] va[%p] pa[%llx]",
			      __func__, task->job.jobid, temp_buffer->va,
			      temp_buffer->pa);
	}
	mutex_unlock(&rb_buf_list_mutex);
	...
	if (!temp_buffer->va && !temp_buffer->pa) {
		...
		temp_buffer->va =
			(u32 *)cmdq_mbox_buf_alloc(clt, &temp_buffer->pa);
...
{
	return container_of(sub_task, struct mml_pq_task, hdr_readback);
}

static struct mml_pq_task *from_rsz_callback(struct mml_pq_sub_task *sub_task)
{
	return container_of(sub_task, struct mml_pq_task, rsz_callback);
}
...
static void dump_pq_param(struct mml_pq_param *pq_param)
{
	...
	mml_pq_dump(" layer_id=%u disp_id=%d", pq_param->layer_id,
		    pq_param->disp_id);
	mml_pq_dump(" src_gamut=%u dst_gamut", pq_param->src_gamut,
		    pq_param->dst_gamut);
	mml_pq_dump(" video_mode=%u", pq_param->src_hdr_video_mode);
	mml_pq_dump(" user=%u", pq_param->user_info);
}

static void mml_pq_check_dup_node(struct mml_pq_chan *chan,
				  struct mml_pq_sub_task *sub_task)
{
	...
	if (!list_empty(&chan->msg_list)) {
		list_for_each_entry_safe (tmp_sub_task, tmp, &chan->msg_list,
					  mbox_list) {
			mml_pq_msg(
				"%s sub_task[%p] chan->msg_list[%p] sub_job_id[%llx]",
				__func__, tmp_sub_task, &chan->msg_list,
				tmp_sub_task->job_id);
	...
	if (!list_empty(&chan->job_list)) {
		list_for_each_entry_safe (tmp_sub_task, tmp, &chan->job_list,
					  mbox_list) {
			mml_pq_msg(
				"%s sub_task[%p] chan->job_list[%p] sub_job_id[%llx]",
				__func__, tmp_sub_task, &chan->job_list,
				tmp_sub_task->job_id);
			if (tmp_sub_task->job_id == job_id) {
				mml_pq_log("%s find sub_task:%p id:%llx",
					   __func__, tmp_sub_task, job_id);
...
static int set_sub_task(struct mml_task *task, struct mml_pq_sub_task *sub_task,
			struct mml_pq_chan *chan, struct mml_pq_param *pq_param,
			bool is_dup_check)
{
	struct mml_pq_task *pq_task = task->pq_task;

	mml_pq_msg(
		"%s called queued[%d] result_ref[%d] job_id[%llx, %d] first_job[%d]",
		__func__, atomic_read(&sub_task->queued),
		atomic_read(&sub_task->result_ref), sub_task->job_id,
		task->job.jobid, sub_task->first_job);

	mutex_lock(&sub_task->lock);
	if (sub_task->mml_task_jobid != task->job.jobid ||
	    sub_task->first_job) {
	...
	if (!atomic_fetch_add_unless(&sub_task->queued, 1, 1)) {
		...
		memcpy(&sub_task->frame_data.frame_out,
		       &task->config->frame_out,
	...
	mml_pq_msg(
		"%s end queued[%d] result_ref[%d] job_id[%llx, %d] first_job[%d]",
		__func__, atomic_read(&sub_task->queued),
		atomic_read(&sub_task->result_ref), sub_task->job_id,
		task->job.jobid, sub_task->first_job);
	return 0;
...
static void put_sub_task_result(struct mml_pq_sub_task *sub_task,
				struct mml_pq_chan *chan)
{
	mml_pq_msg("%s result_ref[%d] queued[%d] msg_cnt[%d]", __func__,
		   atomic_read(&sub_task->result_ref),
		   atomic_read(&sub_task->queued), atomic_read(&chan->msg_cnt));
...
int mml_pq_set_tile_init(struct mml_task *task)
{
	...
	ret = set_sub_task(task, &pq_task->tile_init, chan, &task->pq_param[0],
			   false);
	mml_pq_trace_ex_end();
	return ret;
}
...
int mml_pq_get_tile_init_result(struct mml_task *task, u32 timeout_ms)
{
	...
	mml_pq_trace_ex_begin("%s", __func__);
	mml_pq_msg("%s called, %d job_id[%d]", __func__, timeout_ms,
		   task->job.jobid);
...
static void dump_comp_config(void *data)
{
	...
	for (i = 0; i < AAL_CURVE_NUM; i += 8)
		mml_pq_dump(
			"[curve][%d - %d] = [%08x, %08x, %08x, %08x, %08x, %08x, %08x, %08x]",
			i, i + 7, result->aal_curve[i],
			result->aal_curve[i + 1], result->aal_curve[i + 2],
			result->aal_curve[i + 3], result->aal_curve[i + 4],
			result->aal_curve[i + 5], result->aal_curve[i + 6],
			result->aal_curve[i + 7]);
}
...
static bool set_hist(struct mml_pq_sub_task *sub_task, bool dual, u8 pipe,
		     u32 *phist, s32 engine)
{
	bool ready;
	...
	ready = (dual && atomic_read(&sub_task->readback_data.pipe_cnt) ==
				 MML_PIPE_CNT) ||
		!dual;
...
int mml_pq_aal_readback(struct mml_task *task, u8 pipe, u32 *phist)
{
	...
	if (!pipe) {
		mml_pq_rb_msg(
			"%s job_id[%d] hist[0~4]={%08x, %08x, %08x, %08x, %08x}",
			__func__, task->job.jobid, phist[0], phist[1], phist[2],
			phist[3], phist[4]);

		mml_pq_rb_msg(
			"%s job_id[%d] hist[10~14]={%08x, %08x, %08x, %08x, %08x}",
			__func__, task->job.jobid, phist[10], phist[11],
			phist[12], phist[13], phist[14]);
	} else {
		mml_pq_rb_msg(
			"%s job_id[%d] hist[600~604]={%08x, %08x, %08x, %08x, %08x}",
			__func__, task->job.jobid, phist[600], phist[601],
			phist[602], phist[603], phist[604]);

		mml_pq_rb_msg(
			"%s job_id[%d] hist[610~614]={%08x, %08x, %08x, %08x, %08x}",
...
void mml_pq_init_comp_config_result(struct mml_pq_comp_config_result *result)
{
	if (default_aal_curve)
		memcpy(result->aal_curve, default_aal_curve,
...
static struct mml_pq_sub_task *wait_next_sub_task(struct mml_pq_chan *chan)
{
	...
	for (;;) {
		...
		ret = wait_event_interruptible(
			chan->msg_wq, (atomic_read(&chan->msg_cnt) > 0));
		if (ret) {
			if (ret != -ERESTARTSYS)
				mml_pq_log(
					"%s wakeup wait_result[%d], msg_cnt[%d]",
					__func__, ret,
					atomic_read(&chan->msg_cnt));
			break;
		}
				    ...
				    struct mml_pq_tile_init_job *job)
{
	...
	rsz_regs[0] = kmalloc_array(result->rsz_reg_cnt[0],
				    sizeof(struct mml_pq_reg), GFP_KERNEL);
	...
	if (result->rsz_param_cnt == MML_MAX_OUTPUTS) {
		rsz_regs[1] =
			kmalloc_array(result->rsz_reg_cnt[1],
				      sizeof(struct mml_pq_reg), GFP_KERNEL);
				      ...
				      struct mml_pq_comp_config_job *job)
{
	...
	if (!job->result_job_id) {
		result = kmalloc(sizeof(*result), GFP_KERNEL);
		if (unlikely(!result)) {
			mml_pq_err("err: create result failed: %d",
				   job->result_job_id);
			return;
		}
		...
		if (unlikely(ret)) {
			mml_pq_err(
				"copy job result to last failed!: %d result_job_id[%d]",
		...
		if (unlikely(ret)) {
			mml_pq_err(
				"copy default_aal_curve!: %d result_job_id[%d]",
				ret, job->result_job_id);
			kfree(result);
			return;
		}
		...
		if (unlikely(ret)) {
			mml_pq_err(
				"copy default_hdr_curve!: %d result_job_id[%d]",
				ret, job->result_job_id);
			kfree(result);
			return;
		}
	...
	if (unlikely(result->param_cnt > MML_MAX_OUTPUTS)) {
		mml_pq_err("invalid param count!: %d", result->param_cnt);
		goto free_comp_config_result;
	}
	...
	hdr_curve = kmalloc_array(HDR_CURVE_NUM, sizeof(u32), GFP_KERNEL);
	...
	result->ds_reg_cnt = result->ds_reg_cnt > DS_MAX_REG_NUM ?
				     ...
				     result->ds_reg_cnt;
	ds_regs =
		kmalloc_array(result->ds_reg_cnt, sizeof(*ds_regs), GFP_KERNEL);
	if (unlikely(!ds_regs)) {
	...
	mml_pq_msg("%s result end, result_id[%d] sub_task[%p]", __func__,
		   job->result_job_id, sub_task);
	goto wake_up_prev_comp_config_task;
free_color_regs:
	kfree(color_regs);
free_ds_regs:
...
static int mml_pq_aal_readback_ioctl(unsigned long data)
{
	...
	mml_pq_msg("%s is_dual[%d] cut_pos_x[%d]", __func__, readback->is_dual,
		   readback->cut_pos_x);

	ret = copy_to_user(&job->result->is_dual, &readback->is_dual,
			   sizeof(bool));
	if (unlikely(ret)) {
	...
	ret = copy_to_user(&job->result->cut_pos_x, &readback->cut_pos_x,
			   sizeof(u32));
	if (unlikely(ret)) {
		mml_pq_err("err: fail to copy to width: %d\n", ret);
		goto wake_up_aal_readback_task;
	...
	if (new_sub_task->readback_data.pipe0_hist) {
		...
		mml_pq_err(
			"err: fail to copy to aal_pipe0_hist because of null pointer");
	}
...
static int mml_pq_hdr_readback_ioctl(unsigned long data)
{
	...
	if (new_sub_task->readback_data.pipe0_hist) {
		...
		mml_pq_err(
			"err: fail to copy to hdr_pipe0_hist because of null pointer");
	}
...
static long mml_pq_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
{
	mml_pq_msg("%s called %#x", __func__, cmd);
	mml_pq_msg("%s tile_init=%#lx comp_config=%#lx", __func__,
		   MML_PQ_IOC_TILE_INIT, MML_PQ_IOC_COMP_CONFIG);
	return -EFAULT;
}
...
int mml_pq_core_init(void)
{
	...
	buffer_num = 0;
	default_aal_curve =
		kmalloc_array(AAL_CURVE_NUM, sizeof(u32), GFP_KERNEL);
	last_result = kzalloc(sizeof(*last_result), GFP_KERNEL);
	last_result->aal_curve =
		kzalloc(sizeof(u32) * AAL_CURVE_NUM, GFP_KERNEL);
	last_result->aal_param =
		kzalloc(sizeof(struct mml_pq_aal_config_param), GFP_KERNEL);
	last_result->aal_regs =
		kzalloc(sizeof(struct mml_pq_reg) * MAX_REG_NUM, GFP_KERNEL);
	last_result->color_regs =
		kzalloc(sizeof(struct mml_pq_reg) * MAX_REG_NUM, GFP_KERNEL);
	last_result->ds_regs =
		kzalloc(sizeof(struct mml_pq_reg) * MAX_REG_NUM, GFP_KERNEL);
	last_result->hdr_curve =
		kzalloc(sizeof(u32) * HDR_CURVE_NUM, GFP_KERNEL);
	last_result->hdr_regs =
		kzalloc(sizeof(struct mml_pq_reg) * MAX_REG_NUM, GFP_KERNEL);
