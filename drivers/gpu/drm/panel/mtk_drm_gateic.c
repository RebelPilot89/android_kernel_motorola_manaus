...
bool mtk_gateic_match_lcm_list(const char *lcm_name, const char **list,
			       unsigned int count, const char *gateic_name)
{
	...
	if (IS_ERR_OR_NULL(gateic_name) == false)
		ret = snprintf(owner, MTK_LCM_NAME_LENGTH - 1, "%s",
			       gateic_name);
	if (ret < 0 || ret >= MTK_LCM_NAME_LENGTH)
		DDPMSG("%s, snprintf failed:%d\n", __func__, ret);

	if (count == 1 && strcmp(*list, "default") == 0) {
...
static struct mtk_gateic_funcs *__maybe_unused mtk_drm_gateic_match_lcm_list(
...
int mtk_drm_gateic_write_bytes(unsigned char addr, unsigned char value,
			       char func)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
...
int mtk_drm_gateic_read_bytes(unsigned char addr, unsigned char *value,
			      char func)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
