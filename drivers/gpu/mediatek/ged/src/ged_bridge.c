...
extern int fpsgo_notify_gpu_block(int tid, int i32BridgeFD, int hint)
	__attribute__((weak));
#endif

static unsigned int ged_boost_enable = 1;
//-----------------------------------------------------------------------------
int ged_bridge_log_buf_get(struct GED_BRIDGE_IN_LOGBUFGET *psLogBufGetIN,
...
//-----------------------------------------------------------------------------
int ged_bridge_log_buf_write(struct GED_BRIDGE_IN_LOGBUFWRITE *psLogBufWriteIN,
			     struct GED_BRIDGE_OUT_LOGBUFWRITE *psLogBufWriteOUT)
{
	...
	psLogBufWriteOUT->eError =
		ged_log_buf_print2(psLogBufWriteIN->hLogBuf,
				   psLogBufWriteIN->attrs, "%s",
				   psLogBufWriteIN->acLogBuf);
	return 0;
}
//-----------------------------------------------------------------------------
int ged_bridge_log_buf_reset(struct GED_BRIDGE_IN_LOGBUFRESET *psLogBufResetIn,
	...
	struct GED_BRIDGE_OUT_BOOSTGPUFREQ *psBoostGpuFreqOUT)
{
	psBoostGpuFreqOUT->eError =
		(mtk_set_bottom_gpu_freq(psBoostGpuFreqIN->eGPUFreqLevel) ==
		 true) ?
			GED_OK :
			GED_ERROR_FAIL;
	return 0;
	...
	struct GED_BRIDGE_OUT_MONITOR3DFENCE *psMonitor3DFenceOUT)
{
	if (psMonitor3DFenceINT->dump_flag == 1) {
		mtk_gpu_fence_debug_dump(psMonitor3DFenceINT->fd,
...
//-----------------------------------------------------------------------------
int ged_bridge_query_info(struct GED_BRIDGE_IN_QUERY_INFO *psQueryInfoINT,
...
//-----------------------------------------------------------------------------
int ged_bridge_notify_vsync(struct GED_BRIDGE_IN_NOTIFY_VSYNC *psNotifyVsyncINT,
			    struct GED_BRIDGE_OUT_NOTIFY_VSYNC *psNotifyVsyncOUT)
{
	psNotifyVsyncOUT->eError = ged_notify_sw_vsync(
		psNotifyVsyncINT->eType, &psNotifyVsyncOUT->sQueryData);

	return 0;
}
//-----------------------------------------------------------------------------
int ged_bridge_dvfs_probe(struct GED_BRIDGE_IN_DVFS_PROBE *psDVFSProbeINT,
...
{
	if (ged_boost_enable) {
		psEVENT_NOTIFYOUT->eError = ged_dvfs_vsync_offset_event_switch(
...
/* -------------------------------------------------------------------------- */
int ged_bridge_gpu_timestamp(struct GED_BRIDGE_IN_GPU_TIMESTAMP *psGpuBeginINT,
			     struct GED_BRIDGE_OUT_GPU_TIMESTAMP *psGpuBeginOUT)
{
	if (ged_kpi_enabled() == 1) {
		if (psGpuBeginINT->QedBuffer_length == -2) {
			psGpuBeginOUT->eError = ged_kpi_dequeue_buffer_ts(
				psGpuBeginINT->pid, psGpuBeginINT->ullWnd,
				psGpuBeginINT->i32FrameID,
				psGpuBeginINT->fence_fd, psGpuBeginINT->isSF);
		} else if (psGpuBeginINT->QedBuffer_length != -1) {
			psGpuBeginOUT->eError = ged_kpi_queue_buffer_ts(
				psGpuBeginINT->pid, psGpuBeginINT->ullWnd,
				psGpuBeginINT->i32FrameID,
				psGpuBeginINT->fence_fd,
				psGpuBeginINT->QedBuffer_length);
		} else {
			psGpuBeginOUT->eError = ged_kpi_acquire_buffer_ts(
				psGpuBeginINT->pid, psGpuBeginINT->ullWnd,
				psGpuBeginINT->i32FrameID);
		}
...
//-----------------------------------------------------------------------------
int ged_bridge_gpu_hint_to_cpu(struct GED_BRIDGE_IN_GPU_HINT_TO_CPU *in,
			       struct GED_BRIDGE_OUT_GPU_HINT_TO_CPU *out)
{
	int ret = 0;
#ifdef CONFIG_MTK_FPSGO_V3
	if (fpsgo_notify_gpu_block)
		ret = fpsgo_notify_gpu_block(in->tid, in->i32BridgeFD,
					     in->hint);
#endif
	out->eError = GED_OK;
	out->boost_flag = ret;
	...
	struct GED_BRIDGE_OUT_QUERY_GPU_DVFS_INFO *QueryGPUDVFSInfoOut)
{
	...
	if (QueryGPUDVFSInfoIn->hint) {
		if (QueryGPUDVFSInfoIn->gift_ratio)
			QueryGPUDVFSInfoOut->eError = ged_kpi_set_gift_status(
				QueryGPUDVFSInfoIn->gift_ratio);
		else
			QueryGPUDVFSInfoOut->eError = ged_kpi_set_gift_status(
				QueryGPUDVFSInfoIn->hint);
	}
	QueryGPUDVFSInfoOut->eError =
		ged_kpi_query_gpu_dvfs_info(QueryGPUDVFSInfoOut);
