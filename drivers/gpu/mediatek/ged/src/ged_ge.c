...
GED_ERROR ged_ge_init(void)
{
	...
	gPoolCache = kmem_cache_create("gralloc_extra", sizeof(struct GEEntry),
				       0, flags, NULL);
...
int ged_ge_alloc(int region_num, uint32_t *region_sizes)
{
	...
	entry->file =
		anon_inode_getfile("gralloc_extra", &GEEntry_fops, entry, 0);
...
{
	if (region_id < 0 || region_id >= entry->region_num || u32_offset < 0 ||
...
int ged_ge_get(int ge_fd, int region_id, int u32_offset, int u32_size,
...
int ged_ge_set(int ge_fd, int region_id, int u32_offset, int u32_size,
	       uint32_t *input_data)
{
	unsigned long flags;
	int err = 0;
...
int ged_dmabuf_set_name(int32_t share_fd, char *name)
{
	...
	if (share_fd < 0) {
		GED_PDEBUG("%s: invalid value of share_fd %d", __func__,
			   share_fd);
		return -1;
	}
...
int ged_bridge_ge_alloc(struct GED_BRIDGE_IN_GE_ALLOC *psALLOC_IN,
...
int ged_bridge_ge_get(struct GED_BRIDGE_IN_GE_GET *psGET_IN,
		      ...
		      int output_buffer_size)
{
	...
	if (psGET_IN->uint32_offset < 0 ||
	    psGET_IN->uint32_offset >=
		    (GE_MAX_REGION_SIZE / sizeof(uint32_t)) ||
	    psGET_IN->uint32_size <= 0 ||
	    psGET_IN->uint32_size > (GE_MAX_REGION_SIZE / sizeof(uint32_t))) {
		GED_PDEBUG("[%s] invalid offset(%d) or size(%d)", __func__,
			   psGET_IN->uint32_offset, psGET_IN->uint32_size);
		return -EFAULT;
	}
	...
	if (output_buffer_size !=
		...
		GED_PDEBUG(
			"[%s] output_buffer_size (%d byte) != header_size + u32_size (%d byte)",
			__func__, (unsigned int)output_buffer_size,
			(unsigned int)(header_size + (psGET_IN->uint32_size *
						      sizeof(uint32_t))));
		return -EFAULT;
	}

	psGET_OUT->eError = ged_ge_get(psGET_IN->ge_fd, psGET_IN->region_id,
				       psGET_IN->uint32_offset,
				       psGET_IN->uint32_size, psGET_OUT->data);
	return 0;
}

int ged_bridge_ge_set(struct GED_BRIDGE_IN_GE_SET *psSET_IN,
		      ...
		      int input_buffer_size)
{
	...
	if (psSET_IN->uint32_offset < 0 ||
		...
		GED_PDEBUG("[%s] invalid offset(%d) or size(%d)", __func__,
			   psSET_IN->uint32_offset, psSET_IN->uint32_size);
		return -EFAULT;
	}
	...
	if (input_buffer_size !=
		...
		GED_PDEBUG(
			"[%s] input_buffer_size (%d byte) != header_size + u32_size (%d byte)",
			__func__, (unsigned int)input_buffer_size,
			(unsigned int)(header_size + (psSET_IN->uint32_size *
						      sizeof(uint32_t))));
		return -EFAULT;
	}

	psSET_OUT->eError = ged_ge_set(psSET_IN->ge_fd, psSET_IN->region_id,
				       psSET_IN->uint32_offset,
				       psSET_IN->uint32_size, psSET_IN->data);
	return 0;
}

int ged_bridge_ge_info(struct GED_BRIDGE_IN_GE_INFO *psINFO_IN,
...
int ged_bridge_dmabuf_set_name(struct GED_BRIDGE_IN_DMABUF_SET_NAME *in,
			       struct GED_BRIDGE_OUT_DMABUF_SET_NAME *out)
{
	if (in == NULL) {
		GED_PDEBUG("%s: GED_BRIDGE_IN_DMABUF_SET_NAME is NULL",
			   __func__);
		return -1;
	}

	if (out == NULL) {
		GED_PDEBUG("%s: GED_BRIDGE_OUT_DMABUF_SET_NAME is NULL",
			   __func__);
		return -1;
	}
