...
#elif IS_ENABLED(CONFIG_MTK_HIGH_FRAME_RATE) && __has_include("disp_arr.h")
#include "disp_arr.h"
#endif
...
enum { g_idle_set_finish, g_idle_set_prepare, g_idle_fix };
				     ...
				     unsigned int cur_max_freq, u64 ulID)
{
	if (ged_kpi_output_gfx_info2_fp)
		ged_kpi_output_gfx_info2_fp(t_gpu, cur_freq, cur_max_freq,
					    ulID);
}
...
static GED_BOOL ged_kpi_find_main_head_func(unsigned long ulID, void *pvoid,
					    void *pvParam)
{
...
{
	ged_hashtable_iterator(gs_hashtable, ged_kpi_find_main_head_func,
			       (void *)NULL);
#ifdef GED_KPI_DEBUG
					  ...
					  struct GED_KPI *psKPI)
{
	...
	frame_attr |=
		((psHead->isSF & GED_KPI_IS_SF_MASK) << GED_KPI_IS_SF_SHIFT);
	...
	frame_attr |=
		((psKPI->i32Gpu_uncompleted & GED_KPI_GPU_UNCOMPLETED_LEN_MASK)
	...
	gpu_info |= ((psKPI->gpu_freq_max & GED_KPI_GPU_FREQ_MAX_INFO_MASK)
		     << GED_KPI_GPU_FREQ_MAX_INFO_SHIFT);
	psKPI->frame_attr = frame_attr;

	psKPI->cpu_gpu_info.gpu.gpu_power = ged_get_power_by_idx(
		ged_get_oppidx_by_freq(psKPI->gpu_freq * 1000));

	ged_get_gpu_utli_ex(&util_ex);

	psKPI->cpu_gpu_info.gpu.gpu_util =
		(util_ex.util_active & 0xff) | ((util_ex.util_3d & 0xff) << 8) |
	...
	ged_log_buf_print(
		...
		psHead->pid, psHead->ullWnd, psKPI->i32QueueID,
		psKPI->i32AcquireID, psKPI->frame_attr,
		...
		gpu_info, psKPI->t_cpu_target, psKPI->t_cpu_fpsgo, psKPI->t_gpu,
		psKPI->gpu_done_interval, vsync_period, g_psGIFT->gift_ratio,
				   ...
				   struct GED_TIMESTAMP *psTimeStamp)
{
	...
	if (psHead) {
		...
		if (psKPI) {
			...
			psHead->t_cpu_remained = psTimeStamp->ullTimeStamp -
						 psKPI->ullTimeStamp1;
			psKPI->t_cpu_remained = vsync_period;
			psKPI->t_cpu_remained -= (psKPI->ullTimeStamp1 -
						  psHead->last_TimeStampS);
			psKPI->t_acquire_period = psTimeStamp->ullTimeStamp -
						  psHead->last_TimeStampS;
...
/* ------------------------------------------------------------------- */
static GED_BOOL ged_kpi_h_iterator_func(unsigned long ulID, void *pvoid,
					void *pvParam)
{
	struct GED_KPI_HEAD *psHead = (struct GED_KPI_HEAD *)pvoid;
...
/* ------------------------------------------------------------------- */
static GED_BOOL ged_kpi_iterator_delete_func(unsigned long ulID, void *pvoid,
					     void *pvParam, GED_BOOL *pbDeleted)
{
	struct GED_KPI_HEAD *psHead = (struct GED_KPI_HEAD *)pvoid;
...
static GED_BOOL
ged_kpi_update_TargetTimeAndTargetFps(struct GED_KPI_HEAD *psHead,
				      int target_fps, int target_fps_margin,
				      int cpu_time, int eara_fps_margin,
				      GED_KPI_FRC_MODE_TYPE mode, int client)
{
	GED_BOOL ret = GED_FALSE;
	...
	if (psHead) {
		...
		if (target_fps > 0 && target_fps <= GED_KPI_FPS_LIMIT) {
			psHead->t_cpu_target =
				(int)((int)GED_KPI_SEC_DIVIDER / target_fps);
			psHead->target_fps = target_fps;
		} else {
			psHead->t_cpu_target = (int)((int)GED_KPI_SEC_DIVIDER /
...
#ifdef GED_KPI_DEBUG
		GED_LOGI(
			"[GED_KPI]FPSGO info PID:%d,tfps:%d,fps_margin:%d,eara_diff:%d,t_cpu:%d\n",
			psHead->pid, psHead->target_fps,
			psHead->target_fps_margin, psHead->eara_fps_margin,
			psHead->t_cpu_fpsgo);
#endif /* GED_KPI_DEBUG */
	}
	return ret;
...
static GED_BOOL
ged_kpi_find_and_delete_miss_tag(u64 ulID, int i32FrameID,
				 GED_TIMESTAMP_TYPE eTimeStampType)
{
	GED_BOOL ret = GED_FALSE;
	...
	if (miss_tag_head) {
		...
		list_for_each_prev_safe (psListEntry, psListEntryTemp, psList) {
			psMiss_tag = list_entry(psListEntry,
						struct GED_KPI_MISS_TAG, sList);
			if (psMiss_tag && psMiss_tag->ulID == ulID &&
			    psMiss_tag->i32FrameID == i32FrameID &&
			    psMiss_tag->eTimeStampType == eTimeStampType) {
				list_del(&psMiss_tag->sList);
				INIT_LIST_HEAD(&psMiss_tag->sList);
...
static void ged_kpi_work_cb(struct work_struct *psWork)
{
	...
	GED_LOGD("[GED_KPI] ts type = %d, pid = %d, wnd = %llu, frame = %lu\n",
		 psTimeStamp->eTimeStampType, psTimeStamp->pid,
		 psTimeStamp->ullWnd, psTimeStamp->i32FrameID);
#endif /* GED_KPI_DEBUG */
	...
	case GED_TIMESTAMP_TYPE_D:
		...
		/* remove */
		ulID = psKPI->ullWnd;
		psHead = (struct GED_KPI_HEAD *)ged_hashtable_find(
			gs_hashtable, (unsigned long)ulID);
		if (psHead) {
			psHead->i32Count -= 1;
		...
		if (!psHead) {
			psHead = (struct GED_KPI_HEAD *)ged_alloc_atomic(
				sizeof(struct GED_KPI_HEAD));
			if (psHead) {
	...
	case GED_TIMESTAMP_TYPE_1:
		...
		if (psHead) {
			...
			list_for_each_prev_safe (psListEntry, psListEntryTemp,
						 psList) {
				psKPI = list_entry(psListEntry, struct GED_KPI,
						   sList);

				if (psKPI && (psKPI->i32DeQueueID ==
					      psTimeStamp->i32FrameID))
					break;
				psKPI = NULL;
			}
			...
			if (ged_kpi_find_and_delete_miss_tag(
				    ulID, psTimeStamp->i32FrameID,
	...
	case GED_TIMESTAMP_TYPE_2:
		...
		if (psHead) {
			...
			if (psKPI) {
				...
				psKPI->cpu_gpu_info.gpu.t_gpu_real =
					((unsigned long long)(psHead->last_TimeStamp2 -
							      psHead->pre_TimeStamp2)) *
				...
				if (psTimeStamp->i32GPUloading) {
					...
					time_spent = psKPI->cpu_gpu_info.gpu
							     .t_gpu_real;
				...
				psKPI->cpu_gpu_info.gpu.gpu_dvfs |=
					(((unsigned long)ged_get_dvfs_margin()) &
					 0x3FF);
				psKPI->cpu_gpu_info.gpu.gpu_dvfs |=
					((((unsigned long)
						   ged_get_dvfs_margin_mode()) &
					  0xFF)
					 << 16);

				if (psTimeStamp->pid != pid_sf &&
				    psTimeStamp->pid != pid_sysui) {
					if (main_head == psHead &&
					    !g_force_gpu_dvfs_fallback) {
						psKPI->cpu_gpu_info.gpu
							.gpu_dvfs |= (0x8000);
						if (ged_is_fdvfs_support())
							mtk_gpueb_dvfs_set_frame_base_dvfs(
								1);
					} else {
						if (ged_is_fdvfs_support())
							mtk_gpueb_dvfs_set_frame_base_dvfs(
				...
				if (main_head == psHead &&
					...
					g_psGIFT->target_fps =
						psHead->target_fps;
					g_psGIFT->target_fps_margin =
						psKPI->target_fps_margin;
					g_psGIFT->eara_fps_margin =
				...
				ged_log_perf_trace_counter(
					"t_gpu", psKPI->t_gpu, psTimeStamp->pid,
					psTimeStamp->i32FrameID, ulID);
				if (main_head == psHead)
	...
	case GED_TIMESTAMP_TYPE_P:
		...
		if (psHead) {
			...
			list_for_each_prev_safe (psListEntry, psListEntryTemp,
				...
				if (psListEntry->prev == NULL ||
				    psListEntry->next == NULL)
					GED_LOGI(
						"[GED_KPI] PID: 0x%x, Wnd:0x%llx, count: %d. prev/next: %p/%p",
						psHead->pid, psHead->ullWnd,
						psHead->i32Count,
						psListEntry->prev,
						psListEntry->next);
			}
			...
			if (psKPI) {
				...
				pre_fence_delay = psTimeStamp->ullTimeStamp -
						  psKPI->ullTimeStamp1;
				ged_log_perf_trace_counter(
					"t_pre_fence_delay", pre_fence_delay,
					psTimeStamp->pid,
					psTimeStamp->i32FrameID, ulID);
				psKPI->ulMask |= GED_TIMESTAMP_TYPE_P;
				psKPI->ullTimeStampP =
...
#ifdef GED_KPI_DEBUG
		if (!psHead) {
			GED_PR_DEBUG("[GED_KPI][Exception] TYPE_S:",
	...
	case GED_TIMESTAMP_TYPE_H:
...
#ifdef GED_KPI_DEBUG
		{
			...
			GED_LOGD(
				...
				t_cpu_target, t_cpu_remained, t_gpu_target,
				t_gpu_remained);
		}
#endif /* GED_KPI_DEBUG */
		break;
	...
	case GED_SET_TARGET_FPS:
		...
		if (psHead) {
			ged_kpi_update_TargetTimeAndTargetFps(
				psHead, target_FPS & 0x000000ff,
				(target_FPS & 0x00000700) >> 8,
				(target_FPS & 0xfffff100) >> 11,
				eara_fps_margin, GED_KPI_FRC_DEFAULT_MODE, -1);
		}
#ifdef GED_KPI_DEBUG
		else
...
/* ------------------------------------------------------------------- */
static GED_ERROR ged_kpi_push_timestamp(GED_TIMESTAMP_TYPE eTimeStampType,
					u64 ullTimeStamp, int pid,
					unsigned long long ullWnd,
					int i32FrameID, int QedBuffer_length,
					int isSF, void *fence_addr)
{
	...
	if (g_psWorkQueue && is_GED_KPI_enabled) {
		...
		case GED_TIMESTAMP_TYPE_D:
			break;
		case GED_TIMESTAMP_TYPE_1:
			ged_log_trace_counter(
				"GED_KPI_QedBuffer_CNT",
				atomic_inc_return(&event_QedBuffer_cnt));
			ged_log_trace_counter(
				"GED_KPI_3D_fence_CNT",
			       ...
			       int QedBuffer_length, void *fence_addr)
{
	return ged_kpi_push_timestamp(GED_TIMESTAMP_TYPE_1, ged_get_time(), pid,
				      ullWdnd, i32FrameID, QedBuffer_length, 0,
				      fence_addr);
}
/* ------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------- */
static void ged_kpi_pre_fence_sync_cb(struct dma_fence *sFence,
...
/* ------------------------------------------------------------------- */
static void ged_kpi_gpu_3d_fence_sync_cb(struct dma_fence *sFence,
					 struct dma_fence_cb *waiter)
{
	struct GED_KPI_GPU_TS *psMonitor;
...
#if defined(MTK_GPU_BM_2)
	mtk_bandwidth_update_info(psMonitor->pid, qos_inc_frame_nr(),
				  qos_get_frame_nr());
#endif /* MTK_GPU_BM_2 */
				  ...
				  int fence_fd, int QedBuffer_length)
{
	...
	psMonitor = (struct GED_KPI_GPU_TS *)ged_alloc(
		sizeof(struct GED_KPI_GPU_TS));
...
{
#ifdef MTK_GED_KPI
	return ged_kpi_push_timestamp(GED_TIMESTAMP_TYPE_H, ged_get_time(), 0,
				      0, 0, 0, 0, NULL);
#else
	return GED_OK;
#endif /* MTK_GED_KPI */
...
void ged_kpi_get_latest_perf_state(long long *t_cpu_remained,
				   long long *t_gpu_remained,
				   long *t_cpu_target, long *t_gpu_target)
{
#ifdef MTK_GED_KPI
...
GED_ERROR ged_kpi_system_init(void)
{
	...
	ghLogBuf_KPI =
		ged_log_buf_alloc(GED_KPI_MAX_FPS * 10,
	...
	g_psGIFT = (struct GED_KPI_MEOW_DVFS_FREQ_PRED *)ged_alloc_atomic(
		sizeof(struct GED_KPI_MEOW_DVFS_FREQ_PRED));
	...
	g_psWorkQueue = alloc_ordered_workqueue("ged_kpi",
...
void ged_kpi_set_target_FPS_margin(u64 ulID, int target_FPS,
				   int target_FPS_margin, int eara_fps_margin,
				   int cpu_time)
{
#ifdef MTK_GED_KPI
...
static GED_BOOL ged_kpi_find_riskyBQ_func(unsigned long ulID, void *pvoid,
					  void *pvParam)
{
	struct GED_KPI_HEAD *psHead = (struct GED_KPI_HEAD *)pvoid;
	struct GED_KPI_HEAD *psRiskyBQ = (struct GED_KPI_HEAD *)pvParam;

	if (psRiskyBQ && psHead && psHead->t_gpu_latest > 0 &&
					   ...
					   unsigned long long *pullWnd)
{
	...
	spin_lock_irqsave(&gs_hashtableLock, ulIRQFlags);
	ged_hashtable_iterator(gs_hashtable, ged_kpi_find_riskyBQ_func,
			       (void *)&sRiskyBQ);
	spin_unlock_irqrestore(&gs_hashtableLock, ulIRQFlags);

	if (sRiskyBQ.ullWnd == 0 || sRiskyBQ.last_TimeStamp2 == 0 ||
	    sRiskyBQ.pre_TimeStamp2 == 0 || sRiskyBQ.t_gpu_latest <= 0 ||
	    sRiskyBQ.t_gpu_target <= 0)
		return ret;

	deltaTime = ((unsigned int)(sRiskyBQ.last_TimeStamp2 -
				    sRiskyBQ.pre_TimeStamp2)) /
		    1000U; // ns -> ms
...
/* ------------------------------------------------------------------- */
GED_ERROR ged_kpi_query_dvfs_freq_pred(int *gpu_freq_cur, int *gpu_freq_max,
				       int *gpu_freq_pred)
{
#ifdef MTK_GED_KPI
	if (gpu_freq_cur == NULL || gpu_freq_max == NULL ||
	    gpu_freq_pred == NULL)
		return GED_ERROR_FAIL;
...
/* ------------------------------------------------------------------- */
GED_ERROR
ged_kpi_query_gpu_dvfs_info(struct GED_BRIDGE_OUT_QUERY_GPU_DVFS_INFO *out)
{
#ifdef MTK_GED_KPI
	if (out == NULL)
		return GED_ERROR_FAIL;
	...
	out->gpu_time =
		(g_psGIFT->gpu_time == -1) ? -1 : g_psGIFT->gpu_time / 1000;

#ifdef GED_KPI_DEBUG
	GED_LOGI("[GED_KPI] pid:%d,freq_c:%d,freq_max:%d,freq_pre:%d,tfps:%d",
		 g_psGIFT->target_pid, out->gpu_freq_cur, out->gpu_freq_max,
		 out->gpu_freq_dvfs_pred, out->target_fps);
	GED_LOGI("[GED_KPI] tfps_mar:%d,eara_diff:%d t_gpu:%d,gift_ratio:%d\n",
		 out->target_fps_margin, out->eara_fps_margin, out->gpu_time,
		 g_psGIFT->gift_ratio);
#endif /* GED_KPI_DEBUG */
