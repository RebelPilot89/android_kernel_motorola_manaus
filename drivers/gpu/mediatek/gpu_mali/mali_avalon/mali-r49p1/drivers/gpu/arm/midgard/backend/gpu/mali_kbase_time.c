...
static struct kbase_timeout_info timeout_info[KBASE_TIMEOUT_SELECTOR_COUNT] = {
	[CSF_FIRMWARE_TIMEOUT] = { "CSF_FIRMWARE_TIMEOUT",
				   MIN(CSF_FIRMWARE_TIMEOUT_CYCLES,
				       CSF_FIRMWARE_PING_TIMEOUT_CYCLES) },
#if IS_ENABLED(CONFIG_MALI_MTK_TIMEOUT_REDUCE)
	[CSF_FIRMWARE_TIMEOUT_AFTER_ABNORMAL_TIMEOUT] = { "CSF_FIRMWARE_TIMEOUT_AFTER_ABNORMAL_TIMEOUT",
							  50000000ull },
#endif /* CONFIG_MALI_MTK_TIMEOUT_REDUCE */
	[CSF_PM_TIMEOUT] = { "CSF_PM_TIMEOUT", CSF_PM_TIMEOUT_CYCLES },
	[CSF_GPU_RESET_TIMEOUT] = { "CSF_GPU_RESET_TIMEOUT",
				    CSF_GPU_RESET_TIMEOUT_CYCLES },
	[CSF_CSG_SUSPEND_TIMEOUT] = { "CSF_CSG_SUSPEND_TIMEOUT",
				      CSF_CSG_SUSPEND_TIMEOUT_CYCLES },
	[CSF_CSG_TERM_TIMEOUT] = { "CSF_CSG_TERM_TIMEOUT",
				   CSF_CSG_TERM_TIMEOUT_CYCLES },
#if IS_ENABLED(CONFIG_MALI_MTK_TIMEOUT_REDUCE)
	[CSF_CSG_SUSPEND_TIMEOUT_AFTER_ABNORMAL_TIMEOUT] = { "CSF_CSG_SUSPEND_TIMEOUT_AFTER_ABNORMAL_TIMEOUT",
							     100000000ull },
#endif /* CONFIG_MALI_MTK_TIMEOUT_REDUCE */
	...
	[IPA_INACTIVE_TIMEOUT] = { "IPA_INACTIVE_TIMEOUT",
				   IPA_INACTIVE_TIMEOUT_CYCLES },
 ...
 */
static inline void
kbase_backend_compute_gpu_ts_offset(struct kbase_device *kbdev)
{
	s64 cpu_ts_ticks = 0;
	s64 gpu_ts_ticks = 0;

	if (kbdev->backend_time.gpu_timestamp_offset !=
	    GPU_TIMESTAMP_OFFSET_INVALID)
		return;
...
void kbase_backend_update_gpu_timestamp_offset(struct kbase_device *kbdev)
{
	...
	dev_dbg(kbdev->dev,
		"Setting GPU timestamp offset register to %lld (%lld ns)",
...
{
	return kbase_reg_read64_coherent(kbdev,
					 GPU_CONTROL_ENUM(TIMESTAMP_OFFSET));
}
...
void kbase_backend_get_gpu_time_norequest(struct kbase_device *kbdev,
					  u64 *cycle_counter, u64 *system_time,
					  struct timespec64 *ts)
{
	...
	if (system_time) {
		...
		u64 gpu_ts = kbase_reg_read64_coherent(
			kbdev, GPU_CONTROL_ENUM(TIMESTAMP));

		/* MTK HW: TIMESTAMP may be reset after powering down MFG; fall back to CNTVCT.
		 * TIMESTAMP runs at ~26MHz while CNTVCT is ~13MHz, so scale CNTVCT by 2 to
		 * provide a compatible timestamp value.
		 */
		if (!kbdev->pm.backend.gpu_powered || gpu_ts == 0)
			*system_time = __arch_counter_get_cntvct() * 2;
		else
			*system_time = gpu_ts;
#else
		*system_time = kbase_reg_read64_coherent(
			kbdev, GPU_CONTROL_ENUM(TIMESTAMP));
#endif
	}
...
void kbase_device_set_timeout_ms(struct kbase_device *kbdev,
...
void kbase_device_set_timeout(struct kbase_device *kbdev,
			      enum kbase_timeout_selector selector,
			      u64 timeout_cycles, u32 cycle_multiplier)
{
	...
	if (selector == CSF_CSG_SUSPEND_TIMEOUT ||
	    selector == KCPU_FENCE_SIGNAL_TIMEOUT ||
	 ...
	 */
	if (check_mul_overflow(timeout_cycles, (u64)cycle_multiplier,
			       &final_cycles))
...
static int kbase_timeout_scaling_init(struct kbase_device *kbdev)
{
	...
	for (selector = 0; selector < KBASE_TIMEOUT_SELECTOR_COUNT;
	     selector++) {
		 ...
		 */
		kbase_device_set_timeout(kbdev, selector, nr_cycles,
					 cycle_multiplier);
	}
	return 0;
}
				  ...
				  enum kbase_timeout_selector selector)
{
	if (unlikely(selector >= KBASE_TIMEOUT_SELECTOR_COUNT)) {
		dev_warn(kbdev->dev,
			 "Querying wrong selector, falling back to default\n");
		selector = KBASE_DEFAULT_TIMEOUT;
	}
...
#if MALI_USE_CSF
u64 __maybe_unused
kbase_backend_time_convert_gpu_to_cpu(struct kbase_device *kbdev, u64 gpu_ts)
{
	if (WARN_ON(!kbdev))
		return 0;
...
/* Additional time helper APIs merged from donor + MTK fixes */
...
/**
 * kbase_backend_get_gpu_time_ns - return current monotonic raw time in ns
 * @kbdev: kbase device (unused)
 */
u64 kbase_backend_get_gpu_time_ns(struct kbase_device *kbdev)
{
	(void)kbdev;
	return ktime_get_raw_ns();
}
KBASE_EXPORT_TEST_API(kbase_backend_get_gpu_time_ns);

/**
 * kbase_backend_get_gpu_time_ms - return current monotonic raw time in ms
 */
u64 kbase_backend_get_gpu_time_ms(struct kbase_device *kbdev)
{
	return kbase_backend_get_gpu_time_ns(kbdev) / NSEC_PER_MSEC;
}
KBASE_EXPORT_TEST_API(kbase_backend_get_gpu_time_ms);

/**
 * kbase_backend_get_gpu_time_struct - populate timespec64 with raw time
 */
void kbase_backend_get_gpu_time_struct(struct kbase_device *kbdev,
				       struct timespec64 *ts)
{
	(void)kbdev;
	ktime_get_raw_ts64(ts);
}
KBASE_EXPORT_TEST_API(kbase_backend_get_gpu_time_struct);

/**
 * kbase_backend_get_gpu_boot_time_ns - time since boot in ns (fast path)
 */
u64 kbase_backend_get_gpu_boot_time_ns(struct kbase_device *kbdev)
{
	(void)kbdev;
	return ktime_get_boot_fast_ns();
}
KBASE_EXPORT_TEST_API(kbase_backend_get_gpu_boot_time_ns);
