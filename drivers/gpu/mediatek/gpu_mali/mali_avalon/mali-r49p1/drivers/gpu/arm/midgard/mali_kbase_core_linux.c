...
#include <mali_kbase_caps.h>
	...
	IS_ENABLED(CONFIG_MALI_MTK_DEBUG_DUMP)
...
#define KBASE_API_VERSION(major, minor)                                        \
	((((major) & 0xFFFU) << 20U) | (((minor) & 0xFFFU) << 8U) |            \
	 ((0U & 0xFFU) << 0U))
...
static const struct mali_kbase_capability_def
	kbase_caps_table[MALI_KBASE_NUM_CAPS] = {
#if MALI_USE_CSF
		{ 1, 0 }, /* SYSTEM_MONITOR */
...
bool mali_kbase_supports_cap(unsigned long api_version, enum mali_kbase_cap cap)
{
	...
	required_ver = KBASE_API_VERSION(cap_def->required_major,
					 cap_def->required_minor);
...
static int get_irqs(struct kbase_device *kbdev, struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(irq_names_caps); i++) {
		...
		if (irq < 0) {
			static const char *const irq_names[] = { "job", "mmu",
								 "gpu" };
				  ...
				  struct kbase_ioctl_get_gpuprops *get_props)
{
	...
	err = copy_to_user(u64_to_user_ptr(get_props->buffer),
			   kprops->prop_buffer, kprops->prop_buffer_size);
	if (err)
		return -EFAULT;
	return (int)kprops->prop_buffer_size;
}
				  ...
				  union kbase_ioctl_mem_alloc_ex *alloc_ex)
{
	...
	bool gpu_executable =
		(flags & BASE_MEM_PROT_GPU_EX) && kbase_has_exec_va_zone(kctx);
	...
	if ((flags & BASE_MEM_FIXED) &&
	    (atomic64_read(&kctx->num_fixable_allocs) > 0))
		return -EINVAL;

	if ((flags & BASE_MEM_FIXABLE) &&
	    (atomic64_read(&kctx->num_fixed_allocs) > 0))
		return -EINVAL;
	 ...
	 */
	if (flags & BASE_MEM_FIXED) {
		u64 aligned_fixed_address =
			alloc_ex->in.fixed_address & PAGE_MASK;
	...
	if (flags & BASE_MEM_CSF_EVENT) {
		...
		flags |= (BASE_MEM_SAME_VA | BASE_MEM_CACHED_CPU |
			  BASE_MEM_COHERENT_SYSTEM);
	}

#if IS_ENABLED(CONFIG_MALI_MTK_MEMORY_DEBUG)
	reg = kbase_mem_alloc(kctx, alloc_ex->in.va_pages,
			      alloc_ex->in.commit_pages, alloc_ex->in.extension,
...
{
	return gpu_model_set_dummy_prfcnt_user_sample(
		u64_to_user_ptr(values->data), values->size);
}
			       ...
			       union kbase_ioctl_mem_alias *alias)
{
	...
	alias->out.gpu_va =
		kbase_mem_alias(kctx, &flags, alias->in.stride, alias->in.nents,
				...
				union kbase_ioctl_mem_import *import)
{
	...
	ret = kbase_mem_import(kctx, import->in.type,
			       u64_to_user_ptr(import->in.phandle),
			       import->in.padding, &import->out.gpu_va,
				...
				struct kbase_ioctl_sticky_resource_unmap *unmap)
{
	...
	for (i = 0; i < unmap->count; i++) {
		if (!kbase_sticky_resource_release_force(kctx, NULL,
							 gpu_addr[i])) {
...
static int
kbasep_cs_queue_register_ex(struct kbase_context *kctx,
...
static int kbasep_queue_group_clear_faults(
	struct kbase_context *kctx,
...
static int kbasep_cs_queue_group_create_1_18(
	struct kbase_context *kctx,
			     ...
			     union kbase_ioctl_cs_queue_group_create *create)
{
	...
	if (create->in.reserved != 0) {
		dev_warn(
			kctx->kbdev->dev,
			"Invalid reserved field not 0 in queue group create\n");
		return -EINVAL;
	}
			  ...
			  union kbase_ioctl_cs_tiler_heap_init *heap_init)
{
	...
	return kbase_csf_tiler_heap_init(
		...
		heap_init->in.buf_desc_va, &heap_init->out.gpu_heap_va,
		&heap_init->out.first_chunk_va);
}

static int kbasep_cs_tiler_heap_init_1_13(
	struct kbase_context *kctx,
	union kbase_ioctl_cs_tiler_heap_init_1_13 *heap_init)
{
	...
	return kbase_csf_tiler_heap_init(
		kctx, heap_init->in.chunk_size, heap_init->in.initial_chunks,
		heap_init->in.max_chunks, heap_init->in.target_in_flight, 0,
...
static int kbasep_ioctl_cs_cpu_queue_dump(
	struct kbase_context *kctx,
	struct kbase_ioctl_cs_cpu_queue_info *cpu_queue_info)
{
	return kbase_csf_cpu_queue_dump_buffer(kctx, cpu_queue_info->buffer,
					       cpu_queue_info->size);
}

static int
kbase_ioctl_read_user_page(struct kbase_context *kctx,
			   union kbase_ioctl_read_user_page *user_page)
{
	...
	if (!kbdev->pm.backend.gpu_powered)
		...
		user_page->out.val_lo =
			kbase_reg_read32(kbdev, USER_ENUM(LATEST_FLUSH));
...
static int kbasep_ioctl_context_priority_check(
	struct kbase_context *kctx,
	struct kbase_ioctl_context_priority_check *priority_check)
{
#if MALI_USE_CSF
	priority_check->priority =
		kbase_csf_priority_check(kctx->kbdev, priority_check->priority);
#else
	base_jd_prio req_priority = (base_jd_prio)priority_check->priority;

	priority_check->priority =
		(u8)kbase_js_priority_check(kctx->kbdev, req_priority);
#endif
	return 0;
}
...
static int kbasep_ioctl_internal_fence_wait(
	struct kbase_context *kctx,
	struct kbase_ioctl_internal_fence_wait *fence_wait)
{
#if IS_ENABLED(CONFIG_MALI_MTK_MBRAIN_SUPPORT)
	if (fence_wait->time_in_microseconds == 2000 ||
	    fence_wait->time_in_microseconds == 3000 ||
	    fence_wait->time_in_microseconds == 4000 ||
	    fence_wait->time_in_microseconds == 5000) {
		ged_mali_event_notify_fence_timeout_event(
			kctx->tgid, FENCE_TYPE_INTERNAL,
			fence_wait->time_in_microseconds / 1000);
	}
#endif /* CONFIG_MALI_MTK_MBRAIN_SUPPORT */
	if (fence_wait->time_in_microseconds == 2000 ||
	    fence_wait->time_in_microseconds == 3000) {
		dev_info(
			...
			fence_wait->time_in_microseconds, fence_wait->flags,
			fence_wait->pid);
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_type_print(
			kctx->kbdev,
		...
		if (fence_wait->flags & BASE_INTERNAL_FENCE_WAIT_DUMP_FLAG) {
			...
			mtk_common_debug(MTK_COMMON_DBG_DUMP_INFRA_STATUS, kctx,
					 MTK_DBG_HOOK_FENCE_INTERNAL_TIMEOUT);
			mtk_common_debug(MTK_COMMON_DBG_DUMP_PM_STATUS, kctx,
					 MTK_DBG_HOOK_FENCE_INTERNAL_TIMEOUT);
			mtk_common_debug(MTK_COMMON_DBG_DUMP_DB_BY_SETTING,
					 kctx,
					 MTK_DBG_HOOK_FENCE_INTERNAL_TIMEOUT);
			if (kctx->tgid != (int)fence_wait->pid) {
				list_for_each_entry (kctx_pid,
						     &kctx->kbdev->kctx_list,
						     kctx_list_link) {
					if (kctx_pid->tgid ==
					    (int)fence_wait->pid) {
						mtk_common_debug(
...
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
#if IS_ENABLED(CONFIG_MALI_MTK_MBRAIN_SUPPORT)
			ged_mali_event_update_gpu_reset_nolock(
	...
	if ((fence_wait->time_in_microseconds == 4000 ||
	     fence_wait->time_in_microseconds == 5000 ||
	     fence_wait->time_in_microseconds == 6000 ||
	     fence_wait->time_in_microseconds == 7000 ||
	     fence_wait->time_in_microseconds == 8000 ||
	     fence_wait->time_in_microseconds == 9000) &&
	    (fence_wait->queue != 0)) {
		...
		mtk_logbuffer_type_print(
			...
			fence_wait->time_in_microseconds);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		mtk_qinspect_recovery(kctx, QINSPECT_CPU_QUEUE,
				      &fence_wait->queue);
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case KBASE_IOCTL_READ_USER_PAGE:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_READ_USER_PAGE,
			  ...
			  loff_t *f_pos)
{
	...
	if (!read_event && !read_error) {
		bool dump =
			kbase_csf_cpu_queue_read_dump_req(kctx, &event_data);
...
int kbase_event_pending(struct kbase_context *kctx)
{
	...
	return (atomic_read(&kctx->event_count) != 0) ||
	       kbase_csf_event_error_pending(kctx) ||
...
int kbase_event_pending(struct kbase_context *kctx)
{
	...
	return (atomic_read(&kctx->event_count) != 0) ||
	       (atomic_read(&kctx->event_closed) != 0);
}
#endif
...
static const struct file_operations kbase_fops = {
	...
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY) &&                             \
	IS_ENABLED(CONFIG_MALI_MTK_GPU_DVFS_HINT_26M_LOADING)
static ssize_t dvfs_hint_26m_perf_cnting_show(struct device *dev,
					      ...
					      char *const buf)
{
	...
	if (dvfs_hint_26m_perf_cnting_enable)
		ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				 "dvfs_hint_26m_perf_cnting is enabled\n");
	else
		ret += scnprintf(buf + ret, PAGE_SIZE - ret,
				 "dvfs_hint_26m_perf_cnting is disabled\n");

	return ret;
}
static ssize_t dvfs_hint_26m_perf_cnting_store(struct device *dev,
					       struct device_attribute *attr,
...
static ssize_t ipa_enable_show(struct device *dev,
...
static ssize_t ipa_enable_store(struct device *dev,
 ...
 */
static ssize_t power_policy_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct kbase_device *kbdev;
...
#if IS_ENABLED(CONFIG_MALI_MTK_PAGE_FAULT_WB_TILER_RECLAIM)
static ssize_t force_reclaim_show(struct device *dev,
				  struct device_attribute *attr,
				  char *const buf)
{
	return 0;
...
static ssize_t force_reclaim_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct kbase_device *kbdev;
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t core_mask_show(struct device *dev, struct device_attribute *attr,
			      char *const buf)
{
	struct kbase_device *kbdev;
	unsigned long flags;
...
static int core_mask_parse(struct kbase_device *const kbdev,
...
static int core_mask_set(struct kbase_device *kbdev,
			 struct kbase_core_mask *const new_mask)
{
			   ...
			   struct kbase_core_mask *const mask)
{
	...
	items = sscanf(buf, "%llx %llx %llx", &mask->new_core_mask[0],
		       &mask->new_core_mask[1], &mask->new_core_mask[2]);
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t js_timeouts_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	struct kbase_device *kbdev;
...
#define UPDATE_TIMEOUT(ticks_name, ms_name, default)                           \
	do {                                                                   \
		js_data->ticks_name = timeout_ms_to_ticks(                     \
			kbdev, ms_name, default, js_data->ticks_name);         \
		dev_dbg(kbdev->dev,                                            \
			"Overriding " #ticks_name                              \
			" with %lu ticks (%lu ms)\n",                          \
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t js_timeouts_show(struct device *dev,
				struct device_attribute *attr, char *const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t debug_command_show(struct device *dev,
				  struct device_attribute *attr,
				  char *const buf)
{
	struct kbase_device *kbdev;
	...
	for (i = 0; i < KBASEP_DEBUG_COMMAND_COUNT && ret < (ssize_t)PAGE_SIZE;
	     i++)
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t debug_command_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct kbase_device *kbdev;
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t gpuinfo_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	...
	if (product_model == GPU_ID_PRODUCT_TTUX) {
		const bool rt_supported =
			gpu_props->gpu_features.ray_intersection;
		...
		if (nr_cores < 7) {
			dev_warn(kbdev->dev, "nr_cores(%u) GPU ID must be G615",
				 nr_cores);
			product_name = "Mali-G615";
		} else
			dev_dbg(kbdev->dev, "GPU ID_Name: %s, nr_cores(%u)\n",
				product_name, nr_cores);
	}
	...
	if (product_model == GPU_ID_PRODUCT_TTIX) {
		...
		if ((nr_cores >= 10) && rt_supported)
			...
			product_name =
				(nr_cores >= 6) ? "Mali-G720" : "Mali-G620";

		dev_dbg(kbdev->dev,
			"GPU ID_Name: %s (ID: 0x%x), nr_cores(%u)\n",
			product_name, product_id, nr_cores);
	}
	...
	return scnprintf(buf, PAGE_SIZE, "%s %d cores r%dp%d 0x%08X\n",
			 ...
			 gpu_props->gpu_id.version_major,
				 ...
				 size_t count)
{
	...
	if (ret || dvfs_period <= 0) {
		dev_err(kbdev->dev,
			"Couldn't process dvfs_period write operation.\n"
				   ...
				   const char *buf, size_t count)
{
	...
	if (ret || reset_timeout == 0) {
		dev_err(kbdev->dev,
			"Couldn't process reset_timeout write operation.\n"
	...
	if (reset_timeout < default_reset_timeout)
		dev_warn(
			kbdev->dev,
			"requested reset_timeout(%u) is smaller than default(%u)",
				  ...
				  char *const buf)
{
	...
	return kbase_debugfs_helper_get_attr_to_string(
		buf, PAGE_SIZE, kbdev->mem_pools.small,
				     ...
				     char *const buf)
{
	...
	return kbase_debugfs_helper_get_attr_to_string(
		buf, PAGE_SIZE, kbdev->mem_pools.large,
		MEMORY_GROUP_MANAGER_NR_GROUPS, kbase_mem_pool_debugfs_size);
}
						 ...
						 char *const buf)
{
	...
	return kbase_debugfs_helper_get_attr_to_string(
		buf, PAGE_SIZE, kbdev->mem_pools.small, 1,
		kbase_mem_pool_debugfs_max_size);
}
						...
						const char *buf, size_t count)
{
	...
	for (gid = 0; gid < MEMORY_GROUP_MANAGER_NR_GROUPS; ++gid)
		kbase_mem_pool_debugfs_set_max_size(kbdev->mem_pools.small, gid,
						    (size_t)new_size);
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t show_simplified_lp_mem_pool_max_size(
	struct device *dev, struct device_attribute *attr, char *const buf)
{
	struct kbase_device *const kbdev = to_kbase_device(dev);
	...
	return kbase_debugfs_helper_get_attr_to_string(
		buf, PAGE_SIZE, kbdev->mem_pools.large, 1,
		kbase_mem_pool_debugfs_max_size);
}
 ...
 */
static ssize_t
set_simplified_lp_mem_pool_max_size(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct kbase_device *const kbdev = to_kbase_device(dev);
	...
	for (gid = 0; gid < MEMORY_GROUP_MANAGER_NR_GROUPS; ++gid)
		kbase_mem_pool_debugfs_set_max_size(kbdev->mem_pools.large, gid,
						    (size_t)new_size);
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t show_simplified_ctx_default_max_size(
	struct device *dev, struct device_attribute *attr, char *const buf)
{
 ...
 */
static ssize_t
set_simplified_ctx_default_max_size(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct kbase_device *kbdev;
...
static struct {
	...
	{ "full-reset", KBASE_SERIALIZE_INTRA_SLOT |
...
int kbase_protected_mode_init(struct kbase_device *kbdev)
{
	...
	kbdev->protected_dev =
		kzalloc(sizeof(*kbdev->protected_dev), GFP_KERNEL);
...
static bool kbase_is_pm_enabled(const struct device_node *gpu_node)
{
	...
	if (!power_model_node)
		power_model_node =
			of_get_child_by_name(gpu_node, "power_model");
	...
	operating_point_node =
		of_get_property(gpu_node, "operating-points", NULL);
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	BUILD_BUG_ON(ARRAY_SIZE(regulator_names) <
...
DEFINE_DEBUGFS_ATTRIBUTE(fops_trigger_reset, NULL,
			 &kbase_device_debugfs_reset_write, "%llu\n");
...
static int
kbase_device_debugfs_jit_reclaim_timeout_ms_show(struct seq_file *sfile,
						 void *data)
{
...
static ssize_t kbase_device_debugfs_jit_reclaim_timeout_ms_write(
	struct file *file, const char __user *ubuf, size_t count, loff_t *ppos)
{
	const struct seq_file *const sfile =
							    ...
							    struct file *file)
{
	return single_open(file,
			   kbase_device_debugfs_jit_reclaim_timeout_ms_show,
			   in->i_private);
}
...
static const struct file_operations kbase_device_debugfs_pt_clustering_fops = {
	.owner = THIS_MODULE,
...
debugfs_ctx_defaults_init(struct kbase_device *const kbdev)
{
	...
	dentry = debugfs_create_file(
		"mem_pool_max_size", mode, debugfs_ctx_defaults_directory,
		&kbdev->mem_pool_defaults.small,
		&kbase_device_debugfs_mem_pool_max_size_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create mem_pool_max_size debugfs entry\n");
		return dentry;
	}

	dentry = debugfs_create_file(
		"lp_mem_pool_max_size", mode, debugfs_ctx_defaults_directory,
		&kbdev->mem_pool_defaults.large,
		&kbase_device_debugfs_mem_pool_max_size_fops);
	if (IS_ERR_OR_NULL(dentry))
		dev_err(kbdev->dev,
			"Unable to create lp_mem_pool_max_size debugfs entry\n");
...
static struct dentry *init_debugfs(struct kbase_device *kbdev)
{
	...
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Couldn't create mali debugfs ctx directory\n");
		return dentry;
	}

	dentry = debugfs_create_dir("instrumentation",
	...
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create quirks_sc debugfs entry\n");
		return dentry;
	}
	...
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create quirks_tiler debugfs entry\n");
		return dentry;
	}
	...
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create quirks_mmu debugfs entry\n");
		return dentry;
	}

	dentry = debugfs_create_file("quirks_gpu", 0644,
				     kbdev->mali_debugfs_directory, kbdev,
				     &fops_gpu_quirks);
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create quirks_gpu debugfs entry\n");
		return dentry;
	}
	...
	if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_PROTECTED_DEBUG_MODE)) {
		dentry = debugfs_create_file("protected_debug_mode", 0444,
					     kbdev->mali_debugfs_directory,
					     kbdev, &fops_protected_debug_mode);
		if (IS_ERR_OR_NULL(dentry)) {
			dev_err(kbdev->dev,
	...
	if (IS_ERR_OR_NULL(dentry)) {
		dev_err(kbdev->dev,
			"Unable to create serialize_jobs debugfs entry\n");
		return dentry;
	}
 ...
 * coherency mode.
 */
static bool kbase_device_supports_coherency_mode(struct kbase_device *kbdev,
						 u32 coherency_mode)
{
	u32 supported_coherency_bitmap =
		kbase_device_normalize_coherency_bitmap(kbdev);
	...
	if (!(supported_coherency_bitmap &
	      COHERENCY_FEATURE_BIT(coherency_mode))) {
		dev_warn(
			kbdev->dev,
			"Device does not support coherency mode %u set from dtb",
			coherency_mode);
		return false;
	}
...
int kbase_device_coherency_init(struct kbase_device *kbdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		coherency_override_dts = of_get_property(
			kbdev->dev->of_node, "system-coherency", NULL);
		...
		if (coherency_override_dts)
			override_coherency =
				be32_to_cpup(coherency_override_dts);
		else
		...
		if (!kbase_device_supports_coherency_mode(kbdev,
							  override_coherency)) {
			err = -EINVAL;
			goto early_exit;
		}
...
bool kbasep_adjust_prioritized_process(struct kbase_device *kbdev, bool add,
				       uint32_t tgid)
{
	struct kbase_context *kctx;
...
static ssize_t add_prioritized_process_store(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t count)
{
	struct kbase_device *kbdev;
...
static ssize_t remove_prioritized_process_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct kbase_device *kbdev;
					  ...
					  const char *buf, size_t count)
{
	...
	if (kstrtou32(buf, 0, &dur_us)) {
		dev_err(kbdev->dev,
			"Couldn't process idle_hysteresis_time write operation.\n"
	 ...
	 */
	kbase_csf_firmware_set_gpu_idle_hysteresis_time(
		kbdev, (u64)dur_us * NSEC_PER_USEC);
					 ...
					 char *const buf)
{
	...
	dur_us = div_u64(kbase_csf_firmware_get_gpu_idle_hysteresis_time(kbdev),
			 NSEC_PER_USEC);
	ret = scnprintf(buf, PAGE_SIZE, "%u\n", (u32)dur_us);
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t idle_hysteresis_time_ns_store(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	...
	if (kstrtou64(buf, 0, &dur_ns)) {
		dev_err(kbdev->dev,
			"Couldn't process idle_hysteresis_time_ns write operation.\n"
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t idle_hysteresis_time_ns_show(struct device *dev,
					    struct device_attribute *attr,
					    char *const buf)
{
	struct kbase_device *kbdev;
 ...
 */
static ssize_t mcu_shader_pwroff_timeout_show(struct device *dev,
					      struct device_attribute *attr,
					      char *const buf)
{
	...
	pwroff_us = div_u64(kbase_csf_firmware_get_mcu_core_pwroff_time(kbdev),
			    NSEC_PER_USEC);
	return scnprintf(buf, PAGE_SIZE, "%u\n", (u32)pwroff_us);
}
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t mcu_shader_pwroff_timeout_store(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
	 ...
	 * But the unit of the input parameter of this function is ns, so multiply by 1000
	 */
	kbase_csf_firmware_set_mcu_core_pwroff_time(
		kbdev, (u64)dur_us * NSEC_PER_USEC);
 ...
 *         function succeeded. A Negative value on failure.
 */
static ssize_t mcu_shader_pwroff_timeout_ns_show(struct device *dev,
						 struct device_attribute *attr,
						 char *const buf)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t mcu_shader_pwroff_timeout_ns_store(struct device *dev,
						  struct device_attribute *attr,
						  const char *buf, size_t count)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
 ...
 *         function succeeded. A Negative value on failure.
 */
static ssize_t mmu_dbg_config_show(struct device *dev,
				   struct device_attribute *attr,
				   char *const buf)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
 ...
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t mmu_dbg_config_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
...
static struct attribute *kbase_mempool_attrs[] = {
	&dev_attr_max_size.attr, &dev_attr_lp_max_size.attr,
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	if (err) {
		...
		if (PAGE_SHIFT != 12)
			dev_warn(
				kbdev->dev,
				"Experimental feature: %s with Page Size of %luKiB",
				dev_name(kbdev->mdev.this_device),
				PAGE_SIZE / 1024);
				      ...
				      u8 atom_id)
{
	trace_mali_job_slots_event(dev_id, event,
				   (kctx != NULL ? (u32)kctx->tgid : 0U),
				   (kctx != NULL ? (u32)kctx->pid : 0U),
				   atom_id);
}
