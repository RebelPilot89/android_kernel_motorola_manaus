...
// GPU stall counter (canonical for MT6879)
...
static uint32_t _read_lsc_u_rate(void)
{
	...
	static int pos_ls_mem_write_full, pos_ls_mem_write_short,
		pos_ls_mem_atomic;
	...
	if (!lsc_binited) {
		result |= _find_name_pos(ls_mem_read_full_Str,
					 &pos_ls_mem_read_full);
		result |= _find_name_pos(ls_mem_read_short_str,
					 &pos_ls_mem_read_short);
		result |= _find_name_pos(ls_mem_write_full_str,
					 &pos_ls_mem_write_full);
		result |= _find_name_pos(ls_mem_write_short_str,
					 &pos_ls_mem_write_short);
	...
	if (lsc_binited) {
		...
		if (exec_active) {
			/* Check overflow */
			if (lsc_active <
			    (mali_pmus[pos_ls_mem_read_full].value +
			     mali_pmus[pos_ls_mem_write_full].value))
				urate = 0;
			else
...
static uint32_t _read_shader_u_rate_w_loading(void)
{
	...
	return (!exec_core_active || !active_cycle) ?
		       1 :
		       _cal_urate(exec_core_active, active_cycle);
}
...
static uint32_t _read_alu_u_rate_w_loading(void)
{
	...
	return (!exec_instr_count || !active_cycle) ?
		       1 :
		       _cal_urate(exec_instr_count, active_cycle);
}
...
static uint32_t _read_tex_u_rate_w_loading(void)
{
	...
	return (!tex_coord_issue || !active_cycle) ?
		       1 :
		       _cal_urate(tex_coord_issue, active_cycle);
}
...
static uint32_t _read_lsc_u_rate_w_loading(void)
{
	...
	return (!lsc_active || !active_cycle) ?
		       1 :
		       _cal_urate(lsc_active, active_cycle);
}
...
static uint32_t _read_var_u_rate_w_loading(void)
{
	...
	return (!var_active || !active_cycle) ?
		       1 :
		       _cal_urate(var_active, active_cycle);
}
...
static int _mtk_mfg_update_counter(void)
{
	...
	if (ret != PMU_NG) {
		...
		int shader_block, block_type, i, j, name_offset, data_offset,
			cnt, nr_hwc_blocks;
		...
		for (i = 0; i < nr_hwc_blocks; i++) {
			...
			for (j = 0; j < MALI_COUNTERS_PER_BLOCK; j++) {
				...
				if (strstr(mali_pmus[cnt].name, "GPU_ACTIVE")) {
					active_cycle = (uint32_t)div_u64(
						(u64)(gpu_freq * timd_diff_us),
						1000);
...
FINISH:
	if (handle) {
		...
		start_utime =
			tv_start.tv_sec * 1000000 + (tv_start.tv_nsec / 1000);
	}

	return ret;
}

static int mali_get_gpu_pmu_init(struct GPU_PMU *pmus, int pmu_size,
				 int *ret_size)
{
	int ret = PMU_OK;
	int block[RESERVED_BLOCK + 1] = { 0 };
	...
	if (pmus) {
		...
		spin_lock_irqsave(&counter_info_lock, flags);
#if IS_ENABLED(CONFIG_MTK_GPU_SWPM_SUPPORT)
		...
		spin_unlock_irqrestore(&counter_info_lock, flags);
	}
...
void mtk_gpu_stall_stop(void)
{
	if (io_addr_gpu_stall) {
		writel(0x00000000,
		       io_addr_gpu_stall + OFFSET_STALL_GPU_M0_WR_CNT);
		writel(0x00000000,
		       io_addr_gpu_stall + OFFSET_STALL_GPU_M0_RD_CNT);
		writel(0x00000000,
		       io_addr_gpu_stall + OFFSET_STALL_GPU_M1_WR_CNT);
		writel(0x00000000,
		       io_addr_gpu_stall + OFFSET_STALL_GPU_M1_RD_CNT);
	}
}
...
void mtk_GPU_STALL_RAW(unsigned int *diff, int size)
{
	...
	if (io_addr_gpu_stall) {
		stall_counters[0] =
			((unsigned int)readl(io_addr_gpu_stall +
					     OFFSET_STALL_GPU_M0_WR_CNT)) >>
			1;
		stall_counters[1] =
			((unsigned int)readl(io_addr_gpu_stall +
					     OFFSET_STALL_GPU_M0_RD_CNT)) >>
			1;
		stall_counters[2] =
			((unsigned int)readl(io_addr_gpu_stall +
					     OFFSET_STALL_GPU_M1_WR_CNT)) >>
			1;
		stall_counters[3] =
			((unsigned int)readl(io_addr_gpu_stall +
					     OFFSET_STALL_GPU_M1_RD_CNT)) >>
			1;

		for (i = 0; i < size; i++) {
			if (pre_stall_counters[i] > stall_counters[i]) {
				diff[i] = stall_counters[i] +
					  (0xFFFFFFFF - pre_stall_counters[i]);
			} else {
				diff[i] = stall_counters[i] -
					  pre_stall_counters[i];
			}
		}
		for (i = 0; i < size; i++) {
