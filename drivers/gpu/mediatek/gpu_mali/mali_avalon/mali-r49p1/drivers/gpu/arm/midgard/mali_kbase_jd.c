...
static void __user *get_compat_pointer(struct kbase_context *kctx, const u64 p)
{
	...
	if (kbase_ctx_flag(kctx, KCTX_COMPAT))
		return compat_ptr(p);
#endif
	return (void __user *)(unsigned long)p;
}
...
void kbase_jd_free_external_resources(struct kbase_jd_atom *katom)
{
	 ...
	 * been queued are done before continuing. Any successfully completed
	 * atom would have had all its callbacks completed before the atom was
	 * run, so only flush for failed atoms.
	 */
					   ...
					   const struct base_jd_atom *user_atom)
{
	...
	katom->extres = kmalloc_array(katom->nr_extres, sizeof(*katom->extres),
				      GFP_KERNEL);
	if (!katom->extres)
		return -ENOMEM;
...
static inline void jd_resolve_dep(struct list_head *out_list,
				  struct kbase_jd_atom *katom, u8 d,
				  bool ctx_is_dying)
{
	u8 other_d = !d;
	...
	while (!list_empty(&katom->dep_head[d])) {
		...
		other_dep_atom =
			(struct kbase_jd_atom *)kbase_jd_katom_dep_atom(
				&dep_atom->dep[other_d]);
		...
		if (!dep_atom->in_jd_list &&
			...
			if (likely(dep_count == -1)) {
				...
				/* There are active callbacks or fences not yet handled */
				dep_satisfied = false;
			}
#endif /* CONFIG_MALI_DMA_FENCE */
				   ...
				   struct kbase_jd_atom *node)
{
	...
	for (i = 0; i < 2; i++) {
		...
		list_for_each (pos, &node->dep_head[i]) {
			struct kbase_jd_atom *dep_atom = list_entry(
				pos, struct kbase_jd_atom, dep_item[i]);

			if (IS_GPU_ATOM(dep_atom) && !dep_atom->in_jd_list) {
				/*Check if atom deps look sane*/
				bool dep0_valid =
					is_dep_valid(dep_atom->dep[0].atom);
				bool dep1_valid =
					is_dep_valid(dep_atom->dep[1].atom);
...
static void jd_update_jit_usage(struct kbase_jd_atom *katom)
{
	 ...
	 */
	for (idx = 0; idx < ARRAY_SIZE(katom->jit_ids) && katom->jit_ids[idx];
	     idx++) {
		...
		ptr = kbase_vmap_prot(kctx, reg->heap_info_gpu_addr,
				      size_to_read, KBASE_REG_CPU_RD, &mapping);
		...
		if (used_pages > reg->nr_pages) {
			dev_warn(
				...
				__func__, idx, katom->jit_ids[idx],
				reg->start_pfn << PAGE_SHIFT, used_pages,
				reg->nr_pages, read_val,
...
bool kbase_jd_done_nolock(struct kbase_jd_atom *katom, bool post_immediately)
{
	...
	/* Ensure any dma-fence callbacks / dep counters are cleaned up for this
	 * completed atom to avoid callback leaks or races with the fence worker.
	 */
#ifdef CONFIG_MALI_DMA_FENCE
	...
	while (!list_empty(&completed_jobs)) {
		katom = list_entry(completed_jobs.prev, struct kbase_jd_atom,
				   jd_item);
		list_del(completed_jobs.prev);
		KBASE_DEBUG_ASSERT(katom->status ==
		...
		while (!list_empty(&runnable_jobs)) {
			...
			dev_dbg(kctx->kbdev->dev,
				"List node %pK has status %d\n", node,
				node->status);

			KBASE_DEBUG_ASSERT(node->status !=
					   KBASE_JD_ATOM_STATE_UNUSED);
				 ...
				 int *priority)
{
	...
	KBASE_TLSTREAM_TL_NEW_ATOM(kbdev, katom,
				   (u32)kbase_jd_atom_id(kctx, katom));
	KBASE_TLSTREAM_TL_RET_ATOM_CTX(kbdev, katom, kctx);
	if (priority)
		KBASE_TLSTREAM_TL_ATTRIB_ATOM_PRIORITY(kbdev, katom,
						       (u32)*priority);
			   ...
			   struct kbase_jd_atom *const katom)
{
	...
	/* Initialize fence dependency count so dma-fence paths can rely on it */
#ifdef CONFIG_MALI_DMA_FENCE
	kbase_fence_dep_count_set(katom, -1);
#endif
	...
	for (i = 0; i < 2; i++) {
		...
		if (dep_atom_number) {
			if (dep_atom_type != BASE_JD_DEP_TYPE_ORDER &&
				...
				katom->event_code =
					BASE_JD_EVENT_JOB_CONFIG_FAULT;
				katom->status = KBASE_JD_ATOM_STATE_COMPLETED;
				dev_dbg(kbdev->dev,
	...
	for (i = 0; i < 2; i++) {
		...
		if (dep_atom->status == KBASE_JD_ATOM_STATE_UNUSED ||
			...
			list_add_tail(&katom->dep_item[i],
				      &dep_atom->dep_head[i]);
			kbase_jd_katom_dep_set(&katom->dep[i], dep_atom,
					       dep_atom_type);
			queued = 1;
		}
	}
	...
	} else if (!katom->jc && (katom->core_req & BASE_JD_REQ_ATOM_TYPE) !=
					 BASE_JD_REQ_DEP) {
	...
	if ((katom->core_req & BASE_JD_REQ_EXTERNAL_RESOURCES) &&
	    (((katom->core_req & BASE_JD_REQ_SOFT_JOB_TYPE) ==
...
int kbase_jd_submit(struct kbase_context *kctx, void __user *user_addr,
		    u32 nr_atoms, u32 stride, bool uk6_atom)
{
	struct kbase_jd_context *jctx = &kctx->jctx;
	int err = 0;
	u32 i;
	...
	if (nr_atoms > BASE_JD_ATOM_COUNT) {
		dev_dbg(kbdev->dev,
			"Invalid attempt to submit %u atoms at once for kctx %d_%d",
	...
	for (i = 0; i < nr_atoms; i++) {
		...
		if (unlikely(jd_atom_is_v2)) {
			...
			if (copy_from_user(&user_atom, user_addr, stride) !=
			    0) {
				dev_dbg(kbdev->dev,
					"Invalid atom address %pK passed to job_submit\n",
					user_addr);
...
#endif
		compiletime_assert(
			(1 << (8 * sizeof(user_atom.atom_number))) ==
				BASE_JD_ATOM_COUNT,
			"BASE_JD_ATOM_COUNT and base_atom_id type out of sync");
		compiletime_assert(
			sizeof(user_atom.pre_dep[0].atom_id) ==
...
#undef compiletime_assert_defined
		...
		if (fatal_signal_pending(current)) {
			dev_dbg(kbdev->dev,
				"Fatal signal pending for kctx %d_%d",
				kctx->tgid, kctx->id);
...
{
	struct kbase_jd_atom *katom =
		container_of(data, struct kbase_jd_atom, work);
	...
	if (!atomic_dec_return(&kctx->work_count)) {
		...
		while (!list_empty(&kctx->completed_jobs)) {
			struct kbase_jd_atom *atom =
				list_entry(kctx->completed_jobs.next,
					   struct kbase_jd_atom, jd_item);
 ...
 */
void kbase_jd_done(struct kbase_jd_atom *katom, unsigned int slot_nr,
...
int kbase_jd_init(struct kbase_context *kctx)
{
	...
	kctx->jctx.job_done_wq =
		alloc_workqueue("mali_jd", WQ_HIGHPRI | WQ_UNBOUND, 1);
	...
	for (i = 0; i < BASE_JD_ATOM_COUNT; i++) {
		...
		/* Initialize per-atom fence state */
	...
	if (pcm_device)
		kctx->jctx.max_priority =
			pcm_device->ops.pcm_scheduler_priority_check(
				pcm_device, current,
				KBASE_JS_ATOM_SCHED_PRIO_REALTIME);
