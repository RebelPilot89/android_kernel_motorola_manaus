...
#endif /* CONFIG_MALI_MTK_DIAGNOSIS_MODE */
void mtk_common_debug(enum mtk_common_debug_types type,
		      struct kbase_context *kctx, u64 hook_point)
{
	...
	do {
		...
		case MTK_COMMON_DBG_DUMP_FULL_DB:
			...
			if (diagnosis_dump_mask &
			    MTK_DBG_COMMON_DUMP_ENABLE_GROUPS_QUEUES) {
				mtk_debug_csf_dump_groups_and_queues(kbdev,
								     kctx);
			}
#endif /* CONFIG_MALI_MTK_FENCE_DEBUG */
#if IS_ENABLED(CONFIG_MALI_MTK_KE_DUMP_FWLOG)
			if (!(diagnosis_dump_mask &
			      MTK_DBG_COMMON_DUMP_SKIP_FWLOG)) {
				mtk_kbase_csf_firmware_ke_dump_fwlog(
					kbdev); /* dump fwlog, reserve 1MB for fwlog */
			}
#endif /* CONFIG_MALI_MTK_KE_DUMP_FWLOG */
#if IS_ENABLED(CONFIG_MALI_MTK_CM7_TRACE)
...
#endif /* CONFIG_MALI_MTK_DIAGNOSIS_MODE */
		default:
			dev_info(kbdev->dev, "@%s: unsupported type (%d)",
				 __func__, type);
			break;
		}
...
int mtk_common_gpufreq_commit(int opp_idx)
{
	...
	if (opp_idx >= 0 && mtk_common_pm_is_mfg_active()) {
		...
		ret = mtk_common_gpufreq_bringup() ?
			      -1 :
			      gpufreq_commit(TARGET_DEFAULT, opp_idx);
#else
		ret = mtk_common_gpufreq_bringup() ?
			      -1 :
			      mt_gpufreq_target(opp_idx, KIR_POLICY);
#endif /* CONFIG_MTK_GPUFREQ_V2 */
	}
	mutex_unlock(&mfg_pm_lock);
...
int mtk_common_gpufreq_dual_commit(int gpu_oppidx, int stack_oppidx)
{
	...
	if (stack_oppidx >= 0 && mtk_common_pm_is_mfg_active()) {
		...
		ret = mtk_common_gpufreq_bringup() ?
			      -1 :
			      mt_gpufreq_target(stack_oppidx, KIR_POLICY);
#endif /* CONFIG_MTK_GPUFREQ_V2 */
	}
	mutex_unlock(&mfg_pm_lock);
...
unsigned long
mtk_common_ged_dvfs_write_sysram_last_commit_idx_test(int commit_idx)
{
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) &&                                    \
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY)
...
unsigned long
mtk_common_ged_dvfs_write_sysram_last_commit_top_idx_test(int commit_idx)
{
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) &&                                    \
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY)
	return mtk_common_ged_dvfs_write_sysram_last_commit_idx_test_compat(
...
unsigned long
mtk_common_ged_dvfs_write_sysram_last_commit_stack_idx_test(int commit_idx)
{
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) &&                                    \
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY)
	return mtk_common_ged_dvfs_write_sysram_last_commit_idx_test_compat(
...
unsigned long
mtk_common_ged_dvfs_write_sysram_last_commit_dual_test(int top_idx,
						       int stack_idx)
{
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) &&                                    \
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY)
...
int mtk_common_ged_dvfs_update_step_size(int low_step, int med_step,
					 int high_step)
{
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) &&                                    \
	IS_ENABLED(CONFIG_MALI_MTK_DVFS_POLICY)
...
static void mtk_common_procfs_init(struct kbase_device *kbdev)
{
	...
	if (!proc_root) {
		dev_info(kbdev->dev, "@%s: Cann't create /proc/%s", __func__,
			 PROC_ROOT);
		return;
	}
...
static int mtk_debug_sleep_mode(struct seq_file *file, void *data)
{
	...
	if (!of_property_read_u32(np, "sleep-mode-enable", &sleep_mode_enable))
		...
		seq_printf(
			file,
			"Sleep mode: No dts property setting, default disabled\n");
...
int mtk_debug_sleep_mode_debugfs_init(struct kbase_device *kbdev)
{
	...
	debugfs_create_file("sleep_mode", 0440, kbdev->mali_debugfs_directory,
			    kbdev, &mtk_sleep_mode_debugfs_fops);
...
int mtk_common_device_init(struct kbase_device *kbdev)
{
	...
	if (mtk_platform_pm_init(kbdev)) {
		dev_info(kbdev->dev, "@%s: Failed to init Platform PM",
			 __func__);
		return -1;
	}
...
/* Ensure MFG counters are ready before devfreq/gpufreq services */
#if IS_ENABLED(CONFIG_MTK_GPU_SWPM_SUPPORT)
	...
	if (!of_property_read_u32(kbdev->dev->of_node, "exception-mask",
				  &kbdev->exception_mask))
		dev_info(kbdev->dev, "@%s: exception_mask=0x%x", __func__,
			 kbdev->exception_mask);
	else
		dev_info(
			kbdev->dev,
			"@%s: no dts property setting, default exception_mask=0x%x",
	...
	if (!of_property_read_u32(kbdev->dev->of_node, "reset-exception-mask",
				  &kbdev->reset_exception_mask))
		dev_info(kbdev->dev, "@%s: reset_exception_mask=0x%x", __func__,
			 kbdev->reset_exception_mask);
	else
		dev_info(
			kbdev->dev,
			"@%s: no dts property setting, default reset_exception_mask=0x%x",
