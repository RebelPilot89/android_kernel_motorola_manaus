...
static u32 kbase_hwcnt_block_state_to_prfcnt_block_state(blk_stt_t hw_blk_stt)
{
	...
	for_each_set_bit (bit_shift, &block_state_mask,
			  BITS_PER_TYPE(blk_stt_t)) {
		...
		case KBASE_HWCNT_STATE_PROTECTED:
			...
			break;
		default:
			WARN(true, "Unknown block state bit set: 0x%x",
			     set_bit);
			break;
		}
	}
...
static bool
kbase_kinstr_is_block_type_reserved(const struct kbase_hwcnt_metadata *metadata,
				    size_t blk)
{
	enum prfcnt_block_type block_type =
 ...
 */
int kbasep_kinstr_prfcnt_set_block_meta_items(
	struct kbase_hwcnt_enable_map *enable_map,
	struct kbase_hwcnt_dump_buffer *dst,
	struct prfcnt_metadata **block_meta_base, u8 *base_addr, u8 counter_set)
{
	size_t blk, blk_inst;
	...
	kbase_hwcnt_metadata_for_each_block(metadata, blk, blk_inst)
	{
		...
		if (kbase_kinstr_is_block_type_reserved(metadata, blk) ||
		    !kbase_hwcnt_metadata_block_instance_avail(metadata, blk,
							       blk_inst) ||
		    !kbase_hwcnt_enable_map_block_enabled(enable_map, blk,
							  blk_inst))
			continue;

		dst_blk = (u8 *)kbase_hwcnt_dump_buffer_block_instance(
			dst, blk, blk_inst);
		hw_blk_stt = *kbase_hwcnt_dump_buffer_block_state_instance(
			dst, blk, blk_inst);
		...
		(*ptr_md)->u.block_md.block_state =
			kbase_hwcnt_block_state_to_prfcnt_block_state(
				hw_blk_stt);
		(*ptr_md)->u.block_md.values_offset =
			(u32)(dst_blk - base_addr);
	...
	struct prfcnt_metadata *ptr_md)
{
	...
	if (WARN_ON(kbasep_kinstr_prfcnt_set_block_meta_items(
		    &cli->enable_map, dump_buf, &ptr_md,
		    cli->sample_arr.user_buf, cli->config.counter_set)))
		return;
...
static void kbasep_kinstr_prfcnt_client_output_sample(
	struct kbase_kinstr_prfcnt_client *cli, int buf_idx, u64 user_data,
				 ...
				 u64 user_data)
{
	...
	ret = kbase_hwcnt_virtualizer_client_dump(cli->hvcli, &ts_start_ns,
						  &ts_end_ns, &cli->tmp_buf);
				  ...
				  u64 user_data)
{
	 ...
	 */
	available_samples_count =
		cli->sample_count - (size_t)(write_idx - read_idx);
	...
	cli->enable_map.clk_enable_map =
		(1ull << cli->kinstr_ctx->metadata->clk_cnt) - 1;
				      ...
				      u64 user_data)
{
	...
	ret = kbasep_kinstr_prfcnt_client_dump(
		cli, BASE_HWCNT_READER_EVENT_MANUAL, user_data);
				...
				struct prfcnt_sample_access *sample_access)
{
	...
	sample_offset_bytes =
		(u64)((u8 *)sample_meta - cli->sample_arr.user_buf);
				...
				struct prfcnt_sample_access *sample_access)
{
	...
	read_idx %= cli->sample_arr.sample_count;
	sample_offset_bytes =
		(u64)((u8 *)cli->sample_arr.samples[read_idx].sample_meta -
...
static long kbasep_kinstr_hwcnt_reader_ioctl_set_interval(
	struct kbase_kinstr_prfcnt_client *cli, u32 interval)
{
	mutex_lock(&cli->kinstr_ctx->lock);
						    ...
						    unsigned long arg)
{
	...
	case _IOC_NR(KBASE_HWCNT_READER_SET_INTERVAL):
		rcode = kbasep_kinstr_hwcnt_reader_ioctl_set_interval(cli,
								      (u32)arg);
...
static void kbasep_kinstr_prfcnt_client_destroy_partial(
	struct kbase_kinstr_prfcnt_client *cli,
	enum kbase_kinstr_prfcnt_client_init_state init_state)
{
	...
	while (init_state-- > KINSTR_PRFCNT_UNINITIALISED) {
		...
		case KINSTR_PRFCNT_SAMPLE_ARRAY:
			kbasep_kinstr_prfcnt_sample_array_free(
				&cli->sample_arr);
			break;
...
{
	kbasep_kinstr_prfcnt_client_destroy_partial(cli,
						    KINSTR_PRFCNT_INITIALISED);
}
...
size_t kbasep_kinstr_prfcnt_get_sample_md_count(
	const struct kbase_hwcnt_metadata *metadata,
	struct kbase_hwcnt_enable_map *enable_map)
{
	size_t blk, blk_inst;
	...
	const struct kbase_hwcnt_metadata *metadata)
{
	...
	size_t md_count = kbasep_kinstr_prfcnt_get_sample_md_count(
		metadata, &cli->enable_map);
	...
	const struct kbase_hwcnt_metadata *metadata)
{
	...
	scratch_buffer = kmalloc(scratch_buffer_bytes * sizeof(*scratch_buffer),
				 GFP_KERNEL);
	if (!scratch_buffer) {
		vfree(sample_arr->user_buf);
		kfree(samples);
	...
	for (sample_idx = 0; sample_idx < buffer_count; sample_idx++) {
		...
		samples[sample_idx].sample_meta =
			(struct prfcnt_metadata *)(sample_arr->user_buf +
						   sample_meta_offset);
 ...
 */
static bool
prfcnt_block_supported(const struct kbase_hwcnt_metadata *metadata,
...
static void
kbasep_kinstr_prfcnt_block_enable_req_to_cfg(uint64_t *cfg_cm,
	...
	struct kbase_kinstr_prfcnt_client_config *config)
{
	...
	case PRFCNT_BLOCK_TYPE_FE:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.fe_bm, req_enable->enable_mask);
		break;
	case PRFCNT_BLOCK_TYPE_TILER:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.tiler_bm, req_enable->enable_mask);
		break;
	case PRFCNT_BLOCK_TYPE_MEMORY:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.mmu_l2_bm, req_enable->enable_mask);
		break;
	case PRFCNT_BLOCK_TYPE_SHADER_CORE:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.shader_bm, req_enable->enable_mask);
		break;
	case PRFCNT_BLOCK_TYPE_FW:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.fw_bm, req_enable->enable_mask);
		break;
	case PRFCNT_BLOCK_TYPE_CSG:
		kbasep_kinstr_prfcnt_block_enable_req_to_cfg(
			config->enable_cm.csg_bm, req_enable->enable_mask);
		break;
	default:
		err = -EINVAL;
		break;
...
int kbasep_kinstr_prfcnt_client_create(
	struct kbase_kinstr_prfcnt_context *kinstr_ctx,
	union kbase_ioctl_kinstr_prfcnt_setup *setup,
	...
	struct prfcnt_request_item *req_arr)
{
	...
	for (init_state = KINSTR_PRFCNT_UNINITIALISED;
	     init_state < KINSTR_PRFCNT_INITIALISED; init_state++) {
		err = 0;
		switch (init_state) {
		case KINSTR_PRFCNT_PARSE_SETUP:
			cli->kinstr_ctx = kinstr_ctx;
			err = kbasep_kinstr_prfcnt_parse_setup(
				kinstr_ctx, setup, &cli->config, req_arr);
		...
		case KINSTR_PRFCNT_DUMP_BUFFER:
			kbase_hwcnt_gpu_enable_map_from_cm(
				&cli->enable_map, &cli->config.enable_cm);
			if (mtk_pm_tool == pm_ltr) {
				cli->config.enable_cm.fe_bm[0] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.fe_bm[1] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.shader_bm[0] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.shader_bm[1] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.tiler_bm[0] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.tiler_bm[1] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.mmu_l2_bm[0] =
					0xFFFFFFFFFFFFFFFF;
				cli->config.enable_cm.mmu_l2_bm[1] =
					0xFFFFFFFFFFFFFFFF;
			}
			cli->sample_count = cli->config.buffer_count;
			cli->sample_size = kbasep_kinstr_prfcnt_get_sample_size(
				cli, kinstr_ctx->metadata);

			/* Use virtualizer's metadata to alloc tmp buffer which interacts with
			 * the HWC virtualizer.
			 */
			err = kbase_hwcnt_dump_buffer_alloc(
		...
		case KINSTR_PRFCNT_SAMPLE_ARRAY:
			 ...
			 */
			err = kbasep_kinstr_prfcnt_sample_array_alloc(
				cli, kinstr_ctx->metadata);
		...
		case KINSTR_PRFCNT_VIRTUALIZER_CLIENT:
			...
			err = kbase_hwcnt_virtualizer_client_create(
				kinstr_ctx->hvirt, &cli->enable_map,
				&cli->hvcli);
			break;
...
static size_t kbasep_kinstr_prfcnt_get_enum_info_count(
	const struct kbase_hwcnt_metadata *metadata)
{
...
static void
kbasep_kinstr_prfcnt_get_request_info_list(struct prfcnt_enum_item *item_arr,
					   size_t *arr_idx)
{
...
static void kbasep_kinstr_prfcnt_get_sample_info_item(
	const struct kbase_hwcnt_metadata *metadata,
	...
	struct prfcnt_enum_item *item_arr, size_t *arr_idx)
{
	...
	for (blk = 0; blk < kbase_hwcnt_metadata_block_count(metadata); blk++) {
		...
		size_t blk_inst_count =
			kbase_hwcnt_metadata_block_instance_count(metadata,
								  blk);
		enum prfcnt_block_type block_type =
			kbase_hwcnt_metadata_block_type_to_prfcnt_block_type(
				kbase_hwcnt_metadata_block_type(metadata, blk));
	...
	struct kbase_ioctl_kinstr_prfcnt_enum_info *enum_info)
{
	...
	enum_info->info_item_count =
		kbasep_kinstr_prfcnt_get_enum_info_count(metadata);
	enum_info->info_item_size = sizeof(struct prfcnt_enum_item);

	return 0;
}

int kbasep_kinstr_populate_prfcnt_enum_list(
	const struct kbase_hwcnt_metadata *metadata,
	struct prfcnt_enum_item *prfcnt_item_arr, size_t prfcnt_item_array_size)
{
	size_t arr_idx = 0;
	int err = 0;
	...
	kbasep_kinstr_prfcnt_get_sample_info_item(metadata, prfcnt_item_arr,
						  &arr_idx);
	...
	prfcnt_item_arr[prfcnt_item_array_size - 1].hdr.item_type =
		FLEX_LIST_TYPE_NONE;
	...
	struct kbase_ioctl_kinstr_prfcnt_enum_info *enum_info)
{
	...
	err = kbasep_kinstr_populate_prfcnt_enum_list(
		metadata, prfcnt_item_arr, enum_info->info_item_count);
	...
	union kbase_ioctl_kinstr_prfcnt_setup *setup)
{
	...
	struct prfcnt_request_item req_arr_[] = {
		{
			...
			.u.req_mode.mode_config.periodic.period_ns =
				KINSTR_PRFCNT_PERIOD_NS,
		},
		{
	...
	if (err) {
		...
		// cli->active = true;
		/* Start Client */
		ret = kbasep_kinstr_prfcnt_client_start(
			cli, (__u64)user_data_sample);
	}
...
void MTK_kbasep_vinstr_hwcnt_set_interval(unsigned int interval)
{
	mutex_lock(&gpu_vinstr_mtk_lock);
	if (mtk_cli != NULL) {
		kbasep_kinstr_hwcnt_reader_ioctl_set_interval(mtk_cli,
							      interval);
	}
	mutex_unlock(&gpu_vinstr_mtk_lock);
	return;
...
void MTK_update_gpu_LTR(void)
{
	...
	gpu_perf_counter.counter[VINSTR_GPU_FREQ] =
		gpufreq_get_cur_freq(TARGET_DEFAULT);
	gpu_perf_counter.counter[VINSTR_GPU_VOLT] =
	...
	for (i = VINSTR_GPU_ACTIVE; i <= VINSTR_LS_MEM_ATOMIC; i++) {
		...
		if (i >= VINSTR_STALL0 && i <= VINSTR_STALL3) {
			gpu_perf_counter.counter[i] =
				stall_counter[stall_index];
			stall_index++;
			continue;
		} else if (i >= VINSTR_L2_EXT_WRITE_BEATS &&
			   i <= VINSTR_L2_ANY_LOOKUP) {
			for (j = 0; j < L2_CNT; j++) {
				gpu_perf_counter.counter[i] +=
					kernel_dump[pmu_index];
	...
	IS_ENABLED(CONFIG_MTK_GPU_SWPM_SUPPORT)
	...
	trace_perf_index_gpu(gpu_perf_counter.counter,
