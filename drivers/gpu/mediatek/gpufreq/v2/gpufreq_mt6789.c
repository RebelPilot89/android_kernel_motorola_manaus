...
static void __gpufreq_apply_adjust(struct gpufreq_adj_info *adj_table,
				   int adj_num);
/* dvfs function */
...
done_unlock:
	GPUFREQ_LOGD("PWR_STATUS (0x%x): 0x%08x", (0x10006000 + 0x16C),
		     readl(g_sleep + 0x16C));
...
int __gpufreq_generic_commit_gpu(int target_oppidx, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_LOGD("begin to commit GPU OPP index: (%d->%d)", cur_oppidx,
		     target_oppidx);
	...
	if (target_freq == cur_freq) {
		/* voltage scaling */
		ret = __gpufreq_volt_scale_gpu(cur_volt, target_volt, cur_vsram,
					       target_vsram);
		if (unlikely(ret)) {
			GPUFREQ_LOGE(
		...
		while (target_volt != cur_volt) {
			sb_idx = target_oppidx > sb_table[cur_oppidx].up ?
					 target_oppidx :
					 sb_table[cur_oppidx].up;

			ret = __gpufreq_volt_scale_gpu(cur_volt,
						       opp_table[sb_idx].volt,
						       cur_vsram,
						       opp_table[sb_idx].vsram);
			if (unlikely(ret)) {
				GPUFREQ_LOGE(
					"fail to scale Vgpu: (%d->%d), Vsram_gpu: (%d->%d)",
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE("fail to scale Fgpu: (%d->%d)", cur_freq,
				     target_freq);
			goto done_unlock;
		}
	} else {
...
int __gpufreq_fix_custom_freq_volt_gpu(unsigned int freq, unsigned int volt)
{
	...
	if (freq == 0 && volt == 0) {
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE(
				"fail to custom commit GPU freq: %d, volt: %d (%d)",
				freq, volt, ret);
			mutex_lock(&gpufreq_lock);
			g_dvfs_state &= ~DVFS_DEBUG_KEEP;
			mutex_unlock(&gpufreq_lock);
...
void __gpufreq_dump_infra_status(void)
{
	...
	GPUFREQ_LOGI("GPU[%d] Freq: %d, Vgpu: %d, Vsram: %d", g_gpu.cur_oppidx,
		     g_gpu.cur_freq, g_gpu.cur_volt, g_gpu.cur_vsram);

	/*0x1020E000 */
	if (g_infracfg_base) {
		/* g_infracfg_base */
		GPUFREQ_LOGI("%-7s (0x%x): 0x%08x, (0x%x): 0x%08x", "[EMI]",
			     (0x1020E000 + 0x810),
	...
	if (g_infra_peri_debug4 && g_fmem_ao_debug_ctrl) {
		...
		GPUFREQ_LOGI("%-7s (0x%x): 0x%08x, (0x%x): 0x%08x", "[INFRA]",
			     ...
			     (0x10042000 + 0x000),
			     readl(g_fmem_ao_debug_ctrl + 0x000));
	}
				       ...
				       enum gpufreq_dvfs_state key)
{
	...
	if (target_freq == cur_freq) {
		...
		while (target_volt != cur_volt) {
			...
			ret = __gpufreq_volt_scale_gpu(cur_volt, sb_volt,
						       cur_vsram, sb_vsram);
			if (unlikely(ret)) {
				GPUFREQ_LOGE(
					"fail to scale Vgpu: (%d->%d), Vsram_gpu: (%d->%d)",
	...
	if (clksrc == CLOCK_MAIN) {
		...
		if (unlikely(ret)) {
			__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
					"fail to switch GPU to CLOCK_MAIN (%d)",
					ret);
			goto done;
		}
		...
		if (unlikely(ret)) {
			__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
					"fail to switch GPU to CLOCK_SUB (%d)",
					ret);
			goto done;
		}
	} else {
				    ...
				    unsigned int freq_new)
{
	...
	if (parking) {
		...
		if (freq_new > freq_old) {
			...
			pll = (readl(MFGPLL_CON1) & 0xF8FFFFFF) |
				    ...
				    unsigned int vsram_new)
{
	...
	GPUFREQ_TRACE_START(
		"vgpu_old=%d, vgpu_new=%d, vsram_old=%d, vsram_new=%d",
	...
	if (vgpu_new > vgpu_old) {
		/* scale-up volt */
		t_settle_vgpu =
			__gpufreq_settle_time_vgpu(true, (vgpu_new - vgpu_old));
		t_settle_vsram = __gpufreq_settle_time_vsram(
		...
		ret = regulator_set_voltage(g_pmic->reg_vgpu, vgpu_new * 10,
					    VGPU_MAX_VOLT * 10 + 125);
		if (unlikely(ret)) {
			__gpufreq_abort(GPUFREQ_PMIC_EXCEPTION,
					"fail to set VGPU (%d)", ret);
			goto done;
		}
#if GPUFREQ_HISTORY_ENABLE
		/* update history record to shared memory */
		...
		/* scale-down volt */
		t_settle_vgpu = __gpufreq_settle_time_vgpu(
	...
	t_settle = (t_settle_vgpu > t_settle_vsram) ? t_settle_vgpu :
						      t_settle_vsram;
	udelay(t_settle);
...
static void __gpufreq_dump_bringup_status(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGI("[TOP] FMETER: %d, CON1: %d", __gpufreq_get_fmeter_fgpu(),
...
static unsigned int __gpufreq_get_real_fgpu(void)
{
	...
	GPUFREQ_LOGD("%s: MFGPLL_CON1 = 0x%x", __func__,
		     (g_apmixed_base + 0x026C));
...
static void __gpufreq_mfg1_control(enum gpufreq_power_state power)
{
	...
	if (power == POWER_OFF) {
		...
		while ((readl(INFRA_TOPAXI_PROTECTSTA1_1) &
		...
		while ((readl(INFRA_TOPAXI_PROTECTSTA1_2) &
		...
		while ((readl(INFRA_TOPAXI_PROTECTSTA1) &
...
static int __gpufreq_buck_control(enum gpufreq_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		if (unlikely(ret)) {
			__gpufreq_abort(GPUFREQ_PMIC_EXCEPTION,
					"fail to enable VSRAM_GPU (%d)", ret);
			goto done;
		}
		g_gpu.buck_count++;
		/* power off */
		...
		if (unlikely(ret)) {
			__gpufreq_abort(GPUFREQ_PMIC_EXCEPTION,
					"fail to disable VSRAM_GPU (%d)", ret);
			goto done;
		}
...
static void __gpufreq_set_springboard(void)
{
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGD("springboard_up[%02d]: %d", src_idx,
			     g_gpu.sb_table[src_idx].up);
	}
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGD("springboard_down[%02d]: %d", src_idx,
			     g_gpu.sb_table[src_idx].down);
	}
}
...
static int __gpufreq_init_opp_idx(void)
{
	...
	if (__gpufreq_custom_init_enable()) {
		...
		GPUFREQ_LOGI("custom init GPU OPP index: %d, Freq: %d", oppidx,
			     working_table[oppidx].freq);
	...
	if (!__gpufreq_dvfs_enable()) {
		...
		if (__gpufreq_custom_init_enable()) {
			ret = __gpufreq_generic_commit_gpu(oppidx,
							   DVFS_DISABLE);
			if (unlikely(ret)) {
				GPUFREQ_LOGE(
...
static void __gpufreq_resume_dvfs(void)
{
	...
	if (unlikely(ret < 0))
		GPUFREQ_LOGE("fail to control power state: %d (%d)", POWER_OFF,
			     ret);
...
static void __gpufreq_apply_aging(unsigned int apply_aging)
{
	...
	for (i = 0; i < g_gpu.opp_num; i++) {
		...
		g_gpu.working_table[i].vsram = __gpufreq_get_vsram_by_vgpu(
			g_gpu.working_table[i].volt);

		GPUFREQ_LOGD("apply Vaging: %d, GPU[%02d] vgpu: %d, Vsram: %d",
			     apply_aging, i, g_gpu.working_table[i].volt,
			     g_gpu.working_table[i].vsram);
	}

	for (i = 0; i < signed_opp_num; i++) {
		if (apply_aging)
			g_gpu.signed_table[i].volt -=
				g_gpu.signed_table[i].vaging;
		else
			g_gpu.signed_table[i].volt +=
				g_gpu.signed_table[i].vaging;
...
static void __gpufreq_avs_adjustment(void)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		temp_freq = 0;
		temp_freq |= (val & 0x00070000) >>
			     8; // Get freq[10:8] from efuse[18:16]
		temp_freq |= (val & 0x0000FF00) >>
			     8; // Get freq[7:0] from efuse[15:8]
		...
		temp_volt = 0;
		temp_volt |=
			(val & 0x000000FF); // Get volt[7:0] from efuse[7:0]
	...
	for (i = adj_num - 1; i >= 0; i--) {
		...
		if (i != (adj_num - 1)) {
			if (g_avs_adj[i].volt < g_avs_adj[i + 1].volt) {
				GPUFREQ_LOGW(
					"efuse[%d].volt(%d) < efuse[%d].volt(%d)",
					i, g_avs_adj[i].volt, i + 1,
					g_avs_adj[i + 1].volt);
				g_avs_adj[i].volt =
					g_avs_adj[i + 1].volt + volt_offset;
			}
		}
		...
		if (g_avs_adj[i].volt > g_gpu.signed_table[oppidx].volt) {
			GPUFREQ_LOGW(
				"OPP[%d*]: efuse[%d].volt(%d) > signed-off.volt(%d)",
				oppidx, i, g_avs_adj[i].volt,
				g_gpu.signed_table[oppidx].volt);
			g_avs_adj[i].volt = g_gpu.signed_table[oppidx].volt;
		}
		...
		/* update Vsram */
		g_avs_adj[i].vsram =
			__gpufreq_get_vsram_by_vgpu(g_avs_adj[i].volt);
	}

	for (i = 0; i < adj_num; i++)
		GPUFREQ_LOGI("OPP[%d*]: efuse[%d]: freq(%d), volt(%d)",
			     g_avs_adj[i].oppidx, i, g_avs_adj[i].freq,
			     g_avs_adj[i].volt);
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	g_gpu.sb_table = kcalloc(g_gpu.opp_num, sizeof(struct gpufreq_sb_info),
				 GFP_KERNEL);
	if (!g_gpu.sb_table) {
...
static int __gpufreq_init_segment_id(struct platform_device *pdev)
{
	...
	if (IS_ERR(efuse_cell)) {
		GPUFREQ_LOGE("fail to get efuse_segment_cell (%ld)",
			     PTR_ERR(efuse_cell));
...
static int __gpufreq_init_mtcmos(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg0_dev)) {
		ret = g_mtcmos->mfg0_dev ? PTR_ERR(g_mtcmos->mfg0_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg0_dev (%ld)", ret);
		goto done;
	}
...
static int __gpufreq_init_platform_info(struct platform_device *pdev)
{
	...
	g_fmem_ao_debug_ctrl =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_fmem_ao_debug_ctrl)) {
		GPUFREQ_LOGE("fail to ioremap fmem_ao_debug_ctrl: 0x%llx",
			     res->start);
		goto done;
	}
