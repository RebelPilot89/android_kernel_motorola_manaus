...
void __gpufreq_dump_infra_status(void)
{
	...
	if (g_mfg_rpc_base && g_rgx_base) {
		...
		GPUFREQ_LOGI("%-7s (0x%x): 0x%08x, (0x%x): 0x%08x", "[MFG]",
			     (0x13F90000 + 0x1048),
	...
	if (g_nth_emicfg_base && g_nth_emicfg_ao_mem_base) {
		...
		GPUFREQ_LOGI(
			...
			readl(g_nth_emicfg_base + 0x830), (0x10270000 + 0x228),
	...
	if (g_nth_emi_mpu_base && g_fmem_ao_debug_ctrl) {
		...
		GPUFREQ_LOGI(
			...
			"[EMI]", (0x10351000 + 0x1D8),
			readl(g_nth_emi_mpu_base + 0x1D8), (0x10351000 + 0x3D8),
			readl(g_nth_emi_mpu_base + 0x3D8), (0x10351000 + 0x5D8),
			readl(g_nth_emi_mpu_base + 0x5D8), (0x10042000 + 0x000),
	...
	if (g_sleep) {
		...
		GPUFREQ_LOGI(
			"%-7s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
			"[SPM]", (0x1C001000 + 0xEB8), readl(g_sleep + 0xEB8),
...
static void __gpufreq_dump_bringup_status(struct platform_device *pdev)
{
	 ...
	 */
	GPUFREQ_LOGI(
		"[GPU] RGX_CR_CORE_ID: (0x%08x, 0x%08x), RGX_CR_SYS_BUS_SECURE: 0x%08x",
	...
	GPUFREQ_LOGI(
		...
		__gpufreq_execute_fmeter(CLOCK_MAIN),
		__gpufreq_execute_fmeter(CLOCK_SUB));
	GPUFREQ_LOGI(
		"[MUX] CKMUX_SEL_REF_CORE: 0x%08x, CKMUX_SEL_REF_PARK: 0x%08x",
...
static void __gpufreq_aoc_control(enum gpufreq_power_state power)
{
	...
	if (readl(g_sleep + 0x6AC) & 0x1)
		__gpufreq_abort(GPUFREQ_GPU_EXCEPTION,
				"fail to release SPM_SEMA_M4: 0x%08x",
				readl(g_sleep + 0x6AC));
...
static int __gpufreq_mtcmos_control(enum gpufreq_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		if (unlikely(val != MFG_0_2_PWR_MASK)) {
			__gpufreq_abort(
				GPUFREQ_GPU_EXCEPTION,
				"incorrect MFG0-2 power on status: 0x%08x",
				val);
			ret = GPUFREQ_EINVAL;
			goto done;
		}
		...
		if (unlikely(val))
			...
			GPUFREQ_LOGE("incorrect MFG power off status: 0x%08x",
				     val);
#endif /* GPUFREQ_CHECK_MTCMOS_PWR_STATUS */
...
static int __gpufreq_init_opp_idx(void)
{
	...
	GPUFREQ_LOGI("init GPU idx: %d, Freq: %d, Volt: %d", target_oppidx_gpu,
...
static void __gpufreq_apply_aging(unsigned int apply_aging)
{
	...
	for (i = 0; i < signed_opp_num; i++) {
		...
		signed_table[i].vsram =
			__gpufreq_get_vsram_by_vgpu(signed_table[i].volt);
	}
				   ...
				   int adj_num)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		GPUFREQ_LOGD(
			"[%02d*] Freq: %d, Volt: %d, Vsram: %d, Vaging: %d",
						 ...
						 u32 *efuse_error)
{
	...
	*efuse_error = a_t0_err1 | a_t0_err2 | a_t0_err3 | a_t0_err4;
	...
	GPUFREQ_LOGD("a_t0_efuse1: 0x%08x, a_t0_efuse2: 0x%08x", *a_t0_efuse1,
		     *a_t0_efuse2);
	GPUFREQ_LOGD(
		"a_t0_efuse3: 0x%08x, a_t0_efuse4: 0x%08x, efuse_error: 0x%08x",
					    ...
					    u32 *a_tn_sensor2)
{
	...
	GPUFREQ_LOGD("aging_data1: 0x%08x, aging_data2: 0x%08x", aging_data1,
		     aging_data2);
	...
	*a_tn_sensor2 = (aging_data2 & 0xFFFF);
	...
	GPUFREQ_LOGD("a_tn_sensor1: 0x%08x, a_tn_sensor2: 0x%08x",
		     *a_tn_sensor1, *a_tn_sensor2);
}
#endif /* GPUFREQ_ASENSOR_ENABLE */
	...
	u32 efuse_error, unsigned int is_efuse_read_success)
{
	...
	GPUFREQ_LOGD("tj_max: %d tj: %d, a_diff1: %d, a_diff2: %d", tj_max, tj,
		     a_diff1, a_diff2);
...
static void __gpufreq_avs_adjustment(void)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		temp_freq = 0;
		temp_freq |= (val & 0x00100000) >>
			     10; /* Get freq[10] from efuse[20]     */
		...
		temp_volt |= (val &
			      0x00000030); /* Get volt[5:4] from efuse[5:4]   */
	...
	for (i = adj_num - 1; i >= 0; i--) {
		...
		if (i != (adj_num - 1)) {
			if (g_avs_adj[i].volt < g_avs_adj[i + 1].volt) {
				GPUFREQ_LOGW(
					"AVS efuse[%d].volt(%d) < efuse[%d].volt(%d)",
...
static int __gpufreq_init_platform_info(struct platform_device *pdev)
{
	...
	if (unlikely(!g_nth_emi_mpu_base)) {
		GPUFREQ_LOGE("fail to ioremap NTH_EMI_MPU_REG: 0x%llx",
			     res->start);
		goto done;
	}
