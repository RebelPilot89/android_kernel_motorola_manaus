...
#include <mtk_low_battery_throttling.h>
	...
	__has_include(<leakage_table_v2/mtk_static_power.h>)
...
/* dvfs function */
static int
__gpufreq_generic_scale_gpu(unsigned int freq_old, unsigned int freq_new,
			    unsigned int volt_old, unsigned int volt_new,
			    unsigned int vsram_old, unsigned int vsram_new);
static int
__gpufreq_generic_scale_stack(unsigned int freq_old, unsigned int freq_new,
			      unsigned int volt_old, unsigned int volt_new,
			      unsigned int vsram_old, unsigned int vsram_new);
...
static int __gpufreq_custom_commit_stack(unsigned int target_freq,
					 unsigned int target_volt,
					 enum gpufreq_dvfs_state key);
static int __gpufreq_freq_scale_gpu(unsigned int freq_old,
				    unsigned int freq_new);
static int __gpufreq_freq_scale_stack(unsigned int freq_old,
				      unsigned int freq_new);
static int __gpufreq_volt_scale_gpu(unsigned int vgpu_old,
				    unsigned int vgpu_new,
				    unsigned int vsram_old,
				    unsigned int vsram_new);
static void __gpufreq_switch_clksrc(enum gpufreq_target target,
				    enum gpufreq_clk_src clksrc);
static void __gpufreq_switch_parksrc(enum gpufreq_target target,
				     enum gpufreq_clk_src clksrc);
static unsigned int __gpufreq_calculate_pcw(unsigned int freq,
					    enum gpufreq_posdiv posdiv_power);
static unsigned int __gpufreq_settle_time_vgpu(unsigned int direction,
					       int deltaV);
/* get function */
...
static void __gpufreq_asensor_read_register(u32 *a_tn_lvt_cnt,
					    u32 *a_tn_ulvt_cnt);
static unsigned int __gpufreq_asensor_read_efuse(u32 *a_t0_lvt_rt,
						 u32 *a_t0_ulvt_rt,
						 u32 *a_shift_error,
						 u32 *efuse_error);
static unsigned int
__gpufreq_get_aging_table_idx(u32 a_t0_lvt_rt, u32 a_t0_ulvt_rt,
...
int __gpufreq_power_control(enum gpufreq_power_state power)
{
	...
	GPUFREQ_LOGD(
		"switch power: %s (Power: %d, Buck: %d, MTCMOS: %d, CG: %d)",
		power ? "On" : "Off", g_gpu.power_count, g_gpu.buck_count,
...
done_unlock:
	GPUFREQ_LOGD("PWR_STATUS (0x%x): 0x%08x", (0x1C001000 + PWR_STATUS_OFS),
...
int __gpufreq_generic_commit_gpu(int target_oppidx, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_LOGD("begin to commit GPU OPP index: (%d->%d)", cur_oppidx_gpu,
		     target_oppidx_gpu);

	ret = __gpufreq_generic_scale_gpu(cur_fgpu, target_fgpu, cur_vgpu,
					  target_vgpu, cur_vsram_gpu,
					  target_vsram_gpu);
	if (unlikely(ret)) {
		GPUFREQ_LOGE(
			"fail to scale GPU: Idx(%d->%d), Freq(%d->%d), Volt(%d->%d), Vsram(%d->%d)",
			cur_oppidx_gpu, target_oppidx_gpu, cur_fgpu,
			target_fgpu, cur_vgpu, target_vgpu, cur_vsram_gpu,
			target_vsram_gpu);
		goto done_unlock;
	}
 ...
 * this is the main entrance of generic DVFS
 */
int __gpufreq_generic_commit_stack(int target_oppidx,
				   enum gpufreq_dvfs_state key)
{
...
int __gpufreq_fix_target_oppidx_gpu(int oppidx)
{
	...
	if (unlikely(ret < 0)) {
		GPUFREQ_LOGE("fail to control power state: %d (%d)", POWER_ON,
			     ret);
		goto done;
	}
...
int __gpufreq_fix_custom_freq_volt_gpu(unsigned int freq, unsigned int volt)
{
	...
	if (freq == 0 && volt == 0) {
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE(
				"fail to commit GPU Freq: %d, Volt: %d (%d)",
				freq, volt, ret);
			__gpufreq_set_dvfs_state(false, DVFS_DEBUG_KEEP);
		}
	}
...
void __gpufreq_dump_infra_status(void)
{
	...
	if (g_gpueb_support) {
		...
		GPUFREQ_LOGI("GPU[%d] Freq: %d, Vgpu: %d, Vsram: %d",
			     g_gpu.cur_oppidx, g_gpu.cur_freq, g_gpu.cur_volt,
			     g_gpu.cur_vsram);
	}
	...
	if (g_mfg_top_base && g_mfg_rpc_base) {
		...
		GPUFREQ_LOGI(
			"%-7s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
			"[MFG]", (0x13FBF000 + 0x170),
			readl(g_mfg_top_base + 0x170), (0x13FBF000 + 0x178),
			readl(g_mfg_top_base + 0x178), (0x13F90000 + 0x1048),
			readl(g_mfg_rpc_base + 0x1048));
	}
	...
	if (g_nth_emicfg_base && g_sth_emicfg_base) {
		...
		GPUFREQ_LOGI(
			"%-7s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
			"[EMI]", (0x1021C000 + 0x82C),
			readl(g_nth_emicfg_base + 0x82C), (0x1021C000 + 0x830),
			readl(g_nth_emicfg_base + 0x830), (0x1021E000 + 0x82C),
			readl(g_sth_emicfg_base + 0x82C), (0x1021E000 + 0x830),
			readl(g_sth_emicfg_base + 0x830));
	}
	...
	if (g_nth_emicfg_ao_mem_base && g_sth_emicfg_ao_mem_base) {
		...
		GPUFREQ_LOGI(
			"%-7s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
			"[EMI]", (0x10270000 + 0x228),
			readl(g_nth_emicfg_ao_mem_base + 0x228),
			(0x10270000 + 0x22C),
			readl(g_nth_emicfg_ao_mem_base + 0x22C),
			(0x1030E000 + 0x228),
			readl(g_sth_emicfg_ao_mem_base + 0x228),
			(0x1030E000 + 0x22C),
			readl(g_sth_emicfg_ao_mem_base + 0x22C));
	}
	...
	if (g_infracfg_ao_base && g_infra_ao_debug_ctrl) {
		...
		GPUFREQ_LOGI(
			"%-7s (0x%x): 0x%08x, (0x%x): 0x%08x, (0x%x): 0x%08x",
			"[INFRA]", (0x10001000 + 0xCA0),
			readl(g_infracfg_ao_base + 0xCA0), (0x10001000 + 0xCAC),
			readl(g_infracfg_ao_base + 0xCAC), (0x10023000 + 0x000),
			readl(g_infra_ao_debug_ctrl + 0x000));
	}
	...
	if (g_infra_ao1_debug_ctrl && g_nth_emi_ao_debug_ctrl) {
		...
		GPUFREQ_LOGI("%-7s (0x%x): 0x%08x, (0x%x): 0x%08x", "[INFRA]",
			     (0x1002B000 + 0x000),
			     readl(g_infra_ao1_debug_ctrl + 0x000),
			     (0x10042000 + 0x000),
			     readl(g_nth_emi_ao_debug_ctrl + 0x000));
	}

	/* 0x1C001000 */
	if (g_sleep) {
		/* MFG0_PWR_CON - MFG2_PWR_CON */
		GPUFREQ_LOGI("%-7s (0x%x-%x): 0x%08x 0x%08x 0x%08x", "[SPM]",
		...
		/* XPU_PWR_STATUS */
		/* XPU_PWR_STATUS_2ND */
		GPUFREQ_LOGI("%-7s (0x%x): 0x%08x, (0x%x): 0x%08x", "[SPM]",
			     (0x1C001000 + PWR_STATUS_OFS),
			     readl(g_sleep + PWR_STATUS_OFS),
			    ...
			    unsigned int vsram_old, unsigned int vsram_new)
{
	...
	if (freq_new > freq_old) {
		/* volt scaling */
		ret = __gpufreq_volt_scale_gpu(volt_old, volt_new, vsram_old,
					       vsram_new);
		if (unlikely(ret)) {
			GPUFREQ_LOGE(
				"fail to scale Vgpu: (%d->%d), Vsram_gpu: (%d->%d)",
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE("fail to scale Fgpu: (%d->%d)", freq_old,
				     freq_new);
			goto done;
		}
		/* STACK freq scaling */
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE("fail to scale Fstack: (%d->%d)", freq_old,
				     freq_new);
			goto done;
		}
				    ...
				    unsigned int freq_new)
{
	...
	if (target_posdiv == cur_posdiv) {
		...
		if (unlikely(!ret)) {
			__gpufreq_abort(GPUFREQ_FHCTL_EXCEPTION,
					"fail to hopping PCW: 0x%x (%d)", pcw,
					ret);
			ret = GPUFREQ_EINVAL;
			goto done;
		}
		...
		pll = (readl(MFG_PLL_CON1) & 0xF8FFFFFF) |
	...
	GPUFREQ_LOGD("Fgpu: %d, PCW: 0x%x, CON1: 0x%08x", g_gpu.cur_freq, pcw,
		     pll);
				      ...
				      unsigned int freq_new)
{
	...
	if (target_posdiv == cur_posdiv) {
		...
		/* 2. compute CON1 with target POSDIV */
		pll = (readl(MFGSC_PLL_CON1) & 0xF8FFFFFF) |
		      (target_posdiv << POSDIV_SHIFT);
		/* 3. change POSDIV by writing CON1 */
	...
	GPUFREQ_LOGD("Fstack: %d, PCW: 0x%x, CON1: 0x%08x", g_stack.cur_freq,
		     pcw, pll);
...
static int __gpufreq_volt_scale_gpu(unsigned int volt_old,
				    ...
				    unsigned int vsram_new)
{
	...
	GPUFREQ_TRACE_START(
		"volt_old=%d, volt_new=%d, vsram_old=%d, vsram_new=%d",
		volt_old, volt_new, vsram_old, vsram_new);
	...
	if (volt_new > volt_old) {
		if (vsram_new != vsram_old) {
			...
			t_settle_volt = __gpufreq_settle_time_vgpu(
				true, (volt_park - volt_old));
	...
	GPUFREQ_LOGD("Vgpu: %d, Vsram: %d, udelay: %d", g_gpu.cur_volt,
		     g_gpu.cur_vsram, t_settle_volt);
...
static void __gpufreq_dump_bringup_status(struct platform_device *pdev)
{
	...
	g_mfg_pll_base =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	...
	if (unlikely(!g_mfg_top_base)) {
		GPUFREQ_LOGE("fail to ioremap MFG_TOP_CONFIG: 0x%llx",
			     res->start);
		goto done;
	}
...
static void __gpufreq_aoc_control(enum gpufreq_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		GPUFREQ_LOGD(
			"AOC_VGPU_SRAM_ISO_DIN = 1'b0, AOC_VGPU_SRAM_LATCH_ENB = 1'b0");
		...
		GPUFREQ_LOGD(
			"AOC_VGPU_SRAM_ISO_DIN = 1'b1, AOC_VGPU_SRAM_LATCH_ENB = 1'b1");
	}
...
static int __gpufreq_mtcmos_control(enum gpufreq_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		if (g_sleep) {
			val = readl(g_sleep + PWR_STATUS_OFS) &
			      MFG_0_1_PWR_MASK;
			if (unlikely(val != MFG_0_1_PWR_MASK)) {
				__gpufreq_abort(
					GPUFREQ_GPU_EXCEPTION,
					"incorrect MFG0-1 power on status: 0x%08x",
					val);
				ret = GPUFREQ_EINVAL;
				goto done;
			}
		...
		if (g_sleep) {
			if (g_shader_present == GPU_SHADER_PRESENT_3) {
				...
				if (unlikely(val != MFG_0_5_PWR_MASK)) {
					__gpufreq_abort(
						GPUFREQ_GPU_EXCEPTION,
						"incorrect MFG0-5 power on status: 0x%08x",
						val);
					ret = GPUFREQ_EINVAL;
					goto done;
				}
				...
				if (unlikely(val != MFG_0_4_PWR_MASK)) {
					__gpufreq_abort(
						GPUFREQ_GPU_EXCEPTION,
						"incorrect MFG0-4 power on status: 0x%08x",
						val);
					ret = GPUFREQ_EINVAL;
					goto done;
				}
				...
				if (unlikely(val != MFG_0_3_PWR_MASK)) {
					__gpufreq_abort(
						GPUFREQ_GPU_EXCEPTION,
						"incorrect MFG0-3 power on status: 0x%08x",
						val);
					ret = GPUFREQ_EINVAL;
					goto done;
				}
		...
		if (g_sleep && !g_gpu.mtcmos_count) {
			...
			if (unlikely(val))
				...
				GPUFREQ_LOGE(
					"incorrect MFG power off status: 0x%08x",
					val);
		}
#endif /* GPUFREQ_CHECK_MTCMOS_PWR_STATUS */
	}
...
static int __gpufreq_init_opp_idx(void)
{
	...
	if (__gpufreq_custom_init_enable()) {
		...
		GPUFREQ_LOGI("custom init GPU OPP index: %d, Freq: %d",
			     target_oppidx_gpu,
			     working_table[target_oppidx_gpu].freq);
		/* decide first OPP idx by SRAMRC setting */
	} else {
		target_oppidx_gpu =
			__gpufreq_get_idx_by_vgpu(GPUFREQ_SAFE_VLOGIC);
	...
	if (!__gpufreq_dvfs_enable()) {
		...
		if (__gpufreq_custom_init_enable()) {
			ret = __gpufreq_generic_commit_gpu(target_oppidx_gpu,
							   DVFS_DISABLE);
			if (unlikely(ret)) {
				GPUFREQ_LOGE(
					"fail to commit GPU OPP index: %d (%d)",
...
static int __gpufreq_pause_dvfs(void)
{
	...
	if (unlikely(ret)) {
		GPUFREQ_LOGE(
			"fail to scale GPU: Freq(%d->%d), Volt(%d->%d), Vsram(%d->%d)",
			cur_fgpu, target_fgpu, cur_vgpu, target_vgpu,
			cur_vsram_gpu, target_vsram_gpu);
		goto done_unlock;
	}

	GPUFREQ_LOGI("pause DVFS at GPU(%d, %d), state: 0x%x", target_fgpu,
		     target_vgpu, g_dvfs_state);
...
static void __gpufreq_interpolate_volt(void)
{
	...
	for (i = 1; i < avs_num; i++) {
		...
		if (unlikely(slope < 0))
			__gpufreq_abort(
				GPUFREQ_GPU_EXCEPTION,
				"invalid slope when interpolate OPP Volt: %d",
				slope);
		...
		for (j = 1; j < range; j++) {
			...
			if (inner_volt < previous_volt)
				__gpufreq_abort(
					...
					inner_idx, inner_volt, inner_idx + 1,
					previous_volt);
			...
			GPUFREQ_LOGD(
				"GPU[%02d*] Freq: %d, Volt: %d, vsram: %d,",
				inner_idx, inner_freq * 1000, inner_volt,
				signed_table[inner_idx].vsram);
		}
		GPUFREQ_LOGD("GPU[%02d*] Freq: %d, Volt: %d", front_idx,
			     large_freq * 1000, large_volt);
	}
...
static void __gpufreq_apply_aging(unsigned int apply_aging)
{
	...
	for (i = 0; i < opp_num; i++) {
		...
		working_table[i].vsram =
			__gpufreq_get_vsram_by_vgpu(working_table[i].volt);

		GPUFREQ_LOGD("apply Vaging: %d, GPU[%02d] Volt: %d, Vsram: %d",
			     apply_aging, i, working_table[i].volt,
			     working_table[i].vsram);
	}
				   ...
				   int adj_num)
{
	...
	GPUFREQ_TRACE_START("adj_table=0x%x, adj_num=%d, target=%d", adj_table,
			    adj_num, target);
	...
	for (i = 0; i < adj_num; i++) {
		...
		if (oppidx >= 0 && oppidx < opp_num) {
			signed_table[oppidx].freq =
				adj_table[i].freq ? adj_table[i].freq :
						    signed_table[oppidx].freq;
			signed_table[oppidx].volt =
				adj_table[i].volt ? adj_table[i].volt :
						    signed_table[oppidx].volt;
			signed_table[oppidx].vsram =
				adj_table[i].vsram ? adj_table[i].vsram :
						     signed_table[oppidx].vsram;
			...
			GPUFREQ_LOGE("invalid adj_table[%d].oppidx: %d", i,
				     adj_table[i].oppidx);
		}

		GPUFREQ_LOGD(
			"%s[%02d*] Freq: %d, Volt: %d, Vsram: %d, Vaging: %d",
			(target == TARGET_STACK) ? "STACK" : "GPU", oppidx,
			signed_table[oppidx].freq, signed_table[oppidx].volt,
			signed_table[oppidx].vsram,
			signed_table[oppidx].vaging);
	}
						 ...
						 u32 *efuse_error)
{
	...
	GPUFREQ_LOGD("efuse_val1=0x%08x, efuse_val2=0x%08x", efuse_val1,
		     efuse_val2);
	...
	*efuse_error = efuse_val2 >> 31;
	...
	GPUFREQ_LOGD(
		"a_t0_lvt_rt=%08x, a_t0_ulvt_rt=%08x, a_shift_error=%08x, efuse_error=%08x",
					    ...
					    u32 *a_tn_ulvt_cnt)
{
	...
	GPUFREQ_LOGD("aging_data0=0x%08x, aging_data1=0x%08x", aging_data0,
		     aging_data1);
	...
	*a_tn_ulvt_cnt = (aging_data1 & 0xFFFF);
	...
	GPUFREQ_LOGD("a_tn_lvt_cnt=0x%08x, a_tn_ulvt_cnt=0x%08x", *a_tn_lvt_cnt,
		     *a_tn_ulvt_cnt);
#else
...
static unsigned int __gpufreq_get_aging_table_idx(
	u32 a_t0_lvt_rt, u32 a_t0_ulvt_rt, u32 a_shift_error, u32 efuse_error,
	u32 a_tn_lvt_cnt, u32 a_tn_ulvt_cnt, unsigned int is_efuse_read_success)
{
	...
	GPUFREQ_LOGD("tj1=%d, tj2=%d, tj=%d, adiff1=%d, adiff2=%d", tj1, tj2,
		     tj, adiff1, adiff2);
	...
	if (g_aging_load)
		...
		GPUFREQ_LOGW(
			"non of the condition is true for aging_table_idx");
		aging_table_idx = 3;
	}
#else
	GPUFREQ_UNREFERENCED(a_t0_lvt_rt);
...
static void __gpufreq_aging_adjustment(void)
{
	...
	if (__gpufreq_dvfs_enable()) {
		...
		is_efuse_read_success = __gpufreq_asensor_read_efuse(
			&a_t0_lvt_rt, &a_t0_ulvt_rt, &a_shift_error,
			&efuse_error);
	...
	g_asensor_info.aging_table_idx_most_agrresive =
		GPUFREQ_AGING_MOST_AGRRESIVE;
	...
	/* prepare aging adj */
	aging_adj =
		kcalloc(adj_num, sizeof(struct gpufreq_adj_info), GFP_KERNEL);
	if (!aging_adj) {
...
static void __gpufreq_avs_adjustment(void)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		temp_freq = 0;
		temp_freq |=
			(val & 0x00100000) >> 10; // Get freq[10] from efuse[20]
		temp_freq |= (val & 0x00000C00) >>
			     2; // Get freq[9:8] from efuse[11:10]
		temp_freq |= (val & 0x00000003)
			     << 6; // Get freq[7:6] from efuse[1:0]
		temp_freq |= (val & 0x000000C0) >>
			     2; // Get freq[5:4] from efuse[7:6]
		temp_freq |= (val & 0x000C0000) >>
			     16; // Get freq[3:2] from efuse[19:18]
		temp_freq |= (val & 0x00003000) >>
			     12; // Get freq[1:0] from efuse[13:12]
		...
		temp_volt = 0;
		temp_volt |= (val & 0x0003C000) >>
			     14; // Get volt[3:0] from efuse[17:14]
		temp_volt |=
			(val & 0x00000030); // Get volt[5:4] from efuse[5:4]
		temp_volt |= (val & 0x0000000C)
			     << 4; // Get volt[7:6] from efuse[3:2]
		/* Volt is stored in efuse with 6.25mV unit */
		temp_volt *= 625;
		/* clamp to signoff Volt */
		if (temp_volt > g_gpu.signed_table[oppidx].volt) {
			GPUFREQ_LOGW(
				"OPP[%02d*]: AVS efuse[%d].volt(%d) > signed-off.volt(%d)",
				oppidx, i, temp_volt,
...
static void __gpufreq_init_shader_present(void)
{
	...
	GPUFREQ_LOGD("segment_id: %d, shader_present: %d", segment_id,
		     g_shader_present);
}
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGD(
		"number of signed GPU OPP: %d, upper and lower bound: [%d, %d]",
		g_gpu.signed_opp_num, g_gpu.segment_upbound,
		g_gpu.segment_lowbound);
	GPUFREQ_LOGI(
		"number of working GPU OPP: %d, max and min OPP index: [%d, %d]",
	...
	g_gpu.working_table = kcalloc(
		g_gpu.opp_num, sizeof(struct gpufreq_opp_info), GFP_KERNEL);
	...
	for (i = 0; i < g_gpu.opp_num; i++) {
		...
		GPUFREQ_LOGD(
			"GPU[%02d] Freq: %d, Volt: %d, Vsram: %d, Vaging: %d",
...
static int __gpufreq_init_mtcmos(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg1_dev)) {
		ret = g_mtcmos->mfg1_dev ? PTR_ERR(g_mtcmos->mfg1_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg1_dev (%ld)", ret);
		goto done;
	}
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg2_dev)) {
		ret = g_mtcmos->mfg2_dev ? PTR_ERR(g_mtcmos->mfg2_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg2_dev (%ld)", ret);
		goto done;
	}
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg3_dev)) {
		ret = g_mtcmos->mfg3_dev ? PTR_ERR(g_mtcmos->mfg3_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg3_dev (%ld)", ret);
		goto done;
	}
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg4_dev)) {
		ret = g_mtcmos->mfg4_dev ? PTR_ERR(g_mtcmos->mfg4_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg4_dev (%ld)", ret);
		goto done;
	}
	...
	if (IS_ERR_OR_NULL(g_mtcmos->mfg5_dev)) {
		ret = g_mtcmos->mfg5_dev ? PTR_ERR(g_mtcmos->mfg5_dev) :
					   GPUFREQ_ENODEV;
		__gpufreq_abort(GPUFREQ_CCF_EXCEPTION,
				"fail to get mfg5_dev (%ld)", ret);
		goto done;
	}
...
static int __gpufreq_init_clk(struct platform_device *pdev)
{
	...
	g_clk->clk_sc_main_parent =
		devm_clk_get(&pdev->dev, "clk_sc_main_parent");
...
static int __gpufreq_init_platform_info(struct platform_device *pdev)
{
	...
	of_wrapper =
		of_find_compatible_node(NULL, NULL, "mediatek,gpufreq_wrapper");
	...
	if (unlikely(!g_nth_emicfg_base)) {
		GPUFREQ_LOGE("fail to ioremap NTH_EMICFG_REG: 0x%llx",
			     res->start);
		goto done;
	}
	...
	if (unlikely(!g_sth_emicfg_base)) {
		GPUFREQ_LOGE("fail to ioremap STH_EMICFG_REG: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_nth_emicfg_ao_mem_base =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_nth_emicfg_ao_mem_base)) {
		GPUFREQ_LOGE("fail to ioremap NTH_EMICFG_AO_MEM_REG: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_sth_emicfg_ao_mem_base =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_sth_emicfg_ao_mem_base)) {
		GPUFREQ_LOGE("fail to ioremap STH_EMICFG_AO_MEM_REG: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_infra_ao_debug_ctrl =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_infra_ao_debug_ctrl)) {
		GPUFREQ_LOGE("fail to ioremap INFRA_AO_DEBUG_CTRL: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_infra_ao1_debug_ctrl =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_infra_ao1_debug_ctrl)) {
		GPUFREQ_LOGE("fail to ioremap INFRA_AO1_DEBUG_CTRL: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_nth_emi_ao_debug_ctrl =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_nth_emi_ao_debug_ctrl)) {
		GPUFREQ_LOGE("fail to ioremap NTH_EMI_AO_DEBUG_CTRL: 0x%llx",
			     res->start);
		goto done;
	}
	...
	if (unlikely(!g_infra_ao_mem_base)) {
		GPUFREQ_LOGE("fail to ioremap INFRACFG_AO_MEM: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_mfg_cpe_control_base =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_mfg_cpe_control_base)) {
		GPUFREQ_LOGE("fail to ioremap MFG_CPE_CONTROL: 0x%llx",
			     res->start);
		goto done;
	}
	...
	g_mfg_cpe_sensor_base =
		devm_ioremap(gpufreq_dev, res->start, resource_size(res));
	if (unlikely(!g_mfg_cpe_sensor_base)) {
		GPUFREQ_LOGE("fail to ioremap MFG_CPE_SENSOR: 0x%llx",
			     res->start);
		goto done;
	}
#endif /* GPUFREQ_ASENSOR_ENABLE */
...
static int __gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	if (g_gpueb_support) {
		GPUFREQ_LOGI(
			"gpufreq platform probe only init reg_base/dfd/pmic/fp in EB mode");
		goto register_fp;
	}
