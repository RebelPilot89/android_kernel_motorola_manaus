			       ...
			       size_t count, loff_t *f_pos)
{
	...
	if (!kstrtoint(buf, 10, &value)) {
		if (value == GPU_BW_SPORT_MODE) {
			...
			else if (value >= GPU_BW_RATIO_FLOOR &&
				 value <= GPU_BW_RATIO_CEIL)
				g_mode = value;
		}
...
static void setupfw_work_handler(struct work_struct *work)
{
	...
	ret = qos_ipi_to_sspm_scmi_command(qos_d.cmd, qos_d.u.gpu_info.addr,
...
void MTKGPUQoS_mode(void)
{
	...
	} else if (g_mode >= GPU_BW_RATIO_FLOOR &&
		   g_mode <= GPU_BW_RATIO_CEIL) {
		gpu_info_buf->freq = g_mode;
		/* no bw prediction */
