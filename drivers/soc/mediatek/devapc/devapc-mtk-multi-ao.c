...
static void query_devapc_subsys_status(int slave_type)
{
	...
	if (slave_type == DEVAPC_TYPE_MMUP) {
		arm_smccc_smc(MTK_SIP_KERNEL_DAPC_MMUP_GET, 0, 0, 0, 0, 0, 0, 0,
			      &res);
...
static bool is_devapc_subsys_power_on(int slave_type)
{
	...
	list_for_each_entry (powercb, &powercb_list, list) {
		if (slave_type == DEVAPC_TYPE_ADSP) {
			if (powercb->type == DEVAPC_TYPE_ADSP &&
			    powercb->query_power)
		...
		if (slave_type == DEVAPC_TYPE_MMINFRA) {
			if (powercb->type == DEVAPC_TYPE_MMINFRA &&
			    powercb->query_power)
				return powercb->query_power() &&
				       is_devapc_subsys_enabled(slave_type);
			else
		...
		if (slave_type == DEVAPC_TYPE_MMUP) {
			if (powercb->type == DEVAPC_TYPE_MMUP &&
			    powercb->query_power)
				return powercb->query_power() &&
				       is_devapc_subsys_enabled(slave_type);
			else
				       ...
				       uint32_t index)
{
	...
	if ((slave_type < slave_type_num &&
		...
		pr_err(PFX "%s:0x%x or %s:0x%x or %s:0x%x is out of boundary\n",
		       "slave_type", slave_type, "pd_reg_type", pd_reg_type,
		       "index", index);
		return NULL;
	}
...
static void sramrom_vio_handler(void)
{
	...
	arm_smccc_smc(MTK_SIP_KERNEL_CLR_SRAMROM_VIO, 0, 0, 0, 0, 0, 0, 0,
		      &res);
	...
	if (sramrom_vio_id)
		...
		vio_info->master_id =
			(sramrom_vio_sta & sramrom_vios->vio_id_mask) >>
			sramrom_vios->vio_id_shift;
	vio_info->domain_id =
		(sramrom_vio_sta & sramrom_vios->vio_domain_mask) >>
	...
	pr_info(PFX "%s: %s:0x%x, %s:0x%x, %s:%s, %s:0x%x, %s:0x%x\n", __func__,
		"master_id", vio_info->master_id, "domain_id",
		vio_info->domain_id, "rw", rw ? "Write" : "Read", "vio_addr",
		vio_info->vio_addr, "vio_id", sramrom_vio_id);
}
...
static void mask_module_irq(int slave_type, uint32_t module, bool mask)
{
	...
	if ((slave_type < slave_type_num) &&
		...
		pr_err(PFX "%s: %s, %s:0x%x, %s:0x%x, %s:%s\n", __func__,
		       "out of boundary", "slave_type", slave_type,
		       "module_index", module, "mask", mask ? "true" : "false");
}
	...
	for (slave_type = 0; slave_type < slave_type_num; slave_type++) {
		...
		for (i = 0; i < vio_info->vio_mask_sta_num[slave_type]; i++) {
			pr_info(PFX "%s: %s_%d: 0x%x, %s_%d: 0x%x\n",
				slave_type_to_string(slave_type), "VIO_MASK", i,
				readl(mtk_devapc_pd_get(slave_type, VIO_MASK,
							i)),
				"VIO_STA", i,
				readl(mtk_devapc_pd_get(slave_type, VIO_STA,
							i)));
		}
	}
}
...
static void devapc_vio_info_print(void)
{
	...
	pr_info(PFX "%s%x, %s%x, %s%x, %s%x\n", "Vio Addr:0x",
		vio_info->vio_addr, "High:0x", vio_info->vio_addr_high,
		"Bus ID:0x", vio_info->master_id, "Dom ID:0x",
		vio_info->domain_id);

	pr_info(PFX "%s - %s%s, %s%x\n", "Violation",
		"Current Process:", current->comm, "PID:", current->pid);
}
	...
	if (slave_type == mm2nd_slv_type) {
		...
		if (mdp_vio || disp2_vio || mmsys_vio) {
			...
			mtk_devapc_ctx->soc->mm2nd_vio_handler(
				...
				mtk_devapc_ctx->soc->vio_info, mdp_vio,
				disp2_vio, mmsys_vio);
...
static uint32_t sync_vio_dbg(int slave_type, uint32_t shift_bit)
{
	...
	if (slave_type >= slave_type_num ||
		...
		pr_err(PFX "param check failed, %s:0x%x, %s:0x%x\n",
		       "slave_type", slave_type, "shift_bit", shift_bit);
		return 0;
	}
	...
	pr_debug(PFX "(Post) %s:0x%x, %s:0x%x, %s:0x%x\n", "VIO_SHIFT_STA",
		 readl(pd_vio_shift_sta_reg), "VIO_SHIFT_SEL",
		 readl(pd_vio_shift_sel_reg), "VIO_SHIFT_CON",
		 readl(pd_vio_shift_con_reg));

	return sync_done;
}

static const char *const perm_to_str[] = { "NO_PROTECTION", "SECURE_RW_ONLY",
					   "SECURE_RW_NS_R_ONLY", "FORBIDDEN",
					   "NO_PERM_CTRL" };
...
static uint8_t get_permission(int slave_type, int module_index, int domain)
{
	...
	if (slave_type >= slave_type_num ||
		...
		pr_err(PFX "%s: param check failed, %s:0x%x, %s:0x%x\n",
		       __func__, "slave_type", slave_type, "module_index",
		       module_index);
		return 0xFF;
	}
...
static void mtk_devapc_vio_check(int slave_type, int *shift_bit)
{
	...
	if (!vio_shift_sta) {
		...
		pr_info(PFX "%s: 0x%x is matched with %s:%d\n", "vio_shift_sta",
			vio_shift_sta, "shift_bit", *shift_bit);

	} else {
		pr_info(PFX "%s: 0x%x is not matched with %s:%d\n",
			"vio_shift_sta", vio_shift_sta, "shift_bit",
			*shift_bit);
...
static bool mtk_devapc_dump_vio_dbg(int slave_type, int *vio_idx, int *index)
{
	...
	if (!mtk_devapc_ctx->serror)
		pr_info(PFX
			"check_devapc_vio_status: no violation for %s:0x%x\n",
			"slave_type", slave_type);
	return false;
}
	...
	for (slave_type = 0; slave_type < slave_type_num; slave_type++) {
		...
		if (vio_shift_sta) {
			...
			pr_info(PFX "clear %s:0x%x %s:0x%x to 0x%x\n",
				"slave_type", slave_type, "VIO_SHIFT_STA",
				vio_shift_sta, readl(pd_vio_shift_sta_reg));
		}
		...
		for (i = 0; i < ndevices[slave_type].vio_slave_num; i++) {
			...
			if ((check_vio_status(slave_type, vio_idx) ==
				...
				pr_warn(PFX "%s, %s:0x%x, %s:0x%x, %s:%d\n",
					"clear vio status failed", "slave_type",
					slave_type, "vio_index", vio_idx,
					"retry", retry);
				 ...
				 uint32_t vio_index, uint32_t vio_addr)
{
	...
	if (!strncmp(vio_master, "CPUM_M", 6) ||
	    ...
	    !strncmp(vio_master, "AP", 2) || !strncmp(vio_master, "PCIE", 4) ||
	...
	list_for_each_entry (viocb, &viocb_list, list) {
		...
		if (viocb->id == DEVAPC_SUBSYS_CLKMGR && viocb->debug_dump)
			viocb->debug_dump();
	}
...
static void devapc_dump_info(void)
{
	...
	for (slave_type = 0; slave_type < slave_type_num; slave_type++) {
		...
		vio_master = mtk_devapc_ctx->soc->master_get(
			vio_info->master_id, vio_info->vio_addr, slave_type,
			vio_info->shift_sta_bit, vio_info->domain_id);
		...
		pr_info(PFX "%s - %s:0x%x, %s:0x%x, %s:0x%x, %s:0x%x\n",
			"Violation", "slave_type", slave_type, "sys_index",
			device_info[slave_type][index].sys_index, "ctrl_index",
			device_info[slave_type][index].ctrl_index, "vio_index",
...
static void devapc_ut(uint32_t cmd)
{
	...
	if (cmd == DEVAPC_UT_DAPC_INFRA_VIO || cmd == DEVAPC_UT_DAPC_VLP_VIO ||
			     ...
			     size_t count, loff_t *data)
{
	...
	if (!strncmp(cmd_str, "enable_ut", sizeof("enable_ut"))) {
		...
		pr_info(PFX "%s:0x%x, %s:0x%lx, %s:0x%lx, %s:0x%lx\n",
			"slave_type", slave_type, "sys_index", sys_index,
			"domain_id", domain, "ctrl_index", ctrl_index);
...
static ssize_t set_swp_addr_show(struct device_driver *driver, char *buf)
{
	return snprintf(
		buf, PAGE_SIZE,
		"%s:%s\n\t%s:%s\n\t%s:0x%x\n\t%s:0x%x\n\t%s:0x%x\n\t%s:0x%x\n",
		"devapc_swp", devapc_swp_ctx->swp_enable ? "enable" : "disable",
		"swp_rw", devapc_swp_ctx->swp_rw ? "write" : "read",
		"swp_physical_addr", devapc_swp_ctx->swp_phy_addr, "swp_rg",
		devapc_swp_ctx->swp_rg, "swp_wr_val",
		devapc_swp_ctx->swp_wr_val, "swp_wr_mask",
		devapc_swp_ctx->swp_wr_mask);
}

static ssize_t set_swp_addr_store(struct device_driver *driver, const char *buf,
				  size_t count)
{
...
#ifdef CONFIG_MTK_SERROR_HOOK
static void devapc_arm64_serror_panic_hook(void *data, struct pt_regs *regs,
					   unsigned int esr)
{
...
static int devapc_hre_init(void)
{
	...
	for (slave_type = 0; slave_type < slave_type_num; slave_type++) {
		...
		mtk_devapc_ctx->pd_reg[slave_type].pd_vio_mask_reg =
		...
		mtk_devapc_ctx->pd_reg[slave_type].pd_vio_sta_reg =
			kzalloc(size, GFP_KERNEL);
...
exit:
	for (slave_type = 0; slave_type < slave_type_num; slave_type++) {
		if (mtk_devapc_ctx->pd_reg[slave_type].pd_vio_mask_reg != NULL)
			kfree(mtk_devapc_ctx->pd_reg[slave_type]
				      .pd_vio_mask_reg);
...
int mtk_devapc_probe(struct platform_device *pdev, struct mtk_devapc_soc *soc)
{
	...
	if (ret)
		pr_info(PFX
			"register android_rvh_arm64_serror_panic failed!\n");
#endif
	...
	for (irq_type = 0; irq_type < irq_type_num; irq_type++) {
		mtk_devapc_ctx->devapc_ao_base[irq_type] =
			of_iomap(node, dt_index + irq_type);
	...
	ret = driver_create_file(pdev->dev.driver, &driver_attr_set_swp_addr);
	if (ret)
	...
	for (irq_type = 0; irq_type < irq_type_num; irq_type++) {
		mtk_devapc_ctx->devapc_irq[irq_type] =
			irq_of_parse_and_map(node, irq_type);
	...
	mtk_devapc_ctx->devapc_infra_clk =
		devm_clk_get(&pdev->dev, "devapc-infra-clock");
