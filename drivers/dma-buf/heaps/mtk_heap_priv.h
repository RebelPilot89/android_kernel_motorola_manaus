...
struct mtk_heap_priv_info {
	...
	int (*buf_priv_dump)(const struct dma_buf *dmabuf,
			     struct dma_heap *heap, void *seq_file);
};
...
static inline void dmabuf_release_check(const struct dma_buf *dmabuf)
{
	...
	if (WARN(!list_empty(&dmabuf->attachments),
		 ...
		 dmabuf->size, dmabuf->name, dmabuf->exp_name,
		...
		list_for_each_entry (attach_obj, &dmabuf->attachments, node) {
			...
			attach_cnt++;
			if (attach_obj->sgt &&
			    dev_iommu_fwspec_get(attach_obj->dev))
			...
			dmabuf_dump(
				...
				attach_cnt, iova, attach_obj->dma_map_attrs,
				attach_obj->dir, device_name);
		}
