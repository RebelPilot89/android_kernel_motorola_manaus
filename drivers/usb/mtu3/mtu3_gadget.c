...
void mtu3_req_complete(struct mtu3_ep *mep, struct usb_request *req, int status)
	__releases(mep->mtu->lock) __acquires(mep->mtu->lock)
{
	...
	dev_dbg(mtu->dev, "%s complete req: %p, sts %d, %d/%d\n", mep->name,
		req, req->status, req->actual, req->length);
...
static void nuke(struct mtu3_ep *mep, const int status)
{
	...
	while (!list_empty(&mep->req_list)) {
		mreq = list_first_entry(&mep->req_list, struct mtu3_request,
					list);
...
static int mtu3_gadget_queue(struct usb_ep *ep, struct usb_request *req,
			     gfp_t gfp_flags)
{
	struct mtu3_ep *mep = to_mtu3_ep(ep);
	struct mtu3_request *mreq = to_mtu3_request(req);
	...
	dev_dbg(mtu->dev, "%s %s EP%d(%s), req=%p, maxp=%d, len#%d\n", __func__,
		mep->is_in ? "TX" : "RX", mreq->epnum, ep->name, mreq,
		ep->maxpacket, mreq->request.length);
...
static void init_hw_ep(struct mtu3 *mtu, struct mtu3_ep *mep, u32 epnum,
		       u32 is_in)
{
	mep->epnum = epnum;
	mep->mtu = mtu;
