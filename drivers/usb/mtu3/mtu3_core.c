...
static int ep_fifo_alloc(struct mtu3_ep *mep, u32 seg_size)
{
	...
	start_bit = bitmap_find_next_zero_area(fifo->bitmap, fifo->limit, 0,
					       num_bits, 0);
	...
	dev_dbg(mep->mtu->dev, "%s fifo:%#x/%#x, start_bit: %d\n", __func__,
		mep->fifo_seg_size, mep->fifo_size, start_bit);
...
static void mtu3_intr_enable(struct mtu3 *mtu)
{
	...
	if (mtu->is_u3_ip) {
		...
		value = HOT_RST_INTR | WARM_RST_INTR | ENTER_U3_INTR |
			EXIT_U3_INTR | SS_INACTIVE_INTR | SS_DISABLE_INTR;
...
/* for non-ep0 */
int mtu3_config_ep(struct mtu3 *mtu, struct mtu3_ep *mep, int interval,
		   int burst, int mult)
{
	void __iomem *mbase = mtu->mac_base;
	...
	dev_dbg(mtu->dev, "%s: %s, fifo-addr:%#x, fifo-size:%#x(%#x/%#x)\n",
		__func__, mep->name, mep->fifo_addr, mep->fifo_size, fifo_sgsz,
		mep->fifo_seg_size);
...
static void get_ep_fifo_config(struct mtu3 *mtu)
{
	...
	dev_dbg(mtu->dev, "%s, TX: base-%d, limit-%d; RX: base-%d, limit-%d\n",
		__func__, tx_fifo->base, tx_fifo->limit, rx_fifo->base,
		rx_fifo->limit);
}
...
static irqreturn_t mtu3_link_isr(struct mtu3 *mtu)
{
	...
	case MTU3_SPEED_FULL:
		...
		mtu3_writel(mbase, U3D_USB20_LPM_PARAMETER,
			    LPM_BESLDCK(0xf) | LPM_BESLCK(4) |
				    LPM_BESLCK_U3(0xa));
	...
	mtu3_dbg_trace(mtu->dev, "link speed %s", usb_speed_string(udev_speed));
