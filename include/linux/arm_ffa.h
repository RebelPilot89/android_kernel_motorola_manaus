...
ffa_device_register(const uuid_t *uuid, int vm_id, const struct ffa_ops *ops)
{
	return NULL;
}
...
static inline bool ffa_device_is_valid(struct ffa_device *ffa_dev)
{
	return false;
}
...
#define ffa_unregister(driver) ffa_driver_unregister(driver)
...
#define COMPOSITE_OFFSET(x) (offsetof(struct ffa_mem_region, ep_mem_access[x]))
