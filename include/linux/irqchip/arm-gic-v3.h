...
#define GICR_CTLR_CES			(1UL << 1)	/* from donor */
#define GICR_CTLR_IR			(1UL << 2)	/* from donor */
...
/*
 * Hypervisor interface registers (ICH_*) for GICv3 virtual CPU interface.
 * ICH_LR<n>_EL2 are 64-bit; ICH_HCR_EL2, ICH_VMCR_EL2, ICH_VTR_EL2 are 32-bit.
 */
...
/* ICH_LR<n>_EL2 bits (64-bit List Registers) */
...
#define ICH_VMCR_PMR_MASK	(0xff << ICH_VMCR_PMR_SHIFT)

/* ICH_VTR_EL2 bits */
#define ICH_VTR_TDS_MASK	(1 << 19)
#define ICH_VTR_A3V_MASK	(1 << 21)
#define ICH_VTR_SEIS_MASK	(1 << 22)
...
struct rdists {
	struct {
		...
		u64		flags;			/* replaced lpi_enabled, from donor */
	...
	u32			gicd_typer;
	u32			gicd_typer2;
	int			cpuhp_memreserve_state;	/* from donor */
	bool			has_vlpis;
	bool			has_rvpeid;
	bool			has_direct_lpi;
...
int __init its_lpi_memreserve_init(void);	/* from donor */
