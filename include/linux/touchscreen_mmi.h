...
#define EVENT_PRE_DISPLAY_OFF (event == PANEL_EVENT_PRE_DISPLAY_OFF)

#define EVENT_DISPLAY_OFF (event == PANEL_EVENT_DISPLAY_OFF)

#define EVENT_PRE_DISPLAY_ON (event == PANEL_EVENT_PRE_DISPLAY_ON)

#define EVENT_DISPLAY_ON (event == PANEL_EVENT_DISPLAY_ON)
...
#define REGISTER_PANEL_NOTIFIER                                                               \
	{                                                                                     \
		...
		touch_cdev->panel_nb.notifier_call = ts_mmi_panel_cb;                         \
		if (!ppdata->ctrl_dsi) {                                                      \
			if (mtk_disp_notifier_register("Touch",                               \
						       &touch_cdev->panel_nb))                \
				dev_err(DEV_MMI,                                              \
					"%s: Failed to register disp notifier client:%d",     \
					__func__, ret);                                       \
		} else {                                                                      \
			if (mtk_disp_sub_notifier_register(                                   \
				    "Cli_touch", &touch_cdev->panel_nb))                      \
				dev_err(DEV_MMI,                                              \
					"%s: Failed to register disp sub notifier client:%d", \
					__func__, ret);                                       \
		}                                                                             \
	}
...
#define GET_CONTROL_DSI_INDEX                                                  \
	...
	{                                                                      \
		...
		blank = (int *)evd;                                            \
		dev_dbg(DEV_MMI,                                               \
			"%s: mtk notification: event = %lu, blank = %d\n",     \
...
#define EVENT_DISPLAY_ON                                                       \
	((event == MTK_DISP_EVENT_BLANK) && (*blank == MTK_DISP_BLANK_UNBLANK))
...
#define REGISTER_PANEL_NOTIFIER                                                \
	{                                                                      \
		...
		cookie = panel_event_notifier_register(                        \
			...
			active_panel, &ts_mmi_panel_cb, touch_cdev);           \
		if (!cookie)                                                   \
			ret = -1;                                              \
		else                                                           \
...
#define EVENT_PRE_DISPLAY_OFF                                                  \
	((event == DRM_PANEL_EVENT_BLANK) && (evdata.early_trigger))

#define EVENT_DISPLAY_OFF                                                      \
	((event == DRM_PANEL_EVENT_BLANK) && (!evdata.early_trigger))

#define EVENT_PRE_DISPLAY_ON                                                   \
	((event == DRM_PANEL_EVENT_UNBLANK) && (evdata.early_trigger))
...
#define GET_CONTROL_DSI_INDEX                                                  \
	...
	{                                                                      \
		...
		dev_dbg(DEV_MMI,                                               \
			"%s: drm notification: event = %lu, blank = %d\n",     \
			__func__, event, *blank);                              \
	}
...
#define GET_CONTROL_DSI_INDEX                                                  \
	...
	{                                                                      \
		...
		blank = (int *)evdata->data;                                   \
		dev_dbg(DEV_MMI,                                               \
			"%s: fb notification: event = %lu, blank = %d\n",      \
			__func__, event, *blank);                              \
	}

#define EVENT_PRE_DISPLAY_OFF                                                  \
	((event == FB_EARLY_EVENT_BLANK) && (*blank == FB_BLANK_POWERDOWN))

#define EVENT_DISPLAY_OFF                                                      \
	((event == FB_EVENT_BLANK) && (*blank == FB_BLANK_POWERDOWN))

#define EVENT_PRE_DISPLAY_ON                                                   \
	((event == FB_EARLY_EVENT_BLANK) && (*blank == FB_BLANK_UNBLANK))

#define EVENT_DISPLAY_ON                                                       \
	((event == FB_EVENT_BLANK) && (*blank == FB_BLANK_UNBLANK))
...
#define EVENT_DISPLAY_OFF                                                      \
	((event == MSM_DRM_EVENT_BLANK) && (*blank == MSM_DRM_BLANK_POWERDOWN))
...
#define EVENT_DISPLAY_ON                                                       \
	((event == MSM_DRM_EVENT_BLANK) && (*blank == MSM_DRM_BLANK_UNBLANK))
...
static inline unsigned long long timediff_ms(struct TIME_SPEC start,
					     struct TIME_SPEC end)
{
...
struct ts_mmi_class_methods {
	...
	int (*get_gesture_type)(struct device *dev,
...
struct ts_mmi_methods {
	int (*convert_data)(struct device *dev, struct touch_event_data *data,
			    int max);
	/* GET methods */
	...
	int (*extend_attribute_group)(struct device *dev,
				      struct attribute_group **group);
	/* PM callback */
	int (*panel_state)(struct device *dev, enum ts_mmi_pm_mode from,
...
#define MMI_DEV_TO_TS_DEV(cdev)                                                \
	(((struct ts_mmi_dev *)dev_get_drvdata(dev))->dev)
/* call this after hold method mutex */
#define _TRY_TO_CALL(_method, ...)                                             \
	do {                                                                   \
		if (touch_cdev->mdata->_method) {                              \
			ret = touch_cdev->mdata->_method(DEV_TS,               \
							 ##__VA_ARGS__);       \
		}                                                              \
	} while (0)
...
#define TRY_TO_GET(_method, ...)                                               \
	do {                                                                   \
		...
		if (touch_cdev->mdata->get_##_method) {                        \
			mutex_lock(&touch_cdev->method_mutex);                 \
			ret = touch_cdev->mdata->get_##_method(DEV_TS,         \
							       ##__VA_ARGS__); \
			mutex_unlock(&touch_cdev->method_mutex);               \
		}                                                              \
	} while (0)
...
extern int ts_mmi_parse_dt(struct ts_mmi_dev *touch_cdev,
