...
int vgic_copy_lpi_list(struct kvm *kvm, struct kvm_vcpu *vcpu, u32 **intid_ptr)
{
	...
	intids =
		kmalloc_array(irq_count, sizeof(intids[0]), GFP_KERNEL_ACCOUNT);
	if (!intids)
		return -ENOMEM;
...
static unsigned long vgic_mmio_read_its_typer(struct kvm *kvm,
					      struct vgic_its *its, gpa_t addr,
					      unsigned int len)
{
	...
	reg |= GIC_ENCODE_SZ(abi->ite_esz, 4)
				       ...
				       struct vgic_irq *irq)
{
	...
	cte = list_last_entry(&dist->lpi_translation_cache, typeof(*cte),
			      entry);
...
static void vgic_its_free_collection(struct vgic_its *its, u32 coll_id)
{
	...
	for_each_lpi_its(device, ite,
			 its) if (ite->collection &&
...
static void vgic_mmio_write_its_baser(struct kvm *kvm, struct vgic_its *its,
				      gpa_t addr, unsigned int len,
				      unsigned long val)
{
	const struct vgic_its_abi *abi = vgic_its_get_abi(its);
...
static void its_mmio_write_wi(struct kvm *kvm, struct vgic_its *its, gpa_t addr,
			      unsigned int len, unsigned long val)
{
	/* Ignore */
}
...
static struct vgic_register_region its_registers[] = {
	...
	REGISTER_ITS_DESC_UACCESS(
		GITS_IIDR, vgic_mmio_read_its_iidr, its_mmio_write_wi,
		vgic_mmio_uaccess_write_its_iidr, 4, VGIC_ACCESS_32bit),
	REGISTER_ITS_DESC(GITS_TYPER, vgic_mmio_read_its_typer,
			  its_mmio_write_wi, 8,
	...
	REGISTER_ITS_DESC(GITS_CWRITER, vgic_mmio_read_its_cwriter,
			  ...
			  VGIC_ACCESS_64bit | VGIC_ACCESS_32bit),
	REGISTER_ITS_DESC_UACCESS(GITS_CREADR, vgic_mmio_read_its_creadr,
				  its_mmio_write_wi,
				  vgic_mmio_uaccess_write_its_creadr, 8,
				  VGIC_ACCESS_64bit | VGIC_ACCESS_32bit),
	REGISTER_ITS_DESC(GITS_BASER, vgic_mmio_read_its_baser,
...
void vgic_lpi_translation_cache_destroy(struct kvm *kvm)
{
	...
	list_for_each_entry_safe (cte, tmp, &dist->lpi_translation_cache,
				  entry) {
		list_del(&cte->entry);
		kfree(cte);
	}
				  ...
				  struct kvm_device_attr *attr)
{
	...
	region = vgic_find_mmio_region(its_registers, ARRAY_SIZE(its_registers),
				       offset);
	if (!region)
		return -ENXIO;
...
static int vgic_its_attr_regs_access(struct kvm_device *dev,
				     struct kvm_device_attr *attr, u64 *reg,
				     bool is_write)
{
	const struct vgic_register_region *region;
	struct vgic_its *its;
 ...
 */
static int vgic_its_restore_ite(struct vgic_its *its, u32 event_id, void *ptr,
				void *opaque)
{
 ...
 * next dte otherwise
 */
static int vgic_its_restore_dte(struct vgic_its *its, u32 id, void *ptr,
				void *opaque)
{
	struct its_device *dev;
...
static int vgic_its_save_device_tables(struct vgic_its *its)
{
	...
	list_for_each_entry (dev, &its->device_list, dev_list) {
		...
		if (!vgic_its_check_id(its, baser, dev->device_id, &eaddr))
			return -EINVAL;
...
static int handle_l1_dte(struct vgic_its *its, u32 id, void *addr, void *opaque)
{
	...
	ret = scan_its_table(its, gpa, SZ_64K, dte_esz, l2_start_id,
			     ...
			     struct kvm_device_attr *attr)
{
	...
	case KVM_DEV_ARM_VGIC_GRP_ADDR: {
		...
		ret = vgic_check_iorange(dev->kvm, its->vgic_its_base, addr,
					 SZ_64K, KVM_VGIC_V3_ITS_SIZE);
		if (ret)
			return ret;
