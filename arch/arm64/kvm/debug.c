...
#define MDSCR_EL1_DEBUG_MASK (DBG_MDSCR_SS | DBG_MDSCR_KDE | DBG_MDSCR_MDE)
...
static void kvm_arm_setup_mdcr_el2(struct kvm_vcpu *vcpu)
{
	...
	if ((vcpu->guest_debug & KVM_GUESTDBG_USE_HW) ||
	    !vcpu_get_flag(vcpu, DEBUG_DIRTY) || kvm_vcpu_os_lock_enabled(vcpu))
...
void kvm_arm_setup_debug(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu_read_sys_reg(vcpu, MDSCR_EL1) &
...
/*
	 ...
	 */
#ifdef ID_AA64DFR0_EL1_PMSVer_SHIFT
	if (cpuid_feature_extract_unsigned_field(
		    dfr0, ID_AA64DFR0_EL1_PMSVer_SHIFT) &&
	    !(read_sysreg_s(SYS_PMBIDR_EL1) & BIT(SYS_PMBIDR_EL1_P_SHIFT)))
		vcpu_set_flag(vcpu, DEBUG_STATE_SAVE_SPE);
#endif

/* Check if we have TRBE implemented and available at the host */
#ifdef ID_AA64DFR0_EL1_TraceBuffer_SHIFT
	if (cpuid_feature_extract_unsigned_field(
