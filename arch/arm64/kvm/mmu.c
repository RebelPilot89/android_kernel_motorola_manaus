...
static void kvm_s2_dcache_clean_inval_poc(void *addr, size_t size)
{
	__clean_dcache_guest_page(addr, size);
}

static void kvm_s2_icache_inval_pou(void *addr, size_t size)
{
	__invalidate_icache_guest_page(addr, size);
}
...
static struct kvm_pgtable_mm_ops kvm_s2_mm_ops = {
	...
	.dcache_clean_inval_poc = kvm_s2_dcache_clean_inval_poc,
	.icache_inval_pou = kvm_s2_icache_inval_pou,
};
				 ...
				 struct kvm_memory_slot *memslot)
{
	...
	do {
		...
		if (!(vma->vm_flags & VM_PFNMAP)) {
			...
			unmap_stage2_range(&kvm->arch.mmu, gpa,
					   vm_end - vm_start);
		}
		hva = vm_end;
	} while (hva < reg_end);
...
{
	__clean_dcache_guest_page(page_address(pfn_to_page(pfn)), size);
}

static void invalidate_icache_guest_page(kvm_pfn_t pfn, unsigned long size)
{
	__invalidate_icache_guest_page(page_address(pfn_to_page(pfn)), size);
}
...
transparent_hugepage_adjust(struct kvm_memory_slot *memslot, unsigned long hva,
			    kvm_pfn_t *pfnp, phys_addr_t *ipap)
{
	kvm_pfn_t pfn = *pfnp;
...
static int handle_hva_to_gpa(struct kvm *kvm, unsigned long start,
			     unsigned long end,
			     int (*handler)(struct kvm *kvm, gpa_t gpa,
					    u64 size, void *data),
			     void *data)
{
	struct kvm_memslots *slots;
...
static int kvm_unmap_hva_handler(struct kvm *kvm, gpa_t gpa, u64 size,
				 void *data)
{
...
static int kvm_set_spte_handler(struct kvm *kvm, gpa_t gpa, u64 size,
				void *data)
{
...
void kvm_arch_commit_memory_region(
	...
	struct kvm_memory_slot *old, const struct kvm_memory_slot *new,
				   ...
				   enum kvm_mr_change change)
{
	...
	do {
		...
		if (vma->vm_flags & VM_PFNMAP) {
			...
			ret = kvm_phys_addr_ioremap(
				kvm, gpa, pa, vm_end - vm_start, writable);
			if (ret)
				break;
		}
		hva = vm_end;
