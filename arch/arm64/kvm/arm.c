...
int kvm_vm_ioctl_enable_cap(struct kvm *kvm, struct kvm_enable_cap *cap)
{
	...
	case KVM_CAP_ARM_MTE:
		mutex_lock(&kvm->lock);
		if (!system_supports_mte() || kvm_vm_is_protected(kvm) ||
		    kvm->created_vcpus) {
			r = -EINVAL;
		} else {
			r = 0;
...
static int pkvm_check_extension(struct kvm *kvm, long ext, int kvm_cap)
{
	...
	case KVM_CAP_ARM_SVE:
		r = kvm_cap &&
		    FIELD_GET(ARM64_FEATURE_MASK(ID_AA64PFR0_EL1_SVE),
...
nommu:
	...
	if (is_protected_kvm_enabled()) {
		kvm_call_hyp_nvhe(__pkvm_vcpu_load, vcpu->kvm->arch.pkvm.handle,
			  ...
			  bool line_status)
{
	...
	vcpu_idx +=
		((irq >> KVM_ARM_IRQ_VCPU2_SHIFT) & KVM_ARM_IRQ_VCPU2_MASK) *
...
static void cpu_prepare_hyp_mode(int cpu)
{
	...
	*hyp_cpu_number_ptr = cpu;
	...
	params->hfgwtr_el2 =
		HFGxTR_EL2_nSMPRI_EL1_MASK | HFGxTR_EL2_nTPIDR2_EL0_MASK;
...
static void cpu_hyp_init_context(void)
{
	kvm_init_host_cpu_context(
		&this_cpu_ptr_hyp_sym(kvm_host_data)->host_ctxt);
...
static void teardown_hyp_mode(void)
{
	...
	for_each_possible_cpu (cpu) {
		free_pages(per_cpu(kvm_arm_hyp_stack_base, cpu),
...
static void finalize_init_hyp_mode(void)
{
	...
	for_each_possible_cpu (cpu) {
		kvm_nvhe_sym(kvm_arm_hyp_host_fp_state)[cpu] = kern_hyp_va(
...
static int init_hyp_mode(void)
{
	...
	for_each_possible_cpu (cpu) {
		...
		stack_base = __get_free_pages(GFP_KERNEL,
					      NVHE_STACK_SHIFT - PAGE_SHIFT);
		if (!stack_base) {
			err = -ENOMEM;
			goto out_err;
	...
	for_each_possible_cpu (cpu) {
		...
		err = create_hyp_mappings(
			stack_base, stack_base + NVHE_STACK_SIZE, PAGE_HYP);
		if (err) {
		...
		params->stack_hyp_va = (unsigned long)kern_hyp_va(
			stack_base + NVHE_STACK_SIZE);
	}
