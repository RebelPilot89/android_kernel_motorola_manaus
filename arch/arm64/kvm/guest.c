...
static int core_reg_size_from_offset(const struct kvm_vcpu *vcpu, u64 off)
{
	...
	switch (off) {
	case KVM_REG_ARM_CORE_REG(regs.regs[0])... KVM_REG_ARM_CORE_REG(
		regs.regs[30]):
	...
	case KVM_REG_ARM_CORE_REG(spsr[0])... KVM_REG_ARM_CORE_REG(
		spsr[KVM_NR_SPSR - 1]):
		size = sizeof(__u64);
		break;

	case KVM_REG_ARM_CORE_REG(fp_regs.vregs[0])... KVM_REG_ARM_CORE_REG(
		fp_regs.vregs[31]):
			     ...
			     const struct kvm_one_reg *reg)
{
	...
	const u64 zreg_id_max =
		KVM_REG_ARM64_SVE_ZREG(SVE_NUM_ZREGS - 1, SVE_NUM_SLICES - 1);
	...
	const u64 last_preg_id =
		KVM_REG_ARM64_SVE_PREG(SVE_NUM_PREGS - 1, SVE_NUM_SLICES - 1);
	...
	if (reg->id >= zreg_id_min && reg->id <= zreg_id_max) {
		...
		reqoffset =
			SVE_SIG_ZREG_OFFSET(vq, reg_num) - SVE_SIG_REGS_OFFSET;
		...
		reqoffset =
			SVE_SIG_PREG_OFFSET(vq, reg_num) - SVE_SIG_REGS_OFFSET;
