...
#ifndef KVM_CAP_ARM_PROTECTED_VM_FLAGS_SET_FW_IPA
...
static inline bool pkvm_addrs_overlap(phys_addr_t base1, phys_addr_t size1,
...
{
	return static_branch_likely(&kvm_protected_mode_initialized);
}
...
static struct pkvm_moveable_reg *moveable_regs =
	kvm_nvhe_sym(pkvm_moveable_regs);
...
{
	sort(hyp_memory, *hyp_memblock_nr_ptr, sizeof(struct memblock_region),
	     cmp_hyp_memblock, NULL);
}
...
static int __init register_memblock_regions(void)
{
	...
	for_each_mem_region (reg) {
		...
		if (!*pvmfw_size || pvmfw_in_mem ||
		    !pkvm_addrs_overlap(reg->base, reg->size, *pvmfw_base,
					*pvmfw_size))
			continue;
...
{
	sort(moveable_regs, kvm_nvhe_sym(pkvm_moveable_regs_nr),
	     sizeof(struct pkvm_moveable_reg), cmp_moveable_reg, NULL);
}
...
void __init kvm_hyp_reserve(void)
{
	...
	hyp_mem_base =
		memblock_phys_alloc(ALIGN(hyp_mem_size, PMD_SIZE), PMD_SIZE);
	if (!hyp_mem_base)
...
static int __pkvm_create_hyp_vm(struct kvm *host_kvm)
{
	...
	hyp_vm_sz =
		PAGE_ALIGN(PKVM_HYP_VM_SIZE +
			   array_size(host_kvm->created_vcpus, sizeof(void *)));
...
void pkvm_host_reclaim_page(struct kvm *host_kvm, phys_addr_t ipa)
{
	...
	struct mm_struct *mm = current->mm;
	struct rb_node *node;
	struct rb_node **new = &host_kvm->arch.pkvm.pinned_pages.rb_node;

	spin_lock(&host_kvm->mmu_lock);
	node = NULL;
	while (*new) {
		struct kvm_pinned_page *p =
			container_of(*new, struct kvm_pinned_page, node);
...
static int __init pkvm_firmware_rmem_init(struct reserved_mem *rmem)
{
	...
	if (!of_get_flat_dt_prop(node, "no-map", NULL))
		return pkvm_firmware_rmem_err(rmem,
					      "missing \"no-map\" property");

	if (of_get_flat_dt_prop(node, "reusable", NULL))
		return pkvm_firmware_rmem_err(
			rmem, "\"reusable\" property unsupported");
			      ...
			      struct kvm_protected_vm_info __user *info)
{
	struct kvm_protected_vm_info kinfo = {
		.firmware_size =
			pkvm_firmware_mem ? pkvm_firmware_mem->size : 0,
	};
