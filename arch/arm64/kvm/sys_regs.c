 ...
 */
static bool access_dcsw(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			const struct sys_reg_desc *r)
{
	if (!p->is_write)
 ...
 */
static bool access_vm_reg(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			  const struct sys_reg_desc *r)
{
...
static bool access_actlr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			 const struct sys_reg_desc *r)
{
	u64 mask, shift;
 ...
 */
static bool access_gic_sgi(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			   const struct sys_reg_desc *r)
{
	bool g1;
...
static bool trap_raz_wi(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			const struct sys_reg_desc *r)
{
	if (p->is_write)
		return ignore_write(vcpu, p);
 ...
 * treat it separately.
 */
static bool trap_loregion(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			  const struct sys_reg_desc *r)
{
...
static bool trap_oslar_el1(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			   const struct sys_reg_desc *r)
{
	u64 oslsr;
...
static bool trap_oslsr_el1(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			   const struct sys_reg_desc *r)
{
	if (p->is_write)
		return write_to_read_only(vcpu, p, r);
 ...
 */
static bool trap_debug_regs(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			    const struct sys_reg_desc *r)
{
	if (p->is_write) {
 ...
 */
static void reg_to_dbg(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
		       const struct sys_reg_desc *rd, u64 *dbg_reg)
{
	u64 mask, shift, val;
...
static void dbg_to_reg(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
		       const struct sys_reg_desc *rd, u64 *dbg_reg)
{
	u64 mask, shift;
...
static bool trap_bvr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
...
static bool trap_bcr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
		     const struct sys_reg_desc *rd)
{
...
static bool trap_wvr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
		     const struct sys_reg_desc *rd)
{
...
static bool trap_wcr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
		     const struct sys_reg_desc *rd)
{
...
{
	return check_pmu_access_disabled(vcpu, ARMV8_PMU_USERENR_SW |
...
static bool access_pmu_evcntr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			      const struct sys_reg_desc *r)
{
	u64 idx = ~0UL;
	...
	{ SYS_DESC(SYS_DBGBVRn_EL1(n)),                                        \
	  ...
	  set_bvr },                                                           \
		...
		{ SYS_DESC(SYS_DBGWVRn_EL1(n)),                                \
		  ...
		  set_wvr },                                                   \
	{                                                                      \
		SYS_DESC(SYS_DBGWCRn_EL1(n)), trap_wcr, reset_wcr, 0, 0,       \
			get_wcr, set_wcr                                       \
	}
...
#define PMU_PMEVCNTR_EL0(n)                                                     \
	{                                                                       \
		PMU_SYS_REG(SYS_PMEVCNTRn_EL0(n)), .reset = reset_pmevcntr,     \
		.get_user = get_pmu_evcntr,	   .access = access_pmu_evcntr, \
		.reg = (PMEVCNTR0_EL0 + n),                                     \
	}
...
#define PTRAUTH_KEY(k) __PTRAUTH_KEY(k##KEYLO_EL1), __PTRAUTH_KEY(k##KEYHI_EL1)

static bool access_arch_timer(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			      const struct sys_reg_desc *r)
{
		       ...
		       struct sys_reg_desc const *r)
{
	...
	case SYS_ID_AA64PFR0_EL1:
		...
		if (kvm_vgic_global_state.type == VGIC_V3) {
			...
			val |= FIELD_PREP(
				ARM64_FEATURE_MASK(ID_AA64PFR0_EL1_GIC), 1);
		}
		break;
	...
	case SYS_ID_AA64DFR0_EL1:
		...
		val = cpuid_feature_cap_perfmon_field(
			val, ID_AA64DFR0_EL1_PMUVer_SHIFT,
			kvm_vcpu_has_pmu(vcpu) ? ID_AA64DFR0_EL1_PMUVer_V3P4 :
						 0);
...
static bool access_id_reg(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			  const struct sys_reg_desc *r)
{
	if (p->is_write)
		return write_to_read_only(vcpu, p, r);
...
#define MTE_REG(name)                                                          \
	{                                                                      \
		SYS_DESC(SYS_##name),	      .access = undef_access,          \
		.reset = reset_unknown,	      .reg = name,                     \
...
#define ID_SANITISED(name)                                                     \
	{                                                                      \
		SYS_DESC(SYS_##name),	     .access = access_id_reg,          \
		.get_user = get_id_reg,	     .set_user = set_id_reg,           \
...
static const struct sys_reg_desc sys_reg_descs[] = {
	...
	{ SYS_DESC(SYS_MDCCINT_EL1), trap_debug_regs, reset_val, MDCCINT_EL1,
	  0 },
	...
	{
		SYS_DESC(SYS_OSLSR_EL1),
		trap_oslsr_el1,
		reset_val,
		OSLSR_EL1,
		SYS_OSLSR_OSLM_IMPLEMENTED,
	...
	{ PMU_SYS_REG(SYS_PMINTENSET_EL1), .access = access_pminten,
	  .reg = PMINTENSET_EL1 },
	{ PMU_SYS_REG(SYS_PMINTENCLR_EL1), .access = access_pminten,
	  .reg = PMINTENSET_EL1 },
	...
	{ PMU_SYS_REG(SYS_PMCR_EL0), .access = access_pmcr, .reset = reset_pmcr,
	  .reg = PMCR_EL0 },
	{ PMU_SYS_REG(SYS_PMCNTENSET_EL0), .access = access_pmcnten,
	  .reg = PMCNTENSET_EL0 },
	{ PMU_SYS_REG(SYS_PMCNTENCLR_EL0), .access = access_pmcnten,
	  .reg = PMCNTENSET_EL0 },
	{ PMU_SYS_REG(SYS_PMOVSCLR_EL0), .access = access_pmovs,
	  .reg = PMOVSSET_EL0 },
	/*
	 * PM_SWINC_EL0 is exposed to userspace as RAZ/WI, as it was
	 * previously (and pointlessly) advertised in the past...
	 */
	{ PMU_SYS_REG(SYS_PMSWINC_EL0), .get_user = get_raz_reg,
	  .set_user = set_wi_reg, .access = access_pmswinc, .reset = NULL },
	{ PMU_SYS_REG(SYS_PMSELR_EL0), .access = access_pmselr,
	  .reset = reset_pmselr, .reg = PMSELR_EL0 },
	{ PMU_SYS_REG(SYS_PMCEID0_EL0), .access = access_pmceid,
	  .reset = NULL },
	{ PMU_SYS_REG(SYS_PMCEID1_EL0), .access = access_pmceid,
	  .reset = NULL },
	{ PMU_SYS_REG(SYS_PMCCNTR_EL0), .access = access_pmu_evcntr,
	  .reset = reset_unknown, .reg = PMCCNTR_EL0,
	  .get_user = get_pmu_evcntr },
	{ PMU_SYS_REG(SYS_PMXEVTYPER_EL0), .access = access_pmu_evtyper,
	  .reset = NULL },
	{ PMU_SYS_REG(SYS_PMXEVCNTR_EL0), .access = access_pmu_evcntr,
	  .reset = NULL },
	/*
	...
	{ PMU_SYS_REG(SYS_PMOVSSET_EL0), .access = access_pmovs,
	  .reg = PMOVSSET_EL0 },
...
static bool trap_dbgdidr(struct kvm_vcpu *vcpu, struct sys_reg_params *p,
			 const struct sys_reg_desc *r)
{
	if (p->is_write) {
		return ignore_write(vcpu, p);
		...
		u32 el3 = !!cpuid_feature_extract_unsigned_field(
			pfr, ID_AA64PFR0_EL1_EL3_SHIFT);

		p->regval =
			((((dfr >> ID_AA64DFR0_EL1_WRPs_SHIFT) & 0xf) << 28) |
			 (((dfr >> ID_AA64DFR0_EL1_BRPs_SHIFT) & 0xf) << 24) |
			 (((dfr >> ID_AA64DFR0_EL1_CTX_CMPs_SHIFT) & 0xf)
			  << 20) |
...
#define CP15_PMU_SYS_REG(_map, _Op1, _CRn, _CRm, _Op2)                         \
	AA32(_map), Op1(_Op1), CRn(_CRn), CRm(_CRm), Op2(_Op2),                \
		.visibility = pmu_visibility

/* Macro to expand the PMEVCNTRn register */
#define PMU_PMEVCNTR(n)                                                        \
	{ CP15_PMU_SYS_REG(DIRECT, 0, 0b1110, (0b1000 | (((n) >> 3) & 0x3)),   \
			   ((n) & 0x7)),                                       \
	  .access = access_pmu_evcntr }

/* Macro to expand the PMEVTYPERn register */
#define PMU_PMEVTYPER(n)                                                       \
	{ CP15_PMU_SYS_REG(DIRECT, 0, 0b1110, (0b1100 | (((n) >> 3) & 0x3)),   \
			   ((n) & 0x7)),                                       \
	  .access = access_pmu_evtyper }
/*
...
static const struct sys_reg_desc cp15_regs[] = {
	...
	/* ACTLR */
	{ AA32(LO), Op1(0), CRn(1), CRm(0), Op2(1), access_actlr, NULL,
	  ACTLR_EL1 },
	/* ACTLR2 */
	{ AA32(HI), Op1(0), CRn(1), CRm(0), Op2(3), access_actlr, NULL,
	  ACTLR_EL1 },
	{ Op1(0), CRn(2), CRm(0), Op2(0), access_vm_reg, NULL, TTBR0_EL1 },
	{ Op1(0), CRn(2), CRm(0), Op2(1), access_vm_reg, NULL, TTBR1_EL1 },
	/* TTBCR */
	{ AA32(LO), Op1(0), CRn(2), CRm(0), Op2(2), access_vm_reg, NULL,
	  TCR_EL1 },
	/* TTBCR2 */
	{ AA32(HI), Op1(0), CRn(2), CRm(0), Op2(3), access_vm_reg, NULL,
	  TCR_EL1 },
	...
	{ AA32(LO), Op1(0), CRn(6), CRm(0), Op2(0), access_vm_reg, NULL,
	  FAR_EL1 },
	/* IFAR */
	{ AA32(HI), Op1(0), CRn(6), CRm(0), Op2(2), access_vm_reg, NULL,
	  FAR_EL1 },
	...
	{ AA32(LO), Op1(0), CRn(10), CRm(2), Op2(0), access_vm_reg, NULL,
	  MAIR_EL1 },
	/* NMRR/MAIR1 */
	{ AA32(HI), Op1(0), CRn(10), CRm(2), Op2(1), access_vm_reg, NULL,
	  MAIR_EL1 },
	/* AMAIR0 */
	{ AA32(LO), Op1(0), CRn(10), CRm(3), Op2(0), access_vm_reg, NULL,
	  AMAIR_EL1 },
	/* AMAIR1 */
	{ AA32(HI), Op1(0), CRn(10), CRm(3), Op2(1), access_vm_reg, NULL,
	  AMAIR_EL1 },

	/* ICC_SRE */
	{ Op1(0), CRn(12), CRm(12), Op2(5), access_gic_sre },

	{ Op1(0), CRn(13), CRm(0), Op2(1), access_vm_reg, NULL,
	  CONTEXTIDR_EL1 },
	...
	PMU_PMEVTYPER(30),
	/* PMCCFILTR */
	{ CP15_PMU_SYS_REG(DIRECT, 0, 14, 15, 7),
	  .access = access_pmu_evtyper },
 ...
 */
static bool emulate_cp(struct kvm_vcpu *vcpu, struct sys_reg_params *params,
...
static int kvm_handle_cp_64(struct kvm_vcpu *vcpu,
			    const struct sys_reg_desc *global, size_t nr_global)
{
	struct sys_reg_params params;
...
get_reg_by_id(u64 id, const struct sys_reg_desc table[], unsigned int num)
{
	struct sys_reg_params params;
...
static int demux_c15_get(u64 id, void __user *uaddr)
{
	...
	if (id &
	    ~(KVM_REG_ARCH_MASK | KVM_REG_SIZE_MASK | KVM_REG_ARM_COPROC_MASK |
			    ...
			    const struct kvm_one_reg *reg)
{
	...
	r = id_to_sys_reg_desc(vcpu, reg->id, sys_reg_descs,
...
static int walk_one_sys_reg(const struct kvm_vcpu *vcpu,
			    const struct sys_reg_desc *rd, u64 __user **uind,
			    unsigned int *total)
{
	/*
...
{
	return ARRAY_SIZE(invariant_sys_regs) + num_demux_regs() +
...
int kvm_arm_copy_sys_reg_indices(struct kvm_vcpu *vcpu, u64 __user *uindices)
{
	...
	for (i = 0; i < ARRAY_SIZE(invariant_sys_regs); i++) {
		if (put_user(sys_reg_to_index(&invariant_sys_regs[i]),
			     uindices))
			return -EFAULT;
		uindices++;
	}
...
int kvm_sys_reg_table_init(void)
{
	...
	valid &= check_sysreg_table(cp14_64_regs, ARRAY_SIZE(cp14_64_regs),
				    true);
	valid &= check_sysreg_table(cp15_regs, ARRAY_SIZE(cp15_regs), true);
	valid &= check_sysreg_table(cp15_64_regs, ARRAY_SIZE(cp15_64_regs),
				    true);
	valid &= check_sysreg_table(invariant_sys_regs,
				    ARRAY_SIZE(invariant_sys_regs), false);
