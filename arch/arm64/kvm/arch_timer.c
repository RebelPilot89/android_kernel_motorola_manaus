...
static void kvm_arm_timer_write(struct kvm_vcpu *vcpu,
				...
				enum kvm_arch_timer_regs treg, u64 val);
				     ...
				     u64 val)
{
	...
	if (now < val) {
		...
		ns = cyclecounter_cyc2ns(timecounter->cc, val - now,
...
static u64 kvm_timer_earliest_exp(struct kvm_vcpu *vcpu)
{
	...
	for (i = 0; i < NR_KVM_TIMERS; i++) {
		...
		if (kvm_timer_irq_can_fire(ctx))
			min_delta =
				min(min_delta, kvm_timer_compute_delta(ctx));
	}
		...
		if (irqchip_in_kernel(vcpu->kvm)) {
			...
			if (map.direct_ptimer)
				kvm_vgic_reset_mapped_irq(
					vcpu, map.direct_ptimer->irq.irq);
		}
	}
...
u64 kvm_arm_timer_get_reg(struct kvm_vcpu *vcpu, u64 regid)
{
	switch (regid) {
	case KVM_REG_ARM_TIMER_CTL:
		return kvm_arm_timer_read(vcpu, vcpu_vtimer(vcpu),
					  TIMER_REG_CTL);
	case KVM_REG_ARM_TIMER_CNT:
		return kvm_arm_timer_read(vcpu, vcpu_vtimer(vcpu),
					  TIMER_REG_CNT);
	case KVM_REG_ARM_TIMER_CVAL:
		return kvm_arm_timer_read(vcpu, vcpu_vtimer(vcpu),
					  TIMER_REG_CVAL);
	case KVM_REG_ARM_PTIMER_CTL:
		return kvm_arm_timer_read(vcpu, vcpu_ptimer(vcpu),
					  TIMER_REG_CTL);
			      ...
			      enum kvm_arch_timer_regs treg)
{
	...
	switch (treg) {
	case TIMER_REG_TVAL:
		val = timer_get_cval(timer) - kvm_phys_timer_read() +
		      timer_get_offset(timer);
...
u64 kvm_arm_timer_read_sysreg(struct kvm_vcpu *vcpu, enum kvm_arch_timers tmr,
			      enum kvm_arch_timer_regs treg)
{
	u64 val;
				...
				enum kvm_arch_timer_regs treg, u64 val)
{
	switch (treg) {
	case TIMER_REG_TVAL:
		timer_set_cval(timer, kvm_phys_timer_read() -
					      timer_get_offset(timer) +
					      (s32)val);
		break;
...
void kvm_arm_timer_write_sysreg(struct kvm_vcpu *vcpu, enum kvm_arch_timers tmr,
				enum kvm_arch_timer_regs treg, u64 val)
{
	preempt_disable();
	kvm_timer_vcpu_put(vcpu);
				  ...
				  unsigned int nr_irqs, void *arg)
{
	...
	return irq_domain_set_hwirq_and_chip(domain, virq, hwirq, &timer_chip,
					     NULL);
}
