...
static void pvm_init_traps_aa64dfr0(struct kvm_vcpu *vcpu)
{
	...
	/* Trap/constrain PMU */
	if (!FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_PMUVer),
		       feature_ids)) {
	...
	/* Trap Debug */
	if (!FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_DebugVer),
		       feature_ids))
		mdcr_set |= MDCR_EL2_TDRA | MDCR_EL2_TDA;

	/* Trap OS Double Lock */
	if (!FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_DoubleLock),
		       feature_ids))
		mdcr_set |= MDCR_EL2_TDOSA;

	/* Trap SPE */
	if (!FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_PMSVer),
		       feature_ids)) {
	...
	/* Trap Trace Filter */
	if (!FIELD_GET(ARM64_FEATURE_MASK(ID_AA64DFR0_EL1_TraceFilt),
		       feature_ids))
...
static void pkvm_vcpu_init_features_from_host(struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	if (!pkvm_hyp_vcpu_is_protected(hyp_vcpu)) {
		bitmap_copy(hyp_vcpu->vcpu.arch.features,
			    host_vcpu->arch.features, KVM_VCPU_MAX_FEATURES);
		return;
	}
	...
	if (FIELD_GET(ARM64_FEATURE_MASK(ID_AA64ISAR1_EL1_API),
		      PVM_ID_AA64ISAR1_ALLOW) &&
	    FIELD_GET(ARM64_FEATURE_MASK(ID_AA64ISAR1_EL1_APA),
		      PVM_ID_AA64ISAR1_ALLOW))
		set_bit(KVM_ARM_VCPU_PTRAUTH_ADDRESS, allowed_features);

	if (FIELD_GET(ARM64_FEATURE_MASK(ID_AA64ISAR1_EL1_GPI),
		      PVM_ID_AA64ISAR1_ALLOW) &&
			     ...
			     int *last_ran, unsigned int nr_vcpus)
{
	...
	if (hyp_vm->kvm.arch.pkvm.enabled)
		pvmfw_load_addr =
			READ_ONCE(host_kvm->arch.pkvm.pvmfw_load_addr);
...
static int init_pkvm_hyp_vcpu(struct pkvm_hyp_vcpu *hyp_vcpu,
			      struct pkvm_hyp_vm *hyp_vm,
			      struct kvm_vcpu *host_vcpu, unsigned int vcpu_idx)
{
	int ret = 0;
	...
	if (test_bit(KVM_ARM_VCPU_SVE, hyp_vcpu->vcpu.arch.features)) {
		...
		hyp_vcpu->vcpu.arch.sve_state =
			READ_ONCE(host_vcpu->arch.sve_state);
		hyp_vcpu->vcpu.arch.sve_max_vl =
...
void pkvm_reset_vcpu(struct pkvm_hyp_vcpu *hyp_vcpu)
{
	...
	WRITE_ONCE(hyp_vcpu->vcpu.arch.mp_state.mp_state,
...
static bool pvm_psci_vcpu_on(struct pkvm_hyp_vcpu *hyp_vcpu)
{
	 ...
	 */
	power_state = cmpxchg(&target->power_state, PSCI_0_2_AFFINITY_LEVEL_OFF,
...
{
	unsigned long target_affinity_mask, target_affinity,
