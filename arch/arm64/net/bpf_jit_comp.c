 ...
 *
 * Merge: donor improvements: FP_BOTTOM, fpb_offset, atomic ops, LSE support.
 */
...
/* Macros de validación para saltos ARM64 */
#define check_imm(bits, imm)                                                   \
	do {                                                                   \
		if ((((imm) > 0) && ((imm) >> (bits))) ||                      \
		    (((imm) < 0) && (~(imm) >> (bits)))) {                     \
			pr_info("[%2d] imm %d out of range\n", i, imm);        \
			return -EINVAL;                                        \
		}                                                              \
	} while (0)
...
static const int bpf2a64[] = {
	...
	[BPF_REG_AX] = A64_R(9),
	/* bottom of frame pointer (for stack accesses) */
	[FP_BOTTOM] = A64_R(27),
};
...
static inline void emit_a64_mov_i(const int is64, const int reg, const s32 val,
				  struct jit_ctx *ctx)
{
	u16 hi = val >> 16;
	u16 lo = val & 0xffff;
				    ...
				    struct jit_ctx *ctx)
{
	...
	inverse =
		i64_i16_blocks(nrm_tmp, true) < i64_i16_blocks(nrm_tmp, false);
	...
	shift -= 16;
	while (shift >= 0) {
		if (((nrm_tmp >> shift) & 0xffff) !=
		    (inverse ? 0xffff : 0x0000))
			emit(A64_MOVK(1, reg, (nrm_tmp >> shift) & 0xffff,
				      shift),
			     ctx);
		shift -= 16;
	}
...
/* Return the minimum negative offset from FP, converted to positive and aligned down to 8 bytes */
	...
	for (i = 0; i < prog->len; i++) {
		...
		case BPF_ST:
			...
			if (class == BPF_STX && mode == BPF_ATOMIC &&
			    ((imm == BPF_XCHG || imm == (BPF_FETCH | BPF_ADD) ||
...
/* Prólogo optimizado con soporte FP_BOTTOM y robustecimiento Motorola */
static int build_prologue(struct jit_ctx *ctx, bool ebpf_from_cbpf)
{
	...
	/* Alineación de Stack: 16 bytes para cumplir con la ABI de ARM64 */
	stack_size += 32; /* Espacio extra para registros callee-saved y TCC */

	/* 1. Guardar Frame Pointer y Link Register (Pila de llamada) */
	emit(A64_PUSH(A64_FP, A64_LR, A64_SP), ctx);
	emit(A64_MOV(1, A64_FP, A64_SP), ctx);

	/* 2. Reservar espacio en stack para el programa BPF */
	emit(A64_SUB_I(1, A64_SP, A64_SP, stack_size), ctx);

	/* 3. Setup de BPF Frame Pointer (REG_FP) */
	emit(A64_MOV(1, fp, A64_SP), ctx);

	/* 4. Robustecimiento: Inicializar Tail Call Counter (prevención de bucles infinitos) */
	emit(A64_STR64I(tcc, A64_SP, 16), ctx);
	emit(A64_MOV(1, tcc, A64_ZR), ctx);

	/* 5. Integración Donor: Establecer FP_BOTTOM para unwinding de Android */
	emit(A64_ADD_I(1, fpb, A64_SP, stack_size), ctx);

	/* Si el kernel Motorola usa BPF Cookies para tracing (Android 13+), 
	 * el registro R0 debe ser preservado o inicializado aquí.
	 * (El parámetro ebpf_from_cbpf se puede utilizar para detectar el modo)
	 */
	if (ebpf_from_cbpf) {
		/* Lógica específica para cookies si el donor la requiere */
	}
...
#define jmp_offset (out_offset - (cur_offset))
	...
	emit(A64_ADD_I(1, tmp, tmp,
		       sizeof(u32) *
			       (IS_ENABLED(CONFIG_ARM64_BTI_KERNEL) ? 8 : 7)),
	     ctx);
	...
	if (cur_offset != out_offset) {
		pr_err_once("tail_call out_offset mismatch\n");
		return -1;
	}
	return 0;
#undef cur_offset
...
static int emit_ll_sc_atomic(const struct bpf_insn *insn, struct jit_ctx *ctx)
{
	...
	if (imm == BPF_ADD || imm == BPF_AND || imm == BPF_OR ||
	    imm == BPF_XOR) {
		/* lock *(u32/u64 *)(dst_reg + off) <op>= src_reg */
		      ...
		      bool extra_pass)
{
	...
	const int i = insn - ctx->prog->insnsi;
	const bool is64 =
		BPF_CLASS(code) == BPF_ALU64 || BPF_CLASS(code) == BPF_JMP;
	u8 jmp_cond;
	s32 jmp_offset;
	u32 a64_insn;
	u8 src_adj;
...
struct bpf_prog *bpf_int_jit_compile(struct bpf_prog *prog)
{
	...
	prog_size = sizeof(u32) * ctx.idx;
	extable_offset = round_up(prog_size, extable_align);
	image_size = extable_offset + extable_size;
	header = bpf_jit_binary_alloc(image_size, &image_ptr, sizeof(u32),
				      jit_fill_hole);
	if (header == NULL) {
		prog = orig_prog;
		goto out_off;
...
skip_init_ctx:
	...
	if (!prog->is_func || extra_pass) {
		...
		trace_android_vh_set_memory_ro((unsigned long)header,
					       header->pages);
		trace_android_vh_set_memory_x((unsigned long)header,
					      header->pages);
	} else {
		jit_data->ctx = ctx;
