...
u64 kimage_vaddr __ro_after_init = KIMAGE_VADDR;
			       ...
			       pgprot_t prot, bool page_mappings_only)
{
	...
	__create_pgd_mapping(mm->pgd, phys, virt, size, prot, pgd_pgtable_alloc,
			     flags);
}
...
void mark_rodata_ro(void)
{
	 ...
	 */
	section_size =
		(unsigned long)__init_begin - (unsigned long)__start_rodata;
	update_mapping_prot(__pa_symbol(__start_rodata),
			    (unsigned long)__start_rodata, section_size,
			    PAGE_KERNEL_RO);
...
static int __init map_entry_trampoline(void)
{
	...
	__create_pgd_mapping(tramp_pg_dir, pa_start, TRAMP_VALIAS,
			     entry_tramp_text_size(), prot, __pgd_pgtable_alloc,
			     NO_BLOCK_MAPPINGS);
...
static bool arm64_early_this_cpu_has_bti(void)
{
	...
	return cpuid_feature_extract_unsigned_field(pfr1, ID_AA64PFR1_BT_SHIFT);
}
				    ...
				    struct vmem_altmap *altmap)
{
	...
	do {
		...
		if (pmd_sect(pmd)) {
			...
			flush_tlb_kernel_range(addr, addr + PAGE_SIZE);
			if (free_mapped)
				free_hotplug_page_range(pmd_page(pmd), PMD_SIZE,
							altmap);
			continue;
		}
				    ...
				    struct vmem_altmap *altmap)
{
	...
	do {
		...
		if (pud_sect(pud)) {
			...
			flush_tlb_kernel_range(addr, addr + PAGE_SIZE);
			if (free_mapped)
				free_hotplug_page_range(pud_page(pud), PUD_SIZE,
							altmap);
			continue;
		}
	...
	*size = fdt_totalsize(dt_virt);
	...
	if (offset + *size > SWAPPER_BLOCK_SIZE)
		create_mapping_noalloc(
			round_down(dt_phys, SWAPPER_BLOCK_SIZE), dt_virt_base,
...
int arch_add_memory(int nid, u64 start, u64 size, struct mhp_params *params)
{
	...
	__create_pgd_mapping(swapper_pg_dir, start, __phys_to_virt(start), size,
			     params->pgprot, __pgd_pgtable_alloc, flags);

	memblock_clear_nomap(start, size);

	ret = __add_pages(nid, start >> PAGE_SHIFT, size >> PAGE_SHIFT, params);
	if (ret)
		__remove_pgd_mapping(swapper_pg_dir, __phys_to_virt(start),
				     size);
	else {
				  ...
				  const char *resource_name)
{
	...
	__create_pgd_mapping(init_mm.pgd, start, virt, size, PAGE_KERNEL, NULL,
			     flags);
